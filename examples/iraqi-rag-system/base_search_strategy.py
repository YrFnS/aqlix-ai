...
ARABIC_SIMILARITY_THRESHOLD = (
    0.12  # Lower threshold for Arabic due to morphological complexity
)
...
class IraqiSearchContext:
    ...
    professional_domain: str = (
        "general"  # "legal", "medical", "educational", "government"
    )
...
class IraqiBaseSearchStrategy:
    ...
    async def vector_search(
        ...
        try:
            ...
            similarity_threshold = (
                ARABIC_SIMILARITY_THRESHOLD
                if iraqi_context.language in ["arabic", "mixed"]
                else ENGLISH_SIMILARITY_THRESHOLD
            )

            logger.info(
                f"Iraqi search started - Language: {iraqi_context.language}, "
            ...
            self.search_metrics["cultural_filtered"] += len(response.data) - len(
                validated_results
            )

            logger.info(
                f"Iraqi search completed - {len(final_results)} culturally-validated results, "
        ...
        for result in raw_results:
            try:
                ...
                result["iraqi_metadata"] = {
                    ...
                    "regional_relevance": await self._calculate_regional_relevance(
                        content, context
                    ),
                }
    ...
    async def _apply_cultural_intelligence(
        self, results: List[Dict[str, Any]], context: IraqiSearchContext
    ) -> List[Dict[str, Any]]:
        ...
        for result in results:
            try:
                ...
                cultural_score = await self.cultural_validator.validate_content(
                    content, context.professional_domain, context.islamic_compliance
                )
                ...
                if cultural_score.overall_score >= context.cultural_sensitivity:
                    ...
                    if context.professional_domain != "general":
                        professional_enhancement = (
                            await self._enhance_professional_context(
                    ...
                    logger.debug(
                        f"Content filtered for cultural compliance: {cultural_score.overall_score:.2f} < {context.cultural_sensitivity}"
                    )
    ...
    async def _calculate_regional_relevance(
        self, content: str, context: IraqiSearchContext
    ) -> float:
        ...
        relevance_score = min(
            1.0, matches / max_possible_matches * 2
        )  # Boost factor of 2
    ...
    async def _enhance_professional_context(
        ...
        domain_enhancements = {
            "legal": {
                "relevant_laws": await self._extract_legal_references(
                    result.get("content", "")
                ),
                "legal_terminology": await self._identify_legal_terms(
                    result.get("content", "")
                ),
            ...
            "medical": {
                "medical_terminology": await self._identify_medical_terms(
                    result.get("content", "")
                ),
            ...
            "educational": {
                "curriculum_alignment": await self._check_curriculum_alignment(
                    result.get("content", "")
                ),
                "educational_level": await self._determine_education_level(
                    result.get("content", "")
                ),
            ...
            "government": {
                "government_level": await self._identify_government_level(
                    result.get("content", "")
                ),
                "ministry_alignment": await self._identify_relevant_ministry(
                    result.get("content", "")
                ),
                "administrative_process": True,
                "citizen_services": await self._identify_citizen_services(
                    result.get("content", "")
                ),
            },
        }
    ...
    def get_search_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cultural_compliance_rate": (
                1
                - (
                    self.search_metrics["cultural_filtered"]
                    / max(1, self.search_metrics["total_searches"])
                )
            ),
            "arabic_processing_rate": (
                self.search_metrics["arabic_processed"]
                / max(1, self.search_metrics["total_searches"])
            ),
            "dialect_recognition_rate": (
                self.search_metrics["dialect_recognized"]
                / max(1, self.search_metrics["arabic_processed"])
            ),
        }
