...
class IraqiKeywordExtractor:
    ...
    async def extract_iraqi_keywords(
        ...
        try:
            logger.info(
                f"Extracting Iraqi keywords - Length: {len(text)}, "
            ...
            if await self._contains_arabic(text):
                arabic_extracted = await self._extract_arabic_keywords(
                    text, max_keywords // 2
                )
            ...
            if await self._contains_english(text):
                english_extracted = await self._extract_english_keywords(
                    text, max_keywords // 2
                )
            ...
            for keyword in combined_keywords:
                confidence = await self._calculate_keyword_confidence(
                    keyword, text, iraqi_context
                )
                keyword_confidence[keyword] = confidence

            # 9. Calculate term frequencies
            term_frequencies = await self._calculate_term_frequencies(
                text, combined_keywords
            )
            ...
            if include_variations:
                expanded_keywords = await self._expand_keyword_variations(
                    combined_keywords
                )
            ...
            logger.info(
                f"Iraqi keyword extraction completed - {len(final_keywords)} keywords, "
    ...
    async def _extract_english_keywords(
    ...
    async def _calculate_keyword_confidence(
        self, keyword: str, text: str, iraqi_context: Optional[Any]
    ) -> float:
        ...
        # Cultural term boost
        if keyword in [
            term for terms in self.cultural_keywords.values() for term in terms
        ]:
            confidence += 0.2
        ...
        if iraqi_context and hasattr(iraqi_context, "professional_domain"):
            ...
            if (
                domain in self.professional_keywords
                and keyword in self.professional_keywords[domain]
            ):
                confidence += 0.2

        # Dialect marker boost
        if keyword in [
            term for terms in self.iraqi_dialect_terms.values() for term in terms
        ]:
            confidence += 0.15

        return min(1.0, confidence)

    async def _calculate_term_frequencies(
    ...
    async def _calculate_contextual_relevance(
        self, keywords: List[str], text: str, iraqi_context: Optional[Any]
    ) -> Dict[str, float]:
            ...
            for pos in keyword_positions:
                ...
                if iraqi_context and hasattr(iraqi_context, "professional_domain"):
                    ...
                    if domain in self.professional_keywords:
                        related_terms = sum(
                            1
                            for term in self.professional_keywords[domain]
    ...
    def get_extraction_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "arabic_processing_rate": self.extraction_stats["arabic_extractions"]
            / total,
            "dialect_detection_rate": self.extraction_stats["dialect_detections"]
            / total,
            "cultural_term_detection_rate": self.extraction_stats[
                "cultural_term_detections"
            ]
            / total,
            "professional_term_detection_rate": self.extraction_stats[
                "professional_term_detections"
            ]
            / total,
        }
