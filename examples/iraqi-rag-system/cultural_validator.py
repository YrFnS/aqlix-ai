...
class IraqiCulturalValidator:
    ...
    def _load_islamic_guidelines(self) -> Dict[str, Any]:
        ...
        return {
            "prohibited_content": [
                # Financial/Business
                "interest", "riba", "ربا", "gambling", "قمار", "lottery", "يانصيب",
                "alcohol", "خمر", "pork", "خنزير", "casino", "كازينو",
                
                # Social/Moral
                "inappropriate_relationships", "adultery", "زنا", 
                "immodest_content", "محتوى غير محتشم",
                
                # Religious
                "blasphemy", "تجديف", "religious_mockery", "استهزاء_ديني"
            ],
            
            "encouraged_values": [
                "charity", "زكاة", "community", "مجتمع", "family", "عائلة",
                "education", "تعليم", "honesty", "صدق", "justice", "عدالة",
                "respect", "احترام", "cooperation", "تعاون"
            ],
            
            "business_ethics": [
                "halal_business", "حلال", "fair_trade", "تجارة_عادلة",
                "transparency", "شفافية", "mutual_benefit", "منفعة_متبادلة"
            ]
        }
    ...
    def _load_iraqi_customs(self) -> Dict[str, Any]:
        ...
        return {
            "family_values": [
                "respect_for_elders", "احترام_الكبار", "family_honor", "شرف_العائلة",
                "hospitality", "ضيافة", "traditional_roles", "أدوار_تقليدية"
            ],
            
            "social_etiquette": [
                "formal_address", "خطاب_رسمي", "proper_greetings", "تحيات_مناسبة",
                "professional_titles", "ألقاب_مهنية", "gender_appropriate", "مناسب_للجنس"
            ],
            
            "regional_sensitivity": [
                "avoid_sectarian", "تجنب_الطائفية", "avoid_tribal", "تجنب_القبلية",
                "political_neutrality", "حياد_سياسي", "inclusive_language", "لغة_شاملة"
            ]
        }
    ...
    def _load_professional_standards(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "legal": {
                ...
                "cultural_considerations": ["islamic_law_compatibility", "iraqi_legal_system"],
                "prohibited": ["unlawful_advice", "نصائح_غير_قانونية"]
            },
            
            "medical": {
                "terminology": ["patient", "مريض", "treatment", "علاج", "diagnosis", "تشخيص"],
                "formality_level": "high", 
                "cultural_considerations": ["gender_sensitivity", "islamic_medical_ethics"],
                "prohibited": ["unprofessional_advice", "نصائح_غير_مهنية"]
            },
            
            "educational": {
                "terminology": ["student", "طالب", "curriculum", "منهج", "education", "تعليم"],
            ...
            "government": {
                "terminology": ["citizen", "مواطن", "service", "خدمة", "ministry", "وزارة"],
                "formality_level": "high",
    ...
    def _load_sensitive_topics(self) -> Dict[str, List[str]]:
        ...
        return {
            "political": [
                "sectarian_references", "مراجع_طائفية", "political_parties", "أحزاب_سياسية",
                "government_criticism", "انتقاد_الحكومة", "electoral_bias", "تحيز_انتخابي"
            ],
            
            "social": [
                "tribal_disputes", "نزاعات_عشائرية", "ethnic_tensions", "توترات_عرقية",
                "religious_disputes", "نزاعات_دينية", "gender_controversies", "جدل_جنسي"
            ],
            
            "economic": [
                "corruption_allegations", "ادعاءات_فساد", "economic_bias", "تحيز_اقتصادي",
                "resource_disputes", "نزاعات_الموارد"
            ]
        }
    ...
    async def validate_content(
        ...
        """
        try:
            logger.info(f"Starting cultural validation - Domain: {domain}, "
            ...
            if islamic_compliance_required:
                islamic_score, islamic_issues = await self._validate_islamic_compliance(content)
            ...
            prof_score, prof_issues = await self._validate_professional_appropriateness(content, domain)
            ...
            lang_score, lang_issues = await self._validate_language_appropriateness(content, domain)
            ...
            regional_score, regional_issues = await self._validate_regional_sensitivity(content)
            ...
            recommendations.extend(self._generate_recommendations(validation_scores, domain))
            
            # 8. Filter Content if Needed
            filtered_content = await self._filter_problematic_content(content, issues_found)
            ...
            result = CulturalValidationResult(
                ...
                professional_appropriateness=validation_scores["professional_appropriateness"],
            ...
            logger.info(f"Cultural validation completed - Overall score: {overall_score:.2f}, "
    ...
    async def _validate_islamic_compliance(self, content: str) -> Tuple[float, List[str]]:
    ...
    async def _validate_professional_appropriateness(self, content: str, domain: str) -> Tuple[float, List[str]]:
        ...
        if domain in self.professional_standards:
            ...
            formality_score = await self._assess_formality_level(content, formality_required)
    ...
    async def _validate_language_appropriateness(self, content: str, domain: str) -> Tuple[float, List[str]]:
        ...
        if domain in ["legal", "government", "medical"]:
            formal_indicators = self.language_standards["formal_arabic"]["formal_phrases"]
            has_formal_language = any(phrase in content for phrase in formal_indicators)
            
            if not has_formal_language and len(content) > 100:  # Only for longer content
                issues.append("Consider using more formal language for professional context")
                score -= 0.1
        
        # Check for inappropriate informal language in formal contexts
        if domain in ["legal", "government"] and any(slang in content.lower() for slang in ["lol", "omg", "wtf"]):
            issues.append("Informal language inappropriate for professional context")
            score -= 0.3
        
        return max(0.0, score), issues

    async def _validate_regional_sensitivity(self, content: str) -> Tuple[float, List[str]]:
    ...
    async def _assess_formality_level(self, content: str, required_level: str) -> float:
        ...
        formal_indicators = [
            "please", "thank you", "respectfully", "sincerely",
            "يرجى", "شكرا", "باحترام", "مع التقدير"
        ]
        
        informal_indicators = [
            "hey", "hi", "cool", "awesome", "مرحبا", "زين", "ممتاز"
        ]
        ...
        return 1.0 if formality_ratio >= required_threshold else formality_ratio / required_threshold
    ...
    def _generate_recommendations(self, validation_scores: Dict[str, float], domain: str) -> List[str]:
        ...
        if validation_scores["professional_appropriateness"] < 0.8:
            recommendations.append(f"Enhance content for {domain} professional standards")
    ...
    def get_validation_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "islamic_compliance_rate": 1 - (self.validation_stats["islamic_violations"] / total),
            "professional_compliance_rate": 1 - (self.validation_stats["professional_violations"] / total),
            "language_compliance_rate": 1 - (self.validation_stats["language_violations"] / total),
