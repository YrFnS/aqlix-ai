...
class ArabicTextProcessor:
    ...
    async def process_content(
        ...
        try:
            logger.info(
                f"Processing Arabic content - Length: {len(text)}, "
            ...
            if script_type in [
                ArabicScript.ARABIC_ONLY,
                ArabicScript.MIXED_ARABIC_ENGLISH,
            ]:
                (
                    dialect_type,
                    dialect_confidence,
                    iraqi_markers,
                ) = await self._detect_iraqi_dialect(text)
            ...
            (
                ...
                ltr_segments,
                mixed_direction,
            ) = await self._analyze_text_direction(text)
            ...
            processing_metadata = {
                ...
                "script_confidence": await self._calculate_script_confidence(
                    text, script_type
                ),
            }
            ...
            logger.info(
                f"Arabic processing completed - Script: {script_type.value}, "
    ...
    async def _count_words_by_language(self, text: str) -> Dict[str, int]:
        ...
        return {"total": len(words), "arabic": arabic_words, "english": english_words}

    async def _detect_iraqi_dialect(
        self, text: str
    ) -> Tuple[IraqiDialectType, float, List[str]]:
    ...
    async def _analyze_text_direction(
        self, text: str
    ...
    async def _detect_colloquial_indicators(self, text: str) -> List[str]:
        ...
        for dialect_markers in self.iraqi_dialect_markers.values():
            for marker in dialect_markers:
                if marker in text and marker not in self.iraqi_dialect_markers.get(
                    "formal_markers", []
                ):
    ...
    async def _calculate_script_confidence(
        self, text: str, script_type: ArabicScript
    ) -> float:
    ...
    def get_processing_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "iraqi_dialect_recognition_rate": self.processing_stats[
                "iraqi_dialect_detected"
            ]
            / max(1, self.processing_stats["arabic_detected"]),
            "mixed_content_rate": self.processing_stats["mixed_content_processed"]
            / total,
            "normalization_rate": self.processing_stats["normalized_texts"] / total,
        }

    async def extract_keywords_arabic(
