...
class IraqiRerankingStrategy:
    ...
    def _load_base_model(self) -> Optional[CrossEncoder]:
        ...
        if not CROSSENCODER_AVAILABLE:
            logger.warning(
                "sentence-transformers not available - base reranking disabled"
            )
            return None
        ...
        try:
            ...
            logger.error(
                f"Failed to load reranking model {self.config.base_model_name}: {e}"
            )
            return None
    ...
    async def rerank_results(
        ...
        try:
            ...
            logger.info(
                f"Starting Iraqi reranking - {len(results)} results, "
            ...
            professionally_enhanced_results = (
                await self._apply_professional_domain_scoring(
            ...
            result = IraqiRerankingResult(
                ...
                professional_enhancement_applied=iraqi_context is not None
                and hasattr(iraqi_context, "professional_domain")
                and iraqi_context.professional_domain != "general",
                performance_metrics=performance_metrics,
            )

            logger.info(
                f"Iraqi reranking completed - {len(filtered_results)} final results, "
    ...
    async def _apply_base_reranking(
        self, query: str, results: List[Dict[str, Any]], content_key: str
    ) -> List[Dict[str, Any]]:
        ...
        if not self.base_model:
            ...
            for result in results:
                result["iraqi_processing"]["base_rerank_score"] = result.get(
                    "similarity", 0.0
                )
            return results
        ...
        try:
            ...
            for i, valid_idx in enumerate(valid_indices):
                results[valid_idx]["iraqi_processing"]["base_rerank_score"] = float(
                    base_scores[i]
                )
    ...
    async def _apply_cultural_intelligence_scoring(
        self, results: List[Dict[str, Any]], iraqi_context: Optional[Any]
    ) -> List[Dict[str, Any]]:
        """Apply Iraqi cultural intelligence scoring"""

        if not self.config.enable_cultural_scoring:
            for result in results:
                result["iraqi_processing"]["cultural_score"] = (
                    0.8  # Default neutral score
                )
            return results
        ...
        cultural_count = sum(
            1
            for r in culturally_scored_results
    ...
    async def _apply_arabic_processing_enhancement(
        ...
        if not self.config.enable_arabic_processing:
            for result in results:
                result["iraqi_processing"]["arabic_score"] = (
            ...
            try:
                ...
                if await self.arabic_processor.contains_arabic(content):
                    ...
                    result["arabic_processing_metadata"] = {
                        ...
                        "professional_terms_count": len(
                            arabic_processing.professional_terms
                        ),
    ...
    async def _apply_professional_domain_scoring(
        ...
        if not iraqi_context or not hasattr(iraqi_context, "professional_domain"):
            for result in results:
                result["iraqi_processing"]["professional_score"] = (
    ...
    async def _apply_iraqi_filtering(
        ...
        for result in results:
            ...
            if cultural_score < self.config.cultural_compliance_threshold:
                logger.debug(
                    f"Result filtered for cultural compliance: {cultural_score:.2f}"
                )
                continue
            ...
            arabic_score = iraqi_processing.get("arabic_score", 0.7)
            if (
                await self.arabic_processor.contains_arabic(result.get("content", ""))
                and arabic_score < self.config.arabic_quality_threshold
            ):
            ...
            if iraqi_context and hasattr(iraqi_context, "professional_domain"):
                ...
                if domain != "general":
                    ...
                    if (
                        professional_score
                        < self.config.professional_relevance_threshold
                    ):
                        logger.debug(
                            f"Result filtered for professional relevance: {professional_score:.2f}"
                        )
                        continue
    ...
    async def _calculate_arabic_quality_score(
        ...
        cultural_terms_ratio = len(arabic_processing.cultural_terms) / max(
            1, arabic_processing.word_count
        )
        score += min(0.1, cultural_terms_ratio * 10)

        # Professional terms boost
        professional_terms_ratio = len(arabic_processing.professional_terms) / max(
            1, arabic_processing.word_count
        )
        ...
        formal_ratio = len(arabic_processing.formal_indicators) / max(
            1, arabic_processing.word_count
        )
    ...
    async def _calculate_professional_relevance_score(
        ...
        if domain in domain_keywords:
            keyword_matches = sum(
                1 for keyword in domain_keywords[domain] if keyword in content
            )
    ...
    def _calculate_performance_metrics(
        ...
        return {
            ...
            "filtering_rate": (len(original_results) - len(final_results))
            / max(1, len(original_results)),
            ...
            "average_final_score": sum(
                r.get("iraqi_rerank_score", 0) for r in final_results
            )
            / max(1, len(final_results)),
        }
    ...
    def _create_error_result(
        self, results: List[Dict[str, Any]], error: str
    ) -> IraqiRerankingResult:
