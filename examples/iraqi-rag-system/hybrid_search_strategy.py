...
class IraqiHybridSearchStrategy:
    ...
    async def search_documents_hybrid(
        ...
        try:
            ...
            logger.info(f"Iraqi hybrid search started - Query: '{query[:50]}...', "
            ...
            # Update metrics
            self._update_search_metrics(vector_results, keyword_results, enhanced_results, config)
            
            logger.info(f"Iraqi hybrid search completed - {len(enhanced_results)} culturally-enhanced results")
    ...
    async def search_code_examples_hybrid(
        ...
        try:
            ...
            logger.info(f"Iraqi code search started - Query: '{query[:50]}...', Source: {source_id}")
            
            # Create query embedding
            from .embedding_service import create_embedding  # Would import from actual service
            ...
            keyword_results = await self._perform_iraqi_keyword_search(
                ...
                iraqi_context=iraqi_context,
                config=config,
                search_fields=["content", "summary", "comments"]  # Code-specific fields
            )
            ...
            logger.info(f"Iraqi code search completed - {len(enhanced_results)} enhanced code examples")
        ...
        try:
            ...
            if await self.arabic_processor.contains_arabic(query):
                arabic_processing = await self.arabic_processor.process_content(
                    text=query,
                    dialect_preference=iraqi_context.dialect if iraqi_context else "iraqi"
                )
                arabic_keywords = await self.arabic_processor.extract_keywords_arabic(
                    text=arabic_processing.normalized_text,
                    max_keywords=8
                )
    ...
    async def _execute_iraqi_keyword_search(
        ...
        for keyword in keywords[:8]:  # Limit to avoid too many queries
            try:
                ...
                if len(search_fields) > 1:
                    ...
                    query_builder = query_builder.ilike(search_fields[0], search_pattern)
                ...
                if iraqi_context and iraqi_context.cultural_sensitivity > 0:
                    query_builder = query_builder.gte(
                        ...
                        iraqi_context.cultural_sensitivity
                    )
        ...
        all_results.sort(key=lambda x: x.get("keyword_relevance_score", 0), reverse=True)
    ...
    async def _merge_iraqi_search_results(
        ...
        for keyword_result in keyword_results:
            ...
            if result_id and result_id in vector_lookup and result_id not in seen_ids:
                ...
                cultural_score = vector_result.get("cultural_validation", {}).get("overall_score", 0.5)
        ...
        for keyword_result in keyword_results:
            ...
            if result_id and result_id not in seen_ids and len(combined_results) < match_count:
                ...
                if cultural_score >= iraqi_context.cultural_sensitivity:
                    ...
                    standardized_result = self._standardize_keyword_result(keyword_result, config)
    ...
    def _standardize_keyword_result(
        ...
        cultural_score = keyword_result.get("cultural_validation", {}).get("overall_score", 0.5)
        ...
        for result in results:
            try:
                ...
                if config.enable_cultural_filtering:
                    cultural_enhancement = await self._apply_cultural_enhancement(result, iraqi_context)
                    result.update(cultural_enhancement)
                
                # Apply professional enhancement
                if config.enable_professional_enhancement and iraqi_context.professional_domain != "general":
                    professional_enhancement = await self._apply_professional_enhancement(
                ...
                if config.enable_arabic_keywords:
                    arabic_enhancement = await self._apply_arabic_enhancement(result, query, config)
                    result.update(arabic_enhancement)
                
                enhanced_results.append(result)
                
            except Exception as e:
                logger.warning(f"Enhancement failed for result {result.get('id', 'unknown')}: {e}")
    ...
    async def _apply_cultural_enhancement(
        ...
        if "cultural_validation" in result:
            ...
            enhancement["iraqi_cultural_context"]["validation_details"] = {
                ...
                "professional_appropriateness": cultural_data.get("professional_appropriateness", 0)
            }
    ...
    async def _apply_professional_enhancement(
        ...
        enhancement = {
            "iraqi_professional_context": {
                ...
                "domain_specific_metadata": await self._extract_domain_metadata(result, domain)
            }
        }
    ...
    async def _apply_arabic_enhancement(
        ...
        if await self.arabic_processor.contains_arabic(content):
            ...
            if arabic_processing.dialect_confidence > 0.7:
                ...
                result["similarity"] = min(1.0, current_similarity * config.dialect_recognition_weight)
    ...
    async def _extract_domain_metadata(self, result: Dict[str, Any], domain: str) -> Dict[str, Any]:
        ...
        if domain == "legal":
            domain_metadata["legal_terms_detected"] = "قانون" in content or "law" in content.lower()
        elif domain == "medical":
            domain_metadata["medical_terms_detected"] = "طبيب" in content or "doctor" in content.lower()
        elif domain == "educational":
            domain_metadata["educational_terms_detected"] = "مدرسة" in content or "school" in content.lower()
    ...
    def _update_search_metrics(
        ...
        professional_enhanced = sum(
            1 for r in enhanced_results
            if "iraqi_professional_context" in r
        )
    ...
    def get_search_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "arabic_keyword_usage_rate": self.search_metrics["arabic_keyword_searches"] / total,
            "cultural_filtering_rate": self.search_metrics["cultural_filtered_results"] / total,
            "dialect_enhancement_rate": self.search_metrics["dialect_enhanced_results"] / total,
            "professional_enhancement_rate": self.search_metrics["professional_enhanced_results"] / total,
