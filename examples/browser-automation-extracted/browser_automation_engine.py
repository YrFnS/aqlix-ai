...
class IraqiBrowserAutomationEngine:
    ...
    async def initialize_browser(self):
        ...
        try:
            ...
            self.logger.info(
                f"Browser initialized with engine: {self.config.browser_engine}"
            )
    ...
    async def _initialize_playwright(self):
        ...
        self.context = await self.browser.new_context(
            viewport={
                "width": self.config.viewport_size[0],
                "height": self.config.viewport_size[1],
            },
    ...
    async def _initialize_selenium(self):
        ...
        if self.config.browser_engine == BrowserEngine.SELENIUM_CHROME:
            ...
            options.add_argument(
                f"--window-size={self.config.viewport_size[0]},{self.config.viewport_size[1]}"
            )
    ...
    async def execute_automation_task(self, task: AutomationTask) -> AutomationResult:
        ...
        try:
            ...
            service_url = await self._find_service_url(
                task.ministry, task.service_category
            )
            if service_url:
            ...
            if not submission_result.get("success", False):
                raise Exception(
                    f"Form submission failed: {submission_result.get('error', 'Unknown error')}"
                )
            ...
            result.cultural_compliance = await self._validate_cultural_compliance(
                task, result
            )

            # Arabic processing validation
            result.arabic_processing_success = await self._validate_arabic_processing(
                task, result
            )
            ...
            self.logger.info(
                f"Automation task {task.task_id} completed successfully in {result.completion_time:.2f} seconds"
            )
    ...
    async def _fill_arabic_text(self, field: FormField) -> bool:
        ...
        try:
            ...
            if field.cultural_validation:
                is_culturally_appropriate = self.cultural_validator.validate_text(
                    processed_text
                )
                if not is_culturally_appropriate:
                    self.logger.warning(
                        f"Arabic text in field {field.name} failed cultural validation"
                    )
                    return False
            ...
            self.logger.error(
                f"Failed to fill Arabic text field {field.name}: {str(e)}"
            )
            return False
...
class PortalIntelligence:
    ...
    async def get_portal_mapping(
        self, ministry: IraqiMinistry, service: ServiceCategory
    ...
    async def update_success_rate(
        self, ministry: IraqiMinistry, service: ServiceCategory, success: bool
    ):
