...
class PortalElementSelector:
    ...
    ARABIC_SELECTORS = {
        ...
        "national_id": [
            ...
            'input[placeholder*="الهوية المدنية"]',
        ...
        "birth_date": [
            'input[placeholder*="تاريخ الولادة"]',
            'input[placeholder*="الميلاد"]',
            'input[name*="birth"]',
            'input[type="date"]',
            'select[name*="birth"]',
        ],
    ...
    MINISTRY_SELECTORS = {
        IraqiMinistry.INTERIOR: {
            "passport_number": [
                'input[name*="passport"]',
                'input[placeholder*="جواز السفر"]',
            ],
            "residence_type": [
                'select[name*="residence"]',
                'select[placeholder*="الإقامة"]',
            ],
...
class BasePortalAdapter(ABC):
    ...
    @abstractmethod
    async def navigate_to_service(
        self, page: Page, service_category: ServiceCategory
    ) -> bool:
    ...
    async def execute_full_workflow(
        self, page: Page, task: AutomationTask
    ) -> AutomationResult:
        ...
        try:
            ...
            if not submission_result.get("success", False):
                result.errors.append(
                    f"Submission failed: {submission_result.get('error', 'Unknown')}"
                )
                return result
            ...
            if self.stats["total_attempts"] > 0:
                self.stats["average_duration"] = (
                    self.stats["average_duration"] * (self.stats["total_attempts"] - 1)
                    + result.completion_time
                ) / self.stats["total_attempts"]

        return result

    async def find_element_by_arabic_content(
        self, page: Page, arabic_text: str
        ...
        try:
            ...
            if field.field_type in self.selector.ARABIC_SELECTORS:
                field_selectors.extend(
                    self.selector.ARABIC_SELECTORS[field.field_type.value]
                )
...
class EducationMinistryAdapter(BasePortalAdapter):
    ...
    async def authenticate(self, page: Page, credentials: Dict[str, str]) -> bool:
        ...
        try:
            ...
            student_id = credentials.get(
                "student_id", credentials.get("national_id", "")
            )
            ...
            student_selectors = [
                'input[name*="student"]',
                'input[placeholder*="الطالب"]',
                'input[placeholder*="رقم الطالب"]',
                'input[id*="student"]',
            ]
                ...
                try:
                    ...
                    if element:
                        return {
                            ...
                            "tracking_number": await self._extract_education_tracking(
                                page
                            ),
                        }
                except:
    ...
    async def _verify_education_authentication(self, page: Page) -> bool:
        ...
        indicators = [
            ".student-dashboard",
            ':has-text("لوحة الطالب")',
            ".academic-records",
            ':has-text("السجل الأكاديمي")',
        ]
...
__all__ = [
    ...
    "InteriorMinistryAdapter",
    "EducationMinistryAdapter",
