...
class IraqiPaymentGatewayRouter:
    ...
    async def route_payment(self, request: IraqiPaymentRequest) -> RoutingDecision:
        ...
        try:
            ...
            if not cultural_validation["is_compliant"]:
                self.logger.warning(f"Cultural validation failed: {cultural_validation['reason']}")
            ...
            # Select best gateway
            selected_gateway = max(gateway_scores.keys(), key=lambda g: gateway_scores[g])
            ...
            return RoutingDecision(
                ...
                fallback_sequence=[IraqiPaymentGateway.NASSWALLET, IraqiPaymentGateway.FASTPAY],
    ...
    async def _validate_cultural_compliance(self, request: IraqiPaymentRequest) -> Dict[str, Any]:
            ...
            if request.arabic_description:
                ...
                for keyword in prohibited_keywords:
                    if keyword in description_lower:
                        validation_result["violations"].append(f"Contains prohibited content: {keyword}")
                        validation_result["score"] -= 0.5
        
        # Professional domain validation
        if request.professional_domain:
            domain_rules = self.cultural_rules["professional_domains"].get(request.professional_domain, {})
            
            if "min_amount" in domain_rules and request.amount < domain_rules["min_amount"]:
                validation_result["violations"].append(f"Amount below minimum for {request.professional_domain}")
                validation_result["score"] -= 0.2
        
        # Government payment special handling
        if request.is_government_payment:
            if request.priority != TransactionPriority.HIGH and request.priority != TransactionPriority.CRITICAL:
                validation_result["violations"].append("Government payments require high priority")
    ...
    async def _get_available_gateways(self, request: IraqiPaymentRequest) -> List[IraqiPaymentGateway]:
        ...
        for gateway, status in self.gateway_status.items():
            ...
            # Check amount limits
            if request.amount > config["max_amount"] or request.amount < config["min_amount"]:
                continue
            
            # Check governorate support
            if (request.customer_governorate and 
                config["supported_governorates"] != "all" and
                request.customer_governorate not in config["supported_governorates"]):
                continue
        ...
        if request.is_government_payment and IraqiPaymentGateway.GOVERNMENT not in available:
            if self.gateway_status[IraqiPaymentGateway.GOVERNMENT].is_available:
                available.append(IraqiPaymentGateway.GOVERNMENT)
        
        return available
    
    async def _calculate_gateway_score(self, gateway: IraqiPaymentGateway, request: IraqiPaymentRequest) -> float:
        ...
        time_factor = max(0, 1 - (status.response_time_ms / 60000))  # Normalize to 60s max
        ...
        if request.professional_domain:
            domain_rules = self.cultural_rules["professional_domains"].get(request.professional_domain, {})
            if config["processing_time"] <= domain_rules.get("max_processing_time", 300):
                score += 0.1
    ...
    async def update_gateway_status(self, gateway: IraqiPaymentGateway, 
    ...
    def get_gateway_health(self) -> Dict[str, Any]:
        ...
        health_report = {
            ...
            "available_gateways": sum(1 for s in self.gateway_status.values() if s.is_available),
            "average_response_time": self.metrics["average_response_time"],
            "success_rate": self.metrics["successful_routes"] / max(1, self.metrics["total_requests"]),
        ...
        available_ratio = health_report["available_gateways"] / health_report["total_gateways"]
...
class IraqiPaymentValidator:
    ...
    def format_iraqi_amount(amount: int) -> str:
        ...
        arabic_numerals = str(amount).translate(str.maketrans('0123456789', '٠١٢٣٤٥٦٧٨٩'))
