...
class IraqiProviderManager:
    ...
    def __init__(self):
        ...
        self.cultural_rules = {
            "islamic_compliance": {
                ...
                "required_values": ["respect", "family", "community", "education"],
            },
    ...
    def _initialize_default_providers(self):
        ...
        self.providers[AIProvider.LOCAL_ARABIC] = ProviderConfig(
            ...
            supported_domains=[ProfessionalDomain.GENERAL],
    ...
    async def select_optimal_provider(self, request: IraqiModelRequest) -> AIProvider:
        ...
        self.logger.info(
            f"Selected provider {selected_provider.value} with score {provider_scores[selected_provider]:.3f}"
        )

        return selected_provider

    async def _calculate_provider_score(
        self, provider: AIProvider, config: ProviderConfig, request: IraqiModelRequest
    ) -> float:
        ...
        if request.max_response_time > 0:
            time_factor = min(
                1.0, request.max_response_time / config.average_response_time
            )
    ...
    async def _call_claude_api(
        self, config: ProviderConfig, request: IraqiModelRequest
    ) -> Dict[str, Any]:
        ...
        if request.professional_domain:
            response_text += (
                f" (Professional domain: {request.professional_domain.value})"
            )
    ...
    async def _call_local_arabic_api(
        self, config: ProviderConfig, request: IraqiModelRequest
    ) -> Dict[str, Any]:
    ...
    async def _validate_response(
        self, response: Dict[str, Any], request: IraqiModelRequest
    ) -> Dict[str, Any]:
        ...
        if request.professional_domain:
            domain_score = await self._validate_domain_accuracy(
                response_text, request.professional_domain
            )
    ...
    async def _validate_islamic_compliance(self, response_text: str) -> float:
        ...
        prohibited_terms = self.cultural_rules["islamic_compliance"][
            "prohibited_content"
        ]
    ...
    async def _validate_domain_accuracy(
        self, response_text: str, domain: ProfessionalDomain
    ) -> float:
    ...
    def _update_metrics(
        self, provider: AIProvider, response: Dict[str, Any], validation: Dict[str, Any]
    ):
    ...
    def get_provider_analytics(self) -> Dict[str, Any]:
        ...
        analytics = {
            ...
            "total_cost": sum(
                m["cost_total"] for m in self.performance_metrics.values()
            ),
            "provider_details": {},
        }
        ...
        if total_requests > 0:
            analytics["overall_success_rate"] = (
                sum(m["successful_requests"] for m in self.performance_metrics.values())
                / total_requests
            )
