...
# Configuration
AVAILABLE_DOMAINS = ["legal", "medical", "educational", "government", "business", "engineering"]
...
async def main(message: cl.Message):
    ...
    if settings.get("cultural_validation", True):
        cultural_result = cultural_context.validate_cultural_appropriateness(processed_input)
...
async def handle_auto_mode(user_input: str, settings: Dict[str, Any]):
    ...
    if detected_domain:
        ...
        try:
            ...
            await cl.Message(
                content=f"❌ **Error | خطأ**: {str(e)}"
            ).send()
    
    else:
        # Ask user to specify domain
        await cl.Message(
            content=await create_domain_selection_message()
        ).send()
...
async def handle_multi_agent_mode(user_input: str, settings: Dict[str, Any]):
    ...
    if len(required_domains) > 1:
        ...
        team = agent_generator.create_iraqi_multi_agent_team(required_domains, user_input)
...
async def detect_professional_domain(user_input: str) -> Optional[str]:
    ...
    domain_keywords = {
        "legal": ["قانون", "محكمة", "عقد", "دعوى", "حقوق", "law", "court", "contract", "lawsuit", "rights"],
        "medical": ["طبي", "صحة", "مرض", "علاج", "دواء", "medical", "health", "disease", "treatment", "medicine"],
        "educational": ["تعليم", "مدرسة", "جامعة", "منهج", "دراسة", "education", "school", "university", "curriculum", "study"],
        "government": ["حكومة", "وزارة", "خدمات", "إجراءات", "مدنية", "government", "ministry", "services", "procedures", "civil"],
        "business": ["تجارة", "أعمال", "شركة", "استثمار", "مال", "business", "company", "investment", "money", "trade"],
        "engineering": ["هندسة", "بناء", "مشروع", "تصميم", "تقني", "engineering", "construction", "project", "design", "technical"]
    }
...
async def detect_required_domains(user_input: str) -> List[str]:
    ...
    if any(keyword in user_input_lower for keyword in ["قانون", "محكمة", "عقد", "law", "legal", "court"]):
        required_domains.append("legal")
    
    if any(keyword in user_input_lower for keyword in ["طبي", "صحة", "علاج", "medical", "health", "treatment"]):
        required_domains.append("medical")
    
    if any(keyword in user_input_lower for keyword in ["تعليم", "مدرسة", "منهج", "education", "school", "curriculum"]):
        required_domains.append("educational")
    
    if any(keyword in user_input_lower for keyword in ["حكومة", "وزارة", "خدمات", "government", "ministry", "services"]):
        required_domains.append("government")
    
    if any(keyword in user_input_lower for keyword in ["تجارة", "أعمال", "شركة", "business", "company", "trade"]):
        required_domains.append("business")
    
    if any(keyword in user_input_lower for keyword in ["هندسة", "بناء", "مشروع", "engineering", "construction", "project"]):
        required_domains.append("engineering")
    
    return required_domains if required_domains else ["legal"]  # Default to legal if none detected

async def process_with_agent(agent: Any, user_input: str, settings: Dict[str, Any]) -> Dict[str, Any]:
...
async def process_with_agent_team(team: Dict[str, Any], user_input: str, settings: Dict[str, Any]) -> Dict[str, Any]:
