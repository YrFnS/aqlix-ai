...
export interface IraqiDesktopAPI {
  ...
  arabic: {
    ...
    validateArabicText(text: string): Promise<{ isValid: boolean; issues: string[] }>;
  };
  ...
  static on(channel: string, listener: (...args: any[]) => void): () => void {
    const wrappedListener = (event: any, ...args: any[]) => {
      ...
      if (event.senderFrame && !isValidOrigin(event.senderFrame.origin)) {
        console.warn('Blocked IPC message from invalid origin:', event.senderFrame.origin);
        return;
      }
...
const iraqiDesktopAPI: IraqiDesktopAPI = {
  ...
  arabic: {
    ...
    async validateArabicText(text: string): Promise<{ isValid: boolean; issues: string[] }> {
