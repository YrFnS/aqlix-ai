...
class IraqiAIDesktopApp {
  ...
  private setupSecurityProtocols(): void {
    ...
    app.on('web-contents-created', (event, contents) => {
      contents.on('did-finish-load', () => {
        if (this.config.enableGovernmentMode) {
          contents.insertCSP("default-src 'self' 'unsafe-inline' data: https://fonts.googleapis.com https://fonts.gstatic.com; connect-src 'self' https://*.gov.iq wss://*.gov.iq");
        }
      });
    });
  }
  ...
  private createMainWindow(): void {
    ...
    const title = this.config.defaultLanguage === 'ar' 
  ...
  private setupApplicationMenu(): void {
    const template: Electron.MenuItemConstructorOptions[] = [
      {
        ...
        submenu: [
          {
            label: this.config.defaultLanguage === 'ar' ? 'محادثة جديدة' : 'New Chat',
          ...
          { type: 'separator' },
          {
            label: this.config.defaultLanguage === 'ar' ? 'الإعدادات' : 'Settings',
      ...
      },
      {
        ...
        submenu: [
          { role: 'undo', label: this.config.defaultLanguage === 'ar' ? 'تراجع' : 'Undo' },
          { role: 'redo', label: this.config.defaultLanguage === 'ar' ? 'إعادة' : 'Redo' },
          { type: 'separator' },
          { role: 'cut', label: this.config.defaultLanguage === 'ar' ? 'قص' : 'Cut' },
          { role: 'copy', label: this.config.defaultLanguage === 'ar' ? 'نسخ' : 'Copy' },
          { role: 'paste', label: this.config.defaultLanguage === 'ar' ? 'لصق' : 'Paste' },
          { role: 'selectall', label: this.config.defaultLanguage === 'ar' ? 'تحديد الكل' : 'Select All' }
        ]
      },
      {
        label: this.config.defaultLanguage === 'ar' ? 'عرض' : 'View',
        submenu: [
          { role: 'reload', label: this.config.defaultLanguage === 'ar' ? 'إعادة تحميل' : 'Reload' },
          { role: 'forceReload', label: this.config.defaultLanguage === 'ar' ? 'إعادة تحميل قسرية' : 'Force Reload' },
          { role: 'toggleDevTools', label: this.config.defaultLanguage === 'ar' ? 'أدوات المطور' : 'Developer Tools' },
          { type: 'separator' },
          { role: 'resetZoom', label: this.config.defaultLanguage === 'ar' ? 'إعادة تعيين التكبير' : 'Reset Zoom' },
          { role: 'zoomIn', label: this.config.defaultLanguage === 'ar' ? 'تكبير' : 'Zoom In' },
          { role: 'zoomOut', label: this.config.defaultLanguage === 'ar' ? 'تصغير' : 'Zoom Out' },
          { type: 'separator' },
          { role: 'togglefullscreen', label: this.config.defaultLanguage === 'ar' ? 'ملء الشاشة' : 'Toggle Fullscreen' }
        ]
      }
    ];
    ...
    if (this.config.enableGovernmentMode) {
      template.push({
        ...
        submenu: [
          {
            label: this.config.defaultLanguage === 'ar' ? 'تقرير الأمان' : 'Security Report',
            click: () => {
              this.generateSecurityReport();
            }
          },
          {
            label: this.config.defaultLanguage === 'ar' ? 'تسجيل الأحداث' : 'Audit Log',
            click: () => {
              this.showAuditLog();
            }
          },
          {
            label: this.config.defaultLanguage === 'ar' ? 'التحقق من التحديثات' : 'Check for Updates',
            click: () => {
              this.checkForUpdates();
            }
          }
    ...
    template.push({
      ...
      submenu: [
        {
          label: this.config.defaultLanguage === 'ar' ? 'دليل المستخدم' : 'User Guide',
          click: () => {
            shell.openExternal('https://docs.iraqi-ai.gov.iq');
          }
        },
        {
          label: this.config.defaultLanguage === 'ar' ? 'الدعم الفني' : 'Technical Support',
          click: () => {
        ...
        { type: 'separator' },
        {
          label: this.config.defaultLanguage === 'ar' ? 'حول البرنامج' : 'About',
          click: () => {
  ...
  private setupContextMenu(): void {
    contextMenu({
      prepend: (defaultActions, parameters, browserWindow) => [
        {
          label: this.config.defaultLanguage === 'ar' ? 'ترجمة إلى العربية' : 'Translate to Arabic',
        ...
        {
          label: this.config.defaultLanguage === 'ar' ? 'ترجمة إلى الإنجليزية' : 'Translate to English',
          visible: parameters.selectionText.trim().length > 0,
          click: () => {
            mainWindow?.webContents.send('translate-text', {
              text: parameters.selectionText,
      ...
      labels: {
        ...
        selectAll: this.config.defaultLanguage === 'ar' ? 'تحديد الكل' : 'Select All',
        inspect: this.config.defaultLanguage === 'ar' ? 'فحص العنصر' : 'Inspect Element'
      }
    });
  }
  ...
  private generateSplashHTML(): string {
    ...
    const appName = isArabic ? 'نظام الدردشة الذكي العراقي' : 'Iraqi AI Chat System';
  ...
  private generateSecurityReport(): void {
    ...
    const reportPath = join(app.getPath('documents'), 'iraqi-ai-security-report.json');
    writeFileSync(reportPath, JSON.stringify(report, null, 2));
    
    dialog.showMessageBox(mainWindow!, {
      type: 'info',
      title: this.config.defaultLanguage === 'ar' ? 'تقرير الأمان' : 'Security Report',
      message: this.config.defaultLanguage === 'ar' 
