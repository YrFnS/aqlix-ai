...
export class IraqiOfflineManager {
  ...
  async loadPersona(personaId: string): Promise<OfflinePersona | null> {
    try {
      ...
      personaData.configuration = JSON.parse(this.decryptData(personaData.configuration));
...
export function registerOfflineHandlers(offlineManager: IraqiOfflineManager): void {
  ...
  ipcMain.handle('get-cached-data', (event, key) => offlineManager.getCachedData(key));
  ipcMain.handle('set-cached-data', (event, key, data, ttl) => offlineManager.setCachedData(key, data, ttl));
  ipcMain.handle('clear-cache', () => offlineManager.clearCache());
  ipcMain.handle('get-storage-stats', () => offlineManager.getStorageStats());
  ipcMain.handle('save-offline-chat', (event, chat) => offlineManager.saveChat(chat));
  ipcMain.handle('load-offline-chat', (event, chatId) => offlineManager.loadChat(chatId));
  ipcMain.handle('get-all-offline-chats', () => offlineManager.getAllChats());
  ipcMain.handle('delete-offline-chat', (event, chatId) => offlineManager.deleteChat(chatId));
  ipcMain.handle('save-offline-persona', (event, persona) => offlineManager.savePersona(persona));
  ipcMain.handle('load-offline-persona', (event, personaId) => offlineManager.loadPersona(personaId));
}
