...
class OfflineManager {
  ...
  async cachePersona(personaData: any) {
    ...
    await this.db.personas.add({ name: personaData.name, encryptedData: encrypted, timestamp: Date.now() });
  ...
  async cacheDocument(docData: any) {
    ...
    await this.db.documents.add({ title: docData.title, encryptedContent: encrypted, culturalValidated: true, timestamp: Date.now() });
    await this.syncToSupabase('documents', docData);
  }

  private encryptData(data: any): string {
    const cipher = crypto.createCipher('aes256', process.env.OFFLINE_KEY || 'iraqi-ai-2025-secret');
  ...
  private decryptData(encrypted: string): any {
    const decipher = crypto.createDecipher('aes256', process.env.OFFLINE_KEY || 'iraqi-ai-2025-secret');
  ...
  private async syncToSupabase(table: string, data: any) {
    ...
    this.supabase.channel('offline-sync').on('postgres_changes', { event: '*', schema: 'public', table }, () => {
