...
contextBridge.exposeInMainWorld('electronAPI', {
  requestOfflineCache: (data: any) => ipcRenderer.invoke('offline-cache-request', data),
  ...
  startArabicSTT: (audioStream: any) => {
    ...
    return ipcRenderer.on('arabic-stt-response', (event, transcript) => transcript);
  },
  onPrayerNotification: (callback: (notification: any) => void) => {
    ipcRenderer.on('prayer-notification', (event, notification) => callback(notification));
  },
  triggerOfflineSync: () => ipcRenderer.send('offline-sync'),
});

// Security: Validate exposed methods
const validMethods = ['requestOfflineCache', 'startArabicTTS', 'startArabicSTT', 'onPrayerNotification', 'triggerOfflineSync'];
