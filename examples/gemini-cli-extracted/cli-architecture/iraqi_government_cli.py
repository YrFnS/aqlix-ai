...
class IraqiGovernmentCLI:
    ...
    async def discover_tools(
        self, domain: Optional[GovernmentDomain] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Discovered {len(enhanced_tools)} tools for domain {domain}"
            )
    ...
    async def _check_tool_authorization(self, invocation: ToolInvocation):
        ...
        if clearance_hierarchy.get(user_clearance, 0) < clearance_hierarchy.get(
...
class IraqiAuthenticationManager:
    ...
    async def authenticate(self, credentials: Dict[str, Any]):
        ...
        class AuthResult:
            def __init__(
                self, success: bool, user_context: Dict = None, error: str = ""
            ):
        ...
        if self.auth_type == AuthType.CITIZEN_ID:
            ...
            if citizen_id and len(citizen_id) == 10:
                return AuthResult(
                    True, {"citizen_id": citizen_id, "clearance": "public"}
                )
            else:
            ...
            if oauth_token:
                return AuthResult(
                    True, {"oauth_token": oauth_token, "clearance": "restricted"}
                )
            else:
...
class IraqiToolDiscovery:
    ...
    def __init__(
        self,
        government_domain: Optional[GovernmentDomain],
    ...
    async def discover_tools(
        self,
        domain: Optional[GovernmentDomain],
...
@click.option(
    ...
    default="unified_login",
    help="Authentication method",
)
@click.option(
    "--language",
    type=click.Choice(["ar", "en", "mixed"]),
    default="ar",
...
@click.option(
    "--domain",
    type=click.Choice([e.value for e in GovernmentDomain]),
    help="Government domain to search",
)
@click.option(
    "--format",
    type=click.Choice(["json", "table", "arabic"]),
    default="table",
    help="Output format",
)
@click.pass_context
