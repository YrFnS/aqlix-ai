...
class GovernmentAPISource(IraqiToolDiscoverySource):
    ...
    async def discover_tools(self, config: ToolDiscoveryConfig) -> List[ToolMetadata]:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=config.discovery_timeout)) as session:
    ...
    async def _parse_api_tools(self, data: Dict[str, Any], config: ToolDiscoveryConfig) -> List[ToolMetadata]:
        ...
        for tool_data in data.get("tools", []):
            try:
                tool = ToolMetadata(
                    ...
                    category=ToolCategory(tool_data.get("category", "citizen_services")),
                    ...
                    islamic_compliance_required=tool_data.get("islamic_compliance_required", False),
                    arabic_language_support=tool_data.get("arabic_language_support", False),
                    rtl_interface_support=tool_data.get("rtl_interface_support", False),
                    
                    ministry_owner=tool_data.get("ministry_owner"),
                    inter_ministry_approval=tool_data.get("inter_ministry_approval", False),
...
class MinistryRegistrySource(IraqiToolDiscoverySource):
    ...
    def __init__(self, ministry_name: str, registry_url: str, credentials: Optional[Dict[str, str]] = None):
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=config.discovery_timeout)) as session:
                async with session.get(self.registry_url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        self.logger.warning(f"Ministry registry discovery failed: {response.status}")
    ...
    async def _parse_ministry_tools(self, data: Dict[str, Any], config: ToolDiscoveryConfig) -> List[ToolMetadata]:
        ...
        for service_data in data.get("services", []):
            try:
                tool = ToolMetadata(
                    ...
                    category=ToolCategory(service_data.get("category", "administrative")),
...
class IraqiGovernmentToolDiscovery:
    ...
    async def discover_all_tools(self, force_refresh: bool = False) -> List[ToolMetadata]:
        """
        ...
        if not force_refresh and cache_key in self.tool_cache:
            ...
            if datetime.now() - cached_time < timedelta(seconds=self.config.cache_duration):
        ...
        for i, result in enumerate(discovery_results):
            if isinstance(result, Exception):
                self.logger.error(f"Discovery failed for source {valid_sources[i].get_source_name()}: {result}")
                continue
            
            all_tools.extend(result)
        
        self.logger.info(f"Discovered {len(all_tools)} tools from {len(valid_sources)} sources")
        ...
        self.logger.info(f"Tool discovery completed in {discovery_time:.2f}s - {len(validated_tools)} validated tools")
        
        return validated_tools
    
    async def discover_tools_by_category(self, category: ToolCategory) -> List[ToolMetadata]:
    ...
    async def _validate_sources(self) -> List[IraqiToolDiscoverySource]:
        ...
        for i, result in enumerate(validation_results):
            ...
            if isinstance(result, Exception):
                self.logger.error(f"Source validation failed for {source.get_source_name()}: {result}")
                continue
            ...
            if is_valid:
                ...
                self.logger.warning(f"Source invalid: {source.get_source_name()} - {message}")
        
        return valid_sources
    
    async def _discover_from_source(self, source: IraqiToolDiscoverySource) -> List[ToolMetadata]:
        """Discover tools from single source"""
        try:
            tools = await source.discover_tools(self.config)
            self.logger.info(f"Discovered {len(tools)} tools from {source.get_source_name()}")
            return tools
        except Exception as e:
            self.logger.error(f"Discovery failed for {source.get_source_name()}: {str(e)}")
            return []
    ...
    async def _apply_filters(self, tools: List[ToolMetadata]) -> List[ToolMetadata]:
        ...
        for tool in tools:
            ...
            if self.config.included_ministries and tool.ministry_owner not in self.config.included_ministries:
                continue
        ...
        self.logger.info(f"Filtered to {len(filtered_tools)} tools after applying filters")
    ...
    async def _validate_tools(self, tools: List[ToolMetadata]) -> List[ToolMetadata]:
        ...
        validation_results = await asyncio.gather(*limited_tasks, return_exceptions=True)
        ...
        try:
            ...
            if self.config.require_cultural_validation:
                ...
                if cultural_score < self.config.minimum_compliance_score:
                    self.logger.debug(f"Tool {tool.name} failed cultural validation: {cultural_score}")
                    return False
            
            # Islamic compliance validation
            if self.config.require_islamic_compliance:
                islamic_score = await self.compliance_checker.check_islamic_compliance(tool)
                tool.islamic_compliance_score = islamic_score
                
                if islamic_score < self.config.minimum_compliance_score:
                    self.logger.debug(f"Tool {tool.name} failed Islamic compliance: {islamic_score}")
                    return False
...
class IraqiSecurityValidator:
    ...
    async def validate_tool_security(self, tool: ToolMetadata) -> bool:
        ...
        if tool.security_classification != SecurityClassification.PUBLIC and not tool.encryption_required:
            return False
...
class IraqiComplianceChecker:
    ...
    async def check_islamic_compliance(self, tool: ToolMetadata) -> float:
        ...
        prohibited_keywords = [
            "gambling", "lottery", "alcohol", "interest", "usury", "riba"
        ]
