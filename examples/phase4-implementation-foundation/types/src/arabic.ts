...
export const ArabicTextSchema = z.object({
  ...
  dialect: z.enum(['iraqi', 'standard', 'gulf', 'levantine', 'maghrebi', 'egyptian']).default('iraqi')
});
...
export const RTLConfigSchema = z.object({
  ...
  supportedDialects: z.array(z.enum(['iraqi', 'standard', 'gulf', 'levantine', 'maghrebi', 'egyptian'])).default(['iraqi', 'standard']),
...
export const ArabicProcessingResultSchema = z.object({
  ...
  detectedDialect: z.enum(['iraqi', 'standard', 'gulf', 'levantine', 'maghrebi', 'egyptian']),
...
export type ArabicProcessingResult = z.infer<typeof ArabicProcessingResultSchema>;

// Iraqi Dialect Recognition Schema
export const IraqiDialectSchema = z.object({
  text: z.string(),
  dialectRegion: z.enum(['baghdad', 'basra', 'mosul', 'kurdistan', 'anbar', 'general']).default('general'),
  confidence: z.number().min(0).max(1),
  features: z.array(z.string()).default([]),
  vocabulary: z.array(z.object({
    word: z.string(),
    meaning: z.string(),
    category: z.enum(['colloquial', 'formal', 'religious', 'professional', 'slang']).default('colloquial')
  })).default([])
});
...
export const MixedContentSchema = z.object({
  ...
  processingStrategy: z.enum(['segment-based', 'context-aware', 'unified']).default('context-aware')
});
