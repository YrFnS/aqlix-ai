...
export type IraqiProfessionalStandards = z.infer<typeof IraqiProfessionalStandardsSchema>;
...
export const IraqiLegalDomainSchema = z.object({
  ...
  jurisdictions: z.array(z.enum(['federal', 'regional', 'local'])).default(['federal']),
  practiceAreas: z.array(z.enum([
    'civil_law', 'commercial_law', 'criminal_law', 'family_law',
    'administrative_law', 'constitutional_law', 'international_law',
    'islamic_jurisprudence', 'property_law', 'labor_law'
  ])).default([]),
...
export const IraqiMedicalDomainSchema = z.object({
  ...
  specializations: z.array(z.enum([
    'general_medicine', 'surgery', 'pediatrics', 'cardiology',
    'neurology', 'psychiatry', 'dermatology', 'ophthalmology',
    'orthopedics', 'gynecology', 'emergency_medicine', 'radiology',
    'pathology', 'anesthesiology', 'family_medicine'
  ])).default([]),
...
export const IraqiEducationalDomainSchema = z.object({
  educationLevel: z.array(z.enum([
    'kindergarten', 'primary', 'intermediate', 'secondary',
    'vocational', 'university', 'postgraduate', 'professional'
  ])).default([]),
  curriculum: z.enum(['iraqi_national', 'international', 'islamic', 'mixed']).default('iraqi_national'),
  teachingLanguages: z.array(z.enum(['arabic', 'kurdish', 'english'])).default(['arabic']),
...
export type IraqiEducationalDomain = z.infer<typeof IraqiEducationalDomainSchema>;
...
export const IraqiGovernmentDomainSchema = z.object({
  ...
  ministry: z.enum([
    'interior', 'defense', 'foreign_affairs', 'finance', 'oil',
    'electricity', 'health', 'education', 'justice', 'agriculture',
    'transportation', 'communications', 'labor', 'trade', 'planning',
    'water_resources', 'culture', 'tourism', 'youth_sports', 'science_technology'
  ]).optional(),
  securityClearance: z.enum(['public', 'restricted', 'confidential', 'secret']).default('public'),
  publicServiceRank: z.string().optional(),
  islamicValuesAlignment: z.boolean().default(true),
  bilingualism: z.array(z.enum(['arabic', 'kurdish', 'english'])).default(['arabic']),
...
export const ProfessionalExpertiseSchema = z.object({
  domain: ProfessionalDomainEnum,
  expertiseLevel: z.enum(['entry', 'junior', 'mid', 'senior', 'expert', 'master']).default('mid'),
  ...
  arabicProficiency: z.enum(['native', 'fluent', 'intermediate', 'basic', 'none']).default('fluent'),
  englishProficiency: z.enum(['native', 'fluent', 'intermediate', 'basic', 'none']).default('intermediate')
});
...
export const ProfessionalConsultationRequestSchema = z.object({
  ...
  confidentialityLevel: z.enum(['public', 'private', 'confidential']).default('private'),
...
export type ProfessionalConsultationRequest = z.infer<typeof ProfessionalConsultationRequestSchema>;
...
export type ProfessionalConsultationResponse = z.infer<typeof ProfessionalConsultationResponseSchema>;