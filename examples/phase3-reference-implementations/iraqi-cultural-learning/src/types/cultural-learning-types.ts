...
export interface CulturalLearningContext {
  ...
  user: {
    ...
    demographic: {                          // الديموغرافية - Demographic information
    ...
    preferences: {                          // التفضيلات - User preferences
    ...
    interaction_history: {                  // تاريخ التفاعل - Interaction history
  ...
  temporal: {
    ...
    islamic_calendar?: {                   // التقويم الإسلامي - Islamic calendar context
      month: string;
...
export interface CulturalLearningFeedback {
  ...
  feedback: {
    ...
    dimensions: {                          // أبعاد التقييم - Specific feedback dimensions
    ...
    specific_issues: Array<{               // قضايا محددة - Specific issues identified
      category: 'cultural' | 'religious' | 'linguistic' | 'professional' | 'factual';
...
export interface CulturalModelPerformance {
  ...
  feature_importance: {
    ...
    top_features: Array<{                // المعالم الأهم - Most important features
      feature_name: string;
...
export interface CulturalLearningConfig {
  // Algorithm Selection
  algorithms: {
    primary_algorithm: 'neural_network' | 'decision_tree' | 'random_forest' | 'svm' | 'naive_bayes' | 'ensemble';
  ...
  monitoring: {
    ...
    alert_thresholds: {                  // حدود التنبيه - Performance alert thresholds
...
export interface ProfessionalAdaptationLearning {
  ...
  domains: Record<IraqiProfessionalDomain, {
...
export const CulturalLearningContextSchema = z.object({
  user: z.object({
    ...
    demographic: z.object({
      ...
      education_level: z.enum(['primary', 'secondary', 'undergraduate', 'graduate', 'postgraduate']).optional(),
      profession: z.string().optional(),
      region: z.enum(['baghdad', 'basra', 'mosul', 'erbil', 'najaf', 'karbala', 'other']).optional(),
      religious_school: z.enum(['sunni', 'shia', 'other', 'prefer_not_to_say']).optional()
    }),
  ...
  professional: z.object({
    ...
    institution_type: z.enum(['academic', 'government', 'private', 'religious'])
  }).optional(),
  temporal: z.object({
    timestamp: z.date(),
...
export const CulturalLearningFeedbackSchema = z.object({
  ...
  timestamp: z.date(),
  source: z.enum(['explicit_positive', 'explicit_negative', 'implicit_engagement', 'implicit_avoidance', 'cultural_validator', 'expert_review', 'peer_consensus', 'behavioral_pattern']),
  ...
  feedback: z.object({
    ...
    specific_issues: z.array(z.object({
      category: z.enum(['cultural', 'religious', 'linguistic', 'professional', 'factual']),
