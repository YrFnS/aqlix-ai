...
import { IraqiAgentCoordinator, IraqiAgentCoordinatorConfig } from '@iraqi-ai/a2a';
import { 
  IraqiAgentCard,
...
export class IraqiAgentManager {
  ...
  constructor(config: IraqiAgentManagerConfig) {
    ...
    this.coordinator = new IraqiAgentCoordinator({
      ...
      fallbackTransports: [IraqiTransportProtocol.GRPC, IraqiTransportProtocol.HTTP_JSON],
   ...
   */
  private async createAgentCard(name: string, definition: IraqiAgentDefinition): Promise<IraqiAgentCard> {
    return {
      ...
      capabilities: {
        ...
        arabicSupport: definition.arabicNLPSupport ? {
          rtlProcessing: true,
          iraqiDialectRecognition: definition.dialectRecognition === 'iraqi',
      ...
      iraqiMetadata: {
        ...
        arabicCapabilities: {
          rtlAccuracy: definition.accuracyTarget || (definition.arabicNLPSupport ? 99 : 0),
          dialectRecognition: definition.dialectRecognition === 'iraqi' ? 85 : 0,
  ...
  async coordinateResponse(
    ...
    try {
      ...
      const coordinationResult = await this.coordinator.coordinateAgents(coordinationRequest);
      ...
      return {
        ...enhancedResponse,
        coordinationMetadata: coordinationResult.metadata,
        culturalValidation: coordinationResult.metadata.culturalValidationResults
      };
  ...
  async getHealthStatus(): Promise<any> {
    ...
    return {
      ...
      agentSystemHealth: {
        ...
        culturalComplianceRate: stats.culturalComplianceRate,
        averageCoordinationTime: stats.averageCoordinationTime,
        performanceStatus: stats.averageCoordinationTime < this.config.maxCoordinationTime ? 'optimal' : 'needs-optimization'
      },
      culturalSovereignty: {
  ...
  // Helper methods
  private getAgentDescription(name: string, definition: IraqiAgentDefinition): string {
    const typeDescriptions = {
      'cultural-intelligence': 'Iraqi cultural validation and Islamic compliance specialist',
      'business-intelligence': 'Iraqi business process and market dynamics analyst', 
      'design-intelligence': 'Islamic-appropriate design and RTL interface specialist',
      'research-intelligence': 'Iraqi user behavior and cultural research specialist',
      'technical-intelligence': 'PydanticAI agent with Iraqi cultural context integration',
      'language-processing-tool': 'Real-time Arabic RTL processing and Iraqi dialect recognition',
      'financial-security-tool': 'Iraqi payment gateway security and compliance validator',
      'system-coordination': 'Multi-agent workflow coordination with cultural oversight'
    };

    return typeDescriptions[definition.type] || `Specialized ${definition.type} for Iraqi AI ecosystem`;
  }
  ...
  private generateSkillsFromDefinition(definition: IraqiAgentDefinition): any[] {
  ...
  private mapTypeToProfile(type: string): 'cultural' | 'legal' | 'medical' | 'educational' | 'business' | 'technical' {
    const profileMap = {
      'cultural-intelligence': 'cultural',
      'business-intelligence': 'business',
  ...
  private mergeCoordinationResults(baseResponse: any, coordinationResults: Record<string, any>): any {
    return {
      ...baseResponse,
  ...
  private async getAgentHealthSummary(): Promise<any> {
    const healthSummaries = Array.from(this.agentCards.keys()).map(name => ({
      ...
      arabicCapable: this.agentCards.get(name)?.capabilities.arabicSupport !== undefined
    }));

    return {
      totalAgents: healthSummaries.length,
      operationalAgents: healthSummaries.filter(a => a.operational).length,
      culturallyCompliantAgents: healthSummaries.filter(a => a.culturalCompliant).length,
      arabicCapableAgents: healthSummaries.filter(a => a.arabicCapable).length,
      healthyAgentsPercentage: (healthSummaries.filter(a => a.operational && a.culturalCompliant).length / healthSummaries.length) * 100
    };
  }
}
...
class CoordinationMetrics {
  ...
  recordSuccess(metadata: any): void {
    ...
    this.metrics.averageCoordinationTime = this.metrics.totalCoordinationTime / this.metrics.totalCoordinations;
  }
  ...
  getLastMetrics(): any {
    return {
      ...
      successRate: this.metrics.totalCoordinations > 0 ? 
        (this.metrics.successfulCoordinations / this.metrics.totalCoordinations) * 100 : 0
    };
  }
