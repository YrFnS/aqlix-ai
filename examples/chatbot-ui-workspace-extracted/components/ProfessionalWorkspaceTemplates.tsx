...
const PROFESSIONAL_TEMPLATES: ProfessionalTemplate[] = [
  {
    ...
    description: 'Comprehensive legal workspace for Iraqi law practitioners covering civil, commercial, and family law.',
    descriptionAr: 'مساحة عمل قانونية شاملة للممارسين القانونيين العراقيين تغطي القانون المدني والتجاري وقانون الأسرة.',
    icon: Scale,
    features: [
    ...
    specializations: [
      ...
      },
      {
        ...
        requirements: ['Commercial law license', 'Business registration expertise']
      },
      {
        id: 'family-law',
        name: 'Family Law',
  ...
  },
  {
    ...
    description: 'Complete medical practice management for Iraqi healthcare providers and clinics.',
    descriptionAr: 'إدارة شاملة للممارسة الطبية لمقدمي الرعاية الصحية والعيادات العراقية.',
    icon: Stethoscope,
    features: [
    ...
    specializations: [
      ...
      },
      {
        ...
        requirements: ['Pediatric specialization', 'Child healthcare certification']
      },
      {
        ...
        requirements: ['Internal medicine board certification', 'Adult care specialization']
      }
    ],
    defaultSettings: {
      maxMembers: 20,
      maxFileSize: 200,
      allowedFileTypes: ['pdf', 'doc', 'docx', 'dcm', 'nii', 'jpg', 'png', 'tiff'],
      culturalStrictness: 'strict',
      requiresLicense: true,
      complianceLevel: 'maximum'
    },
  ...
  },
  {
    ...
    description: 'Comprehensive educational management for Iraqi schools, universities, and training centers.',
    descriptionAr: 'إدارة تعليمية شاملة للمدارس والجامعات ومراكز التدريب العراقية.',
    icon: GraduationCap,
    features: [
    ...
    specializations: [
      {
        ...
        requirements: ['Education ministry certification', 'Primary teaching qualification']
      },
      {
        id: 'higher-education',
        ...
        requirements: ['Higher education license', 'Academic administration certification']
      },
      {
        ...
        requirements: ['Vocational training certification', 'Industry expertise']
      }
    ],
    defaultSettings: {
      maxMembers: 100,
      maxFileSize: 100,
      allowedFileTypes: ['pdf', 'doc', 'docx', 'ppt', 'pptx', 'xls', 'xlsx', 'mp4', 'mp3'],
  ...
  },
  {
    ...
    description: 'Complete business management solution for Iraqi companies and startups.',
    ...
    specializations: [
      ...
      },
      {
        ...
        requirements: ['Service provider license', 'Professional certification']
      }
    ],
    defaultSettings: {
      maxMembers: 50,
  ...
  },
  {
    ...
    nameAr: 'شركة هندسية',
    description: 'Professional engineering practice management for Iraqi engineering firms and consultancies.',
    descriptionAr: 'إدارة الممارسة الهندسية المهنية للشركات الهندسية والاستشارات العراقية.',
    icon: Wrench,
    features: [
    ...
    specializations: [
      ...
      },
      {
        ...
        requirements: ['Mechanical engineering certification', 'Manufacturing experience']
      },
      {
        ...
        requirements: ['Electrical engineering license', 'Power systems certification']
      }
    ],
    defaultSettings: {
      maxMembers: 30,
      maxFileSize: 1000,
      allowedFileTypes: ['pdf', 'doc', 'docx', 'dwg', 'dxf', 'step', 'iges', 'stl', 'obj'],
      culturalStrictness: 'standard',
      requiresLicense: true,
      complianceLevel: 'high'
    },
...
export default function ProfessionalWorkspaceTemplates({
  ...
  const text = {
    ...
    en: {
      ...
      subtitle: 'Choose from pre-configured templates designed for Iraqi professions',
      filterAll: 'All',
      filterLegal: 'Legal',
  ...
  const filteredTemplates = selectedType === 'all' 
    ...
    : PROFESSIONAL_TEMPLATES.filter(template => template.type === selectedType);
      ...
      <Card key={template.id} className="relative hover:shadow-lg transition-shadow">
        ...
        <CardContent className="space-y-4">
          ...
          <div className={`flex flex-wrap gap-2 ${isRTL ? 'justify-end' : ''}`}>
            ...
            <Badge 
              variant={template.defaultSettings.complianceLevel === 'maximum' ? 'default' : 'secondary'}
              className={`text-xs ${isRTL ? 'font-arabic' : ''}`}
            >
              <CheckCircle className="h-3 w-3 mr-1" />
              {template.defaultSettings.complianceLevel === 'maximum' ? t.maxCompliance : t.highCompliance}
            </Badge>
          </div>
          ...
          {isExpanded && (
            <div className="space-y-4 pt-4 border-t">
              <div>
                <h4 className={`font-semibold mb-2 ${isRTL ? 'text-right font-arabic' : ''}`}>
                  {t.features}
                </h4>
                <ul className={`text-sm text-gray-600 space-y-1 ${isRTL ? 'text-right font-arabic' : ''}`}>
                  {(locale === 'ar' ? template.featuresAr : template.features).map((feature, index) => (
              ...
              <div>
                ...
                <ul className={`text-sm text-gray-600 space-y-1 ${isRTL ? 'text-right font-arabic' : ''}`}>
                  {(locale === 'ar' ? template.culturalRequirementsAr : template.culturalRequirements).map((req, index) => (
                    <li key={index} className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              <div>
                ...
                <div className={`grid grid-cols-1 gap-2 ${isRTL ? 'text-right' : ''}`}>
                  {template.specializations.map((spec) => (
                    <div key={spec.id} className="bg-gray-50 p-3 rounded-lg">
                      <h5 className={`font-medium ${isRTL ? 'font-arabic' : ''}`}>
                        {locale === 'ar' ? spec.nameAr : spec.name}
                      </h5>
                      <p className={`text-sm text-gray-600 mt-1 ${isRTL ? 'font-arabic' : ''}`}>
                        {locale === 'ar' ? spec.descriptionAr : spec.description}
                      </p>
                    </div>
                  ))}
                </div>
  ...
  return (
    <div className={`max-w-6xl mx-auto p-6 ${isRTL ? 'font-arabic' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
      <div className={`text-center mb-8 ${isRTL ? 'text-right' : ''}`}>
        <h1 className={`text-3xl font-bold mb-2 ${isRTL ? 'font-arabic' : ''}`}>{t.title}</h1>
        <p className={`text-gray-600 ${isRTL ? 'font-arabic' : ''}`}>{t.subtitle}</p>
      </div>
      ...
      <div className={`flex flex-wrap gap-2 mb-6 ${isRTL ? 'justify-end' : ''}`}>
        {[
          ...
          { key: 'educational', label: t.filterEducational, labelAr: t.filterEducational },
          { key: 'business', label: t.filterBusiness, labelAr: t.filterBusiness },
          { key: 'engineering', label: t.filterEngineering, labelAr: t.filterEngineering },
        ].map((filter) => (
          <Button
            key={filter.key}
