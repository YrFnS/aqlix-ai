...
interface WorkspaceOrganizerProps {
  ...
  onWorkspaceAction: (
    workspaceId: string,
    action: "star" | "unstar" | "archive" | "unarchive" | "settings",
  ) => void;
...
export default function ArabicWorkspaceOrganizer({
  ...
  const [sortBy, setSortBy] = useState<
    "name" | "activity" | "members" | "compliance"
  >("activity");
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("desc");
  const [showArchivedWorkspaces, setShowArchivedWorkspaces] =
    useState(showArchived);
  const [categories, setCategories] =
    useState<OrganizationCategory[]>(DEFAULT_CATEGORIES);
  ...
  const updateCategoryCounts = () => {
    ...
    const updatedCategories = categories.map((cat) => {
      ...
      switch (cat.id) {
        case "recent":
          count = filteredWorkspaces.filter(
            (w) => w.lastActivity > recentThreshold,
          ).length;
          break;
        case "starred":
          ...
          break;
        case "professional":
          count = filteredWorkspaces.filter((w) =>
            ["legal", "medical", "educational"].includes(w.type),
          ).length;
          break;
        case "business":
          count = filteredWorkspaces.filter((w) =>
            ["business", "engineering"].includes(w.type),
          ).length;
          break;
        case "personal":
          count = filteredWorkspaces.filter(
        ...
        case "shared":
          count = filteredWorkspaces.filter(
            (w) => w.visibility !== "private" && w.members > 1,
          ).length;
          break;
        default:
  ...
  const formatTimeAgo = (date: Date): string => {
    ...
    if (minutes < 60)
      return `${minutes} ${minutes === 1 ? t.timeAgo.minute : t.timeAgo.minutes}`;
    if (hours < 24)
      return `${hours} ${hours === 1 ? t.timeAgo.hour : t.timeAgo.hours}`;
    if (days < 7)
      return `${days} ${days === 1 ? t.timeAgo.day : t.timeAgo.days}`;
    if (weeks < 4)
      return `${weeks} ${weeks === 1 ? t.timeAgo.week : t.timeAgo.weeks}`;
    if (months < 12)
      return `${months} ${months === 1 ? t.timeAgo.month : t.timeAgo.months}`;
      ...
      <CardHeader className="pb-2">
        ...
        >
          <div className={`flex-1 ${isRTL ? "text-right" : ""}`}>
            <CardTitle
              className={`text-lg leading-tight ${isRTL ? "font-arabic" : ""}`}
            >
              {locale === "ar" ? workspace.nameAr : workspace.name}
            </CardTitle>
            {workspace.description && (
              <p
                className={`text-sm text-gray-600 mt-1 line-clamp-2 ${isRTL ? "font-arabic" : ""}`}
              >
                {locale === "ar"
              ...
              >
                <DropdownMenuItem
                  onClick={(e) => {
                    ...
                    onWorkspaceAction(
                      workspace.id,
                      workspace.isStarred ? "unstar" : "star",
                    );
                  }}
                >
      ...
      <CardContent className="pt-2">
        <div className="space-y-3">
          ...
          >
            ...
            <Badge
              variant="outline"
              className={`flex items-center gap-1 ${isRTL ? "flex-row-reverse font-arabic" : ""}`}
            >
          ...
          {/* Stats */}
          <div
            className={`flex items-center justify-between text-sm text-gray-600 ${isRTL ? "flex-row-reverse" : ""}`}
          >
            <div
          ...
          {/* Owner */}
          <div
            className={`text-xs text-gray-500 ${isRTL ? "text-right font-arabic" : ""}`}
          >
            {t.owner}:{" "}
            {locale === "ar" ? workspace.owner.nameAr : workspace.owner.name}
          </div>
        </div>
      </CardContent>
    </Card>
        ...
        >
          ...
          >
            ...
            <div className={`flex-1 min-w-0 ${isRTL ? "text-right" : ""}`}>
              <h3
                className={`font-semibold truncate ${isRTL ? "font-arabic" : ""}`}
              >
                {locale === "ar" ? workspace.nameAr : workspace.name}
              </h3>
              <p
                className={`text-sm text-gray-600 truncate ${isRTL ? "font-arabic" : ""}`}
              >
                {locale === "ar"
                  ? workspace.descriptionAr
                  : workspace.description}
          ...
          >
            ...
            <div
              className={`text-sm text-gray-600 ${isRTL ? "text-right font-arabic" : ""}`}
            >
              {workspace.members}/{workspace.maxMembers} {t.members}
            </div>

            <div
              className={`text-sm text-gray-600 w-20 ${isRTL ? "text-right font-arabic" : ""}`}
            >
              {formatTimeAgo(workspace.lastActivity)}
            </div>
  ...
  return (
    <div
      className={`max-w-7xl mx-auto p-6 ${isRTL ? "font-arabic" : ""}`}
      dir={isRTL ? "rtl" : "ltr"}
    >
      {/* Header */}
      <div
      ...
      {/* Search and Controls */}
      <div
        className={`flex flex-col sm:flex-row gap-4 mb-6 ${isRTL ? "sm:flex-row-reverse" : ""}`}
      >
        <div className={`relative flex-1 ${isRTL ? "text-right" : ""}`}>
          <Search
            className={`absolute top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400 ${isRTL ? "right-3" : "left-3"}`}
          />
          <Input
        ...
        >
          ...
          >
            <SelectTrigger
              className={`w-40 ${isRTL ? "text-right font-arabic" : ""}`}
            >
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
