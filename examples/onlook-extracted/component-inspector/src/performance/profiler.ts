...
export class PerformanceProfiler extends EventEmitter {
  ...
  private async analyzeRTLPerformance(
    dom: DOMElementInfo | null,
  ): Promise<RTLMetrics> {
    if (!dom) {
  ...
  private async analyzeArabicFontPerformance(
    dom: DOMElementInfo | null,
  ): Promise<ArabicFontMetrics> {
    if (!dom) {
  ...
  private analyzeBundleSize(
    ...
    if (size === 0) {
      ...
      const culturalDeps = componentInfo.dependencies.filter(
        (dep) => dep.culturalRelevance,
      );
  ...
  private hasRTLOptimizations(dom: DOMElementInfo): boolean {
    ...
    return (
      className.includes("rtl-optimized") ||
  ...
  private hasHeavyContent(dom: DOMElementInfo): boolean {
    return (
      this.hasImages(dom) || this.hasVideos(dom) || this.hasComplexSVG(dom)
    );
  }
  ...
  private hasImages(dom: DOMElementInfo): boolean {
    return (
      dom.tagName === "img" ||
      dom.children.some((child) => this.hasImages(child))
    );
  }
  ...
  private hasVideos(dom: DOMElementInfo): boolean {
    return (
      dom.tagName === "video" ||
      dom.children.some((child) => this.hasVideos(child))
    );
  }
  ...
  private hasHeavyOperations(dom: DOMElementInfo): boolean {
    ...
    return (
      heavyClasses.some((heavy) => className.includes(heavy)) ||
  ...
  private measureCurrentMemoryUsage(): number {
    if (typeof window !== "undefined" && (window as any).performance?.memory) {
      return Math.round(
        (window as any).performance.memory.usedJSHeapSize / 1024,
      ); // KB
    }
