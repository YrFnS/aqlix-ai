...
export class ComponentParser {
  ...
  private async parseContent(content: string, filePath: string, ext: string) {
    ...
    if (['.ts', '.tsx', '.js', '.jsx'].includes(ext)) {
      ast = this.parseJavaScriptAST(content, ext.includes('tsx') || ext.includes('jsx'));
  ...
  private extractDOMFromContent(content: string, ext: string): DOMElementInfo | null {
    try {
      if (ext === '.html') {
  ...
  private extractDOMInfo(element: cheerio.Cheerio, $: cheerio.CheerioAPI): DOMElementInfo {
  ...
  private extractComputedStyles(element: cheerio.Cheerio): any {
    ...
    if (className) {
      ...
      if (className.includes('font-arabic') || className.includes('arabic-font')) {
  ...
  private analyzeCulturalMetadata(element: cheerio.Cheerio, attributes: Record<string, string>): any {
    ...
    if (ast && ast.type === 'File' && ast.program) {
      ...
      traverse(ast, {
        FunctionDeclaration(path) {
          if (path.node.id?.name && this.isComponentFunction(path.node.id.name)) {
            componentName = path.node.id.name;
          }
        },
        VariableDeclarator(path) {
          if (t.isIdentifier(path.node.id) && this.isComponentFunction(path.node.id.name)) {
            componentName = path.node.id.name;
          }
        },
  ...
  private detectComponentType(ast?: ASTNode | null, domInfo?: DOMElementInfo | null): ComponentType {
  ...
  private hasPageStructure(domInfo: DOMElementInfo): boolean {
    ...
    return pageIndicators.includes(domInfo.tagName) || 
  ...
  private hasLayoutStructure(domInfo: DOMElementInfo): boolean {
    ...
    return domInfo.children.some(child => layoutIndicators.includes(child.tagName));
  }

  private detectFramework(ext: string, filePath: string): 'react' | 'vue' | 'angular' | 'svelte' {
    if (ext === '.vue') return 'vue';
    if (ext === '.svelte') return 'svelte';
    if (filePath.includes('angular') || filePath.includes('.component.ts')) return 'angular';
  ...
  private checkIslamicCompliance(text: string): boolean {
    ...
    const islamicTerms = ['الله', 'محمد', 'القرآن', 'الإسلام', 'المسجد', 'الصلاة'];
    const inappropriateTerms = ['خمر', 'قمار', 'ربا'];
    
    const hasIslamicTerms = islamicTerms.some(term => text.includes(term));
    const hasInappropriateTerms = inappropriateTerms.some(term => text.includes(term));
  ...
  private checkGovernmentRelevance(text: string): boolean {
    const governmentTerms = [
      'وزارة', 'حكومة', 'دولة', 'مواطن', 'خدمات', 
      'هوية', 'جواز', 'رخصة', 'شهادة'
    ];
