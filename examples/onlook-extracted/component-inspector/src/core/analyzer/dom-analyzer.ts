...
export class DOMAnalyzer {
  ...
  private hasArabicFontSupport(dom: DOMElementInfo): boolean {
    ...
    const arabicFonts = ['Noto Sans Arabic', 'Cairo', 'Amiri', 'Tahoma', 'Arial Unicode MS'];
  ...
  private hasArabicFontClasses(dom: DOMElementInfo): boolean {
    const className = dom.attributes.class || '';
    return className.includes('font-arabic') || 
  ...
  private hasRTLAttributes(element: DOMElementInfo): boolean {
    return element.attributes.dir === 'rtl' || 
           element.styles.direction === 'rtl';
  }
  
  private hasAccessibilityAttributes(element: DOMElementInfo): boolean {
    const a11yAttrs = ['role', 'aria-label', 'aria-describedby', 'alt', 'title'];
  ...
  private checkWCAGCompliance(dom: DOMElementInfo): AccessibilityViolation[] {
    ...
    if (['input', 'textarea', 'select'].includes(dom.tagName) && 
        !dom.attributes['aria-label'] && !dom.attributes['aria-labelledby']) {
      violations.push({
        rule: 'WCAG 3.3.2',
        impact: 'serious',
    ...
    if (dom.styles.color && dom.styles.backgroundColor) {
      const contrast = this.calculateColorContrast(dom.styles.color, dom.styles.backgroundColor);
      if (contrast < 4.5) {
        violations.push({
          rule: 'WCAG 1.4.3',
          impact: 'serious',
  ...
  private checkGovernmentAccessibility(dom: DOMElementInfo): AccessibilityViolation[] {
    const violations: AccessibilityViolation[] = [];
    ...
    if (this.isFormElement(dom)) {
      ...
      if (!this.hasErrorHandling(dom)) {
        violations.push({
          rule: 'GOV-2',
          impact: 'moderate',
          description: 'Form field lacks error handling for government compliance',
          element: dom.tagName,
  ...
  private isGovernmentForm(dom: DOMElementInfo): boolean {
    ...
    return className.includes('government') || 
  ...
  private isMinistryHeader(dom: DOMElementInfo): boolean {
    ...
    const className = dom.attributes.class || '';
    return className.includes('ministry') || 
  ...
  private isPrayerTimeNotice(dom: DOMElementInfo): boolean {
    ...
    return className.includes('prayer') || 
  ...
  private hasGovernmentFormFields(dom: DOMElementInfo): boolean {
    const govFieldNames = ['nationalId', 'passportNo', 'residencyCard', 'birthCertificate'];
    return this.findElementsByAttribute(dom, 'name', govFieldNames).length > 0;
  }
  
  private hasMinistryLogo(dom: DOMElementInfo): boolean {
    return this.findElementsByAttribute(dom, 'alt', ['logo', 'ministry', 'government']).length > 0 ||
           this.findElementsByClassName(dom, ['logo', 'ministry-logo', 'gov-logo']).length > 0;
  }
  ...
  private hasNavigationStructure(dom: DOMElementInfo): boolean {
    return this.findElementsByTagName(dom, 'nav').length > 0 ||
           this.findElementsByClassName(dom, ['nav', 'navigation', 'menu']).length > 0;
  }
  
  private hasErrorHandling(dom: DOMElementInfo): boolean {
    return this.findElementsByAttribute(dom, 'aria-invalid', ['true']).length > 0 ||
           this.findElementsByClassName(dom, ['error', 'invalid', 'validation']).length > 0;
  }
  
  // DOM traversal utilities
  
  private findElementsByTagName(dom: DOMElementInfo, tagName: string): DOMElementInfo[] {
  ...
  private findElementsByClassName(dom: DOMElementInfo, classNames: string[]): DOMElementInfo[] {
    const results: DOMElementInfo[] = [];
    const className = dom.attributes.class || '';
  ...
  private hasTextMatching(dom: DOMElementInfo, regex: RegExp): boolean {
    ...
    return regex.test(searchText) || dom.children.some(child => this.hasTextMatching(child, regex));
  }
  
  private isFormElement(dom: DOMElementInfo): boolean {
    return ['input', 'textarea', 'select', 'button'].includes(dom.tagName.toLowerCase());
  }
  ...
  private isRequiredField(dom: DOMElementInfo): boolean {
    ...
    return requiredFields.some(field => name.includes(field) || id.includes(field));
  }
  ...
  private hasComplexLayout(dom: DOMElementInfo): boolean {
    return dom.children.length > 10 || 
           Object.keys(dom.styles).length > 15;
  }
  
  private hasDynamicContent(dom: DOMElementInfo): boolean {
    const className = dom.attributes.class || '';
    return className.includes('dynamic') || 
  ...
  private calculateColorContrast(color: string, backgroundColor: string): number {
