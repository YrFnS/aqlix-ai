...
const DIALECT_MARKERS: DialectMarkers = {
  iraqi: [
    'شلونك', 'شكو ماكو', 'بسة', 'دازة', 'كلش', 'لية', 'عدكم', 'وية', 'يالة', 'شوف',
    'هسة', 'بعدين', 'كمان', 'خوش', 'زين', 'مال', 'داخل', 'طالع', 'جاي', 'رايح'
  ],
  gulf: [
    'شلونك', 'وايد', 'خوش', 'يالله', 'شصاير', 'يسلموا', 'تكفى', 'خلاص', 'زين',
    'حيل', 'مدري', 'اي والله', 'صج', 'لا والله', 'ماشي', 'عادي', 'طيب'
  ],
  msa: [
    'كيف حالك', 'ما أخبارك', 'كثيراً', 'ممتاز', 'جيد جداً', 'شكراً جزيلاً',
    'على الرحب والسعة', 'أهلاً وسهلاً', 'مع السلامة', 'إلى اللقاء', 'بالطبع'
  ],
  levantine: [
    'كيفك', 'شو اخبارك', 'كتير', 'منيح', 'حلو', 'يلا', 'خلص', 'بدك', 'عم بحكي',
    'شو رايك', 'ما بعرف', 'أكيد', 'مبسوط', 'زهقان', 'تعبان'
  ]
};
...
export function detectArabicDialect(text: string): {
  ...
  if (!containsArabic(text)) {
    return { dialect: null, confidence: 0, scores: { iraqi: 0, gulf: 0, msa: 0, levantine: 0 } };
  }
  ...
  const dialect = Object.entries(scores).find(([_, score]) => score === maxScore)?.[0] as keyof DialectMarkers;
...
export function formatArabicText(text: string, options?: {
  ...
  let formatted = text;
  const opts = { preserveWhitespace: false, normalizeNumbers: true, addBidiMarks: true, ...options };
  ...
  if (opts.normalizeNumbers) {
    ...
    for (let i = 0; i < 10; i++) {
      formatted = formatted.replace(new RegExp(arabicDigits[i], 'g'), westernDigits[i]);
    }
  }
 ...
 */
export function validateArabicText(text: string, context?: {
  ...
  if (ctx.islamicCompliance) {
    const islamicPhrases = ['بسم الله', 'إن شاء الله', 'الحمد لله', 'اللهم', 'صلى الله عليه وسلم'];
  ...
  if (ctx.professionalDomain === 'legal') {
    if (!text.includes('قانون') && !text.includes('حق') && !text.includes('محكمة')) {
      suggestions.push('Consider adding legal terminology for professional legal context');
    }
  } else if (ctx.professionalDomain === 'medical') {
    if (!text.includes('طب') && !text.includes('صحة') && !text.includes('مرض')) {
      suggestions.push('Consider adding medical terminology for professional medical context');
    }
  }
...
export function generateArabicSuggestions(
  ...
  if (dialectInfo.dialect !== dialect) {
    const dialectName = DIALECT_MARKERS[dialect] ? 
      (dialect === 'iraqi' ? 'العراقية' : 
       dialect === 'gulf' ? 'الخليجية' : 
       dialect === 'msa' ? 'الفصحى' : 'الشامية') : 'المحلية';
...
export function arabicNumeralsToText(num: number): string {
  ...
  if (num >= 11 && num <= 19) {
    const special = ['', 'أحد عشر', 'اثنا عشر', 'ثلاثة عشر', 'أربعة عشر', 'خمسة عشر', 
