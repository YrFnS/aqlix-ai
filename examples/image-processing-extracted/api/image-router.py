...
class ImageGenerationRequest(BaseModel):
    ...
    negative_prompt_ar: Optional[str] = Field(None, description="Arabic negative prompt")
    
    provider: ImageProvider = Field(default=ImageProvider.DALLE3, description="Image generation provider")
    model: Optional[str] = Field(default="dall-e-3", description="Specific model version")
    ...
    cultural_validation: bool = Field(default=True, description="Enable Iraqi cultural validation")
    islamic_compliance: bool = Field(default=True, description="Ensure Islamic compliance")
...
class ImageResponse(BaseModel):
    ...
    cultural_score: float = Field(..., ge=0, le=1, description="Cultural appropriateness score")
    islamic_compliant: bool = Field(..., description="Islamic compliance status")
    professional_appropriate: bool = Field(..., description="Professional context appropriate")
...
async def generate_images(request: ImageGenerationRequest) -> ImageListResponse:
    ...
    try:
        ...
        for img_data in generated_images:
            if request.cultural_validation:
                ...
                if not image_validation.is_valid:
                    logger.warning(f"Generated image failed validation: {image_validation.reason}")
                    continue
            
            # Create response object
            validated_images.append(ImageResponse(
                id=str(uuid.uuid4()),
                data=img_data.data,
                url=img_data.url,
                ...
                cultural_score=image_validation.cultural_score if request.cultural_validation else 1.0,
                islamic_compliant=image_validation.islamic_compliant if request.cultural_validation else True,
                professional_appropriate=image_validation.professional_appropriate if request.cultural_validation else True,
                generation_time=(datetime.utcnow() - start_time).total_seconds(),
                validation_time=image_validation.processing_time if request.cultural_validation else 0.0
            ))
        ...
        logger.info(f"Successfully generated {len(validated_images)} images for user {request.user_id}")
        ...
        for img_data in edited_images:
            if request.cultural_validation:
                ...
                if not edit_validation.is_valid:
                    logger.warning(f"Edited image failed validation: {edit_validation.reason}")
                    continue
            
            validated_edits.append(ImageResponse(
                id=str(uuid.uuid4()),
                data=img_data.data,
                url=img_data.url,
                provider=request.provider,
                ...
                size=request.size,
                format=img_data.format,
                cultural_score=edit_validation.cultural_score if request.cultural_validation else 1.0,
                islamic_compliant=edit_validation.islamic_compliant if request.cultural_validation else True,
                professional_appropriate=edit_validation.professional_appropriate if request.cultural_validation else True,
                generation_time=(datetime.utcnow() - start_time).total_seconds(),
                validation_time=edit_validation.processing_time if request.cultural_validation else 0.0
            ))
        
        if not validated_edits:
            raise HTTPException(
                status_code=400,
                detail="No edited images passed cultural validation."
            )
...
async def get_image_config() -> Dict[str, Any]:
    ...
    try:
        config = {
            ...
            "default_sizes": ["256x256", "512x512", "1024x1024", "1792x1024", "1024x1792"],
