...
class ArabicRTLProcessor:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.enable_cultural_optimization = self.config.get(
            "enable_cultural_optimization", True
        )
        self.professional_domain_support = self.config.get(
            "professional_domain_support", True
        )

        # Initialize agents and utilities
        self.rtl_agent = ArabicRTLProcessorClient(agent_type="arabic-rtl-processor")
    ...
    async def process_prompts(
        ...
        try:
            ...
            text_direction = self._determine_text_direction(
                primary_analysis, mixed_language
            )
            ...
            if processing_data.get("dialect_detected"):
                ...
                detected_dialect = (
                    ArabicDialect(dialect_name)
                    if dialect_name in [d.value for d in ArabicDialect]
                    else None
                )
            ...
            if self.enable_cultural_optimization:
                ...
                if professional_domain != "general":
                    ...
                    if domain_enhancements:
                        domain_enhanced_prompt = processing_data.get(
                            "domain_enhanced_prompt", optimized_prompt
                        )
    ...
    def _determine_text_direction(
        self, analysis: Optional[TextAnalysis], mixed_language: bool
    ) -> TextDirection:
    ...
    async def _apply_domain_enhancements(
        self, prompt: str, domain: str, dialect: Optional[ArabicDialect]
    ) -> List[str]:
    ...
    async def _fallback_processing(
        ...
        try:
            ...
            if not analysis:
                ...
                text_direction = (
                    TextDirection.RTL if analysis.is_arabic else TextDirection.LTR
                )
