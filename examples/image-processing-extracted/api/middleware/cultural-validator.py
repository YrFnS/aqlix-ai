...
class IraqiCulturalValidator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.strict_professional_validation = self.config.get(
            "strict_professional_validation", True
        )
    ...
    async def validate_prompt(
        ...
        try:
            ...
            if not cultural_validation.get("success", False):
                logger.error(
                    f"Cultural validation failed: {cultural_validation.get('error')}"
                )
                return ValidationResult(
                    is_valid=False,
                    cultural_score=0.0,
                    ...
                    professional_appropriate=False,
                    severity=ValidationSeverity.BLOCKED,
                    reason=cultural_validation.get(
                        "error", "Cultural validation service unavailable"
                    ),
            ...
            if self.arabic_processor.contains_arabic(prompt):
                ...
                arabic_analysis = await self.arabic_processor.analyze_text(
                    text=prompt, detect_dialect=True, cultural_context="iraqi"
                )
            ...
            is_valid = (
                cultural_score >= self.min_cultural_score
                and (not self.require_islamic_compliance or islamic_compliant)
                and (
            ...
            if cultural_score < self.min_cultural_score:
                recommendations.extend(
                    validation_data.get("cultural_recommendations", [])
                )

            if not islamic_compliant and self.require_islamic_compliance:
                recommendations.extend(
                    validation_data.get("islamic_recommendations", [])
                )

            if not professional_appropriate:
                recommendations.append(
                    f"Ensure content is appropriate for {domain} professional context"
                )
            ...
            if not optimized_prompt and validation_data.get("suggested_improvements"):
                optimized_prompt = validation_data["suggested_improvements"].get(
                    "improved_prompt"
                )
            ...
            logger.info(
                f"Prompt validation completed: score={cultural_score:.3f}, valid={is_valid}"
            )
            return result
    ...
    async def validate_arabic_prompt(
        self, prompt: str, domain: str = "general"
    ) -> ValidationResult:
        """
        ...
        try:
            ...
            result = ValidationResult(
                ...
                islamic_compliant=validation_data.get("islamic_compliant", False),
                professional_appropriate=validation_data.get(
                    "professional_appropriate", False
                ),
                severity=ValidationSeverity.APPROVED
                if validation_data.get("is_valid")
                else ValidationSeverity.BLOCKED,
                reason=validation_data.get(
                    "validation_reason", "Arabic validation completed"
                ),
    ...
    async def validate_image_content(
        ...
        try:
            logger.info(
                f"Validating image content for user {user_id} in domain {domain}"
            )
            ...
            professional_appropriate = analysis_data.get(
                "professional_appropriate", False
            )
            ...
            if policy_violations:
                recommendations.extend(
                    [f"Address: {violation}" for violation in policy_violations]
                )

            if cultural_score < self.min_cultural_score:
                recommendations.extend(
                    analysis_data.get("cultural_recommendations", [])
                )
            ...
            logger.info(
                f"Image validation completed: score={cultural_score:.3f}, valid={is_valid}"
            )
            return result
    ...
    async def validate_generated_image(
        ...
        try:
            ...
            base_validation = await self.validate_image_content(
                image_data=image_data, domain=domain, user_id=user_id
            )
            ...
            alignment_analysis = (
                await self.cultural_agent.validate_prompt_image_alignment(
                    {
                        "image_data": image_data,
            ...
            return ImageValidationResult(
                ...
                and alignment_data.get("alignment_appropriate", True),
                ...
                recommendations=base_validation.recommendations
                + alignment_data.get("recommendations", []),
            )
    ...
    async def _validate_professional_context(
        self, prompt: str, domain: str, validation_data: Dict[str, Any]
    ) -> float:
