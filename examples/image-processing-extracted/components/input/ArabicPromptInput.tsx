...
export const ArabicPromptInput: React.FC<ArabicPromptInputProps> = ({
  ...
  // UI State
  const [showTranslationPanel, setShowTranslationPanel] = useState(showTranslation);
  const [showDialectPanel, setShowDialectPanel] = useState(false);
  const [showSuggestions, setShowSuggestions] = useState(false);
  const [isRecording, setIsRecording] = useState(false);
  const [isPlayingAudio, setIsPlayingAudio] = useState(false);
  const [copiedText, setCopiedText] = useState<'input' | 'translation' | null>(null);
  ...
  const detectDialect = useCallback(async (text: string) => {
    if (!enableDialectDetection || !text.trim()) return;

    // Simple dialect detection based on common phrases/words
    const iraqiMarkers = ['شلونك', 'شكو ماكو', 'بسة', 'دازة', 'كلش', 'لية', 'عدكم'];
  ...
  // Cultural validation
  const validateCulturalContent = useCallback(async (text: string) => {
    ...
    try {
      ...
      const islamicPhrases = ['بسم الله', 'إن شاء الله', 'الحمد لله', 'اللهم'];
  ...
  // Translate text
  const translateText = useCallback(async (text: string, fromDialect: string) => {
    if (!text.trim()) return;
  ...
  const handleInputChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    onChange?.(newValue, translation, { dialect: selectedDialect, culturalScore });
  ...
  }, [maxLength, translation, selectedDialect, culturalScore, onChange, detectLanguageDirection, detectDialect, validateCulturalContent, autoTranslate, translateText]);
  ...
  // Copy to clipboard
  const copyToClipboard = useCallback(async (text: string, type: 'input' | 'translation') => {
    try {
  ...
  const inputDirection = textDirection === 'auto' 
    ? (inputValue && /[\u0600-\u06FF]/.test(inputValue) ? 'rtl' : 'ltr')
    : textDirection;
        ...
        <div className={`flex items-center gap-1 ${flexDir}`}>
          ...
          {showTranslation && (
            <button
              ...
              title={isRtlMode ? 'إظهار/إخفاء الترجمة' : 'Show/hide translation'}
            >
              {showTranslationPanel ? <Eye className="w-3 h-3" /> : <EyeOff className="w-3 h-3" />}
            </button>
          )}
        </div>
      </div>
      ...
      {showDialectPanel && (
        <div className="mb-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
          ...
          {dialectConfigs[selectedDialect]?.examples && (
            <div className="mt-4">
              <h5 className={`text-xs font-medium text-gray-700 mb-2 ${textAlign}`}>
                {isRtlMode ? 'أمثلة' : 'Examples'}:
              </h5>
              <div className="text-xs text-gray-600 space-y-1">
                {dialectConfigs[selectedDialect].examples.slice(0, 3).map((example, idx) => (
                  <div key={idx} className={`${textAlign}`}>{example}</div>
                ))}
              </div>
            </div>
          )}
        ...
        <div className="space-y-3">
            ...
            <div className={`flex items-center gap-1 ${flexDir}`}>
              {enableCulturalValidation && (
                <div className={`flex items-center gap-1 text-xs ${
                  culturalScore >= 0.8 ? 'text-green-600' : culturalScore >= 0.6 ? 'text-yellow-600' : 'text-red-600'
                }`}>
              ...
              >
                {copiedText === 'input' ? <Check className="w-3 h-3 text-green-600" /> : <Copy className="w-3 h-3" />}
              </button>
          ...
          <div className="relative">
            <textarea
              ...
              placeholder={inputDirection === 'rtl' ? placeholderAr : placeholder}
              disabled={disabled}
              readOnly={readOnly}
              className={`w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-50 disabled:cursor-not-allowed resize-none transition-colors ${
                inputDirection === 'rtl' ? 'text-right font-arabic' : 'text-left'
          ...
          {dialectConfigs[selectedDialect]?.commonPhrases && (
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1">
                {dialectConfigs[selectedDialect].commonPhrases.slice(0, 4).map((phrase, idx) => (
                  <button
                    key={idx}
                    onClick={() => {
                      const newValue = inputValue ? `${inputValue} ${phrase.ar}` : phrase.ar;
                      setInputValue(newValue);
                      onChange?.(newValue, translation, { dialect: selectedDialect, culturalScore });
                    }}
                    disabled={disabled || readOnly}
          ...
          <div className="space-y-3">
            ...
            <div className={`flex items-center justify-between ${flexDir}`}>
              ...
              <div className={`flex items-center gap-1 ${flexDir}`}>
                ...
                <button
                  onClick={() => inputValue && translateText(inputValue, selectedDialect)}
                ...
                >
                  {copiedText === 'translation' ? <Check className="w-3 h-3 text-green-600" /> : <Copy className="w-3 h-3" />}
                </button>
              </div>
            </div>
            ...
            {translationResult?.suggestions && translationResult.suggestions.length > 0 && (
              <div className="space-y-2">
                <h5 className={`text-xs font-medium text-gray-700 ${textAlign}`}>
                  {isRtlMode ? 'اقتراحات للتحسين' : 'Suggestions for improvement'}:
                </h5>
                <ul className="space-y-1">
                  {translationResult.suggestions.map((suggestion, idx) => (
                    <li key={idx} className={`text-xs text-gray-600 flex items-start gap-1 ${flexDir}`}>
          ...
          <div className={`flex items-start gap-2 ${flexDir}`}>
            ...
            <div className="text-xs text-green-700">
              <div className={`font-medium ${textAlign}`}>
                {isRtlMode ? 'التحقق الثقافي مفعل' : 'Cultural validation enabled'}
              </div>
