...
import { Download, Eye, EyeOff, RotateCcw, Share2, AlertTriangle, CheckCircle, Info } from 'lucide-react';
...
export const ImageDisplay: React.FC<ImageDisplayProps> = ({
  ...
  const arabicPromptDirection = originalPromptAr ? getTextDirection(originalPromptAr) : 'ltr';
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const domain = professionalDomain !== 'general' ? `_${professionalDomain}` : '';
  ...
  const handleShare = async () => {
    ...
    try {
      if (navigator.share && navigator.canShare) {
        const displayUrl = imageUrl || `data:image/png;base64,${imageData}`;
        const shareTitle = originalPromptAr || originalPrompt || 'صورة من النظام العراقي للذكاء الاصطناعي';
        ...
        {showMetadata && (
          <div className={cn(
            ...
            <div className="space-y-1">
              {/* Original Prompt */}
              <div className={cn('text-sm text-white/90', promptDirection === 'rtl' && 'text-right')}>
              ...
              <div className="flex items-center gap-2 text-xs text-white/80">
                <span>{isRtlMode ? 'النقاط الثقافية:' : 'Cultural Score:'}</span>
                <span className="font-medium">{(culturalScore * 100).toFixed(1)}%</span>
                {islamicCompliant ? (
              ...
              >
                {showPromptDetails 
                  ? (isRtlMode ? 'إخفاء التفاصيل' : 'Hide Details')
                  : (isRtlMode ? 'عرض التفاصيل' : 'Show Details')
                }
              </button>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="space-y-2">
                {originalPrompt && (
                  <div className={cn('p-2 bg-white rounded border', promptDirection === 'rtl' && 'text-right')}>
                ...
                {originalPromptAr && originalPromptAr !== originalPrompt && (
                  <div className="p-2 bg-white rounded border text-right">
                    <span className="text-xs text-gray-500 uppercase">عربي</span>
            ...
            {(revisedPrompt || revisedPromptAr) && (
              <div>
                ...
                <div className="space-y-2">
                  {revisedPrompt && (
                    <div className={cn('p-2 bg-blue-50 rounded border', promptDirection === 'rtl' && 'text-right')}>
                      <span className="text-xs text-gray-500 uppercase">
                        {getTextDirection(revisedPrompt) === 'rtl' ? 'عربي' : 'English'}
                      </span>
                      <p>{revisedPrompt}</p>
                    </div>
                  )}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 text-xs">
                <div className="space-y-1">
                  ...
                  <div className="flex justify-between">
                    <span>{isRtlMode ? 'الامتثال الإسلامي:' : 'Islamic Compliant:'}</span>
                    <span className={islamicCompliant ? 'text-green-600' : 'text-red-600'}>
                      {islamicCompliant 
                        ? (isRtlMode ? 'نعم' : 'Yes')
                        : (isRtlMode ? 'لا' : 'No')
                      }
                ...
                <div className="space-y-1">
                  <div className="flex justify-between">
                    <span>{isRtlMode ? 'السياق المهني:' : 'Professional Domain:'}</span>
                    <span className="font-medium capitalize">{professionalDomain}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{isRtlMode ? 'مناسب مهنياً:' : 'Professionally Appropriate:'}</span>
                    <span className={professionalAppropriate ? 'text-green-600' : 'text-red-600'}>
              ...
              <div className={cn(
                ...
                <div className="max-w-2xl mx-auto space-y-2">
                  ...
                  <div className="flex items-center gap-4 text-xs text-white/80">
                    <span>
                      {isRtlMode ? 'النقاط الثقافية:' : 'Cultural Score:'} {(culturalScore * 100).toFixed(1)}%
                    </span>
                    <span>
                    ...
                    {islamicCompliant && (
                      <span className="text-green-400">
                        {isRtlMode ? '✓ متوافق إسلامياً' : '✓ Islamic Compliant'}
                      </span>
                    )}
                  </div>
                </div>
