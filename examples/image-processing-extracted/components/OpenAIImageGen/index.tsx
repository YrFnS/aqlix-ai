...
export const OpenAIImageGen: React.FC<OpenAIImageGenProps> = ({
  ...
  const handleConfigUpdate = useCallback((field: keyof OpenAIConfig, value: any) => {
    const updatedConfig = {
      ...openaiConfig,
      [field]: value
    };
  ...
  const verifyConnection = async (configToTest: OpenAIConfig = openaiConfig) => {
    ...
    try {
      ...
      const response = await fetch(`${configToTest.endpoint}/models`, {
        headers: {
          ...
          ...(configToTest.organization && { 'OpenAI-Organization': configToTest.organization })
        }
      });
      ...
      if (response.ok) {
        ...
        if (hasImageModels) {
          ...
          throw new Error(isRtlMode ? 'نماذج الصور غير متاحة' : 'Image models not available');
        }
      } else {
  ...
  // Usage Tracking
  const trackUsage = useCallback((model: string, size: string, count: number) => {
    ...
    <div className={`openai-image-gen ${rtlClass} ${className}`}>
      ...
      <div className={`flex items-center justify-between mb-6 ${flexDir}`}>
        <div className={`flex items-center gap-3 ${flexDir}`}>
          ...
          <div className={`flex items-center gap-2 px-3 py-1 rounded-full text-xs ${
            ...
            {connectionStatus === 'error' && <AlertTriangle className="w-3 h-3" />}
            {connectionStatus === 'connecting' && (
              <div className="animate-spin rounded-full h-3 w-3 border-b border-current" />
            )}
            <span>
              {connectionStatus === 'connected' && (isRtlMode ? 'متصل' : 'Connected')}
              {connectionStatus === 'error' && (isRtlMode ? 'خطأ' : 'Error')}
              {connectionStatus === 'connecting' && (isRtlMode ? 'اتصال...' : 'Connecting...')}
              {connectionStatus === 'idle' && (isRtlMode ? 'غير متصل' : 'Not Connected')}
            </span>
          </div>
        </div>
          ...
          {showUsageStats && (
            <div className="text-xs text-gray-600 space-y-1">
              <div className={textAlign}>
                ...
                <span className="ml-1 font-medium">${usageStats.remainingCredits.toFixed(2)}</span>
              </div>
            </div>
          )}
        ...
        <div className="mb-6 p-6 bg-gray-50 border border-gray-200 rounded-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleConfigUpdate('organization', e.target.value)}
                placeholder="org-..."
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex gap-3">
                {['dall-e-2', 'dall-e-3'].map((model) => (
                  <label key={model} className={`flex items-center gap-2 ${flexDir}`}>
                    <input
                      type="checkbox"
                      ...
                      onChange={(e) => {
                        const newModels = e.target.checked
                          ...
                          : openaiConfig.enabledModels.filter(m => m !== model);
                        handleConfigUpdate('enabledModels', newModels);
                      }}
                      className="rounded"
                    />
                    <span className="text-sm text-gray-700">{model.toUpperCase()}</span>
                  </label>
                ))}
              </div>
            </div>
          ...
          {lastError && (
            <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
              ...
              <p className={`mt-1 text-xs text-red-600 ${textAlign}`}>{lastError}</p>
            </div>
          )}
      ...
      {showUsageStats && isConfigured && (
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <h4 className={`text-sm font-semibold text-blue-800 mb-3 ${textAlign}`}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
            ...
            <div className={textAlign}>
              ...
              <span className="font-medium">${usageStats.sessionCredits.toFixed(3)}</span>
            </div>
            <div className={textAlign}>
              <div className="flex items-center gap-1 text-blue-700">
              ...
              <span className="font-medium">
                {usageStats.rateLimitStatus['dall-e-2'].current}/{usageStats.rateLimitStatus['dall-e-2'].limit}
              </span>
            </div>
            <div className={textAlign}>
              <div className="flex items-center gap-1 text-blue-700">
              ...
              <span className="font-medium">
                {usageStats.rateLimitStatus['dall-e-3'].current}/{usageStats.rateLimitStatus['dall-e-3'].limit}
              </span>
            </div>
          </div>
          
          {usageStats.remainingCredits <= 1.0 && (
            <div className={`mt-3 flex items-center gap-2 text-xs text-orange-700 ${flexDir}`}>
      ...
      {culturalValidationRequired && isConfigured && (
        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className={`flex items-start gap-3 ${flexDir}`}>
            ...
            <div>
              <h4 className={`text-sm font-semibold text-green-800 mb-1 ${textAlign}`}>
