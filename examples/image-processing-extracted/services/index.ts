...
export async function performSystemHealthCheck(): Promise<{
  ...
  try {
    ...
    results.service_status.agentCoordinator =
      coordinatorStatus.success_rate >= 0.9;
    results.performance_metrics.cultural_validation_accuracy =
      coordinatorStatus.success_rate;
    ...
    results.performance_metrics.arabic_processing_accuracy =
      arabicStats.dialect_detection_accuracy;

    // Check payment security
    const paymentMetrics = paymentSecurity.getSecurityMetrics();
    results.service_status.paymentSecurity =
      paymentMetrics.success_rate >= 0.95;
    results.performance_metrics.payment_security_score =
    ...
    results.performance_metrics.agent_communication_health =
      commHealth.communication_metrics.success_rate;

    // Check workflow orchestrator
    const workflowMetrics = workflowOrchestrator.getOrchestrationMetrics();
    results.service_status.workflowOrchestrator =
      workflowMetrics.success_rate >= 0.9;
    results.performance_metrics.workflow_success_rate =
      workflowMetrics.success_rate;

    // Determine overall health
    results.overall_healthy = Object.values(results.service_status).every(
      (status) => status,
    );
    ...
    if (!results.service_status.agentCoordinator) {
      results.recommendations.push(
        "Agent coordinator performance below threshold - review cultural validation processes",
      );
    }
    if (!results.service_status.arabicProcessor) {
      results.recommendations.push(
        "Arabic processing accuracy needs improvement - update dialect recognition models",
      );
    }
    if (!results.service_status.paymentSecurity) {
      results.recommendations.push(
        "Payment security issues detected - review fraud detection algorithms",
      );
    }
    if (!results.service_status.agentCommunication) {
      results.recommendations.push(
        "Agent communication system unstable - check network and service availability",
      );
    }
    if (!results.service_status.workflowOrchestrator) {
      results.recommendations.push(
        "Workflow execution issues - optimize orchestration and retry mechanisms",
      );
    }
    ...
    return {
      ...
      recommendations: [
        "Critical system error - immediate technical review required",
      ],
    };
  }
}
