...
class DALLEService {
  ...
  async generateImages(params) {
    ...
    try {
      logger.info(
        `Starting DALL-E generation for user ${user_id} with model ${model}`,
      );
      ...
      if (n > modelConfig.maxImages) {
        throw new Error(
          `Model ${model} supports maximum ${modelConfig.maxImages} images per request`,
        );
      }
      ...
      if (cultural_validation) {
        ...
        for (const image of processedImages) {
          const imageValidation =
            await this.culturalValidator.validateGeneratedImage({
          ...
          if (imageValidation.isValid) {
            ...
            logger.warn(
              `Generated image failed validation: ${imageValidation.reason}`,
              {
                imageId: image.id,
                user: user_id,
              },
        ...
        logger.info(
          `${validatedImages.length}/${processedImages.length} images passed cultural validation`,
        );
      ...
      if (error.response?.data?.error) {
        ...
        if (openaiError.code === "content_policy_violation") {
          throw new Error(
            "Content policy violation: Please modify your prompt to comply with usage guidelines",
          );
        }
        ...
        if (openaiError.code === "billing_hard_limit_reached") {
          throw new Error(
            "Service temporarily unavailable: Billing limit reached",
          );
        }
  ...
  async editImages(params) {
    ...
    try {
      ...
      if (cultural_validation) {
        ...
        const imageValidation =
          await this.culturalValidator.validateImageContent({
        ...
        if (!imageValidation.isValid) {
          throw new Error(
            `Original image validation failed: ${imageValidation.reason}`,
          );
        }

        // Validate edit instruction
        const instructionValidation =
          await this.culturalValidator.validatePrompt({
            prompt: instruction,
            domain: professional_domain,
            islamicCompliance: true,
            userId: user_id,
        ...
        if (!instructionValidation.isValid) {
          throw new Error(
            `Edit instruction validation failed: ${instructionValidation.reason}`,
          );
        }
      }
      ...
      if (cultural_validation) {
        ...
        for (const image of editedImages) {
          ...
          if (validation.isValid) {
            ...
            logger.warn(
              `Edited image failed validation: ${validation.reason}`,
              {
                imageId: image.id,
                user: user_id,
              },
