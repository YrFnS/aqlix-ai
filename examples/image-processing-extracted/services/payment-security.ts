...
export interface PaymentSecurityRequest {
  operation: 'image_generation' | 'image_editing' | 'bulk_generation' | 'premium_features';
  amount: number;
  currency: 'IQD' | 'USD';
...
export interface PaymentSecurityResult {
  ...
  fraud_indicators: Array<{
    type: 'suspicious_amount' | 'unusual_location' | 'rapid_requests' | 'content_violation' | 'gateway_mismatch';
    severity: 'low' | 'medium' | 'high';
    description: string;
    recommendation: string;
  }>;
  compliance_issues: Array<{
    category: 'iraqi_banking' | 'islamic_finance' | 'cbi_regulations' | 'anti_fraud';
    issue: string;
    requirement: string;
    resolution: string;
  }>;
...
export class PaymentSecurityService {
  ...
  private securityCache: Map<string, { result: PaymentSecurityResult; timestamp: number }>;
   ...
   */
  async validatePaymentSecurity(request: PaymentSecurityRequest): Promise<PaymentSecurityResult> {
...
Please provide comprehensive security analysis with risk scoring and recommendations.`
      ...
      const localSecurityChecks = this.performLocalSecurityChecks(request, gatewayConfig);
      
      // Combine agent analysis with local checks
      const result: PaymentSecurityResult = {
        secure: localSecurityChecks.secure && localSecurityChecks.risk_level !== 'critical',
        ...
        cultural_security: {
          content_appropriate: (request.cultural_context?.cultural_validation_score || 0.9) >= 0.8,
          islamic_compliant: request.cultural_context?.islamic_compliance !== false,
          professional_verified: request.cultural_context?.professional_verification !== false,
          cultural_risk_score: Math.max(0, 1 - (request.cultural_context?.cultural_validation_score || 0.9))
        },
        recommendations: this.generateSecurityRecommendations(request, localSecurityChecks),
        processing_metadata: {
  ...
  async validateBulkPayment(requests: PaymentSecurityRequest[]): Promise<{
    ...
    const average_score = individual_results.reduce((sum, r) => sum + r.security_score, 0) / individual_results.length;
    ...
    const overall_security: PaymentSecurityResult = {
      ...
      fraud_indicators: [],
      compliance_issues: [],
      gateway_validation: individual_results[0]?.gateway_validation || {} as any,
      cultural_security: {
        content_appropriate: individual_results.every(r => r.cultural_security.content_appropriate),
        islamic_compliant: individual_results.every(r => r.cultural_security.islamic_compliant),
        professional_verified: individual_results.every(r => r.cultural_security.professional_verified),
        cultural_risk_score: Math.max(...individual_results.map(r => r.cultural_security.cultural_risk_score))
      },
      recommendations: [
   ...
   */
  async getGatewayStatus(): Promise<Record<IraqiPaymentGateway, {
    available: boolean;
...
Please provide comprehensive gateway status report.`
      ...
      return {
        zaincash: { available: true, response_time: 1000, success_rate: 0.95, current_load: 0.5 },
        fastpay: { available: true, response_time: 800, success_rate: 0.93, current_load: 0.4 },
        nasswallet: { available: true, response_time: 1500, success_rate: 0.90, current_load: 0.6 }
      };
    }
  }
  ...
  getSecurityMetrics(): {
    ...
    gateway_performance: Record<IraqiPaymentGateway, { count: number; success_rate: number; }>;
  } {
    const transactions = Array.from(this.transactionLogs.values());
    const successful = transactions.filter(t => t.status === 'approved').length;
    const fraudulent = transactions.filter(t => 
    ...
    // Gateway performance
    const gatewayPerformance: Record<IraqiPaymentGateway, { count: number; success_rate: number; }> = {
    ...
    return {
      ...
      success_rate: transactions.length > 0 ? successful / transactions.length : 0,
      fraud_detection_rate: transactions.length > 0 ? fraudulent / transactions.length : 0,
  ...
  private performLocalSecurityChecks(
    ...
    gatewayConfig: any
  ): Pick<PaymentSecurityResult, 'secure' | 'risk_level' | 'security_score' | 'fraud_indicators' | 'compliance_issues'> {
    ...
    if (request.amount > gatewayConfig.max_amount) {
      fraudIndicators.push({
        ...
        recommendation: 'Split into multiple transactions or use alternative gateway'
      });
      securityScore -= 0.3;
      riskLevel = 'high';
    }
    ...
    const userTransactions = Array.from(this.transactionLogs.values())
      .filter(t => t.user_id === request.user_id && Date.now() - t.timestamp < 300000); // 5 minutes
    ...
    if (request.cultural_context?.cultural_validation_score && request.cultural_context.cultural_validation_score < 0.8) {
      fraudIndicators.push({
  ...
  private generateSecurityRecommendations(
    ...
    if (request.amount > 100000) { // 100K IQD
  ...
  private getSecureFallbackResult(request: PaymentSecurityRequest, startTime: number): PaymentSecurityResult {
    return {
      secure: false,
      risk_level: 'high',
      security_score: 0.3,
      ...
      gateway_validation: {
        ...
        gateway_limits: { min_amount: 0, max_amount: 0, daily_limit: 0, monthly_limit: 0 },
      ...
      recommendations: ['Manual security review required', 'Do not process automatically'],
      processing_metadata: {
