...
class ImageServiceFactory {
  ...
  async generateImages(params) {
    ...
    let lastError = null;
    const providersToTry = [provider, ...this.config.fallbackProviders].filter(
    ...
    for (const currentProvider of providersToTry) {
      try {
        logger.info(
          `Attempting image generation with provider: ${currentProvider}`,
        );
        ...
        logger.info(
          `Successfully generated ${result.length} images with ${currentProvider}`,
        );
        return result;
      } catch (error) {
        ...
        if (
          error.message.includes("Cultural validation failed") ||
  ...
  async editImages(params) {
    ...
    try {
      ...
      if (provider !== "dall-e-2" && provider !== "openai-image-tools") {
        logger.info(
          `Provider ${provider} doesn't support editing, falling back to DALL-E 2`,
        );
  ...
  async createVariations(params) {
    ...
    try {
      ...
      if (provider !== "dall-e-2") {
        logger.info(
          `Provider ${provider} doesn't support variations, falling back to DALL-E 2`,
        );
        params.provider = "dall-e-2";
      }
  ...
  async getAvailableModels() {
    ...
    for (const [providerId, service] of this.services) {
      try {
        ...
        logger.warn(
          `Failed to get models for provider ${providerId}: ${error.message}`,
        );
