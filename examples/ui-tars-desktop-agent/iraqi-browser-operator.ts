...
export interface IraqiBrowserConfig {
  ...
  onProfessionalSiteDetected?: (domain: ProfessionalDomain, url: string) => Promise<void>;
...
export class IraqiBrowserOperator implements IraqiOperator {
  ...
  constructor(config: IraqiBrowserConfig) {
    ...
    this.highlightClickableElements = this.config.uiEnhancements.highlightClickableElements;
    ...
    this.uiHelper = {
      ...
      showDragIndicator: async (x1: number, y1: number, x2: number, y2: number) => {},
      cleanupTemporaryVisuals: async () => {},
      removeClickableHighlights: async () => {},
      cleanup: async () => {},
    };
    
    this.logger.info('[IraqiBrowserOperator] Initialized with cultural enhancements');
    this.logger.info(`[IraqiBrowserOperator] Professional domain: ${this.config.culturalValidation.professionalDomain}`);
    this.logger.info(`[IraqiBrowserOperator] Arabic support: ${this.config.arabicSupport.enabled}`);
  }
  ...
  async screenshot(): Promise<{ base64: string; scaleFactor: number }> {
    this.logger.info('[IraqiBrowserOperator] Taking enhanced screenshot with Arabic analysis');
    ...
    try {
      ...
      this.logger.info(`[IraqiBrowserOperator] Device scale factor: ${deviceScaleFactor}`);

      // Highlight clickable elements if enabled
      if (this.highlightClickableElements) {
        this.logger.info('[IraqiBrowserOperator] Highlighting clickable elements for Arabic interface');
      ...
      if (this.config.uiEnhancements.arabicTextHighlighting && this.config.arabicSupport.enabled) {
        await this.highlightArabicTextElements(page);
      }

      // Take screenshot with enhanced quality for Arabic text
      this.logger.info('[IraqiBrowserOperator] Capturing screenshot with Arabic optimization');
      ...
      this.logger.info(`[IraqiBrowserOperator] Screenshot captured in ${duration}ms`);
      ...
      try {
        ...
        this.logger.error('[IraqiBrowserOperator] Error in onScreenshot callback:', error);
      }
  ...
  async execute(params: any): Promise<any> {
    this.logger.info('[IraqiBrowserOperator] Starting execute with cultural validation');

    const { parsedPrediction, screenWidth, screenHeight, culturalContext } = params;
    const { action_type, action_inputs } = parsedPrediction;

    // Log cultural context if available
    if (culturalContext) {
      this.logger.info(`[IraqiBrowserOperator] Cultural context - Domain: ${culturalContext.domain}, Compliance: ${culturalContext.complianceScore}%, Arabic elements: ${culturalContext.arabicElements?.length || 0}`);
    }
    ...
    if (this.config.culturalValidation.enabled) {
      ...
      if (validation === ActionValidation.FORBIDDEN) {
        this.logger.warn(`[IraqiBrowserOperator] Action ${action_type} blocked by cultural validation`);
    ...
    this.logger.info(`[IraqiBrowserOperator] Parsed coordinates: (${coordinates.x}, ${coordinates.y})`);
    ...
    try {
      ...
      this.logger.info(`[IraqiBrowserOperator] Action ${action_type} completed successfully`);
      ...
      this.logger.error(`[IraqiBrowserOperator] Failed to execute ${action_type}:`, error);
  ...
  async analyzeCulturalContext(screenshot: string): Promise<IraqiCulturalStatus> {
    ...
    try {
      ...
      if (this.config.culturalValidation.contentFiltering) {
        const hasInappropriateContent = await this.detectInappropriateContent(page);
      ...
      this.logger.info(`[IraqiBrowserOperator] Cultural analysis result: ${status} for domain: ${domain}`);
      return status;

    } catch (error) {
      this.logger.error('[IraqiBrowserOperator] Cultural analysis failed:', error);
  ...
  async processArabicText(screenshot: string): Promise<ArabicGUIElement[]> {
    ...
    try {
      ...
      this.logger.info(`[IraqiBrowserOperator] Detected ${elements.length} Arabic elements on page`);
      return elements;

    } catch (error) {
      this.logger.error('[IraqiBrowserOperator] Arabic text processing failed:', error);
      return [];
    }
  }
  ...
  async validateAction(action: any): Promise<ActionValidation> {
    ...
    if (action_type === 'navigate' && action_inputs?.url) {
      ...
      if (this.config.security.domainWhitelist && this.config.security.domainWhitelist.length > 0) {
        const isWhitelisted = this.config.security.domainWhitelist.some(whitelistedDomain =>
    ...
    const professionalDomain = this.config.culturalValidation.professionalDomain;
    if (professionalDomain !== ProfessionalDomain.GENERAL) {
      if (this.requiresProfessionalValidation(action_type, professionalDomain)) {
        return ActionValidation.REQUIRES_CONFIRMATION;
      }
    }
  ...
  private async executeActionWithCulturalContext(
    ...
    screenContext: { screenWidth: number; screenHeight: number; deviceScaleFactor: number },
    culturalContext?: any
  ): Promise<void> {
    
    switch (actionType) {
      case 'navigate':
        await this.handleNavigateWithCulturalValidation(actionInputs, culturalContext);
        break;
      ...
      case 'left_click':
      case 'left_single':
        await this.handleClickWithRTLSupport(coordinates.x, coordinates.y, culturalContext);
        break;

      case 'double_click':
      case 'left_double':
        await this.handleDoubleClickWithRTLSupport(coordinates.x, coordinates.y, culturalContext);
        break;

      case 'right_click':
        await this.handleRightClickWithRTLSupport(coordinates.x, coordinates.y, culturalContext);
        break;

      case 'drag':
        await this.handleDragWithRTLSupport(actionInputs, screenContext, culturalContext);
        break;
      ...
      case 'finished':
        if (this.config.onFinalAnswer) {
          await this.config.onFinalAnswer(actionInputs?.thought || 'Task completed');
        }
      ...
      default:
        this.logger.warn(`[IraqiBrowserOperator] Unsupported action: ${actionType}`);
        break;
    }
  }

  private async handleNavigateWithCulturalValidation(actionInputs: any, culturalContext?: any): Promise<void> {
    const page = await this.getActivePage();
    ...
    this.logger.info(`[IraqiBrowserOperator] Navigating with cultural validation: ${url}`);

    // Cultural validation before navigation
    if (culturalValidation && this.config.culturalValidation.enabled) {
      const domain = this.extractDomain(url);
      if (this.isPotentiallyInappropriateDomain(domain)) {
        this.logger.warn(`[IraqiBrowserOperator] Navigation blocked to potentially inappropriate domain: ${domain}`);
        return;
      }
    }
    ...
    try {
      ...
      this.logger.info(`[IraqiBrowserOperator] Navigation completed to: ${url}`);
  ...
  private async handleTypeWithArabicSupport(actionInputs: any, culturalContext?: any): Promise<void> {
    const page = await this.getActivePage();
    ...
    this.logger.info(`[IraqiBrowserOperator] Typing with Arabic support - Language: ${language}, Dialect: ${dialect}`);

    // Cultural validation for content
    if (culturalCheck && this.config.culturalValidation.enabled) {
      if (this.containsInappropriateContent(content)) {
        this.logger.warn('[IraqiBrowserOperator] Content blocked by cultural validation');
        return;
      }
    }
    ...
    try {
      ...
      this.logger.info(`[IraqiBrowserOperator] Successfully typed ${content.length} characters`);
  ...
  private async handleClickWithRTLSupport(x: number | null, y: number | null, culturalContext?: any): Promise<void> {
    if (x === null || y === null) {
      throw new Error('Invalid click coordinates');
    }

    const page = await this.getActivePage();
    this.logger.info(`[IraqiBrowserOperator] Clicking with RTL support at (${x}, ${y})`);
    ...
    try {
      ...
      this.logger.info('[IraqiBrowserOperator] Click completed with RTL support');

    } catch (error) {
      this.logger.error('[IraqiBrowserOperator] Click operation failed:', error);
      throw error;
    }
  }

  private async handleDoubleClickWithRTLSupport(x: number | null, y: number | null, culturalContext?: any): Promise<void> {
    if (x === null || y === null) {
      throw new Error('Invalid double click coordinates');
    }

    const page = await this.getActivePage();
    this.logger.info(`[IraqiBrowserOperator] Double clicking with RTL support at (${x}, ${y})`);
    ...
    try {
      ...
      const delayTime = culturalContext?.arabicElements?.length > 0 ? 1000 : 800;
      await this.delay(delayTime);

      this.logger.info('[IraqiBrowserOperator] Double click completed');

    } catch (error) {
      this.logger.error('[IraqiBrowserOperator] Double click operation failed:', error);
      throw error;
    }
  }

  private async handleRightClickWithRTLSupport(x: number | null, y: number | null, culturalContext?: any): Promise<void> {
    if (x === null || y === null) {
      throw new Error('Invalid right click coordinates');
    }

    const page = await this.getActivePage();
    this.logger.info(`[IraqiBrowserOperator] Right clicking with RTL support at (${x}, ${y})`);
    ...
    try {
      ...
      this.logger.error('[IraqiBrowserOperator] Right click operation failed:', error);
      throw error;
    }
  }

  private async handleScrollWithRTLSupport(actionInputs: any, culturalContext?: any): Promise<void> {
    const page = await this.getActivePage();
    ...
    this.logger.info(`[IraqiBrowserOperator] Scrolling with RTL support: ${direction}, RTL aware: ${rtlAware}`);
    ...
    if (rtlAware && (direction === 'left' || direction === 'right')) {
      ...
      this.logger.info(`[IraqiBrowserOperator] Adjusted scroll direction for RTL: ${direction} -> ${adjustedDirection}`);
    }
    ...
    try {
      switch (adjustedDirection) {
        ...
        case 'right':
          ...
          break;
        default:
          this.logger.warn(`[IraqiBrowserOperator] Unsupported scroll direction: ${direction}`);
          return;
      }

      // Enhanced delay for Arabic content reflow
      const delayTime = culturalContext?.arabicElements?.length > 0 
      ...
      this.logger.error('[IraqiBrowserOperator] Scroll operation failed:', error);
      throw error;
    }
  }
  ...
  private async setupArabicPageContext(page: any): Promise<void> {
    try {
      ...
      this.logger.info('[IraqiBrowserOperator] Arabic page context setup completed');

    } catch (error) {
      this.logger.error('[IraqiBrowserOperator] Arabic page setup failed:', error);
    }
  }
  ...
  private async highlightArabicTextElements(page: any): Promise<void> {
    try {
      ...
      this.logger.info('[IraqiBrowserOperator] Arabic text elements highlighted');

    } catch (error) {
      this.logger.error('[IraqiBrowserOperator] Arabic highlighting failed:', error);
    }
  }
  ...
  private async switchToArabicLayout(page: any): Promise<void> {
    ...
    try {
      ...
      this.logger.info('[IraqiBrowserOperator] Switched to Arabic keyboard layout');

    } catch (error) {
      this.logger.error('[IraqiBrowserOperator] Arabic layout switch failed:', error);
    }
  }

  private async typeArabicContent(page: any, content: string, dialect: string): Promise<void> {
    try {
      ...
      this.logger.info(`[IraqiBrowserOperator] Arabic content typed: ${content.length} characters`);
  ...
  private async handleDragWithRTLSupport(actionInputs: any, screenContext: any, culturalContext?: any): Promise<void> {
    ...
    const startCoords = this.parseCoordinatesWithRTLSupport(startBoxStr, screenWidth, screenHeight, deviceScaleFactor, rtlAware);
    const endCoords = this.parseCoordinatesWithRTLSupport(endBoxStr, screenWidth, screenHeight, deviceScaleFactor, rtlAware);
    ...
    try {
      await this.uiHelper.showDragIndicator(startCoords.x, startCoords.y, endCoords.x, endCoords.y);
      await this.delay(300);
      ...
      for (let i = 1; i <= steps; i++) {
        const stepX = startCoords.x + ((endCoords.x - startCoords.x) * i) / steps;
        const stepY = startCoords.y + ((endCoords.y - startCoords.y) * i) / steps;
      ...
      this.logger.info('[IraqiBrowserOperator] Drag completed with RTL support');
  ...
  private async handleFillArabicForm(actionInputs: any, culturalContext?: any): Promise<void> {
    const fields = actionInputs?.fields || [];
    const culturalValidation = actionInputs?.cultural_validation !== false;

    this.logger.info(`[IraqiBrowserOperator] Filling Arabic form with ${fields.length} fields`);

    if (culturalValidation && this.config.culturalValidation.enabled) {
      // Validate form content before filling
      for (const field of fields) {
        if (field.value && this.containsInappropriateContent(field.value)) {
          this.logger.warn(`[IraqiBrowserOperator] Skipping field with inappropriate content: ${field.name}`);
          continue;
        }
      }
    }
  ...
  private async handleSubmitProfessionalForm(actionInputs: any, culturalContext?: any): Promise<void> {
    const domain = actionInputs?.domain || this.config.culturalValidation.professionalDomain;
    const complianceCheck = actionInputs?.compliance_check !== false;

    this.logger.info(`[IraqiBrowserOperator] Submitting professional form for domain: ${domain}`);
    ...
    if (complianceCheck && this.config.culturalValidation.enabled) {
      ...
      if (!isCompliant) {
        this.logger.warn('[IraqiBrowserOperator] Form submission blocked by compliance check');
        return;
      }
    }
    ...
    try {
      ...
      const submitButton = await page.$('button[type="submit"], input[type="submit"], .submit-btn, .إرسال');
      
      if (submitButton) {
        await submitButton.click();
        await this.waitForPossibleNavigation(page);
        this.logger.info('[IraqiBrowserOperator] Professional form submitted successfully');
      } else {
        // Try pressing Enter as fallback
        await page.keyboard.press('Enter');
        await this.waitForPossibleNavigation(page);
        this.logger.info('[IraqiBrowserOperator] Form submitted using Enter key');
      }

    } catch (error) {
      this.logger.error('[IraqiBrowserOperator] Form submission failed:', error);
      throw error;
    }
  }

  private async handleNavigateGovernmentPortal(actionInputs: any): Promise<void> {
    ...
    this.logger.info(`[IraqiBrowserOperator] Navigating government portal - Ministry: ${ministry}, Service: ${service}`);
    ...
    const url = governmentUrls[ministry as keyof typeof governmentUrls] || `https://${ministry}.gov.iq`;
    ...
    try {
      ...
      this.logger.info(`[IraqiBrowserOperator] Government portal navigation completed: ${url}`);

    } catch (error) {
      this.logger.error('[IraqiBrowserOperator] Government portal navigation failed:', error);
      throw error;
    }
  }
  ...
  private parseCoordinatesWithRTLSupport(
    ...
    try {
      ...
      if (rtlAware) {
        ...
        x1 = adjustedX1;
        x2 = adjustedX2;
        this.logger.info('[IraqiBrowserOperator] RTL coordinate adjustment applied');
      }
      ...
      this.logger.error('[IraqiBrowserOperator] Coordinate parsing failed:', error);
      return { x: null, y: null };
    }
  }
  ...
  private detectProfessionalDomain(url: string): ProfessionalDomain {
    ...
    for (const [category, domains] of Object.entries(IRAQI_PROFESSIONAL_DOMAINS)) {
      if (domains.some(professionalDomain => domain.includes(professionalDomain))) {
        switch (category) {
          case 'government': return ProfessionalDomain.GOVERNMENTAL;
          case 'legal': return ProfessionalDomain.LEGAL;
          case 'medical': return ProfessionalDomain.MEDICAL;
          case 'educational': return ProfessionalDomain.EDUCATIONAL;
        }
      }
    }
  ...
  private detectDialectType(text: string): 'iraqi' | 'standard' | 'mixed' | 'unknown' {
    // Iraqi dialect markers
    const iraqiMarkers = ['چ', 'پ', 'ڤ', 'گ', 'شلونك', 'آني', 'انت'];
    const hasIraqiMarkers = iraqiMarkers.some(marker => text.includes(marker));
  ...
  private requiresProfessionalValidation(actionType: string, domain: ProfessionalDomain): boolean {
    const sensitiveActions = ['submit_professional_form', 'navigate_government_portal', 'type'];
    return sensitiveActions.includes(actionType) && domain !== ProfessionalDomain.GENERAL;
  }

  private async validateActionCulturally(actionType: string, actionInputs: any, culturalContext?: any): Promise<ActionValidation> {
    // Delegate to interface method
    return await this.validateAction({ action_type: actionType, action_inputs: actionInputs });
  }
  ...
  private async validateProfessionalFormSubmission(domain: string): Promise<boolean> {
    // Placeholder for professional form validation
    return true;
  }

  private async fillFormField(page: any, field: any, culturalContext?: any): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`[IraqiBrowserOperator] Failed to fill field ${field.name}:`, error);
    }
  }
  ...
  private logSecurityEvent(actionType: string, actionInputs: any, status: string, culturalContext?: any): void {
    const event = {
      timestamp: Date.now(),
      action: actionType,
      inputs: actionInputs,
  ...
  private async getDeviceScaleFactor(): Promise<number> {
    ...
    try {
      ...
      this.logger.error('[IraqiBrowserOperator] Failed to get device scale factor:', error);
      return 1;
    }
  }
  ...
  private async waitForPossibleNavigation(page: any): Promise<void> {
    try {
      ...
      this.logger.info('[IraqiBrowserOperator] Navigation completed or timed out');

    } catch (error) {
      this.logger.error('[IraqiBrowserOperator] Navigation wait failed:', error);
    }
  }
  ...
  private async handleNavigateBack(actionInputs: any): Promise<void> {
    ...
    this.logger.info(`[IraqiBrowserOperator] Navigating back with Arabic context: ${arabicContext}`);
  ...
  private async handleHotkeyWithArabicLayout(actionInputs: any, culturalContext?: any): Promise<void> {
    const page = await this.getActivePage();
    const keyStr = actionInputs?.key || actionInputs?.hotkey;
    const arabicLayout = actionInputs?.arabic_layout !== false && this.config.arabicSupport.enabled;
    const professionalDomain = actionInputs?.professional_domain || this.config.culturalValidation.professionalDomain;
    ...
    this.logger.info(`[IraqiBrowserOperator] Executing hotkey with Arabic layout: ${keyStr}, Arabic: ${arabicLayout}, Domain: ${professionalDomain}`);
    ...
    try {
      ...
      this.logger.error('[IraqiBrowserOperator] Hotkey execution failed:', error);
      throw error;
    }
  }
  ...
  private async handleValidatePageContent(actionInputs: any): Promise<void> {
    ...
    const professionalStandards = actionInputs?.professional_standards !== false;

    this.logger.info(`[IraqiBrowserOperator] Validating page content - Islamic: ${islamicCompliance}, Professional: ${professionalStandards}`);
    ...
    try {
      ...
      if (islamicCompliance) {
        ...
        if (!isCompliant) {
          ...
          validationResults.issues.push('Content may not comply with Islamic principles');
        }
      }
      ...
      if (professionalStandards) {
        ...
        if (!isProfessional) {
          ...
          validationResults.issues.push('Content may not meet professional standards');
        }
      }

      this.logger.info('[IraqiBrowserOperator] Content validation completed:', validationResults);

    } catch (error) {
      this.logger.error('[IraqiBrowserOperator] Content validation failed:', error);
      throw error;
    }
  }
...
export function createIraqiBrowserOperator(
  ...
  const mergedConfig: IraqiBrowserConfig = {
    ...
    professionalNavigation: { ...defaultConfig.professionalNavigation, ...config.professionalNavigation },
    uiEnhancements: { ...defaultConfig.uiEnhancements, ...config.uiEnhancements },
