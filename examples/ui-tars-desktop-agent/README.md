...
await guiAgent.run('Create Arabic legal document with Islamic compliance validation');
```
...
await guiAgent.run('Open legal document system and create new contract with Islamic compliance');
```
...
await guiAgent.run('Access Ministry of Education portal and submit bilingual academic credentials');
```
...
```typescript
type ProfessionalDomain = 'legal' | 'medical' | 'educational' | 'governmental' | 'business';
