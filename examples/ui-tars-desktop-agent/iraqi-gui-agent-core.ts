...
export abstract class IraqiOperator extends EventEmitter {
  ...
  async validateIslamicCompliance(
    action: PredictionParsed,
  ): Promise<IslamicWorkflowValidationResult> {
...
export abstract class BaseModel {
  ...
  processVlmParamsWithCulture(
    ...
    const enhancedMessages = conversations.map((msg) => ({
      ...msg,
      culturalContext,
      content: this.culturalEnhancer.enhanceMessageContent(
        msg.content,
        culturalContext,
      ),
    }));
...
export class IraqiCulturalValidator {
  ...
  async validate(instruction: string): Promise<CulturalValidationResult> {
    ...
    if (scores[2] < 0.85) {
      ...
      recommendations.push(
        `Enhance content for ${this.culturalContext.professionalDomain} domain`,
      );
  ...
  async validateContext(
    ...
    const validationChecks = [
      ...
      this.validateLanguageSettings(
        ...
        context.arabicDialect,
      ),
      this.validateOrganizationalContext(
        context.organizationType,
        context.securityLevel,
      ),
      this.validateTimeSettings(
        context.prayerTimeAware,
        context.workingDaysPattern,
      ),
    ];
  ...
  private initializeCulturalRules(): void {
    this.culturalRules = new Map([
      [
        "islamic_compliance",
        (content: string) => this.scoreIslamicCompliance(content),
      ],
      [
        "cultural_sensitivity",
        (content: string) => this.scoreCulturalSensitivity(content),
      ],
      [
        "professional_appropriateness",
        (content: string) => this.scoreProfessionalAppropriatenesss(content),
      ],
      [
        "language_appropriateness",
        (content: string) => this.scoreLanguageAppropriatenesss(content),
      ],
  ...
  private async validateCulturalSensitivity(content: string): Promise<number> {
    ...
    return (
      culturalChecks.filter((check) => check).length / culturalChecks.length
    );
  }

  private async validateProfessionalAppropriatenesss(
    content: string,
  ): Promise<number> {
  ...
  private async validateLanguageAppropriatenesss(
    content: string,
  ): Promise<number> {
    ...
    return (
      languageChecks.filter((check) => check).length / languageChecks.length
    );
  }
  ...
  private getMinimumThreshold(): number {
    switch (this.culturalContext.islamicCompliance) {
      case IslamicComplianceLevel.STRICT:
        return 0.95;
      case IslamicComplianceLevel.STANDARD:
        return 0.9;
      case IslamicComplianceLevel.FLEXIBLE:
        return 0.85;
      default:
        return 0.9;
    }
  ...
  private respectsIslamicValues(content: string): boolean {
    return true;
  }
  private alignsWithIslamicEthics(content: string): boolean {
    return true;
  }
  private conflictsWithIslamicPrinciples(content: string): boolean {
    return false;
  }
  private respectsIraqiTraditions(content: string): boolean {
    return true;
  }
  private avoidsTabooTopics(content: string): boolean {
    return true;
  }
  private usesCulturallyAppropriateLanguage(content: string): boolean {
    return true;
  }
  private respectsTribalSensitivities(content: string): boolean {
    return true;
  }
  private maintainsPoliticalNeutrality(content: string): boolean {
    return true;
  }

  private getProfessionalDomainChecks(
  ...
  private respectsLegalTerminology(content: string): boolean {
    return true;
  }
  private followsLegalProcedures(content: string): boolean {
    return true;
  }
  private maintainsLegalEthics(content: string): boolean {
    return true;
  }
  private respectsMedicalPrivacy(content: string): boolean {
    return true;
  }
  private followsMedicalEthics(content: string): boolean {
    return true;
  }
  private usesMedicalTerminology(content: string): boolean {
    return true;
  }
  private respectsEducationalStandards(content: string): boolean {
    return true;
  }
  private followsEducationalEthics(content: string): boolean {
    return true;
  }
  private usesEducationalTerminology(content: string): boolean {
    return true;
  }
  private respectsGovernmentProtocols(content: string): boolean {
    return true;
  }
  private followsOfficialProcedures(content: string): boolean {
    return true;
  }
  private maintainsOfficialTone(content: string): boolean {
    return true;
  }

  // Language validation methods
  private usesAppropriateArabicDialect(content: string): boolean {
    return true;
  }
  private respectsLanguageFormality(content: string): boolean {
    return true;
  }
  private avoidsInappropriateSlang(content: string): boolean {
    return true;
  }
  private maintainsConsistentLanguage(content: string): boolean {
    return true;
  }

  // Social norms validation methods
  private respectsGenderNorms(content: string): boolean {
    return true;
  }
  private respectsAgeHierarchy(content: string): boolean {
    return true;
  }
  private respectsFamilyValues(content: string): boolean {
    return true;
  }
  private respectsCommunityValues(content: string): boolean {
    return true;
  }

  // Context validation helpers
  private validateIslamicComplianceLevel(
    level: IslamicComplianceLevel,
  ): Promise<number> {
    return Promise.resolve(1.0);
  }

  private validateProfessionalDomain(
    domain: IraqiProfessionalDomain,
  ): Promise<number> {
    return Promise.resolve(1.0);
  }
  ...
  private validateOrganizationalContext(
    orgType: string,
    secLevel: string,
  ): Promise<number> {
    return Promise.resolve(1.0);
  ...
  private scoreIslamicCompliance(content: string): number {
    return 0.95;
  }
  private scoreCulturalSensitivity(content: string): number {
    return 0.9;
  }
  private scoreProfessionalAppropriatenesss(content: string): number {
    return 0.85;
  }
  private scoreLanguageAppropriatenesss(content: string): number {
    return 0.9;
  }
  private scoreSocialNorms(content: string): number {
    return 0.9;
  }
}
...
export class ArabicRTLProcessor extends EventEmitter {
  ...
  constructor(culturalContext: IraqiCulturalContext) {
    super();
    this.culturalContext = culturalContext;
    this.dialectRecognizer = new IraqiDialectRecognizer(
      culturalContext.arabicDialect,
    );
  }
  ...
  async processScreenshot(
    ...
    try {
      ...
      this.emit("processing_completed", {
        ...
        processingTime: Date.now() - startTime,
        averageConfidence:
          arabicElements.reduce((sum, el) => sum + el.confidence, 0) /
          arabicElements.length,
      });
  ...
  private async assessCulturalContext(
    text: string,
  ): Promise<ArabicGUIElement["culturalContext"]> {
    return {
  ...
  private async assessCulturalAppropriateness(text: string): Promise<number> {
    return 0.95;
  }
  private async assessIslamicCompliance(text: string): Promise<number> {
    return 0.98;
  }
  private async assessProfessionalRelevance(text: string): Promise<number> {
    return 0.85;
  }
}
...
export class IslamicWorkflowValidator extends EventEmitter {
  ...
  async validateAction(
    ...
    try {
      ...
      const complianceScore =
        validationChecks.reduce((sum, score) => sum + score, 0) /
  ...
  private async validateActionType(actionType: ActionType): Promise<number> {
    const islamicActionValidation = new Map([
      ...
      [ActionType.TYPE, 0.95],
      [ActionType.DRAG, 1.0],
      [ActionType.SCROLL, 1.0],
      [ActionType.HOTKEY, 1.0],
      [ActionType.SCREENSHOT, 0.9],
      [ActionType.CLICK_ARABIC, 1.0],
      [ActionType.TYPE_ARABIC, 0.95],
      [ActionType.NAVIGATE_RTL_FORM, 1.0],
      [ActionType.ISLAMIC_WORKFLOW_ACTION, 1.0],
      [ActionType.LEGAL_DOCUMENT_ACTION, 0.95],
      [ActionType.MEDICAL_RECORD_ACTION, 0.9],
  ...
  private async validateActionContent(
  ...
  private async validateTiming(timestamp: Date): Promise<number> {
    ...
    for (const prayerTime of prayerTimes) {
      ...
      if (
        currentTime >= prayerStart - 10 * 60 * 1000 &&
        currentTime < prayerStart
      )
        return 0.5;
    }
  ...
  private async validateCulturalSensitivity(
    ...
    switch (this.culturalContext.professionalDomain) {
      case IraqiProfessionalDomain.MEDICAL:
        ...
        break;
      case IraqiProfessionalDomain.LEGAL:
        if (action.action_inputs.content?.includes("شهادة"))
          sensitivityScore += 0.1;
        break;
      case IraqiProfessionalDomain.EDUCATIONAL:
        if (action.action_inputs.content?.includes("امتحان"))
          sensitivityScore += 0.1;
        break;
    }
  ...
  private async validateProfessionalEthics(
    action: PredictionParsed,
  ): Promise<number> {
    if (
      action.action_type === ActionType.SCREENSHOT &&
      this.culturalContext.securityLevel === "confidential"
    )
      return 0.5;

    if (
      action.action_type === ActionType.TYPE_ARABIC &&
      action.action_inputs.content?.includes("سر")
    )
      return 0.3;
  ...
  private containsCulturalInsensitivity(content: string): boolean {
    return false;
  }
}
...
RTL Elements: ${arabicElements.filter((el) => el.textDirection === "rtl").length}`;
    ...
    enhancedPrompt += this.getProfessionalDomainInstructions(
      culturalContext.professionalDomain,
    );
    enhancedPrompt += this.getIslamicComplianceInstructions(
      culturalContext.islamicCompliance,
    );
  ...
  enhanceMessageContent(
    ...
    if (culturalContext.rtlLayout)
      enhancedContent = `[RTL Context] ${enhancedContent}`;
    if (culturalContext.arabicDialect !== "iraqi")
      enhancedContent = `[${culturalContext.arabicDialect} Dialect] ${enhancedContent}`;
    enhancedContent = `[${culturalContext.professionalDomain} Domain] ${enhancedContent}`;

    return enhancedContent;
  }

  private getProfessionalDomainInstructions(
    domain: IraqiProfessionalDomain,
  ): string {
  ...
  private getIslamicComplianceInstructions(
    level: IslamicComplianceLevel,
  ): string {
    const instructions = new Map([
      [
  ...
  async run(
    ...
    try {
      ...
      if (!culturalValidation.isValid) {
        ...
        throw new CulturalViolationError(
          "Instruction violates cultural guidelines",
          culturalValidation,
        );
      }
      ...
      while (
        ...
        this.performanceMetrics.arabicProcessingTime +=
  ...
  private async executeWithValidation(
    ...
    try {
      ...
      if (
        ...
        this.culturalContext.islamicCompliance === IslamicComplianceLevel.STRICT
      ) {
      ...
      this.performanceMetrics.culturalValidationTime +=
...
Respond with structured action predictions that include cultural context validation.`;
  ...
  private setupEventListeners(): void {
    this.operator.on("screenshot_captured", (data) =>
      this.emit("screenshot_captured", data),
    );
    this.operator.on("action_executed", (data) =>
      this.emit("action_executed", data),
    );
    this.culturalValidator.on("validation_completed", (data) =>
      this.emit("cultural_validation_completed", data),
    );
    this.arabicProcessor.on("processing_completed", (data) =>
      this.emit("arabic_processing_completed", data),
    );
    this.islamicCompliance.on("validation_completed", (data) =>
      this.emit("islamic_validation_completed", data),
    );
  }
  ...
  private getMaxIterations(): number {
    ...
    const multiplier =
      domainMultiplier.get(this.culturalContext.professionalDomain) || 1.0;
  ...
  getCulturalContext(): IraqiCulturalContext {
    return { ...this.culturalContext };
  }

  async updateCulturalContext(
    newContext: Partial<IraqiCulturalContext>,
  ): Promise<void> {
