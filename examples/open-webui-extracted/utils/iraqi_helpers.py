...
def detect_iraqi_dialect(text: str) -> IraqiDialect:
    ...
    baghdadi_count = sum(1 for indicator in baghdadi_indicators if indicator in text_lower)
    
    # Basra dialect indicators  
    basrawi_indicators = ["شلونكم", "شجان", "مسگوف", "شط", "ويّاكم"]
    basrawi_count = sum(1 for indicator in basrawi_indicators if indicator in text_lower)
...
def validate_professional_domain(
    requested_profession: IraqiProfession, 
    user_profession: IraqiProfession
) -> bool:
    """
    Validate if user can access requested professional domain
    """
    # Admin-level professions (can access most domains)
    admin_professions = {IraqiProfession.DOCTOR, IraqiProfession.LAWYER, IraqiProfession.ENGINEER}
    ...
    if user_profession == IraqiProfession.TEACHER and requested_profession in {
        IraqiProfession.STUDENT, IraqiProfession.TEACHER
    }:
        return True
    ...
    if user_profession == IraqiProfession.STUDENT and requested_profession in {
        IraqiProfession.TEACHER, IraqiProfession.STUDENT, IraqiProfession.OTHER
    }:
        return True
    ...
    if user_profession == IraqiProfession.BUSINESSMAN and requested_profession in {
        IraqiProfession.ENGINEER, IraqiProfession.OTHER
    }:
        return True
    ...
    terminology = {
        IraqiProfession.LAWYER: {
            "arabic_terms": [
                ...
                "التحقيق", "الشهادة", "الوثائق", "العقد", "الميراث", "الطلاق"
            ],
            "english_terms": [
                "court", "judge", "lawyer", "lawsuit", "verdict", "law",
                "constitution", "crime", "penalty", "procedures", "appeal",
                "investigation", "testimony", "documents", "contract", "inheritance"
            ]
        },
        IraqiProfession.DOCTOR: {
            "arabic_terms": [
                ...
                "الأشعة", "التحليل", "الصيدلية", "الجراحة", "القلب", "الدم"
            ],
            "english_terms": [
                "doctor", "patient", "hospital", "treatment", "medicine", "diagnosis",
                "surgery", "emergency", "health", "disease", "prevention", "vaccination",
                "radiology", "analysis", "pharmacy", "surgery", "heart", "blood"
            ]
        },
        IraqiProfession.TEACHER: {
            "arabic_terms": [
                ...
                "الدرجات", "التقييم", "البحث", "الرسالة", "المكتبة", "الكتاب"
            ],
            "english_terms": [
                "school", "university", "student", "teacher", "professor", "lesson",
                "exam", "certificate", "education", "pedagogy", "curriculum", "classroom",
                "grades", "assessment", "research", "thesis", "library", "book"
            ]
        },
        IraqiProfession.ENGINEER: {
            "arabic_terms": [
                "الهندسة", "المهندس", "التصميم", "البناء", "الإنشاء", "المشروع",
                "الخطة", "الحاسوب", "البرمجة", "الشبكة", "الكهرباء", "الميكانيك",
                "المدني", "المعمار", "الجسر", "الطريق", "النفط", "الطاقة"
            ],
            "english_terms": [
                "engineering", "engineer", "design", "construction", "building", "project",
                "plan", "computer", "programming", "network", "electrical", "mechanical",
                "civil", "architecture", "bridge", "road", "oil", "energy"
            ]
        }
    }
...
def extract_arabic_keywords(text: str) -> List[str]:
    ...
    stop_words = {
        ...
        "هو", "هي", "أن", "أو", "لا", "نعم", "كان", "كانت", "يكون", "تكون"
    }
