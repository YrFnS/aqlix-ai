...
def create_access_token(user: UserModel, expires_delta: Optional[int] = None) -> str:
    ...
    to_encode = {
        ...
        "cultural_context": {
            "islamic_compliance": user.cultural_settings.islamic_compliance_level if user.cultural_settings else "moderate",
            "regional_context": "baghdad",  # Default or from user preferences
            "sectarian_sensitivity": user.cultural_settings.sectarian_sensitivity if user.cultural_settings else "neutral"
        }
    }
    
    encoded_jwt = jwt.encode(to_encode, SECRET_KEY, algorithm=ALGORITHM)
    
    log.info(f"Access token created for Iraqi user: {user.email} ({user.profession.value})")
    return encoded_jwt
...
async def get_current_iraqi_user(user: UserModel = Depends(get_current_user)) -> UserModel:
    """
...
async def get_professional_user(
    required_profession: IraqiProfession,
...
async def authenticate_with_iraqi_phone(phone: str, password: str) -> Optional[UserModel]:
    """
    ...
    if verify_password(password, user.password_hash if hasattr(user, 'password_hash') else ""):
...
class IraqiSessionManager:
    ...
    def create_session_context(user: UserModel) -> Dict[str, Any]:
        ...
        return {
            ...
            "profession": user.profession.value,
            "dialect": user.dialect_preference.value,
            "cultural_settings": user.cultural_settings.model_dump() if user.cultural_settings else {},
            "regional_context": "baghdad",  # Could be derived from user preferences
            "session_start": time.time(),
            "last_activity": time.time(),
            "islamic_calendar_enabled": user.cultural_settings.arabic_calendar_preference if user.cultural_settings else False,
            "prayer_time_awareness": user.cultural_settings.prayer_reminders if user.cultural_settings else False,
        }
    ...
    @staticmethod
    def is_session_valid(session_context: Dict[str, Any], timeout_hours: int = 24) -> bool:
