...
class CulturalValidator:
    ...
    def validate_text_content(
        ...
        if not content or not content.strip():
            return ValidationResult(
                is_appropriate=True, score=1.0, reason="Empty content"
            )
        ...
        inappropriate_found = [
            term
            for term in self.patterns.INAPPROPRIATE_CONTENT
        ...
        sectarian_found = [
            term for term in self.patterns.SECTARIAN_TERMS if term in content_lower
        ]
        if sectarian_found:
        ...
        political_found = [
            term for term in self.patterns.POLITICAL_SENSITIVE if term in content_lower
        ]
        if political_found:
        ...
        islamic_terms_found = [
            term for term in self.patterns.ISLAMIC_POSITIVE if term in content_lower
        ]
        ...
        dialect_found = [
            term
            for term in self.patterns.IRAQI_DIALECT_POSITIVE
            if term in content_lower
        ]
        if dialect_found:
    ...
    def _validate_professional_terminology(
        ...
        if profession == "lawyer":
            legal_terms = [
                term for term in self.patterns.PROFESSIONAL_LEGAL if term in content
            ]
            if legal_terms:
                bonus += 0.1
            ...
            medical_terms = [
                term for term in self.patterns.PROFESSIONAL_MEDICAL if term in content
            ]
            if medical_terms:
                bonus += 0.1
            ...
            education_terms = [
                term for term in self.patterns.PROFESSIONAL_EDUCATION if term in content
            ]
            if education_terms:
                bonus += 0.1
    ...
    async def validate_user_profile(
        ...
        if cultural_settings:
            islamic_compliance = cultural_settings.get(
...
async def cultural_validation_middleware(
    content: str, user_context: Dict[str, Any]
) -> ValidationResult:
    """
    ...
    cultural_context = create_cultural_context(
        ...
        sensitivity_level=user_context.get("cultural_settings", {}).get(
            "islamic_compliance_level", "moderate"
        ),
        regional_context=user_context.get("regional_context", "baghdad"),
        sectarian_preference=user_context.get("cultural_settings", {}).get(
            "sectarian_sensitivity", "neutral"
        ),
