...
export class ArabicTextProcessor {
  ...
  private readonly RTL_REGEX =
    /[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/;
  private readonly LTR_REGEX = /[A-Za-z0-9]/;
  private readonly MIXED_REGEX =
    /[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF].*[A-Za-z0-9]|[A-Za-z0-9].*[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/;
  ...
  async processText(text: string, domain?: string): Promise<ArabicTextResult> {
    try {
      ...
      const processedText = await this.processTextContent(
        text,
        direction,
        domain,
      );
  ...
  private detectDialect(text: string): { dialect: string; confidence: number } {
    ...
    for (const [dialectName, patterns] of Object.entries(
      this.DIALECT_PATTERNS,
    )) {
  ...
  private async processTextContent(
    ...
    if (domain && this.PROFESSIONAL_TERMS[domain]) {
      const termResult = this.applyProfessionalTerminology(
        processedText,
        domain,
      );
  ...
  private processMixedLanguageText(text: string): {
    ...
    if (segments.length > 5) {
      ...
      recommendations.push(
        "Consider reducing language switching for better readability",
      );
    }
   ...
   */
  private applyProfessionalTerminology(
    text: string,
    domain: string,
  ): {
    text: string;
    ...
    for (const [arabic, english] of Object.entries(terms.bilingual)) {
      if (text.includes(arabic) && !text.includes(english)) {
        recommendations.push(
          `Consider adding English translation for "${arabic}" (${english})`,
        );
      }

      if (text.includes(english) && !text.includes(arabic)) {
        recommendations.push(
          `Consider adding Arabic translation for "${english}" (${arabic})`,
        );
      }
    }
    ...
    for (const [dialectName, patterns] of Object.entries(
      ...
      for (let i = 0; i < patterns.grammar.length; i++) {
        ...
        if (normalizedText.includes(dialectGrammar)) {
          ...
          recommendations.push(
            `Normalized ${dialectName} grammar: "${dialectGrammar}" → "${standardGrammar}"`,
          );
        }
      }
    }
  ...
  async validateTextProcessing(data: any): Promise<number> {
    try {
      ...
      if (this.RTL_REGEX.test(dataStr)) {
        const hasRTLFormatting =
          dataStr.includes('dir="rtl"') ||
          dataStr.includes('class="arabic-text"');
        if (!hasRTLFormatting) {
  ...
  async generateValidationReport(
    ...
    const rtlCompliance = this.calculateRTLCompliance(
      text,
    ...
    const overallScore =
      (processingScore + rtlCompliance + dialectAccuracy) / 3;
   ...
   */
  private calculateRTLCompliance(
    originalText: string,
    processedText: string,
  ): number {
    ...
    if (
      this.MIXED_REGEX.test(originalText) &&
      processedText.includes("<span dir=")
    ) {
      score += 0.2;
    }
