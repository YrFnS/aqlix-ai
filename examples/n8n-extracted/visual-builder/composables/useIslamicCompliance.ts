...
export function useIslamicCompliance() {
  ...
  const ISLAMIC_BUSINESS_RULES: BusinessValidationRule[] = [
    {
      ...
      validator: (data) => {
        ...
        return !FORBIDDEN_KEYWORDS.slice(0, 4).some((keyword) =>
    ...
    },
    {
      ...
      validator: (data) => {
        const content = JSON.stringify(data).toLowerCase();
        return !["gambling", "lottery", "betting", "قمار", "مراهنة"].some(
    ...
    },
    {
      ...
      description:
        "Content must be appropriate according to Islamic guidelines",
      validator: (data) => {
        const content = JSON.stringify(data).toLowerCase();
        return !FORBIDDEN_KEYWORDS.slice(8, 12).some((keyword) =>
          content.includes(keyword),
        );
      },
      severity: "high",
    ...
    },
    {
      ...
      validator: (data) => {
        const content = JSON.stringify(data).toLowerCase();
        return !["fraud", "deception", "احتيال", "خداع"].some((keyword) =>
          content.includes(keyword),
        );
      },
      severity: "high",
  ...
  const PROFESSIONAL_DOMAIN_RULES = {
    health: [
      ...
      },
      {
        ...
        description:
          "Medical procedures must align with Islamic medical ethics",
        validator: (data: any) => {
          const content = JSON.stringify(data).toLowerCase();
          return !["unlawful_procedures", "non_emergency_friday"].some((term) =>
    ...
    justice: [
      {
        ...
        description:
          "Legal operations must be consistent with Islamic jurisprudence",
        validator: (data: any) => {
          const content = JSON.stringify(data).toLowerCase();
          return !["islamic_law_contradiction", "unfair_judgment"].some(
  ...
  const getCurrentPrayerStatus = (): {
    ...
    for (const [prayer, time] of prayers) {
      ...
      const timeDiff =
        Math.abs(currentTimeObj.getTime() - prayerTime.getTime()) / (1000 * 60);
  ...
  const validateNodeCompliance = async (
    ...
    for (const rule of ISLAMIC_BUSINESS_RULES) {
      if (!rule.validator(node)) {
        const issue: IslamicComplianceIssue = {
          ...
          arabicMessage: rule.arabicName
            ? `العقدة تنتهك القاعدة: ${rule.arabicName}`
            : undefined,
          nodeId: node.id,
          details: {
    ...
    const domainRules =
      PROFESSIONAL_DOMAIN_RULES[
        config.value
          .professionalDomain as keyof typeof PROFESSIONAL_DOMAIN_RULES
      ];
    if (domainRules) {
      for (const rule of domainRules) {
        if (!rule.validator(node)) {
          issues.push({
            ...
            arabicMessage: rule.arabicName
              ? `العقدة تنتهك قاعدة المجال المهني: ${rule.arabicName}`
              : undefined,
            nodeId: node.id,
            details: {
              violationType: rule.id,
    ...
    if (prayerStatus.isRestrictedTime) {
      issues.push({
        ...
        details: {
          ...
          islamicRuling:
            "Prayer times should be respected in workflow scheduling",
        },
        resolution: {
          required: false,
          alternatives: [
            "Schedule execution outside prayer times",
            "Add prayer time delays",
          ],
          islamicGuidance:
            "It is recommended to pause work activities during prayer times",
        },
      });
      timingScore -= 0.3;
    }
      ...
      if (fridayPrayerTime) {
        issues.push({
          ...
          details: {
            ...
            islamicRuling:
              "Friday prayer is obligatory and should not be interrupted",
          },
          resolution: {
            required: true,
            alternatives: [
              "Reschedule before/after Friday prayer",
              "Implement automatic delays",
            ],
            islamicGuidance:
              "Friday prayer attendance is obligatory for Muslim men",
          },
        });
        timingScore -= 0.5;
      }
      ...
      if (iftarTime) {
        recommendations.push({
          ...
          islamic: {
            ...
            contemporaryRuling:
              "Recommended to adjust work schedules during Ramadan",
          },
        });
      }
    }
  ...
  const validateWorkflowContent = async (workflow: IraqiWorkflow) => {
    ...
    for (const keyword of FORBIDDEN_KEYWORDS) {
      if (workflowContent.includes(keyword)) {
        const severity =
          keyword.includes("ربا") ||
          keyword.includes("riba") ||
        ...
        issues.push({
          ...
          type:
            keyword.includes("riba") || keyword.includes("interest")
              ? "riba"
              : "haram-content",
    ...
    for (const rule of ISLAMIC_BUSINESS_RULES) {
      if (!rule.validator(workflow)) {
        issues.push({
          ...
          arabicMessage: rule.arabicName
            ? `سير العمل يخالف قاعدة الأعمال الإسلامية: ${rule.arabicName}`
            : undefined,
          details: {
            violationType: rule.id,
            islamicRuling: rule.description,
    ...
    for (const rule of domainRules) {
      if (!rule.validator(workflow)) {
        issues.push({
          ...
          arabicMessage: rule.arabicName
            ? `سير العمل يخالف قاعدة المجال المهني: ${rule.arabicName}`
            : undefined,
          details: {
            violationType: rule.id,
            professionalContext: config.value.professionalDomain,
  ...
  const checkPrayerTimeConflicts = async (
    ...
    if (now.getDay() === 5 && prayerContext.current.fridayJumahTime) {
      const fridayTime = new Date(
        `1970-01-01T${prayerContext.current.fridayJumahTime}:00`,
      );
      const currentTimeObj = new Date(`1970-01-01T${currentTime}:00`);
      const timeDiff =
        Math.abs(currentTimeObj.getTime() - fridayTime.getTime()) / (1000 * 60);

      if (timeDiff <= 60) {
        // Within 1 hour of Friday prayer
        conflicts.push({
          type: "friday-prayer",
          severity: "high",
          prayer: "jumah",
          ...
          arabicRecommendation:
            "تجنب تنفيذ سير العمل قبل/بعد صلاة الجمعة بساعة",
        });
      }
    }
    ...
    if (prayerContext.ramadan?.isRamadan && prayerContext.ramadan.iftar) {
      const iftarTime = new Date(
        `1970-01-01T${prayerContext.ramadan.iftar}:00`,
      );
      const currentTimeObj = new Date(`1970-01-01T${currentTime}:00`);
      const timeDiff =
        Math.abs(currentTimeObj.getTime() - iftarTime.getTime()) / (1000 * 60);

      if (timeDiff <= 30) {
        // Within 30 minutes of Iftar
        conflicts.push({
          type: "ramadan-iftar",
          severity: "medium",
  ...
  const getIslamicRulingForKeyword = (keyword: string): string => {
    const rulings: Record<string, string> = {
      ...
      gambling:
        "Gambling is forbidden as it involves chance and can lead to addiction",
    ...
    return (
      rulings[keyword] ||
      "This content may not be appropriate according to Islamic guidelines"
    );
  };
  ...
  const getAlternativesForRule = (ruleId: string): string[] => {
    const alternatives: Record<string, string[]> = {
      ...
      "no-gambling": [
        ...
        "Fixed-outcome rewards",
        "Educational contests",
      ],
      "halal-content": [
        "Halal-certified products",
        "Islamic-compliant content",
        "Family-appropriate material",
      ],
      "honest-dealing": [
        "Transparent pricing",
  ...
  const getIslamicGuidanceForRule = (ruleId: string): string => {
    const guidance: Record<string, string> = {
      ...
      "halal-content":
        "Enjoin what is right and forbid what is wrong (Quran 3:104)",
  ...
  const getProfessionalGuidanceForRule = (ruleId: string): string => {
    const guidance: Record<string, string> = {
      "no-riba":
        "Use Islamic banking products and avoid interest-based transactions",
      "no-gambling": "Implement skill-based reward systems with clear criteria",
      "halal-content":
        "Ensure all content meets Islamic standards for appropriateness",
      "honest-dealing":
        "Maintain transparency and honesty in all business communications",
      "fair-pricing":
        "Use ethical pricing strategies that provide fair value to customers",
    };
    return (
      guidance[ruleId] ||
      "Follow professional ethics aligned with Islamic principles"
    );
  };
