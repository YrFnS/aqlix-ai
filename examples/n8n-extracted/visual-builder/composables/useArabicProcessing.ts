...
export function useArabicProcessing() {
  ...
  const PROFESSIONAL_TERMS = {
    health: {
      arabic: ['طبيب', 'مستشفى', 'علاج', 'دواء', 'مريض', 'صحة', 'عملية', 'فحص', 'تشخيص', 'وصفة'],
      english: ['doctor', 'hospital', 'treatment', 'medicine', 'patient', 'health', 'surgery', 'examination', 'diagnosis', 'prescription'],
      bilingual: new Map([
        ['طبيب', 'doctor'],
        ['مستشفى', 'hospital'],
        ['علاج', 'treatment'],
    ...
    education: [
      {
        arabic: ['مدرس', 'طالب', 'مدرسة', 'جامعة', 'درس', 'امتحان', 'كتاب', 'تعليم', 'منهج', 'شهادة'],
        english: ['teacher', 'student', 'school', 'university', 'lesson', 'exam', 'book', 'education', 'curriculum', 'certificate'],
        bilingual: new Map([
          ['مدرس', 'teacher'],
          ['طالب', 'student'],
          ['مدرسة', 'school'],
    ...
    government: {
      arabic: ['وزارة', 'حكومة', 'مواطن', 'هوية', 'جواز', 'وثيقة', 'خدمة', 'إجراء', 'طلب', 'موافقة'],
      english: ['ministry', 'government', 'citizen', 'identity', 'passport', 'document', 'service', 'procedure', 'application', 'approval'],
      bilingual: new Map([
        ['وزارة', 'ministry'],
        ['حكومة', 'government'],
        ['مواطن', 'citizen'],
  ...
  const analyzeLanguageComposition = (text: string) => {
    ...
    return {
      ...
      other: totalChars > 0 ? (totalChars - arabicChars - latinChars - numeralChars) / totalChars : 0
    }
  }
  ...
  const detectDialect = (text: string): ArabicDialect => {
    ...
    Object.entries(DIALECT_PATTERNS).forEach(([dialectName, patterns]) => {
      patterns.forEach(pattern => {
        ...
        if (matches) {
          ...
          characteristics.lexical.push(...pattern.examples.filter(ex => text.includes(ex)))
        }
      })
    })
    ...
    const hasFormalTerms = PROFESSIONAL_TERMS.government.arabic.some(term => text.includes(term))
    
    let formalRegister: 'colloquial' | 'formal' | 'professional' | 'academic' = 'colloquial'
  ...
  const segmentMixedText = (text: string): ArabicTextSegment[] => {
    ...
    let currentSegment = ''
    let currentType: 'arabic' | 'english' | 'number' | 'punctuation' = 'english'
    let startIndex = 0
  ...
  // RTL text formatting
  const formatRTLText = (text: string, segments?: ArabicTextSegment[]): string => {
    ...
    if (terms.bilingual) {
      terms.bilingual.forEach((english, arabic) => {
        if (text.includes(arabic) && !text.includes(english)) {
          enhancements.push(`Add English term "${english}" for Arabic "${arabic}"`)
        }
        if (text.includes(english) && !text.includes(arabic)) {
          enhancements.push(`Add Arabic term "${arabic}" for English "${english}"`)
        }
      })
    }
  ...
  const processArabicText = async (
      ...
      return {
        ...
        analysis: {
          ...
          dialect: {
            ...
            secondary: [],
            confidence: 0,
            characteristics: { phonetic: [], lexical: [], grammatical: [], cultural: [] },
            professional: { domainTerminology: [], formalRegister: 'colloquial', audienceAppropriate: true }
          },
          confidence: 0,
    ...
    try {
      ...
      const terminologyResult = enhanceProfessionalTerminology(processedText, domain)
      ...
      if (direction === 'mixed' && segments.length > 8) {
        issues.push({
          ...
          location: { start: 0, end: text.length, context: text.substring(0, 50) },
          resolution: {
            required: false,
            suggestions: ['Reduce language switching', 'Group similar language content'],
            arabicSuggestions: ['قلل التبديل بين اللغات', 'اجمع المحتوى باللغة المشابهة'],
            automaticFix: false
          }
        })
      }
      ...
      if (dialect.confidence < 0.5 && languageRatio.arabic > 0.3) {
        issues.push({
          ...
          resolution: {
            required: false,
            suggestions: ['Use more consistent dialect patterns', 'Consider standard Arabic'],
            arabicSuggestions: ['استخدم أنماط لهجة أكثر اتساقاً', 'فكر في العربية الفصحى'],
            automaticFix: false
          }
        })
      }
      ...
      if (terminologyResult.enhancements.length > 0) {
        recommendations.push({
          ...
          priority: 'medium',
          message: 'Professional terminology can be enhanced with bilingual labels',
          arabicMessage: 'يمكن تحسين المصطلحات المهنية بإضافة التسميات ثنائية اللغة',
          implementation: {
            effort: 'minimal',
            impact: 'medium',
      ...
      if (segments.length > 5 || dialect.secondary?.length > 0) complexity = 'moderate'
  ...
  const validateRTLLayout = (html: string): { score: number; issues: string[] } => {
    const issues: string[] = []
    let score = 1.0
    ...
    if (arabicContent) {
      const withDirAttribute = html.match(/<[^>]*dir\s*=\s*["']rtl["'][^>]*>.*?[\u0600-\u06FF].*?<\/[^>]*>/g)
    ...
    const mixedContent = html.match(/<span[^>]*dir\s*=\s*["'](ltr|rtl)["'][^>]*>/g)
  ...
  return {
    ...
    // Computed
    isConfigured: computed(() => config.value.enableRTL || config.value.dialectRecognition),
