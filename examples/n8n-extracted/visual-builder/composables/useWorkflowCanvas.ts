...
export function useWorkflowCanvas(workflow: IraqiWorkflow) {
  ...
  const viewport = reactive<CanvasViewport>({
    ...
    rtl: {
      ...
      arabicFontEnabled: workflow.layout.primaryLanguage === 'arabic' || workflow.layout.primaryLanguage === 'bilingual',
    ...
    cultural: {
      ...
      showPrayerTimeIndicators: workflow.scheduling?.prayerTimeExclusions || false,
      culturalTheme: workflow.domain?.type !== 'general' ? 'ministry' : 'default'
    }
  })
  ...
  const temporaryConnection = ref<Partial<IraqiWorkflowConnection> | null>(null)
  ...
  const hasSelection = computed(() => selectedNodes.value.length > 0 || selectedConnections.value.length > 0)
  ...
  const zoomIn = () => {
    ...
    zoomToPoint({ x: window.innerWidth / 2, y: window.innerHeight / 2 }, newZoom)
  }
  ...
  const selectNodesInRect = (rect: SelectionRect) => {
    ...
    const nodesInRect = workflow.nodes.filter(node => {
      ...
      return nodeX >= minX && nodeX + nodeSize <= maxX &&
             nodeY >= minY && nodeY + nodeSize <= maxY
    })
  ...
  const moveNode = (nodeId: string, newPosition: { x: number; y: number }) => {
    ...
    if (selectedNodes.value.length > 1 && selectedNodes.value.includes(nodeId)) {
  ...
  const getNodeAt = (x: number, y: number): IraqiWorkflowNode | null => {
    ...
    for (const node of workflow.nodes) {
      const nodeSize = 150 // Approximate node size
      if (x >= node.position.x && x <= node.position.x + nodeSize &&
          y >= node.position.y && y <= node.position.y + nodeSize) {
        return node
      }
    }
    return null
  ...
  const getConnectionAt = (x: number, y: number): IraqiWorkflowConnection | null => {
