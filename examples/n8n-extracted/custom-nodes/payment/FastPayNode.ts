...
interface IFastPayStatusResponse {
  paymentId: string;
  orderId: string;
  status: 'created' | 'pending' | 'processing' | 'completed' | 'failed' | 'cancelled' | 'refunded' | 'disputed';
  amount: number;
  currency: 'IQD';
  fees: number;
  netAmount: number;
...
export class FastPayNode extends IraqiGovernmentNodeBase {
  description: INodeTypeDescription = {
    ...
    description: 'Process payments through FastPay digital gateway with Islamic compliance',
    descriptionArabic: 'معالجة المدفوعات عبر بوابة فاست باي الرقمية مع الامتثال الإسلامي',
    defaults: {
    ...
    properties: [
      ...
      },
      {
        ...
        descriptionArabic: 'مبلغ الدفع بالدينار العراقي (الحد الأدنى 500 دينار)',
        displayOptions: {
          show: {
      ...
      },
      {
        ...
        default: '',
        required: true,
        placeholder: 'FP-{{$now.format("YYYYMMDD")}}-{{$randomInt(10000, 99999)}}',
      ...
      },
      {
        ...
        type: 'boolean',
        default: true,
        description: 'Enable device fingerprinting and advanced fraud detection'
      },
      {
      ...
      {
        displayName: 'AML Compliance Level / مستوى الامتثال لمكافحة غسيل الأموال',
  ...
  private async createPayment(itemIndex: number): Promise<IFastPayPaymentResponse> {
    ...
    try {
      ...
      const result: IFastPayPaymentResponse = {
        status: response.data.status === 'created' ? 'success' : response.data.status,
  ...
  private async checkStatus(itemIndex: number): Promise<IFastPayStatusResponse> {
  ...
  private async processRefund(itemIndex: number): Promise<any> {
    ...
    const reasonArabic = this.getNodeParameter('refundReasonArabic', itemIndex, '') as string;
  ...
  private async tokenizeCard(itemIndex: number): Promise<any> {
    ...
    const expiryMonth = this.getNodeParameter('expiryMonth', itemIndex) as string;
  ...
  private verifyWebhookSignature(payload: string, signature: string, timestamp: string): boolean {
  ...
  private async performAdvancedSecurity(data: any): Promise<any> {
    ...
    if (data.amount > 50000000) { // 50M IQD
      fraudScore += 40;
      flags.push('Very large amount');
    } else if (data.amount > 10000000) { // 10M IQD
      fraudScore += 20;
    ...
    // AML screening
    let amlStatus: 'clear' | 'review' | 'flagged' = 'clear';
    if (data.amount > 25000000) { // 25M IQD requires enhanced AML
      amlStatus = 'review';
  ...
  private async validateIslamicTransaction(data: any): Promise<any> {
    ...
    if (data.amount > 500000000) { // 500M IQD
      complianceScore -= 15;
      violations.push('Excessive amount may violate Islamic modesty principles');
    }
  ...
  private generateDeviceFingerprint(): string {
    ...
    return createHash('sha256').update(`${timestamp}-${random}-fastpay`).digest('hex').substring(0, 16);
  }
  ...
  private async validatePaymentRequest(request: IFastPayPaymentRequest): Promise<any> {
    const errors: string[] = [];
    ...
    const culturalScore = errors.length === 0 ? 100 : Math.max(0, 100 - (errors.length * 20));
