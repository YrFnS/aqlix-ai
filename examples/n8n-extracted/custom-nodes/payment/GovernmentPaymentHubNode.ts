...
export class GovernmentPaymentHubNode extends IraqiGovernmentNodeBase {
  description: INodeTypeDescription = {
    ...
    version: 1,
    subtitle:
      '={{$parameter["operation"] + " via " + ($parameter["preferredGateway"] || "Auto-Select")}}',
    description:
      "Unified payment orchestration with intelligent gateway routing",
    ...
    properties: [
      ...
      },
      {
        ...
        required: true,
        placeholder:
          'HUB-{{$now.format("YYYYMMDD")}}-{{$randomInt(100000, 999999)}}',
      ...
      },
      {
        ...
        default: "auto",
        description:
          "Preferred payment gateway (auto-select for optimal routing)",
      },
      {
        ...
        options: [
          ...
          {
            name: "Performance First / الأداء أولاً",
            value: "performance_first",
          },
          {
            name: "Reliability Focus / التركيز على الموثوقية",
  ...
  private async processPayment(itemIndex: number): Promise<IPaymentResponse> {
    ...
    const paymentRequest: IPaymentRequest = {
      ...
      customerPhone: this.getNodeParameter(
        "customerPhone",
        itemIndex,
      ) as string,
      ...
      preferredGateway: this.getNodeParameter(
        "preferredGateway",
        itemIndex,
        "auto",
      ) as string,
    ...
    // Fraud detection
    const enableFraudDetection = this.getNodeParameter(
    ...
    if (enableFraudDetection) {
      const fraudResult =
        await this.performCrossGatewayFraudDetection(paymentRequest);
    ...
    // Gateway selection
    const routingStrategy = this.getNodeParameter(
      "routingStrategy",
      itemIndex,
      "balanced",
    ) as string;
    ...
    const enableFailover = this.getNodeParameter(
      "enableFailover",
      itemIndex,
      true,
    ) as boolean;
    ...
    try {
      paymentResult = await this.processPaymentThroughGateway(
        paymentRequest,
        selectedGateway.id,
      );
    } catch (error) {
      if (enableFailover && failoverCount < 2) {
        ...
        const alternativeGateways = this.getAlternativeGateways(
          selectedGateway.id,
          paymentRequest,
        );
    ...
    const analytics = this.calculatePaymentAnalytics(
      selectedGateway,
      paymentResult,
      processingTime,
    );
    ...
    const culturalCompliance =
      await this.validateCulturalCompliance(paymentRequest);
    ...
    const response: IPaymentResponse = {
      ...
      netAmount:
        ...
        this.calculateFees(paymentRequest.amount, selectedGateway),
      paymentUrl: paymentResult.paymentUrl,
      qrCode: paymentResult.qrCode,
      expiresAt:
        paymentResult.expiresAt || new Date(Date.now() + 15 * 60 * 1000),
      processingTime,
      routing: {
  ...
  private async checkPaymentStatus(itemIndex: number): Promise<any> {
    ...
    const statusResults = await Promise.allSettled(
      this.availableGateways.map((gateway) =>
        this.checkStatusOnGateway(orderId, gateway.id),
      ),
    );
  ...
  private async checkGatewayHealth(itemIndex: number): Promise<any> {
    const healthChecks = await Promise.all(
      this.availableGateways.map((gateway) =>
        this.checkGatewayHealthStatus(gateway),
      ),
    );
  ...
  private async generateAnalytics(
    itemIndex: number,
  ): Promise<IGatewayAnalytics[]> {
    const period = this.getNodeParameter(
      "analyticsPeriod",
      itemIndex,
      "daily",
    ) as any;
    ...
    const analytics = await Promise.all(
      this.availableGateways.map((gateway) =>
        this.generateGatewayAnalytics(gateway, period),
      ),
    );
  ...
  private async selectOptimalGateway(
    ...
    if (request.preferredGateway && request.preferredGateway !== "auto") {
      const preferred = eligibleGateways.find(
        (g) => g.id === request.preferredGateway,
      );
    ...
    switch (strategy) {
      case "cost_optimized":
        return this.selectCostOptimizedGateway(
          eligibleGateways,
          request.amount,
        );
  ...
  private selectCostOptimizedGateway(
  ...
  private selectPerformanceGateway(
    gateways: IPaymentGateway[],
  ): IPaymentGateway {
    return gateways.reduce((best, current) => {
      const bestScore =
        best.performance.successRate * 0.7 +
        (2000 - best.performance.averageResponseTime) * 0.3;
      const currentScore =
        current.performance.successRate * 0.7 +
        (2000 - current.performance.averageResponseTime) * 0.3;
  ...
  private selectReliableGateway(gateways: IPaymentGateway[]): IPaymentGateway {
    return gateways.reduce((best, current) => {
      const bestScore =
        best.performance.uptime * 0.6 + best.performance.successRate * 0.4;
      const currentScore =
        current.performance.uptime * 0.6 +
        current.performance.successRate * 0.4;
      return currentScore > bestScore ? current : best;
    });
  }

  private selectBalancedGateway(
    gateways: IPaymentGateway[],
    amount: number,
  ): IPaymentGateway {
    return gateways.reduce((best, current) => {
  ...
  private getAlternativeGateways(
  ...
  private async processPaymentThroughGateway(
    ...
    return {
      ...
      fees: this.calculateFees(
        request.amount,
        this.availableGateways.find((g) => g.id === gatewayId)!,
      ),
    };
  }

  private async checkStatusOnGateway(
    orderId: string,
    gatewayId: string,
  ): Promise<any> {
  ...
  private calculateFees(amount: number, gateway: IPaymentGateway): number {
    const percentageFee = Math.min(
      (amount * (gateway.fees.percentage - gateway.fees.governmentDiscount)) /
        100,
      gateway.fees.maximum,
    );
  ...
  private calculatePaymentAnalytics(
    ...
    return {
      ...
      overallScore:
        (gateway.performance.successRate + gateway.performance.uptime) / 2,
    };
  }
  ...
  private async performCrossGatewayFraudDetection(
    request: IPaymentRequest,
  ): Promise<any> {
    let score = 0;
    const flags: string[] = [];
  ...
  private async checkGatewayHealthStatus(
    gateway: IPaymentGateway,
  ): Promise<any> {
  ...
  private calculateOverallHealth(healthChecks: any[]): number {
    const totalHealth = healthChecks.reduce(
      (sum, check) => sum + check.healthScore,
      0,
    );
  ...
  private async generateGatewayAnalytics(
    gateway: IPaymentGateway,
    period: string,
  ): Promise<IGatewayAnalytics> {
  ...
  private analyzeRoutingOptimization(analytics: IGatewayAnalytics[]): any {
    return {
      current: {
        averageSuccessRate:
          analytics.reduce((sum, a) => sum + a.metrics.successRate, 0) /
          analytics.length,
        averageCost:
          analytics.reduce((sum, a) => sum + a.costs.costPerTransaction, 0) /
          analytics.length,
      },
      strategy: "balanced",
      improvement: {
