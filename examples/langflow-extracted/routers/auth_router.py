...
class LoginRequest(BaseModel):
    ...
    phone_number: Optional[str] = Field(
        None, description="Iraqi phone number (+964 format)"
    )
    ...
    def at_least_one_identifier(cls, v, values):
        if not any(
            [values.get("email"), values.get("phone_number"), values.get("username"), v]
        ):
            raise ValueError(
                "At least one identifier (email, phone, username) required"
            )
        return v
...
class LoginResponse(BaseModel):
    ...
    professional_verified: bool = Field(
        False, description="Professional credentials verified"
    )
    government_authorized: bool = Field(
        False, description="Government access authorized"
    )
...
class RegisterRequest(BaseModel):
    ...
    cultural_sensitivity_level: str = Field(
        "high", description="Cultural sensitivity level"
    )
...
class TokenValidationResponse(BaseModel):
    ...
    cultural_context: Dict[str, Any] = Field(
        default_factory=dict, description="Cultural context"
    )
...
class APIKeyRequest(BaseModel):
    ...
    iraqi_compliance_level: str = Field(
        "basic", description="Iraqi regulatory compliance level"
    )
    professional_access: Optional[str] = Field(
        None, description="Professional domain access"
    )
    ...
    expires_in_days: Optional[int] = Field(
        90, ge=1, le=365, description="Expiration in days"
    )
...
class APIKeyResponse(BaseModel):
    ...
    professional_access_authorized: bool = Field(
        False, description="Professional access"
    )
...
async def login(
    ...
    try:
        ...
        await check_rate_limit(
            http_request, "auth_login", limit=5, window=300
        ...
        if request.email:
            ...
            user = (
                db.query(User).filter(User.phone_number == request.phone_number).first()
            )
        ...
        if not bcrypt.checkpw(
            ...
            await log_security_event(
                "login_failed", "invalid_password", http_request, user.id
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid credentials"
            )
        ...
        if user.mfa_enabled and request.mfa_token:
            if not await verify_mfa_token(user, request.mfa_token):
                await log_security_event(
                    "mfa_failed", "invalid_token", http_request, user.id
                )
                raise HTTPException(
        ...
        await log_security_event(
            "login_success", "user_authenticated", http_request, user.id
        )
...
async def register(
    ...
    try:
        ...
        # Log registration
        await log_security_event(
            "user_registered", "new_user_created", http_request, new_user.id
        )
        ...
        await log_security_event(
            "registration_error", f"system_error: {str(e)}", http_request
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def logout(
    ...
    try:
        ...
        # Log logout event
        await log_security_event(
            "user_logout", "session_terminated", None, current_user.id
        )
...
async def get_current_user_info(
    ...
    try:
        ...
        return {
            ...
            "cultural_settings": cultural_settings.to_dict()
...
async def create_api_key(
    ...
    try:
        ...
        api_key, key_hash = APIKey.generate_api_key(
            key_type=request.key_type, prefix="aq"
        )
        ...
        new_api_key = APIKey(
            ...
            professional_domain_access={
                "authorized": bool(request.professional_access)
            },
        ...
        await log_security_event(
            "api_key_created", f"key_name: {request.key_name}", None, current_user.id
        )
        ...
        return APIKeyResponse(
            ...
            professional_access_authorized=new_api_key.legal_access_authorized
            or new_api_key.medical_access_authorized
            or new_api_key.educational_access_authorized,
        )
...
async def log_security_event(
    event_type: str, details: str, request: Request, user_id: str = None
):
