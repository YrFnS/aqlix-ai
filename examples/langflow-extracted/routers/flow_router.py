...
# Core imports
from ..models.flow_models import Flow, FlowExecution, FlowVersion, FlowTemplate, IraqiFlowContext
...
flow_router = APIRouter(
    ...
    responses={
        400: {"description": "Bad Request - Invalid workflow or cultural non-compliance"},
        401: {"description": "Unauthorized - Authentication required"},
        403: {"description": "Forbidden - Workflow access denied or cultural restrictions"},
...
class FlowNodeRequest(BaseModel):
    ...
    description: Optional[str] = Field(None, max_length=500, description="Node description")
    ...
    cultural_validation_required: bool = Field(default=True, description="Require cultural validation")
    professional_context: Optional[str] = Field(None, description="Professional domain context")
...
class FlowCreateRequest(BaseModel):
    ...
    professional_type: Optional[ProfessionalWorkflowType] = Field(None, description="Professional workflow type")
    professional_domain: Optional[str] = Field(None, description="Professional domain")
    nodes: List[FlowNodeRequest] = Field(..., min_items=1, max_items=MAX_WORKFLOW_NODES, description="Workflow nodes")
    connections: List[FlowConnectionRequest] = Field(default=[], description="Node connections")
    input_schema: Dict[str, Any] = Field(default={}, description="Workflow input schema")
    output_schema: Dict[str, Any] = Field(default={}, description="Workflow output schema")
    cultural_compliance_level: str = Field(default="high", description="Cultural compliance level")
    is_public: bool = Field(default=False, description="Make workflow publicly accessible")
...
class FlowExecutionRequest(BaseModel):
    ...
    execution_mode: str = Field(default="sync", description="Execution mode (sync/async)")
    cultural_validation: bool = Field(default=True, description="Enable cultural validation")
    professional_context: Optional[str] = Field(None, description="Professional context")
    timeout_minutes: int = Field(default=30, ge=1, le=MAX_EXECUTION_TIME_MINUTES, description="Execution timeout")
    priority: int = Field(default=0, ge=0, le=10, description="Execution priority")

class FlowUpdateRequest(BaseModel):
    """Workflow update request"""
    name: Optional[str] = Field(None, min_length=2, max_length=100, description="Workflow name")
    description: Optional[str] = Field(None, max_length=1000, description="Workflow description")
    ...
    connections: Optional[List[FlowConnectionRequest]] = Field(None, description="Updated connections")
    input_schema: Optional[Dict[str, Any]] = Field(None, description="Updated input schema")
    output_schema: Optional[Dict[str, Any]] = Field(None, description="Updated output schema")
...
async def create_workflow(
    ...
    try:
        ...
        for node in request.nodes:
            ...
            if node.description and any('\u0600' <= char <= '\u06FF' for char in node.description):
                processed_arabic = await arabic_processor.process_text(
                    node.description,
        ...
        iraqi_context = IraqiFlowContext(
            ...
            professional_context_data=validation_result.get("professional_analysis", {}),
            arabic_processing_metadata={
                "nodes_with_arabic": len([n for n in processed_nodes if any('\u0600' <= char <= '\u06FF' for char in str(n.get("name", "")))]),
                "dialect_support_required": any("iraqi" in str(n).lower() for n in processed_nodes)
            },
            created_at=datetime.utcnow()
        )
        ...
        raise HTTPException(
            status_code=500,
            detail="Workflow creation failed due to system error"
        )
    ...
    try:
        ...
        if flow.created_by != current_user.id and not flow.is_public:
            if current_user.role not in ["admin", "moderator"]:
                raise HTTPException(
                    status_code=403,
                    detail="Access denied to private workflow"
                )
...
async def update_workflow(
    ...
    try:
        ...
        if request.nodes or request.connections:
            ...
            nodes_to_validate = request.nodes if request.nodes else [FlowNodeRequest(**node) for node in flow.nodes]
            connections_to_validate = request.connections if request.connections else [FlowConnectionRequest(**conn) for conn in flow.connections]
        ...
        if any("name" in change or "description" in change or "nodes" in change for change in changes_made):
            workflow_text = f"{flow.name} {flow.description or ''}"
            for node in flow.nodes:
                workflow_text += f" {node.get('name', '')} {node.get('description', '')}"
        ...
        raise HTTPException(
            status_code=500,
            detail="Workflow update failed due to system error"
        )
    ...
    try:
        ...
        if flow.created_by != current_user.id and not flow.is_public:
            if current_user.role not in ["admin", "moderator"]:
                raise HTTPException(
                    status_code=403,
                    detail="Access denied to execute private workflow"
                )
        ...
        if request.cultural_validation:
            input_text = " ".join(str(v) for v in request.input_data.values() if isinstance(v, str))
        ...
        execution = FlowExecution(
            ...
            input_data=request.input_data,
            cultural_compliance_score=cultural_compliance_score,
            professional_context=request.professional_context or flow.professional_domain,
        ...
        if request.execution_mode == "async":
            ...
            execution.execution_time_seconds = execution_result.get("execution_time_seconds")
            execution.professional_analysis = execution_result.get("professional_analysis")
            ...
            if execution_result["status"] == ExecutionStatus.COMPLETED:
                if flow.average_execution_time:
                    flow.average_execution_time = (flow.average_execution_time + execution_result["execution_time_seconds"]) / 2
                else:
                    flow.average_execution_time = execution_result["execution_time_seconds"]
        ...
        raise HTTPException(
            status_code=500,
            detail="Workflow execution failed due to system error"
        )
...
async def get_workflow_executions(
    ...
    try:
        ...
        # Apply pagination
        offset = (page - 1) * size
        executions = query.order_by(FlowExecution.started_at.desc()).offset(offset).limit(size).all()
        
        # Convert to response format
        execution_responses = []
        for execution in executions:
            execution_responses.append(FlowExecutionResponse(
                execution_id=execution.id,
                flow_id=execution.flow_id,
                status=execution.status,
                input_data=execution.input_data if execution.user_id == current_user.id else {"[filtered]": "privacy_protected"},
                output_data=execution.output_data if execution.user_id == current_user.id else {"[filtered]": "privacy_protected"},
                execution_time_seconds=execution.execution_time_seconds,
                cultural_compliance_score=execution.cultural_compliance_score,
                professional_analysis=execution.professional_analysis,
                error_message=execution.error_message,
                started_at=execution.started_at,
                completed_at=execution.completed_at,
                node_executions=execution.node_executions[:5] if execution.user_id == current_user.id else []  # Privacy protection
            ))
...
async def get_professional_workflow_templates(
    professional_domain: Optional[str] = Query(None, description="Filter by professional domain"),
    professional_type: Optional[ProfessionalWorkflowType] = Query(None, description="Filter by workflow type"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    try:
        ...
        if professional_domain:
            query = query.filter(FlowTemplate.professional_domain == professional_domain)
        ...
        for template in templates:
            template_responses.append(FlowTemplateResponse(
                id=template.id,
                name=template.name,
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve professional templates"
        )

@flow_router.post("/templates/{template_id}/instantiate", response_model=FlowResponse, status_code=201)
async def instantiate_workflow_template(
    ...
    try:
        ...
        if template.professional_domain:
            domain_access = await professional_validator.validate_user_domain_access(
                user_id=current_user.id,
                domain=template.professional_domain,
                db=db
            )
        ...
        raise HTTPException(
            status_code=500,
            detail="Template instantiation failed due to system error"
        )
...
async def list_workflows(
    ...
    cultural_compliance_min: Optional[float] = Query(None, ge=0.0, le=1.0, description="Minimum cultural compliance"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
) -> FlowListResponse:
    """
    ...
    try:
        ...
        if cultural_compliance_min:
            query = query.filter(Flow.cultural_compliance_score >= cultural_compliance_min)
        if tags:
        ...
        for flow in flows:
            flow_responses.append(FlowResponse(
                ...
                version=flow.version,
                cultural_compliance_score=flow.cultural_compliance_score,
                nodes=flow.nodes if flow.created_by == current_user.id else [],  # Privacy protection
                connections=flow.connections if flow.created_by == current_user.id else [],
                input_schema=flow.input_schema,
                output_schema=flow.output_schema,
                is_public=flow.is_public,
                tags=flow.tags,
        ...
        raise HTTPException(
            status_code=500,
            detail="Workflow listing failed"
        )
...
async def delete_workflow(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Workflow deletion failed"
        )

# === Workflow Execution Helper Functions ===

async def _execute_workflow_sync(execution_id: int, flow: Flow, input_data: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "output_data": None,
            "execution_time_seconds": (datetime.utcnow() - start_time).total_seconds() if 'start_time' in locals() else 0,
            "professional_analysis": None,
            "error_message": str(e),
            "node_executions": []
        }

async def _execute_workflow_async(execution_id: int, flow: Flow, input_data: Dict[str, Any], context: Dict[str, Any], db: Session):
    ...
    try:
        ...
        execution_result = await _execute_workflow_sync(execution_id, flow, input_data, context)
