...
from ..models.ocr_models import ArabicOCRResult, ProfessionalTemplate, DocumentGeneration
...
file_router = APIRouter(
    prefix="/files",
    tags=["File Processing", "Arabic OCR", "Document Generation", "Cultural Compliance"],
    responses={
...
class FileUploadRequest(BaseModel):
    ...
    ocr_language: OCRLanguage = Field(default=OCRLanguage.MIXED, description="OCR language preference")
    auto_generate_metadata: bool = Field(default=True, description="Auto-generate file metadata")
...
class OCRRequest(BaseModel):
    ...
    cultural_validation: bool = Field(default=True, description="Validate extracted text culturally")
...
async def upload_file(
    ...
    professional_context: Optional[str] = Form(None, description="Professional context"),
    cultural_validation_required: bool = Form(True, description="Require cultural validation"),
    ...
    try:
        ...
        user_file_count = db.query(FileStorage).filter(
            and_(
        ...
        if not _is_file_type_allowed(file_type_detected):
            raise HTTPException(
                status_code=400,
                detail=f"File type not allowed: {file_type_detected}"
            )
        ...
        if cultural_validation_required and file_category in [FileType.IMAGE, FileType.DOCUMENT]:
            try:
                # Extract text if document or perform image analysis
                extracted_text = ""
                if file_category == FileType.DOCUMENT:
                    extracted_text = await _extract_text_from_document(file_content, file_type_detected)
                ...
                if extracted_text:
                    ...
                    if cultural_compliance_score < 0.70:
                        file_record.processing_status = ProcessingStatus.CULTURALLY_RESTRICTED
                    ...
                    if not islamic_validation["compliant"]:
                        ...
                        metadata["islamic_compliance_issues"] = islamic_validation["violations"]
                    
            except Exception as e:
                logger.warning(f"Cultural validation failed for file {file_record.id}: {str(e)}")
        ...
        if file_category in [FileType.IMAGE, FileType.DOCUMENT] and cultural_compliance_score >= 0.70:
            asyncio.create_task(_schedule_ocr_processing(
                file_record.id,
                file_content,
                ocr_language,
        ...
        raise HTTPException(
            status_code=500,
            detail="File upload failed due to system error"
        )
    ...
    try:
        ...
        if file_record.user_id != current_user.id and not file_record.is_public:
            if current_user.role not in ["admin", "moderator"]:
                raise HTTPException(
                    status_code=403,
                    detail="Access denied to private file"
                )
        ...
        ocr_results = None
        ocr_record = db.query(ArabicOCRResult).filter(
            ArabicOCRResult.file_id == file_id
        ).first()
...
async def process_ocr(
    ...
    try:
        ...
        existing_ocr = db.query(ArabicOCRResult).filter(
            ArabicOCRResult.file_id == file_id
        ).first()
        ...
        if request.professional_context and extracted_text.strip():
            professional_context = await professional_validator.analyze_document_context(
                extracted_text,
        ...
        raise HTTPException(
            status_code=500,
            detail="OCR processing failed due to system error"
        )

# === Document Generation Endpoints ===

@file_router.post("/generate-document", response_model=DocumentGenerationResponse, status_code=201)
async def generate_document(
    ...
    try:
        ...
        content_text = " ".join(str(v) for v in request.content_data.values() if isinstance(v, str))
        ...
        raise HTTPException(
            status_code=500,
            detail="Document generation failed due to system error"
        )
    ...
    try:
        ...
        return {
            ...
            "professional_domains": [
                "legal", "medical", "educational", "governmental", 
...
async def list_user_files(
    ...
    cultural_compliance_min: Optional[float] = Query(None, ge=0.0, le=1.0, description="Minimum cultural compliance score"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
) -> FileListResponse:
    """
    ...
    try:
        ...
        if cultural_compliance_min:
            query = query.filter(FileStorage.cultural_compliance_score >= cultural_compliance_min)
        if tags:
        ...
        # Apply pagination
        offset = (page - 1) * size
        files = query.order_by(FileStorage.created_at.desc()).offset(offset).limit(size).all()
        ...
        for file_record in files:
            ...
            if ocr_record:
                ocr_results = {
                    "extracted_text": ocr_record.extracted_text[:200] + "..." if len(ocr_record.extracted_text) > 200 else ocr_record.extracted_text,
                    "confidence_score": ocr_record.confidence_score,
                    "language_detected": ocr_record.language_detected
                }
            ...
            file_responses.append(FileInfoResponse(
                ...
                expires_at=file_record.expires_at,
                ocr_results=ocr_results,
                metadata={k: v for k, v in file_record.metadata.items() if k not in ["file_hash", "upload_ip"]},  # Privacy protection
                download_count=file_record.download_count
            ))
...
async def cleanup_expired_files(
    ...
    try:
        ...
        if current_user.role != "admin":
            raise HTTPException(
                status_code=403,
                detail="Admin access required for file cleanup"
            )
        
        # Find expired files
        expired_files = db.query(FileStorage).filter(
            FileStorage.expires_at < datetime.utcnow()
        ).all()
...
async def _generate_file_metadata(file_content: bytes, mime_type: str, file_category: FileType, user_id: int) -> Dict[str, Any]:
    """Generate comprehensive file metadata"""
    metadata = {
        "content_analysis": {
            "character_count": len(file_content),
            "estimated_pages": max(1, len(file_content) // 3000) if file_category == FileType.TEXT else None
        }
    }
...
async def _schedule_ocr_processing(file_id: int, file_content: bytes, language: OCRLanguage, professional_context: Optional[str]):
