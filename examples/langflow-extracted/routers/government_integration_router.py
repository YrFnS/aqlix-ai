...
from ..services.document_verification_service import (
    GovernmentDocumentVerificationService,
)
...
class GovernmentServiceRequest(BaseModel):
    ...
    ministry: MinistryDepartment = Field(
        ..., description="Target ministry or department"
    )
    ...
    priority_level: PriorityLevel = Field(
        default=PriorityLevel.NORMAL, description="Processing priority"
    )
    ...
    # Additional Options
    expedited_processing: bool = Field(
        default=False, description="Request expedited processing"
    )
    digital_signature: bool = Field(default=False, description="Use digital signature")
    biometric_verification: bool = Field(
        default=False, description="Require biometric verification"
    )
    ...
    def validate_applicant_info(cls, v):
        ...
        if v.get("nationality") == "Iraqi":
            ...
            if not re.match(r"^\d{12}$", national_id):
                raise ValueError(
                    "Invalid Iraqi national ID format (12 digits required)"
                )
...
class DocumentUploadRequest(BaseModel):
    ...
    require_authentication: bool = Field(
        default=True, description="Require document authentication"
    )
    verify_against_database: bool = Field(
        default=True, description="Cross-verify with government databases"
    )
...
class FormAutomationRequest(BaseModel):
    ...
    auto_populate: bool = Field(
        default=True, description="Auto-populate from citizen profile"
    )
    validate_before_submit: bool = Field(
        default=True, description="Validate form before submission"
    )

    # Processing Options
    save_as_draft: bool = Field(
        default=False, description="Save as draft instead of submitting"
    )
    schedule_submission: Optional[datetime] = Field(
        None, description="Schedule future submission"
    )
...
class GovernmentServiceResponse(BaseModel):
    ...
    required_actions: List[str] = Field(
        default_factory=list, description="Required actions from applicant"
    )
    required_documents: List[str] = Field(
        default_factory=list, description="Additional required documents"
    )
    ...
    contact_information: Dict[str, str] = Field(
        ..., description="Ministry contact details"
    )
...
class DocumentUploadResponse(BaseModel):
    ...
    authentication_status: Optional[bool] = Field(
        None, description="Document authentication result"
    )
...
class StatusCheckResponse(BaseModel):
    ...
    pending_actions: List[str] = Field(
        default_factory=list, description="Pending applicant actions"
    )
    ...
    # Communication
    recent_notifications: List[Dict[str, Any]] = Field(
        ..., description="Recent notifications"
    )
    next_contact_date: Optional[datetime] = Field(
        None, description="Next scheduled contact"
    )
...
class FeeCalculationResponse(BaseModel):
    ...
    exemptions: List[str] = Field(
        default_factory=list, description="Applicable exemptions"
    )
...
@government_integration_router.post(
    "/services/apply", response_model=GovernmentServiceResponse
)
async def apply_for_government_service(
    ...
    try:
        logger.info(
            f"Processing government service application {application_id} for user {current_user.id}"
        )
        ...
        service_availability = await ministry_service.check_service_availability(
            service_type=request.service_type, service_code=request.service_code
        )
        ...
        reference_number = await ministry_service.generate_reference_number(
            application_id=application_id, service_type=request.service_type
        )
        ...
        application_record = GovernmentApplication(
            ...
            + timedelta(days=application_result.validity_period_days)
            if application_result.validity_period_days
            else None,
        )
        ...
        contact_info = await ministry_service.get_contact_information(
            service_type=request.service_type, language=request.language_preference
        )
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Government service application failed: {str(e)}"
        )
...
async def upload_government_document(
    ...
    try:
        logger.info(
            f"Uploading government document {document_id} for user {current_user.id}"
        )

        # Validate file
        if not document_file.content_type.startswith(
...
async def automate_form_filling(
    ...
    try:
        ...
        if not form_template:
            raise HTTPException(
                status_code=404, detail=f"Form {request.form_id} not found"
            )

        # Auto-populate from citizen profile if requested
        if request.auto_populate:
            citizen_profile = await form_automation_service.get_citizen_profile(
                current_user.id
            )
        ...
        raise HTTPException(status_code=500, detail=f"Form automation failed: {str(e)}")


@government_integration_router.get(
    "/applications/{application_id}/status", response_model=StatusCheckResponse
)
async def check_application_status(
    ...
    try:
        ...
        application = (
            db.query(GovernmentApplication)
            .filter(
        ...
        # Get ministry service
        ministry_service = ministry_services.get(
            MinistryDepartment(application.ministry)
        )
        if not ministry_service:
            raise HTTPException(
                status_code=400,
        ...
        logger.info(
            f"Application status retrieved: {application_id} - {application.status}"
        )
        return response
...
@government_integration_router.get(
    "/fees/calculate", response_model=FeeCalculationResponse
)
async def calculate_government_fees(
    ...
    priority_level: PriorityLevel = Query(
        PriorityLevel.NORMAL, description="Priority level"
    ),
    ...
    try:
        ...
        discount_info = await fee_calculation_service.check_discount_eligibility(
            user_id=current_user.id, ministry=ministry, service_type=service_type
        )
        ...
        logger.info(
            f"Fee calculation completed: {fee_result.total_amount} {fee_result.currency}"
        )
        return response
...
async def list_ministry_services(
    ...
    language: Literal["arabic", "english", "kurdish"] = Query(
        "arabic", description="Response language"
    ),
    current_user: User = Depends(get_current_user),
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not ministry_service:
            raise HTTPException(
                status_code=400, detail=f"Ministry {ministry} not available"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to list ministry services: {str(e)}"
        )


# Administrative endpoints
@government_integration_router.post(
    "/admin/sync-forms", dependencies=[Depends(require_permissions(["admin"]))]
)
...
government_integration_router.tags = [
