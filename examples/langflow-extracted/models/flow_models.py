...
class Flow(Base):
    ...
    # Flow management
    status = Column(
        String(50), nullable=False, default=FlowStatus.DRAFT.value, index=True
    )
    ...
    islamic_compliance_score = Column(
        Float, nullable=True, comment="Islamic compliance score (0.0-1.0)"
    )
    cultural_appropriateness_score = Column(
        Float, nullable=True, comment="Iraqi cultural appropriateness (0.0-1.0)"
    )
    sectarian_neutrality_validated = Column(
        Boolean, default=False, comment="Sectarian neutrality validation status"
    )
    ...
    shared_with_users = Column(
        ARRAY(UUID), nullable=True, comment="User IDs with access to this flow"
    )
    ...
    cultural_contexts = relationship(
        "IraqiFlowContext", back_populates="flow", cascade="all, delete-orphan"
    )
            ...
            result.update(
                {
                    ...
                    "shared_with_users": [str(uid) for uid in self.shared_with_users]
    ...
    def is_culturally_compliant(self) -> bool:
        ...
        return (
            self.cultural_validation_status == CulturalValidationStatus.APPROVED.value
            and self.islamic_compliance_score is not None
            and self.islamic_compliance_score >= 0.85
            and self.cultural_appropriateness_score is not None
            and self.cultural_appropriateness_score >= 0.90
            and self.sectarian_neutrality_validated
        )
...
class FlowExecution(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    flow_id = Column(
        UUID(as_uuid=True), ForeignKey("flows.id"), nullable=False, index=True
    )
    user_id = Column(
        UUID(as_uuid=True), ForeignKey("users.id"), nullable=False, index=True
    )
    ...
    # Execution tracking
    status = Column(
        String(50), nullable=False, default=FlowStatus.RUNNING.value, index=True
    )
    ...
    cultural_violations = Column(
        JSON, nullable=True, comment="Cultural violations detected during execution"
    )
    ...
    data_retention_until = Column(
        DateTime, nullable=True, comment="Automatic data expiration (1-hour default)"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "ix_executions_cultural_validation",
            "cultural_validation_passed",
    ...
    def to_dict(self, include_sensitive: bool = False) -> Dict[str, Any]:
        ...
        result = {
            ...
            "data_retention_until": self.data_retention_until.isoformat()
    ...
    def is_culturally_valid(self) -> bool:
        ...
        return (
            self.cultural_validation_passed
            and self.islamic_compliance_validated
            and (not self.cultural_violations or len(self.cultural_violations) == 0)
        )
...
class FlowVersion(Base):
    ...
    # Cultural versioning
    cultural_improvements = Column(
        JSON, nullable=True, comment="Cultural enhancements in this version"
    )
    islamic_compliance_changes = Column(
        JSON, nullable=True, comment="Islamic compliance improvements"
    )
    professional_domain_updates = Column(
        JSON, nullable=True, comment="Professional domain enhancements"
    )
    arabic_processing_improvements = Column(
        JSON, nullable=True, comment="Arabic language processing updates"
    )
    ...
    performance_delta = Column(
        JSON, nullable=True, comment="Performance changes from previous version"
    )
...
class FlowTemplate(Base):
    ...
    use_cases = Column(
        ARRAY(String), nullable=False, comment="Iraqi professional use cases"
    )
    use_cases_arabic = Column(ARRAY(String), nullable=False)
    target_organizations = Column(
        ARRAY(String), nullable=True, comment="Types of Iraqi organizations"
    )
    ...
    cultural_validation_level = Column(
        String(50), nullable=False, default=CulturalValidationStatus.APPROVED.value
    )
    ...
    __table_args__ = (
        Index(
            "ix_templates_domain_complexity", "professional_domain", "complexity_level"
        ),
        Index("ix_templates_featured_published", "is_featured", "published_at"),
        Index(
            "ix_templates_cultural_validation",
            "cultural_validation_level",
...
class IraqiFlowContext(Base):
    ...
    dialect_preference = Column(
        String(50), nullable=True, comment="Iraqi dialect preference"
    )
    ...
    professional_terminology_preference = Column(
        String(50), nullable=True, comment="formal, colloquial, mixed"
    )
