...
class VoiceMessage(Base):
    ...
    file_size_bytes = Column(
        Integer, nullable=False, comment="Audio file size in bytes"
    )
    mime_type = Column(String(50), nullable=False, comment="Audio MIME type")
    duration_seconds = Column(
        Float, nullable=False, comment="Audio duration in seconds"
    )

    # Audio quality and processing
    audio_quality = Column(
        String(20), nullable=False, default=AudioQuality.STANDARD.value, index=True
    )
    ...
    channels = Column(
        Integer, nullable=False, default=1, comment="Audio channels (1=mono, 2=stereo)"
    )
    audio_format = Column(
        String(10), nullable=False, comment="Audio format: mp3, wav, ogg, m4a"
    )

    # Iraqi accent and dialect processing
    detected_accent = Column(
        String(50), nullable=True, index=True, comment="Detected Iraqi accent"
    )
    accent_confidence = Column(
        Float, nullable=True, comment="0.0-1.0 accent detection confidence"
    )
    dialect_recognized = Column(
        String(50), nullable=True, index=True, comment="Iraqi Arabic dialect"
    )
    ...
    transcription_text = Column(
        Text, nullable=True, comment="Speech-to-text transcription"
    )
    transcription_arabic = Column(
        Text, nullable=True, comment="Arabic transcription with proper RTL"
    )
    transcription_english = Column(
        Text, nullable=True, comment="English portions of transcription"
    )
    transcription_confidence = Column(
        Float, nullable=True, comment="0.0-1.0 transcription confidence"
    )
    language_detected = Column(
        String(10), nullable=True, index=True, comment="ar, en, ar-en"
    )
    ...
    speech_clarity_score = Column(
        Float, nullable=True, comment="0.0-1.0 speech clarity"
    )
    background_noise_level = Column(
        Float, nullable=True, comment="0.0-1.0 background noise"
    )
    ...
    islamic_compliance_level = Column(
        String(50), nullable=False, default="neutral", index=True
    )
    cultural_appropriateness_score = Column(
        Float, nullable=True, comment="0.0-1.0 cultural appropriateness"
    )
    ...
    professional_voice_template = Column(
        String(100),
        nullable=True,
        index=True,
    ...
    primary_emotion = Column(
        String(20), nullable=True, index=True, comment="Detected primary emotion"
    )
    emotion_confidence = Column(
        Float, nullable=True, comment="0.0-1.0 emotion confidence"
    )
    sentiment_score = Column(
        Float, nullable=True, comment="-1.0 to 1.0 sentiment polarity"
    )
    tone_analysis = Column(
        String(50), nullable=True, comment="Voice tone characteristics"
    )
    ...
    status = Column(
        String(20),
        nullable=False,
        default=VoiceMessageStatus.UPLOADING.value,
        index=True,
    )
    ...
    processing_error_message = Column(
        Text, nullable=True, comment="Error details if processing failed"
    )
    retry_count = Column(
        Integer, default=0, nullable=False, comment="Processing retry attempts"
    )

    # Privacy and security
    encrypted = Column(
        Boolean, default=True, nullable=False, comment="Audio file encryption status"
    )
    encryption_key_id = Column(
        String(100), nullable=True, comment="Encryption key reference"
    )
    auto_delete_after_hours = Column(
        Integer,
        default=1,
        nullable=False,
    ...
    user_transcription_correction = Column(
        Text, nullable=True, comment="User correction to transcription"
    )
    user_rating = Column(
        Integer, nullable=True, comment="1-5 user rating for voice processing"
    )
    user_feedback = Column(
        Text, nullable=True, comment="User feedback on voice processing"
    )

    # Performance metrics
    upload_time_ms = Column(
        Integer, nullable=True, comment="Upload time in milliseconds"
    )
    processing_time_ms = Column(
        Integer, nullable=True, comment="Processing time in milliseconds"
    )
    transcription_time_ms = Column(
        Integer, nullable=True, comment="Transcription time in milliseconds"
    )
    ...
    audio_analysis_data = Column(
        JSONB, nullable=True, comment="Detailed audio analysis results"
    )
    accent_analysis_data = Column(
        JSONB, nullable=True, comment="Iraqi accent analysis results"
    )
    cultural_context_data = Column(
        JSONB, nullable=True, comment="Cultural context information"
    )
    ai_processing_scores = Column(
        JSONB, nullable=True, comment="AI processing confidence scores"
    )
    ...
    validation_records = relationship(
        "MessageValidation", back_populates="voice_message"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "duration_seconds > 0", name="ck_voice_message_duration_positive"
        ),
        CheckConstraint(
            "file_size_bytes > 0", name="ck_voice_message_file_size_positive"
        ),
        CheckConstraint(
            "sample_rate > 0", name="ck_voice_message_sample_rate_positive"
        ),
        CheckConstraint(
            "channels >= 1 AND channels <= 2", name="ck_voice_message_channels"
        ),
        CheckConstraint(
            "auto_delete_after_hours >= 1", name="ck_voice_message_auto_delete_hours"
        ),
        CheckConstraint("retry_count >= 0", name="ck_voice_message_retry_count"),
        # Score validation constraints
        CheckConstraint(
            "accent_confidence >= 0.0 AND accent_confidence <= 1.0",
        ...
        CheckConstraint(
            "speech_clarity_score >= 0.0 AND speech_clarity_score <= 1.0",
            name="ck_voice_message_speech_clarity_score",
        ),
        CheckConstraint(
            "background_noise_level >= 0.0 AND background_noise_level <= 1.0",
        ...
        CheckConstraint(
            "stress_level_detected >= 0.0 AND stress_level_detected <= 1.0",
            name="ck_voice_message_stress_level",
        ),
        CheckConstraint(
            "user_rating >= 1 AND user_rating <= 5", name="ck_voice_message_user_rating"
        ),
        # Performance indexes
        Index(
            "idx_voice_message_conversation_created", "conversation_id", "created_at"
        ),
        Index("idx_voice_message_user_status", "user_id", "status"),
        Index(
            "idx_voice_message_cultural_validation",
            "cultural_validation_status",
            "islamic_compliance_level",
        ),
        Index(
            "idx_voice_message_professional", "professional_domain", "detected_accent"
        ),
    ...
    def is_high_quality_audio(self) -> bool:
        ...
        return (
            self.audio_quality
            in [AudioQuality.HIGH.value, AudioQuality.PROFESSIONAL.value]
            and self.speech_clarity_score
            and self.speech_clarity_score >= 0.8
            and self.background_noise_level
            and self.background_noise_level <= 0.2
        )

    def __repr__(self):
        return (
            f"<VoiceMessage(id={self.id}, conversation_id={self.conversation_id}, "
...
def set_voice_message_expiration(mapper, connection, target):
    ...
    if target.auto_delete_after_hours and not target.expires_at:
        target.expires_at = datetime.utcnow() + timedelta(
            hours=target.auto_delete_after_hours
        )
...
def update_processing_metrics(mapper, connection, target):
    ...
    if (
        target.status == VoiceMessageStatus.READY.value
        and target.processing_started_at
        and target.processing_completed_at
...
class MessageValidation(Base):
    ...
    voice_message_id = Column(
        UUID(as_uuid=True), ForeignKey("voice_messages.id"), nullable=True, index=True
    )
    conversation_id = Column(
        UUID(as_uuid=True),
        ForeignKey("chat_conversations.id"),
    ...
    validation_type = Column(
        ...
        nullable=False,
        index=True,
        comment="cultural, islamic, professional",
    )
    content_type = Column(
        String(20), nullable=False, index=True, comment="text, voice, mixed"
    )
    validation_scope = Column(
        String(50),
        nullable=False,
        index=True,
    ...
    cultural_appropriateness_score = Column(
        Float, nullable=False, comment="0.0-1.0 appropriateness score"
    )
    islamic_compliance_score = Column(
        Float, nullable=False, comment="0.0-1.0 Islamic compliance"
    )
    sectarian_neutrality_score = Column(
        Float, nullable=False, comment="0.0-1.0 sectarian neutrality"
    )
    political_neutrality_score = Column(
        Float, nullable=False, comment="0.0-1.0 political neutrality"
    )
    professional_appropriateness_score = Column(
        Float, nullable=True, comment="0.0-1.0 professional score"
    )
    ...
    escalation_reason = Column(
        String(200), nullable=True, comment="Reason for human review"
    )

    # Detailed validation results
    validation_details = Column(
        JSONB, nullable=True, comment="Detailed validation analysis"
    )
    flagged_elements = Column(
        JSONB, nullable=True, comment="Specific flagged content elements"
    )
    suggested_modifications = Column(
        JSONB, nullable=True, comment="AI-suggested content modifications"
    )
    cultural_context_factors = Column(
        JSONB, nullable=True, comment="Cultural context considerations"
    )
    ...
    validation_model_version = Column(
        String(20), nullable=True, comment="AI model version used"
    )
    processing_time_ms = Column(
        Integer, nullable=True, comment="Validation time in milliseconds"
    )
    confidence_score = Column(
        Float, nullable=True, comment="0.0-1.0 validation confidence"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "sectarian_neutrality_score >= 0.0 AND sectarian_neutrality_score <= 1.0",
            name="ck_message_validation_sectarian_neutrality_score",
        ),
        CheckConstraint(
            "political_neutrality_score >= 0.0 AND political_neutrality_score <= 1.0",
            name="ck_message_validation_political_neutrality_score",
        ),
        CheckConstraint(
            "professional_appropriateness_score >= 0.0 AND professional_appropriateness_score <= 1.0",
        ...
        CheckConstraint(
            "(message_id IS NOT NULL) OR (voice_message_id IS NOT NULL)",
            name="ck_message_validation_has_target",
        ),
        CheckConstraint(
            "processing_time_ms >= 0", name="ck_message_validation_processing_time"
        ),
        # Performance indexes
        Index(
            "idx_message_validation_conversation_status",
            "conversation_id",
            "validation_status",
        ),
        Index(
            "idx_message_validation_scores",
            "cultural_appropriateness_score",
            "islamic_compliance_score",
        ),
        Index("idx_message_validation_review", "requires_human_review", "created_at"),
        Index(
            "idx_message_validation_professional",
            "professional_domain",
            "validation_status",
        ),
    ...
    def __repr__(self):
        return (
            f"<MessageValidation(id={self.id}, conversation_id={self.conversation_id}, "
...
class CulturalContext(Base):
    ...
    context_name = Column(
        String(100), nullable=False, index=True, comment="Cultural context identifier"
    )
    ...
    cultural_background = Column(
        String(100), nullable=True, comment="Detailed cultural background"
    )
    dialect_preference = Column(
        String(50), nullable=False, index=True, comment="Preferred Iraqi dialect"
    )
    religious_context = Column(
        String(50), nullable=True, index=True, comment="Religious background context"
    )

    # Professional and social context
    professional_domain = Column(
        String(50), nullable=True, index=True, comment="Primary professional domain"
    )
    social_context = Column(
        String(100), nullable=True, comment="Social and family context"
    )
    educational_background = Column(
        String(100), nullable=True, comment="Educational context"
    )
    age_group = Column(
        String(20),
        nullable=True,
        index=True,
        comment="Age group for cultural adaptation",
    )

    # Islamic compliance and cultural preferences
    islamic_compliance_level = Column(
        String(50), nullable=False, default="standard", index=True
    )
    ...
    communication_style = Column(
        String(30), nullable=False, default="formal", index=True
    )
    ...
    cultural_appropriateness_threshold = Column(
        Float,
        nullable=False,
        default=0.85,
        comment="Required cultural appropriateness score",
    )
    islamic_compliance_threshold = Column(
        Float, nullable=False, default=0.90, comment="Required Islamic compliance score"
    )
    professional_appropriateness_threshold = Column(
        Float, nullable=True, comment="Professional context threshold"
    )
    ...
    successful_interactions = Column(
        Integer, default=0, nullable=False, comment="Successful interactions"
    )
    cultural_violations = Column(
        Integer, default=0, nullable=False, comment="Cultural violations"
    )
    last_used_at = Column(DateTime, nullable=True, index=True)
    effectiveness_score = Column(
        Float, nullable=True, comment="0.0-1.0 context effectiveness"
    )
    ...
    cultural_preferences = Column(
        JSONB, nullable=True, comment="Detailed cultural preferences"
    )
    cultural_learning_data = Column(
        JSONB, nullable=True, comment="Adaptive learning information"
    )
    professional_context_data = Column(
        JSONB, nullable=True, comment="Professional domain data"
    )
    regional_customizations = Column(
        JSONB, nullable=True, comment="Regional cultural customizations"
    )
    ...
    __table_args__ = (
        # Score validation constraints
        CheckConstraint(
            "cultural_appropriateness_threshold >= 0.0 AND cultural_appropriateness_threshold <= 1.0",
            name="ck_cultural_context_cultural_appropriateness_threshold",
        ),
        CheckConstraint(
            "islamic_compliance_threshold >= 0.0 AND islamic_compliance_threshold <= 1.0",
            name="ck_cultural_context_islamic_compliance_threshold",
        ),
        CheckConstraint(
            "professional_appropriateness_threshold >= 0.0 AND professional_appropriateness_threshold <= 1.0",
        ...
        CheckConstraint(
            ...
            name="ck_cultural_context_successful_interactions",
        ),
        CheckConstraint(
            "cultural_violations >= 0", name="ck_cultural_context_cultural_violations"
        ),
        # Performance indexes
        Index("idx_cultural_context_user_region", "user_id", "regional_context"),
        Index(
            "idx_cultural_context_professional",
            "professional_domain",
            "dialect_preference",
        ),
        Index(
            "idx_cultural_context_compliance",
            "islamic_compliance_level",
            "cultural_sensitivity_level",
        ),
        Index("idx_cultural_context_usage", "last_used_at", "effectiveness_score"),
        Index(
            "idx_cultural_context_language", "primary_language", "communication_style"
        ),
    )

    def __repr__(self):
        return (
            f"<CulturalContext(id={self.id}, user_id={self.user_id}, "
...
def update_context_effectiveness(mapper, connection, target):
    ...
    if (
        ...
        and target.successful_interactions >= 0
        and target.cultural_violations >= 0
    ):
        success_rate = target.successful_interactions / target.usage_count
        violation_penalty = min(
            target.cultural_violations * 0.1, 0.5
        )  # Max 50% penalty
