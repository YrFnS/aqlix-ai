...
class ChatConversation(Base):
    ...
    cultural_context_id = Column(
        UUID(as_uuid=True), ForeignKey("cultural_contexts.id"), nullable=True
    )
    regional_context = Column(
        String(50), nullable=True, index=True, comment="Baghdad, Basra, Kurdistan, etc."
    )
    dialect_preference = Column(
        String(50), nullable=True, index=True, comment="Iraqi Arabic dialect variation"
    )
    ...
    islamic_compliance_level = Column(
        String(50),
        nullable=False,
        default=IslamicComplianceLevel.NEUTRAL.value,
        index=True,
    )
    cultural_validation_status = Column(
        String(50),
        nullable=False,
        default=CulturalValidationStatus.PENDING.value,
        index=True,
    )
    ...
    islamic_compliance_score = Column(
        Float, nullable=True, comment="0.0-1.0 Islamic compliance score"
    )
    sectarian_neutrality_validated = Column(
        Boolean,
        default=False,
        index=True,
    ...
    status = Column(
        String(20), nullable=False, default=ConversationStatus.ACTIVE.value, index=True
    )
    privacy_mode = Column(
        Boolean,
        default=True,
    ...
    text_direction = Column(
        String(10),
        nullable=False,
        default=ArabicTextDirection.AUTO.value,
        comment="RTL support for Arabic text",
    )
    arabic_dialect_detected = Column(
        String(50), nullable=True, index=True, comment="Detected Iraqi Arabic dialect"
    )
    mixed_language_support = Column(
        Boolean,
        default=True,
        nullable=False,
    ...
    voice_accent_preference = Column(
        String(50), nullable=True, index=True, comment="Iraqi accent optimization"
    )
    ...
    professional_template_used = Column(
        String(100),
        nullable=True,
        index=True,
    ...
    average_response_time = Column(
        Float, nullable=True, comment="Average response time in seconds"
    )
    ...
    # Advanced features
    metadata = Column(
        JSONB, nullable=True, comment="Conversation metadata and settings"
    )
    cultural_preferences = Column(
        JSONB, nullable=True, comment="User cultural preferences"
    )
    ...
    voice_messages = relationship(
        "VoiceMessage", back_populates="conversation", cascade="all, delete-orphan"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "user_satisfaction_score >= 0.0 AND user_satisfaction_score <= 5.0",
            name="ck_chat_conversation_user_satisfaction_score",
        ),
        CheckConstraint(
            "data_retention_days >= 1", name="ck_chat_conversation_data_retention_days"
        ),
        # Performance indexes
        Index("idx_chat_conversation_user_status", "user_id", "status"),
        Index(
            "idx_chat_conversation_cultural",
            "cultural_validation_status",
            "islamic_compliance_level",
        ),
        Index(
            "idx_chat_conversation_professional",
            "professional_domain",
            "regional_context",
        ),
        Index("idx_chat_conversation_expires", "session_expires_at", "status"),
        Index(
            "idx_chat_conversation_language",
            "primary_language",
            "arabic_dialect_detected",
        ),
        # Full-text search support
        Index(
            "idx_chat_conversation_title_fts",
            "title",
...
class ChatMessage(Base):
    ...
    message_type = Column(
        String(30), nullable=False, default=MessageType.TEXT.value, index=True
    )
    content = Column(Text, nullable=False, comment="Primary message content")
    content_arabic = Column(
        Text, nullable=True, comment="Arabic content with RTL formatting"
    )
    content_english = Column(
        Text, nullable=True, comment="English content for mixed language"
    )
    ...
    detected_language = Column(
        String(10), nullable=True, index=True, comment="ar, en, or ar-en"
    )
    text_direction = Column(
        String(10), nullable=False, default=ArabicTextDirection.AUTO.value
    )
    arabic_dialect = Column(
        String(50), nullable=True, index=True, comment="Detected Iraqi dialect"
    )
    rtl_formatted = Column(
        Boolean, default=False, nullable=False, comment="RTL formatting applied"
    )
    ...
    professional_domain = Column(
        String(50), nullable=True, index=True, comment="Message professional context"
    )
    ...
    processing_status = Column(
        String(30), nullable=False, default="processed", index=True
    )
    ...
    voice_message_id = Column(
        UUID(as_uuid=True), ForeignKey("voice_messages.id"), nullable=True
    )
    ...
    response_time_ms = Column(
        Integer, nullable=True, comment="AI response time in milliseconds"
    )
    ...
    user_rating = Column(
        Integer, nullable=True, comment="1-5 user rating for AI responses"
    )
    ...
    moderation_flags = Column(
        Integer, default=0, nullable=False, comment="Number of moderation flags"
    )

    # Privacy and session management
    expires_at = Column(
        DateTime, nullable=True, index=True, comment="Message expiration time"
    )
    auto_delete_after_session = Column(Boolean, default=True, nullable=False)
    encrypted_content = Column(
        Text, nullable=True, comment="Encrypted sensitive content"
    )
    ...
    metadata = Column(
        JSONB, nullable=True, comment="Message metadata and processing info"
    )
    cultural_context_data = Column(
        JSONB, nullable=True, comment="Cultural context information"
    )
    linguistic_analysis = Column(
        JSONB, nullable=True, comment="Arabic linguistic analysis results"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "arabic_word_count >= 0", name="ck_chat_message_arabic_word_count"
        ),
        CheckConstraint(
            "english_word_count >= 0", name="ck_chat_message_english_word_count"
        ),
        CheckConstraint(
            "attachment_count >= 0", name="ck_chat_message_attachment_count"
        ),
        CheckConstraint(
            "moderation_flags >= 0", name="ck_chat_message_moderation_flags"
        ),
        ...
        CheckConstraint(
            "user_rating >= 1 AND user_rating <= 5", name="ck_chat_message_user_rating"
        ),
        # Performance indexes
        ...
        Index(
            "idx_chat_message_cultural_validation",
            "cultural_validation_status",
            "islamic_compliance_level",
        ),
        Index(
            "idx_chat_message_professional", "professional_domain", "processing_status"
        ),
        Index("idx_chat_message_arabic", "detected_language", "arabic_dialect"),
        Index(
            "idx_chat_message_moderation",
            "reported_inappropriate",
            "requires_human_review",
        ),
        Index("idx_chat_message_expires", "expires_at", "auto_delete_after_session"),
        # Full-text search support for Arabic and English
        Index(
            "idx_chat_message_content_fts",
            "content",
            postgresql_using="gin",
            postgresql_ops={"content": "gin_trgm_ops"},
        ),
        Index(
            "idx_chat_message_arabic_fts",
            "content_arabic",
            postgresql_using="gin",
