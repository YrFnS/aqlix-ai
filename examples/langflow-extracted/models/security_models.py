...
class APIKey(Base):
    ...
    key_type = Column(
        String(20), nullable=False, default=APIKeyType.USER.value, index=True
    )
    security_level = Column(
        String(20), nullable=False, default=SecurityLevel.MEDIUM.value, index=True
    )

    # Iraqi regulatory and compliance
    iraqi_compliance_level = Column(
        String(50),
        nullable=False,
        default=IraqiRegulatoryCompliance.BASIC.value,
        index=True,
        comment="Iraqi regulatory compliance level",
    )
    islamic_compliance_level = Column(
        ...
        index=True,
        comment="Islamic security compliance",
    )
    government_authorized = Column(
        Boolean,
        default=False,
        nullable=False,
        index=True,
    ...
    professional_domain_access = Column(
        JSONB, nullable=True, comment="Iraqi professional domain access permissions"
    )
    ...
    status = Column(
        String(30), nullable=False, default=APIKeyStatus.ACTIVE.value, index=True
    )
    is_active = Column(Boolean, default=True, nullable=False, index=True)
    is_system_key = Column(Boolean, default=False, nullable=False, index=True)
    auto_rotate_enabled = Column(
        Boolean, default=True, nullable=False, comment="Automatic key rotation enabled"
    )

    # Rate limiting and usage controls
    rate_limit_requests_per_minute = Column(
        Integer, default=60, nullable=False, comment="Requests per minute limit"
    )
    rate_limit_requests_per_hour = Column(
        Integer, default=1000, nullable=False, comment="Requests per hour limit"
    )
    rate_limit_requests_per_day = Column(
        Integer, default=10000, nullable=False, comment="Requests per day limit"
    )
    monthly_usage_limit = Column(Integer, nullable=True, comment="Monthly usage limit")
    cultural_content_rate_limit = Column(
        Integer,
        default=100,
        nullable=False,
    ...
    restricted_endpoints = Column(
        JSONB, nullable=True, comment="Restricted API endpoints"
    )
    ip_whitelist = Column(JSONB, nullable=True, comment="Allowed IP addresses")
    user_agent_restrictions = Column(
        JSONB, nullable=True, comment="User agent restrictions"
    )

    # Security features and monitoring
    require_https = Column(
        Boolean, default=True, nullable=False, comment="Require HTTPS connections"
    )
    require_signature = Column(
        Boolean, default=False, nullable=False, comment="Require request signatures"
    )
    mfa_required = Column(
        Boolean, default=False, nullable=False, comment="MFA required for key usage"
    )
    ...
    total_requests = Column(
        Integer, default=0, nullable=False, comment="Total API requests made"
    )
    successful_requests = Column(
        Integer, default=0, nullable=False, comment="Successful requests"
    )
    failed_requests = Column(
        Integer, default=0, nullable=False, comment="Failed requests"
    )
    blocked_requests = Column(
        Integer, default=0, nullable=False, comment="Blocked/rate-limited requests"
    )
    cultural_validation_requests = Column(
        Integer, default=0, nullable=False, comment="Cultural validation API requests"
    )

    # Expiration and rotation
    expires_at = Column(
        DateTime, nullable=True, index=True, comment="Key expiration time"
    )
    auto_expire_days = Column(
        Integer, default=90, nullable=False, comment="Auto-expire after N days"
    )
    ...
    security_incidents = Column(
        Integer, default=0, nullable=False, comment="Number of security incidents"
    )
    ...
    security_metadata = Column(
        JSONB, nullable=True, comment="Security configuration metadata"
    )
    compliance_data = Column(
        JSONB, nullable=True, comment="Iraqi compliance validation data"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "monthly_usage_limit > 0", name="ck_api_key_monthly_usage_limit"
        ),
        CheckConstraint(
            "cultural_content_rate_limit > 0",
            name="ck_api_key_cultural_content_rate_limit",
        ),
        CheckConstraint("auto_expire_days > 0", name="ck_api_key_auto_expire_days"),
        CheckConstraint(
            "rotation_interval_days > 0", name="ck_api_key_rotation_interval_days"
        ),
        # Usage statistics constraints
        CheckConstraint("total_requests >= 0", name="ck_api_key_total_requests"),
        CheckConstraint(
            "successful_requests >= 0", name="ck_api_key_successful_requests"
        ),
        ...
        CheckConstraint(
            "cultural_validation_requests >= 0",
            name="ck_api_key_cultural_validation_requests",
        ),
        CheckConstraint(
            "security_incidents >= 0", name="ck_api_key_security_incidents"
        ),
        CheckConstraint(
            "suspicious_activity_count >= 0",
            name="ck_api_key_suspicious_activity_count",
        ),
        # Logical constraints
        CheckConstraint(
        ...
        Index(
            "idx_api_key_compliance",
            "iraqi_compliance_level",
            "islamic_compliance_level",
        ),
        Index(
            "idx_api_key_professional_access",
            "legal_access_authorized",
            "medical_access_authorized",
            "educational_access_authorized",
        ...
        Index(
            "idx_api_key_security_incidents",
            "security_incidents",
            "blocked_due_to_abuse",
        ),
        Index("idx_api_key_usage", "last_used_at", "total_requests"),
        # Security indexes
        Index(
            "idx_api_key_hash", "key_hash"
        ),  # Already unique, but ensures performance
        Index("idx_api_key_prefix", "key_prefix"),
    )

    @classmethod
    def generate_api_key(
        cls, key_type: str = "user", prefix: str = "aq"
    ) -> tuple[str, str]:
    ...
    def __repr__(self):
        return (
            f"<APIKey(id={self.id}, user_id={self.user_id}, name='{self.key_name}', "
...
class SecuritySettings(Base):
    ...
    islamic_compliance_enabled = Column(
    ...
    iraqi_regulatory_compliance = Column(
        String(50),
        nullable=False,
        default=IraqiRegulatoryCompliance.BASIC.value,
        index=True,
    ...
    government_security_standards = Column(
        Boolean, default=False, nullable=False, index=True
    )
    data_residency_iraq = Column(
        Boolean, default=True, nullable=False, comment="Data must remain in Iraq"
    )
    ...
    legal_professional_security = Column(
        Boolean,
        default=False,
        nullable=False,
        comment="Iraqi legal profession security",
    )
    medical_professional_security = Column(
        Boolean,
        default=False,
        nullable=False,
        comment="Iraqi medical profession security",
    )
    educational_professional_security = Column(
        Boolean, default=False, nullable=False, comment="Iraqi educational security"
    )
    government_professional_security = Column(
        Boolean, default=False, nullable=False, comment="Iraqi government security"
    )
    ...
    mfa_required = Column(
        ...
        default=False,
        nullable=False,
        comment="Multi-factor authentication required",
    )
    mfa_methods_allowed = Column(JSONB, default=list, comment="Allowed MFA methods")
    password_complexity_level = Column(
    ...
    auto_delete_sessions = Column(
        Boolean,
        default=True,
        nullable=False,
        comment="Auto-delete sessions after 1 hour",
    )
    data_encryption_level = Column(
    ...
    cultural_content_validation = Column(
        Boolean, default=True, nullable=False, index=True
    )
    ...
    third_party_integration_restrictions = Column(
        JSONB, nullable=True, comment="Third-party integration restrictions"
    )
    ...
    security_configuration = Column(
        JSONB, nullable=True, comment="Advanced security settings"
    )
    islamic_compliance_config = Column(
        JSONB, nullable=True, comment="Islamic compliance configuration"
    )
    regulatory_compliance_config = Column(
        JSONB, nullable=True, comment="Iraqi regulatory configuration"
    )
    professional_security_config = Column(
        JSONB, nullable=True, comment="Professional domain security"
    )
    threat_protection_config = Column(
        JSONB, nullable=True, comment="Threat protection configuration"
    )
    audit_configuration = Column(
        JSONB, nullable=True, comment="Audit and logging configuration"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "session_timeout_minutes > 0 AND session_timeout_minutes <= 60",
            name="ck_security_settings_session_timeout_minutes",
        ),
        CheckConstraint(
            "concurrent_sessions_allowed > 0",
            name="ck_security_settings_concurrent_sessions_allowed",
        ),
        CheckConstraint(
            "max_login_attempts > 0", name="ck_security_settings_max_login_attempts"
        ),
        CheckConstraint(
            "account_lockout_duration_minutes > 0",
            name="ck_security_settings_account_lockout_duration_minutes",
        ),
        CheckConstraint(
            "password_expiry_days > 0", name="ck_security_settings_password_expiry_days"
        ),
        CheckConstraint(
            "api_key_expiry_days > 0", name="ck_security_settings_api_key_expiry_days"
        ),
        CheckConstraint(
            "security_incident_threshold > 0",
            name="ck_security_settings_security_incident_threshold",
        ),
        # Performance indexes
        Index("idx_security_settings_user_scope", "user_id", "setting_scope"),
        Index(
            "idx_security_settings_islamic_compliance",
            "islamic_compliance_enabled",
            "islamic_compliance_level",
        ),
        Index(
            "idx_security_settings_regulatory",
            "iraqi_regulatory_compliance",
            "central_bank_compliance",
        ),
        Index(
            "idx_security_settings_professional",
            "professional_security_enabled",
            "legal_professional_security",
            "medical_professional_security",
            "educational_professional_security",
        ),
        Index(
            "idx_security_settings_privacy",
            "privacy_first_enabled",
            "data_encryption_level",
        ),
        Index(
            "idx_security_settings_cultural",
            "cultural_content_validation",
            "islamic_content_filtering",
        ),
    ...
    def is_high_security_configuration(self) -> bool:
        ...
        return (
            self.mfa_required
            and self.data_encryption_level == "high"
            and self.threat_detection_enabled
            and self.comprehensive_audit_logging
            and self.islamic_compliance_enabled
            and self.cultural_content_validation
        )

    def __repr__(self):
        return (
            f"<SecuritySettings(id={self.id}, user_id={self.user_id}, "
...
def set_api_key_expiration(mapper, connection, target):
    ...
    if target.auto_expire_days and not target.expires_at:
        ...
        target.next_rotation_due = datetime.utcnow() + timedelta(
            days=target.rotation_interval_days
        )
