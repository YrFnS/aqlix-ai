...
class User(Base):
    ...
    phone_number = Column(String(20), nullable=True, index=True, comment="+964 Iraqi phone format")
    ...
    status = Column(String(50), nullable=False, default=UserStatus.PENDING_VERIFICATION.value, index=True)
    ...
    subscription_tier = Column(String(50), nullable=False, 
                             default=SubscriptionTier.FREE.value, index=True)
    credit_balance = Column(Float, default=0.0, comment="Available credits")
    monthly_usage_limit = Column(Integer, nullable=True, comment="Monthly message limit")
    current_month_usage = Column(Integer, default=0, comment="Current month messages used")
    usage_reset_date = Column(DateTime, nullable=True, comment="Next usage reset date")
    
    # Privacy and data preferences
    data_retention_preference = Column(String(50), nullable=False, default="session_only")
    ...
    cultural_appropriateness_score = Column(Float, nullable=True, 
                                          comment="User's cultural content score")
    islamic_compliance_adherence = Column(Float, nullable=True,
                                        comment="User's Islamic compliance rate")
    ...
    professional_profile = relationship("IraqiProfessionalProfile", back_populates="user", 
                                      uselist=False, cascade="all, delete-orphan")
    sessions = relationship("UserSession", back_populates="user", cascade="all, delete-orphan")
    preferences = relationship("UserPreferences", back_populates="user", 
                             uselist=False, cascade="all, delete-orphan")
    cultural_settings = relationship("CulturalSettings", back_populates="user",
                                   uselist=False, cascade="all, delete-orphan")
    
    # Indexes for performance
    __table_args__ = (
        Index('ix_users_professional_regional', 'professional_domain', 'regional_context'),
        Index('ix_users_subscription_status', 'subscription_tier', 'status'),
        Index('ix_users_cultural_compliance', 'islamic_compliance_level', 'cultural_sensitivity_level'),
    ...
    def can_send_message(self) -> bool:
        ...
        if self.subscription_tier in [SubscriptionTier.BUSINESS.value, SubscriptionTier.ENTERPRISE.value]:
            return True
    ...
    def to_dict(self, include_sensitive: bool = False) -> Dict[str, Any]:
        ...
        result = {
            ...
            "current_month_usage": self.current_month_usage if include_sensitive else None,
...
class UserProfile(Base):
    ...
    preferred_communication_style = Column(String(50), nullable=True)  # formal, casual, mixed
    
    # Privacy settings
    profile_visibility = Column(String(50), nullable=False, default="private")  # public, private, professional
...
class IraqiProfessionalProfile(Base):
    ...
    license_verification_status = Column(String(50), nullable=True)  # verified, pending, expired
    ...
    disciplinary_actions = Column(JSON, nullable=True)  # Any professional disciplinary history
    ...
    # Indexes
    __table_args__ = (
        Index('ix_prof_profile_license_status', 'iraqi_professional_license', 'license_verification_status'),
        ...
        Index('ix_prof_profile_syndicate', 'professional_syndicate_member', 'syndicate_membership_number'),
    )
    ...
    def to_dict(self, include_sensitive: bool = False) -> Dict[str, Any]:
        ...
        result = {
            "id": str(self.id),
            "user_id": str(self.user_id),
            "iraqi_professional_license": self.iraqi_professional_license if include_sensitive else None,
            ...
            "last_professional_development": self.last_professional_development.isoformat() if self.last_professional_development else None,
...
class UserSession(Base):
    ...
    cultural_context = Column(JSON, nullable=True, comment="Session cultural preferences")
...
class CulturalSettings(Base):
    ...
    regional_context = Column(String(50), nullable=True, index=True)
    dialect_preference = Column(String(50), nullable=True, index=True)
    cultural_formality_level = Column(String(50), nullable=False, default="formal")  # formal, casual, mixed
    ...
    inappropriate_content_threshold = Column(String(50), nullable=False, default="strict")
    cultural_appropriateness_threshold = Column(Float, default=0.9)
    
    # Language and communication
    arabic_script_preference = Column(String(50), nullable=False, default="standard")  # standard, simplified
    ...
    __table_args__ = (
        ...
        Index('ix_cultural_content_filtering', 'halal_content_filter', 'sectarian_content_filter'),
    )
