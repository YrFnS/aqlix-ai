...
class ArabicOCRResult(Base):
    ...
    document_processing_id = Column(UUID(as_uuid=True), ForeignKey("document_processing.id"), 
                                   nullable=False, index=True)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False, index=True)
    
    # OCR processing configuration
    ocr_engine_used = Column(String(50), nullable=False, comment="OCR engine identifier")
    ocr_model_version = Column(String(20), nullable=False, comment="OCR model version")
    processing_language = Column(String(10), nullable=False, index=True, comment="ar, en, ar-en")
    target_dialect = Column(String(50), nullable=True, index=True, comment="Target Iraqi dialect")
    
    # Extracted text content
    extracted_text_raw = Column(Text, nullable=False, comment="Raw OCR extracted text")
    extracted_text_cleaned = Column(Text, nullable=True, comment="Cleaned and processed text")
    extracted_text_arabic = Column(Text, nullable=True, comment="Arabic portions with RTL formatting")
    extracted_text_english = Column(Text, nullable=True, comment="English portions")
    extracted_text_formatted = Column(Text, nullable=True, comment="Final formatted text")
    
    # Page-level OCR results
    page_number = Column(Integer, nullable=False, index=True, comment="Page number (1-based)")
    total_pages = Column(Integer, nullable=False, comment="Total pages in document")
    page_text_confidence = Column(Float, nullable=False, comment="0.0-1.0 page text confidence")
    page_layout_confidence = Column(Float, nullable=True, comment="0.0-1.0 layout confidence")
    
    # Language and dialect detection
    detected_language = Column(String(10), nullable=True, index=True, comment="Detected language")
    language_confidence = Column(Float, nullable=True, comment="0.0-1.0 language confidence")
    detected_dialect = Column(String(50), nullable=True, index=True, comment="Detected Iraqi dialect")
    ...
    character_confidence_avg = Column(Float, nullable=True, comment="0.0-1.0 average character confidence")
    word_confidence_avg = Column(Float, nullable=True, comment="0.0-1.0 average word confidence")
    line_confidence_avg = Column(Float, nullable=True, comment="0.0-1.0 average line confidence")
    overall_confidence_level = Column(String(20), nullable=True, index=True, comment="Confidence level")
    
    # Document structure and layout
    layout_type_detected = Column(String(30), nullable=True, index=True, comment="Document layout type")
    text_regions_count = Column(Integer, nullable=True, comment="Number of text regions")
    table_regions_count = Column(Integer, nullable=True, comment="Number of table regions")
    image_regions_count = Column(Integer, nullable=True, comment="Number of image regions")
    rtl_regions_detected = Column(Boolean, default=False, nullable=False, comment="RTL text regions found")
    ...
    arabic_character_count = Column(Integer, nullable=True, comment="Arabic character count")
    english_character_count = Column(Integer, nullable=True, comment="English character count")
    ...
    sensitive_content_flags = Column(Integer, default=0, nullable=False, comment="Sensitive content flags")
    ...
    quality_score = Column(Float, nullable=True, comment="0.0-1.0 overall quality score")
    user_feedback_score = Column(Float, nullable=True, comment="0.0-1.0 user feedback score")
    
    # Error detection and correction
    ocr_errors_detected = Column(Integer, default=0, nullable=False, comment="Number of detected OCR errors")
    spelling_errors_detected = Column(Integer, default=0, nullable=False, comment="Spelling errors")
    grammar_errors_detected = Column(Integer, default=0, nullable=False, comment="Grammar errors")
    formatting_errors_detected = Column(Integer, default=0, nullable=False, comment="Formatting errors")
    auto_corrections_applied = Column(Integer, default=0, nullable=False, comment="Auto-corrections applied")
    
    # Performance metrics
    processing_time_ms = Column(Integer, nullable=True, comment="OCR processing time in milliseconds")
    post_processing_time_ms = Column(Integer, nullable=True, comment="Post-processing time")
    total_processing_time_ms = Column(Integer, nullable=True, comment="Total processing time")
    memory_usage_mb = Column(Float, nullable=True, comment="Memory usage during processing")
    ...
    confidence_map = Column(JSONB, nullable=True, comment="Character/word level confidence mapping")
    layout_analysis_data = Column(JSONB, nullable=True, comment="Document layout analysis results")
    dialect_analysis_data = Column(JSONB, nullable=True, comment="Iraqi dialect analysis results")
    professional_analysis_data = Column(JSONB, nullable=True, comment="Professional terminology analysis")
    cultural_analysis_data = Column(JSONB, nullable=True, comment="Cultural content analysis")
    quality_metrics = Column(JSONB, nullable=True, comment="Detailed quality assessment metrics")
    correction_history = Column(JSONB, nullable=True, comment="History of corrections and improvements")

    # Relationships
    file_storage = relationship("FileStorage", back_populates="ocr_results")
    document_processing = relationship("DocumentProcessing", back_populates="ocr_results")
    ...
    __table_args__ = (
        ...
        CheckConstraint('page_number <= total_pages', name='ck_arabic_ocr_result_page_within_total'),
        CheckConstraint('character_count >= 0', name='ck_arabic_ocr_result_character_count'),
        ...
        CheckConstraint('text_regions_count >= 0', name='ck_arabic_ocr_result_text_regions_count'),
        CheckConstraint('table_regions_count >= 0', name='ck_arabic_ocr_result_table_regions_count'),
        CheckConstraint('image_regions_count >= 0', name='ck_arabic_ocr_result_image_regions_count'),
        
        # Terminology count constraints
        CheckConstraint('legal_terminology_count >= 0', name='ck_arabic_ocr_result_legal_terminology_count'),
        CheckConstraint('medical_terminology_count >= 0', name='ck_arabic_ocr_result_medical_terminology_count'),
        CheckConstraint('educational_terminology_count >= 0', name='ck_arabic_ocr_result_educational_terminology_count'),
        CheckConstraint('government_terminology_count >= 0', name='ck_arabic_ocr_result_government_terminology_count'),
        CheckConstraint('religious_terminology_count >= 0', name='ck_arabic_ocr_result_religious_terminology_count'),
        CheckConstraint('sensitive_content_flags >= 0', name='ck_arabic_ocr_result_sensitive_content_flags'),
        CheckConstraint('ocr_errors_detected >= 0', name='ck_arabic_ocr_result_ocr_errors_detected'),
        CheckConstraint('spelling_errors_detected >= 0', name='ck_arabic_ocr_result_spelling_errors_detected'),
        CheckConstraint('grammar_errors_detected >= 0', name='ck_arabic_ocr_result_grammar_errors_detected'),
        CheckConstraint('formatting_errors_detected >= 0', name='ck_arabic_ocr_result_formatting_errors_detected'),
        CheckConstraint('auto_corrections_applied >= 0', name='ck_arabic_ocr_result_auto_corrections_applied'),
        
        # Score validation constraints
        CheckConstraint('page_text_confidence >= 0.0 AND page_text_confidence <= 1.0',
                       name='ck_arabic_ocr_result_page_text_confidence'),
        CheckConstraint('page_layout_confidence >= 0.0 AND page_layout_confidence <= 1.0',
        ...
        CheckConstraint('character_confidence_avg >= 0.0 AND character_confidence_avg <= 1.0',
                       name='ck_arabic_ocr_result_character_confidence_avg'),
        CheckConstraint('word_confidence_avg >= 0.0 AND word_confidence_avg <= 1.0',
                       name='ck_arabic_ocr_result_word_confidence_avg'),
        CheckConstraint('line_confidence_avg >= 0.0 AND line_confidence_avg <= 1.0',
        ...
        CheckConstraint('user_feedback_score >= 0.0 AND user_feedback_score <= 1.0',
                       name='ck_arabic_ocr_result_user_feedback_score'),
        
        # Performance indexes
        Index('idx_arabic_ocr_result_file_page', 'file_storage_id', 'page_number'),
        Index('idx_arabic_ocr_result_processing', 'document_processing_id', 'created_at'),
        Index('idx_arabic_ocr_result_language', 'detected_language', 'detected_dialect'),
        Index('idx_arabic_ocr_result_confidence', 'page_text_confidence', 'overall_confidence_level'),
        Index('idx_arabic_ocr_result_cultural', 'cultural_validation_status', 'inappropriate_content_detected'),
        Index('idx_arabic_ocr_result_quality', 'quality_score', 'requires_human_review'),
        Index('idx_arabic_ocr_result_professional', 'professional_terminology_detected', 'layout_type_detected'),
        
        # Full-text search support
        Index('idx_arabic_ocr_result_text_fts', 'extracted_text_cleaned', postgresql_using='gin',
              postgresql_ops={'extracted_text_cleaned': 'gin_trgm_ops'}),
        Index('idx_arabic_ocr_result_arabic_fts', 'extracted_text_arabic', postgresql_using='gin',
    ...
    def is_high_quality_ocr(self) -> bool:
        ...
        return (self.page_text_confidence >= 0.90 and
                self.quality_score and self.quality_score >= 0.85 and
                self.ocr_errors_detected <= 5)

    def __repr__(self):
        return (f"<ArabicOCRResult(id={self.id}, file_id={self.file_storage_id}, "
...
class ProfessionalTemplate(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    template_name = Column(String(100), nullable=False, index=True, comment="Template display name")
    ...
    category = Column(String(30), nullable=False, index=True, comment="Template category")
    template_type = Column(String(30), nullable=False, index=True, comment="Specific template type")
    professional_domain = Column(String(50), nullable=False, index=True, 
                                comment="Iraqi professional domain")
    government_compatible = Column(Boolean, default=False, nullable=False, index=True,
    ...
    cultural_context = Column(String(100), nullable=True, comment="Cultural usage context")
    ...
    template_content = Column(Text, nullable=False, comment="Template content with placeholders")
    template_content_arabic = Column(Text, nullable=True, comment="Arabic template content")
    template_content_english = Column(Text, nullable=True, comment="English template content")
    template_variables = Column(JSONB, nullable=False, comment="Template variable definitions")
    ...
    islamic_compliance_validated = Column(Boolean, default=False, nullable=False, index=True)
    islamic_compliance_score = Column(Float, nullable=True, comment="0.0-1.0 Islamic compliance")
    cultural_appropriateness_score = Column(Float, nullable=True, comment="0.0-1.0 cultural score")
    sectarian_neutral = Column(Boolean, default=True, nullable=False, comment="Sectarian neutrality")
    ...
    compliance_notes = Column(Text, nullable=True, comment="Compliance validation notes")
    
    # Template usage and analytics
    usage_count = Column(Integer, default=0, nullable=False, comment="Number of times used")
    success_rate = Column(Float, nullable=True, comment="0.0-1.0 successful generation rate")
    user_satisfaction_avg = Column(Float, nullable=True, comment="0.0-5.0 average user rating")
    generation_time_avg_ms = Column(Integer, nullable=True, comment="Average generation time")
    error_rate = Column(Float, nullable=True, comment="0.0-1.0 error rate")
    
    # Template versioning and maintenance
    version_number = Column(String(20), nullable=False, default="1.0.0", comment="Template version")
    is_active = Column(Boolean, default=True, nullable=False, index=True)
    is_deprecated = Column(Boolean, default=False, nullable=False, index=True)
    replacement_template_id = Column(UUID(as_uuid=True), ForeignKey("professional_templates.id"), 
    ...
    template_metadata = Column(JSONB, nullable=True, comment="Template configuration and settings")
    ...
    generation_config = Column(JSONB, nullable=True, comment="AI generation configuration")
    cultural_guidelines = Column(JSONB, nullable=True, comment="Cultural usage guidelines")
    professional_standards = Column(JSONB, nullable=True, comment="Professional compliance standards")
    ...
    __table_args__ = (
        ...
        CheckConstraint('usage_count >= 0', name='ck_professional_template_usage_count'),
        CheckConstraint('success_rate >= 0.0 AND success_rate <= 1.0',
                       name='ck_professional_template_success_rate'),
        CheckConstraint('user_satisfaction_avg >= 0.0 AND user_satisfaction_avg <= 5.0',
                       name='ck_professional_template_user_satisfaction_avg'),
        CheckConstraint('generation_time_avg_ms >= 0', name='ck_professional_template_generation_time_avg_ms'),
        CheckConstraint('error_rate >= 0.0 AND error_rate <= 1.0',
        ...
        # Performance indexes
        Index('idx_professional_template_category_domain', 'category', 'professional_domain'),
        Index('idx_professional_template_active_deprecated', 'is_active', 'is_deprecated'),
        Index('idx_professional_template_cultural', 'islamic_compliance_validated', 'sectarian_neutral'),
        Index('idx_professional_template_professional', 'legal_reviewed', 'medical_validated', 
              'educational_approved', 'government_certified'),
        Index('idx_professional_template_language', 'language_primary', 'bilingual_template'),
        Index('idx_professional_template_usage', 'usage_count', 'success_rate'),
        Index('idx_professional_template_regional', 'regional_context', 'government_compatible'),
        
        # Full-text search support
        Index('idx_professional_template_name_fts', 'template_name', postgresql_using='gin',
              postgresql_ops={'template_name': 'gin_trgm_ops'}),
        Index('idx_professional_template_content_fts', 'template_content', postgresql_using='gin',
    ...
    def is_high_quality_template(self) -> bool:
        ...
        return (self.success_rate and self.success_rate >= 0.90 and
                self.user_satisfaction_avg and self.user_satisfaction_avg >= 4.0 and
                self.islamic_compliance_validated and
                self.cultural_appropriateness_score and self.cultural_appropriateness_score >= 0.85)

    def __repr__(self):
        return (f"<ProfessionalTemplate(id={self.id}, code='{self.template_code}', "
...
class DocumentGeneration(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False, index=True)
    template_id = Column(UUID(as_uuid=True), ForeignKey("professional_templates.id"), 
                        nullable=False, index=True)
    conversation_id = Column(UUID(as_uuid=True), ForeignKey("chat_conversations.id"), 
                            nullable=True, index=True)
    
    # Generation request information
    generation_request_id = Column(UUID(as_uuid=True), default=uuid.uuid4, index=True,
                                  comment="Unique generation request identifier")
    document_title = Column(String(200), nullable=False, comment="Generated document title")
    requested_language = Column(String(10), nullable=False, default="ar", index=True)
    requested_format = Column(String(20), nullable=False, default="pdf", index=True, 
                            comment="pdf, docx, html, txt")
    
    # Generation input data
    input_data = Column(JSONB, nullable=False, comment="Input data for document generation")
    template_variables = Column(JSONB, nullable=False, comment="Template variable values")
    customization_options = Column(JSONB, nullable=True, comment="Document customization options")
    cultural_preferences = Column(JSONB, nullable=True, comment="Cultural customization preferences")
    
    # Generation status and progress
    status = Column(String(30), nullable=False, default=GenerationStatus.REQUESTED.value, index=True)
    progress_percentage = Column(Integer, default=0, nullable=False, comment="0-100 generation progress")
    current_stage = Column(String(50), nullable=True, comment="Current generation stage")
    estimated_completion_time = Column(DateTime, nullable=True)
    
    # Generated content
    generated_content = Column(Text, nullable=True, comment="Generated document content")
    generated_content_arabic = Column(Text, nullable=True, comment="Arabic content with RTL formatting")
    generated_content_english = Column(Text, nullable=True, comment="English content portions")
    generated_file_path = Column(String(500), nullable=True, comment="Generated file storage path")
    generated_file_size_bytes = Column(Integer, nullable=True, comment="Generated file size")
    ...
    professional_standards_met = Column(Boolean, default=False, nullable=False, index=True)
    
    # Quality assessment and validation
    generation_quality_score = Column(Float, nullable=True, comment="0.0-1.0 generation quality")
    template_adherence_score = Column(Float, nullable=True, comment="0.0-1.0 template adherence")
    language_quality_score = Column(Float, nullable=True, comment="0.0-1.0 language quality")
    formatting_quality_score = Column(Float, nullable=True, comment="0.0-1.0 formatting quality")
    overall_satisfaction_score = Column(Float, nullable=True, comment="0.0-5.0 user satisfaction")
    
    # Human review and validation
    requires_human_review = Column(Boolean, default=False, nullable=False, index=True)
    human_review_completed = Column(Boolean, default=False, nullable=False)
    reviewed_by_user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    review_notes = Column(Text, nullable=True, comment="Human review notes and feedback")
    approved_by_reviewer = Column(Boolean, default=False, nullable=False)
    
    # Error handling and issues
    generation_errors = Column(Integer, default=0, nullable=False, comment="Number of generation errors")
    validation_warnings = Column(Integer, default=0, nullable=False, comment="Number of validation warnings")
    error_messages = Column(JSONB, nullable=True, comment="Error messages and details")
    retry_count = Column(Integer, default=0, nullable=False, comment="Generation retry attempts")
    recovery_attempted = Column(Boolean, default=False, nullable=False)
    
    # Performance metrics
    generation_time_ms = Column(Integer, nullable=True, comment="Generation time in milliseconds")
    validation_time_ms = Column(Integer, nullable=True, comment="Validation time in milliseconds")
    total_processing_time_ms = Column(Integer, nullable=True, comment="Total processing time")
    template_processing_time_ms = Column(Integer, nullable=True, comment="Template processing time")
    ai_processing_time_ms = Column(Integer, nullable=True, comment="AI processing time")
    
    # User interaction and feedback
    user_rating = Column(Integer, nullable=True, comment="1-5 user rating for generated document")
    user_feedback = Column(Text, nullable=True, comment="User feedback on generated document")
    regeneration_requested = Column(Boolean, default=False, nullable=False)
    modifications_requested = Column(JSONB, nullable=True, comment="Requested modifications")
    ...
    generation_metadata = Column(JSONB, nullable=True, comment="Generation configuration and settings")
    ai_model_data = Column(JSONB, nullable=True, comment="AI model information and parameters")
    cultural_analysis_data = Column(JSONB, nullable=True, comment="Cultural content analysis results")
    professional_validation_data = Column(JSONB, nullable=True, comment="Professional domain validation")
    quality_metrics = Column(JSONB, nullable=True, comment="Detailed quality assessment metrics")

    # Relationships
    user = relationship("User", foreign_keys=[user_id], back_populates="generated_documents")
    template = relationship("ProfessionalTemplate", back_populates="document_generations")
    ...
    __table_args__ = (
        ...
        CheckConstraint('generation_errors >= 0', name='ck_document_generation_generation_errors'),
        CheckConstraint('validation_warnings >= 0', name='ck_document_generation_validation_warnings'),
        CheckConstraint('retry_count >= 0', name='ck_document_generation_retry_count'),
        CheckConstraint('generated_file_size_bytes >= 0', name='ck_document_generation_generated_file_size_bytes'),
        ...
        CheckConstraint('generation_quality_score >= 0.0 AND generation_quality_score <= 1.0',
                       name='ck_document_generation_generation_quality_score'),
        CheckConstraint('template_adherence_score >= 0.0 AND template_adherence_score <= 1.0',
                       name='ck_document_generation_template_adherence_score'),
        CheckConstraint('language_quality_score >= 0.0 AND language_quality_score <= 1.0',
                       name='ck_document_generation_language_quality_score'),
        CheckConstraint('formatting_quality_score >= 0.0 AND formatting_quality_score <= 1.0',
                       name='ck_document_generation_formatting_quality_score'),
        CheckConstraint('overall_satisfaction_score >= 0.0 AND overall_satisfaction_score <= 5.0',
        ...
        Index('idx_document_generation_cultural_validation', 'cultural_validation_status', 'professional_standards_met'),
        Index('idx_document_generation_quality', 'generation_quality_score', 'requires_human_review'),
        Index('idx_document_generation_language_format', 'requested_language', 'requested_format'),
        Index('idx_document_generation_timing', 'generation_started_at', 'generation_completed_at'),
        Index('idx_document_generation_review', 'requires_human_review', 'human_review_completed'),
        
        # Full-text search support
        Index('idx_document_generation_title_fts', 'document_title', postgresql_using='gin',
              postgresql_ops={'document_title': 'gin_trgm_ops'}),
        Index('idx_document_generation_content_fts', 'generated_content', postgresql_using='gin',
    ...
    def is_high_quality_generation(self) -> bool:
        ...
        return (self.generation_quality_score and self.generation_quality_score >= 0.85 and
                self.islamic_compliance_score and self.islamic_compliance_score >= 0.90 and
                self.cultural_appropriateness_score and self.cultural_appropriateness_score >= 0.85 and
                self.professional_standards_met and
                self.generation_errors == 0)

    def __repr__(self):
        return (f"<DocumentGeneration(id={self.id}, user_id={self.user_id}, "
...
def update_generation_metrics(mapper, connection, target):
    ...
    if (target.status == GenerationStatus.COMPLETED.value and 
        target.generation_started_at and 
        target.generation_completed_at and 
