...
class FileStorage(Base):
    ...
    stored_filename = Column(
        String(255), nullable=False, comment="Secure storage filename"
    )
    ...
    file_extension = Column(
        String(10), nullable=False, index=True, comment="File extension"
    )

    # File classification and categorization
    file_type = Column(
        String(20), nullable=False, default=FileType.DOCUMENT.value, index=True
    )
    ...
    content_language = Column(
        String(10), nullable=True, index=True, comment="Detected content language"
    )
    ...
    professional_domain = Column(
        String(50), nullable=True, index=True, comment="Iraqi professional context"
    )
    cultural_validation_status = Column(
        String(30), nullable=False, default="pending", index=True
    )
    islamic_compliance_level = Column(
        String(30), nullable=False, default="neutral", index=True
    )
    cultural_appropriateness_score = Column(
        Float, nullable=True, comment="0.0-1.0 cultural appropriateness score"
    )
    contains_sensitive_content = Column(
        Boolean, default=False, nullable=False, index=True
    )
    requires_privacy_protection = Column(
        Boolean, default=False, nullable=False, index=True
    )

    # Security and access control
    encrypted = Column(
        Boolean, default=True, nullable=False, comment="File encryption status"
    )
    ...
    virus_scan_result = Column(
        String(20), nullable=True, index=True, comment="clean, infected, suspicious"
    )

    # Processing status and capabilities
    processing_status = Column(
        String(30), nullable=False, default=ProcessingStatus.UPLOADED.value, index=True
    )
    ocr_enabled = Column(
        Boolean, default=True, nullable=False, comment="OCR processing enabled"
    )
    ...
    quality_score = Column(
        Float, nullable=True, comment="0.0-1.0 document quality score"
    )
    ...
    retention_policy = Column(
        String(50), nullable=True, comment="Data retention policy"
    )
    deletion_scheduled_at = Column(DateTime, nullable=True, index=True)
    requires_legal_hold = Column(
        Boolean, default=False, nullable=False, comment="Legal hold prevents deletion"
    )

    # Access tracking and auditing
    download_count = Column(
        Integer, default=0, nullable=False, comment="Number of downloads"
    )
    ...
    professional_context_data = Column(
        JSONB, nullable=True, comment="Professional domain analysis"
    )
    ...
    ocr_results = relationship(
        "ArabicOCRResult", back_populates="file_storage", cascade="all, delete-orphan"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "file_size_bytes > 0", name="ck_file_storage_file_size_positive"
        ),
        CheckConstraint("page_count >= 0", name="ck_file_storage_page_count"),
        CheckConstraint("word_count >= 0", name="ck_file_storage_word_count"),
        CheckConstraint(
            "arabic_word_count >= 0", name="ck_file_storage_arabic_word_count"
        ),
        CheckConstraint(
            "english_word_count >= 0", name="ck_file_storage_english_word_count"
        ),
        ...
        CheckConstraint(
            "auto_delete_after_days >= 1", name="ck_file_storage_auto_delete_days"
        ),
        ...
        Index(
            "idx_file_storage_category_domain",
            "document_category",
            "professional_domain",
        ),
        Index(
            "idx_file_storage_cultural_validation",
    ...
    def is_arabic_document(self) -> bool:
        ...
        return self.contains_arabic and (
            self.content_language == "ar"
            or self.content_language == "ar-en"
            or (self.arabic_word_count and self.arabic_word_count > 0)
        )

    def __repr__(self):
        return (
            f"<FileStorage(id={self.id}, filename='{self.original_filename}', "
...
class DocumentProcessing(Base):
    ...
    processing_type = Column(
        String(30), nullable=False, index=True, comment="ocr, analysis, validation"
    )
    ocr_language = Column(
        String(10), nullable=False, default=OcrLanguage.AUTO_DETECT.value, index=True
    )
    processing_priority = Column(
        String(20), nullable=False, default="normal", index=True
    )
    batch_processing = Column(
        Boolean, default=False, nullable=False, comment="Part of batch processing"
    )
    ...
    progress_percentage = Column(
        Integer, default=0, nullable=False, comment="0-100 processing progress"
    )
    current_stage = Column(
        String(50), nullable=True, comment="Current processing stage"
    )
    estimated_completion_time = Column(
        DateTime, nullable=True, comment="ETA for processing completion"
    )
    ...
    extracted_text_arabic = Column(
        Text, nullable=True, comment="Arabic text with RTL formatting"
    )
    extracted_text_english = Column(
        Text, nullable=True, comment="English text portions"
    )
    text_extraction_confidence = Column(
        Float, nullable=True, comment="0.0-1.0 text extraction confidence"
    )
    ocr_accuracy_score = Column(
        Float, nullable=True, comment="0.0-1.0 OCR accuracy estimate"
    )

    # Document structure and layout analysis
    page_count = Column(Integer, nullable=True, comment="Number of processed pages")
    layout_detected = Column(
        Boolean, default=False, nullable=False, comment="Document layout detected"
    )
    tables_detected = Column(
        Boolean, default=False, nullable=False, comment="Tables found and processed"
    )
    images_detected = Column(
        Boolean, default=False, nullable=False, comment="Images found in document"
    )
    forms_detected = Column(
        Boolean, default=False, nullable=False, comment="Form fields detected"
    )
    signatures_detected = Column(
        Boolean, default=False, nullable=False, comment="Signatures detected"
    )

    # Language and content analysis
    primary_language_detected = Column(String(10), nullable=True, index=True)
    language_confidence_score = Column(
        Float, nullable=True, comment="0.0-1.0 language detection confidence"
    )
    arabic_dialect_detected = Column(
    ...
    rtl_layout_detected = Column(
        Boolean, default=False, nullable=False, comment="RTL layout detected"
    )

    # Iraqi professional document analysis
    document_type_detected = Column(
        String(50), nullable=True, index=True, comment="Detected Iraqi document type"
    )
    professional_domain = Column(
        String(50), nullable=True, index=True, comment="Detected professional domain"
    )
    government_form_detected = Column(
        Boolean, default=False, nullable=False, comment="Iraqi government form detected"
    )
    ...
    sensitive_content_detected = Column(
        Boolean, default=False, nullable=False, index=True
    )
    requires_human_review = Column(Boolean, default=False, nullable=False, index=True)

    # Quality metrics and validation
    processing_quality_score = Column(
        Float, nullable=True, comment="0.0-1.0 overall processing quality"
    )
    text_clarity_score = Column(Float, nullable=True, comment="0.0-1.0 text clarity")
    layout_accuracy_score = Column(
        Float, nullable=True, comment="0.0-1.0 layout analysis accuracy"
    )
    errors_detected = Column(
        Integer, default=0, nullable=False, comment="Number of processing errors"
    )
    warnings_generated = Column(
        Integer, default=0, nullable=False, comment="Number of warnings"
    )
    ...
    ocr_processing_time_ms = Column(
        Integer, nullable=True, comment="OCR processing time"
    )
    analysis_time_ms = Column(Integer, nullable=True, comment="Document analysis time")
    cpu_usage_percent = Column(
        Float, nullable=True, comment="CPU usage during processing"
    )
    ...
    processing_metadata = Column(
        JSONB, nullable=True, comment="Processing configuration and settings"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "errors_detected >= 0", name="ck_document_processing_errors_detected"
        ),
        CheckConstraint(
            "warnings_generated >= 0", name="ck_document_processing_warnings_generated"
        ),
        CheckConstraint("retry_count >= 0", name="ck_document_processing_retry_count"),
        # Score validation constraints
        CheckConstraint(
            "text_extraction_confidence >= 0.0 AND text_extraction_confidence <= 1.0",
            name="ck_document_processing_text_extraction_confidence",
        ),
        CheckConstraint(
            "ocr_accuracy_score >= 0.0 AND ocr_accuracy_score <= 1.0",
            name="ck_document_processing_ocr_accuracy_score",
        ),
        CheckConstraint(
            "language_confidence_score >= 0.0 AND language_confidence_score <= 1.0",
        ...
        CheckConstraint(
            "processing_quality_score >= 0.0 AND processing_quality_score <= 1.0",
        ...
        CheckConstraint(
            "layout_accuracy_score >= 0.0 AND layout_accuracy_score <= 1.0",
        ...
        Index(
            "idx_document_processing_language",
            "primary_language_detected",
            "arabic_dialect_detected",
        ),
        Index(
            "idx_document_processing_professional",
            "professional_domain",
            "document_type_detected",
        ),
        Index(
            "idx_document_processing_cultural",
            "cultural_validation_status",
            "sensitive_content_detected",
        ),
        Index(
            "idx_document_processing_quality",
            "processing_quality_score",
            "requires_human_review",
        ),
        Index(
            "idx_document_processing_timing",
            "processing_started_at",
            "processing_completed_at",
        ),
        # Full-text search support
        Index(
            "idx_document_processing_text_fts",
            "extracted_text",
            postgresql_using="gin",
            postgresql_ops={"extracted_text": "gin_trgm_ops"},
        ),
        Index(
            "idx_document_processing_arabic_fts",
    ...
    def is_high_quality_processing(self) -> bool:
        ...
        return (
            self.processing_quality_score
            and self.processing_quality_score >= 0.85
            and self.ocr_accuracy_score
            and self.ocr_accuracy_score >= 0.90
            and self.errors_detected == 0
        )

    def __repr__(self):
        return (
            f"<DocumentProcessing(id={self.id}, file_id={self.file_storage_id}, "
...
def update_processing_metrics(mapper, connection, target):
    ...
    if (
        target.status == ProcessingStatus.COMPLETED.value
        and target.processing_started_at
        and target.processing_completed_at
        and not target.processing_time_ms
    ):
...
def set_file_expiration(mapper, connection, target):
    ...
    if target.auto_delete_after_days and not target.expires_at:
        target.expires_at = datetime.utcnow() + timedelta(
            days=target.auto_delete_after_days
        )
