...
class IraqiI18nManager:
    ...
    def _setup_logging(self):
        ...
        handler.setFormatter(
            CulturalFormatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s")
        )
    ...
    async def register_translations(
        self, translations: Dict[str, Any], context: LocalizationContext
    ) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Registered {len(translations)} translations for {language_key}"
            )
            return True
    ...
    async def translate(
        ...
        if language_key in self.translations and key in self.translations[language_key]:
            ...
            fallback_key = (
                f"{IraqiDialect.STANDARD_ARABIC.value}_{context.domain.value}"
            )
            if (
                fallback_key in self.translations
                and key in self.translations[fallback_key]
            ):
                ...
                if (
                    general_key in self.translations
                    and key in self.translations[general_key]
                ):
    ...
    def format_cultural_content(
        self, content_type: str, value: Any, context: LocalizationContext
    ) -> str:
    ...
    # Cultural formatters
    def _format_islamic_date(
        self, date_value: datetime, context: LocalizationContext
    ) -> str:
    ...
    def _format_islamic_time(
        self, time_value: datetime, context: LocalizationContext
    ) -> str:
    ...
    def _format_arabic_numbers(
        self, number: Union[int, float], context: LocalizationContext
    ) -> str:
    ...
    def _format_iraqi_currency(
        self, amount: float, context: LocalizationContext
    ) -> str:
    ...
    def _format_arabic_name(self, name: str, context: LocalizationContext) -> str:
        ...
        if (
            context.gender_context == GenderContext.FEMALE
            and context.formality_level == "formal"
        ):
    ...
    def _format_iraqi_address(
        self, address: Dict[str, str], context: LocalizationContext
    ) -> str:
    ...
    def _format_rtl_table(self, table_data: List[List[str]]) -> str:
        ...
        for row in table_data:
            formatted_row = " | ".join(
                [self._format_rtl_text(cell) for cell in reversed(row)]
            )
    ...
    async def _create_translation_entry(
        self, key: str, translation_data: Any, context: LocalizationContext
    ) -> TranslationEntry:
        ...
        if isinstance(translation_data, str):
            ...
            arabic_text = translation_data.get(
                "ar", translation_data.get("arabic", key)
            )
            english_text = translation_data.get(
                "en", translation_data.get("english", key)
            )
        else:
    ...
    async def _select_text_variant(
        self, entry: TranslationEntry, context: LocalizationContext
    ) -> str:
        """Select appropriate text variant based on context"""

        # Check for gender-specific variants
        if entry.gender_variants and context.gender_context != GenderContext.NEUTRAL:
        ...
        if (
            entry.formality_variants
            and context.formality_level in entry.formality_variants
        ):
    ...
    async def _apply_parameter_substitution(
        self, text: str, parameters: Dict[str, Any], context: LocalizationContext
    ) -> str:
        ...
        for param_name, param_value in parameters.items():
            ...
            if placeholder in text:
                ...
                elif isinstance(param_value, (int, float)) and param_name.endswith(
                    "_amount"
                ):
    ...
    async def _apply_cultural_formatting(
        self, text: str, context: LocalizationContext
    ) -> str:
        """Apply final cultural formatting to translated text"""

        # Apply RTL formatting if needed
        if self._contains_arabic(text):
            text = self.apply_rtl_formatting(
                text, "mixed" if self._has_mixed_content(text) else "text"
            )
