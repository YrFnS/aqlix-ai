...
class PrayerTimeIslamicCalendarSystem:
    ...
    def __init__(self, default_city: IraqiCity = IraqiCity.BAGHDAD):
        ...
        self.logger.info(
            f"Prayer Time & Islamic Calendar System initialized for {default_city.english}"
        )
                ...
                IslamicMonth.MUHARRAM.number: [
                    {
                        ...
                        "english": "Islamic New Year",
                        "significance": "Beginning of Islamic year",
                    },
                    {
                        "day": 10,
                        "name": "يوم عاشوراء",
                        "english": "Day of Ashura",
                ...
                IslamicMonth.DHUL_HIJJAH.number: [
                    {
                        "day": 8,
                        "name": "يوم التروية",
                        "english": "Day of Tarwiyah",
                        "significance": "Hajj preparation day",
                    },
                    {
                        "day": 9,
                        "name": "يوم عرفة",
                        "english": "Day of Arafah",
    ...
    async def calculate_daily_prayer_times(
        self, date_input: Union[date, datetime], city: Optional[IraqiCity] = None
    ) -> List[PrayerTime]:
        """
        ...
        try:
            ...
            target_date = (
                date_input.date() if isinstance(date_input, datetime) else date_input
            )
            ...
            fajr_time = await self._calculate_fajr_time(
                target_date, target_city, params
            )
            ...
            maghrib_time = await self._calculate_maghrib_time(
                target_date, target_city, params
            )
            isha_time = await self._calculate_isha_time(
                target_date, target_city, params, maghrib_time
            )
            ...
            prayers = [
                ...
                PrayerTime(
                    ...
                    recommended_supplications=["اللهم أعني على ذكرك وشكرك وحسن عبادتك"],
                ),
                PrayerTime(
                    name=PrayerName.ASR,
                    time=asr_time,
                    ...
                    recommended_supplications=["حافظوا على الصلوات والصلاة الوسطى"],
                ),
                PrayerTime(
            ...
            self.performance_metrics["average_calculation_time_ms"] = (
                ...
                * (self.performance_metrics["prayer_calculations_today"] - 1)
                + calculation_time
            ) / self.performance_metrics["prayer_calculations_today"]
    ...
    async def coordinate_workflow_with_prayer_times(
        ...
        try:
            ...
            prayer_times = await self.calculate_daily_prayer_times(
                workflow_date, target_city
            )
            ...
            for prayer in prayer_times:
                ...
                prayer_buffer_end = prayer.time + timedelta(
                    minutes=30
                )  # Typical prayer duration

                # Check if workflow overlaps with prayer time + buffer
                if (
                    workflow_start < prayer_buffer_end
                    and workflow_end > prayer_buffer_start
                ):
    ...
    async def generate_ramadan_configuration(
        self, ramadan_year: int, city: Optional[IraqiCity] = None
        ...
        try:
            ...
            fasting_schedule = {
                ...
                "suhoor_recommended": (
                    fajr_time - timedelta(minutes=45)
                ).time(),  # Recommended suhoor time
            }
            ...
            cultural_activities = await self._get_ramadan_cultural_activities(
                current_phase
            )
            ...
            self.logger.info(
                f"Ramadan configuration generated for {current_phase.value} phase"
            )
    ...
    async def send_cultural_prayer_notification(
        self, prayer_time: PrayerTime, minutes_before: int = 10
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if time_until_prayer.total_seconds() <= minutes_before * 60:
                notification = {
                    ...
                    "cultural_greeting": await self._get_prayer_greeting(
                        prayer_time.name
                    ),
                    "recommended_preparation": await self._get_prayer_preparation(
                        prayer_time.name
                    ),
                ...
                self.logger.info(
                    f"Cultural prayer notification sent for {prayer_time.name.arabic}"
                )
                return notification
    ...
    async def _calculate_fajr_time(
        self, date_input: date, city: IraqiCity, params: Dict[str, float]
    ) -> datetime:
    ...
    async def _calculate_sunrise_time(
        self, date_input: date, city: IraqiCity
    ) -> datetime:
        """Calculate sunrise time"""
        solar_data = await self._get_solar_data(date_input, city)
        return solar_data["sunrise"].replace(tzinfo=self.iraq_timezone)

    async def _calculate_dhuhr_time(
        self, date_input: date, city: IraqiCity
    ) -> datetime:
    ...
    async def _calculate_qibla_direction(self, city: IraqiCity) -> float:
        ...
        x = math.cos(city_lat) * math.sin(makkah_lat) - math.sin(city_lat) * math.cos(
            makkah_lat
        ) * math.cos(delta_lon)
    ...
    async def _determine_current_prayer(
        self, prayers: List[PrayerTime], current_time: datetime
    ):
            ...
            for blessed_day in yearly_blessed:
                if blessed_day["day"] == day:
                    return True, {
                        ...
                        "cultural_observances": blessed_day.get(
...
if __name__ == "__main__":
    ...
    async def test_prayer_calendar_system():
        ...
        for prayer in prayer_times:
            print(
                f"{prayer.name.arabic} ({prayer.name.english}): {prayer.arabic_time_format}"
            )
        ...
        print(
            f"Hijri Date: {islamic_date.hijri_day} {islamic_date.hijri_month.arabic} {islamic_date.hijri_year}"
        )
        ...
        adjustment = await system.coordinate_workflow_with_prayer_times(
            workflow_start, workflow_duration, WorkflowPriority.MEDIUM
        )
        ...
        print(
            f"Cultural Activities: {len(ramadan_config.cultural_activities)} activities"
        )
        ...
        if prayer_times:
            next_prayer = next(
                (p for p in prayer_times if p.next_prayer_in), prayer_times[0]
            )
            notification = await system.send_cultural_prayer_notification(next_prayer)

            if notification:
                print(
                    f"Prayer: {notification['prayer_name_arabic']} ({notification['prayer_name_english']})"
                )
