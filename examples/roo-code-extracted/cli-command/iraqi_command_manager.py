...
class CommandMetadata:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'professional_domains': [domain.value for domain in self.professional_domains],
            'cultural_category': self.cultural_category.value,
            ...
            'cultural_validation': self.cultural_validation.to_dict() if self.cultural_validation else None
        }
...
class IraqiCulturalValidator:
    ...
    def validate_command(self, command: str, metadata: CommandMetadata) -> CulturalValidationResult:
        ...
        try:
            ...
            is_compliant = compliance_score >= self.cultural_config['validation_strictness']
            ...
            suggested_alternatives = self._generate_alternatives(command, metadata, compliance_score)
            ...
            return CulturalValidationResult(
                ...
                government_compatibility=professional_scores.get(ProfessionalDomain.GOVERNMENT, 0.5) >= 0.7,
            ...
            return CulturalValidationResult(
                ...
                professional_compatibility={domain: False for domain in ProfessionalDomain},
    ...
    def _validate_islamic_compliance(self, command: str, metadata: CommandMetadata) -> float:
        """Validate Islamic compliance requirements"""
        command_lower = command.lower()
        
        # Check for Islamic content markers
        islamic_content = any(keyword in command_lower for keyword in self.islamic_keywords)
    ...
    def _validate_family_appropriateness(self, command: str, metadata: CommandMetadata) -> float:
    ...
    def _validate_professional_domains(self, command: str, metadata: CommandMetadata) -> Dict[ProfessionalDomain, float]:
    ...
    def _validate_security_requirements(self, command: str, metadata: CommandMetadata) -> float:
    ...
    def _validate_arabic_compatibility(self, command: str, metadata: CommandMetadata) -> float:
    ...
    def _determine_cultural_category(self, command: str, metadata: CommandMetadata) -> CulturalCommandCategory:
    ...
    def _generate_alternatives(self, command: str, metadata: CommandMetadata, score: float) -> List[str]:
...
class IraqiCommandParser:
    ...
    def _parse_command_structure(self, command: str) -> Tuple[str, List[str], List[str]]:
    ...
    def _identify_professional_indicators(self, command: str) -> List[ProfessionalDomain]:
...
class IraqiCommandManager:
    ...
    def __init__(self, 
        ...
        """
        self.global_commands_dir = Path(global_commands_dir) if global_commands_dir else Path.home() / ".iraqi_ai" / "commands"
        self.project_commands_dir = Path(project_commands_dir) if project_commands_dir else Path.cwd() / ".iraqi_ai" / "commands"
        ...
        self.performance_metrics = {
            ...
            'professional_usage_stats': {domain.value: 0 for domain in ProfessionalDomain},
    ...
    async def _load_all_commands(self):
        ...
        try:
            ...
            await self._scan_command_directory(self.global_commands_dir, CommandSource.GLOBAL)
            
            # Load project commands (these override global ones)
            await self._scan_command_directory(self.project_commands_dir, CommandSource.PROJECT)
        ...
        for file_path in directory.rglob("*.md"):
            try:
                ...
                if command:
                    ...
                    if source == CommandSource.PROJECT or command_name not in self.commands:
                        self.commands[command_name] = command
                        
            except Exception as e:
                logging.warning(f"Failed to load command from {file_path}: {e}")
    
    async def _load_command_file(self, file_path: Path, command_name: str, source: CommandSource) -> Optional[IraqiCommand]:
        """Load a single command file"""
        try:
            content = file_path.read_text(encoding='utf-8')
            
            # Parse frontmatter and content
            frontmatter_data, command_content = self._parse_command_file_content(content)
            
            # Determine professional domain from path
            professional_domains = self._determine_professional_domains_from_path(file_path)
            ...
            metadata = CommandMetadata(
                ...
                cultural_category=CulturalCommandCategory(frontmatter_data.get('cultural_category', 'general')),
                security_level=CommandSecurityLevel(frontmatter_data.get('security_level', 'safe')),
                requires_cultural_validation=frontmatter_data.get('requires_cultural_validation', False),
            ...
            validation_result = self.cultural_validator.validate_command(command_content, metadata)
    ...
    def _determine_professional_domains_from_path(self, file_path: Path) -> Set[ProfessionalDomain]:
    ...
    async def get_commands(self, 
                          ...
                          include_arabic: bool = True) -> List[IraqiCommand]:
        ...
        if not include_arabic:
            commands = [
                cmd for cmd in commands
                if not cmd.metadata.is_arabic_compatible
            ]
        
        return commands
    
    async def get_command(self, name: str, source_preference: Optional[CommandSource] = None) -> Optional[IraqiCommand]:
        ...
        if source_preference:
            for command in self.commands.values():
                if command.metadata.name == name and command.metadata.source == source_preference:
    ...
    async def validate_command_execution(self, command_string: str, 
                                       user_domain: ProfessionalDomain = ProfessionalDomain.GENERAL) -> Dict[str, Any]:
        ...
        try:
            ...
            # Cultural validation
            cultural_analysis = await self._analyze_cultural_compliance(command_string, user_domain, parsing_result)
            
            # Professional domain validation
            professional_analysis = self._analyze_professional_compatibility(parsing_result, user_domain)
            
            # Make execution decision
            decision = self._make_execution_decision(security_analysis, cultural_analysis, professional_analysis)
            
            # Update performance metrics
            validation_time = time.time() - start_time
            self._update_performance_metrics(validation_time, cultural_analysis, security_analysis)
    ...
    def _analyze_security_risks(self, parsing_result: CommandParsingResult) -> Dict[str, Any]:
    ...
    async def _analyze_cultural_compliance(self, command_string: str, 
                                         user_domain: ProfessionalDomain,
                                         parsing_result: CommandParsingResult) -> Dict[str, Any]:
        ...
        # Perform cultural validation
        validation_result = self.cultural_validator.validate_command(command_string, temp_metadata)
    ...
    def _analyze_professional_compatibility(self, parsing_result: CommandParsingResult, 
                                          user_domain: ProfessionalDomain) -> Dict[str, Any]:
        """Analyze professional domain compatibility"""
        detected_domains = parsing_result['professional_indicators']
        
        # Check compatibility
        is_compatible = (
            not detected_domains or  # No specific domain detected
            user_domain in detected_domains or  # User domain matches
            user_domain == ProfessionalDomain.GENERAL  # General user can access most
        )
        
        # Check if professional approval is required
        requires_approval = any(
            domain in [ProfessionalDomain.LEGAL, ProfessionalDomain.MEDICAL, 
                      ProfessionalDomain.GOVERNMENT, ProfessionalDomain.RELIGIOUS]
    ...
    def _make_execution_decision(self, security_analysis: Dict[str, Any], 
                               cultural_analysis: Dict[str, Any],
                               professional_analysis: Dict[str, Any]) -> CommandDecision:
        ...
        if (security_analysis['requires_review'] or 
            cultural_analysis['compliance_score'] < 0.9):
            return CommandDecision.ASK_USER
        
        # Auto approve
        return CommandDecision.AUTO_APPROVE
    
    def _update_performance_metrics(self, validation_time: float, 
                                  cultural_analysis: Dict[str, Any],
        ...
        self.performance_metrics['cultural_compliance_rate'] = (
            (current_rate * (total_validations - 1) + (1.0 if is_compliant else 0.0)) / total_validations
        )
    ...
    async def get_command_suggestions(self, partial_input: str, 
        ...
        for command in self.commands.values():
            ...
            if (command.metadata.name.lower().startswith(partial_lower) or
                partial_lower in command.metadata.description.lower()):
                
                # Check domain compatibility
                if (user_domain in command.metadata.professional_domains or
                    ProfessionalDomain.GENERAL in command.metadata.professional_domains):
                    ...
                    suggestion = {
                        ...
                        'professional_domains': [domain.value for domain in command.metadata.professional_domains],
                        'is_arabic_compatible': command.metadata.is_arabic_compatible,
                        'match_score': self._calculate_match_score(partial_input, command)
                    }
    ...
    def _calculate_match_score(self, partial_input: str, command: IraqiCommand) -> float:
    ...
    async def create_command(self, name: str, content: str, 
                           metadata: CommandMetadata,
                           source: CommandSource = CommandSource.USER) -> bool:
        """Create a new command"""
        try:
            # Validate command content
            validation_result = self.cultural_validator.validate_command(content, metadata)
            ...
            file_path = self._get_command_file_path(name, source, metadata.professional_domains)
    ...
    def _get_command_file_path(self, name: str, source: CommandSource, 
                             domains: Set[ProfessionalDomain]) -> Path:
        """Get file path for command"""
        base_dir = self.global_commands_dir if source == CommandSource.GLOBAL else self.project_commands_dir
    ...
    async def _save_command_to_file(self, command: IraqiCommand, file_path: Path):
        ...
        try:
            ...
            if command.metadata.description_arabic:
                frontmatter_data['description_arabic'] = command.metadata.description_arabic
            ...
            if command.metadata.argument_hint_arabic:
                frontmatter_data['argument_hint_arabic'] = command.metadata.argument_hint_arabic
