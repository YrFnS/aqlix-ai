...
class ValidationConfig:
    ...
    professional_approval_required: Dict[str, bool] = field(
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "domain_compatibility": {
                domain.value: compat
                for domain, compat in self.domain_compatibility.items()
            },
...
class IraqiCommandSecurityAnalyzer:
    ...
    def analyze_security(self, command: str) -> SecurityAnalysis:
        ...
        try:
            ...
            security_risks = self._identify_security_risks(
                command, has_subshells, dangerous_patterns
            )
            ...
            risk_level = self._calculate_risk_level(
                has_subshells, dangerous_patterns, security_risks
            )

            # Specific risk categorization
            command_injection_risk = has_subshells or any(
                "injection" in risk for risk in security_risks
            )
            privilege_escalation_risk = any(
                "privilege" in pattern for pattern in dangerous_patterns
            )
            file_system_risk = any(
                "file_system" in pattern for pattern in dangerous_patterns
            )
    ...
    def _identify_security_risks(
        self, command: str, has_subshells: bool, dangerous_patterns: List[str]
    ) -> List[str]:
...
class IraqiCulturalAnalyzer:
    ...
    def analyze_cultural_compliance(self, command: str) -> CulturalAnalysis:
        ...
        try:
            ...
            concerns = self._identify_cultural_concerns(
                command, islamic_score, family_score, ethics_score
            )
    ...
    def _analyze_islamic_compliance(self, command: str) -> float:
        ...
        positive_content = any(
            keyword in command_lower for keyword in self.islamic_keywords["positive"]
        )

        # Check for negative/haram content
        negative_content = any(
            keyword in command_lower for keyword in self.islamic_keywords["negative"]
        )
    ...
    def _analyze_family_appropriateness(self, command: str) -> float:
        ...
        has_inappropriate = any(
            re.search(pattern, command_lower) for pattern in inappropriate_patterns
        )
    ...
    def _analyze_professional_ethics(self, command: str) -> float:
        ...
        ethics_content = any(
            keyword in command_lower for keyword in self.professional_ethics
        )
        ...
        has_unethical = any(
            re.search(pattern, command_lower) for pattern in unethical_patterns
        )
    ...
    def _identify_cultural_concerns(
        self,
        command: str,
        islamic_score: float,
        family_score: float,
...
class IraqiProfessionalAnalyzer:
    ...
    def analyze_professional_context(
        self, command: str, user_domain: ProfessionalDomain
        ...
        try:
            ...
            domain_compatibility = self._assess_domain_compatibility(
                detected_domains, user_domain
            )
            ...
            approval_domains = [
                ...
                if domain in self.approval_required_domains
            ]

            # Identify access restrictions
            access_restrictions = self._identify_access_restrictions(
                detected_domains, user_domain
            )

            # Calculate professional context score
            context_score = self._calculate_professional_score(
                detected_domains, user_domain, domain_compatibility
            )
    ...
    def _assess_domain_compatibility(
        ...
        for domain in ProfessionalDomain:
            if domain == ProfessionalDomain.GENERAL:
                ...
                domain in detected_domains and domain in self.approval_required_domains
            ):
    ...
    def _requires_professional_approval(
        self, detected_domains: List[ProfessionalDomain]
    ) -> bool:
        """Check if professional approval is required"""
        return any(
            domain in self.approval_required_domains for domain in detected_domains
        )

    def _identify_access_restrictions(
        self,
        detected_domains: List[ProfessionalDomain],
        user_domain: ProfessionalDomain,
    ) -> List[str]:
    ...
    def _calculate_professional_score(
        self,
        detected_domains: List[ProfessionalDomain],
        user_domain: ProfessionalDomain,
        compatibility: Dict[ProfessionalDomain, bool],
    ) -> float:
        """Calculate professional context score"""
        ...
        compatible_count = sum(
            1 for domain in detected_domains if compatibility.get(domain, False)
        )
...
class IraqiCommandValidator:
    ...
    async def validate_command(
        self, command: str, user_domain: ProfessionalDomain = ProfessionalDomain.GENERAL
    ) -> ValidationResult:
        ...
        try:
            ...
            cultural_analysis = self.cultural_analyzer.analyze_cultural_compliance(
                command
            )
            professional_analysis = (
                self.professional_analyzer.analyze_professional_context(
                    command, user_domain
                )
            )
    ...
    def _extract_command_patterns(self, command: str) -> List[str]:
        ...
        for subcmd in subcommands:
            ...
            for i in range(1, min(len(tokens), 3)):
                ...
                if tokens[i].startswith("-") or any(
                    char in tokens[i] for char in ["/", "\\", "~", ":"]
                ):
                    break
    ...
    def _find_longest_prefix_matches(
        self, command: str
    ...
    def _make_validation_decision(
        self,
        longest_allowed: Optional[str],
        longest_denied: Optional[str],
        security_analysis: SecurityAnalysis,
        cultural_analysis: CulturalAnalysis,
        professional_analysis: ProfessionalAnalysis,
    ) -> Tuple[CommandDecision, float, List[str]]:
        ...
        if not cultural_analysis.is_culturally_compliant:
            if cultural_analysis.cultural_score < 0.3:
                return (
                    CommandDecision.AUTO_DENY,
                    confidence,
                    ["Severe cultural compliance violation"],
                )
            else:
                return (
                    CommandDecision.CULTURAL_REVIEW,
                    0.7,
        ...
        if professional_analysis.requires_professional_approval:
            return (
                CommandDecision.PROFESSIONAL_APPROVE,
                0.8,
                ["Professional approval required"],
            )
        ...
        if security_analysis.risk_level == SecurityRiskLevel.HIGH:
            return (
                CommandDecision.ASK_USER,
                0.6,
                ["High security risk requires user confirmation"],
            )
        ...
        if longest_denied and longest_allowed:
            if len(longest_denied) > len(longest_allowed):
                ...
                rationale.append(
                    f"Approved by longer prefix match: '{longest_allowed}'"
                )
                return CommandDecision.AUTO_APPROVE, confidence, rationale
            else:
                # Equal length - default to denial for safety
                rationale.append(
                    "Equal prefix matches - defaulting to denial for safety"
                )
                return CommandDecision.AUTO_DENY, confidence, rationale
        elif longest_denied:
        ...
        if (
            security_analysis.risk_level == SecurityRiskLevel.MEDIUM
            or cultural_analysis.requires_review
        ):
            return (
                CommandDecision.ASK_USER,
                0.5,
                ["Review required due to security or cultural concerns"],
            )
        ...
        return (
            CommandDecision.ASK_USER,
            0.3,
            ["No specific rules found - user decision required"],
        )
    ...
    def _update_metrics(self, result: ValidationResult):
        ...
        self.metrics["average_validation_time"] = (
            current_avg * (total - 1) + result.validation_time_ms
        ) / total
    ...
    async def main():
        ...
        for command in test_commands:
            result = await validator.validate_command(
                command, ProfessionalDomain.GENERAL
            )
