...
class IraqiToolRepetitionDetector:
    ...
    def record_tool_usage(self, 
        ...
        record = ToolUsageRecord(
            ...
            result_hash=self._calculate_result_hash(result) if result is not None else None,
            success=success,
        ...
        if len(self.parameter_patterns[tool_name]) > 100:
            self.parameter_patterns[tool_name] = self.parameter_patterns[tool_name][-50:]
    ...
    async def detect_repetition_patterns(self, 
                                       tool_name: Optional[str] = None) -> List[RepetitionPattern]:
        ...
        patterns.sort(key=lambda p: (p.pattern_confidence, p.last_occurrence), reverse=True)
        
        return patterns
    
    async def get_optimization_suggestions(self, 
                                         tool_name: Optional[str] = None) -> Dict[str, List[str]]:
        ...
        for tool, tool_suggestions in suggestions.items():
            cultural_suggestions = await self._generate_cultural_optimization_suggestions(tool)
    ...
    def get_usage_statistics(self) -> Dict[str, Any]:
        ...
        now = datetime.now()
        recent_calls = sum(1 for record in self.tool_history 
                          if now - record.timestamp <= self.detection_window)
        
        # Cultural context analysis
        arabic_content_calls = sum(1 for record in self.tool_history 
                                 if record.arabic_content_detected)
        ...
        top_tools = sorted(self.tool_frequencies.items(), 
        ...
        if any(record.execution_time_ms for record in self.tool_history):
            execution_times = [record.execution_time_ms for record in self.tool_history 
        ...
        return {
            ...
            'arabic_content_percentage': (arabic_content_calls / total_calls * 100) if total_calls > 0 else 0,
    ...
    async def _analyze_tool_patterns(self, tool_name: str) -> List[RepetitionPattern]:
        ...
        tool_records = [record for record in self.tool_history 
    ...
    async def _detect_identical_patterns(self, records: List[ToolUsageRecord]) -> List[RepetitionPattern]:
        ...
        for param_hash, group_records in parameter_groups.items():
            if len(group_records) >= 2:
                ...
                recent_records = [r for r in group_records 
                ...
                if len(recent_records) >= 2:
                    ...
                    pattern.cultural_considerations = await self._analyze_cultural_implications(
    ...
    async def _detect_similar_patterns(self, records: List[ToolUsageRecord]) -> List[RepetitionPattern]:
        ...
        for i in range(len(records)):
            ...
            if len(similar_records) >= 2:
                ...
                recent_records = [r for r in similar_records 
                                if datetime.now() - r.timestamp <= self.detection_window]
                
                if len(recent_records) >= 2:
                    avg_similarity = sum(
                        self._calculate_parameter_similarity(base_record.parameters, r.parameters)
    ...
    async def _detect_iterative_patterns(self, records: List[ToolUsageRecord]) -> List[RepetitionPattern]:
    ...
    async def _detect_circular_patterns(self, records: List[ToolUsageRecord]) -> List[RepetitionPattern]:
        ...
        if len(records) >= 4:
            ...
            if len(recent_records) >= 4:
                param_sequence = [self._calculate_parameter_hash(r.parameters) 
                                for r in recent_records[-6:]]  # Last 6 calls
    ...
    async def _detect_cultural_loop_patterns(self, records: List[ToolUsageRecord]) -> List[RepetitionPattern]:
        ...
        if len(cultural_records) >= 3:
            recent_cultural = [r for r in cultural_records 
                             if datetime.now() - r.timestamp <= self.detection_window]
            ...
            if len(recent_cultural) >= 3:
                ...
                for context, count in context_counts.items():
                    if count >= 3:
                        matching_records = [r for r in recent_cultural 
                                          if r.cultural_context == context]
    ...
    async def _generate_optimization_suggestions(self, pattern: RepetitionPattern) -> List[str]:
        """Generate optimization suggestions based on pattern type"""
    ...
    async def _generate_cultural_optimization_suggestions(self, tool_name: str) -> List[str]:
        ...
        if arabic_records:
            ...
            mixed_content_count = sum(1 for r in arabic_records 
                                    if self._has_mixed_content(r.parameters))
        ...
        if domain_counts:
            ...
            if dominant_domain[1] > len(tool_records) * 0.6:
                suggestions.append(f"Optimize for {dominant_domain[0]} domain-specific patterns")
        
        return suggestions
    
    # Cultural analysis methods
    
    def _analyze_cultural_context(self, tool_name: str, parameters: Dict[str, Any]) -> Optional[str]:
        """Analyze cultural context of tool usage"""
        
        # Check for cultural validation tools
        cultural_tools = {
            'cultural_validator', 'islamic_compliance_checker', 
            'political_neutrality_validator', 'professional_etiquette_checker'
        }
        ...
        if any(term in param_text for term in ['islamic', 'cultural', 'appropriate', 'compliance']):
            return "cultural_validation"
        elif any(term in param_text for term in ['professional', 'formal', 'business']):
            return "professional_context"
        elif any(term in param_text for term in ['educational', 'academic', 'learning']):
    ...
    def _detect_professional_domain(self, tool_name: str, parameters: Dict[str, Any]) -> Optional[str]:
        ...
        domain_indicators = {
            ...
            'government': ['government', 'ministry', 'official', 'public', 'administrative']
        }
        
        for domain, indicators in domain_indicators.items():
            if any(indicator in param_text or indicator in tool_text 
    ...
    async def _analyze_cultural_implications(self, tool_name: str, 
                                          records: List[ToolUsageRecord]) -> List[str]:
        """Analyze cultural implications of repetition patterns"""
        
        implications = []
        
        # Check for cultural validation patterns
        if any('cultural' in tool_name.lower() or 'validation' in tool_name.lower() 
              for record in records):
        ...
        if len(domains) == 1:
            domain = domains.pop()
            implications.append(f"Domain-specific optimization opportunity for {domain}")
    ...
    def _initialize_optimization_patterns(self):
        ...
        self.cultural_validation_chains = {
            'islamic_compliance': ['content_analysis', 'cultural_check', 'final_validation'],
            'professional_etiquette': ['domain_check', 'formality_analysis', 'appropriateness_validation'],
            'political_neutrality': ['content_scan', 'bias_detection', 'neutrality_confirmation']
        }
    
    # Cultural analyzer implementations (simplified)
    
    async def _analyze_islamic_compliance_pattern(self, records: List[ToolUsageRecord]) -> Dict[str, Any]:
        """Analyze Islamic compliance validation patterns"""
        return {"pattern": "islamic_compliance", "optimization": "cache_validation_results"}
    
    async def _analyze_political_neutrality_pattern(self, records: List[ToolUsageRecord]) -> Dict[str, Any]:
        """Analyze political neutrality validation patterns"""
        return {"pattern": "political_neutrality", "optimization": "batch_content_analysis"}
    
    async def _analyze_professional_etiquette_pattern(self, records: List[ToolUsageRecord]) -> Dict[str, Any]:
        """Analyze professional etiquette validation patterns"""
        return {"pattern": "professional_etiquette", "optimization": "domain_specific_cache"}
    
    async def _analyze_gender_sensitivity_pattern(self, records: List[ToolUsageRecord]) -> Dict[str, Any]:
        """Analyze gender sensitivity validation patterns"""
        return {"pattern": "gender_sensitivity", "optimization": "context_aware_validation"}
    
    # Domain pattern detectors (simplified implementations)
    
    async def _detect_legal_domain_patterns(self, records: List[ToolUsageRecord]) -> Dict[str, Any]:
        """Detect legal domain-specific patterns"""
        return {"domain": "legal", "patterns": ["contract_analysis", "case_law_lookup"]}
    
    async def _detect_medical_domain_patterns(self, records: List[ToolUsageRecord]) -> Dict[str, Any]:
        """Detect medical domain-specific patterns"""
        return {"domain": "medical", "patterns": ["diagnosis_support", "medical_terminology"]}
    
    async def _detect_educational_domain_patterns(self, records: List[ToolUsageRecord]) -> Dict[str, Any]:
        """Detect educational domain-specific patterns"""
        return {"domain": "educational", "patterns": ["curriculum_analysis", "learning_assessment"]}
    
    async def _detect_government_domain_patterns(self, records: List[ToolUsageRecord]) -> Dict[str, Any]:
        """Detect government domain-specific patterns"""
        return {"domain": "government", "patterns": ["policy_analysis", "regulatory_compliance"]}
    
    # Arabic processing optimizers (simplified implementations)
    
    async def _optimize_rtl_formatting_calls(self, records: List[ToolUsageRecord]) -> Dict[str, Any]:
        """Optimize RTL formatting tool calls"""
        return {"optimization": "rtl_cache", "savings_potential": "40%"}
    
    async def _optimize_dialect_detection_calls(self, records: List[ToolUsageRecord]) -> Dict[str, Any]:
        """Optimize dialect detection tool calls"""
        return {"optimization": "dialect_batch_processing", "savings_potential": "60%"}
    
    async def _optimize_mixed_content_processing(self, records: List[ToolUsageRecord]) -> Dict[str, Any]:
