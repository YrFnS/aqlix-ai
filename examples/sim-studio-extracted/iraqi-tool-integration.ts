...
export class IraqiToolIntegrationManager extends EventEmitter {
  ...
  public getAvailableTools(
    ...
    if (context) {
      tools = tools.filter((tool) => this.isToolAccessible(tool, context));
    }

    return tools.sort(
      (a, b) => a.metadata.performanceRating - b.metadata.performanceRating,
    );
  }
  ...
  public async executeTool(
    ...
    try {
      ...
      if (
        culturalValidation.culturalScore < 70 ||
        islamicValidation.complianceScore < 70
      ) {
        const criticalIssues = [
          ...culturalValidation.culturalIssues.filter(
            (i) => i.severity === "critical",
          ),
          ...islamicValidation.islamicIssues.filter(
            (i) => i.severity === "critical",
          ),
        ];
        ...
        if (criticalIssues.length > 0) {
          throw new Error(
            `Critical cultural/Islamic compliance issues prevent tool execution: ${criticalIssues
              .map((i) => i.description)
              .join(", ")}`,
          );
        }
      ...
      const executionResult = await tool.executionHandler.execute(
        parameters,
        context,
      );
        ...
        errors: [
          {
            ...
            suggestions: [
              ...
              "Review Islamic settings",
            ],
            suggestionsArabic: [
              "تحقق من معاملات الأداة",
              "تحقق من السياق الثقافي",
              "راجع الإعدادات الإسلامية",
            ],
          },
        ],
      };
  ...
  private validateParameter(
    ...
    if (param.validation) {
      ...
      if (
        ...
        !param.validation.pattern.test(String(value))
      ) {
        return {
          valid: false,
    ...
    switch (type) {
      ...
      case ToolParameterType.FILE:
        return (
          ...
          (typeof value === "object" && value.name && value.data)
        );
  ...
  private isToolAccessible(
    ...
    if (tool.culturalSensitivity === CulturalSensitivityLevel.CRITICAL) {
      return (
        context.culturalValidationRequired &&
  ...
  private async enhanceExecutionResult(
    ...
    const enhancedCulturalValidation =
      await this.culturalValidator.enhanceValidation(
    ...
    const enhancedIslamicValidation =
      await this.islamicValidator.enhanceValidation(
  ...
  private validateToolDefinition(tool: IraqiToolDefinition): void {
    ...
    if (
      ...
      typeof tool.executionHandler.execute !== "function"
    ) {
    ...
    if (
      tool.culturalSensitivity >= CulturalSensitivityLevel.HIGH &&
      tool.culturalValidationRules.length === 0
    ) {
      throw new Error(
        "High cultural sensitivity tools must include validation rules",
      );
    }

    if (
      tool.islamicCompliance >= IslamicComplianceLevel.COMPLIANT &&
  ...
  private async registerBuiltInCulturalTools(): Promise<void> {
    ...
    this.registerTool({
      ...
      description:
        "Advanced Arabic natural language processing with Iraqi dialect support",
      descriptionArabic:
        "معالجة متقدمة للغة الطبيعية العربية مع دعم اللهجة العراقية",
...
class IraqiCulturalValidator {
  ...
  async enhanceValidation(
    existing: CulturalValidationResult,
    output: any,
    context: IraqiCulturalContext,
  ): Promise<CulturalValidationResult> {
...
class IraqiIslamicValidator {
  ...
  async enhanceValidation(
    existing: IslamicValidationResult,
    output: any,
    settings: IslamicComplianceSettings,
  ): Promise<IslamicValidationResult> {
