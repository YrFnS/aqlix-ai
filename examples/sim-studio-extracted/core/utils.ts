...
export function validateCulturalCompliance(
  ...
  if (config.islamicCompliance.enabled) {
    const islamicResult = validateIslamicCompliance(
      content,
      config.islamicCompliance.level,
    );
  ...
  if (config.languageSupport.arabic) {
    const languageResult = validateArabicSupport(
      content,
...
function validateIraqiDialect(content: string): {
  ...
  const hasIraqiPatterns = iraqiPatterns.some((pattern) =>
...
export function processArabicText(
  ...
  const iraqiPatterns = [/شلونك/, /شكو ماكو/, /يا زين/, /عاش/, /زين/];
...
export function executeWithCulturalValidation(
  ...
  return new Promise(async (resolve) => {
    ...
    try {
      ...
      const culturalValidation = validateCulturalCompliance(
        output,
