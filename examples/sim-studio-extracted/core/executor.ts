...
export class WorkflowExecutor {
  ...
  async executeWorkflow(startNodeId?: string): Promise<{
    ...
    try {
      ...
      culturalResults.overallScore = validationCount > 0 ? totalScore / validationCount : 100
  ...
  private async executeBlockLogic(
    ...
    const processedInputs = await this.processInputs(inputs, blockConfig, context)
    ...
    switch (node.type) {
      ...
      default:
        ...
        return this.executeGenericBlock(node, processedInputs, blockConfig, context)
    }
  }
  ...
  private async resolveNodeInputs(node: WorkflowNode): Promise<Record<string, any>> {
    const inputs: Record<string, any> = { ...node.data }

    // Find incoming edges and resolve connected values
    const incomingEdges = this.context.edges.filter(edge => edge.target === node.id)
    ...
    for (const edge of incomingEdges) {
      ...
      if (sourceResult && sourceResult.success && sourceResult.output) {
        ...
        inputs[targetKey] = sourceResult.output[sourceKey] || sourceResult.output
      }
    }
    ...
    for (const [key, value] of Object.entries(inputs)) {
      ...
      if (inputConfig) {
        ...
        if (inputConfig.culturalValidation && typeof value === 'string') {
          const validation = validateCulturalCompliance(value, context.culturalSettings)
          if (!validation.passed) {
            throw new Error(`Cultural validation failed for input ${key}: ${validation.issues.join(', ')}`)
          }
        }
  ...
  private getStartingNodes(): WorkflowNode[] {
    ...
    const startingNodeIds = Array.from(nodeIds).filter(id => !targetIds.has(id))
    return this.context.nodes.filter(node => startingNodeIds.includes(node.id))
  }
