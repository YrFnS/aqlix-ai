...
export const getProfessionalDomainBlocks = (): BlockConfig[] => {
  return Object.values(iraqiBlockRegistry).filter(
    (block) => block.category === "professional-domains",
  );
};
...
export const isValidBlockType = (type: string): type is string =>
  type in iraqiBlockRegistry;

export const getAllBlocks = (): BlockConfig[] =>
  Object.values(iraqiBlockRegistry);
...
export const getCulturallyAppropriateBlocnxks = (
  ...
  return Object.values(iraqiBlockRegistry).filter((block) => {
    ...
    if (professionalContext && block.iraqiEnhancements?.professionalDomains) {
      return block.iraqiEnhancements.professionalDomains.supportedDomains.includes(
        professionalContext,
      );
    }
