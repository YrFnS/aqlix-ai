...
const getAvailableWorkflows = (): Array<{ label: string; id: string }> => {
  try {
    ...
    const availableWorkflows = [
      ...
      { label: 'سير عمل تعليمي / Educational Workflow', id: 'educational-workflow' },
      { label: 'سير عمل تنظيمي / Organizational Workflow', id: 'organizational-workflow' },
      { label: 'معالجة الدفع / Payment Processing', id: 'payment-workflow' },
      { label: 'معالجة النصوص العربية / Arabic Text Processing', id: 'arabic-processing-workflow' },
...
export const WorkflowBlock: BlockConfig = {
  ...
  longDescription: 'Execute child workflows with Iraqi cultural intelligence and professional domain validation',
  category: 'blocks',
  bgColor: '#705335',
  icon: WorkflowIcon,
  ...
  subBlocks: [
...

    {
      ...
      description: 'This variable will be available as start.input in the child workflow with Arabic support',
      required: false,
      rtlSupport: true,
      arabicKeyboard: true,
    },
