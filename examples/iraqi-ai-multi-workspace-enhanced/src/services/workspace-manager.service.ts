...
export class IraqiWorkspaceManager extends EventEmitter {
  ...
  async createWorkspace(
    ...
    const limits =
      validatedData.limits || this.getDefaultLimits(validatedData.type);
    ...
    const workspace: IraqiWorkspace = {
      ...
      certificationLevel: this.inferCertificationLevel(
    ...
    await this.redis.sadd(
      `workspaces:visibility:${validatedData.visibility}`,
      workspaceId,
    );
    ...
    if (filters) {
      ...
      if (filters.visibility) {
        workspaces = workspaces.filter(
          (w) => w.visibility === filters.visibility,
        );
      }
    }
  ...
  async getWorkspacesByType(
    ...
    if (options?.visibility) {
      workspaces = workspaces.filter((w) =>
        options.visibility!.includes(w.visibility),
      );
    }
  ...
  async updateWorkspace(
    ...
    await this.redis.hset(
      `workspace:${workspaceId}`,
      this.serializeWorkspace(updatedWorkspace),
    );
    ...
    if (updates.visibility && updates.visibility !== workspace.visibility) {
      await this.redis.srem(
        `workspaces:visibility:${workspace.visibility}`,
        workspaceId,
      );
      await this.redis.sadd(
        `workspaces:visibility:${updates.visibility}`,
        workspaceId,
      );
    }
  ...
  async createWorkspaceFromTemplate(
    ...
    const template = this.getWorkspaceTemplates().find(
      (t) => t.id === templateId,
    );
    if (!template) {
  ...
  private getDefaultSubscriptionTier(
    type: ProfessionalDomain,
  ): "free" | "basic" | "professional" | "enterprise" | "government" {
    const tiers = {
  ...
  private inferCertificationLevel(
    specializations: string[],
  ): "basic" | "standard" | "expert" | "master" {
  ...
  private serializeWorkspace(
    ...
    return {
      ...
      customFields: workspace.customFields
        ? JSON.stringify(workspace.customFields)
        : "",
      governorate: workspace.governorate
        ? JSON.stringify(workspace.governorate)
        : "",
      allowedIpRanges: workspace.allowedIpRanges
        ? JSON.stringify(workspace.allowedIpRanges)
        : "",
      ...
      subscriptionExpiresAt:
        workspace.subscriptionExpiresAt?.toISOString() || "",
    };
  }
  ...
  private deserializeWorkspace(data: any): IraqiWorkspace {
    return {
      ...
      enabledFeatures:
        typeof data.enabledFeatures === "string"
      ...
      integrations:
        typeof data.integrations === "string"
          ...
          : data.integrations,
      customFields:
        data.customFields && typeof data.customFields === "string"
          ? JSON.parse(data.customFields)
          : data.customFields,
      governorate:
        data.governorate && typeof data.governorate === "string"
          ? JSON.parse(data.governorate)
          : data.governorate,
      allowedIpRanges:
        data.allowedIpRanges && typeof data.allowedIpRanges === "string"
  ...
  private serializeMember(member: WorkspaceMember): Record<string, string> {
    return {
      ...
      professionalCertifications: JSON.stringify(
        member.professionalCertifications,
      ),
      governorate: member.governorate ? JSON.stringify(member.governorate) : "",
      ipRestrictions: member.ipRestrictions
        ? JSON.stringify(member.ipRestrictions)
        : "",
  ...
  private deserializeMember(data: any): WorkspaceMember {
    return {
      ...
      professionalCertifications:
        typeof data.professionalCertifications === "string"
      ...
      governorate:
        data.governorate && typeof data.governorate === "string"
          ...
          : data.governorate,
      ipRestrictions:
        data.ipRestrictions && typeof data.ipRestrictions === "string"
  ...
  async getWorkspaceCount(filters?: {
    ...
    if (filters?.type) {
      ...
      count = await this.redis.scard(
        `workspaces:visibility:${filters.visibility}`,
      );
    } else {
