...
export class ArabicWorkspaceOrganizerService {
  ...
  public generateArabicWorkspaceName(
    ...
    const primaryName = this.constructProfessionalArabicName(
      baseName,
      domain,
      convention,
      governorate,
  ...
  private constructProfessionalArabicName(
    ...
    switch (convention) {
      case "formal":
        arabicName = this.applyFormalConvention(
          arabicName,
          domain,
          governorate,
        );
      ...
      case "religious":
        ...
        break;
      case "government":
        arabicName = this.applyGovernmentConvention(
          arabicName,
          domain,
          governorate,
        );
  ...
  private applyFormalConvention(
    ...
    switch (domain) {
      ...
      case "government":
        if (governorate) {
          const govName =
            this.governorateNames[governorate]?.name || governorate;
   ...
   */
  private applyTraditionalConvention(
    name: string,
    domain: ProfessionalDomain,
  ): string {
    const domainTerms = this.professionalTerminology[domain];
   ...
   */
  private applyReligiousConvention(
    name: string,
    domain: ProfessionalDomain,
  ): string {
  ...
  private applyGovernmentConvention(
    ...
    if (governorate) {
      ...
      governmentTitle = govData
        ? `${domainTerms.directorate} ${govData.name} - `
        : "";
    }
   ...
   */
  private applyAcademicConvention(
    name: string,
    domain: ProfessionalDomain,
  ): string {
    const domainTerms = this.professionalTerminology[domain];
   ...
   */
  private applyCommercialConvention(
    name: string,
    domain: ProfessionalDomain,
  ): string {
    const domainTerms = this.professionalTerminology[domain];
  ...
  private enhanceArabicName(
    ...
    return this.addProfessionalContext(
      arabicName,
      domain,
      convention,
      governorate,
   ...
   */
  private analyzeFormalityLevel(
    content: string,
  ): ArabicContentAnalysis["formalityLevel"] {
  ...
  private extractProfessionalTerms(content: string): string[] {
    const allProfessionalTerms = Object.values(
      this.professionalTerminology,
   ...
   */
  private categorizeWorkspaceName(
    name: string,
    domain: ProfessionalDomain,
  ): ArabicNameCategory {
   ...
   */
  private containsProfessionalTerms(
    name: string,
    domain: ProfessionalDomain,
  ): boolean {
  ...
  private translateToEnglish(
    ...
    for (const [english, arabic] of Object.entries(
      this.professionalTerminology[domain],
    )) {
   ...
   */
  private organizeByCulturalCategory(
    workspaces: IraqiWorkspace[],
  ): Record<string, IraqiWorkspace[]> {
    const categories = {
      ديني: "religious",
   ...
   */
  private organizeAlphabetically(
    workspaces: IraqiWorkspace[],
  ): Record<string, IraqiWorkspace[]> {
  ...
  public generateWorkspaceSuggestions(
    ...
    const conventions: ArabicNamingConvention[] = [
      "modern",
      "formal",
      "traditional",
    ];
    ...
    for (const convention of conventions) {
      ...
      const suggestion = this.generateArabicWorkspaceName(
        input,
        domain,
        convention,
      );
    ...
    const majorGovernorates: IraqiGovernorate[] = [
      "baghdad",
      "basra",
      "nineveh",
      "arbil",
...
export const arabicWorkspaceOrganizerService =
  new ArabicWorkspaceOrganizerService();
