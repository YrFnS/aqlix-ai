...
export const Messages: React.FC<MessagesProps> = ({
  ...
  const { processMessage, detectLanguage, formatMessage, addMessageMetadata } =
  ...
  const { formatArabicText, detectTextDirection, renderBidirectionalText } =
  ...
  const processedMessages = useMemo(() => {
    return messages.map((message) => {
      ...
      const textDirection = rtlSupport
      ...
      return {
        ...
        textDirection,
        metadata: {
          culturalScore: culturalValidation
            ? getCulturalScore(message.content)
            : undefined,
          islamicCompliance: culturalValidation
            ? getIslamicComplianceStatus(message.content)
            : undefined,
          professionalScore: professionalDomain
            ? getProfessionalAppropriatenessScore(message.content)
            : undefined,
    ...
    >
      ...
      {showActions && (
        <MessageActions
          ...
          onEdit={
            onMessageEdit
              ? (newContent) => onMessageEdit(message.id, newContent)
              : undefined
          }
          onDelete={
            onMessageDelete ? () => onMessageDelete(message.id) : undefined
          }
          className={
            message.textDirection === "rtl" ? "flex-row-reverse" : "flex-row"
          }
