...
import type { IraqiChatConfig, ProfessionalDomain, ArabicLanguage } from '~/types/iraqi-chat';
...
export const BaseChat = React.forwardRef<HTMLDivElement, BaseChatProps>(({
  ...
  const [currentLanguage, setCurrentLanguage] = useState<ArabicLanguage>(language);
  const [selectedDomain, setSelectedDomain] = useState<ProfessionalDomain | undefined>(professionalDomain);
  ...
  const handleSubmit = useCallback(async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const contextualText = await addCulturalContext(optimizedText, selectedDomain);
  ...
  }, [voiceSupported, voiceEnabled, isListening, startListening, stopListening]);
      ...
      try {
        ...
        if (file.type.startsWith('text/')) {
          ...
          if (culturalValidation) {
            ...
            if (!validation.isValid) {
              toast.error(`File "${file.name}" contains inappropriate content`);
              continue;
            }
          }
        }
  ...
  const handleLanguageChange = useCallback((newLanguage: ArabicLanguage) => {
    setCurrentLanguage(newLanguage);
    updateSettings({ language: newLanguage });
    toast.success(`Language changed to ${newLanguage === 'arabic' ? 'Arabic' : 'English'}`);
  }, [updateSettings]);

  const handleDomainChange = useCallback((domain: ProfessionalDomain | undefined) => {
    ...
    <div className={`flex items-center justify-between p-4 border-b bg-white/80 backdrop-blur-sm ${
      ...
      <div className="flex items-center gap-3">
        <h1 className="text-xl font-bold text-gray-800">
          {currentLanguage === 'arabic' ? 'مساعد الذكاء الاصطناعي العراقي' : 'Iraqi AI Assistant'}
        </h1>
        {selectedDomain && (
