...
class PassportRenewalAutomation:
    ...
    async def renew_passport(self, renewal_data: PassportRenewalData) -> RenewalResult:
        ...
        try:
            ...
            return RenewalResult(success=False, errors=[f"Automation error: {e}"])
        finally:
            if self.browser:
    ...
    async def _check_portal_availability(self) -> bool:
        ...
        if not (8 <= now.hour < 14):
            logger.warning(
                f"Outside working hours (8 AM - 2 PM). Current time: {now.hour}:00"
            )
            return False
    ...
    async def _select_renewal_service(self) -> None:
        ...
        analysis = await self.llm_provider.analyze_webpage(
            page_content, "Find and click the passport renewal service option"
        )
        ...
        try:
            ...
            if data.photo_path:
                for selector in upload_selectors:
                    try:
                        await self.browser.page.set_input_files(
                            selector, data.photo_path
                        )
                        uploaded_count += 1
            ...
            for selector in appointment_selectors:
                try:
                    ...
                    if available_dates:
                        ...
                        fill_result = await self.form_handler.fill_form(
                            appointment_data
                        )
