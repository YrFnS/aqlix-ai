...
class UniversityApplicationAutomation:
    ...
    async def initialize(self, portal_type: str = "general") -> None:
        ...
        try:
            ...
            logger.info(
                f"University application automation initialized for {portal_type}"
            )
    ...
    async def submit_application(
        self, student_data: StudentData, portal_type: str = "general"
    ) -> ApplicationResult:
        ...
        try:
            ...
            if not await self._check_application_availability():
                result.errors.append(
                    "Application period not active or portal unavailable"
                )
                return result
            ...
            return ApplicationResult(success=False, errors=[f"Automation error: {e}"])
        finally:
            if self.browser:
                await self.browser.close()

    async def check_application_status(
        self, application_number: str, portal_type: str = "general"
    ) -> Dict[str, Any]:
    ...
    async def _check_application_availability(self) -> bool:
        ...
        try:
            ...
            active_keywords = ["متاح", "available", "مفتوح", "open", "تطبيق", "apply"]
    ...
    async def _fill_personal_information(self, data: StudentData) -> bool:
        ...
        try:
            ...
            if not fill_result.success:
                logger.error(
                    f"Personal info filling failed: {fill_result.validation_errors}"
                )
                return False
    ...
    async def _fill_academic_information(self, data: StudentData) -> bool:
        ...
        try:
            ...
            if not fill_result.success:
                logger.error(
                    f"Academic info filling failed: {fill_result.validation_errors}"
                )
                return False
    ...
    async def _select_preferences(self, data: StudentData) -> bool:
        ...
        try:
            ...
            for i, choice in enumerate(
                data.alternative_choices[:3]
            ):  # Usually 3 alternatives max
                try:
    ...
    async def _select_dropdown_option(self, field_type: str, option_text: str) -> None:
        ...
        selectors = dropdown_selectors.get(
            field_type, [f'select[name*="{field_type}"]']
        )
        ...
        try:
            ...
            document_uploads = [
                (
                    "photo",
                    data.photo_path,
                    ["#photo", ".photo-upload", 'input[name*="photo"]'],
                ),
                (
                    "certificate",
                    data.certificate_path,
                    ["#certificate", ".cert-upload", 'input[name*="certificate"]'],
                ),
                (
                    "id_copy",
                    data.id_copy_path,
                    ["#id_copy", ".id-upload", 'input[name*="id"]'],
                ),
            ]
    ...
    async def _extract_status_info(self, page_content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                return {
                    "status": "Unknown",
                    "error": "Could not parse status information",
                }
                            ...
                            departments.append(
                                {
                                    "name": text.strip(),
                                    "value": value,
                                    "name_arabic": text.strip()
                                    if self.arabic_processor._contains_arabic(text)
                                    else "",
                                    "name_english": text.strip()
                                    if not self.arabic_processor._contains_arabic(text)
                                    else "",
                                }
                            )
