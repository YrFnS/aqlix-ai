...
class ArabicLLMProcessor:
    ...
    def process_prompt(self, prompt: str, 
                      context: CulturalContext = CulturalContext.IRAQI_GENERAL) -> str:
        ...
        try:
            ...
            enhanced_prompt = self._enhance_prompt_with_guidelines(prompt, context, is_arabic)
            
            # Add Islamic compliance guidelines if needed
            if context in [CulturalContext.IRAQI_GOVERNMENT, CulturalContext.ISLAMIC_FORMAL]:
    ...
    def validate_response(self, response: str, 
                         context: CulturalContext = CulturalContext.IRAQI_GENERAL) -> CulturalValidation:
        ...
        try:
            ...
            cultural_score, cultural_issues = self._validate_cultural_alignment(response, context)
            ...
            validation.suggestions = self._generate_improvement_suggestions(response, context)
            
            # Calculate overall confidence
            validation.confidence_score = min(
                validation.islamic_compliance,
                validation.cultural_alignment
            )
        ...
        try:
            ...
            if result.language_detected == "ar":
                ...
                if result.cultural_context:
                    result.validation = self.validate_response(text, result.cultural_context)
    ...
    def translate_cultural_terms(self, text: str, 
                               source_lang: str = "en", 
    ...
    def generate_system_prompt(self, task_type: str, 
    ...
    def _load_iraqi_dialect_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            'common_words': [
                'شلونك', 'شنو', 'وين', 'أني', 'إنت', 'إنتي',
                'هاي', 'هذا', 'هذه', 'يالله', 'ماكو', 'إيا'
            ],
            'government_terms': [
                'دائرة', 'مديرية', 'أمانة', 'وزارة', 'هيئة',
                'مصلحة', 'مؤسسة', 'مركز', 'قسم', 'شعبة'
            ],
            'formal_expressions': [
                'حضرة المواطن', 'حضرة المواطنة', 'المحترم',
                'المحترمة', 'سيادتكم', 'حضرتكم'
            ]
        }
    ...
    def _detect_cultural_context(self, text: str) -> Optional[CulturalContext]:
        ...
        islamic_indicators = ['إن شاء الله', 'الحمد لله', 'بإذن الله', 'islamic', 'allah']
    ...
    def _validate_cultural_alignment(self, text: str, 
                                   context: CulturalContext) -> Tuple[float, List[str]]:
        ...
        if context == CulturalContext.IRAQI_GOVERNMENT:
            ...
            if not any(indicator in text for indicator in formal_indicators):
                score -= 0.2
                issues.append("Consider using more formal language for government context")
    ...
    def _generate_improvement_suggestions(self, text: str, 
        ...
        if context == CulturalContext.IRAQI_GOVERNMENT:
            if 'إن شاء الله' not in text:
                suggestions.append("Consider adding 'إن شاء الله' for appropriate religious expression")
    ...
    def _enhance_prompt_with_guidelines(self, prompt: str, 
        ...
        if context == CulturalContext.IRAQI_GOVERNMENT:
            enhancements.append("Use formal language appropriate for government communications.")
