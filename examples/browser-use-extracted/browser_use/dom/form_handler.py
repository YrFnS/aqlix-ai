    ...
    def validate_passport_number(self, passport: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if not re.match(pattern, clean_passport):
                return False, "Iraqi passport format: One letter followed by 7 digits (e.g., A1234567)"
    ...
    def validate_phone_number(self, phone: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if len(clean_phone) != 10:
                return False, f"Iraqi phone number must be 10 digits after country code, got {len(clean_phone)}"
            ...
            if not any(clean_phone.startswith(prefix) for prefix in valid_prefixes):
                return False, f"Invalid Iraqi mobile prefix. Must start with: {', '.join(valid_prefixes)}"
...
class FormHandler:
    ...
    async def analyze_form_fields(self, form_selector: str = "form") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Analyzed form: {len(fields)} fields, {len(field_analysis['required_fields'])} required")
    ...
    async def fill_form(self, form_data: Dict[str, str], 
                       form_selector: str = "form") -> FormFillResult:
        ...
        try:
            ...
            missing_required = [name for name in required_field_names 
                              if name not in result.filled_fields]
            
            if missing_required:
                result.warnings.append(f"Missing required fields: {', '.join(missing_required)}")
            
            result.success = len(result.filled_fields) > 0 and len(missing_required) == 0
            
            logger.info(f"Form filling completed: {len(result.filled_fields)} fields filled, "
    ...
    async def _analyze_field(self, field: ElementHandle, index: int) -> Dict[str, Any]:
        ...
        try:
            ...
            iraqi_field_type = self._detect_iraqi_field_type(label_text, placeholder, name)
            ...
            return {
                ...
                "is_arabic": self.arabic_processor._contains_arabic(label_text + placeholder)
            }
    ...
    def _detect_iraqi_field_type(self, label: str, placeholder: str, name: str) -> Optional[FieldType]:
    ...
    def _validate_field_value(self, field_type: FieldType, value: str) -> Tuple[bool, str]:
