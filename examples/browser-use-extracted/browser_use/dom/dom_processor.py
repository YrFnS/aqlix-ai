...
class DOMProcessor:
    ...
    async def extract_form_data(self, form_selector: str = "form") -> FormInfo:
        ...
        try:
            ...
            if form_info.is_iraqi_form:
                form_info.required_fields = self._detect_iraqi_required_fields(
                    form_info
                )
    ...
    def _is_government_portal(self) -> bool:
        ...
        return any(
            indicator in page_content or indicator in title for indicator in indicators
        )
            ...
            tables.append(
                {
                    ...
                    "row_count": len(rows),
                    "has_arabic": any(
                        self._contains_arabic(header) for header in headers
                    ),
                }
            )
    ...
    def _analyze_page_structure(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "main_content": bool(
                self.soup.find(["main", ".main", ".content", "#content"])
            ),
    ...
    def _is_iraqi_form(self, form_info: FormInfo) -> bool:
        ...
        form_text = form_info.action + " ".join(
            field.text for field in form_info.fields
        )
    ...
    def _detect_iraqi_required_fields(self, form_info: FormInfo) -> List[str]:
        ...
        for field in form_info.fields:
            ...
            if any(
                pattern in field_name or pattern in placeholder
