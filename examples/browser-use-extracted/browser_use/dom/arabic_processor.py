...
class ArabicTextProcessor:
    ...
    def _detect_text_type(self, text: str) -> ArabicTextType:
        ...
        dialect_count = sum(1 for pattern in self.iraqi_patterns['vocabulary'] if pattern in text)
        if dialect_count >= 1:
    ...
    def _calculate_confidence(self, text: str, text_type: ArabicTextType) -> float:
        ...
        if text_type == ArabicTextType.IRAQI_DIALECT:
            dialect_matches = sum(1 for pattern in self.iraqi_patterns['vocabulary'] if pattern in text)
    ...
    def _is_government_content(self, text: str) -> bool:
        ...
        gov_indicators = sum(1 for term in self.government_terms if term in text.lower())
    ...
    def _load_iraqi_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            'greetings': [
                'أهلاً وسهلاً', 'مرحبا', 'السلام عليكم',
                'صباح الخير', 'مساء الخير', 'تسلم'
            ],
            'expressions': [
                'إن شاء الله', 'الحمد لله', 'بإذن الله',
                'ما شاء الله', 'سبحان الله', 'الله يعطيك العافية'
            ]
        }
    ...
    def _load_government_terms(self) -> List[str]:
        ...
        return [
            'وزارة', 'ministry', 'حكومة', 'government', 'جواز', 'passport',
            'هوية', 'identity', 'رسمي', 'official', 'طلب', 'application',
            'استمارة', 'form', 'شهادة', 'certificate', 'تصديق', 'authentication',
            'موافقة', 'approval', 'رخصة', 'license', 'إجازة', 'permit',
            'قانون', 'law', 'نظام', 'regulation', 'تعليمات', 'instructions',
            'دائرة', 'department', 'مديرية', 'directorate', 'أمانة', 'secretariat'
        ]
    
    def _load_common_form_terms(self) -> List[str]:
        """Load common form field terms"""
        return [
            'اسم', 'name', 'رقم', 'number', 'تاريخ', 'date', 'مكان', 'place',
            'عنوان', 'address', 'هاتف', 'phone', 'بريد', 'email', 'جنسية', 'nationality',
            'مهنة', 'occupation', 'توقيع', 'signature', 'صورة', 'photo',
            'إرسال', 'submit', 'إلغاء', 'cancel', 'حفظ', 'save', 'طباعة', 'print',
            'تحميل', 'download', 'رفع', 'upload', 'بحث', 'search', 'تصفية', 'filter'
        ]
...
class RTLLayoutHandler:
    ...
    def adjust_coordinates_for_rtl(self, x: int, y: int, 
                                   element_width: int, container_width: int,
                                   is_rtl: bool = True) -> Tuple[int, int]:
    ...
    def identify_mixed_content_elements(self, elements: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
