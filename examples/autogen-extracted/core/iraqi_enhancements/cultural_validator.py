...
class IraqiCulturalValidator:
    ...
    def __init__(self):
        ...
        self.prohibited_terms = {
            ...
            "mixed_gender_private",
            "unmarried_cohabitation",
            "public_drinking",
            "nightclub_culture",
        }
        ...
        self.domain_contexts = {
            ...
            ProfessionalDomain.MEDICAL: {
                ...
                "prohibited_terms": [
                    "mixed_gender_treatment",
                    "against_islamic_ethics",
                ],
            },
    ...
    def validate_message_content(
        ...
        found_prohibited = [
            term for term in self.prohibited_terms if term in content_lower
        ]
        ...
        if found_prohibited:
            ...
            recommendations.append(
                "Remove or replace prohibited terms with Islamic alternatives"
            )
        ...
        if domain_issues:
            ...
            recommendations.append(
                f"Ensure content meets {domain.value} professional standards"
            )
        ...
        return CulturalValidationResult(
            ...
            requires_human_review=(
                compliance_level
                in [
                    CulturalCompliance.NON_COMPLIANT,
    ...
    def validate_multi_agent_conversation(
        self, messages: List[Dict[str, Any]], conversation_context: Dict[str, Any]
    ) -> CulturalValidationResult:
        """
        ...
        for i, message in enumerate(messages):
            ...
            if result.issues:
                all_issues.extend(
                    [f"Message {i + 1} ({sender}): {issue}" for issue in result.issues]
                )
            ...
            if (
                result.compliance_level == CulturalCompliance.NON_COMPLIANT
    ...
    def _validate_domain_requirements(
        ...
        if required_terms:
            missing_required = [
                term for term in required_terms if term not in content_lower
            ]
            if missing_required and len(content) > 100:  # Only for substantial content
                issues.append(
                    f"Missing {domain.value} required elements: {missing_required}"
                )

        # Check prohibited terms for domain
        prohibited_terms = domain_config.get("prohibited_terms", [])
        found_prohibited = [term for term in prohibited_terms if term in content_lower]
        if found_prohibited:
            issues.append(
                f"Contains {domain.value} prohibited elements: {found_prohibited}"
            )
    ...
    def _check_cultural_sensitivity(self, content: str) -> List[str]:
        ...
        sensitive_patterns = [
            ...
            (
                r"\b(western culture|foreign values)\b",
                "Be respectful when discussing cultural differences",
            ),
        ]
    ...
    def _validate_conversation_patterns(
        ...
        if len(messages) > 1:
            ...
            if "salam" not in first_message and "greeting" not in first_message:
                issues.append(
                    "Professional conversations should begin with appropriate greeting"
                )
        ...
        if mixed_gender_participants:
            ...
            for message in messages:
                ...
                if any(topic in content for topic in personal_topics):
                    issues.append(
                        "Maintain professional boundaries in mixed-gender discussions"
                    )
                    break
...
class IraqiProfessionalHierarchy:
    ...
    def get_coordination_pattern(
        self, participants: List[Dict[str, Any]], task_type: str
    ) -> Dict[str, Any]:
        """
