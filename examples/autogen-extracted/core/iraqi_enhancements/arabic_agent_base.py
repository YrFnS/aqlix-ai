...
from .cultural_validator import IraqiCulturalValidator, ProfessionalDomain, CulturalValidationResult
...
class ArabicAgentBase(BaseAgent):
    ...
    def __init__(
        ...
        self._arabic_patterns = {
            'greeting_patterns': [
                ...
                'أهلاً وسهلاً', 'مرحباً', 'شلونك'  # Iraqi greeting
            ],
            'respect_patterns': [
                'أستاذ', 'دكتور', 'مهندس', 'الأستاذ المحترم',
                'سيادة الدكتور', 'المهندس الفاضل'
            ],
            'closing_patterns': [
                'بارك الله فيك', 'جزاك الله خيراً', 'والله يعطيك العافية',
                'مع التقدير', 'وفقك الله'
            ],
            'iraqi_expressions': [
                'ان شاء الله', 'ماشاء الله', 'الله يعطيك العافية',
                'يسلمو', 'الله وياك', 'عاشت ايدك'  # Iraqi expressions
            ]
        }
        ...
        self._preserve_english = {
            'technical_terms': [
                ...
                'algorithm', 'software', 'hardware', 'network', 'protocol'
            ],
            'professional_terms': [
                'management', 'strategy', 'analysis', 'implementation',
                'evaluation', 'optimization', 'coordination'
            ]
        }
    ...
    def _detect_language(self, content: str) -> Dict[str, Any]:
        ...
        iraqi_markers = [
            'شلونك', 'اكو', 'ماكو', 'يلا', 'حبيبي',
            'والله', 'انشالله', 'ماشالله'
        ]
    ...
    async def _handle_validation_issues(self, validation_result: CulturalValidationResult):
        """
    ...
    def _process_mixed_content(self, content: str) -> str:
        ...
        if self._arabic_config.preserve_english_terms:
            ...
            for word in words:
                ...
                if (word_lower in self._preserve_english['technical_terms'] or
                    word_lower in self._preserve_english['professional_terms']):
    ...
    def _ensure_appropriate_greeting(self, content: str, language_info: Dict[str, Any]) -> str:
        """
        ...
        if not has_greeting and len(content) > 50:  # Only for substantial messages
            if language_info['primary'] == 'arabic':
                ...
                return f"{greeting}\n\n{content}"
            elif language_info['primary'] == 'english' and self._arabic_config.cultural_greetings:
    ...
    async def _post_process_response(
        ...
        if isinstance(response, str):
            ...
            if language_info['arabic_ratio'] > 0.3:
                processed_response = self._add_cultural_closing(processed_response, language_info)
            
            # Format for RTL if needed
            if (language_info['direction'] == 'rtl' and 
                self._arabic_config.rtl_support):
    ...
    def _add_cultural_closing(self, content: str, language_info: Dict[str, Any]) -> str:
        ...
        has_closing = any(
            closing in content 
            for closing in self._arabic_patterns['closing_patterns']
        )
    ...
    def get_language_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            'arabic_support': True,
            'iraqi_dialect': self._arabic_config.dialect_support == DialectSupport.IRAQI_DIALECT,
...
class IraqiProfessionalAgent(ArabicAgentBase):
    ...
    def __init__(
        ...
        self._professional_patterns = {
            ...
            'formal_language': [
                'نتشرف', 'يسعدنا', 'نقدر تعاونكم',
                'مع فائق الاحترام', 'بكل التقدير'
            ]
        }
