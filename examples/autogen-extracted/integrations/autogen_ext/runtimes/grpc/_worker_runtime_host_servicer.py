...
async def get_client_id_or_abort(context: grpc.aio.ServicerContext[Any, Any]) -> str:  # type: ignore
    ...
    if (client_id := metadata.get("client-id")) is None:
        await context.abort(
            grpc.StatusCode.INVALID_ARGUMENT, "client-id metadata not found."
        )
...
class ChannelConnection(ABC, Generic[SendT, ReceiveT]):
    def __init__(
        self, request_iterator: AsyncIterator[ReceiveT], client_id: str
    ) -> None:
        self._request_iterator = request_iterator
        self._client_id = client_id
        self._send_queue: asyncio.Queue[SendT] = asyncio.Queue()
        self._receiving_task = asyncio.create_task(
            self._receive_messages(client_id, request_iterator)
        )

    async def _receive_messages(
        self, client_id: ClientConnectionId, request_iterator: AsyncIterator[ReceiveT]
    ) -> None:
...
class GrpcWorkerAgentRuntimeHostServicer(agent_worker_pb2_grpc.AgentRpcServicer):
    ...
    def __init__(self) -> None:
        self._data_connections: Dict[
            ClientConnectionId,
            ChannelConnection[agent_worker_pb2.Message, agent_worker_pb2.Message],
        ] = {}
        self._control_connections: Dict[
            ClientConnectionId,
            ChannelConnection[
                agent_worker_pb2.ControlMessage, agent_worker_pb2.ControlMessage
            ],
        ] = {}
        ...
        self._client_id_to_subscription_id_mapping: Dict[
            ClientConnectionId, set[str]
        ] = {}
    ...
    async def OpenChannel(  # type: ignore
        ...
        context: grpc.aio.ServicerContext[
            agent_worker_pb2.Message, agent_worker_pb2.Message
        ],
        ...
        connection = CallbackChannelConnection[
            agent_worker_pb2.Message, agent_worker_pb2.Message
    ...
    async def _on_client_disconnect(self, client_id: ClientConnectionId) -> None:
        async with self._agent_type_to_client_id_lock:
            agent_types = [
                agent_type
                for agent_type, id_ in self._agent_type_to_client_id.items()
                if id_ == client_id
            ]
            for agent_type in agent_types:
                logger.info(
                    f"Removing agent type {agent_type} from agent type to client id mapping"
                )
                del self._agent_type_to_client_id[agent_type]
            for sub_id in self._client_id_to_subscription_id_mapping.get(
                client_id, set()
            ):
                logger.info(
                    f"Client id {client_id} disconnected. Removing corresponding subscription with id {id}"
                )
                try:
    ...
    async def _receive_message(
        self, client_id: ClientConnectionId, message: agent_worker_pb2.Message
    ) -> None:
        logger.info(f"Received message from client {client_id}: {message}")
    ...
    async def _receive_control_message(
        ...
        if destination.startswith("agentid="):
            ...
            if target_client_id is None:
                logger.error(
                    f"Agent client id not found for agent type {agent_id.type}."
                )
                return
        ...
        if target_send_queue is None:
            logger.error(
                f"Client {target_client_id} not found, failed to deliver message."
            )
            return
        await target_send_queue.send(message)

    async def _process_request(
        self, request: agent_worker_pb2.RpcRequest, client_id: ClientConnectionId
    ) -> None:
        ...
        if target_client_id is None:
            logger.error(
                f"Agent {request.target.type} not found, failed to deliver message."
            )
            return
        ...
        self._pending_responses.setdefault(target_client_id, {})[request.request_id] = (
            future
        )

        # Create a task to wait for the response and send it back to the client.
        send_response_task = asyncio.create_task(
            self._wait_and_send_response(future, client_id)
        )
    ...
    async def _wait_and_send_response(
        self,
        future: Future[agent_worker_pb2.RpcResponse],
        ...
        if send_queue is None:
            logger.error(
                f"Client {client_id} not found, failed to send response message."
            )
            return
        await send_queue.send(message)

    async def _process_response(
        self, response: agent_worker_pb2.RpcResponse, client_id: ClientConnectionId
    ) -> None:
        ...
        async with self._agent_type_to_client_id_lock:
            ...
            for recipient in recipients:
                ...
                if client_id is not None:
                    ...
                    logger.error(
                        f"Agent {recipient.type} and its client not found for topic {topic_id}."
                    )
        # Deliver the event to clients.
        for client_id in client_ids:
            await self._data_connections[client_id].send(
                agent_worker_pb2.Message(cloudEvent=event)
            )
    ...
    async def AddSubscription(  # type: ignore
        ...
        try:
            ...
            subscription_ids = self._client_id_to_subscription_id_mapping.setdefault(
                client_id, set()
            )
