...
class HttpTool(BaseTool[BaseModel, Any], Component[HttpToolConfig]):
    ...
    async def run(self, args: BaseModel, cancellation_token: CancellationToken) -> Any:
            ...
            match self.server_params.method:
                case "GET":
                    response = await client.get(
                        url, headers=self.server_params.headers, params=model_dump
                    )
                case "PUT":
                    response = await client.put(
                        url, headers=self.server_params.headers, json=model_dump
                    )
                case "DELETE":
        ...
        match self.server_params.return_type:
            ...
            case _:
                raise ValueError(
                    f"Invalid return type: {self.server_params.return_type}"
                )
