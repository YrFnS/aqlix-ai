...
sys.path.append(os.path.join(os.path.dirname(__file__), '..', '..', 'agentchat', 'autogen_agentchat'))
...
class IraqiMinistryAgent(IraqiProfessionalAgent):
    ...
    def __init__(
        ...
        self.government_knowledge = {
            ...
            'parliamentary_procedures': position_level in ['minister', 'deputy_minister'],
    ...
    def _get_ministry_authorities(self) -> Dict[str, Any]:
        ...
        authorities = {
            ...
            'policy_making': self.position_level in ['minister', 'deputy_minister', 'director_general'],
            'operational_decisions': True,
            'inter_ministry_agreements': self.position_level in ['minister', 'deputy_minister'],
        ...
        if self.ministry == IraqiMinistry.FINANCE:
            ministry_specific.update({
                ...
                'taxation_policy': self.position_level in ['minister', 'deputy_minister'],
                'international_loans': self.position_level == 'minister'
            })
        elif self.ministry == IraqiMinistry.HEALTH:
            ministry_specific.update({
                'healthcare_policy': self.position_level in ['minister', 'deputy_minister'],
            ...
            ministry_specific.update({
                'curriculum_approval': self.position_level in ['minister', 'deputy_minister'],
                'school_oversight': True,
                'teacher_standards': True,
                'educational_policy': self.position_level in ['minister', 'deputy_minister']
            })
    ...
    async def _generate_government_response(
        ...
        ministry_position = await self._provide_ministry_position(content, project_context)
    ...
    async def _provide_ministry_position(
        ...
        if is_primary_ministry:
            position_parts.append(f"وزارة {self.ministry.value} تتولى الدور الرئيسي في هذا المشروع")
        elif is_supporting_ministry:
            position_parts.append(f"وزارة {self.ministry.value} تقدم الدعم المطلوب ضمن اختصاصها")
        else:
            position_parts.append(f"وزارة {self.ministry.value} تقدم المشورة الفنية المطلوبة")
    ...
    def _is_primary_ministry(self, project_context: GovernmentProjectContext) -> bool:
        ...
        primary_ministry_map = {
            ...
            GovernmentProjectType.EDUCATIONAL_REFORM: [IraqiMinistry.EDUCATION, IraqiMinistry.HIGHER_EDUCATION],
            GovernmentProjectType.INFRASTRUCTURE: [IraqiMinistry.PLANNING, IraqiMinistry.TRANSPORTATION],
            GovernmentProjectType.ECONOMIC_DEVELOPMENT: [IraqiMinistry.FINANCE, IraqiMinistry.PLANNING],
            GovernmentProjectType.SECURITY_COOPERATION: [IraqiMinistry.INTERIOR, IraqiMinistry.DEFENSE],
            GovernmentProjectType.DIGITAL_TRANSFORMATION: [IraqiMinistry.PLANNING],
            GovernmentProjectType.ENVIRONMENTAL_PROTECTION: [IraqiMinistry.HEALTH, IraqiMinistry.AGRICULTURE]
        }
        
        primary_ministries = primary_ministry_map.get(project_context.project_type, [])
        return self.ministry in primary_ministries
    
    def _is_supporting_ministry(self, project_context: GovernmentProjectContext) -> bool:
        """Check if this ministry has supporting role"""
        
        return self.ministry in project_context.involved_ministries and not self._is_primary_ministry(project_context)
    
    def _identify_expertise_area(self, project_type: GovernmentProjectType) -> Optional[str]:
    ...
    def _assess_resource_availability(self, project_context: GovernmentProjectContext) -> str:
    ...
    def _assess_authority_scope(self, project_context: GovernmentProjectContext) -> str:
        ...
        if self.ministry_authorities.get('budget_authority') and project_context.budget_range in ['large', 'very_large']:
        ...
        if (self.ministry_authorities.get('inter_ministry_agreements') and 
            len(project_context.involved_ministries) > 1):
            authority_parts.append("- يمكننا التوقيع على الاتفاقيات بين الوزارات")
        
        return "\n".join(authority_parts)
    
    def _identify_coordination_needs(self, project_context: GovernmentProjectContext) -> str:
        ...
        required_ministries = self._get_required_ministries(project_context.project_type)
        missing_ministries = set(required_ministries) - set(project_context.involved_ministries)
        
        if missing_ministries:
            missing_names = [ministry.value for ministry in missing_ministries]
            coordination_parts.append(f"- يجب إشراك الوزارات التالية: {', '.join(missing_names)}")
    ...
    def _get_required_ministries(self, project_type: GovernmentProjectType) -> List[IraqiMinistry]:
        """Get required ministries for project type"""
        
        required_ministries_map = {
            GovernmentProjectType.HEALTHCARE_IMPROVEMENT: [
                IraqiMinistry.HEALTH, IraqiMinistry.FINANCE, IraqiMinistry.PLANNING
            ],
            GovernmentProjectType.EDUCATIONAL_REFORM: [
                IraqiMinistry.EDUCATION, IraqiMinistry.HIGHER_EDUCATION, 
                IraqiMinistry.FINANCE, IraqiMinistry.PLANNING
            ],
            GovernmentProjectType.INFRASTRUCTURE: [
                IraqiMinistry.PLANNING, IraqiMinistry.FINANCE, IraqiMinistry.TRANSPORTATION,
                IraqiMinistry.ELECTRICITY, IraqiMinistry.OIL
            ],
            GovernmentProjectType.SECURITY_COOPERATION: [
                IraqiMinistry.INTERIOR, IraqiMinistry.DEFENSE, IraqiMinistry.JUSTICE
            ],
            GovernmentProjectType.ECONOMIC_DEVELOPMENT: [
                IraqiMinistry.FINANCE, IraqiMinistry.PLANNING, IraqiMinistry.TRADE,
                IraqiMinistry.OIL, IraqiMinistry.AGRICULTURE
            ]
        }
        
        return required_ministries_map.get(project_type, [])
    
    def _analyze_budget_implications(self, project_context: GovernmentProjectContext) -> str:
    ...
    def _check_regulatory_compliance(self, project_context: GovernmentProjectContext) -> str:
    ...
    def _provide_government_recommendations(self, project_context: GovernmentProjectContext) -> str:
...
class IraqiGovernmentCoordination:
    ...
    async def setup_government_coordination(
        ...
        if missing_ministries:
            raise ValueError(f"Missing ministry representatives: {[m.value for m in missing_ministries]}")
        ...
        for ministry, agent in self.ministry_agents.items():
            if ministry in project_context.involved_ministries:
                ...
                seniority = self._calculate_government_seniority(agent.position_level, ministry)
    ...
    def _get_required_ministries_for_project(
        ...
        if (project_context.project_type in [
            GovernmentProjectType.INFRASTRUCTURE,
            GovernmentProjectType.ECONOMIC_DEVELOPMENT,
    ...
    def _calculate_government_seniority(self, position_level: str, ministry: IraqiMinistry) -> int:
        ...
        strategic_ministries = [
            IraqiMinistry.FINANCE, IraqiMinistry.PLANNING, 
            IraqiMinistry.DEFENSE, IraqiMinistry.INTERIOR,
            IraqiMinistry.OIL
        ]
    ...
    def _determine_decision_pattern(
        ...
        elif project_context.requires_parliament_approval or project_context.requires_pm_approval:
            return IraqiDecisionPattern.HIERARCHICAL
    ...
    async def coordinate_government_project(
        ...
        self.coordination_history.append({
            ...
            'involved_ministries': [m.value for m in project_context.involved_ministries],
            'result_status': final_result['status'],
            'cultural_compliance': final_result.get('cultural_compliance', {}).get('total_validations', 0)
        })
    ...
    def _format_government_request(
        ...
        formatted_parts.append(f"- مستوى الأولوية: {project_context.urgency_level.value}")
        formatted_parts.append(f"- المدة الزمنية: {project_context.timeline_months} شهر")
        formatted_parts.append(f"- النطاق المالي: {project_context.budget_range}")
        
        # Involved ministries
        ministry_names = [f"وزارة {m.value}" for m in project_context.involved_ministries]
    ...
    def _process_government_results(
        ...
        for message in messages:
            ...
            if ministry_name:
                ministry_positions[ministry_name] = {
                    ...
                    'position_summary': self._extract_position_summary(message.get('response', ''))
                }
        ...
        government_analysis = {
            ...
            'timeline_feasibility': self._assess_timeline_feasibility(messages, project_context),
        ...
        return {
            ...
            'government_coordination': {
                ...
                'next_steps': self._determine_next_steps(government_analysis, project_context)
            },
            'cultural_compliance': coordination_result.get('cultural_compliance', {}),
            'islamic_compliance_verified': coordination_result.get('islamic_compliance_verified', True),
    ...
    def _extract_position_summary(self, response: str) -> str:
        ...
        summary_lines = [line for line in lines if any(keyword in line for keyword in ['توصي', 'نقترح', 'نرى', 'الموقف'])]
    ...
    def _extract_budget_implications(self, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for message in messages:
            ...
            # Budget concerns
            if any(keyword in response for keyword in ['ميزانية', 'تمويل', 'مالية', 'تكلفة']):
                budget_implications['total_concerns'] += 1
                
                if any(keyword in response for keyword in ['صعوبة', 'تحدي', 'نقص']):
                    budget_implications['major_issues'].append(message.get('agent', 'unknown'))
    ...
    def _check_government_compliance(self, messages: List[Dict[str, Any]]) -> Dict[str, bool]:
        ...
        for message in messages:
            ...
            if any(keyword in response for keyword in ['مخالفة', 'غير قانوني', 'غير شرعي']):
                compliance_checks['regulatory'] = False
            
            if 'شريعة' in response and any(keyword in response for keyword in ['مخالف', 'غير متوافق']):
                compliance_checks['islamic'] = False
        
        return compliance_checks
    
    def _identify_dependencies(self, messages: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        ...
        for message in messages:
            ...
            if any(keyword in response for keyword in ['يتطلب', 'نحتاج', 'التنسيق مع', 'بالتعاون مع']):
                dependencies.append({
                    'from_ministry': agent,
    ...
    def _generate_government_recommendation(
        ...
        if not budget_issues:
            ...
            recommendation_parts.append(f"💰 مخاوف مالية من: {', '.join(budget_issues)}")
        ...
        # Final decision
        if (consensus in ['high_consensus', 'moderate_consensus'] and 
            timeline in ['feasible', 'moderate'] and
            len(budget_issues) <= 1):
            
            recommendation_parts.append("\n✅ التوصية: الموافقة على المشروع مع التنسيق المطلوب")
            
            if project_context.requires_pm_approval:
                recommendation_parts.append("📋 المرحلة التالية: رفع التوصية لرئيس الوزراء")
            elif project_context.requires_parliament_approval:
                recommendation_parts.append("📋 المرحلة التالية: رفع مشروع القانون للبرلمان")
            else:
    ...
    def get_coordination_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'recent_activity': self.coordination_history[-5:] if self.coordination_history else [],
            'active_projects': {
                project_id: {
...
async def main():
    ...
    project_context = GovernmentProjectContext(
        ...
        involved_ministries=[IraqiMinistry.HEALTH, IraqiMinistry.FINANCE, IraqiMinistry.PLANNING],
        budget_range="large",
        timeline_months=18,
