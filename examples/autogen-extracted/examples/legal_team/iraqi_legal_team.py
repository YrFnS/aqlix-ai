...
class IraqiLawyerAgent(IraqiProfessionalAgent):
    ...
    async def _provide_legal_analysis(
        self, content: str, case_context: Optional[LegalCaseContext]
    ) -> str:
        ...
        if self.role == LegalRole.SENIOR_PARTNER:
            analysis_parts.append(
                "بصفتي الشريك الأول في المكتب، أقدم التحليل القانوني التالي:"
            )
        elif self.role == LegalRole.ASSOCIATE_LAWYER:
            analysis_parts.append(
                f"بصفتي محامي متخصص في {self.specialization}، أرى أن:"
            )
        else:
        ...
        if case_context:
            if case_context.case_type == CaseType.FAMILY_LAW:
                analysis_parts.append(
                    "في قضايا الأحوال الشخصية، يجب مراعاة أحكام الشريعة الإسلامية"
                )
            elif case_context.case_type == CaseType.COMMERCIAL_LAW:
                analysis_parts.append(
                    "في القانون التجاري، نلتزم بأحكام الشريعة في المعاملات المالية"
                )
            elif case_context.case_type == CaseType.CIVIL_LAW:
                analysis_parts.append(
                    "وفقاً للقانون المدني العراقي ومبادئ الشريعة الإسلامية"
                )
    ...
    def _check_islamic_compliance(
        self, content: str, case_context: LegalCaseContext
    ) -> str:
    ...
    def _provide_legal_recommendations(
        self, content: str, case_context: Optional[LegalCaseContext]
    ) -> str:
...
class IraqiLegalTeam:
    ...
    def add_team_member(
        self, agent_id: str, role: LegalRole, specialization: str, experience_years: int
    ) -> IraqiLawyerAgent:
    ...
    async def setup_team_coordination(self) -> None:
        ...
        for agent_id, agent in self.team_members.items():
            await agent.register_instance(
                runtime=self.runtime, agent_id=AgentId(agent_id, "iraqi_lawyer")
            )
    ...
    async def handle_legal_consultation(
        self, case_context: LegalCaseContext, client_query: str
    ) -> Dict[str, Any]:
        """
        ...
        coordination_pattern = self.hierarchy_manager.get_coordination_pattern(
            participants=participants, task_type=case_context.case_type.value
        )
    ...
    async def _execute_consultation(
        ...
        if (
            senior_partners
            and coordination_pattern["decision_type"] == "senior_approval"
        ):
        ...
        for agent_id, agent in self.team_members.items():
            if (
                agent.role != LegalRole.SENIOR_PARTNER
                or coordination_pattern["decision_type"] != "senior_approval"
            ):
    ...
    def _synthesize_recommendations(
        ...
        if coordination_pattern["decision_type"] == "senior_approval":
            senior_input = [
                msg for msg in messages if msg["role"] == LegalRole.SENIOR_PARTNER.value
            ]
            if senior_input:
        ...
        synthesis_parts.append(
            "نوصي بالإجراءات التالية مع مراعاة الشريعة الإسلامية والقانون العراقي"
        )
...
async def main():
    ...
    legal_team.add_team_member(
        "ahmed_partner", LegalRole.SENIOR_PARTNER, "civil_law", 20
    )

    legal_team.add_team_member(
        "fatima_associate", LegalRole.ASSOCIATE_LAWYER, "commercial_law", 8
    )

    legal_team.add_team_member(
        "hassan_researcher", LegalRole.LEGAL_RESEARCHER, "islamic_jurisprudence", 5
    )
    ...
    result = await legal_team.handle_legal_consultation(
        case_context=case_context, client_query=client_query
    )
