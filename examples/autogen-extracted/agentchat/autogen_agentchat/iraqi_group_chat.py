...
class IraqiGroupChatManager(BaseGroupChatManager):
    ...
    def _analyze_hierarchy(self) -> Dict[str, Any]:
        ...
        for participant in self.iraqi_participants.values():
            ...
            seniority_range["min"] = min(
                seniority_range["min"], participant.seniority_level
            )
            seniority_range["max"] = max(
                seniority_range["max"], participant.seniority_level
            )
    ...
    def _identify_decision_makers(self) -> List[str]:
        ...
        for agent_id, participant in self.iraqi_participants.items():
            if participant.role in [
                IraqiGroupRole.SENIOR_EXECUTIVE,
                IraqiGroupRole.DEPARTMENT_HEAD,
            ]:
    ...
    def _identify_religious_advisors(self) -> List[str]:
        ...
        for agent_id, participant in self.iraqi_participants.items():
            if (
                participant.role == IraqiGroupRole.RELIGIOUS_ADVISOR
                or participant.religious_role
                or "islamic" in participant.specialization.lower()
                or "shariah" in participant.specialization.lower()
            ):
    ...
    def _build_language_matrix(self) -> Dict[str, Any]:
        ...
        language_matrix["translation_needed"] = (
            language_matrix["arabic_speakers"] > 0
            and language_matrix["english_speakers"] > 0
        )

        return language_matrix

    async def select_speaker(
        self, messages: List[ChatMessage], ctx: MessageContext
    ) -> AgentId:
        """
    ...
    async def _select_hierarchical_speaker(
        ...
        if len(messages) > 0:
            ...
            relevant_heads = [
                ...
                for agent_id, participant in self.iraqi_participants.items()
                if (
                    participant.role == IraqiGroupRole.DEPARTMENT_HEAD
                    and self._is_relevant_to_domain(
                        last_message.content, participant.specialization
                    )
                )
            ]
        ...
        recent_speakers = (
            [msg.source for msg in messages[-3:]] if len(messages) >= 3 else []
        )
    ...
    async def _select_consensus_speaker(
        ...
        if not messages:
            ...
            senior_member = max(
                self.iraqi_participants.items(), key=lambda x: x[1].seniority_level
            )
        ...
        candidates = [
            agent_id
            for agent_id, count in participant_speak_count.items()
            if count == min_speaks
        ]
    ...
    async def _select_expert_speaker(
        ...
        for agent_id, participant in self.iraqi_participants.items():
            score = self._calculate_expertise_relevance(
                participant.specialization, topic_keywords
            )
    ...
    async def _select_religious_speaker(
        ...
        religious_advisors = self.cultural_context["hierarchy_levels"][
            "religious_advisors"
        ]
        ...
        islamic_experts = [
            ...
            for agent_id, participant in self.iraqi_participants.items()
            if (
                "islamic" in participant.specialization.lower()
                or "shariah" in participant.specialization.lower()
                or participant.religious_role
            )
        ]
    ...
    async def _select_collaborative_speaker(
        ...
        for agent_id, participation_ratio in participation_balance.items():
            ...
            candidates.append((agent_id, (1.0 - participation_ratio) * cultural_weight))
    ...
    def _calculate_participation_balance(
        ...
        for agent_id in self.iraqi_participants.keys():
            ...
            participation_count[agent_id] = (
                count / total_messages if total_messages > 0 else 0.0
            )
    ...
    async def _handle_cultural_violation(
        self, message: ChatMessage, validation_result: CulturalValidationResult
    ) -> None:
        ...
        violation_record = {
            ...
            "issues": validation_result.issues,
            "severity": "high"
            if "islamic" in str(validation_result.issues).lower()
            else "medium",
        }
    ...
    def _calculate_expertise_relevance(
        self, specialization: str, keywords: List[str]
    ) -> float:
        ...
        for keyword in keywords:
            for spec_term in spec_terms:
                if keyword in spec_term or spec_term in keyword:
                    ...
                    abs(len(keyword) - len(spec_term)) <= 2
                ):  # Similar length heuristic
                    relevance_score += 0.3
...
class IraqiGroupChat(BaseGroupChat):
    ...
    async def run_iraqi_consultation(
        self, initial_message: str, cultural_context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def get_participant_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "seniority_distribution": {
                "senior": sum(
                    1 for p in self.iraqi_participants if p.seniority_level >= 8
                ),
                "mid": sum(
                    1 for p in self.iraqi_participants if 5 <= p.seniority_level < 8
                ),
                "junior": sum(
                    1 for p in self.iraqi_participants if p.seniority_level < 5
                ),
            },
            "specializations": list(
                set(p.specialization for p in self.iraqi_participants)
            ),
            "language_preferences": {
                "arabic": sum(
                    1
                    for p in self.iraqi_participants
                    if p.language_preference == "arabic"
                ),
                "english": sum(
                    1
                    for p in self.iraqi_participants
                    if p.language_preference == "english"
                ),
                "mixed": sum(
                    1
                    for p in self.iraqi_participants
                    if p.language_preference == "mixed"
                ),
            },
            "cultural_context": {
                "domain": self.config.professional_domain.value,
