...
class IraqiMessageValidator:
    ...
    async def validate_message(
        ...
        if isinstance(message, ChatMessage):
            ...
            message_id = str(message.source) if hasattr(message, 'source') else 'unknown'
        else:
            content = message
        ...
        issues.extend(await self._check_cultural_appropriateness(content, domain, context))
        issues.extend(await self._check_professional_etiquette(content, domain, context))
    ...
    async def _check_islamic_compliance(
        ...
        if context and context.get('formal_context', False):
            ...
            has_blessed_beginning = any(
                beginning in content for beginning in self.islamic_terms['blessed_beginnings']
            )
    ...
    async def _check_cultural_appropriateness(
        ...
        for issue_desc in validation_result.issues:
            ...
            if validation_result.compliance_level == CulturalCompliance.NON_COMPLIANT:
                severity = MessageSeverity.ERROR
            elif validation_result.compliance_level == CulturalCompliance.REQUIRES_REVIEW:
                severity = MessageSeverity.CRITICAL
            ...
            issues.append(ValidationIssue(
                ...
                context={'cultural_compliance': validation_result.compliance_level.value}
            ))
    ...
    async def _check_professional_etiquette(
        ...
        if (len(content) > 50 and 
            any(word in content.lower() for word in ['أول', 'بداية', 'استهلال', 'مرحبا']) and
            not has_appropriate_greeting):
    ...
    async def _check_language_quality(
        ...
        if arabic_matches > 0 and english_matches > 0:
            switch_ratio = min(arabic_matches, english_matches) / max(arabic_matches, english_matches)
            ...
            if switch_ratio > 0.3:  # More than 30% code-switching
                issues.append(ValidationIssue(
                    ...
                    confidence_score=0.7,
                    context={'arabic_ratio': arabic_matches / (arabic_matches + english_matches)}
                ))
    ...
    async def _check_hierarchical_respect(
        ...
        if context and context.get('recipient_seniority', 0) > context.get('sender_seniority', 5):
            has_respectful_title = any(
                title in content for title in self.hierarchy_markers['respectful_titles']
            )
    ...
    async def _check_gender_sensitivity(
        ...
        if any(term in content.lower() for term in ['جميع', 'كل', 'الموظفين']):
            has_inclusive = any(
                inclusive in content for inclusive in self.gender_sensitive_terms['inclusive_terms']
            )
    ...
    async def _check_religious_sensitivity(
        ...
        if any(keyword in content.lower() for keyword in religious_keywords):
            ...
            has_religious_opening = any(
                opening in content for opening in self.islamic_terms['blessed_beginnings']
            )
    ...
    def _determine_compliance_level(
        ...
        critical_issues = [issue for issue in issues if issue.severity == MessageSeverity.CRITICAL]
        if critical_issues:
            return CulturalCompliance.NON_COMPLIANT
        
        # Check for error-level issues
        error_issues = [issue for issue in issues if issue.severity == MessageSeverity.ERROR]
        if error_issues:
    ...
    def _generate_recommendations(
        ...
        if ValidationRule.GENDER_SENSITIVITY in issues_by_type:
            recommendations.append(
                "Use inclusive language that respects all genders"
            )
    ...
    def _is_auto_correctable(self, issues: List[ValidationIssue]) -> bool:
        ...
        for issue in issues:
            if (issue.rule_type not in auto_correctable_types or 
                issue.severity == MessageSeverity.CRITICAL):
                return False
        
        return True
    
    def _requires_human_review(self, issues: List[ValidationIssue], score: float) -> bool:
        ...
        sensitive_issues = [issue for issue in issues if issue.rule_type in sensitive_types]
        if sensitive_issues:
            return True
    ...
    async def auto_correct_message(
        ...
        for issue in language_issues:
            if 'Error:' in issue.location:
                ...
                correction = issue.suggested_fix.split("'")[1] if "'" in issue.suggested_fix else ""
                
                if mistake in corrected_content and correction:
                    corrected_content = corrected_content.replace(mistake, correction)
                    applied_corrections.append(f"Corrected '{mistake}' to '{correction}'")
        ...
        for issue in etiquette_issues:
            if "lacks appropriate professional greeting" in issue.description:
                if issue.suggested_fix and "Consider starting with:" in issue.suggested_fix:
                    greeting = issue.suggested_fix.split("Consider starting with: ")[1]
                    corrected_content = f"{greeting}\n\n{corrected_content}"
                    applied_corrections.append(f"Added professional greeting: {greeting}")
