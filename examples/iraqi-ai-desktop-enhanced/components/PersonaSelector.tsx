...
const PersonaSelector: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleValidationFailed = (
      event: CustomEvent<{ score: number; message: string }>,
    ) => {
      setValidationError(
        `فشل التحقق الثقافي (درجة: ${event.detail.score}%). يرجى إعادة المحاولة.`,
      );
  ...
  const applyPersona = async (personaId: string) => {
    try {
      ...
      const evalResult = await iraqiDesktop.personas.evaluateTraits(
        personaId,
        traits,
      );
      ...
      await iraqiDesktop.personas.applyMemory(personaId, {
        /* app context */
      });
      setSelectedPersona(personaId);

      // Save selection (caches offline if needed)
      await iraqiDesktop.personas.createPersona({
        ...persona,
        lastUsed: new Date(),
      } as any); // Update with usage
      console.log(
        `Applied persona: ${persona.name} - Cultural validation passed`,
      );
    } catch (error) {
  ...
  return (
    <div className="persona-selector iraqi-container" lang="ar" dir="rtl">
          ...
          >
            <div className="persona-item">
              <span className="arabic-body">
                {persona.name} ({persona.englishName})
              </span>
      ...
      <section className="theme-section">
        <label htmlFor="theme-select" className="arabic-body sr-only">
          اختر سمة المنظمة المهنية
        </label>
        <select
          id="theme-select"
      ...
      <section className="security-section">
        <label htmlFor="security-select" className="arabic-body sr-only">
          اختر مستوى التصريح الأمني
        </label>
        <select
          id="security-select"
          className="iraqi-select"
      ...
      {selectedPersona && (
        <div
          className="selection-feedback loading"
          role="status"
          aria-live="polite"
        >
          <span className="sr-only">
            تم اختيار الشخصية بنجاح. جاري التحقق الثقافي...
          </span>
          <span className="arabic-body">
            الشخصية المختارة:{" "}
