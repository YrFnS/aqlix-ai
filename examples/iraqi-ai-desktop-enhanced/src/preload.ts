...
export interface IraqiDesktopAPI {
  ...
  notifications: {
    showGovernmentNotification(title: string, body: string, urgency?: 'normal' | 'high'): Promise<void>;
    schedulePrayerTimeNotification(title: string, body: string, prayerTime: Date): Promise<void>;
  };
}
...
class SecureIPC {
  private static validateChannel(channel: string): boolean {
    const allowedChannels = [
      ...
      'create-persona', 'load-persona', 'load-all-personas', 'evaluate-traits', 'apply-memory', 'generate-domain-response',
      // New notifications
      'show-government-notification', 'schedule-prayer-time-notification'
    ];
  ...
  static on(channel: string, listener: (...args: unknown[]) => void): () => void {
    const wrappedListener = (event: Electron.IpcRendererEvent, ...args: unknown[]) => {
      if (event.senderFrame && !isValidOrigin(event.senderFrame.url)) {
        console.warn('Blocked IPC message from invalid origin:', event.senderFrame.url);
        return;
      }
...
const iraqiDesktopAPI: IraqiDesktopAPI = {
  ...
  // New: Notifications
  notifications: {
    async showGovernmentNotification(title: string, body: string, urgency: 'normal' | 'high' = 'normal'): Promise<void> {
      return SecureIPC.invoke('show-government-notification', title, body, urgency) as Promise<void>;
    },

    async schedulePrayerTimeNotification(title: string, body: string, prayerTime: Date): Promise<void> {
      return SecureIPC.invoke('schedule-prayer-time-notification', title, body, prayerTime) as Promise<void>;
    }
  }
};
...
SecureIPC.on('cultural-validation-failed', (data: { score: number; message: string }) => {
