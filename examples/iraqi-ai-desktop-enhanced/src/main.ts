...
} from "./services/offline-manager"; // Updated import
...
class IraqiAIDesktopApp {
  ...
  private registerPersonaIPC(): void {
    const handleValidationFailure = (
      ...
      if (result.score < 85) {
        this.auditLogger.logSecurityEvent("cultural-validation-failed", {
          score: result.score,
          personaId: personaData.id,
        });
        mainWindow?.webContents.send("cultural-validation-failed", {
          score: result.score,
          message: "Resend required for cultural compliance",
        });
        ...
        throw new Error(
          `Cultural validation failed (score: ${result.score}%). Resend required.`,
        );
      }
      return result;
    };
    ...
    ipcMain.handle(
      "create-persona",
      async (event, personaData: Omit<Persona, "id">): Promise<Persona> => {
        try {
          const persona = await this.personaService.createPersona(personaData);
          const traits = await this.personaService.evaluateTraits(
            persona.id,
            personaData.traits || [],
          );
          const validation = await this.personaService.evaluateTraits(
            persona.id,
            traits,
          );
          handleValidationFailure(
            event,
            { score: validation.culturalScore, issues: [] },
            persona,
          );

          // Offline save via manager
          await this.offlineManager.savePersona({
            ...persona,
            syncStatus: this.offlineManager.isOffline ? "pending" : "synced",
          } as any);

          this.auditLogger.logEvent("persona-created", { id: persona.id });
          return persona;
        } catch (error) {
          this.auditLogger.logEvent("persona-creation-failed", {
    ...
    ipcMain.handle(
      "load-persona",
      async (event, id: string): Promise<Persona | null> => {
        try {
          ...
          if (persona) {
            const memory = await this.personaService.applyMemory(persona.id, {
              /* context */
            });
          ...
          this.auditLogger.logEvent("persona-load-failed", {
            id,
            error: (error as Error).message,
          });
          return null;
    ...
    ipcMain.handle(
      "apply-memory",
      async (event, id: string, context: unknown): Promise<unknown> => {
    ...
    ipcMain.handle(
      ...
      async (
        ...
        try {
          const response = await this.personaService.generateDomainResponse(
            id,
            prompt,
            domain,
          );
          if (this.offlineManager.isOffline) {
            await this.offlineManager.setDomainCache(
              domain,
              { prompt, response },
              1,
            ); // Cache for 1 day
    ...
    ipcMain.handle("load-all-personas", async (): Promise<Persona[]> => {
      try {
        ...
        if (this.offlineManager.isOnline) {
          const offlinePersonas =
            await this.offlineManager.getAllOfflinePersonas();
          await this.offlineManager.syncWhenOnline();
          personas = [
            ...personas,
            ...offlinePersonas.filter(
        ...
        this.auditLogger.logEvent("load-all-personas-failed", {
          error: (error as Error).message,
        });
        return [];
      }
  ...
  private showGovernmentNotification(
    title: string,
    body: string,
    urgency: "normal" | "high",
  ): void {
    if (this.isPrayerTime()) {
      console.log(
        "Notification suppressed during prayer time for cultural compliance.",
      );
    ...
    notification.show();
    this.auditLogger.logEvent("notification-shown", {
      title,
      urgency,
    ...
    if (mainWindow) {
      mainWindow.webContents.send("notification-displayed", {
        title,
        body,
        urgency,
      });
