...
export class IraqiOfflineManager {
    ...
    try {
      ...
      const personaData: OfflinePersona = {
        ...
        configuration: JSON.parse(
          this.encryptData(JSON.stringify(persona.configuration)),
        ),
      };
  ...
  async loadPersona(personaId: string): Promise<OfflinePersona | null> {
    try {
      ...
      const personaDataRaw = JSON.parse(
      ...
      const personaData: OfflinePersona = {
        ...personaDataRaw,
        configuration: JSON.parse(
          this.decryptData(JSON.stringify(personaDataRaw.configuration)),
        ),
  ...
  getStorageStats(): Record<string, unknown> {
    try {
      ...
      stats.totalSize =
        Math.round(((stats.totalSize as number) / (1024 * 1024)) * 100) / 100; // MB
...
export function registerOfflineHandlers(
  ...
  ipcMain.handle("get-cached-data", (event, key: string) =>
    offlineManager.getCachedData(key),
  );
  ipcMain.handle(
    "set-cached-data",
    (event, key: string, data: unknown, ttl?: number) =>
  ...
  ipcMain.handle("save-offline-chat", (event, chat: OfflineChat) =>
    offlineManager.saveChat(chat),
  );
  ipcMain.handle("load-offline-chat", (event, chatId: string) =>
    offlineManager.loadChat(chatId),
  );
  ipcMain.handle("get-all-offline-chats", () => offlineManager.getAllChats());
  ipcMain.handle("delete-offline-chat", (event, chatId: string) =>
    offlineManager.deleteChat(chatId),
  );
  ipcMain.handle("save-offline-persona", (event, persona: OfflinePersona) =>
    offlineManager.savePersona(persona),
  );
  ipcMain.handle("load-offline-persona", (event, personaId: string) =>
