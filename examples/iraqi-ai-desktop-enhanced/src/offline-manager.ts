...
export class IraqiOfflineManager {
  ...
  private initializeEncryption(): void {
    ...
    if (existsSync(keyFile)) {
      try {
        this.encryptionKey = readFileSync(keyFile) as Buffer;
      } catch (error) {
  ...
  private setupNetworkMonitoring(): void {
    ...
    process.on('online' as any, updateOnlineStatus);
    process.on('offline' as any, updateOnlineStatus);
  ...
  private encryptData(data: string, level: 'standard' | 'government' = 'standard'): string {
  ...
  private decryptData(encryptedData: string): string {
    ...
    try {
      ...
      const decipher = crypto.createDecipher(algorithm as any, this.encryptionKey);
  ...
  async saveChat(chat: OfflineChat): Promise<boolean> {
    try {
      ...
      const encryptionLevel = chat.encryptionLevel === 'government' ? 'government' : 'standard';
      
      // Encrypt sensitive data
      const chatData: OfflineChat = {
        ...chat,
  ...
  async loadChat(chatId: string): Promise<OfflineChat | null> {
    try {
      ...
      const chatDataRaw = JSON.parse(readFileSync(chatFile, 'utf-8')) as OfflineChat;
      
      // Decrypt messages
      const chatData: OfflineChat = {
        ...chatDataRaw,
        messages: chatDataRaw.messages.map((msg: OfflineMessage) => ({
          ...msg,
          content: this.decryptData(msg.content),
          timestamp: new Date(msg.timestamp)
        })),
        createdAt: new Date(chatDataRaw.createdAt),
        updatedAt: new Date(chatDataRaw.updatedAt)
      };
  ...
  async getAllChats(): Promise<OfflineChat[]> {
    try {
      const chatFiles = readdirSync(this.chatsDir).filter(file => file.endsWith('.json'));
  ...
  async deleteChat(chatId: string): Promise<boolean> {
    try {
      ...
      if (existsSync(chatFile)) {
        ...
        // Note: Use fs.unlinkSync(chatFile) in production for deletion after wipe
  ...
  // Persona Management (Integration with IraqiPersonaManagementService via IPC)
  async savePersona(persona: OfflinePersona): Promise<boolean> {
    try {
      ...
      const personaData: OfflinePersona = {
        ...persona,
        configuration: JSON.parse(this.encryptData(JSON.stringify(persona.configuration)))
      };
  ...
  async loadPersona(personaId: string): Promise<OfflinePersona | null> {
    try {
      ...
      const personaDataRaw = JSON.parse(readFileSync(personaFile, 'utf-8')) as OfflinePersona;
      
      // Decrypt configuration
      const personaData: OfflinePersona = {
        ...personaDataRaw,
        configuration: JSON.parse(this.decryptData(JSON.stringify(personaDataRaw.configuration))),
        lastUsed: new Date(personaDataRaw.lastUsed)
      };
  ...
  async saveAttachment(attachment: OfflineAttachment): Promise<boolean> {
    try {
      ...
      const encryptedAttachment: OfflineAttachment = {
        ...attachment,
        data: undefined,
        encryptedData: this.encryptData(attachment.data, 'government')
      };
      
      writeFileSync(attachmentFile, JSON.stringify(encryptedAttachment, null, 2));
  ...
  // Cache Management (Iraqi-specific: professional domains, cultural data)
  async setCachedData(key: string, data: unknown, ttl?: number): Promise<boolean> {
    try {
      const cacheFile = join(this.cacheDir, `${crypto.createHash('md5').update(key).digest('hex')}.json`);
  ...
  async getCachedData(key: string): Promise<unknown> {
    try {
      ...
      if (cacheEntry.expiresAt && new Date() > new Date(cacheEntry.expiresAt)) {
        ...
        // Note: Use fs.unlinkSync(cacheFile) in production
        return null;
      }
  ...
  private cleanupCache(): void {
    try {
      const cacheFiles = readdirSync(this.cacheDir).filter(file => file.endsWith('.json'));
      let totalSize = 0;
      const cacheEntries: Array<{ file: string; size: number; createdAt: Date }> = [];
      ...
      if (totalSize > this.config.maxCacheSize) {
        cacheEntries.sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime());
        
        let removedSize = 0;
        for (const entry of cacheEntries) {
          if (totalSize - (removedSize / (1024 * 1024)) <= this.config.maxCacheSize * 0.8) {
            break;
          }
          
          const filePath = join(this.cacheDir, entry.file);
          // Note: Use fs.unlinkSync(filePath) in production
    ...
    for (const item of itemsToProcess) {
      try {
        ...
        if (item.attempts < item.maxAttempts) {
          ...
          console.error(`Failed to sync ${item.type} ${item.id} after ${item.maxAttempts} attempts:`, error);
        }
      }
    }
  }
  ...
  getStorageStats(): Record<string, unknown> {
    try {
      ...
      const allDirs = [this.chatsDir, this.personasDir, this.attachmentsDir, this.cacheDir];
      ...
      stats.totalSize = Math.round((stats.totalSize as number) / (1024 * 1024) * 100) / 100; // MB
...
export function registerOfflineHandlers(offlineManager: IraqiOfflineManager): void {
  ipcMain.handle('enable-offline-mode', () => offlineManager.enableOfflineMode());
  ipcMain.handle('disable-offline-mode', () => offlineManager.disableOfflineMode());
  ipcMain.handle('sync-when-online', () => offlineManager.syncWhenOnline());
  ipcMain.handle('get-cached-data', (event, key: string) => offlineManager.getCachedData(key));
  ipcMain.handle('set-cached-data', (event, key: string, data: unknown, ttl?: number) => offlineManager.setCachedData(key, data, ttl));
  ipcMain.handle('clear-cache', () => offlineManager.clearCache());
  ipcMain.handle('get-storage-stats', () => offlineManager.getStorageStats());
  ipcMain.handle('save-offline-chat', (event, chat: OfflineChat) => offlineManager.saveChat(chat));
  ipcMain.handle('load-offline-chat', (event, chatId: string) => offlineManager.loadChat(chatId));
  ipcMain.handle('get-all-offline-chats', () => offlineManager.getAllChats());
  ipcMain.handle('delete-offline-chat', (event, chatId: string) => offlineManager.deleteChat(chatId));
  ipcMain.handle('save-offline-persona', (event, persona: OfflinePersona) => offlineManager.savePersona(persona));
  ipcMain.handle('load-offline-persona', (event, personaId: string) => offlineManager.loadPersona(personaId));
}
