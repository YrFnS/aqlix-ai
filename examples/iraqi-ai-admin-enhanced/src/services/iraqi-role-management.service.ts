...
export interface CulturalRequirement {
  type: 'islamic-knowledge' | 'arabic-fluency' | 'cultural-sensitivity' | 'professional-ethics';
...
export class IraqiRoleManagementService extends EventEmitter {
  ...
  private initializeRoleDefinitions(): void {
    ...
    this.roleDefinitions.set(IraqiAdminRole.SUPER_ADMIN, {
      ...
      nameAr: 'المدير العام',
      description: 'Complete system control with cultural oversight responsibility',
      ...
      culturalRequirements: [
        ...
        },
        {
          type: 'arabic-fluency',
          level: 'expert',
          description: 'Native or near-native Arabic fluency with Iraqi dialect knowledge',
          descriptionAr: 'طلاقة عربية أصلية أو شبه أصلية مع معرفة اللهجة العراقية',
          validationRequired: true
        },
        {
          type: 'cultural-sensitivity',
          level: 'expert',
          description: 'Expert knowledge of Iraqi cultural norms and sensitivities',
    ...
    this.roleDefinitions.set(IraqiAdminRole.CULTURAL_VALIDATOR, {
      ...
      description: 'Validates content and features for Islamic values and cultural appropriateness',
      descriptionAr: 'يصادق على المحتوى والميزات للقيم الإسلامية والملاءمة الثقافية',
      hierarchy: 2,
      permissions: [
        {
      ...
      culturalRequirements: [
        {
          type: 'islamic-knowledge',
          level: 'expert',
          description: 'Scholarly knowledge of Islamic jurisprudence and ethics',
    ...
    this.roleDefinitions.set(IraqiAdminRole.ORGANIZATION_ADMIN, {
      ...
      nameAr: 'مدير المؤسسة',
      description: 'Manages organizational structure, departments, and regional operations',
    ...
    this.roleDefinitions.set(IraqiAdminRole.DOMAIN_EXPERT, {
      ...
      description: 'Subject matter expert for specific professional domains (legal, medical, etc.)',
      descriptionAr: 'خبير موضوعي للمجالات المهنية المحددة (قانونية، طبية، إلخ)',
      hierarchy: 4,
      permissions: [
        {
          id: 'domain-validate',
    ...
    this.roleDefinitions.set(IraqiAdminRole.SECURITY_OFFICER, {
      ...
      nameAr: 'ضابط الأمن',
      description: 'Manages system security, access controls, and threat monitoring',
    ...
    this.roleDefinitions.set(IraqiAdminRole.CONTENT_MODERATOR, {
      ...
      nameAr: 'مشرف المحتوى',
      description: 'Moderates user content for appropriateness and cultural compliance',
    ...
    this.roleDefinitions.set(IraqiAdminRole.USER_MANAGER, {
      ...
      description: 'Manages user accounts, profiles, and basic access permissions',
      descriptionAr: 'يدير حسابات المستخدمين والملفات الشخصية وأذونات الوصول الأساسية',
      hierarchy: 6,
      permissions: [
        {
          id: 'user-create',
    ...
    this.roleDefinitions.set(IraqiAdminRole.SUPPORT_SPECIALIST, {
      ...
      nameAr: 'أخصائي الدعم',
      description: 'Provides user support and assistance with cultural sensitivity',
    ...
    this.roleDefinitions.set(IraqiAdminRole.VIEWER, {
      ...
      name: 'Viewer',
      nameAr: 'مشاهد',
      description: 'Read-only access to basic system information and public content',
  ...
  public async assignRole(
    ...
    if (culturalScore < 70) {
      throw new Error(`Cultural validation failed for role ${role}. Score: ${culturalScore}`);
    }
  ...
  public revokeRole(userId: string, role: IraqiAdminRole, revokedBy: string): boolean {
  ...
  public checkPermission(
    ...
    for (const roleAssignment of userRoles) {
      ...
      const hasPermission = roleDefinition.permissions.some(p => p.id === permissionId);
      if (hasPermission) {
        // Check governorate restriction
        if (roleAssignment.governorateRestriction && context?.governorate && 
            roleAssignment.governorateRestriction !== context.governorate) {
          continue;
        }

        // Check professional domain restriction
        if (roleAssignment.professionalDomainRestriction && context?.professionalDomain && 
            roleAssignment.professionalDomainRestriction !== context.professionalDomain) {
          continue;
        }
  ...
  public getUserActiveRoles(userId: string): RoleAssignment[] {
    ...
    return roles.filter(r => r.isActive && (!r.expiresAt || r.expiresAt > new Date()));
  }

  // Cultural Validation
  private async validateCulturalRequirements(userId: string, role: IraqiAdminRole): Promise<number> {
    const roleDefinition = this.roleDefinitions.get(role);
    ...
    for (const requirement of roleDefinition.culturalRequirements) {
      const weight = this.getCulturalRequirementWeight(requirement.type, requirement.level);
      const score = await this.mockValidateCulturalRequirement(userId, requirement);
  ...
  private async mockValidateCulturalRequirement(userId: string, requirement: CulturalRequirement): Promise<number> {
  ...
  private getCulturalContextForRole(role: IraqiAdminRole): string {
    const contexts = {
      [IraqiAdminRole.SUPER_ADMIN]: 'Full cultural oversight and responsibility',
      [IraqiAdminRole.CULTURAL_VALIDATOR]: 'Islamic and cultural compliance focus',
  ...
  private getCulturalImpact(role: IraqiAdminRole): 'low' | 'medium' | 'high' {
    const highImpactRoles = [IraqiAdminRole.SUPER_ADMIN, IraqiAdminRole.CULTURAL_VALIDATOR];
    const mediumImpactRoles = [IraqiAdminRole.ORGANIZATION_ADMIN, IraqiAdminRole.DOMAIN_EXPERT, IraqiAdminRole.CONTENT_MODERATOR];
  ...
  public getRoleHierarchy(): { role: IraqiAdminRole; hierarchy: number; name: string; nameAr: string }[] {
    const roles = [];
  ...
  public getAuditLog(filters?: {
    ...
    if (filters) {
      ...
      if (filters.culturalImpact) {
        filtered = filtered.filter(entry => entry.culturalImpact === filters.culturalImpact);
      }
    }
