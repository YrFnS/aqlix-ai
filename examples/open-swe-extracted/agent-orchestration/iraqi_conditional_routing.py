...
class IraqiRoutingCondition:
    ...
    async def _evaluate_cultural_compliance(self, state: Dict[str, Any], 
                                          cultural_context: Dict[str, Any]) -> float:
        ...
        if not any([cultural_context.get("family_context"), 
    ...
    async def _evaluate_islamic_compliance(self, state: Dict[str, Any], 
                                         cultural_context: Dict[str, Any]) -> float:
    ...
    async def _evaluate_arabic_processing(self, state: Dict[str, Any], 
                                        cultural_context: Dict[str, Any]) -> float:
    ...
    async def _evaluate_professional_domain(self, state: Dict[str, Any], 
                                          cultural_context: Dict[str, Any]) -> float:
...
class IraqiConditionalRouter(ABC):
    ...
    def add_condition(self, condition: IraqiRoutingCondition):
        ...
        self.logger.info(f"Added condition {condition.condition_id} for criteria {condition.criteria}")
    
    async def route(self, state: Dict[str, Any], 
                   cultural_context: Dict[str, Any],
                   available_nodes: List[str]) -> IraqiRoutingDecision:
        ...
        target_node = await self._select_target_node(
            state, cultural_context, available_nodes, condition_evaluations, confidence_score
        )
        ...
        professional_factors = await self._extract_professional_factors(state, cultural_context)
        ...
        self.logger.info(f"Routing decision: {target_node} (confidence: {confidence_score:.2f})")
        
        return decision
    
    @abstractmethod
    async def _select_target_node(self, state: Dict[str, Any], 
                                 cultural_context: Dict[str, Any],
    ...
    async def _generate_decision_rationale(self, target_node: str,
                                         condition_evaluations: List[Dict[str, Any]],
                                         confidence_score: float) -> str:
        """Generate human-readable decision rationale"""
        
        met_conditions = [eval for eval in condition_evaluations 
                         if eval.get("threshold_met", False)]
        failed_conditions = [eval for eval in condition_evaluations 
    ...
    async def _extract_cultural_factors(self, state: Dict[str, Any], 
    ...
    async def _extract_islamic_factors(self, state: Dict[str, Any], 
                                      cultural_context: Dict[str, Any]) -> Dict[str, Any]:
...
class IraqiManagerRouter(IraqiConditionalRouter):
    ...
    def _initialize_manager_conditions(self):
        ...
        islamic_condition = IraqiRoutingCondition(
            ...
            threshold=0.9,
            weight=0.3,
            islamic_requirements={"family_respect": True, "religious_sensitivity": True}
        )
    ...
    async def _select_target_node(self, state: Dict[str, Any], 
        ...
        if confidence_score > 0.8:
            ...
            elif action == "validate_culture" and "cultural_validator" in available_nodes:
                return "cultural_validator"
        
        # Medium confidence routing with cultural considerations
        elif confidence_score > 0.6:
            # If cultural validation is needed
            cultural_eval = next((eval for eval in condition_evaluations 
                                if eval["criteria"] == "cultural_compliance"), None)
...
class IraqiRoutingOrchestrator:
    ...
    async def route_from_node(self, current_node: str, 
                             state: Dict[str, Any],
                             cultural_context: Dict[str, Any],
                             available_nodes: List[str]) -> IraqiRoutingDecision:
        ...
        try:
            ...
            error_decision = IraqiRoutingDecision(
                target_node="error_handler" if "error_handler" in available_nodes else "END",
    ...
    async def _update_metrics(self, decision: IraqiRoutingDecision, routing_time: float):
        ...
        self.performance_metrics["average_confidence"] = (
            (current_avg * (total_decisions - 1) + decision.confidence_score) / total_decisions
        )
        ...
        self.performance_metrics["cultural_validation_rate"] = (
            (current_cultural * (total_decisions - 1) + cultural_score) / total_decisions
        )
        self.performance_metrics["islamic_compliance_rate"] = (
            (current_islamic * (total_decisions - 1) + islamic_score) / total_decisions
        )
        self.performance_metrics["professional_compliance_rate"] = (
            (current_professional * (total_decisions - 1) + professional_score) / total_decisions
        )
    ...
    async def get_routing_analytics(self) -> Dict[str, Any]:
        ...
        return {
            "total_routing_decisions": self.performance_metrics["total_routing_decisions"],
            ...
            "success_rate": (
                ...
                if self.performance_metrics["total_routing_decisions"] > 0 else 0.0
            ),
            "average_confidence": self.performance_metrics["average_confidence"],
            "cultural_validation_rate": self.performance_metrics["cultural_validation_rate"],
            "islamic_compliance_rate": self.performance_metrics["islamic_compliance_rate"],
            "professional_compliance_rate": self.performance_metrics["professional_compliance_rate"],
            ...
            "recent_decisions": [decision.to_dict() for decision in self.routing_history[-5:]]
        }
