...
class CulturalContext(BaseModel):
    ...
    islamic_compliance_score: float = Field(
        ...
        ge=0.0, 
        le=1.0,
        description="Islamic compliance score (0-1)"
    )
    political_sensitivity: str = Field(
        default="neutral",
        description="Political sensitivity assessment"
    )
    family_appropriateness: bool = Field(
        default=True,
        description="Family and community appropriateness"
    )
    professional_domain: IraqiDomain = Field(
        default=IraqiDomain.GENERAL,
        description="Primary professional domain"
    )
    arabic_content_ratio: float = Field(
        default=0.0,
        ge=0.0,
        le=1.0,
        description="Percentage of Arabic content (0-1)"
    )
    dialect_region: str = Field(
        default="baghdad",
        description="Iraqi dialect region"
    )
    
    def requires_cultural_validation(self) -> bool:
        """Check if cultural validation is required."""
        return (
            self.islamic_compliance_score < 0.95 or
            self.political_sensitivity != "neutral" or
            not self.family_appropriateness or
            self.arabic_content_ratio > 0.1
        )
...
class IraqiContextGatherer:
    ...
    def __init__(self):
        self.cultural_patterns = [
            "cultural-validator", "islamic-compliance", "arabic-processor",
            "rtl-layout", "iraqi-dialect", "family-appropriate",
            "political-neutral", "professional-standard"
        ]
        self.payment_patterns = [
            "zaincash", "fastpay", "nasswallet", "payment-security",
            "iraqi-gateway", "dinar-processing"
        ]
        self.government_patterns = [
            "ministry", "government-service", "iraqi-workflow",
            "department", "official-process", "compliance"
        ]
        self.arabic_patterns = [
            "arabic-nlp", "rtl-text", "arabic-font", "dialect-processing",
            "mixed-content", "arabic-typography"
        ]
    ...
    async def gather_cultural_context(self, state: PlanningState) -> Dict[str, Any]:
        ...
        for pattern in self.cultural_patterns:
            if pattern.replace("-", " ") in user_request or pattern.replace("-", "_") in user_request:
        ...
        for pattern in self.arabic_patterns:
            if pattern.replace("-", " ") in user_request or "arabic" in user_request or "rtl" in user_request:
    ...
    def analyze_cultural_requirements(self, context: Dict[str, Any], user_request: str) -> CulturalContext:
        ...
        islamic_score = sum(1 for indicator in islamic_indicators if indicator in user_request.lower()) / len(islamic_indicators)
        cultural_ctx.islamic_compliance_score = min(islamic_score + 0.7, 1.0)  # Base compliance + indicators
...
class IraqiPlanGenerator:
    ...
    async def generate_plan(self, state: PlanningState, context: Dict[str, Any]) -> List[str]:
    ...
    def validate_plan_cultural_compliance(self, plan: List[str]) -> Dict[str, Any]:
        ...
        islamic_keywords = ["islamic", "halal", "cultural-validator", "family-appropriate"]
        islamic_score = sum(1 for keyword in islamic_keywords if keyword in plan_text) / len(islamic_keywords)
        ...
        validation["arabic_processing_included"] = any(keyword in plan_text for keyword in arabic_keywords)
        
        # Check professional validation
        professional_keywords = ["professional", "domain-expert", "regulatory", "compliance"]
        validation["professional_validation_included"] = any(keyword in plan_text for keyword in professional_keywords)
        
        # Check cultural testing
        testing_keywords = ["cultural-tester", "cultural validation", "islamic compliance"]
        validation["cultural_testing_included"] = any(keyword in plan_text for keyword in testing_keywords)
...
class IraqiPlanningAgent:
    ...
    def create_session(self, user_request: str, session_id: Optional[str] = None) -> str:
    ...
    async def process_planning_request(self, user_request: str, session_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def _analyze_cultural_context(self, state: PlanningState) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "arabic_processing_needed": "arabic" in user_request.lower() or "rtl" in user_request.lower(),
        ...
        cultural_ctx.arabic_content_ratio = 0.3 if analysis["arabic_processing_needed"] else 0.0
        cultural_ctx.professional_domain = IraqiDomain(analysis["professional_domain_detected"]) if analysis["professional_domain_detected"] else IraqiDomain.GENERAL
    ...
    async def _generate_plan(self, state: PlanningState, context: Dict[str, Any]) -> List[str]:
        """Generate culturally-intelligent execution plan."""
    ...
    async def _validate_cultural_compliance(self, state: PlanningState) -> Dict[str, Any]:
        """Validate plan for comprehensive cultural compliance."""
        validation = self.plan_generator.validate_plan_cultural_compliance(state.proposed_plan)
        ...
        cultural_indicators = [
            "iraqi-cultural-validator", "islamic compliance", "family-appropriate",
            "political-neutral", "cultural validation"
        ]
        cultural_score = sum(1 for indicator in cultural_indicators if indicator in plan_text) / len(cultural_indicators)
        enhanced_validation["cultural_compliance_score"] = cultural_score
        
        # Calculate Iraqi agent integration score
        iraqi_agents = [
            "iraqi-cultural-validator", "arabic-rtl-processor", "iraqi-payment-tester",
            "iraqi-security-specialist", "iraqi-professional-domain-expert"
        ]
        agent_score = sum(1 for agent in iraqi_agents if agent in plan_text) / len(iraqi_agents)
        ...
        if domain in domain_keywords:
            domain_coverage = sum(1 for keyword in domain_keywords[domain] if keyword in plan_text) / len(domain_keywords[domain])
    ...
    async def _finalize_plan(self, state: PlanningState, validation: Dict[str, Any]) -> List[str]:
    ...
    def _calculate_performance_metrics(self, state: PlanningState) -> Dict[str, float]:
        ...
        metrics = {
            ...
            "cultural_compliance_score": state.validation_results.get("cultural_compliance_score", 0.0),
    ...
    def get_session_status(self, session_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "cultural_context": state.cultural_context.dict(),
            "plan_items_count": len(state.proposed_plan),
            "status": "in_progress" if state.phase != PlanningPhase.FINALIZATION else "completed"
        }
    ...
    for test_case in test_cases:
        ...
        try:
            ...
            if result.get("status") == "completed":
                ...
                print(f"   - Islamic Compliance: {cultural['islamic_compliance_score']:.2f}")
                print(f"   - Professional Domain: {cultural['professional_domain']}")
                print(f"   - Arabic Content Ratio: {cultural['arabic_content_ratio']:.2f}")
                
                print(f"\n📊 Validation Results:")
                validation = result['validation_results']
                print(f"   - Cultural Compliance: {validation.get('cultural_compliance_score', 0):.2f}")
                print(f"   - Agent Integration: {validation.get('iraqi_agent_integration_score', 0):.2f}")
                print(f"   - Overall Readiness: {validation.get('overall_cultural_readiness', False)}")
                
                print(f"\n⚡ Performance Metrics:")
                metrics = result['performance_metrics']
                print(f"   - Planning Duration: {metrics['planning_duration_seconds']:.2f}s")
