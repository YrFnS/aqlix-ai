...
class IraqiCulturalValidator:
    ...
    def _load_arabic_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "iraqi_dialect_markers": ["شلونك", "وين", "شنو", "هسه", "جان", "وياه"],
        }
    ...
    async def validate_cultural_compliance(
        self, code: str, context: CulturalContext
        ...
        professional_score, professional_violations = (
            self._check_professional_compliance(code, context.professional_domain)
        )
    ...
    def _check_islamic_compliance(self, code: str) -> Tuple[float, List[str]]:
        ...
        interest_patterns = [r"\b(interest|usury|riba)\b", r"\b(فائدة|ربا)\b"]
        for pattern in interest_patterns:
            if re.search(pattern, code.lower()):
                violations.append(
                    "Contains interest-based calculations - requires Islamic banking compliance"
                )
                score -= 10.0
                break
    ...
    def _check_professional_compliance(
        self, code: str, domain: ProfessionalDomain
        ...
        if terminology:
            ...
            if found_terms == 0 and any(term in code for term in ["arabic", "عربي"]):
                violations.append(
                    f"Missing {domain.value} terminology for Arabic content"
                )
                score -= 15.0
    ...
    def _check_arabic_support(self, code: str) -> Tuple[float, List[str]]:
        ...
        if has_mixed_content:
            ...
            if not any(handler in code.lower() for handler in mixed_handlers):
                violations.append(
                    "Mixed Arabic-English content without proper handling"
                )
                score -= 20.0
...
- Beneficial to society and the Iraqi community
    ...
    def generate_system_prompt(
        self, context: CulturalContext, task_plan: List[TaskPlanItem] = None
    ) -> str:
        ...
        if context.professional_domain != ProfessionalDomain.GENERAL:
            domain_prompt = self.professional_prompts.get(
...
class IraqiToolSystem:
    ...
    async def execute_tool(
        self, tool_name: str, parameters: Dict[str, Any], context: CulturalContext
                ...
                return (
                    {
                        ...
                        "details": pre_validation_result,
                    },
                    0.0,
                    pre_validation_result["violations"],
                )
        ...
        if tool_config.get("cultural_validation", False):
            (
                ...
                violations,
                validation_details,
            ) = await self._post_execution_validation(tool_name, result, context)
    ...
    async def _pre_execution_validation(
        ...
        for key, value in parameters.items():
            if isinstance(value, str):
                islamic_score, islamic_violations = (
                    self.validator._check_islamic_compliance(value)
                )
                if islamic_violations:
                    violations.extend(
                        [f"Parameter '{key}': {v}" for v in islamic_violations]
                    )
        ...
        if tool_name == "cultural_shell_executor":
            ...
            if any(
                ...
                for forbidden in ["rm -rf", "sudo rm", "format", "delete"]
            ):
    ...
    async def _post_execution_validation(
        self, tool_name: str, result: Dict[str, Any], context: CulturalContext
    ) -> Tuple[float, List[str], Dict[str, Any]]:
        ...
        (
            ...
            violations,
            validation_details,
        ) = await self.validator.validate_cultural_compliance(text_content, context)
    ...
    async def _execute_code_generator(
        self, parameters: Dict[str, Any], context: CulturalContext
    ) -> Dict[str, Any]:
    ...
    def validate_cultural_compliance(self) -> bool:
        ...
        customized_code = template.format(
            ...
            description=parameters.get(
                "description", "Culturally compliant implementation"
            ),
    ...
    async def _execute_rtl_validator(
        ...
        rtl_support_found = any(
            indicator in content.lower() for indicator in rtl_indicators
        )

        # Check for mixed content
        mixed_content = bool(
            re.search(
                r"[\u0600-\u06FF]+.*[a-zA-Z]+|[a-zA-Z]+.*[\u0600-\u06FF]+", content
            )
        )
    ...
    async def _execute_islamic_checker(
        self, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            if violations
            else ["Code passes Islamic compliance check"],
    ...
    async def _execute_domain_validator(
        ...
        return {
            ...
            "violations": violations,
            "domain_requirements": self.validator.professional_standards.get(
                domain, {}
            ),
    ...
    async def _execute_cultural_shell(
        ...
        dangerous_commands = [
            ...
            "chmod 777",
            "curl",
            "wget",  # Restrict external access without validation
        ]
...
class IraqiProgrammingAgent:
    ...
    def __init__(
        self, cultural_context: CulturalContext = None, enable_logging: bool = True
    ):
    ...
    async def _generate_cultural_task_plan(
        ...
        cultural_requirements = await self._analyze_cultural_requirements(
            task_description
        )
    ...
    async def _analyze_cultural_requirements(
        ...
        if (
            re.search(r"[\u0600-\u06FF]+", task_description)
            or "arabic" in task_description.lower()
        ):
        ...
        for domain, keywords in domain_keywords.items():
            if any(keyword in task_description.lower() for keyword in keywords):
                self.state.cultural_context.professional_domain = ProfessionalDomain(
                    domain
                )
                requirements["professional"] = [
                    f"{domain.title()} domain compliance required"
                ]
                break
        ...
        if any(
            indicator in task_description.lower() for indicator in islamic_indicators
        ):
            self.state.cultural_context.compliance_level = (
                CulturalComplianceLevel.ISLAMIC_CERTIFIED
            )
            requirements["islamic"] = [
                "Islamic certification level compliance required"
            ]
    ...
    async def _generate_cultural_action(self, task: TaskPlanItem) -> Dict[str, Any]:
        ...
        system_prompt = self.prompt_system.generate_system_prompt(
            self.state.cultural_context, self.state.task_plan
        )
        ...
        action_requirements = {
            ...
            "professional_validation": self.state.cultural_context.professional_domain
        ...
        for tool_name in recommended_tools:
            try:
                ...
                (
                    ...
                    cultural_score,
                    violations,
                ) = await self.tool_system.execute_tool(
                ...
                self.logger.info(
                    f"Executed {tool_name} with cultural score: {cultural_score}"
                )
    ...
    async def _validate_task_completion(
        self, task: TaskPlanItem, execution_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        self.performance_metrics["average_cultural_score"] = (
            ...
            * (self.performance_metrics["cultural_validations"] - 1)
            + overall_cultural_score
        ) / self.performance_metrics["cultural_validations"]
    ...
    async def _generate_cultural_summary(self) -> Dict[str, Any]:
        ...
        cultural_scores = [
            task.cultural_score for task in completed_tasks if task.cultural_score > 0
        ]

        return {
            "total_tasks": len(self.state.task_plan),
            "completed_tasks": len(completed_tasks),
            "culturally_validated_tasks": len(
                [task for task in completed_tasks if task.cultural_validated]
            ),
            "islamic_compliant_tasks": len(
                [task for task in completed_tasks if task.islamic_compliant]
            ),
            ...
            "cultural_features_utilized": [
                ...
                if self.state.cultural_context.arabic_support_required
                else None,
                "RTL layout support"
                if self.state.cultural_context.rtl_support_required
                else None,
    ...
    async def get_cultural_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "task_progress": {
                ...
                "completed_tasks": len(
                    [task for task in self.state.task_plan if task.completed]
                ),
                "culturally_validated": len(
                    [task for task in self.state.task_plan if task.cultural_validated]
                ),
            },
            "compliance_metrics": {
                "average_cultural_score": self.performance_metrics[
                    "average_cultural_score"
                ],
                "total_validations": self.performance_metrics["cultural_validations"],
                "islamic_compliance_checks": self.performance_metrics[
                    "islamic_compliance_checks"
                ],
                "arabic_processing_operations": self.performance_metrics[
                    "arabic_processing_operations"
                ],
            },
