...
class CulturalComplianceMetrics:
    ...
    def meets_approval_threshold(self) -> bool:
        ...
        return (
            self.islamic_compliance_score >= 95.0
            and self.cultural_appropriateness_score >= 90.0
            and self.overall_score() >= 90.0
        )
...
class ReviewState:
    ...
    cultural_metrics: CulturalComplianceMetrics = field(
        default_factory=CulturalComplianceMetrics
    )
    ...
    # Quality gates
    scripts_identified: Dict[str, str] = field(
    ...
    def add_message(self, role: str, content: str, cultural_context: str = ""):
        ...
        logger.info(
            f"Added {role} message in {self.phase.value} phase: {content[:100]}..."
        )
...
class IraqiReviewerAgent:
    ...
    def __init__(self, repository_path: str, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(
            f"Initialized Iraqi Reviewer Agent for repository: {self.repository_path}"
        )
    ...
    async def review_changes(
        ...
        try:
            logger.info(
                f"Starting Iraqi-enhanced review for {len(changed_files)} files"
            )
            state.add_message(
                "system",
                f"Starting comprehensive Iraqi review for {len(changed_files)} changed files",
            )
            ...
            logger.info(
                f"Cultural compliance score: {state.cultural_metrics.overall_score():.1f}%"
            )
            logger.info(
                f"Final decision: {state.decision.value if state.decision else 'pending'}"
            )
    ...
    async def _initialize_review_context(self, state: ReviewState):
        ...
        state.add_message(
            "system", "Review context initialized with cultural awareness"
        )
    ...
    async def _identify_project_structure(self, state: ReviewState):
        ...
        try:
            ...
            state.add_scratchpad_note(
                f"Project structure analysis failed: {str(e)}", "error"
            )
    ...
    async def _detect_cultural_context(self, state: ReviewState):
        ...
        try:
                        ...
                        with open(full_path, "r", encoding="utf-8") as f:
                            ...
                            if self.arabic_analyzer.contains_arabic(content):
                                cultural_indicators.append(
                                    f"{file_path}: Contains Arabic text"
                                )
                            ...
                            for keyword in islamic_keywords:
                                if keyword.lower() in content.lower():
                                    cultural_indicators.append(
                                        f"{file_path}: Contains Islamic keyword '{keyword}'"
                                    )

                            # Detect RTL indicators
                            if (
                                "direction: rtl" in content.lower()
                                or 'dir="rtl"' in content.lower()
                            ):
                                cultural_indicators.append(
                                    f"{file_path}: Contains RTL layout indicators"
                                )
            ...
            if cultural_indicators:
                ...
                state.add_scratchpad_note(
                    "No obvious cultural indicators detected in changed files",
                    "cultural_context",
                )
            ...
            state.add_scratchpad_note(
                f"Cultural context detection failed: {str(e)}", "error"
            )
        ...
        try:
            ...
            if package_json_path.exists():
                ...
                for package_name in all_deps.keys():
                    if any(
                        indicator in package_name.lower()
                        for indicator in [
                            "arabic",
                            "rtl",
                            "islamic",
                        ...
                        cultural_packages.append(
                            f"{package_name}: {all_deps[package_name]}"
                        )
            ...
            state.add_scratchpad_note(
                f"Dependency validation failed: {str(e)}", "error"
            )
    ...
    async def _analyze_file_cultural_compliance(
        self, state: ReviewState, file_path: str
    ):
        ...
        try:
            ...
            islamic_issues = await self.islamic_validator.validate_content(
                content, file_path
            )
            if islamic_issues:
            ...
            cultural_issues = await self.cultural_analyzer.analyze_appropriateness(
                content, file_path
            )
            if cultural_issues:
            ...
            if self.arabic_analyzer.contains_arabic(content):
                arabic_issues = await self.arabic_analyzer.analyze_arabic_code(
                    content, file_path
                )
                if arabic_issues:
            ...
            if self.professional_validator.is_professional_content(content):
                professional_issues = (
                    await self.professional_validator.validate_domain_compliance(
                        content, file_path
                    )
                )
            ...
            logger.error(
                f"Failed to analyze cultural compliance for {file_path}: {str(e)}"
            )
            state.add_scratchpad_note(
                f"Cultural analysis failed for {file_path}: {str(e)}", "error"
            )
    ...
    async def _calculate_cultural_compliance_scores(self, state: ReviewState):
        ...
        try:
            ...
            state.cultural_metrics.islamic_compliance_score = max(
                0, 100 - (islamic_issues_count * 10)
            )

            # Cultural appropriateness scoring
            cultural_issues_count = len(state.cultural_appropriateness_issues)
            state.cultural_metrics.cultural_appropriateness_score = max(
                0, 100 - (cultural_issues_count * 8)
            )

            # Professional domain scoring
            professional_issues_count = len(state.professional_domain_issues)
            state.cultural_metrics.professional_domain_score = max(
                0, 100 - (professional_issues_count * 12)
            )

            # Arabic processing scoring
            arabic_issues_count = len(state.arabic_processing_issues)
            state.cultural_metrics.arabic_processing_score = max(
                0, 100 - (arabic_issues_count * 15)
            )

            # Government compliance scoring (placeholder - would integrate with actual validators)
            state.cultural_metrics.government_compliance_score = (
                95.0  # Default high score
            )

            logger.info(f"Cultural compliance scores calculated:")
            logger.info(
                f"  Islamic compliance: {state.cultural_metrics.islamic_compliance_score:.1f}%"
            )
            logger.info(
                f"  Cultural appropriateness: {state.cultural_metrics.cultural_appropriateness_score:.1f}%"
            )
            logger.info(
                f"  Professional domain: {state.cultural_metrics.professional_domain_score:.1f}%"
            )
            logger.info(
                f"  Arabic processing: {state.cultural_metrics.arabic_processing_score:.1f}%"
            )
            logger.info(
                f"  Overall score: {state.cultural_metrics.overall_score():.1f}%"
            )
    ...
    async def _generate_culturally_aware_review_actions(self, state: ReviewState):
        ...
        try:
            ...
            state.review_actions.sort(
                key=lambda a: (a.priority, -len(a.cultural_impact))
            )
    ...
    async def _execute_review_actions_with_cultural_validation(
        self, state: ReviewState
    ):
        """Execute review actions with cultural validation at each step"""
        state.phase = ReviewPhase.EXECUTE_ACTIONS
        logger.info(
            f"Executing {len(state.review_actions)} review actions with cultural validation"
        )
        ...
        try:
            for action in state.review_actions:
                ...
                logger.info(
                    f"Executing action: {action.action_type} - {action.description}"
                )
    ...
    async def _execute_script_identification(
        self, state: ReviewState, action: ReviewAction
    ):
        ...
        try:
            ...
            action.results = {
                ...
                "cultural_scripts": [
                    name
                    for name in scripts_found.keys()
                    if any(
                        indicator in name
                        for indicator in ["cultural", "islamic", "arabic"]
                    )
                ],
            }
            ...
            state.add_scratchpad_note(
                ...
                + "\n".join(
                    f"- {name}: {command}" for name, command in scripts_found.items()
                ),
                "scripts",
            )
    ...
    async def _execute_file_analysis(self, state: ReviewState, action: ReviewAction):
        ...
        try:
            ...
            diff_result = await self.cultural_diff.analyze_diff(
                file_path, state.base_branch
            )
            ...
            change_relevance = await self._validate_change_relevance(
                file_path, state.user_request
            )
    ...
    async def _analyze_file_location(self, file_path: str) -> Dict[str, Any]:
        ...
        return {"appropriate_location": True, "issues": [], "recommendations": []}

    async def _validate_change_relevance(
        self, file_path: str, user_request: str
    ) -> Dict[str, Any]:
    ...
    async def _execute_islamic_validation(
        self, state: ReviewState, action: ReviewAction
    ):
        ...
        try:
            ...
            for issue in state.islamic_compliance_issues:
                ...
                validation_result = (
                    await self.islamic_validator.validate_issue_resolution(issue)
                )
                validation_results.append(validation_result)

            # Calculate new Islamic compliance score
            resolved_issues = [
                r for r in validation_results if r.get("resolved", False)
            ]
            resolution_rate = (
                len(resolved_issues) / len(validation_results)
                if validation_results
                else 1.0
            )

            # Update Islamic compliance score
            state.cultural_metrics.islamic_compliance_score = min(
                100.0,
                state.cultural_metrics.islamic_compliance_score
                + (resolution_rate * 20),
            )
    ...
    async def _execute_cultural_validation(
        self, state: ReviewState, action: ReviewAction
    ):
        ...
        try:
            ...
            for issue in state.cultural_appropriateness_issues:
                validation_result = (
                    await self.cultural_analyzer.validate_issue_resolution(issue)
                )
                validation_results.append(validation_result)
            ...
            state.cultural_metrics.cultural_appropriateness_score = min(
                100.0,
                state.cultural_metrics.cultural_appropriateness_score
                + (resolution_rate * 15),
            )
    ...
    async def _execute_arabic_validation(
        self, state: ReviewState, action: ReviewAction
    ):
        ...
        try:
            ...
            for issue in state.arabic_processing_issues:
                validation_result = (
                    await self.arabic_analyzer.validate_issue_resolution(issue)
                )
                validation_results.append(validation_result)
            ...
            state.cultural_metrics.arabic_processing_score = min(
                100.0,
    ...
    async def _execute_professional_validation(
        self, state: ReviewState, action: ReviewAction
    ):
        ...
        try:
            ...
            for issue in state.professional_domain_issues:
                validation_result = (
                    await self.professional_validator.validate_issue_resolution(issue)
                )
                validation_results.append(validation_result)
            ...
            state.cultural_metrics.professional_domain_score = min(
                100.0,
                state.cultural_metrics.professional_domain_score
                + (resolution_rate * 20),
            )
    ...
    async def _perform_islamic_compliance_validation(self, state: ReviewState):
        ...
        try:
            ...
            compliance_results = (
                await self.cultural_quality_gate.validate_islamic_compliance(
                    state.changed_files, self.repository_path
                )
            )

            # Update Islamic compliance score based on validation results
            if compliance_results.get("compliance_score"):
                state.cultural_metrics.islamic_compliance_score = compliance_results[
                    "compliance_score"
                ]
            ...
            compliance_threshold_met = (
                state.cultural_metrics.islamic_compliance_score >= 95.0
            )
            ...
            state.add_message(
                "error", f"Islamic compliance validation failed: {str(e)}"
            )
    ...
    async def _perform_professional_domain_validation(self, state: ReviewState):
        ...
        try:
            ...
            professional_results = (
                await self.professional_standards.validate_professional_compliance(
                    state.changed_files, self.repository_path, state.user_request
                )
            )

            # Update professional domain score
            if professional_results.get("compliance_score"):
                state.cultural_metrics.professional_domain_score = professional_results[
                    "compliance_score"
                ]
            ...
            professional_threshold_met = (
                state.cultural_metrics.professional_domain_score >= 85.0
            )
    ...
    async def _make_final_culturally_informed_decision(self, state: ReviewState):
        ...
        try:
            ...
            if meets_threshold and overall_score >= 90.0:
                ...
                if (
                    len(state.islamic_compliance_issues) == 0
                    and len(state.cultural_appropriateness_issues) == 0
                    and len(state.arabic_processing_issues) == 0
                ):
    ...
    async def _generate_final_review_summary(self, state: ReviewState):
        ...
        try:
            ...
            summary_parts.append(
                f"**Review Duration:** {state.total_review_time_seconds:.2f} seconds"
            )
            summary_parts.append("")
            ...
            summary_parts.append(
                f"- Islamic Compliance: {state.cultural_metrics.islamic_compliance_score:.1f}% (Threshold: >95%)"
            )
            summary_parts.append(
                f"- Cultural Appropriateness: {state.cultural_metrics.cultural_appropriateness_score:.1f}% (Threshold: >90%)"
            )
            summary_parts.append(
                f"- Professional Domain: {state.cultural_metrics.professional_domain_score:.1f}% (Threshold: >85%)"
            )
            summary_parts.append(
                f"- Arabic Processing: {state.cultural_metrics.arabic_processing_score:.1f}% (Threshold: >90%)"
            )
            summary_parts.append(
                f"- Government Compliance: {state.cultural_metrics.government_compliance_score:.1f}% (Threshold: >95%)"
            )
            summary_parts.append("")

            # Issues summary
            total_issues = (
                len(state.islamic_compliance_issues)
            ...
            if state.islamic_compliance_issues:
                summary_parts.append(
                    f"**Islamic Compliance Issues ({len(state.islamic_compliance_issues)}):**"
                )
                for issue in state.islamic_compliance_issues[:5]:  # Limit to first 5
                    summary_parts.append(f"- {issue}")
                if len(state.islamic_compliance_issues) > 5:
                    summary_parts.append(
                        f"- ... and {len(state.islamic_compliance_issues) - 5} more"
                    )
                summary_parts.append("")

            if state.cultural_appropriateness_issues:
                summary_parts.append(
                    f"**Cultural Appropriateness Issues ({len(state.cultural_appropriateness_issues)}):**"
                )
                for issue in state.cultural_appropriateness_issues[:5]:
                    summary_parts.append(f"- {issue}")
                if len(state.cultural_appropriateness_issues) > 5:
                    summary_parts.append(
                        f"- ... and {len(state.cultural_appropriateness_issues) - 5} more"
                    )
                summary_parts.append("")

            if state.arabic_processing_issues:
                summary_parts.append(
                    f"**Arabic Processing Issues ({len(state.arabic_processing_issues)}):**"
                )
                for issue in state.arabic_processing_issues[:3]:
                    summary_parts.append(f"- {issue}")
                if len(state.arabic_processing_issues) > 3:
                    summary_parts.append(
                        f"- ... and {len(state.arabic_processing_issues) - 3} more"
                    )
                summary_parts.append("")
            ...
            if state.decision == ReviewDecision.APPROVED:
                summary_parts.append(
                    "✅ All Iraqi cultural compliance requirements met. Ready for approval."
                )
            elif state.decision == ReviewDecision.NEEDS_ISLAMIC_COMPLIANCE:
                summary_parts.append(
                    "🕌 Islamic compliance improvements required before approval."
                )
                summary_parts.append(
                    "- Review content for Islamic principles alignment"
                )
                ...
                summary_parts.append(
                    "- Review cultural sensitivity in user-facing elements"
                )
            ...
            summary_parts.append(
                f"- Cultural Analysis: {state.cultural_analysis_time_seconds:.2f}s"
            )
            summary_parts.append(
                f"- Islamic Validation: {state.islamic_validation_time_seconds:.2f}s"
            )
            summary_parts.append(
                f"- Professional Validation: {state.professional_validation_time_seconds:.2f}s"
            )
            summary_parts.append(
                f"- Actions Completed: {len([a for a in state.review_actions if a.completed])}/{len(state.review_actions)}"
            )
            ...
            state.add_message(
                "error", f"Final review summary generation failed: {str(e)}"
            )
...
async def main():
    ...
    try:
        ...
        review_result = await reviewer.review_changes(
            changed_files=changed_files, user_request=user_request, base_branch="main"
        )
        ...
        print(
            f"Islamic Compliance: {review_result.cultural_metrics.islamic_compliance_score:.1f}%"
        )
