...
class IraqiCulturalValidator:
    ...
    async def validate_content(
        ...
        try:
            ...
            islamic_score = await IraqiCulturalValidator._validate_islamic_compliance(content)
            ...
            political_score = await IraqiCulturalValidator._validate_political_neutrality(content)
            ...
            if professional_domain:
                domain_score = await IraqiCulturalValidator._validate_professional_domain(
                    content, professional_domain
                )
                if domain_score < 0.85:
                    issues.append(f"Content may not meet {professional_domain.value} domain standards")
                    recommendations.append(f"Ensure compliance with Iraqi {professional_domain.value} requirements")
    ...
    async def _validate_professional_domain(
        ...
        domain_requirements = {
            ProfessionalDomain.LEGAL: [
                "legal disclaimer", "iraqi law", "compliance"
            ],
            ProfessionalDomain.MEDICAL: [
                "medical disclaimer", "healthcare standards", "patient privacy"
            ],
            ProfessionalDomain.EDUCATIONAL: [
                "educational content", "learning objectives", "curriculum"
            ],
            ProfessionalDomain.GOVERNMENT: [
                "government service", "citizen privacy", "official procedures"
            ]
        }
...
class IraqiArabicProcessor:
    ...
    async def process_arabic_content(content: str) -> ArabicProcessingResult:
        ...
        try:
            ...
            dialect_recognition = await IraqiArabicProcessor._recognize_iraqi_dialect(content)
    ...
    async def _calculate_rtl_accuracy(content: str) -> float:
        ...
        rtl_issues = len(re.findall(r'[a-zA-Z][\u0600-\u06FF]', content))  # Latin before Arabic
        rtl_issues += len(re.findall(r'[\u0600-\u06FF][a-zA-Z]', content))  # Arabic before Latin
    ...
    async def _validate_mixed_content(content: str) -> float:
        ...
        mixed_transitions = re.findall(r'[a-zA-Z][\u0600-\u06FF]|[\u0600-\u06FF][a-zA-Z]', content)
...
class IraqiGitHubIntegration:
            ...
            try:
                ...
                if hasattr(result, 'body') and result.body:
                    ...
                    if not validation_result.is_compliant:
                        logger.warning(f"⚠️ Cultural validation issues detected: {validation_result.cultural_issues}")
                
                return result
                
            except Exception as error:
                if attempt < max_retries and "401" in str(error):
                    logger.warning(f"🔄 Retrying operation due to 401 error (attempt {attempt + 1})")
                    # Get fresh installation token
                    fresh_token = await self.auth.get_installation_token(kwargs.get('installation_id', ''))
                    if fresh_token:
    ...
    async def create_culturally_compliant_pull_request(
        ...
        try:
            ...
            if not cultural_validation.is_compliant:
                logger.error(f"❌ PR content fails cultural validation: {cultural_validation.cultural_issues}")
                return None
            
            # Arabic processing if needed
            arabic_result = None
            if any(lang in [IraqiLanguageType.ARABIC, IraqiLanguageType.IRAQI_DIALECT, IraqiLanguageType.MIXED] 
                   for lang in repo_info.supported_languages):
                logger.info("🔤 Processing Arabic content...")
                arabic_result = await self.arabic_processor.process_arabic_content(f"{title}\n\n{body}")
                
                if arabic_result.rtl_accuracy < 0.99:
                    logger.warning(f"⚠️ RTL accuracy below threshold: {arabic_result.rtl_accuracy}")
                
                if arabic_result.dialect_recognition < 0.85:
                    logger.warning(f"⚠️ Iraqi dialect recognition below threshold: {arabic_result.dialect_recognition}")
            ...
            installation_token = await self.auth.get_installation_token(repo_info.installation_id)
            if not installation_token:
                logger.error("❌ Failed to get installation token")
                return None
            
            async def create_pr_operation(token: str, **kwargs) -> Optional[Dict[str, Any]]:
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        logger.info(f"✅ Pull request created successfully: {pr_result.get('html_url')}")
    ...
    async def get_issue_with_iraqi_task_plan(
        ...
        try:
            ...
            async def get_issue_operation(token: str, **kwargs) -> Optional[Dict[str, Any]]:
                async with httpx.AsyncClient() as client:
    ...
    async def update_issue_with_iraqi_task_plan(
        ...
        try:
            # Get current issue
            issue, _ = await self.get_issue_with_iraqi_task_plan(repo_info, issue_number)
            if not issue:
            ...
            async def update_issue_operation(token: str, **kwargs) -> Optional[Dict[str, Any]]:
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(f"✅ Issue #{issue_number} updated with Iraqi task plan")
                        return result
    ...
    async def create_issue_comment_with_cultural_validation(
        ...
        try:
            ...
            if not cultural_validation.is_compliant:
                logger.error(f"❌ Comment fails cultural validation: {cultural_validation.cultural_issues}")
                return None
            ...
            if any(lang in [IraqiLanguageType.ARABIC, IraqiLanguageType.IRAQI_DIALECT, IraqiLanguageType.MIXED] 
                ...
                arabic_result = await self.arabic_processor.process_arabic_content(comment_body)
            ...
            async def create_comment_operation(token: str, **kwargs) -> Optional[Dict[str, Any]]:
                async with httpx.AsyncClient() as client:
                    response = await client.post(
                    ...
                    if response.status_code == 201:
                        result = response.json()
                        logger.info(f"✅ Comment created successfully on issue #{issue_number}")
                        return result
    ...
    async def get_issue_comments_filtered(
        ...
        try:
            ...
            async def get_comments_operation(token: str, **kwargs) -> Optional[List[Dict[str, Any]]]:
                async with httpx.AsyncClient() as client:
                    response = await client.get(
                        f"https://api.github.com/repos/{repo_info.owner}/{repo_info.repo}/issues/{issue_number}/comments",
                        headers={
                        ...
                        if filter_bot_comments:
                            comments = [
                                ...
                                if comment.get('user', {}).get('type') != 'Bot' and
                                   '[bot]' not in comment.get('user', {}).get('login', '')
                            ]
                            ...
                            for comment in comments:
                                if comment.get('body'):
                                    validation = await self.cultural_validator.validate_content(
                                        comment['body'], CulturalValidationLevel.MINIMAL
                                    )
                                    if validation.is_compliant:
                                        filtered_comments.append(comment)
                                    else:
                                        logger.info(f"ℹ️ Filtered comment due to cultural compliance issues")
                                else:
                                    filtered_comments.append(comment)
                            comments = filtered_comments
                        
                        logger.info(f"✅ Retrieved {len(comments)} filtered comments for issue #{issue_number}")
                        return comments
    ...
    async def get_branch_info(
        ...
        try:
            ...
            async def get_branch_operation(token: str, **kwargs) -> Optional[Dict[str, Any]]:
                async with httpx.AsyncClient() as client:
                    response = await client.get(
                    ...
                    logger.error(f"❌ Failed to get branch info: {response.status_code} - {response.text}")
                    return None
...
"""
    ...
    async def _enhance_comment_with_validation(
        ...
        if cultural_validation.islamic_compliance < 1.0:
            footer_parts.append(f"☪️ Islamic Compliance {cultural_validation.islamic_compliance:.1%}")
    ...
    async def _extract_iraqi_task_plan_from_content(self, content: str) -> Optional[IraqiTaskPlan]:
        """Extract Iraqi task plan from issue content."""
        if self.IRAQI_TASK_OPEN_TAG not in content or self.IRAQI_TASK_CLOSE_TAG not in content:
            return None
        
        try:
            task_plan_str = content.split(self.IRAQI_TASK_OPEN_TAG)[1].split(self.IRAQI_TASK_CLOSE_TAG)[0].strip()
            ...
            if task_plan_data.get('professional_domain'):
                professional_domain = ProfessionalDomain(task_plan_data['professional_domain'])
            ...
            task_plan = IraqiTaskPlan(
                ...
                islamic_compliance_verified=task_plan_data.get('islamic_compliance_verified', False),
    ...
    async def _insert_iraqi_task_plan_to_body(self, body: str, task_plan: IraqiTaskPlan) -> str:
        """Insert Iraqi task plan into issue body."""
        task_plan_json = json.dumps(asdict(task_plan), indent=2, ensure_ascii=False)
        wrapped_plan = f"{self.IRAQI_TASK_OPEN_TAG}\n{task_plan_json}\n{self.IRAQI_TASK_CLOSE_TAG}"
        
        if self.IRAQI_TASK_OPEN_TAG not in body and self.IRAQI_TASK_CLOSE_TAG not in body:
            # Add new task plan
            return f"{body}\n\n<details>\n<summary>🇮🇶 Iraqi AI Task Plan</summary>\n\n{wrapped_plan}\n\n</details>"
        else:
            # Replace existing task plan
            content_before = body.split(self.IRAQI_TASK_OPEN_TAG)[0]
            content_after = body.split(self.IRAQI_TASK_CLOSE_TAG)[1] if self.IRAQI_TASK_CLOSE_TAG in body else ""
        ...
        try:
            async with httpx.AsyncClient() as client:
                await client.post(
                    ...
                    json={"labels": ["iraqi-ai", "cultural-validated", "arabic-processed"]}
                )
...
async def main():
    ...
    if task_plan:
        ...
        print(f"Cultural compliance: {task_plan.cultural_validation.compliance_score:.1%}")
