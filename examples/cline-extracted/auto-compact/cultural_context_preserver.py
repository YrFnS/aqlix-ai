...
class CulturalContextPreserver:
    ...
    async def extract_cultural_decisions(
        ...
        islamic_decisions = await self._extract_islamic_decisions(
            conversation_history, cultural_context
        )
        professional_decisions = await self._extract_professional_decisions(
            conversation_history, cultural_context
        )
        family_decisions = await self._extract_family_decisions(
            conversation_history, cultural_context
        )
        government_decisions = await self._extract_government_decisions(
            conversation_history, cultural_context
        )
        cultural_pattern_decisions = await self._extract_cultural_pattern_decisions(
            conversation_history, cultural_context
        )
        ...
        all_decisions = (
            ...
            + cultural_pattern_decisions
        )

        # Sort by preservation priority (critical first)
        prioritized_decisions = sorted(
            all_decisions, key=lambda d: d.preservation_priority, reverse=True
        )

        # Limit to configuration maximum
        preserved_decisions = prioritized_decisions[
            : self.config["cultural_decision_retention"]
        ]

        # Generate cultural compliance summary
        cultural_summary = await self._generate_cultural_summary(
            preserved_decisions, cultural_context
        )

        # Calculate preservation metrics
        preservation_metrics = await self._calculate_preservation_metrics(
            preserved_decisions, all_decisions
        )
    ...
    async def validate_preservation(
        ...
        cultural_validation = await self._validate_cultural_compliance_preservation(
            summary, original_context
        )

        # Validate Islamic context preservation
        islamic_validation = await self._validate_islamic_context_preservation(
            summary, original_context
        )

        # Validate professional context preservation
        professional_validation = (
            await self._validate_professional_context_preservation(
                summary, original_context
            )
        )

        # Validate family context preservation
        family_validation = await self._validate_family_context_preservation(
            summary, original_context
        )
    ...
    async def _extract_islamic_decisions(
        self,
        conversation_history: List[Dict[str, Any]],
        cultural_context: Dict[str, Any],
    ) -> List[CulturalDecision]:
        ...
        for i, message in enumerate(conversation_history):
            ...
            if await self._contains_islamic_content(content):
                decision = CulturalDecision(
                    ...
                    compliance_score=await self._calculate_islamic_compliance_score(
                        content
                    ),
    ...
    async def _extract_professional_decisions(
        self,
        conversation_history: List[Dict[str, Any]],
        cultural_context: Dict[str, Any],
    ) -> List[CulturalDecision]:
        ...
        for i, message in enumerate(conversation_history):
            ...
            if await self._contains_professional_content(content, professional_domain):
                decision = CulturalDecision(
                    ...
                    content=await self._extract_professional_context(
                        content, professional_domain
                    ),
                    validation_level=CulturalValidationLevel.HIGH,
                    compliance_score=await self._calculate_professional_compliance_score(
                        content, professional_domain
                    ),
                    cultural_context=cultural_context,
                    timestamp=datetime.now().isoformat(),
    ...
    async def _extract_family_decisions(
        self,
        conversation_history: List[Dict[str, Any]],
        cultural_context: Dict[str, Any],
    ) -> List[CulturalDecision]:
        ...
        for i, message in enumerate(conversation_history):
            ...
            if await self._contains_family_content(content):
                decision = CulturalDecision(
                    ...
                    validation_level=CulturalValidationLevel.HIGH,
                    compliance_score=await self._calculate_family_appropriateness_score(
                        content
                    ),
                    cultural_context=cultural_context,
                    timestamp=datetime.now().isoformat(),
    ...
    async def _extract_government_decisions(
        self,
        conversation_history: List[Dict[str, Any]],
        cultural_context: Dict[str, Any],
    ) -> List[CulturalDecision]:
        ...
        for i, message in enumerate(conversation_history):
            ...
            if await self._contains_government_content(content):
                decision = CulturalDecision(
                    ...
                    validation_level=CulturalValidationLevel.CRITICAL,
                    compliance_score=await self._calculate_government_compliance_score(
                        content
                    ),
                    cultural_context=cultural_context,
                    timestamp=datetime.now().isoformat(),
    ...
    async def _extract_cultural_pattern_decisions(
        self,
        conversation_history: List[Dict[str, Any]],
        cultural_context: Dict[str, Any],
    ) -> List[CulturalDecision]:
        ...
        for i, message in enumerate(conversation_history):
            ...
            if await self._contains_cultural_patterns(content):
                decision = CulturalDecision(
                    ...
                    compliance_score=await self._calculate_cultural_pattern_score(
                        content
                    ),
                    cultural_context=cultural_context,
                    timestamp=datetime.now().isoformat(),
    ...
    async def _contains_professional_content(
        self, content: str, professional_domain: str
    ) -> bool:
    ...
    async def _extract_islamic_context(self, content: str) -> str:
        ...
        islamic_sentences = [
            s.strip() for s in sentences if await self._contains_islamic_content(s)
        ]
        return ". ".join(islamic_sentences[:3])  # Keep top 3 sentences

    async def _extract_professional_context(
        self, content: str, professional_domain: str
    ) -> str:
        ...
        professional_sentences = [
            s.strip()
            for s in sentences
            if await self._contains_professional_content(s, professional_domain)
        ]
    ...
    async def _extract_family_context(self, content: str) -> str:
        ...
        sentences = content.split(".")
        family_sentences = [
            s.strip() for s in sentences if await self._contains_family_content(s)
        ]
    ...
    async def _extract_government_context(self, content: str) -> str:
        ...
        government_sentences = [
            s.strip() for s in sentences if await self._contains_government_content(s)
        ]
    ...
    async def _extract_cultural_patterns(self, content: str) -> str:
        ...
        sentences = content.split(".")
        cultural_sentences = [
            s.strip() for s in sentences if await self._contains_cultural_patterns(s)
        ]
    ...
    async def _calculate_professional_compliance_score(
        self, content: str, professional_domain: str
    ) -> float:
