...
class ProfessionalContextManager:
            ...
            "professional_decisions": [
                {
                    ...
                    "compliance_frameworks": [
                        cf.value for cf in decision.compliance_frameworks
                    ],
            ...
            "professional_workflows": [
                {
                    ...
                    "completion_status": len(wf.completed_stages)
                    / (len(wf.completed_stages) + len(wf.pending_stages))
                    if (wf.completed_stages or wf.pending_stages)
                    else 1.0,
    ...
    async def validate_preservation(
        ...
        decision_preservation = await self._validate_decision_preservation(
            summary, original_context
        )

        # Validate terminology preservation
        terminology_preservation = await self._validate_terminology_preservation(
            summary, original_context
        )

        # Validate compliance preservation
        compliance_preservation = await self._validate_compliance_preservation(
            summary, original_context
        )

        # Validate workflow preservation
        workflow_preservation = await self._validate_workflow_preservation(
            summary, original_context
        )
    ...
    async def _extract_professional_decisions_by_type(
        self,
        conversation_history: List[Dict[str, Any]],
        domain: IraqiProfessionalDomain,
    ) -> List[ProfessionalDecision]:
        ...
        for i, message in enumerate(conversation_history):
            ...
            if await self._contains_professional_content(content, domain):
                ...
                context_type = await self._identify_professional_context_type(
                    content, domain
                )
                ...
                compliance_frameworks = await self._identify_compliance_frameworks(
                    content, domain
                )

                # Analyze cultural considerations
                cultural_considerations = await self._analyze_cultural_considerations(
                    content, domain
                )
                ...
                preservation_priority = (
                    await self._calculate_professional_preservation_priority(
    ...
    async def _analyze_professional_workflows(
        self,
        conversation_history: List[Dict[str, Any]],
        domain: IraqiProfessionalDomain,
    ) -> List[ProfessionalWorkflowState]:
        ...
        workflow_keywords = {
            ...
            IraqiProfessionalDomain.ENGINEERING: [
                "design",
                "planning",
                "construction",
                "testing",
            ...
            IraqiProfessionalDomain.TECHNOLOGY: [
                "development",
                "testing",
                "deployment",
                "maintenance",
        ...
        domain_keywords = workflow_keywords.get(
            domain, ["process", "step", "stage", "phase"]
        )
            ...
            if any(keyword in content for keyword in domain_keywords):
                ...
                if workflow_context:
                    workflow = ProfessionalWorkflowState(
                        ...
                        domain=domain,
                        current_stage=workflow_context.get(
                        ...
                        preservation_requirements=workflow_context.get(
    ...
    async def _extract_professional_terminology(
        self,
        conversation_history: List[Dict[str, Any]],
        domain: IraqiProfessionalDomain,
    ) -> Dict[str, List[str]]:
        ...
        terminology = {"arabic": [], "english": [], "technical": [], "cultural": []}
        ...
        for key in terminology:
            terminology[key] = list(set(terminology[key]))[
                :20
            ]  # Limit to 20 terms per category
    ...
    async def _contains_professional_content(
        self, content: str, domain: IraqiProfessionalDomain
    ) -> bool:
        ...
        domain_keywords = {
            ...
            IraqiProfessionalDomain.AGRICULTURE: [
                "agriculture",
                "farming",
                "crop",
                "irrigation",
            ...
            IraqiProfessionalDomain.OIL_GAS: [
                "oil",
                "gas",
                "petroleum",
                "energy",
    ...
    async def _identify_professional_context_type(
        self, content: str, domain: IraqiProfessionalDomain
        ...
        if any(
            word in content_lower for word in ["decision", "decide", "choose", "select"]
        ):
            return ProfessionalContextType.DECISION_MAKING
        elif any(
            word in content_lower
            for word in ["workflow", "process", "procedure", "step"]
        ):
            return ProfessionalContextType.WORKFLOW
        elif any(
            word in content_lower
            for word in ["compliance", "regulation", "standard", "requirement"]
        ):
            return ProfessionalContextType.COMPLIANCE
        elif any(
            word in content_lower
            for word in ["hierarchy", "authority", "management", "supervisor"]
        ):
            return ProfessionalContextType.HIERARCHY
        elif any(
            word in content_lower
            for word in ["communication", "meeting", "discussion", "presentation"]
        ):
            return ProfessionalContextType.COMMUNICATION
        elif any(
            word in content_lower
            for word in ["terminology", "term", "definition", "vocabulary"]
        ):
