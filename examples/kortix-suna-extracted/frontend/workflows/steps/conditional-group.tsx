...
export function ConditionalGroup({
  ...
  const [deleteConfirmStep, setDeleteConfirmStep] =
    useState<ConditionalStep | null>(null);
  ...
  const elseIfCount = conditionSteps.filter(
    (step) => step.conditions?.type === "elseif",
  ).length;
  ...
  React.useEffect(() => {
    if (!displaySteps.find((s) => s.id === activeConditionTab)) {
      ...
      if (activeConditionTab === "virtual-else" && hasElse) {
        const elseStep = conditionSteps.find(
          (step) => step.conditions?.type === "else",
        );
        if (elseStep) {
  ...
  const activeStep =
    displaySteps.find((s) => s.id === activeConditionTab) || displaySteps[0];
  ...
  const handleDragEnd = React.useCallback(
    (event: DragEndEvent) => {
      ...
      if (over && active.id !== over.id && activeStep?.children) {
        ...
        const oldIndex = activeStep.children.findIndex(
          (child) => child.id === activeId,
        );
        const newIndex = activeStep.children.findIndex(
  ...
  const handleDeleteConfirm = React.useCallback(() => {
    if (deleteConfirmStep) {
      ...
      if (activeConditionTab === deleteConfirmStep.id) {
        const remainingSteps = conditionSteps.filter(
          (s) => s.id !== deleteConfirmStep.id,
        );
          ...
          {canAddElseIf && (
            <Button
              onClick={() => {
                ...
                if (hasElse) {
                  const elseStepIndex = conditionSteps.findIndex(
                    (step) => step.conditions?.type === "else",
                  );
                  const beforeElseStep =
                    elseStepIndex > 0
                      ? conditionSteps[elseStepIndex - 1]
                      : conditionSteps[0];
          ...
          <div className="bg-white dark:bg-zinc-800 rounded-2xl p-4 border border-zinc-200 dark:border-zinc-700">
            ...
            <div className="space-y-3">
              {activeStep.children && activeStep.children.length > 0 ? (
                ...
                >
                  <SortableContext
                    items={activeStep.children.map(
                      (child) => `${groupKey}-${child.id}`,
                    )}
                    ...
                    <div className="space-y-2">
                      {activeStep.children.map((child, index) => {
                        ...
                        if (child.type === "condition") {
                          ...
                          const conditionalGroup = activeStep.children?.filter(
                            (sibling) =>
                              sibling.parentConditionalId === child.id ||
                              sibling.id === child.id,
                          ) || [child];
                          ...
                          const sortedGroup = conditionalGroup.sort((a, b) => {
                            ...
                            return (
                              getTypeOrder(a.conditions?.type || "if") -
                              getTypeOrder(b.conditions?.type || "if")
                            );
                          });
                          ...
                          const handleNestedAddElseIf = (
                            ...
                            const elseIndex = activeStep.children?.findIndex(
                              (c) =>
                                c.parentConditionalId === child.id &&
                                c.conditions?.type === "else",
                            );
                    ...
                    {activeDragId ? (
                      <div className="opacity-75">
                        {(() => {
                          ...
                          const childId = activeDragId.replace(
                            `${groupKey}-`,
                            "",
                          );
                          const draggedChild = activeStep.children?.find(
                          ...
                          if (draggedChild.type === "condition") {
                            ...
                            const conditionalGroup =
                              activeStep.children?.filter(
                                (sibling) =>
                                  sibling.parentConditionalId ===
                                    draggedChild.id ||
                                  sibling.id === draggedChild.id,
                              ) || [draggedChild];

                            // Sort by type to ensure proper if/else-if/else sequence
                            const sortedGroup = conditionalGroup.sort(
                              (a, b) => {
                                const getTypeOrder = (type: string) => {
                                  if (type === "if") return 0;
                                  if (type === "elseif") return 1;
                            ...
                            const conditionalSteps = sortedGroup.map(
                              (step) => ({
                                ...step,
                                conditions: step.conditions || {
                                  type: "if" as const,
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete this "Else If" condition? This
              action cannot be undone and will remove all steps within this
              condition.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
