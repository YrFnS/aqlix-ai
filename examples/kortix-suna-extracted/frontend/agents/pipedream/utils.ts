...
export const createConnectedAppsFromProfiles = (
  ...
  const profilesByApp = connectedProfiles.reduce(
    (acc, profile) => {
  ...
  return Object.entries(profilesByApp).map(([appSlug, profiles]) => {
    ...
    const registryApp = allApps.find(
      (app) =>
...
export const getAgentPipedreamProfiles = (
  ...
  const pipedreamMcps = customMcps.filter(
    (mcp: any) =>
      mcp.config?.profile_id && mcp.config?.url?.includes("pipedream"),
  );

  const profileIds = pipedreamMcps
    .map((mcp: any) => mcp.config?.profile_id)
    .filter(Boolean);
  ...
  return usedProfiles.map((profile) => {
    const mcpConfig = pipedreamMcps.find(
      (mcp: any) => mcp.config?.profile_id === profile.profile_id,
    );
    const enabledTools =
      mcpConfig?.enabledTools || mcpConfig?.enabled_tools || [];
