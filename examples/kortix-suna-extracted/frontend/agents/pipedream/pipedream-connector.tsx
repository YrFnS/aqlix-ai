...
export const PipedreamConnector: React.FC<PipedreamConnectorProps> = ({
  ...
  const [connectionSuccessProfileId, setConnectionSuccessProfileId] = useState<
    string | null
  >(null);
  ...
  const availableProfiles = useMemo(() => {
    return connectedProfiles.filter(
      (p) => !existingProfileIds.includes(p.profile_id),
    );
  ...
  const proceedToTools = useCallback(async () => {
    ...
    try {
      const servers = await pipedreamApi.discoverMCPServers(
        selectedProfile.external_user_id,
        app.name_slug,
      );
      ...
      if (server?.available_tools) {
        ...
        setSelectedTools(
          new Set(server.available_tools.map((tool) => tool.name)),
        );
      }
    } catch (error) {
                  ...
                  <span className="text-sm font-medium text-green-800">
                    {
                      connectedProfiles.find(
                        (p) => p.profile_id === connectionSuccessProfileId,
                      )?.profile_name
                    }{" "}
