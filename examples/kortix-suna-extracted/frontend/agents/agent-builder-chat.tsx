...
export const AgentBuilderChat = React.memo(
  function AgentBuilderChat({
    ...
    useEffect(() => {
      if (
        ...
        chatHistoryQuery.status === "success" &&
        !hasInitiallyLoadedRef.current
      ) {
        console.log(
          "[AgentBuilderChat] Loading chat history for agent:",
          agentId,
        );
        ...
        if (historyMessages && historyMessages.length > 0) {
          const unifiedMessages = historyMessages
            ...
            .map((msg: any) => ({
              message_id:
                msg.message_id || `msg-${Date.now()}-${Math.random()}`,
        ...
        console.error(
          "[AgentBuilderChat] Error loading chat history:",
    ...
    useEffect(() => {
      if (threadId && agentRunsQuery.data && !agentRunsCheckedRef.current) {
        ...
        if (activeRun) {
          console.log(
            "[AgentBuilderChat] Found active run on load:",
            activeRun.id,
          );
        ...
        setMessages((prev) => {
          ...
          if (message.type === "user") {
            ...
            if (optimisticIndex !== -1) {
              console.log(
                `[AGENT BUILDER] Replacing optimistic message with real message`,
              );
        ...
        switch (status) {
          ...
          case "failed":
            ...
            if (status === "completed") {
              ...
              queryClient.invalidateQueries({
                queryKey: agentKeys.builderChatHistory(agentId),
              });
    ...
    useEffect(() => {
      if (agentRunId && agentRunId !== currentHookRunId) {
        console.log(
          `[AgentBuilderChat] Target agentRunId set to ${agentRunId}, initiating stream...`,
        );
    ...
    const handleSubmitFirstMessage = async (
      ...
      try {
        ...
        if (options?.model_name)
          agentFormData.append("model_name", options.model_name);
        ...
        if (result.thread_id) {
          ...
          if (result.agent_run_id) {
            console.log(
              "[AGENT BUILDER] Setting agent run ID:",
              result.agent_run_id,
            );
      ...
      async (
        ...
        try {
          ...
          if (results[1].status === "rejected") {
            ...
            throw new Error(
              `Failed to start agent: ${error?.message || error}`,
            );
          }
              ...
              emptyStateComponent={
                <div className="mt-6 flex flex-col items-center text-center text-muted-foreground/80">
                  ...
                  <p className="w-[60%] text-2xl">
                    ...
                    . Describe the exact workflows and tasks you want to
                    automate, and I'll configure your agent to handle them.
                  </p>
                </div>
              }
            />
