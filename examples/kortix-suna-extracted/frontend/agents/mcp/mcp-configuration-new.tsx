...
import {
  ...
  MCPConfiguration as MCPConfigurationType,
} from "./types";
...
export const MCPConfigurationNew: React.FC<MCPConfigurationProps> = ({
  ...
  const [showPipedreamToolsManager, setShowPipedreamToolsManager] =
    useState(false);
  const [showCustomToolsManager, setShowCustomToolsManager] = useState(false);
  const [selectedMCPForTools, setSelectedMCPForTools] =
    useState<MCPConfigurationType | null>(null);
  ...
  const handleToolsSelected = async (
    profileId: string,
    ...
    if (selectedAgentId && saveMode === "direct") {
      try {
        const response = await fetch(
          `/api/agents/${selectedAgentId}/pipedream-tools/${profileId}`,
          {
            method: "PUT",
            headers: {
    ...
    const nonPipedreamMCPs = configuredMCPs.filter(
      (mcp) =>
        mcp.customType !== "pipedream" || mcp.selectedProfileId !== profileId,
    );
  ...
  return (
    <div className="h-full flex flex-col">
        ...
        {configuredMCPs.length === 0 && (
          <div className="text-center py-12 px-6 bg-muted/30 rounded-xl border-2 border-dashed border-border">
            ...
            <p className="text-sm text-muted-foreground mb-6 max-w-sm mx-auto">
              Browse the app registry to connect your apps through Pipedream or
      ...
      {selectedMCPForTools &&
        ...
        (selectedMCPForTools.selectedProfileId ||
          ...
          <ToolsManager
            ...
            initialEnabledTools={(() => {
              console.log(
                "[MCPConfiguration] Rendering Pipedream ToolsManager with:",
                {
                  selectedMCPForTools,
      ...
      {selectedMCPForTools &&
        selectedMCPForTools.customType !== "pipedream" && (
          <ToolsManager
            ...
            initialEnabledTools={(() => {
              console.log(
                "[MCPConfiguration] Rendering Custom ToolsManager with:",
                {
                  selectedMCPForTools,
                  enabledTools: selectedMCPForTools.enabledTools,
