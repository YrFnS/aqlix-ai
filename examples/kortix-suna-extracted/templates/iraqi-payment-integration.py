...
class ZainCashGateway:
    ...
    async def create_payment(self, request: IraqiPaymentRequest) -> IraqiPaymentResponse:
    ...
    async def verify_payment(self, payment_id: str) -> IraqiPaymentResponse:
        ...
        async with self.session.get(
            ...
            return IraqiPaymentResponse(
                ...
                transaction_id=result.get("transactionId"),
                gateway_response=result,
                created_at=datetime.fromisoformat(result.get("createdAt", datetime.now().isoformat())),
                updated_at=datetime.now()
            )
...
class IraqiPaymentService:
    ...
    def __init__(self, 
        ...
        if nasswallet_config:
            self.gateways[PaymentGateway.NASSWALLET] = NassWalletGateway(nasswallet_config)
    ...
    async def verify_payment(self, payment_id: str, gateway: PaymentGateway) -> IraqiPaymentResponse:
