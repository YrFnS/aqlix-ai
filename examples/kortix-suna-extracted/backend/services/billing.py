...
def is_plan_change_allowed(
    current_price_id: str, new_price_id: str
) -> tuple[bool, str]:
    """
    ...
    if (
        current_plan["type"] == "monthly"
        and new_plan["type"] == "monthly"
        and new_plan["tier"] < current_plan["tier"]
    ):
        return (
            False,
    ...
    if (
        current_plan["type"] == "yearly_commitment"
        and new_plan["type"] == "yearly_commitment"
        and new_plan["tier"] < current_plan["tier"]
    ):
        return (
            False,
...
def get_model_pricing(model: str) -> tuple[float, float] | None:
    ...
    if model in HARDCODED_MODEL_PRICES:
        ...
        return pricing["input_cost_per_million_tokens"], pricing[
...
SUBSCRIPTION_TIERS = {
    ...
    config.STRIPE_TIER_2_20_YEARLY_ID: {
        ...
        "minutes": 120,
        "cost": 20 + 5,
    },  # 2 hours/month, $204/year
    config.STRIPE_TIER_6_50_YEARLY_ID: {
        "name": "tier_6_50",
        "minutes": 360,
        "cost": 50 + 5,
    },  # 6 hours/month, $510/year
    config.STRIPE_TIER_12_100_YEARLY_ID: {
        "name": "tier_12_100",
        "minutes": 720,
        "cost": 100 + 5,
    },  # 12 hours/month, $1020/year
    config.STRIPE_TIER_25_200_YEARLY_ID: {
        "name": "tier_25_200",
        "minutes": 1500,
        "cost": 200 + 5,
    },  # 25 hours/month, $2040/year
    config.STRIPE_TIER_50_400_YEARLY_ID: {
        "name": "tier_50_400",
        "minutes": 3000,
        "cost": 400 + 5,
    },  # 50 hours/month, $4080/year
    config.STRIPE_TIER_125_800_YEARLY_ID: {
        "name": "tier_125_800",
        "minutes": 7500,
        "cost": 800 + 5,
    },  # 125 hours/month, $8160/year
    config.STRIPE_TIER_200_1000_YEARLY_ID: {
        "name": "tier_200_1000",
        "minutes": 12000,
        "cost": 1000 + 5,
    },  # 200 hours/month, $10200/year
    # Yearly commitment tiers (15% discount, monthly payments with 12-month commitment via schedules)
    config.STRIPE_TIER_2_17_YEARLY_COMMITMENT_ID: {
        "name": "tier_2_17_yearly_commitment",
        "minutes": 120,
        "cost": 20 + 5,
    },  # 2 hours/month, $17/month (12-month commitment)
    config.STRIPE_TIER_6_42_YEARLY_COMMITMENT_ID: {
        "name": "tier_6_42_yearly_commitment",
        "minutes": 360,
        "cost": 50 + 5,
    },  # 6 hours/month, $42.50/month (12-month commitment)
    config.STRIPE_TIER_25_170_YEARLY_COMMITMENT_ID: {
        "name": "tier_25_170_yearly_commitment",
        "minutes": 1500,
        "cost": 200 + 5,
    },  # 25 hours/month, $170/month (12-month commitment)
}
...
class CreateCheckoutSessionRequest(BaseModel):
    ...
    commitment_type: Optional[str] = (
        "monthly"  # "monthly", "yearly", or "yearly_commitment"
    )
...
async def get_user_subscription(user_id: str) -> Optional[Dict]:
    ...
    try:
        ...
        if len(our_subscriptions) > 1:
            logger.warning(
                f"User {user_id} has multiple active subscriptions: {[sub['id'] for sub in our_subscriptions]}"
            )
            ...
            for sub in our_subscriptions:
                if sub["id"] != most_recent["id"]:
                    try:
                        ...
                        logger.info(
                            f"Cancelled subscription {sub['id']} for user {user_id}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Error cancelling subscription {sub['id']}: {str(e)}"
                        )
...
async def calculate_monthly_usage(client, user_id: str) -> float:
    ...
    logger.info(
        f"Calculate monthly usage took {execution_time:.3f} seconds, total cost: {total_cost}"
    )
...
async def get_usage_logs(
    client, user_id: str, page: int = 0, items_per_page: int = 1000
) -> Dict:
    ...
    for message in messages_result.data:
        try:
            ...
            if (
                message.get("threads")
                and isinstance(message["threads"], list)
                and len(message["threads"]) > 0
            ):
            ...
            logger.warning(
                f"Error processing usage log entry for message {message.get('message_id', 'unknown')}: {str(e)}"
            )
            continue
    ...
    return {"logs": processed_logs, "has_more": has_more}
...
def calculate_token_cost(
    ...
    try:
        ...
        hardcoded_pricing = get_model_pricing(model) or get_model_pricing(
            resolved_model
        )
        if hardcoded_pricing:
            ...
            try:
                ...
                for model_name in models_to_try:
                    try:
                        prompt_token_cost, completion_token_cost = cost_per_token(
                            model_name, prompt_tokens, completion_tokens
                        )
                        if (
                            prompt_token_cost is not None
                            and completion_token_cost is not None
                        ):
                            message_cost = prompt_token_cost + completion_token_cost
                            break
                    except Exception as e:
                        logger.debug(
                            f"Failed to get pricing for model variation {model_name}: {str(e)}"
                        )
                        continue

                if message_cost is None:
                    logger.warning(
                        f"Could not get pricing for model {model} (resolved: {resolved_model}), returning 0 cost"
                    )
                    return 0.0

            except Exception as e:
                logger.warning(
                    f"Could not get pricing for model {model} (resolved: {resolved_model}): {str(e)}, returning 0 cost"
                )
                return 0.0
...
async def get_allowed_models_for_user(client, user_id: str):
    ...
    return MODEL_ACCESS_TIERS.get(
        tier_name, MODEL_ACCESS_TIERS["free"]
...
async def can_use_model(client, user_id: str, model_name: str):
    if config.ENV_MODE == EnvMode.LOCAL:
        ...
        return (
            True,
            "Local development mode - billing disabled",
            {
                "price_id": "local_dev",
    ...
    return (
        False,
        f"Your current subscription plan does not include access to {model_name}. Please upgrade your subscription or choose from your available models: {', '.join(allowed_models)}",
        allowed_models,
    )
...
async def check_billing_status(
    ...
    if not tier_info:
        logger.warning(
            f"Unknown subscription tier: {price_id}, defaulting to free tier"
        )
    ...
    if current_usage >= tier_info["cost"]:
        return (
            False,
            f"Monthly limit of {tier_info['cost']} dollars reached. Please upgrade your plan or wait until next month.",
            subscription,
        )
...
async def check_subscription_commitment(subscription_id: str) -> dict:
    ...
    try:
        ...
        is_yearly_commitment = (
            ...
            or price_id in yearly_commitment_price_ids
        )
        ...
        if is_yearly_commitment:
            ...
            if current_time < commitment_end_timestamp:
                ...
                months_remaining = (
                    commitment_end_date.year - current_date.year
                ) * 12 + (commitment_end_date.month - current_date.month)
                if current_date.day > commitment_end_date.day:
                    months_remaining -= 1
                months_remaining = max(0, months_remaining)

                logger.info(
                    f"Subscription {subscription_id} has active yearly commitment: {months_remaining} months remaining"
                )
                ...
                logger.info(
                    f"Subscription {subscription_id} yearly commitment period has ended"
                )
                return {
        ...
        # No commitment
        return {"has_commitment": False, "can_cancel": True, "price_id": price_id}

    except Exception as e:
        logger.error(f"Error checking subscription commitment: {str(e)}", exc_info=True)
        return {"has_commitment": False, "can_cancel": True}
    ...
    try:
        ...
        try:
            price = await stripe.Price.retrieve_async(
                request.price_id, expand=["product"]
            )
        ...
        if existing_subscription:
            ...
            try:
                ...
                if current_price_id == request.price_id:
                    return {
                        ...
                        "details": {
                            "is_upgrade": None,
                            "effective_date": None,
                            "current_price": round(price["unit_amount"] / 100, 2)
                            if price.get("unit_amount")
                            else 0,
                            "new_price": round(price["unit_amount"] / 100, 2)
                            if price.get("unit_amount")
                            else 0,
                        },
                    }
                ...
                is_allowed, restriction_reason = is_plan_change_allowed(
                    current_price_id, request.price_id
                )
                if not is_allowed:
                    raise HTTPException(
                        status_code=400,
                ...
                current_interval = current_price.get("recurring", {}).get(
                    "interval", "month"
                )
                new_interval = new_price.get("recurring", {}).get("interval", "month")

                is_upgrade = (
                    new_price["unit_amount"]
                    > current_price["unit_amount"]  # Traditional price upgrade
                    or (
                        current_interval == "month" and new_interval == "year"
                    )  # Monthly to yearly upgrade
                )

                logger.info(
                    f"Price comparison: current={current_price['unit_amount']}, new={new_price['unit_amount']}, "
                ...
                if commitment_info.get("has_commitment"):
                    if is_upgrade:
                        ...
                        logger.info(
                            f"Upgrading commitment subscription {subscription_id}"
                        )
                        ...
                        logger.info(
                            f"Updated customer {customer_id} active status to TRUE after subscription upgrade"
                        )
                        ...
                        if updated_subscription.latest_invoice:
                            ...
                            latest_invoice = await stripe.Invoice.retrieve_async(
                                latest_invoice_id
                            )

                            try:
                                logger.info(
                                    f"Latest invoice {latest_invoice_id} status: {latest_invoice.status}"
                                )
                                ...
                                if latest_invoice.status == "draft":
                                    ...
                                    logger.info(
                                        f"Finalized invoice {latest_invoice_id} for immediate payment"
                                    )
                                    ...
                                    if finalized_invoice.status == "open":
                                        ...
                                        logger.info(
                                            f"Paid invoice {latest_invoice_id} immediately, status: {paid_invoice.status}"
                                        )
                                    ...
                                    logger.info(
                                        f"Paid existing open invoice {latest_invoice_id}, status: {paid_invoice.status}"
                                    )
                                    latest_invoice = paid_invoice
                                else:
                                    logger.info(
                                        f"Invoice {latest_invoice_id} is in status {latest_invoice.status}, no action needed"
                                    )

                            except Exception as invoice_error:
                                logger.error(
                                    f"Error processing invoice for immediate payment: {str(invoice_error)}"
                                )
                        ...
                        return {
                            ...
                            "details": {
                                ...
                                "current_price": round(
                                    current_price["unit_amount"] / 100, 2
                                )
                                if current_price.get("unit_amount")
                                else 0,
                                "new_price": round(new_price["unit_amount"] / 100, 2)
                                if new_price.get("unit_amount")
                                else 0,
                                "invoice": {
                                    ...
                                    "amount_due": round(
                                        latest_invoice["amount_due"] / 100, 2
                                    )
                                    if latest_invoice
                                    else 0,
                                    "amount_paid": round(
                                        latest_invoice["amount_paid"] / 100, 2
                                    )
                                    if latest_invoice
                                    else 0,
                                }
                        ...
                        if not commitment_info.get("can_cancel"):
                            return {
                                ...
                                "details": {
                                    "is_upgrade": False,
                                    "effective_date": commitment_info.get(
                                        "commitment_end_date"
                                    ),
                                    "current_price": round(
                                        current_price["unit_amount"] / 100, 2
                                    ...
                                    "new_price": round(
                                        new_price["unit_amount"] / 100, 2
                                    )
                                    if new_price.get("unit_amount")
                                    else 0,
                                    "commitment_end_date": commitment_info.get(
                                        "commitment_end_date"
                                    ),
                                    "months_remaining": commitment_info.get(
                                        "months_remaining", 0
                                    ),
                                },
                            }
                ...
                if is_upgrade:
                    ...
                    logger.info(
                        f"Updated customer {customer_id} active status to TRUE after subscription downgrade"
                    )
                ...
                logger.exception(
                    f"Error updating subscription {existing_subscription.get('id') if existing_subscription else 'N/A'}: {str(e)}"
                )
                raise HTTPException(
            ...
            logger.info(
                f"Updated customer {customer_id} active status to TRUE after creating checkout session"
            )
        ...
        if hasattr(e, "json_body") and e.json_body and "error" in e.json_body:
            ...
            error_detail = str(e)
        raise HTTPException(
            status_code=500, detail=f"Error creating checkout session: {error_detail}"
        )
        ...
        try:
            ...
            for config in configurations.get("data", []):
                ...
                if subscription_update.get("enabled", False):
                    ...
                    logger.info(
                        f"Found existing portal configuration with subscription_update enabled: {config['id']}"
                    )
                    break
            ...
            if not active_config:
                ...
                if configurations.get("data", []):
                    ...
                    logger.info(
                        f"Updating default portal configuration: {default_config['id']} to enable subscription_update"
                    )

                    active_config = (
                        await stripe.billing_portal.Configuration.update_async(
                            default_config["id"],
                            features={
                                "subscription_update": {
                                    "enabled": True,
                                ...
                                "customer_update": default_config.get(
                                    "features", {}
                                ).get(
                                    "customer_update",
                                    {
                    ...
                    logger.info(
                        "Creating new portal configuration with subscription_update enabled"
                    )
                    active_config = (
            ...
            logger.info(
                f"Using portal configuration: {active_config['id']} with subscription_update: {active_config.get('features', {}).get('subscription_update', {}).get('enabled', False)}"
            )

        except Exception as config_error:
            logger.warning(
                f"Error configuring portal: {config_error}. Continuing with default configuration."
            )

        # Create portal session using the proper configuration if available
        portal_params = {"customer": customer_id, "return_url": request.return_url}
...
async def get_subscription(
    ...
    try:
        ...
        if schedule_id:
            try:
                ...
                if next_phase:
                    ...
                    scheduled_price_id = scheduled_item[
                        "price"
                    ]  # Price ID might be string here
                    ...
                    status_response.scheduled_plan_name = (
                        scheduled_tier_info.get("name", "unknown")
                        if scheduled_tier_info
                        else "unknown"
                    )
                    status_response.scheduled_price_id = scheduled_price_id
                    status_response.scheduled_change_date = datetime.fromtimestamp(
                        next_phase["start_date"], tz=timezone.utc
                    )

            except Exception as schedule_error:
                logger.error(
                    f"Error retrieving or parsing schedule {schedule_id} for sub {subscription['id']}: {schedule_error}"
                )
        ...
        logger.exception(
            f"Error getting subscription status for user {current_user_id}: {str(e)}"
        )  # Use logger.exception
        raise HTTPException(
            status_code=500, detail="Error retrieving subscription status."
        )


@router.get("/check-status")
async def check_status(current_user_id: str = Depends(get_current_user_id_from_jwt)):
    ...
    try:
        ...
        return {"can_run": can_run, "message": message, "subscription": subscription}
...
async def stripe_webhook(request: Request):
    ...
    try:
        ...
        if event.type in [
            ...
            if not customer_id:
                logger.warning(
                    f"No customer ID found in subscription event: {event.type}"
                )
            ...
            if event.type == "customer.subscription.created":
                ...
                if subscription.get("status") in ["active", "trialing"]:
                    ...
                    logger.info(
                        f"Webhook: Updated customer {customer_id} active status to TRUE based on {event.type}"
                    )
                ...
                if subscription.get("status") in ["active", "trialing"]:
                    ...
                    if not has_active:
                        ...
                        logger.info(
                            f"Webhook: Updated customer {customer_id} active status to FALSE based on {event.type}"
                        )
                ...
                if not has_active:
                    ...
                    logger.info(
                        f"Webhook: Updated customer {customer_id} active status to FALSE after subscription deletion"
                    )
    ...
    try:
        ...
        for short_name, full_name in MODEL_NAME_ALIASES.items():
            ...
            if (
                ...
                and not short_name.startswith("openai/")
                and not short_name.startswith("anthropic/")
                and not short_name.startswith("openrouter/")
                and not short_name.startswith("xai/")
            ):
        ...
        for model in all_models:
            display_name = model_aliases.get(
            ...
            if hardcoded_pricing:
                ...
                try:
                    ...
                    if (
                        ...
                        pricing_info = {
                            "input_cost_per_million_tokens": input_cost_per_token
                            * TOKEN_PRICE_MULTIPLIER,
                            "output_cost_per_million_tokens": output_cost_per_million
                            * TOKEN_PRICE_MULTIPLIER,
...
async def get_usage_logs_endpoint(
    ...
    try:
        ...
        if config.ENV_MODE == EnvMode.LOCAL:
            logger.info(
                "Running in local development mode - usage logs are not available"
            )
            return {
                "logs": [],
                "has_more": False,
        ...
        if items_per_page < 1 or items_per_page > 1000:
            raise HTTPException(
                status_code=400, detail="Items per page must be between 1 and 1000"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting usage logs: {str(e)}"
        )


@router.get("/subscription-commitment/{subscription_id}")
async def get_subscription_commitment(
    subscription_id: str, current_user_id: str = Depends(get_current_user_id_from_jwt)
):
    ...
    try:
        ...
        if not user_subscription or user_subscription.get("id") != subscription_id:
            raise HTTPException(
                status_code=404, detail="Subscription not found or access denied"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Error retrieving commitment information"
        )
...
async def get_subscription_details(
    ...
    try:
        ...
        return {"subscription": subscription_details, "commitment": commitment_info}
...
async def cancel_subscription(
    ...
    try:
        ...
        if commitment_info.get("has_commitment") and not commitment_info.get(
            "can_cancel"
        ):
            # Schedule cancellation at the end of the commitment period (1 year anniversary)
            commitment_end_date = datetime.fromisoformat(
                commitment_info.get("commitment_end_date").replace("Z", "+00:00")
            )
            ...
            logger.info(
                f"Subscription {subscription_id} scheduled for cancellation at commitment end: {commitment_end_date}"
            )
        ...
        if not current_period_end:
            logger.warning(
                f"No current_period_end found in cached data for subscription {subscription_id}, fetching fresh data from Stripe"
            )
            try:
                fresh_subscription = await stripe.Subscription.retrieve_async(
                    subscription_id
                )
        ...
        if not current_period_end:
            logger.error(
                f"No current_period_end found in subscription {subscription_id} even after fresh fetch"
            )
            raise HTTPException(
                status_code=500, detail="Unable to determine subscription period end"
            )
...
async def reactivate_subscription(
    ...
    try:
        ...
        is_cancelled = subscription.get("cancel_at_period_end") or subscription.get(
            "cancel_at"
        )
        if not is_cancelled:
            return {
        ...
        raise HTTPException(
            status_code=500, detail="Error processing reactivation request"
        )
