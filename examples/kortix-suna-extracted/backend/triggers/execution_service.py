...
class ExecutionService:
    ...
    async def execute_trigger_result(
        ...
        try:
            logger.info(
                f"Executing trigger for agent {agent_id}: workflow={trigger_result.should_execute_workflow}, agent={trigger_result.should_execute_agent}"
            )
...
class SessionManager:
    ...
    async def create_agent_session(
        self, agent_id: str, agent_config: Dict[str, Any], trigger_event: TriggerEvent
    ) -> Tuple[str, str]:
    ...
    async def create_workflow_session(
        self, account_id: str, workflow_id: str, workflow_name: str
    ) -> Tuple[str, str]:
        client = await self._db.client
        ...
        logger.info(
            f"Created workflow session: project={project_id}, thread={thread_id}"
        )
        return thread_id, project_id
...
class AgentExecutor:
    ...
    async def _get_agent_config(self, agent_id: str) -> Dict[str, Any]:
        try:
            ...
            active_version = await version_service.get_active_version(
                agent_id, "system"
            )
            ...
            return {
                ...
                if isinstance(active_version.agentpress_tools, dict)
                else {},
    ...
    async def _create_initial_message(
        self, thread_id: str, prompt: str, trigger_data: Dict[str, Any]
    ) -> None:
        client = await self._db.client
...
class WorkflowExecutor:
    ...
    async def execute_workflow(
        ...
        try:
            workflow_config, steps_json = await self._get_workflow_data(
            ...
            await self._create_workflow_message(
                thread_id, workflow_config, workflow_input
            )
...
{workflow_prompt}"""
    ...
    async def _start_workflow_agent_execution(
        self, thread_id: str, project_id: str, agent_config: Dict[str, Any]
    ) -> str:
        client = await self._db.client
        model_name = config.MODEL_TO_USE or "anthropic/claude-sonnet-4-20250514"
