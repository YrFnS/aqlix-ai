...
class TriggerProvider(ABC):
    ...
    async def process_event(
        self, trigger: Trigger, event: TriggerEvent
    ) -> TriggerResult:
        pass
...
class ScheduleProvider(TriggerProvider):
    def __init__(self):
        ...
        if not self._qstash_token:
            logger.warning(
                "QSTASH_TOKEN not found. Schedule provider will not work without it."
            )
            self._qstash = None
        else:
    ...
    async def validate_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        if not self._qstash:
            raise ValueError(
                "QSTASH_TOKEN environment variable is required for scheduled triggers"
            )
    ...
    async def setup_trigger(self, trigger: Trigger) -> bool:
        ...
        try:
            webhook_url = (
                f"{self._webhook_base_url}/api/triggers/{trigger.trigger_id}/webhook"
            )
            ...
            if user_timezone != "UTC":
                cron_expression = self._convert_cron_to_utc(
                    cron_expression, user_timezone
                )
            ...
            logger.info(
                f"Created QStash schedule {schedule_id} for trigger {trigger.trigger_id}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to setup QStash schedule for trigger {trigger.trigger_id}: {e}"
            )
            return False
        ...
        try:
            ...
            if schedule_id:
                try:
                    ...
                    logger.info(
                        f"Deleted QStash schedule {schedule_id} for trigger {trigger.trigger_id}"
                    )
                    return True
                except Exception as e:
                    logger.warning(
                        f"Failed to delete QStash schedule {schedule_id}: {e}"
                    )
            ...
            for schedule in schedules:
                if schedule.get("destination") == webhook_url:
                    await asyncio.to_thread(
                        self._qstash.schedule.delete, schedule["scheduleId"]
                    )
                    logger.info(
                        f"Deleted QStash schedule {schedule['scheduleId']} for trigger {trigger.trigger_id}"
                    )
                    return True
            ...
            logger.error(
                f"Failed to teardown QStash schedule for trigger {trigger.trigger_id}: {e}"
            )
            return False
        ...
        try:
            ...
            try:
                ...
                if hour.isdigit() and minute.isdigit():
                    user_time = user_tz.localize(
                        datetime(now.year, now.month, now.day, int(hour), int(minute))
                    )
...
class WebhookProvider(TriggerProvider):
    ...
    async def process_event(
        ...
        try:
            ...
            return TriggerResult(
                success=False, error_message=f"Error processing webhook event: {str(e)}"
            )
...
class ProviderService:
    ...
    async def process_event(
        ...
        if not provider:
            return TriggerResult(
                success=False, error_message=f"Unknown provider: {trigger.provider_id}"
            )
