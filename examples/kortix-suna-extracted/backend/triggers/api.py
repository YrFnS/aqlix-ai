    ...
    try:
        ...
        for trigger in schedule_triggers:
            ...
            try:
                ...
                human_readable = get_human_readable_schedule(
                ...
                logger.warning(
                    f"Error calculating next run for trigger {trigger.trigger_id}: {e}"
                )
                continue
        ...
        return UpcomingRunsResponse(
            upcoming_runs=upcoming_runs, total_count=len(upcoming_runs)
        )
...
async def get_trigger(
    trigger_id: str, user_id: str = Depends(get_current_user_id_from_jwt)
):
    """Get a trigger by ID"""
    if not await is_enabled("agent_triggers"):
        raise HTTPException(status_code=403, detail="Agent triggers are not enabled")
...
async def trigger_webhook(trigger_id: str, request: Request):
    ...
    try:
        ...
        if result.should_execute_agent or result.should_execute_workflow:
            ...
            if trigger:
                logger.info(
                    f"Executing agent {trigger.agent_id} for trigger {trigger_id}"
                )
...
async def execute_agent_workflow(
    ...
    execution_service = get_execution_service(db)
    execution_result = await execution_service.execute_trigger_result(
        agent_id=agent_id, trigger_result=trigger_result, trigger_event=trigger_event
    )
