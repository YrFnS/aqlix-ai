...
class TriggerService:
    ...
    async def create_trigger(
        ...
        validated_config = await provider_service.validate_trigger_config(
            provider_id, config
        )
    ...
    async def update_trigger(
        ...
        if config is not None:
            ...
            provider_service = get_provider_service(self._db)
            config = await provider_service.validate_trigger_config(
    ...
    async def process_trigger_event(
        self, trigger_id: str, raw_data: Dict[str, Any]
    ) -> TriggerResult:
        trigger = await self.get_trigger(trigger_id)
        if not trigger:
            return TriggerResult(
                success=False, error_message=f"Trigger not found: {trigger_id}"
            )

        if not trigger.is_active:
            return TriggerResult(
                success=False, error_message=f"Trigger is inactive: {trigger_id}"
            )
    ...
    async def _log_trigger_event(
        self, event: TriggerEvent, result: TriggerResult
    ) -> None:
        client = await self._db.client

        await (
            client.table("trigger_event_logs")
            .insert(
                {
