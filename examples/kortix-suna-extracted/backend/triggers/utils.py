...
class WorkflowParser:
    ...
    def _parse_conditional_group(
        self, conditional_group: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        parsed_step = {"step": step_name, "step_number": self.step_counter}
    ...
    def _parse_condition_step(self, step: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if condition_type == "if":
            ...
            parsed_condition["condition"] = (
                f"else if {expression}" if expression else "else if true"
            )
    ...
    def get_workflow_summary(self, steps: List[Dict[str, Any]]) -> Dict[str, Any]:
        def count_steps_recursive(steps_list):
            ...
            for step in steps_list:
                ...
                if "conditions" in step:
                    ...
                    for condition in step["conditions"]:
                        if "then" in condition:
                            sub_count, sub_conditions, sub_depth = (
                                count_steps_recursive(condition["then"])
                            )
                            count += sub_count
                ...
                if "then" in step:
                    sub_count, sub_conditions, sub_depth = count_steps_recursive(
                        step["then"]
                    )
                    count += sub_count
                    conditions_count += sub_conditions
...
def format_workflow_for_llm(
    ...
    tools_list = (
        ...
        if available_tools
        else "Use any available tools from your system prompt"
    )
