...
class AgentVersion:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            if self.is_active
            else VersionStatus.INACTIVE.value,
...
class VersionService:
    ...
    async def _verify_agent_access(
        ...
        is_public = bool(
            public_result.data and public_result.data[0].get("is_public", False)
        )
    ...
    async def _update_agent_current_version(
        self, agent_id: str, version_id: str, version_count: int
    ):
        client = await self._get_client()

        data = {"current_version_id": version_id, "version_count": version_count}

        result = (
            await client.table("agents").update(data).eq("agent_id", agent_id).execute()
        )
    ...
    async def create_version(
        ...
        if not is_owner:
            raise UnauthorizedError(
                "You don't have permission to create versions for this agent"
            )
        ...
        if current_active_result.data:
            ...
            await (
                ...
                .eq("version_id", previous_version_id)
                .execute()
            )
        ...
        await self._update_agent_current_version(
    ...
    async def get_active_version(
        self, agent_id: str, user_id: str = "system"
    ...
    async def compare_versions(
        self, agent_id: str, version1_id: str, version2_id: str, user_id: str
    ) -> Dict[str, Any]:
    ...
    def _calculate_differences(
        self, v1: AgentVersion, v2: AgentVersion
    ) -> List[Dict[str, Any]]:
        differences = []
