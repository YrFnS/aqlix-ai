...
def extract_agent_config(
    agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    ...
    if version_data:
        logger.info(
            f"Using active version data for agent {agent_id} (version: {version_data.get('version_name', 'unknown')})"
        )
    ...
    if agent_data.get("config"):
        ...
        config["agentpress_tools"] = _extract_agentpress_tools_for_run(
    ...
    logger.warning(
        f"No config found for agent {agent_id}, creating default configuration"
    )
...
def extract_tools_for_agent_run(config: Dict[str, Any]) -> Dict[str, Any]:
    logger.warning(
        "extract_tools_for_agent_run is deprecated, using config-based extraction"
    )
    tools = config.get("tools", {})
