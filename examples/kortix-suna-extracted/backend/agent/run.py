...
class ToolManager:
    ...
    def register_agent_builder_tools(self, agent_id: str):
        ...
        from agent.tools.agent_builder_tools.credential_profile_tool import (
    ...
    async def register_mcp_tools(self, agent_config: dict) -> Optional[MCPToolWrapper]:
        ...
        if agent_config.get("custom_mcps"):
            for custom_mcp in agent_config["custom_mcps"]:
                custom_type = custom_mcp.get(
                    "customType", custom_mcp.get("type", "sse")
                )
                        ...
                        if profile_id:
                            try:
                                ...
                                profile = await profile_service.get_profile(
                                    UUID(self.account_id), UUID(profile_id)
                                )
                                if profile:
                                    custom_mcp["config"]["external_user_id"] = (
                                        profile.external_user_id
                                    )
                            except Exception as e:
                                logger.error(
                                    f"Error retrieving external_user_id from profile {profile_id}: {e}"
                                )

                    if (
                        "headers" in custom_mcp["config"]
                        and "x-pd-app-slug" in custom_mcp["config"]["headers"]
                    ):
                        custom_mcp["config"]["app_slug"] = custom_mcp["config"][
                            "headers"
                        ]["x-pd-app-slug"]
        ...
        if await is_enabled("knowledge_base"):
            try:
                ...
                kb_result = await kb_client.rpc(
                    "get_combined_knowledge_base_context",
                    {
                ...
                logger.error(
                    f"Error retrieving knowledge base context for thread {thread_id}: {e}"
                )
...
class AgentRunner:
    ...
    async def setup_tools(self):
        tool_manager = ToolManager(
            self.thread_manager, self.config.project_id, self.config.thread_id
        )

        if self.config.agent_config and self.config.agent_config.get(
        ...
        if self.config.agent_config and "agentpress_tools" in self.config.agent_config:
            ...
            if isinstance(raw_tools, dict):
                if (
                    self.config.agent_config.get("is_suna_default", False)
                    and not raw_tools
                ):
                    enabled_tools = None
                else:
    ...
    async def run(self) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        while continue_execution and iteration_count < self.config.max_iterations:
            ...
            generation = (
                self.config.trace.generation(name="thread_manager.run_thread")
                if self.config.trace
                else None
            )
            try:
                ...
                try:
                    ...
                    if agent_should_terminate or last_tool_call in [
                        ...
                        if generation:
                            generation.end(
                                output=full_response, status_message="agent_stopped"
                            )
                        continue_execution = False
