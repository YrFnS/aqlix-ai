...
class SunaAgentRepository:
    ...
    async def find_suna_agents_needing_sync(
        self, target_version_tag: str
    ) -> List[SunaAgentRecord]:
        agents = await self.find_all_suna_agents()
        return [
            agent for agent in agents if agent.current_version_tag != target_version_tag
        ]

    async def update_agent_record(
        self, agent_id: str, config_data: Dict[str, Any], unified_config: Dict[str, Any]
    ) -> bool:
        try:
            client = await self.db.client
            ...
            current_agent_result = (
                await client.table("agents")
                .select("configured_mcps, custom_mcps, metadata")
            ...
            preserved_configured_mcps = config_data.get(
                "configured_mcps", current_agent.get("configured_mcps", [])
            )
            preserved_custom_mcps = config_data.get(
            ...
            for i, mcp in enumerate(preserved_custom_mcps):
                ...
                logger.info(
                    f"Agent {agent_id} - Preserving custom MCP {i + 1} ({mcp.get('name', 'Unknown')}) with {tools_count} enabled tools"
                )
            ...
            update_data = {
                ...
                "metadata": {**current_metadata, **config_data["metadata"]},
            }
            ...
            logger.info(
                f"Surgically updated agent {agent_id} - preserved MCPs and customizations"
            )
    ...
    async def update_agent_version_pointer(
    ...
    async def get_agent_stats(self) -> Dict[str, Any]:
        try:
            ...
            return {
                ...
                "last_updated": max(
                    [a.last_sync_date for a in agents], default="unknown"
                ),
            }
    ...
    async def update_agent_metadata(self, agent_id: str, version_tag: str) -> bool:
        try:
            client = await self.db.client
    ...
    async def _create_initial_version(
        ...
        try:
            ...
            logger.error(
                f"Failed to create initial version for Suna agent {agent_id}: {e}"
            )
