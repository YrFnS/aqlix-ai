...
class SandboxWebSearchTool(SandboxToolsBase):
    ...
    async def web_search(self, query: str, num_results: int = 20) -> ToolResult:
        ...
        try:
            ...
            logging.info(
                f"Retrieved search results for query: '{query}' with answer and {len(results)} results"
            )

            # Consider search successful if we have either results OR an answer
            if len(results) > 0 or (answer and answer.strip()):
                return ToolResult(
                    success=True, output=json.dumps(search_response, ensure_ascii=False)
                )
            else:
                # No results or answer found
                logging.warning(
                    f"No search results or answer found for query: '{query}'"
                )
                return ToolResult(
                    success=False,
    ...
    async def scrape_webpage(self, urls: str) -> ToolResult:
        ...
        try:
            ...
            if len(url_list) == 1:
                logging.warning(
                    "Only a single URL provided - for efficiency you should scrape multiple URLs at once"
                )
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    processed_results.append(
                        {"url": url_list[i], "success": False, "error": str(result)}
                    )
                else:
            ...
            if successful == len(results):
                message = (
                    f"Successfully scraped all {len(results)} URLs. Results saved to:"
                )
                for r in results:
                    if r.get("file_path"):
                        message += f"\n- {r.get('file_path')}"
                ...
                error_details = "; ".join(
                    [
                        f"{r.get('url')}: {r.get('error', 'Unknown error')}"
                        for r in results
                    ]
                )
                return self.fail_response(
                    f"Failed to scrape all {len(results)} URLs. Errors: {error_details}"
                )
            ...
            return self.fail_response(
                f"Error processing scrape request: {error_message[:200]}"
            )
    ...
    async def _scrape_single_url(self, url: str) -> dict:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                while retry_count < max_retries:
                    try:
                        ...
                        data = response.json()
                        logging.info(
                            f"Successfully received response from Firecrawl for {url}"
                        )
                        break
                    except (
                        httpx.ReadTimeout,
            ...
            logging.info(
                f"Extracted content from {url}: title='{title}', content length={len(markdown_content)}"
            )
            ...
            logging.info(
                f"Saving content to file: {results_file_path}, size: {len(json_content)} bytes"
            )
