...
class MCPSearchTool(AgentBuilderBaseTool):
    ...
    async def search_mcp_servers(
        ...
        try:
            search_result = await app_service.search_apps(
                query=query, category=category, page=1, limit=limit
            )
            ...
            for app in apps:
                formatted_apps.append(
                    {
                        "name": app.name,
                        "app_slug": app.slug.value
                        if hasattr(app.slug, "value")
                        else str(app.slug),
            ...
            if not formatted_apps:
                return ToolResult(
                    success=False, output=json.dumps([], ensure_ascii=False)
                )

            return ToolResult(
                success=True, output=json.dumps(formatted_apps, ensure_ascii=False)
            )
        ...
        try:
            ...
            if not app_data:
                return self.fail_response(
                    f"Could not find app details for '{app_slug}'"
                )

            formatted_app = {
                "name": app_data.name,
                "app_slug": app_data.slug.value
                if hasattr(app_data.slug, "value")
                else str(app_data.slug),
            ...
            try:
                ...
                logger.warning(
                    f"Could not fetch MCP tools for {app_slug}: {tools_error}"
                )
    ...
    async def discover_user_mcp_servers(
        ...
        try:
            ...
            connected_servers = [
                s for s in formatted_servers if s["status"] == "connected"
            ]
