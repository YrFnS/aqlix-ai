            ...
            if config_changed:
                try:
                    ...
                    if not current_version:
                        return self.fail_response(
                            "No current version found to update from"
                        )

                    current_system_prompt = (
                        system_prompt
                        if system_prompt is not None
                        else current_version.get("system_prompt", "")
                    )
                    ...
                    if agentpress_tools is not None:
                        ...
                        for tool_name, tool_config in agentpress_tools.items():
                            if isinstance(tool_config, dict):
                                formatted_tools[tool_name] = tool_config.get(
                                    "enabled", False
                                )
                            else:
                        ...
                        current_agentpress_tools = current_version.get(
                            "agentpress_tools", {}
                        )
                    ...
                    if configured_mcps is not None:
                        ...
                        logger.info(
                            f"MCP merge result: {len(current_configured_mcps)} total MCPs (was {len(current_version.get('configured_mcps', []))}, adding {len(configured_mcps)})"
                        )
                    ...
                    logger.info(
                        f"Created new version {new_version.version_id} for agent {self.agent_id}"
                    )
        ...
        try:
            ...
            if agent_data.get("current_version_id"):
                try:
                    ...
                    logger.warning(
                        f"Failed to get version data for agent config tool: {e}"
                    )
            ...
            config_summary = {
                ...
                "current_version": agent_config.get("version_name", "v1")
                if version_data
            ...
            tools_count = len(
                [
                    t
                    for t, cfg in config_summary["agentpress_tools"].items()
                    if cfg.get("enabled")
                ]
            )
            ...
            return self.success_response(
                {"summary": summary_text, "configuration": config_summary}
            )
