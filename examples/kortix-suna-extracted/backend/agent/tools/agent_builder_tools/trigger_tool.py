...
class TriggerTool(AgentBuilderBaseTool):
        ...
        try:
            if execution_type not in ["workflow", "agent"]:
                return self.fail_response(
                    "execution_type must be either 'workflow' or 'agent'"
                )

            if execution_type == "workflow" and not workflow_id:
                return self.fail_response(
                    "workflow_id is required when execution_type is 'workflow'"
                )

            if execution_type == "agent" and not agent_prompt:
                return self.fail_response(
                    "agent_prompt is required when execution_type is 'agent'"
                )
            ...
            if execution_type == "workflow":
                ...
                if not workflow_result.data:
                    return self.fail_response(
                        f"Workflow {workflow_id} not found or doesn't belong to this agent"
                    )

                workflow = workflow_result.data[0]
                if workflow["status"] != "active":
                    return self.fail_response(
                        f"Workflow '{workflow['name']}' is not active. Please activate it first."
                    )
            ...
            try:
                ...
                if execution_type == "workflow":
                    ...
                    if workflow_input:
                        result_message += (
                            f"**Input Data**: {json.dumps(workflow_input, indent=2)}\n"
                        )
                else:
    ...
    async def get_scheduled_triggers(self) -> ToolResult:
        try:
            ...
            schedule_triggers = [
                t for t in triggers if t.trigger_type == TriggerType.SCHEDULE
            ]
            ...
            for trigger in schedule_triggers:
                if trigger.config.get(
                    "execution_type"
                ) == "workflow" and trigger.config.get("workflow_id"):
            ...
            for trigger in schedule_triggers:
                ...
                if trigger.config.get("execution_type") == "workflow":
                    ...
                    formatted["workflow_name"] = workflows.get(
    ...
    async def delete_scheduled_trigger(self, trigger_id: str) -> ToolResult:
        try:
            ...
            if trigger_config.agent_id != self.agent_id:
                return self.fail_response(
                    "This trigger doesn't belong to the current agent"
                )
    ...
    @usage_example("""
        ...
        """)
    async def toggle_scheduled_trigger(
        self, trigger_id: str, is_active: bool
    ) -> ToolResult:
        try:
            trigger_svc = get_trigger_service(self.db)
