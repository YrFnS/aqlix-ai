...
class CredentialProfileTool(AgentBuilderBaseTool):
    ...
    async def get_credential_profiles(
        self, app_slug: Optional[str] = None
    ) -> ToolResult:
        try:
                ...
                formatted_profiles.append(
                    {
                        ...
                        if hasattr(profile.profile_name, "value")
                        else str(profile.profile_name),
                        "display_name": profile.display_name,
                        "app_slug": profile.app_slug.value
                        if hasattr(profile.app_slug, "value")
                        else str(profile.app_slug),
                        "app_name": profile.app_name,
                        "external_user_id": profile.external_user_id.value
                        if hasattr(profile.external_user_id, "value")
                        else str(profile.external_user_id),
    ...
    async def create_credential_profile(
        self, app_slug: str, profile_name: str, display_name: Optional[str] = None
    ) -> ToolResult:
        try:
            from uuid import UUID
    ...
    async def connect_credential_profile(self, profile_id: str) -> ToolResult:
        try:
            ...
            profile = await profile_service.get_profile(
                UUID(account_id), UUID(profile_id)
            )
            if not profile:
            ...
            app_slug = AppSlug(
                profile.app_slug.value
                if hasattr(profile.app_slug, "value")
                else str(profile.app_slug)
            )
            connection_result = await connection_token_service.create(
    ...
    async def check_profile_connection(self, profile_id: str) -> ToolResult:
        try:
            ...
            raw_connections = await connection_service.get_connections_for_user(
                external_user_id
            )
            connections = []
            for conn in raw_connections:
                connections.append(
                    {
                        "external_user_id": conn.external_user_id.value
                        if hasattr(conn.external_user_id, "value")
                        else str(conn.external_user_id),
                        "app_slug": conn.app.slug.value
                        if hasattr(conn.app.slug, "value")
                        else str(conn.app.slug),
            ...
            if profile.is_connected and connections:
                try:
                    from pipedream.mcp_service import (
                        ConnectionStatus,
                        ExternalUserId,
                        AppSlug,
                    )
                    ...
                    connected_servers = [
                        s for s in servers if s.status == ConnectionStatus.CONNECTED
                    ]
                    if connected_servers:
                        ...
                        response_data["message"] = (
                            f"Profile '{profile.display_name}' is connected with {len(tools)} available tools"
                        )
                    else:
                        response_data["message"] = (
                            f"Profile '{profile.display_name}' is connected but no MCP tools are available yet"
                        )
                except Exception as mcp_error:
                    logger.error(f"Error getting MCP tools for profile: {mcp_error}")
                    response_data["message"] = (
                        f"Profile '{profile.display_name}' is connected but could not retrieve MCP tools"
                    )
            else:
                response_data["message"] = (
                    f"Profile '{profile.display_name}' is not connected yet"
                )
        ...
        try:
            ...
            if not profile.is_connected:
                return self.fail_response(
                    "Profile is not connected yet. Please connect the profile first."
                )
            ...
            if not agent_result.data or not agent_result.data[0].get(
                "current_version_id"
            ):
            ...
            if agent_result.data and agent_result.data[0].get("current_version_id"):
                ...
                if version_result.data and version_result.data.get("config"):
                    ...
                    updated_mcps = [
                        mcp
                        for mcp in current_custom_mcps
                        if mcp.get("config", {}).get("profile_id")
                    ...
                    if len(updated_mcps) != len(current_custom_mcps):
                        ...
                        try:
                            ...
                            await version_service.create_version(
                                ...
                                user_id=account_id,
                                system_prompt=current_config.get("system_prompt", ""),
                                configured_mcps=current_config.get("tools", {}).get(
                                    "mcp", []
                                ),
                                custom_mcps=updated_mcps,
                                agentpress_tools=current_config.get("tools", {}).get(
                                    "agentpress", {}
                                ),
                            ...
                            return self.fail_response(
                                f"Failed to update agent config: {str(e)}"
                            )
