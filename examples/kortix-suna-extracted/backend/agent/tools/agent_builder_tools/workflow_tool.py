...
class WorkflowTool(AgentBuilderBaseTool):
    ...
    async def _get_available_tools_for_agent(self) -> List[str]:
        try:
            ...
            logger.error(
                f"Error getting available tools for agent {self.agent_id}: {e}"
            )
            return []

    def _validate_tool_steps(
        self, steps: List[Dict[str, Any]], available_tools: List[str]
    ) -> List[str]:
        errors = []
            ...
            for i, step in enumerate(step_list):
                ...
                if step.get("type") == "tool":
                    ...
                    if tool_name and tool_name not in available_tools:
                        errors.append(
                            f"{current_path}: Tool '{tool_name}' is not available for this agent"
                        )
    ...
    async def create_workflow(
        ...
        try:
            ...
            result = (
                await client.table("agent_workflows").insert(workflow_data).execute()
            )
    ...
    async def update_workflow(
        ...
        try:
            ...
            if not workflow_result.data:
                return self.fail_response(
                    "Workflow not found or doesn't belong to this agent"
                )
            ...
            if steps is not None:
                ...
                if validate_tools:
                    available_tools = await self._get_available_tools_for_agent()
                    validation_errors = self._validate_tool_steps(
                        steps, available_tools
                    )
                    if validation_errors:
                        return self.fail_response(
                            f"Tool validation failed:\n" + "\n".join(validation_errors)
    ...
    async def activate_workflow(
        self, workflow_id: str, active: bool = True
    ) -> ToolResult:
        try:
            client = await self.db.client
            workflow_result = (
