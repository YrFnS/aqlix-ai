...
class SandboxFilesTool(SandboxToolsBase):
    ...
    def __init__(self, project_id: str, thread_manager: ThreadManager):
        ...
        self.workspace_path = (
            "/workspace"  # Ensure we're always operating in /workspace
        )
    ...
    async def _call_morph_api(
        ...
        """
        try:
            morph_api_key = getattr(config, "MORPH_API_KEY", None) or os.getenv(
                "MORPH_API_KEY"
            )
            openrouter_key = getattr(config, "OPENROUTER_API_KEY", None) or os.getenv(
                "OPENROUTER_API_KEY"
            )
            ...
            if morph_api_key:
                ...
                client = openai.AsyncOpenAI(
                    api_key=morph_api_key, base_url="https://api.morphllm.com/v1"
                )
                ...
                logger.debug(
                    "Morph API key not set, falling back to OpenRouter for file editing via litellm."
                )
                ...
                error_msg = (
                    "No Morph or OpenRouter API key found, cannot perform AI edit."
                )
            ...
            if hasattr(e, "response") and hasattr(e.response, "text"):
                ...
                error_message += f"\n\nAPI Response Body:\n{e.body}"
            logger.error(
                f"Error calling Morph/OpenRouter API: {error_message}", exc_info=True
            )
...
// ... existing code ...
        ...
        """)
    async def edit_file(
        self, target_file: str, instructions: str, code_edit: str
    ) -> ToolResult:
        ...
        try:
            ...
            logger.info(
                f"Attempting AI-powered edit for file '{target_file}' with instructions: {instructions[:100]}..."
            )
            new_content, error_message = await self._call_morph_api(
            ...
            try:
                full_path_on_error = (
                    f"{self.workspace_path}/{self.clean_path(target_file)}"
                )
                if await self._file_exists(full_path_on_error):
                    original_content_on_error = (
                        await self.sandbox.fs.download_file(full_path_on_error)
                    ).decode()
            except:
                pass
