...
class SandboxBrowserTool(SandboxToolsBase):
    ...
    def _validate_base64_image(
        ...
        try:
            ...
            if len(image_data) > max_size_bytes:
                return (
                    False,
                    f"Image size ({len(image_data)} bytes) exceeds limit ({max_size_bytes} bytes)",
                )
                ...
                with Image.open(image_stream) as img:
                    ...
                    with Image.open(image_stream) as img_check:
                        ...
                        if width > max_dimension or height > max_dimension:
                            return (
                                False,
                                f"Image dimensions ({width}x{height}) exceed limit ({max_dimension}x{max_dimension})",
                            )
                        ...
                        logger.debug(
                            f"Valid image detected: {img.format}, {width}x{height}, {len(image_data)} bytes"
                        )
    ...
    async def _execute_browser_action(
        ...
        try:
            ...
            if response.exit_code == 0:
                try:
                    ...
                    if "screenshot_base64" in result:
                        try:
                            ...
                            if is_valid:
                                logger.debug(
                                    f"Screenshot validation passed: {validation_message}"
                                )
                    ...
                    if result.get("success"):
                        ...
                        success_response["message"] = result.get(
                    ...
                    if result.get("pixels_below"):
                        success_response["scrollable_content"] = (
                            result["pixels_below"] > 0
                        )
                    ...
                    logger.error(
                        f"Failed to parse response JSON: {response.result} {e}"
                    )
                ...
                return self.fail_response(
                    f"Browser automation request failed 2: {response}"
                )
    ...
    async def browser_get_dropdown_options(self, index: int) -> ToolResult:
        ...
        """
        logger.debug(
            f"\033[95mGetting options from dropdown with index: {index}\033[0m"
        )
        return await self._execute_browser_action(
    ...
    async def browser_select_dropdown_option(self, index: int, text: str) -> ToolResult:
        ...
        """
        logger.debug(
            f"\033[95mSelecting option '{text}' from dropdown with index: {index}\033[0m"
        )
        return await self._execute_browser_action(
    ...
    async def browser_drag_drop(
        ...
        element_target: str = None,
        coord_source_x: int = None,
        coord_source_y: int = None,
        coord_target_x: int = None,
        coord_target_y: int = None,
    ) -> ToolResult:
        ...
        if element_source and element_target:
            ...
            logger.debug(
                f"\033[95mDragging from element '{element_source}' to '{element_target}'\033[0m"
            )
        elif all(
            coord is not None
            for coord in [
            ...
            logger.debug(
                f"\033[95mDragging from coordinates ({coord_source_x}, {coord_source_y}) to ({coord_target_x}, {coord_target_y})\033[0m"
            )
        else:
            return self.fail_response(
                "Must provide either element selectors or coordinates for drag and drop"
            )
