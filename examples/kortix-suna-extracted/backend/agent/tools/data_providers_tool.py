...
<parameter name="service_name">linkedin</parameter>
        ...
        try:
            ...
            if service_name not in self.register_data_providers:
                return self.fail_response(
                    f"Data provider '{service_name}' not found. Available data providers: {list(self.register_data_providers.keys())}"
                )
            ...
            error_message = str(e)
            simplified_message = (
                f"Error getting data provider endpoints: {error_message[:200]}"
            )
    ...
    async def execute_data_provider_call(
        ...
        try:
            ...
            if service_name not in self.register_data_providers:
                return self.fail_response(
                    f"API '{service_name}' not found. Available APIs: {list(self.register_data_providers.keys())}"
                )

            data_provider = self.register_data_providers[service_name]
            if route == service_name:
                return self.fail_response(
                    f"route '{route}' is the same as service_name '{service_name}'. YOU FUCKING IDIOT!"
                )
            ...
            print(error_message)
            simplified_message = (
                f"Error executing data provider call: {error_message[:200]}"
            )
            if len(error_message) > 200:
                simplified_message += "..."
            return self.fail_response(simplified_message)
