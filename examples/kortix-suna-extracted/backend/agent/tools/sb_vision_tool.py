    ...
    def download_image_from_url(self, url: str) -> Tuple[bytes, str]:
        ...
        try:
            ...
            if content_length and content_length > MAX_IMAGE_SIZE:
                raise Exception(
                    f"Image is too large ({(content_length) / (1024 * 1024):.2f}MB) for the maximum allowed size of {MAX_IMAGE_SIZE / (1024 * 1024):.2f}MB"
                )
            ...
            if len(image_bytes) > MAX_IMAGE_SIZE:
                raise Exception(
                    f"Downloaded image is too large ({(len(image_bytes)) / (1024 * 1024):.2f}MB). Maximum allowed size of {MAX_IMAGE_SIZE / (1024 * 1024):.2f}MB"
                )
            ...
            if not mime_type or not mime_type.startswith("image/"):
                raise Exception(
                    f"URL does not point to an image (Content-Type: {mime_type}): {url}"
                )
        ...
        try:
            ...
            if is_url:
                ...
                try:
                    ...
                    return self.fail_response(
                        f"Could not read image file: {cleaned_path}"
                    )
            ...
            return self.success_response(
                f"Successfully loaded and compressed the image '{cleaned_path}' (reduced from {original_size / 1024:.1f}KB to {len(compressed_bytes) / 1024:.1f}KB)."
            )
