...
class TaskListTool(SandboxToolsBase):
    ...
    def _format_response(
        self, sections: List[Section], tasks: List[Task]
    ) -> Dict[str, Any]:
        ...
        for section in sections:
            ...
            if section_tasks:
                formatted_sections.append(
                    {"id": section.id, "title": section.title, "tasks": section_tasks}
                )
    ...
    async def create_tasks(
        ...
        try:
            ...
            if sections:
                ...
                for section_data in sections:
                    ...
                    for task_content in task_list:
                        new_task = Task(
                            content=task_content, section_id=target_section.id
                        )
                ...
                if not task_contents:
                    return ToolResult(
                        success=False,
                        output="❌ Must provide either 'sections' array or 'task_contents' with section info",
                    )

                if not section_id and not section_title:
                    return ToolResult(
                        success=False,
                        output="❌ Must specify either 'section_id' or 'section_title' when using 'task_contents'",
                    )
                ...
                if section_id:
                    ...
                    if section_id not in section_map:
                        return ToolResult(
                            success=False,
                            output=f"❌ Section ID '{section_id}' not found",
                        )
            ...
            return ToolResult(
                success=False, output=f"❌ Error creating tasks: {str(e)}"
            )
    ...
    async def update_tasks(
        ...
        try:
            ...
            if missing_tasks:
                return ToolResult(
                    success=False, output=f"❌ Task IDs not found: {missing_tasks}"
                )

            # Validate section ID if provided
            if section_id and section_id not in section_map:
                return ToolResult(
                    success=False, output=f"❌ Section ID '{section_id}' not found"
                )
            ...
            return ToolResult(
                success=False, output=f"❌ Error updating tasks: {str(e)}"
            )
    ...
    async def delete_tasks(
        self, task_ids=None, section_ids=None, confirm: bool = False
    ) -> ToolResult:
        ...
        try:
            ...
            if not task_ids and not section_ids:
                return ToolResult(
                    success=False,
                    output="❌ Must provide either task_ids or section_ids",
                )

            # Validate confirm parameter for section deletion
            if section_ids and not confirm:
                return ToolResult(
                    success=False, output="❌ Must set confirm=true to delete sections"
                )
            ...
            if section_ids:
                ...
                missing_sections = [
                    sid for sid in target_section_ids if sid not in section_map
                ]
                if missing_sections:
                    return ToolResult(
                        success=False,
                        output=f"❌ Section IDs not found: {missing_sections}",
                    )
                ...
                remaining_tasks = [
                    t for t in remaining_tasks if t.section_id not in section_id_set
                ]
            ...
            return ToolResult(
                success=False, output=f"❌ Error deleting tasks/sections: {str(e)}"
            )
    ...
    async def clear_all(self, confirm: bool) -> ToolResult:
        ...
        try:
            if not confirm:
                return ToolResult(
                    success=False, output="❌ Must set confirm=true to clear all data"
                )
            ...
            return ToolResult(
                success=False, output=f"❌ Error clearing all data: {str(e)}"
            )
