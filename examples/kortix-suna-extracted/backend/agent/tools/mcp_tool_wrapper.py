...
class MCPToolWrapper(Tool):
    ...
    async def _initialize_servers(self):
        standard_configs = [
            cfg for cfg in self.mcp_configs if not cfg.get("isCustom", False)
        ]
    ...
    async def _initialize_standard_servers(
        self, standard_configs: List[Dict[str, Any]]
    ):
        for config in standard_configs:
            try:
                logger.info(
                    f"Attempting to connect to MCP server: {config['qualifiedName']}"
                )
                await self.mcp_manager.connect_server(config)
                logger.info(
                    f"Successfully connected to MCP server: {config['qualifiedName']}"
                )
            except Exception as e:
                logger.error(
                    f"Failed to connect to MCP server {config['qualifiedName']}: {e}"
                )
    ...
    async def _create_dynamic_tools(self):
        try:
            ...
            dynamic_methods = self.tool_builder.create_dynamic_methods(
                available_tools, custom_tools, self._execute_mcp_tool
            )
            ...
            logger.info(
                f"Re-registered schemas after creating dynamic tools - total: {len(self._schemas)}"
            )
        ...
        if hasattr(self, "_dynamic_tools") and self._dynamic_tools:
            for tool_name, tool_data in self._dynamic_tools.items():
                ...
                if method and hasattr(method, "tool_schemas"):
                    ...
                    logger.debug(
                        f"Registered dynamic method schemas for '{method_name}'"
                    )

        logger.debug(
            f"Registration complete for MCPToolWrapper - total schemas: {len(self._schemas)}"
        )
    ...
    def __getattr__(self, name: str):
        ...
        if hasattr(self, "_dynamic_tools") and self._dynamic_tools:
            ...
            for tool_name, tool_data in self._dynamic_tools.items():
                if (
                    tool_data.get("method_name") == name
                    or tool_name == name_with_hyphens
                ):
                    return tool_data.get("method")
    ...
    async def initialize_and_register_tools(self, tool_registry=None):
        ...
        if tool_registry and self._dynamic_tools:
            logger.info(
                f"Updating tool registry with {len(self._dynamic_tools)} MCP tools"
            )
