...
class MCPConnectionManager:
    ...
    async def connect_sse_server(
        self, server_name: str, server_config: Dict[str, Any], timeout: int = 15
    ) -> Dict[str, Any]:
            ...
            try:
                async with sse_client(url, headers=headers) as (read, write):
                    async with ClientSession(read, write) as session:
                        ...
                        logger.info(
                            f"Connected to {server_name} via SSE ({len(tools_info)} tools)"
                        )
                        return server_info
        ...
        async with asyncio.timeout(timeout):
            async with streamablehttp_client(url) as (read_stream, write_stream, _):
                async with ClientSession(read_stream, write_stream) as session:
                    ...
                    self.connected_servers[server_name] = server_info
                    logger.info(
                        f"Connected to {server_name} via HTTP ({len(tools_info)} tools)"
                    )
                    return server_info
        ...
        async with asyncio.timeout(timeout):
            async with stdio_client(server_params) as (read, write):
                async with ClientSession(read, write) as session:
                    ...
                    self.connected_servers[server_name] = server_info
                    logger.info(
                        f"Connected to {server_name} via stdio ({len(tools_info)} tools)"
                    )
                    return server_info
