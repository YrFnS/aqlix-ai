...
class DynamicToolBuilder:
    ...
    def create_dynamic_methods(
        ...
        execute_callback: Callable[[str, Dict[str, Any]], Awaitable[ToolResult]],
    ) -> Dict[str, Callable]:
        methods = {}
        ...
        for tool_info in tools_info:
            ...
            if tool_name:
                method = self._create_dynamic_method(
                    tool_name, tool_info, execute_callback
                )
                if method:
        ...
        for tool_name, tool_info in custom_tools.items():
            ...
            method = self._create_dynamic_method(
                tool_name, openapi_tool_info, execute_callback
            )
            if method:
                methods[method["method_name"]] = method["method"]
    ...
    def _create_dynamic_method(
        ...
        logger.info(
            f"Creating dynamic method for tool '{tool_name}': clean_tool_name='{clean_tool_name}', method_name='{method_name}', server='{server_name}'"
        )
        ...
        logger.debug(
            f"Created dynamic method '{method_name}' for MCP tool '{tool_name}' from server '{server_name}'"
        )
    ...
    def _create_tool_schema(
        self, method_name: str, description: str, tool_info: Dict[str, Any]
    ) -> ToolSchema:
        ...
        return ToolSchema(
            schema_type=SchemaType.OPENAPI, schema=openapi_function_schema
        )
