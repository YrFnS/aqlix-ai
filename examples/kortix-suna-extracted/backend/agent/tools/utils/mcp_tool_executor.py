...
class MCPToolExecutor:
        ...
        if isinstance(result, dict):
            if result.get("isError", False):
                return self._create_error_result(
                    result.get("content", "Tool execution failed")
                )
            else:
    ...
    async def _execute_custom_tool(
        ...
        if custom_type == "pipedream":
            ...
            return self._create_error_result(
                f"Unsupported custom MCP type: {custom_type}"
            )

    async def _execute_pipedream_tool(
        self, tool_name: str, arguments: Dict[str, Any], tool_info: Dict[str, Any]
    ) -> ToolResult:
        ...
        try:
            ...
            return self._create_error_result(
                f"Error executing Pipedream tool: {str(e)}"
            )

    async def _execute_sse_tool(
        self, tool_name: str, arguments: Dict[str, Any], tool_info: Dict[str, Any]
    ) -> ToolResult:
        custom_config = tool_info["custom_config"]
        original_tool_name = tool_info["original_name"]
    ...
    def _create_success_result(self, content: Any) -> ToolResult:
        ...
        return ToolResult(success=True, content=str(content), metadata={})
    ...
    def _create_error_result(self, error_message: str) -> ToolResult:
        ...
        return ToolResult(success=False, content=error_message, metadata={})
