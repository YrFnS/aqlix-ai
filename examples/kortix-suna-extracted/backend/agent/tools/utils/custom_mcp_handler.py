...
class CustomMCPHandler:
    ...
    async def initialize_custom_mcps(
        self, custom_configs: List[Dict[str, Any]]
        ...
        for config in custom_configs:
            try:
                ...
                logger.error(
                    f"Failed to initialize custom MCP {config.get('name', 'Unknown')}: {e}"
                )
                continue
    ...
    async def _initialize_single_custom_mcp(self, config: Dict[str, Any]):
        ...
        if custom_type == "pipedream":
            await self._initialize_pipedream_mcp(
                server_name, server_config, enabled_tools
            )
    ...
    async def _initialize_pipedream_mcp(
        self, server_name: str, server_config: Dict[str, Any], enabled_tools: List[str]
    ):
        app_slug = server_config.get("app_slug")
        if (
            not app_slug
            and "headers" in server_config
            and "x-pd-app-slug" in server_config["headers"]
        ):
        ...
        if not external_user_id:
            logger.error(
                f"Custom MCP {server_name}: Missing external_user_id for Pipedream"
            )
            return
        ...
        logger.info(
            f"Initializing Pipedream MCP for {app_slug} (user: {external_user_id}, oauth_app_id: {oauth_app_id})"
        )
        ...
        try:
            ...
            if (
                hasattr(connection_service, "rate_limit_token")
                and connection_service.rate_limit_token
            ):
            ...
            async with streamablehttp_client(url, headers=headers) as (
                ...
                async with ClientSession(read_stream, write_stream) as session:
                    ...
                    self._register_custom_tools(
                        tools, server_name, enabled_tools, "pipedream", server_config
                    )
    ...
    async def _initialize_sse_mcp(
        ...
        server_info = await self.connection_manager.connect_sse_server(
            server_name, server_config
        )
        if server_info.get("status") == "connected":
            tools_info = server_info.get("tools", [])
            self._register_custom_tools_from_info(
                tools_info, server_name, enabled_tools, "sse", server_config
            )
        else:
            logger.error(f"Failed to connect to custom MCP {server_name}")

    async def _initialize_http_mcp(
        self, server_name: str, server_config: Dict[str, Any], enabled_tools: List[str]
    ):
        if "url" not in server_config:
            logger.error(f"Custom MCP {server_name}: Missing 'url' in config")
        ...
        server_info = await self.connection_manager.connect_http_server(
            server_name, server_config
        )
        if server_info.get("status") == "connected":
            tools_info = server_info.get("tools", [])
            self._register_custom_tools_from_info(
                tools_info, server_name, enabled_tools, "http", server_config
            )
        else:
            logger.error(f"Failed to connect to custom MCP {server_name}")

    async def _initialize_json_mcp(
        self, server_name: str, server_config: Dict[str, Any], enabled_tools: List[str]
    ):
        ...
        server_info = await self.connection_manager.connect_stdio_server(
            server_name, server_config
        )
        if server_info.get("status") == "connected":
            tools_info = server_info.get("tools", [])
            self._register_custom_tools_from_info(
                tools_info, server_name, enabled_tools, "json", server_config
            )
        else:
            logger.error(f"Failed to connect to custom MCP {server_name}")
    ...
    async def _resolve_external_user_id(self, server_config: Dict[str, Any]) -> str:
        ...
        try:
            ...
            result = (
                await supabase.table("user_mcp_credential_profiles")
            ...
            if result.data:
                ...
                if external_user_id and external_user_id != profile_external_user_id:
                    logger.warning(
                        f"Overriding external_user_id {external_user_id} with profile's external_user_id {profile_external_user_id}"
                    )
    ...
    def _register_custom_tools(
        ...
        logger.info(
            f"Successfully initialized custom MCP {server_name} with {tools_registered} tools"
        )

    def _register_custom_tools_from_info(
        self,
