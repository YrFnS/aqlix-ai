...
from .versioning.api import (
    router as version_router,
    initialize as initialize_versioning,
)
...
async def stop_agent_run(agent_run_id: str, error_message: Optional[str] = None):
    ...
    if not update_success:
        logger.error(
            f"Failed to update database status for stopped/failed run {agent_run_id}"
        )
        raise HTTPException(
            status_code=500, detail="Failed to update agent run status in database"
        )
    ...
    try:
        ...
        logger.debug(
            f"Found {len(instance_keys)} active instance keys for agent run {agent_run_id}"
        )
...
async def get_agent_run_with_access_check(client, agent_run_id: str, user_id: str):
    agent_run = (
        await client.table("agent_runs").select("*").eq("id", agent_run_id).execute()
    )
...
async def start_agent(
    ...
    if effective_agent_id:
        ...
        if not agent_result.data:
            if body.agent_id:
                ...
                logger.warning(
                    f"Stored agent_id {effective_agent_id} not found, falling back to default"
                )
            ...
            if agent_data.get("current_version_id"):
                try:
                    ...
                    logger.info(
                        f"[AGENT LOAD] Got version data from version manager: {version_data.get('version_name')}"
                    )
                except Exception as e:
                    logger.warning(f"[AGENT LOAD] Failed to get version data: {e}")

            logger.info(
                f"[AGENT LOAD] About to call extract_agent_config with agent_data keys: {list(agent_data.keys())}"
            )
            logger.info(
                f"[AGENT LOAD] version_data type: {type(version_data)}, has data: {version_data is not None}"
            )
            ...
            if version_data:
                logger.info(
                    f"Using agent {agent_config['name']} ({effective_agent_id}) version {agent_config.get('version_name', 'v1')}"
                )
            else:
                logger.info(
                    f"Using agent {agent_config['name']} ({effective_agent_id}) - no version data"
                )
    ...
    if not agent_config:
        ...
        logger.info(
            f"[AGENT LOAD] Default agent query result: found {len(default_agent_result.data) if default_agent_result.data else 0} default agents"
        )
        ...
        if default_agent_result.data:
            ...
            if agent_data.get("current_version_id"):
                try:
                    ...
                    version_data = version_obj.to_dict()
                    logger.info(
                        f"[AGENT LOAD] Got default agent version from version manager: {version_data.get('version_name')}"
                    )
                except Exception as e:
                    logger.warning(
                        f"[AGENT LOAD] Failed to get default agent version data: {e}"
                    )

            logger.info(
                f"[AGENT LOAD] About to call extract_agent_config for DEFAULT agent with version data: {version_data is not None}"
            )
            ...
            if version_data:
                logger.info(
                    f"Using default agent: {agent_config['name']} ({agent_config['agent_id']}) version {agent_config.get('version_name', 'v1')}"
                )
            else:
                logger.info(
                    f"Using default agent: {agent_config['name']} ({agent_config['agent_id']}) - no version data"
                )
        else:
            logger.warning(
                f"[AGENT LOAD] No default agent found for account {account_id}"
            )
    ...
    if agent_config:
        ...
        logger.info(
            f"Using agent {agent_config['agent_id']} for this agent run (thread remains agent-agnostic)"
        )

    can_use, model_message, allowed_models = await can_use_model(
        client, account_id, model_name
    )
    if not can_use:
        raise HTTPException(
            status_code=403,
            detail={"message": model_message, "allowed_models": allowed_models},
        )

    can_run, message, subscription = await check_billing_status(client, account_id)
    if not can_run:
        raise HTTPException(
            status_code=402, detail={"message": message, "subscription": subscription}
        )
    ...
    try:
        ...
        logger.info(
            f"Successfully started sandbox {sandbox_id} for project {project_id}"
        )
    except Exception as e:
...
async def get_agent_runs(
    ...
    agent_runs = (
        await client.table("agent_runs")
        .select(
            "id, thread_id, status, started_at, completed_at, error, created_at, updated_at"
        )
        .eq("thread_id", thread_id)
                        ...
                        try:
                            ...
                            if task in tasks:
                                ...
                                if (
                                    message
                                    and isinstance(message, dict)
                                    and message.get("channel") == response_channel
                                ):
                                    tasks.append(
                                        asyncio.create_task(response_reader.__anext__())
                                    )
                                    ...
                                    message
                                    and isinstance(message, dict)
                                    and message.get("channel") == control_channel
                                ):
                                    tasks.append(
                                        asyncio.create_task(control_reader.__anext__())
                                    )
...
async def generate_and_update_project_name(project_id: str, prompt: str):
    ...
    try:
        ...
        response = await make_llm_api_call(
            messages=messages, model_name=model_name, max_tokens=20, temperature=0.7
        )
...
async def initiate_agent_with_files(
    ...
    if agent_id:
        ...
        logger.info(
            f"[AGENT INITIATE] No agent_id provided, querying for default agent"
        )
        ...
        logger.info(
            f"[AGENT INITIATE] Default agent query result: found {len(default_agent_result.data) if default_agent_result.data else 0} default agents"
        )
        ...
        if default_agent_result.data:
            ...
            if agent_data.get("current_version_id"):
                try:
                    ...
                    version_data = version_obj.to_dict()
                    logger.info(
                        f"[AGENT INITIATE] Got default agent version from version manager: {version_data.get('version_name')}"
                    )
                except Exception as e:
                    logger.warning(
                        f"[AGENT INITIATE] Failed to get default agent version data: {e}"
                    )

            logger.info(
                f"[AGENT INITIATE] About to call extract_agent_config for DEFAULT agent with version data: {version_data is not None}"
            )
            ...
            logger.warning(
                f"[AGENT INITIATE] No default agent found for account {account_id}"
            )
    ...
    try:
        ...
        if agent_config:
            logger.info(
                f"Using agent {agent_config['agent_id']} for this conversation (thread remains agent-agnostic)"
            )
            structlog.contextvars.bind_contextvars(
...
async def get_agents(
    ...
    try:
        ...
        query = (
            client.table("agents").select("*", count="exact").eq("account_id", user_id)
        )
...
async def get_agent(
    ...
    try:
        ...
        if agent_data["account_id"] != user_id and not agent_data.get(
            "is_public", False
        ):
...
async def import_agent(
    ...
    try:
        ...
        if not import_data.name or not import_data.system_prompt:
            raise HTTPException(
                status_code=400, detail="Agent name and system prompt are required"
            )
        ...
        if not new_agent.data:
            raise HTTPException(
                status_code=500, detail="Failed to create agent from import"
            )
...
async def update_agent(
    ...
    try:
        ...
        if is_suna_agent:
            logger.warning(
                f"Update attempt on Suna default agent {agent_id} by user {user_id}"
            )
            ...
            if (
                ...
                logger.error(
                    f"User {user_id} attempted to modify restricted name of Suna agent {agent_id}"
                )
                raise HTTPException(
                    status_code=403,
            ...
            if (
                ...
                logger.error(
                    f"User {user_id} attempted to modify restricted description of Suna agent {agent_id}"
                )
                raise HTTPException(
                    status_code=403, detail="Suna's description cannot be modified."
                )
            ...
            if (
                ...
                logger.error(
                    f"User {user_id} attempted to modify restricted system prompt of Suna agent {agent_id}"
                )
                raise HTTPException(
                    status_code=403,
            ...
            if (
                ...
                logger.error(
                    f"User {user_id} attempted to modify restricted tools of Suna agent {agent_id}"
                )
                raise HTTPException(
                    status_code=403,
            ...
            if (
                ...
                logger.error(
                    f"User {user_id} attempted to modify restricted MCPs of Suna agent {agent_id}"
                )
                raise HTTPException(
                    status_code=403, detail="Suna's integrations cannot be modified."
                )

            logger.info(
                f"Suna agent update validation passed for agent {agent_id} by user {user_id}"
            )
        ...
        if current_version_data is None:
            ...
            try:
                ...
                if version_result.data:
                    ...
                    await (
                        client.table("agents")
                        .update({"current_version_id": version_id, "version_count": 1})
                        .eq("agent_id", agent_id)
                        .execute()
                    )
        ...
        logger.info(
            f"Agent {agent_id} configuration updated. Users can create a new version if needed."
        )
...
async def get_agent_builder_chat_history(
    ...
    try:
        ...
        logger.info(
            f"Found {len(messages_result.data)} messages for agent builder chat history"
        )
        return {"messages": messages_result.data, "thread_id": latest_thread_id}
