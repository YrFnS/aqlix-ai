...
class KnowledgeBaseEntry(BaseModel):
    ...
    usage_context: str = Field(
        default="always", pattern="^(always|on_request|contextual)$"
    )
    is_active: bool = True
...
class UpdateKnowledgeBaseEntryRequest(BaseModel):
    ...
    usage_context: Optional[str] = Field(
        None, pattern="^(always|on_request|contextual)$"
    )
...
async def get_agent_knowledge_base(
    ...
    if not await is_enabled("knowledge_base"):
        raise HTTPException(
            status_code=403, detail="This feature is not available at the moment."
        )
    ...
    try:
        ...
        return KnowledgeBaseListResponse(
            entries=entries, total_count=len(entries), total_tokens=total_tokens
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve agent knowledge base"
        )
...
async def create_agent_knowledge_base_entry(
    ...
    try:
        ...
        result = (
            await client.table("agent_knowledge_base_entries")
            .insert(insert_data)
            .execute()
        )
        ...
        if not result.data:
            raise HTTPException(
                status_code=500, detail="Failed to create agent knowledge base entry"
            )
        ...
        logger.error(
            f"Error creating knowledge base entry for agent {agent_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500, detail="Failed to create agent knowledge base entry"
        )
...
async def upload_file_to_agent_kb(
    ...
    try:
        ...
        if not job_id.data:
            raise HTTPException(
                status_code=500, detail="Failed to create processing job"
            )
...
async def update_knowledge_base_entry(
    ...
    try:
        ...
        if not entry_result.data:
            raise HTTPException(
                status_code=404, detail="Knowledge base entry not found"
            )
        ...
        if not result.data:
            raise HTTPException(
                status_code=500, detail="Failed to update knowledge base entry"
            )

        updated_entry = result.data[0]

        logger.info(
            f"Updated agent knowledge base entry {entry_id} for agent {agent_id}"
        )
...
async def delete_knowledge_base_entry(
    entry_id: str, user_id: str = Depends(get_current_user_id_from_jwt)
):
    if not await is_enabled("knowledge_base"):
        raise HTTPException(
            status_code=403, detail="This feature is not available at the moment."
    ...
    try:
        ...
        logger.info(
            f"Deleted agent knowledge base entry {entry_id} for agent {agent_id}"
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to delete knowledge base entry"
        )
...
async def get_knowledge_base_entry(
    ...
    try:
        ...
        logger.info(
            f"Retrieved agent knowledge base entry {entry_id} for agent {agent_id}"
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve knowledge base entry"
        )


@router.get(
    "/agents/{agent_id}/processing-jobs", response_model=List[ProcessingJobResponse]
)
async def get_agent_processing_jobs(
    agent_id: str, limit: int = 10, user_id: str = Depends(get_current_user_id_from_jwt)
):
    if not await is_enabled("knowledge_base"):
        raise HTTPException(
            status_code=403, detail="This feature is not available at the moment."
    ...
    try:
        ...
        result = await client.rpc(
            "get_agent_kb_processing_jobs", {"p_agent_id": agent_id, "p_limit": limit}
        ).execute()
...
async def process_file_background(
    ...
    try:
        await client.rpc(
            "update_agent_kb_job_status", {"p_job_id": job_id, "p_status": "processing"}
        ).execute()
        ...
        try:
            await client.rpc(
                "update_agent_kb_job_status",
                {"p_job_id": job_id, "p_status": "failed", "p_error_message": str(e)},
            ).execute()
        except:
            pass
...
async def get_agent_knowledge_base_context(
    ...
    try:
        ...
        result = await client.rpc(
            ...
            {"p_agent_id": agent_id, "p_max_tokens": max_tokens},
        ).execute()

        context = result.data if result.data else None

        return {"context": context, "max_tokens": max_tokens, "agent_id": agent_id}

    except HTTPException:
        raise
    except Exception as e:
        logger.error(
            f"Error getting knowledge base context for agent {agent_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500, detail="Failed to retrieve agent knowledge base context"
        )
