...
class FileProcessor:
    ...
    async def process_file_upload(
        ...
        try:
            ...
            if file_extension == ".zip":
                return await self._process_zip_file(
                    agent_id, account_id, file_content, filename
                )
    ...
    async def _process_zip_file(
        self, agent_id: str, account_id: str, zip_content: bytes, zip_filename: str
    ) -> Dict[str, Any]:
        try:
            client = await self.db.client
            ...
            with zipfile.ZipFile(io.BytesIO(zip_content), "r") as zip_ref:
                ...
                if len(file_list) > self.MAX_ZIP_ENTRIES:
                    raise ValueError(
                        f"ZIP contains too many files: {len(file_list)} (max: {self.MAX_ZIP_ENTRIES})"
                    )
                        ...
                        if content and content.strip():
                            extracted_entry_data = {
                                ...
                                "source_metadata": {
                                    ...
                                    "file_size": len(file_content),
                                    "extraction_method": self._get_extraction_method(
                                        Path(filename).suffix.lower(), mime_type
                                    ),
                                },
                                "file_size": len(file_content),
                                "file_mime_type": mime_type,
            ...
            return {"success": False, "zip_filename": zip_filename, "error": str(e)}
    ...
    async def process_git_repository(
        ...
        try:
            ...
            for root, dirs, files in os.walk(temp_dir):
                ...
                for file in files:
                    ...
                    if not self._should_include_file(
                        relative_path, include_patterns, exclude_patterns
                    ):
                        continue
                    ...
                    try:
                        ...
                        if content and content.strip():
                            file_entry_data = {
                                ...
                                "source_metadata": {
                                    ...
                                    "file_size": len(file_content),
                                    "extraction_method": self._get_extraction_method(
                                        Path(file).suffix.lower(), mime_type
                                    ),
                                },
                                "file_size": len(file_content),
                                "file_mime_type": mime_type,
                        ...
                        logger.error(
                            f"Error processing {relative_path} from git repo: {str(e)}"
                        )
                        failed_files.append(
                            {
                                "filename": file,
            ...
            return {"success": False, "git_url": git_url, "error": str(e)}
    ...
    async def _extract_file_content(
        ...
        try:
            if file_extension in self.SUPPORTED_TEXT_EXTENSIONS or mime_type.startswith(
                "text/"
            ):
                ...
                raise ValueError(
                    f"Unsupported file format: {file_extension}. Only .txt, .pdf, and .docx files are supported."
                )
    ...
    def _should_include_file(
        self, file_path: str, include_patterns: List[str], exclude_patterns: List[str]
    ) -> bool:
        import fnmatch
