...
async def validate_template_ownership_and_get(
    ...
    if template.creator_id != user_id:
        logger.warning(
            f"User {user_id} attempted to access template {template_id} owned by {template.creator_id}"
        )
        raise HTTPException(
            status_code=403, detail="You don't have permission to access this template"
        )

    return template


async def validate_template_access_and_get(
    template_id: str, user_id: str
) -> AgentTemplate:
    """
    ...
    if template.creator_id != user_id and not template.is_public:
        logger.warning(
            f"User {user_id} attempted to access private template {template_id} owned by {template.creator_id}"
        )
...
async def validate_agent_ownership(agent_id: str, user_id: str) -> Dict[str, Any]:
    ...
    if agent["account_id"] != user_id:
        logger.warning(
            f"User {user_id} attempted to access agent {agent_id} owned by {agent['account_id']}"
        )
        raise HTTPException(
            status_code=403, detail="You don't have permission to access this agent"
        )
...
async def create_template_from_agent(
    request: CreateTemplateRequest, user_id: str = Depends(get_current_user_id_from_jwt)
):
    """
    ...
    try:
        ...
        logger.info(
            f"Successfully created template {template_id} from agent {request.agent_id}"
        )
        ...
        logger.error(
            f"Error creating template from agent {request.agent_id}: {e}", exc_info=True
        )
...
async def publish_template(
    ...
    try:
        ...
        if not success:
            logger.warning(
                f"Failed to publish template {template_id} for user {user_id}"
            )
...
async def unpublish_template(
    template_id: str, user_id: str = Depends(get_current_user_id_from_jwt)
):
    """
    ...
    try:
        ...
        if not success:
            logger.warning(
                f"Failed to unpublish template {template_id} for user {user_id}"
            )
...
async def delete_template(
    ...
    try:
        ...
        if not success:
            logger.warning(
                f"Failed to delete template {template_id} for user {user_id}"
            )
...
async def install_template(
    ...
    try:
        ...
        logger.info(
            f"Successfully installed template {request.template_id} as instance {result.instance_id}"
        )
        ...
        logger.error(
            f"Error installing template {request.template_id}: {e}", exc_info=True
        )
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_my_templates(user_id: str = Depends(get_current_user_id_from_jwt)):
    """
    ...
    try:
        ...
        return [
            TemplateResponse(
                **format_template_for_response(template), creator_name=None
            )
            for template in templates
        ]
...
async def get_template(
    ...
    try:
        ...
        logger.warning(
            f"Access denied to template {template_id} for user {user_id}: {e}"
        )
