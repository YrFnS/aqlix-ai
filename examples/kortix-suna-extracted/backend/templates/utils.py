...
def build_unified_config(
    ...
    try:
        ...
        return {
            ...
            "metadata": {"avatar": avatar, "avatar_color": avatar_color},
        }


def build_mcp_config(requirement: MCPRequirementValue, profile: Any) -> ConfigType:
    if requirement.is_custom():
        ...
        return {
            ...
            if hasattr(profile, "mcp_qualified_name")
...
def format_template_for_response(template: AgentTemplate) -> Dict[str, Any]:
    return {
        ...
        "mcp_requirements": format_mcp_requirements_for_response(
...
def format_mcp_requirements_for_response(
    requirements: List[MCPRequirementValue],
) -> List[Dict[str, Any]]:
    return [
        {
...
def filter_templates_by_tags(
    templates: List[AgentTemplate], tags: List[str]
) -> List[AgentTemplate]:
    if not tags:
        return templates
...
def search_templates_by_name(
    templates: List[AgentTemplate], query: str
) -> List[AgentTemplate]:
    if not query:
        return templates
