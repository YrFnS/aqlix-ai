...
class MCPRequirementValue:
    ...
    def is_custom(self) -> bool:
        if self.qualified_name.startswith("pipedream:"):
            return False
        return self.custom_type is not None and self.qualified_name.startswith(
            "custom_"
        )
...
class AgentTemplate:
    ...
    def with_public_status(
        self, is_public: bool, published_at: Optional[datetime] = None
    ) -> "AgentTemplate":
        ...
        for mcp in custom_mcps:
            if isinstance(mcp, dict) and mcp.get("name"):
                ...
                if mcp_type == "pipedream":
                    app_slug = (
                        mcp.get("config", {}).get("headers", {}).get("x-pd-app-slug")
                    )
                    if not app_slug:
                        app_slug = (
                            mcp_name.lower()
...
class TemplateService:
    ...
    async def create_from_agent(
        ...
        if agent["account_id"] != creator_id:
            raise TemplateAccessDeniedError(
                "You can only create templates from your own agents"
            )

        if self._is_suna_default_agent(agent):
            raise SunaDefaultAgentTemplateError(
                "Cannot create template from Suna default agent"
            )
    ...
    async def delete_template(self, template_id: str, creator_id: str) -> bool:
        ...
        if template["creator_id"] != creator_id:
            logger.warning(
                f"User {creator_id} cannot delete template {template_id} (owned by {template['creator_id']})"
            )
            return False
    ...
    async def _get_agent_version_config(
    ...
    async def _sanitize_config_for_template(
        ...
        result = await client.rpc(
            "sanitize_config_for_template", {"input_config": config}
        ).execute()
            ...
            if isinstance(mcp, dict):
                ...
                if mcp.get("type") == "pipedream":
                    ...
                    sanitized_mcp["config"] = {
                        ...
                        "headers": {
                            k: v
                            for k, v in original_config.get("headers", {}).items()
                            if k != "profile_id"
                        },
                    }
                else:
    ...
    async def _save_template(self, template: AgentTemplate) -> None:
        ...
        template_data = {
            ...
            "marketplace_published_at": template.marketplace_published_at.isoformat()
            if template.marketplace_published_at
            else None,
