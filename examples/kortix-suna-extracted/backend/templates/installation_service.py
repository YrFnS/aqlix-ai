...
class InstallationService:
    ...
    async def install_template(
        self, request: TemplateInstallationRequest
    ) -> TemplateInstallationResult:
        logger.info(
            f"Installing template {request.template_id} for user {request.account_id}"
        )
        ...
        if not request.profile_mappings:
            request.profile_mappings = await self._auto_map_profiles(
                mcp_requirements, request.account_id
            )

        (
            missing_profiles,
            missing_configs,
        ) = await self._validate_installation_requirements(
            mcp_requirements, request.profile_mappings, request.custom_mcp_configs
        )
        ...
        agent_config = await self._build_agent_config(
            template, request, mcp_requirements
        )

        agent_id = await self._create_agent(template, request, agent_config)
        ...
        logger.info(
            f"Successfully installed template {template.template_id} as agent {agent_id}"
        )

        return TemplateInstallationResult(
            status="installed", instance_id=agent_id, name=agent_name
        )
    ...
    async def _auto_map_profiles(
        self, requirements: List[MCPRequirementValue], account_id: str
        ...
        for req in requirements:
            if not req.is_custom():
                ...
                if default_profile:
                    ...
                    logger.info(
                        f"Auto-mapped {req.qualified_name} to profile {default_profile.profile_id}"
                    )
    ...
    async def _build_agent_config(
        ...
        agent_config = {
            "tools": {"agentpress": agentpress_tools, "mcp": [], "custom_mcp": []},
        ...
        for req in requirements:
            if req.is_custom():
                ...
                if (
                    req.qualified_name.startswith("custom_")
                    and "_" in req.qualified_name[7:]
                ):
                ...
                if profile_id:
                    profile = await profile_service.get_profile(
                        request.account_id, profile_id
                    )
                    if profile:
                        if req.qualified_name.startswith("pipedream:"):
                            app_slug = profile.config.get(
                                "app_slug", req.qualified_name.split(":")[1]
                            )
    ...
    async def _increment_download_count(self, template_id: str) -> None:
        ...
        try:
            await client.rpc(
                "increment_template_download_count", {"template_id_param": template_id}
            ).execute()
        except Exception as e:
            logger.warning(
                f"Failed to increment download count for template {template_id}: {e}"
            )
