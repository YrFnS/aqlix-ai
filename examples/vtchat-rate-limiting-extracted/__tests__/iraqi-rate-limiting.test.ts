...
import {
  RateLimitContext,
  RateLimitResult,
} from "../lib/types/rate-limit.types";
...
describe("Iraqi Rate Limiting System", () => {
  ...
  describe("Cultural Adaptations", () => {
    test("should apply prayer time reductions", async () => {
      ...
      const prayerTimeConfig =
        IraqiRateLimitConfiguration.getConfigForCulturalMode("prayer_aware");
    ...
    test("should apply Ramadan multipliers", async () => {
      ...
      const ramadanConfig =
        IraqiRateLimitConfiguration.getConfigForCulturalMode("ramadan_mode");
    ...
    test("should handle government emergency mode", async () => {
      ...
      const emergencyConfig =
        IraqiRateLimitConfiguration.getConfigForCulturalMode("emergency_mode");
  ...
  describe("Iraqi Payment Integration", () => {
    ...
    test("should process FastPay payment with governorate validation", async () => {
      ...
      const availableMethods =
        paymentService.getAvailablePaymentMethods("Basra");
  ...
  describe("Professional Domain Quotas", () => {
    test("should enforce legal domain quotas", async () => {
      ...
      const quotas =
        await domainService.getProfessionalQuotas(professionalContext);

      expect(quotas.domain).toBe("legal");
      expect(quotas.baseLimit).toBe(1600); // 800 * 2.0 (advanced multiplier)
      expect(quotas.culturalConsiderations.islamicComplianceRequired).toBe(
        true,
      );
    ...
    test("should validate professional credentials", async () => {
      ...
      const validation =
        await domainService.validateCredentials(invalidContext);
  ...
  describe("Arabic Translation Rate Limiting", () => {
    test("should enforce Baghdad dialect translation limits", async () => {
      ...
      const result =
        await translationService.checkTranslationRateLimit(translationContext);
    ...
    test("should reject Kurdish dialect for basic subscription", async () => {
      ...
      const result =
        await translationService.checkTranslationRateLimit(kurdishContext);

      expect(result.allowed).toBe(false);
      expect(result.culturalMessage).toContain(
        "kurdish dialect requires expert subscription",
      );
  ...
  describe("Budget Tracking Integration", () => {
    test("should track budget accurately across services", async () => {
      ...
      await budgetService.trackRequest(
        userId,
        "chat",
        undefined,
        subscriptionTier,
    ...
    test("should enforce budget limits", async () => {
      ...
      const result = await budgetService.trackRequest(
        userId,
        "chat",
        undefined,
        subscriptionTier,
    ...
    test("should calculate professional domain multipliers", async () => {
      ...
      const legalResult = await budgetService.trackRequest(
        userId,
        "professional_query",
        "legal",
        subscriptionTier,
      );
      const medicalResult = await budgetService.trackRequest(
        userId,
        "professional_query",
        "medical",
        subscriptionTier,
      ...
      expect(medicalResult.cost.totalCost).toBeGreaterThan(
        legalResult.cost.totalCost,
      );
    });
  });
  ...
  describe("Integration Testing", () => {
    test("should handle complete user journey from registration to premium usage", async () => {
      ...
      const premiumResult =
        await rateLimitService.checkRateLimit(premiumContext);
    ...
    test("should handle professional user with Arabic translation workflow", async () => {
      ...
      const credValidation =
        await domainService.validateCredentials(professionalContext);
  ...
  describe("Error Handling and Edge Cases", () => {
    ...
    test("should handle payment gateway timeouts", async () => {
      ...
      jest
        .spyOn(paymentService as any, "processZainCashPayment")
        .mockImplementation(() => {
...
export const generateIraqiUser = (
