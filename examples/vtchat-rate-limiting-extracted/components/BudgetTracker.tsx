...
export function BudgetTracker({
  ...
  const [currentStatus, setCurrentStatus] = useState<BudgetStatus | null>(status || null);
  ...
  const getPaymentGatewayIcon = (gateway?: string) => {
    switch (gateway) {
      case 'zaincash': return '💳';
      case 'fastpay': return '⚡';
      case 'nasswallet': return '🏦';
      default: return '💰';
    }
  };
      ...
      <Card className={`w-full ${isRTL ? 'text-right' : 'text-left'}`}>
        <CardContent className="p-6">
          <p className="text-muted-foreground">
            {showArabic ? 'لا يمكن تحميل معلومات الميزانية' : 'Unable to load budget information'}
          </p>
        </CardContent>
      </Card>
    );
  ...
  const currentSpent = selectedPeriod === 'daily' ? currentStatus.dailySpent : currentStatus.monthlySpent;
  const currentLimit = selectedPeriod === 'daily' ? currentStatus.dailyLimit : currentStatus.monthlyLimit;
  const currentRemaining = selectedPeriod === 'daily' ? currentStatus.dailyRemaining : currentStatus.monthlyRemaining;
  const currentPercentage = selectedPeriod === 'daily' ? currentStatus.percentageUsed.daily : currentStatus.percentageUsed.monthly;

  return (
    <div className={`space-y-6 ${isRTL ? 'text-right font-arabic' : 'text-left'}`}>
      {/* Main Budget Overview */}
      <Card className={currentStatus.warningLevel ? getWarningColor(currentStatus.warningLevel) : ''}>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <div className="text-right">
                ...
                <div className="text-sm font-medium text-green-600">
                  {formatCurrency(currentRemaining)} {showArabic ? 'متبقي' : 'remaining'}
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-between text-sm text-muted-foreground">
              <span>{currentPercentage.toFixed(1)}% {showArabic ? 'مستخدم' : 'used'}</span>
              <span>
                {selectedPeriod === 'daily' 
                  ? (showArabic ? 'يتجدد كل يوم' : 'Resets daily')
                  : (showArabic ? 'يتجدد شهرياً' : 'Resets monthly')
                }
              </span>
            </div>
          </div>
                ...
                <div className="space-y-2">
                  <p className="text-sm font-medium text-orange-800">
                    {currentStatus.warningLevel === 'CRITICAL' 
                      ? (showArabic ? 'تحذير حرج: الميزانية شبه منتهية!' : 'Critical Warning: Budget Nearly Exhausted!')
                      : (showArabic ? 'تحذير: استهلاك عالي للميزانية' : 'Warning: High Budget Usage')
                    }
                  </p>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base font-semibold flex items-center gap-2">
            ...
            {showArabic ? 'تفصيل التكاليف حسب نوع الطلب' : 'Cost Breakdown by Request Type'}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {Object.entries(currentStatus.requestCosts).map(([type, cost]) => {
              const labels = {
                ...
                cultural_validation: showArabic ? 'تحقق ثقافي' : 'Cultural Validation',
                image_generation: showArabic ? 'إنتاج صور' : 'Image Generation',
                professional_query: showArabic ? 'استفسار مهني' : 'Professional Query',
              };
              ...
              return (
                <div key={type} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-sm font-medium">
                    {formatCurrency(cost)} {showArabic ? 'لكل طلب' : 'per request'}
                  </div>
                </div>
              );
            })}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base font-semibold flex items-center gap-2">
            ...
            {showArabic ? 'معلومات الاشتراك والدفع' : 'Subscription & Billing Info'}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            ...
            {currentStatus.paymentGateway && (
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1 text-sm font-medium">
                  ...
                  {currentStatus.paymentGateway === 'nasswallet' && 'NassWallet'}
                </div>
              </div>
            )}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <div className="text-xs text-muted-foreground">
                  {getDaysUntilBilling(currentStatus.nextBillingDate)} {showArabic ? 'أيام متبقية' : 'days remaining'}
                </div>
              </div>
            </div>
