...
from accessibility_checker import (
    TerminalAccessibilityValidator,
    ArabicTerminalOptimizer,
)
...
class IraqiTerminalInterface:
    ...
    def __init__(
        ...
        self.console = Console(
            width=120, height=40, force_terminal=True, legacy_windows=False
        )
    ...
    def _initialize_terminal_display(self):
        ...
        try:
            ...
            self.console.set_window_title(
                f"Terminal - {ministry_name} - جمهورية العراق"
            )

            self.logger.info(
                f"Terminal initialized for ministry: {self.session.ministry.value}"
            )
    ...
    async def render_header(self) -> Panel:
        ...
        if self.enable_cultural_validation:
            ...
            if not validation_result.get("compliant", True):
                self.logger.warning(
                    f"Header cultural validation issues: {validation_result.get('issues', [])}"
                )
    ...
    async def render_command_area(self) -> Panel:
        ...
        command_text = Text(
            "\n".join(history_lines)
            if history_lines
            else "مرحباً بك في النظام الإلكتروني الحكومي"
        )
    ...
    async def render_status_bar(self) -> Panel:
        ...
        mode_text = (
            "تفاعلي"
            if self.current_mode == TerminalMode.INTERACTIVE
            else self.current_mode.value
        )
        status_items.append(
            f"النمط: {mode_text}"
            if self.session.language_preference == "ar"
        ...
        if self.enable_cultural_validation:
            compliance_status = (
                "✓ ملتزم"
                if self.session.cultural_context.get("compliant", True)
                else "⚠ يتطلب مراجعة"
            )
        ...
        return Panel(
            Text(status_text, style="white on blue"), style="blue", padding=(0, 1)
        )
    ...
    async def _validate_terminal_content(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            islamic_result = await self.islamic_checker.check_compliance(
                content, check_level="government"
            )
    ...
    async def process_command(self, command: str) -> Dict[str, Any]:
        ...
        if not validation_result.get("compliant", True):
            return {
                "status": "error",
                "message": "الأمر لا يتوافق مع المعايير الثقافية"
                if self.session.language_preference == "ar"
                else "Command violates cultural standards",
        ...
        self.logger.info(
            f"Command executed: {command[:50]}... - Status: {result.get('status', 'unknown')}"
        )
    ...
    async def _execute_command(self, command: str) -> Dict[str, Any]:
        ...
        if cmd in ["help", "مساعدة"]:
            ...
            return {
                "status": "error",
                "message": f"أمر غير معروف: {cmd}"
                if self.session.language_preference == "ar"
                else f"Unknown command: {cmd}",
                "suggestion": "استخدم 'مساعدة' لعرض الأوامر المتاحة"
                if self.session.language_preference == "ar"
                else "Use 'help' to see available commands",
            }
...
- Cultural Compliance: {status_info["cultural_compliance"]}
    ...
    async def _show_prayer_times(self) -> Dict[str, Any]:
        ...
        if not self.prayer_times_enabled:
            message = (
                "أوقات الصلاة غير مفعلة"
                if self.session.language_preference == "ar"
                else "Prayer times not enabled"
            )
        ...
        if prayer_schedule:
            title = (
                "أوقات الصلاة لليوم:"
                if self.session.language_preference == "ar"
                else "Today's Prayer Times:"
            )
            return {"status": "success", "message": f"{title}\n{prayer_schedule}"}
        else:
            message = (
                "غير قادر على جلب أوقات الصلاة"
                if self.session.language_preference == "ar"
...
class IraqiTerminalSessionFactory:
    ...
    def create_government_session(
        ...
        session_id = (
            f"IQ-{ministry.value.upper()}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
        )
...
async def main():
    ...
    test_commands = ["help", "status", "prayer", "ministry", "security", "cultural"]
