...
class PermissionRule:
    ...
    cultural_policy: CulturalPermissionPolicy = field(
        default_factory=CulturalPermissionPolicy
    )
    islamic_compliance_required: bool = True
    ...
    def evaluate_permission(
        ...
        if not self._check_permission_level(employee):
            violations.append(
                f"Insufficient permission level: required {self.required_permission_level.value}"
            )

        # Check security clearance
        if not employee.has_security_clearance_for(self.required_security_clearance):
            violations.append(
                f"Insufficient security clearance: required {self.required_security_clearance.value}"
            )
        ...
        if self.allowed_ministries and not any(
            employee.has_ministry_access(ministry)
            for ministry in self.allowed_ministries
        ):
        ...
        if self.allowed_professional_domains:
            if not any(
                domain in employee.professional_domains
                for domain in self.allowed_professional_domains
            ):
        ...
        if context.get("content") and self.islamic_compliance_required:
            cultural_check = self.cultural_policy.validate_cultural_compliance(
                context["content"], context.get("context_type", "general")
            )
        ...
        return {
            ...
            "cultural_compliant": context.get("content") is None
            or self.cultural_policy.validate_cultural_compliance(
                context.get("content", ""), context.get("context_type", "general")
            )["compliant"],
        }
    ...
    def _check_permission_level(self, employee: GovernmentEmployee) -> bool:
        ...
        if employee.permission_level in [
            PermissionLevel.SYSTEM_ADMIN,
        ...
        if (
            self.required_permission_level in level_hierarchy
            and employee.permission_level in level_hierarchy
        ):
    ...
    def _check_time_access(self) -> bool:
        ...
        if (
            current_hour < self.allowed_hours_start
            or current_hour > self.allowed_hours_end
        ):
            return False
...
class IraqiGovernmentPermissionSystem:
    ...
    def __init__(self, enable_cultural_validation: bool = True):
        ...
        self.cultural_validator = (
            IraqiCulturalValidator() if enable_cultural_validation else None
        )
        self.islamic_checker = (
            IslamicComplianceChecker() if enable_cultural_validation else None
        )
    ...
    async def revoke_permission(
        self, employee_id: str, permission: str, revoked_by: str, reason: str
    ) -> bool:
    ...
    async def _validate_content_culturally(
        ...
        try:
            ...
            cultural_result = await self.cultural_validator.validate_content(
                content, context_type=context_type, check_level="government"
            )
    ...
    async def _check_default_permissions(
        ...
        if (
            ...
            and employee.permission_level != PermissionLevel.CITIZEN
        ):
            return {
                "granted": True,
    ...
    async def _log_permission_check(
        ...
        if decision["granted"]:
            self.logger.info(
                f"Permission granted: {employee.employee_id} - {decision['action']}"
            )
        else:
            self.logger.warning(
                f"Permission denied: {employee.employee_id} - {decision['action']}"
            )

    def _log_audit_event(
        self, event_type: str, employee_id: str, details: Dict[str, Any]
    ):
        """Log audit event"""
        audit_entry = {
    ...
    async def get_employee_permissions(self, employee_id: str) -> Dict[str, Any]:
        ...
        effective_permissions = {
            ...
            "temporary_permissions": {
                perm: exp.isoformat()
                for perm, exp in employee.temporary_permissions.items()
                if exp > datetime.now()
            },
            "role_based_access": {
                "permission_level": employee.permission_level.value,
                ...
                "professional_domains": [
                    domain.value for domain in employee.professional_domains
                ],
            },
        }
    ...
    async def get_audit_report(
        ...
        stats = {
            ...
            "permission_checks": len(
                [e for e in filtered_logs if e["event_type"] == "permission_check"]
            ),
            "permissions_granted": len(
                [
                    ...
                    and e.get("decision", {}).get("granted", False)
                ]
            ),
            "permissions_denied": len(
                [
                    ...
                    for e in filtered_logs
                    if e["event_type"] == "permission_check"
                    and not e.get("decision", {}).get("granted", True)
                ]
            ),
...
async def main():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        if result.get("cultural_compliance"):
            ...
            print(
                f"   Cultural Compliance: {'✅' if compliance['compliant'] else '❌'}"
            )
            if compliance.get("issues"):
                print(f"   Cultural Issues: {compliance['issues']}")

    # Get employee permissions summary
    permissions_summary = await permission_system.get_employee_permissions(
        employee.employee_id
    )
    print(f"\n📋 Permissions Summary for {employee.full_name}:")
    print(
        f"   Permission Level: {permissions_summary['permissions']['role_based_access']['permission_level']}"
    )
    print(
        f"   Security Clearance: {permissions_summary['permissions']['role_based_access']['security_clearance']}"
    )
    print(
        f"   Ministry: {permissions_summary['permissions']['role_based_access']['ministry']}"
    )
    print(
        f"   Account Valid: {'✅' if permissions_summary['account_status']['valid'] else '❌'}"
    )
    ...
    print(
        f"   Permissions Granted: {audit_report['statistics']['permissions_granted']}"
    )
    print(f"   Permissions Denied: {audit_report['statistics']['permissions_denied']}")
    print(
        f"   Cultural Violations: {audit_report['statistics']['cultural_violations']}"
    )
