...
async def passport_renewal_example():
    ...
    try:
        ...
        print(f"   LLM Provider: {type(agent.llm).__name__ if hasattr(agent, 'llm') else 'Default'}")
...
async def university_application_example():
    ...
    logger.info(f"🕌 Cultural score threshold: {agent.settings.cultural_validation_threshold}")
    logger.info(f"☪️ Islamic compliance threshold: {agent.settings.islamic_compliance_threshold}")
