...
class IraqiAIChatModel(IraqiChatModelMixin, BaseChatModel):
    ...
    async def _invoke_cultural_validator(self, content: str) -> tuple[float, list[str]]:
        ...
        try:
            ...
            warnings = (
                ["Minor cultural sensitivity improvement needed"]
                if score < 0.95
                else []
            )
    ...
    async def _invoke_islamic_compliance_checker(
        self, content: str
    ) -> tuple[float, bool]:
        """
    ...
    async def _calculate_iraqi_metrics(
        ...
        political_neutrality = (
            0.90  # Would be calculated by political neutrality checker
        )

        validation_passed = (
            cultural_score >= self.cultural_validation_threshold
            and islamic_score >= self.islamic_compliance_threshold
            and political_neutrality >= self.political_neutrality_threshold
        )
    ...
    async def ainvoke(
        ...
        try:
            if hasattr(self.underlying_model, "ainvoke"):
                completion = await self.underlying_model.ainvoke(
                    processed_messages, output_format
                )
            else:
        ...
        iraqi_metrics = await self._calculate_iraqi_metrics(
            processed_messages, response_content
        )
