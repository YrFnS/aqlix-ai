...
class SchemaOptimizer:
    ...
    def create_optimized_json_schema(model_class: Type[BaseModel]) -> Dict[str, Any]:
        ...
        optimized_schema = SchemaOptimizer._add_iraqi_ai_enhancements(optimized_schema, model_class)
        optimized_schema = SchemaOptimizer._optimize_for_llm_consumption(optimized_schema)
        ...
        if 'properties' in optimized:
            for prop_name, prop_schema in optimized['properties'].items():
                ...
                if 'description' not in prop_schema:
                    prop_schema['description'] = SchemaOptimizer._generate_field_description(prop_name)
                
                # Optimize string fields for Arabic content
                if prop_schema.get('type') == 'string':
                    prop_schema = SchemaOptimizer._optimize_string_field(prop_name, prop_schema)
    ...
    @staticmethod
    def _add_iraqi_ai_enhancements(schema: Dict[str, Any], model_class: Type[BaseModel]) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def _optimize_string_field(field_name: str, field_schema: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        arabic_field_indicators = ['arabic', 'text', 'content', 'message', 'description', 'title']
        if any(indicator in field_name.lower() for indicator in arabic_field_indicators):
    ...
    @staticmethod
    def _enhance_field_for_iraqi_ai(field_name: str, field_schema: Dict[str, Any], field_type: Any) -> Dict[str, Any]:
        """
        ...
        enhanced['$iraqi_validation'] = {
            'cultural_check_required': SchemaOptimizer._requires_cultural_validation(field_name),
            'arabic_processing_needed': SchemaOptimizer._needs_arabic_processing(field_name),
            'islamic_compliance_check': SchemaOptimizer._requires_islamic_compliance(field_name)
        }
        ...
        if SchemaOptimizer._is_professional_field(field_name):
            enhanced['$professional_domain'] = {
                ...
                'medical_relevance': 'medical' in field_name.lower() or 'health' in field_name.lower(),
                'educational_relevance': 'education' in field_name.lower() or 'academic' in field_name.lower()
            }
    ...
    def _is_iraqi_ai_field(field_name: str, field_schema: Dict[str, Any]) -> bool:
        ...
        iraqi_field_names = [
            'cultural_validation_score', 'islamic_compliance', 'arabic_content_detected',
            'iraqi_dialect_confidence', 'political_neutrality_score', 'professional_domain'
        ]
    ...
    def _requires_cultural_validation(field_name: str) -> bool:
        ...
        validation_required_fields = [
            'content', 'message', 'text', 'description', 'comment', 'title', 'subject'
        ]
        return any(keyword in field_name.lower() for keyword in validation_required_fields)
    
    @staticmethod 
    def _needs_arabic_processing(field_name: str) -> bool:
        """Check if field needs Arabic text processing."""
        arabic_processing_fields = [
            'text', 'content', 'message', 'title', 'description', 'name', 'subject'
        ]
        return any(keyword in field_name.lower() for keyword in arabic_processing_fields)
    ...
    def _requires_islamic_compliance(field_name: str) -> bool:
        ...
        compliance_required_fields = [
            'content', 'message', 'description', 'instruction', 'advice', 'recommendation'
        ]
        return any(keyword in field_name.lower() for keyword in compliance_required_fields)
    
    @staticmethod
    def _is_professional_field(field_name: str) -> bool:
        """Check if field relates to professional domains."""
        professional_keywords = [
            'legal', 'medical', 'health', 'education', 'academic', 'government',
            'ministry', 'department', 'official', 'professional', 'clinical'
        ]
