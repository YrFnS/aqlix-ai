...
from ..messages import BaseMessage, ContentPartTextParam, ContentPartImageParam, ContentPartRefusalParam
...
class OpenAIMessageSerializer:
    ...
    def _serialize_single_message(message: BaseMessage) -> Dict[str, Any]:
        ...
        if message.role == "user" and (
            message.arabic_content_detected or 
            message.cultural_validation_score > 0.0 or
            message.iraqi_dialect_confidence > 0.0
        ):
    ...
    @staticmethod
    def _serialize_content(content: Union[str, List[Union[ContentPartTextParam, ContentPartImageParam, ContentPartRefusalParam]]]) -> Union[str, List[Dict[str, Any]]]:
        """
        ...
        for part in content:
            if isinstance(part, ContentPartTextParam):
                ...
                openai_content.append({
                    "type": "text", 
                    "text": f"[Content refused: {part.reason}]"
                })
    ...
    def _create_cultural_context(message: BaseMessage) -> str:
        ...
        if message.iraqi_dialect_confidence > 0.7:
            context_parts.append(f"Iraqi dialect confidence: {message.iraqi_dialect_confidence:.1%}")
            
        if message.cultural_validation_score > 0.0:
            context_parts.append(f"Cultural validation score: {message.cultural_validation_score:.1%}")
