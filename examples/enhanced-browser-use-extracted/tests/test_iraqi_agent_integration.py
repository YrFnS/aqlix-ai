...
class TestIraqiEnhancedAgent:
    ...
    def mock_cultural_validator(self):
        ...
        validator.validate_task = AsyncMock(
            return_value=CulturalValidationResult(
                is_valid=True, compliance_score=0.98, islamic_compliance_score=0.96
            )
        )
        validator.validate_actions = AsyncMock(
            return_value=CulturalValidationResult(
                is_valid=True, compliance_score=0.97, islamic_compliance_score=0.95
            )
        )
        return validator
    ...
    def mock_arabic_processor(self):
        ...
        processor.analyze_text = AsyncMock(
            return_value={"has_arabic": False, "rtl_content": False, "dialect": None}
        )
        return processor
    ...
    def test_agent_initialization_with_cultural_features(self, basic_agent_config):
        ...
        with (
            patch("agent.service.CulturalValidator"),
    ...
    @pytest.mark.asyncio
    async def test_cultural_validation_pipeline(
        self, basic_agent_config, mock_cultural_validator
    ):
        """Test cultural validation is properly integrated"""
        with (
            patch(
                "agent.service.CulturalValidator", return_value=mock_cultural_validator
            ),
            patch("agent.service.ArabicProcessor"),
            patch("agent.service.CulturalWatchdog"),
            patch("agent.service.IraqiTelemetryService"),
            ...
            mock_cultural_validator.validate_task.assert_called_once_with(
                task="Test task", islamic_compliance=True
            )

    @pytest.mark.asyncio
    async def test_arabic_processing_integration(
        self, basic_agent_config, mock_arabic_processor
    ):
    ...
    def test_factory_government_portal_agent(self):
        ...
        with (
            ...
            agent = IraqiAgentFactory.create_government_portal_agent(
                task="Navigate passport renewal", portal_type="passport_services"
            )
...
class TestMessageManagerIntegration:
    ...
    def mock_cultural_context(self):
        ...
        context = Mock()
        context.get_current_context = AsyncMock(
            return_value={
    ...
    def test_message_manager_initialization(self):
        ...
        with (
            patch("agent.message_manager.service.IraqiCulturalContext"),
            patch("agent.message_manager.service.ArabicProcessor"),
        ):
            manager = MessageManager(
                cultural_context=True, arabic_processing=True, portal_context=True
            )
    ...
    async def test_cultural_context_message_generation(self, mock_cultural_context):
        ...
        with (
            patch(
                "agent.message_manager.service.IraqiCulturalContext",
                return_value=mock_cultural_context,
            ),
            patch("agent.message_manager.service.ArabicProcessor"),
        ):
            ...
            cultural_message = await manager._create_cultural_context_message(
                agent_state
            )
...
class TestCompatibilityWithExistingSystem:
    ...
    def test_agent_preserves_existing_interfaces(self):
        ...
        with (
            ...
            agent = IraqiEnhancedAgent(task="Test task", cultural_compliance=True)
