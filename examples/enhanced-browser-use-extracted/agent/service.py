...
class IraqiEnhancedAgent(Generic[Context, AgentStructuredOutput]):
        ...
        **kwargs
        ...
        if iraqi_portal_mode:
            self.portal_agent = IraqiPortalAgent(
                portal_type=government_portal_type
            )
    ...
    async def run(self, max_steps: int | None = None) -> AgentHistoryList:
        ...
        if self.cultural_compliance:
            ...
            if not cultural_validation.is_valid:
                self._logger.error(f"❌ Cultural validation failed: {cultural_validation.reason}")
                return self._create_error_history("Cultural validation failed")
        
        try:
            while not self.agent_state.stopped and (max_steps is None or step_count < max_steps):
                step_count += 1
                ...
                if self._step_failed(step_result):
                    ...
                    if self.agent_state.consecutive_failures >= self.settings.max_failures:
    ...
    async def _execute_step(self) -> AgentHistory:
        ...
        try:
            ...
            if self.cultural_compliance and agent_output:
                validation_result = await self._validate_actions_culturally(agent_output.action)
            ...
            if self.arabic_processing and agent_output:
                agent_output = await self._enhance_with_arabic_processing(agent_output, action_results)
    ...
    async def _validate_task_culturally(self, task: str) -> CulturalValidationResult:
        ...
        return await self.cultural_validator.validate_task(
            task=task,
            islamic_compliance=self.islamic_values_compliance
        )
    
    async def _validate_actions_culturally(self, actions: list) -> CulturalValidationResult:
    ...
    async def _enhance_with_arabic_processing(self, agent_output: AgentOutput, action_results: list) -> AgentOutput:
        ...
        for result in action_results:
            if hasattr(result, 'text') and self.arabic_processor.contains_arabic(result.text):
                result.arabic_metadata = await self.arabic_processor.analyze_text(result.text)
                result.text = await self.arabic_processor.enhance_rtl_display(result.text)
            enhanced_results.append(result)
        
        return agent_output
    
    async def _generate_agent_output(self, browser_state: BrowserStateSummary) -> AgentOutput:
    ...
    def _create_step_metadata(self, start_time: float, end_time: float) -> AgentStepInfo:
    ...
    def _create_cultural_error_step(self, validation_result: CulturalValidationResult) -> AgentHistory:
    ...
    async def _finalize_execution(self, history: AgentHistoryList, execution_time: float):
        """Finalize agent execution"""
        self._logger.info(f"🏁 Agent execution completed in {execution_time:.2f} seconds")
        
        # Cultural compliance reporting
        if self.cultural_compliance:
            compliance_report = await self.cultural_validator.generate_compliance_report(history)
            self._logger.info(f"📊 Cultural compliance: {compliance_report.compliance_percentage:.1f}%")
...
class IraqiAgentFactory:
    ...
    @staticmethod  
    def create_cultural_validation_agent(
        task: str,
        **kwargs
    ) -> IraqiEnhancedAgent:
