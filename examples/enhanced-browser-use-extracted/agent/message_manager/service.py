...
class MessageManager:
    ...
    async def get_messages(
        ...
        system_message = await self._create_enhanced_system_message(agent_state, settings)
...
- Maintain data privacy and security standards"""
        ...
        full_prompt = base_prompt + cultural_requirements + arabic_instructions + portal_instructions
...
Page Load Status: {browser_state.load_status if hasattr(browser_state, 'load_status') else 'Unknown'}"""
        ...
        if self.arabic_processing_enabled and hasattr(browser_state, 'has_arabic_content'):
...
Dialect Detected: {agent_state.dialect_detected or 'None'}"""
    ...
    async def _get_optimized_history_messages(
        ...
        for item in recent_history:
            ...
            if item.agent_output:
                agent_content = await self._format_agent_output_for_history(item.agent_output)
                messages.append(AssistantMessage(content=agent_content))
            
            # User message (action results)
            if item.action_results:
                results_content = await self._format_action_results_for_history(item.action_results)
    ...
    async def _format_agent_output_for_history(
        ...
        if isinstance(agent_output, IraqiAgentOutput):
            if agent_output.cultural_assessment:
                content_parts.append(f"Cultural Assessment: {agent_output.cultural_assessment}")
            
            if agent_output.language_detection:
                content_parts.append(f"Language Detection: {agent_output.language_detection}")
    ...
    async def _format_action_results_for_history(
        ...
        for i, result in enumerate(action_results, 1):
            ...
            if isinstance(result, IraqiActionResult) and result.cultural_validation_result:
                validation = result.cultural_validation_result
                result_summary += f" (Cultural Score: {validation.compliance_score:.2f})"
    ...
    def get_conversation_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "successful_steps": sum(1 for item in self.history if item.was_successful()),
            "failed_steps": sum(1 for item in self.history if not item.was_successful()),
