...
class IraqiAgentOutput(AgentOutput):
    ...
    def model_json_schema(cls, **kwargs):
        ...
        schema["required"].extend(
            ["cultural_assessment", "islamic_values_consideration"]
        )
        return schema
...
class AgentHistoryList(BaseModel):
    ...
    def failed_steps(self) -> int:
        ...
        return sum(
            1 for step in self.steps if step.error or not self._step_successful(step)
        )
    ...
    def get_cultural_compliance_report(self) -> dict[str, Any]:
        ...
        for step in self.steps:
            if step.cultural_validation_result:
                ...
                islamic_scores.append(
                    step.cultural_validation_result.islamic_compliance_score
                )
                if step.cultural_validation_result.violations:
                    violations.extend(step.cultural_validation_result.violations)

        return {
            "average_cultural_compliance": sum(cultural_scores) / len(cultural_scores)
            if cultural_scores
            else 1.0,
            ...
            "steps_analyzed": len(
                [s for s in self.steps if s.cultural_validation_result]
            ),
        }
