...
class IslamicComplianceWatchdog(BaseWatchdog):
    ...
    compliance_level: IslamicComplianceLevel = Field(
        default=IslamicComplianceLevel.MODERATE
    )
    ...
    prohibited_text_patterns: List[str] = Field(
    ...
    questionable_text_patterns: List[str] = Field(
        default_factory=lambda: [
    ...
    positive_islamic_patterns: List[str] = Field(
        default_factory=lambda: [
    ...
    prayer_times: Dict[str, str] = Field(
        default_factory=lambda: {
            "fajr": "05:30",
            "dhuhr": "12:30",
            "asr": "15:45",
    ...
    halal_business_categories: Set[str] = Field(
        default_factory=lambda: {
            "food_halal",
            "islamic_finance",
            "islamic_education",
    ...
    async def on_FormSubmissionEvent(self, event: FormSubmissionEvent) -> None:
        ...
        try:
            ...
            await self.emit_error(
                f"Form submission Islamic validation failed: {str(e)}"
            )

    async def _validate_page_islamic_compliance(
        ...
        try:
            ...
            if self.enable_halal_verification:
                ...
                if halal_score > 0.8:
                    compliance_result["positive_indicators"].append(
                        {"type": "halal_business", "score": halal_score}
                    )
            ...
            compliance_result["overall_score"] = max(
                0.0, min(1.0, base_score - violation_penalty + positive_bonus)
            )
            ...
            await self.emit_error(
                f"Page Islamic compliance validation failed: {str(e)}"
            )
            return {"overall_score": 0.5, "violations": [], "error": str(e)}

    async def _validate_text_content(
        self, text: str, url: str, element_info: Dict
    ) -> Dict:
        ...
        if positive_matches:
            ...
            if result["compliance_level"] in [
                ...
                IslamicComplianceLevel.PERMISSIBLE,
            ]:
    ...
    async def _validate_image_content(
        self, image_url: str, alt_text: str, page_url: str
    ) -> Dict:
    ...
    async def _validate_media_content(
        self, media_url: str, media_type: str, page_url: str
    ) -> Dict:
        ...
        # Video content validation
            ...
            if any(
                term in media_url.lower()
                for term in ["adult", "explicit", "uncensored"]
            ):
                result["violations"].append(
                    {
    ...
    async def _validate_form_submission(
        self, form_data: Dict, form_action: str, page_url: str
    ) -> Dict:
        """Validate form submission for Islamic compliance."""
        result = {"is_compliant": True, "violations": [], "form_action": form_action}
    ...
    async def _check_prayer_time_notifications(
        self, current_time: datetime, url: str
    ) -> None:
    ...
    async def emit_islamic_content_violation(
        self, url: str, content_type: ContentType, violation_data: Dict
    ):
        ...
        if self.cultural_validation_enabled:
            ...
            event = IslamicContentViolationEvent(
                data={
                    "url": url,
                    ...
                    "severity": "high"
                    if violation_data.get("compliance_level") == "prohibited"
                    else "medium",
                }
            )
            self.event_bus.dispatch(event)

    async def emit_islamic_positive_content(
        self, url: str, content_type: ContentType, positive_score: float
    ):
        """Emit positive Islamic content detection event."""
        if self.cultural_validation_enabled:
            from browser_use.agent.events import IslamicPositiveContentEvent

            event = IslamicPositiveContentEvent(
                data={
                    "url": url,
                    "content_type": content_type.value,
    ...
    async def emit_prayer_time_notification(
        self, prayer_name: str, prayer_time: str, url: str
    ):
        """Emit prayer time notification event."""
        if self.enable_prayer_time_awareness:
            from browser_use.agent.events import PrayerTimeNotificationEvent

            event = PrayerTimeNotificationEvent(
                data={
