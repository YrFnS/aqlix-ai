...
class IraqiCulturalWatchdog(BaseWatchdog):
    ...
    # Cultural patterns
    inappropriate_content_patterns: List[str] = Field(default_factory=lambda: [
        # Sectarian content
        'sunni vs shia', 'sectarian conflict', 'religious division',
        # Political bias
        'political party', 'partisan politics', 'political bias',
        # Cultural insensitivity
        'cultural stereotype', 'ethnic discrimination', 'tribal conflict',
        # Religious offense
        'religious mockery', 'blasphemy', 'religious insult'
    ])
    
    islamic_compliance_patterns: List[str] = Field(default_factory=lambda: [
        # Prohibited content
        'gambling', 'alcohol promotion', 'haram content',
        'inappropriate imagery', 'non-halal business',
        # Financial compliance
        'interest-based lending', 'riba', 'usury'
    ])
    
    iraqi_cultural_markers: Dict[str, float] = Field(default_factory=lambda: {
    ...
    async def on_NavigationCompleteEvent(self, event) -> None:
        ...
        try:
            ...
            if content_analysis['cultural_score'] < self.cultural_validation_threshold:
                await self._handle_cultural_violation(url, content_analysis, 'low_cultural_score')
            
            # Validate Islamic compliance
            if content_analysis['islamic_compliance_score'] < self.islamic_compliance_threshold:
                await self._handle_cultural_violation(url, content_analysis, 'islamic_non_compliance')
            
            # Check political neutrality
            if content_analysis['political_neutrality_score'] < self.political_neutrality_threshold:
                await self._handle_cultural_violation(url, content_analysis, 'political_bias')
    ...
    async def on_InputEvent(self, event) -> None:
        ...
        try:
            ...
            if input_text and len(input_text.strip()) > 10:  # Only analyze substantial input
    ...
    async def _analyze_page_content(self, url: str, event) -> Dict[str, any]:
        ...
        analysis = {
            ...
            'islamic_compliance_score': await self._calculate_islamic_compliance_score(page_content),
            'political_neutrality_score': await self._calculate_political_neutrality_score(page_content),
    ...
    async def _calculate_political_neutrality_score(self, content: str) -> float:
        ...
        political_bias_patterns = [
            'partisan', 'political party', 'sectarian', 'tribal politics',
            'government criticism', 'political propaganda'
        ]
        ...
        neutral_patterns = [
            'objective', 'factual', 'informational', 'educational'
        ]
    ...
    def _identify_professional_domain(self, content: str) -> str:
        ...
        domain_patterns = {
            ...
            'educational': ['تعليم', 'جامعة', 'education', 'university', 'school', 'academic'],
    ...
    def _analyze_language_distribution(self, content: str) -> Dict[str, float]:
        ...
        total_chars = len(re.findall(r'[a-zA-Z\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]', content))
    ...
    async def _analyze_content_snippet(self, content: str) -> Dict[str, any]:
        ...
        """
        return {
            'requires_validation': len(content) > 50 and (
                self.is_arabic_rtl_content(content) or
                self.is_iraqi_content_detected(content) or
                any(pattern.lower() in content.lower() for pattern in self.inappropriate_content_patterns)
            ),
            'cultural_risk_score': await self._calculate_cultural_score(content),
            'content_type': 'user_generated' if self._appears_user_generated(content) else 'page_content'
        }
    ...
    async def _analyze_user_input(self, input_text: str) -> Dict[str, any]:
        ...
        analysis = {
            ...
            'islamic_compliance_score': await self._calculate_islamic_compliance_score(input_text),
        ...
        for pattern in self.inappropriate_content_patterns:
            if pattern.lower() in input_lower:
                ...
                analysis['severity'] = 'high' if len(analysis['violation_types']) > 2 else 'medium'
    ...
    async def _handle_cultural_violation(self, url: str, analysis: Dict, violation_type: str) -> None:
        """
    ...
    async def _handle_input_violation(self, input_text: str, analysis: Dict, event) -> None:
        """
    ...
    async def _validate_content_snippet(self, url: str, content: str, analysis: Dict) -> None:
        """
    ...
    async def _process_arabic_content(self, url: str, analysis: Dict) -> None:
        ...
        self._arabic_processing_stats['total_processed'] = self._arabic_processing_stats.get('total_processed', 0) + 1
    ...
    def _calculate_violation_severity(self, analysis: Dict, violation_type: str) -> str:
        ...
        if (analysis.get('cultural_score', 1.0) < 0.5 or
            analysis.get('islamic_compliance_score', 1.0) < 0.5):
            base_severity = 'high'
        
        # Critical severity for multiple violations
        if (analysis.get('cultural_score', 1.0) < 0.3 and
            analysis.get('islamic_compliance_score', 1.0) < 0.3):
            base_severity = 'critical'
        
        # Lower severity for minor issues
        if (analysis.get('cultural_score', 1.0) > 0.8 and
            violation_type in ['dynamic_content_violation', 'user_input_violation']):
            base_severity = 'low'
    ...
    async def _cleanup_analysis_cache(self) -> None:
        ...
        expired_keys = [
            key for key, analysis in self._content_analysis_cache.items()
