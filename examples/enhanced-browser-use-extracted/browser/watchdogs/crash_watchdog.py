...
from ..watchdog_base import BaseWatchdog, IraqiSecurityThreatEvent
...
class NetworkRequestTracker:
    ...
    def __init__(
        ...
        self.expected_timeout = (
            15.0 if self.is_iraqi_portal else 10.0
        )  # Longer timeout for Iraqi portals
...
class IraqiCrashWatchdog(BaseWatchdog):
    ...
    network_timeout_seconds: float = Field(
        default=12.0
    )  # Extended for Iraqi network conditions
    iraqi_portal_timeout_seconds: float = Field(
        default=20.0
    )  # Even longer for government portals
    ...
    async def on_BrowserConnectedEvent(self, event) -> None:
        ...
        if not self._monitoring_task:
            self._monitoring_task = asyncio.create_task(
                self._enhanced_monitoring_loop()
            )
            ...
            if current_time - last_check > check_interval:
                try:
                    ...
                    if not health_status.get("healthy", True):
                        await self._handle_portal_health_issue(
                            portal_url, health_status
                        )

                except Exception as e:
                    self.logger.error(
                        f"Error checking Iraqi portal health {portal_url}: {e}"
                    )
    ...
    async def _check_system_performance(self) -> None:
        ...
        try:
            ...
            performance_data = {
                ...
                "healthy": (
                    process_memory_mb <= self.memory_usage_threshold_mb
                    and cpu_percent <= self.cpu_usage_threshold_percent
                ),
            }
    ...
    async def _check_cultural_validation_health(self) -> None:
        ...
        try:
            ...
            if not validation_healthy:
                self.logger.warning(
                    "⚠️ Cultural validation service health issue detected"
                )
    ...
    async def _initialize_iraqi_portal_monitoring(self) -> None:
        ...
        self.logger.info(
            f"🇮🇶 Initialized monitoring for {len(iraqi_portals)} Iraqi portals"
        )
    ...
    async def _complete_portal_monitoring(self, url: str, event) -> None:
        ...
        if url in self._iraqi_portal_health:
            ...
            navigation_time = time.time() - portal_data.get(
                "navigation_start_time", time.time()
            )
    ...
    async def _perform_portal_health_check(self, portal_url: str) -> Dict[str, any]:
        ...
        try:
            ...
            return {
                ...
                else -1,  # Simulate Iraqi network latency
    ...
    async def _handle_network_failure(self, tracker: NetworkRequestTracker) -> None:
        ...
        if tracker.is_iraqi_portal:
            if tracker.url in self._iraqi_portal_health:
                ...
                portal_data["consecutive_failures"] = (
                    portal_data.get("consecutive_failures", 0) + 1
                )
                portal_data["last_failure_time"] = time.time()

                # Trigger recovery if enabled
                if (
                    self.enable_automatic_recovery
                    and portal_data["consecutive_failures"]
                    >= self.max_consecutive_failures
                ):
                    await self._attempt_portal_recovery(tracker.url)

    async def _handle_portal_health_issue(
        self, portal_url: str, health_status: Dict
    ) -> None:
        """
    ...
    async def _attempt_portal_recovery(self, portal_url: str) -> None:
        ...
        if current_attempts >= 3:  # Max 3 recovery attempts
            self.logger.warning(
                f"🚫 Maximum recovery attempts reached for {portal_url}"
            )
            return
        ...
        self.logger.info(
            f"🔄 Attempting portal recovery for {portal_url} (attempt {current_attempts + 1})"
        )
    ...
    async def _attach_to_target(self, target_id: str, url: str) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"📎 Attached crash monitoring to target {target_id}: {url}"
            )
            ...
            if self.is_iraqi_government_url(url):
                portal_type = self._classify_iraqi_portal(url)
                self.logger.info(
                    f"🇮🇶 Monitoring {portal_type} portal target: {target_id}"
                )
