...
class IraqiSecurityWatchdog(BaseWatchdog):
    ...
    iraqi_portal_whitelist: List[str] = Field(default_factory=lambda: [
        '*.gov.iq',
        '*.iraq.gov.iq',
        '*.cabinet.iq',
        '*.cbi.iq',
    ...
    cultural_threat_patterns: List[str] = Field(default_factory=lambda: [
        'sectarian',
        'political bias',
    ...
    def _match_domain_pattern(self, host: str, pattern: str, scheme: str) -> bool:
        ...
        if '*' in pattern:
            ...
            return host == pattern or (pattern.startswith('.') and host.endswith(pattern))
    ...
    async def _calculate_threat_score(self, url: str) -> float:
        ...
        if host in self._threat_history:
            recent_threats = [
                threat for threat in self._threat_history[host]
    ...
    async def _validate_iraqi_portal_access(self, url: str, event) -> None:
        ...
        if not validation_result['allowed']:
            ...
            if self.block_non_compliant_content:
                raise ValueError(f'Access blocked to Iraqi portal: {validation_result["reason"]}')
    ...
    async def _perform_portal_validation(self, url: str, portal_type: str) -> Dict[str, any]:
        """
    ...
    async def _handle_blocked_navigation(self, url: str, reason: str) -> None:
        ...
        """
        self.logger.warning(f'⛔️ Blocking navigation to disallowed URL: {url} (Reason: {reason})')
        ...
        raise ValueError(f'Navigation to {url} blocked by Iraqi security policy: {reason}')
    ...
    async def _handle_security_threat(self, url: str, threat_score: float, event) -> None:
        """
        ...
        Args:
            ...
            event: Associated event
        """
        self.logger.warning(f'🚨 Security threat detected: {url} (Score: {threat_score:.2f})')
    ...
    async def _update_threat_tracking(self, url: str, activity_type: str) -> None:
        ...
        for tracked_host in list(self._threat_history.keys()):
            self._threat_history[tracked_host] = [
                threat for threat in self._threat_history[tracked_host]
