...
class IraqiPerformanceWatchdog(BaseWatchdog):
    ...
    iraqi_portal_load_threshold_ms: float = Field(
        default=5000.0, gt=0.0
    )  # Higher threshold for government portals
    ...
    async def on_NavigateToUrlEvent(self, event) -> None:
        ...
        performance_context = {
            ...
            "expected_threshold": (
                self.iraqi_portal_load_threshold_ms
    ...
    async def _update_iraqi_portal_performance(
        ...
        portal_stats["best_load_time"] = min(
            portal_stats["best_load_time"], load_time_ms
        )
        portal_stats["worst_load_time"] = max(
            portal_stats["worst_load_time"], load_time_ms
        )
    ...
    def _classify_iraqi_portal(self, url: str) -> str:
        ...
        if "gov.iq" in url_lower or "cabinet.iq" in url_lower:
            ...
            payment in url_lower for payment in ["zaincash", "fastpay", "nasswallet"]
        ):
            return "payment"
        else:
            return "other_iraqi"
    ...
    async def _provide_optimization_recommendations(self, context: Dict) -> None:
        ...
        if is_iraqi_portal:
            recommendations.append(
                "Iraqi portal detected - network latency may be factor"
            )
            ...
            if portal_type == "government":
                recommendations.append(
                    "Government portal - access during business hours (8AM-5PM)"
                )
    ...
    async def _optimize_arabic_processing(self) -> None:
        ...
        if len(recent_times) >= 5:
            ...
            if avg_time > self.arabic_processing_threshold_ms:
                ...
                self.logger.info(
                    "🎯 Arabic processing optimizations:\n  • "
                    + "\n  • ".join(optimizations)
                )
        ...
        try:
            ...
            return {
                ...
                "arabic_processing_avg_ms": (
                    mean(self._arabic_processing_times[-10:])
                    if self._arabic_processing_times
                    else 0
                ),
                "cultural_validation_avg_ms": (
                    mean(self._cultural_validation_times[-10:])
                    if self._cultural_validation_times
                    else 0
                ),
            }
        except Exception as e:
    ...
    async def _handle_high_memory_usage(self, metrics: Dict) -> None:
        ...
        if len(self._performance_history) < old_count:
            optimizations_applied.append(
                f"Performance history reduced by {old_count - len(self._performance_history)} entries"
            )
    ...
    def _cleanup_processing_times(self) -> None:
        ...
        if len(self._cultural_validation_times) > max_entries:
            self._cultural_validation_times = self._cultural_validation_times[
                -max_entries:
            ]
    ...
    def get_performance_summary(self) -> Dict[str, any]:
        ...
        summary = {
            ...
            "total_navigations_tracked": len(
                [
                    e
                    for e in self._performance_history
            ...
            "arabic_processing_stats": {
                ...
                "average_time_ms": mean(self._arabic_processing_times)
                if self._arabic_processing_times
                else 0,
                "recent_average_ms": mean(self._arabic_processing_times[-10:])
                if self._arabic_processing_times
                else 0,
            },
            "cultural_validation_stats": {
                "total_validations": len(self._cultural_validation_times),
                "average_time_ms": mean(self._cultural_validation_times)
                if self._cultural_validation_times
                else 0,
                "recent_average_ms": mean(self._cultural_validation_times[-10:])
                if self._cultural_validation_times
                else 0,
            },
