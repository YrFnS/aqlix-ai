...
from ..watchdog_base import BaseWatchdog, ArabicContentProcessedEvent
...
class ArabicContentWatchdog(BaseWatchdog):
    ...
    arabic_content_threshold: float = Field(
        default=0.1, ge=0.0, le=1.0
    )  # 10% Arabic chars minimum
    iraqi_dialect_confidence_threshold: float = Field(default=0.7, ge=0.0, le=1.0)
    mixed_content_threshold: float = Field(
    ...
    iraqi_dialect_patterns: Dict[str, float] = Field(
    ...
    arabic_typography_rules: Dict[str, str] = Field(
        default_factory=lambda: {
    ...
    _dialect_recognition_stats: Dict[str, int] = PrivateAttr(
        ...
        try:
            ...
            if page_content and self._contains_arabic_content(page_content):
                processing_result = await self._process_arabic_content(
                    content=page_content, source_url=url, content_type="page_content"
                )
    ...
    async def _process_arabic_content(
        self, content: str, source_url: str, content_type: str
    ) -> Dict[str, any]:
        """
        ...
        try:
            ...
            rtl_analysis = await self._analyze_rtl_formatting_needs(
                content, arabic_analysis
            )
            processing_result["rtl_analysis"] = rtl_analysis
            processing_result["rtl_formatting_needed"] = rtl_analysis[
                "formatting_needed"
            ]
            ...
            self._processing_performance.append(
                {
                    ...
                    "dialect_confidence": processing_result.get(
                        "dialect_analysis", {}
                    ).get("confidence", 0.0),
                }
            )
            ...
            if "dialect_analysis" in processing_result:
                dialect_type = processing_result["dialect_analysis"].get(
    ...
    async def _analyze_arabic_content(self, content: str) -> Dict[str, any]:
        ...
        arabic_percentage = (
            (arabic_char_count / total_chars) if total_chars > 0 else 0.0
        )
        ...
        return {
            ...
            "predominant_direction": "rtl"
            if arabic_percentage > 0.5
            else "mixed"
            if arabic_percentage > 0.1
            else "ltr",
        }
    ...
    async def _recognize_iraqi_dialect(self, content: str) -> Dict[str, any]:
        ...
        for pattern, confidence in self.iraqi_dialect_patterns.items():
            if pattern in content:
                ...
                match_info = {
                    ...
                    "positions": [
                        m.start() for m in re.finditer(re.escape(pattern), content)
                    ],
                }
        ...
        if dialect_matches:
            ...
            normalized_confidence = min(
                1.0, total_confidence / max(1, content_words * 0.1)
            )
        else:
    ...
    async def _analyze_mixed_content(self, content: str) -> Dict[str, any]:
        ...
        is_mixed = (
            min(arabic_percentage, english_percentage) >= self.mixed_content_threshold
        )
    ...
    async def _analyze_rtl_formatting_needs(
        self, content: str, arabic_analysis: Dict
    ) -> Dict[str, any]:
        """
        ...
        return {
            ...
            "requires_bidi_support": arabic_analysis["has_rtl_content"]
            and arabic_analysis["arabic_percentage"] < 0.9,
        }
    ...
    async def _cleanup_performance_data(self) -> None:
        ...
        self._processing_performance = [
            entry
            for entry in self._processing_performance
            ...
            "recent_performance": {
                "average_processing_time_ms": (
                    sum(
                        ...
                        for entry in self._processing_performance[-50:]
                    )
