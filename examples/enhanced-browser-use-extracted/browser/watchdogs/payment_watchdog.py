...
class IraqiPaymentWatchdog(BaseWatchdog):
    ...
    supported_gateways: Dict[PaymentGateway, Dict] = Field(default_factory=lambda: {
        PaymentGateway.ZAINCASH: {
            'domains': ['*.zaincash.iq', '*.zain.iq'],
            'api_endpoints': ['/payment/init', '/payment/verify', '/payment/callback'],
            'min_amount': 1000,  # IQD
    ...
    required_security_headers: Set[str] = Field(default_factory=lambda: {
    ...
    # Fraud Detection
    fraud_detection_rules: List[Dict] = Field(default_factory=lambda: [
        {
    ...
    def _matches_domain_pattern(self, url: str, pattern: str) -> bool:
        ...
        try:
            ...
            return domain.endswith(pattern_clean) or fnmatch.fnmatch(domain, pattern.lower())
        except Exception:
            return False
    ...
    async def on_FormSubmissionEvent(self, event: FormSubmissionEvent) -> None:
        ...
        try:
            ...
            await self.emit_error(f"Payment form submission monitoring failed: {str(e)}")
    ...
    async def on_ResponseEvent(self, event: ResponseEvent) -> None:
        ...
        try:
            ...
            if gateway != PaymentGateway.UNKNOWN:
                ...
                if response_time > self.performance_thresholds['api_response_timeout'] * 1000:
    ...
    async def _start_payment_session_monitoring(self, session_id: str, url: str, gateway: PaymentGateway) -> None:
    ...
    async def _validate_payment_security(self, url: str, gateway: PaymentGateway) -> Dict:
    ...
    async def _extract_payment_info(self, form_data: Dict, gateway: PaymentGateway) -> Optional[Dict]:
    ...
    async def _validate_payment_transaction(self, transaction: PaymentTransaction, form_data: Dict) -> Dict:
        ...
        # Security validation
        validation_results['has_security_fields'] = await self._validate_security_fields(
            form_data, transaction.gateway
        )
    ...
    async def _evaluate_fraud_rule(self, transaction: PaymentTransaction, rule: Dict) -> bool:
    ...
    async def _check_rapid_payments(self, transaction: PaymentTransaction, rule: Dict) -> bool:
    ...
    async def _check_unusual_amount(self, transaction: PaymentTransaction, rule: Dict) -> bool:
    ...
    async def _check_suspicious_location(self, transaction: PaymentTransaction, rule: Dict) -> bool:
    ...
    def _is_payment_status_response(self, url: str, status_code: int) -> bool:
        ...
        return (
            status_code in [200, 201] and 
            any(keyword in url.lower() for keyword in ['status', 'verify', 'callback', 'result'])
        )
    ...
    def _is_authentic_gateway_domain(self, url: str, gateway: PaymentGateway) -> bool:
        ...
        return any(self._matches_domain_pattern(url, domain) for domain in authentic_domains)
    
    async def _validate_security_fields(self, form_data: Dict, gateway: PaymentGateway) -> bool:
        ...
        for security_type in required_security:
            ...
            if not any(indicator in str(form_data.keys()).lower() for indicator in indicators):
                return False
    ...
    async def emit_payment_gateway_access(self, url: str, gateway: PaymentGateway):
        ...
        if self.cultural_validation_enabled:
            ...
            event = PaymentGatewayAccessEvent(data={
                'url': url,
                'gateway': gateway.value,
                'timestamp': datetime.now().isoformat()
            })
    ...
    async def emit_payment_transaction_initiated(self, transaction: PaymentTransaction):
        ...
        if self.cultural_validation_enabled:
            ...
            event = PaymentTransactionInitiatedEvent(data={
    ...
    async def emit_payment_security_violation(self, url: str, gateway: PaymentGateway, violation_data: Dict):
        """Emit payment security violation event."""
        if self.cultural_validation_enabled:
            from browser_use.agent.events import PaymentSecurityViolationEvent
            event = PaymentSecurityViolationEvent(data={
                'url': url,
                'gateway': gateway.value,
    ...
    async def emit_payment_fraud_alert(self, transaction: PaymentTransaction, fraud_rule: Dict):
        """Emit payment fraud alert event."""
        if self.cultural_validation_enabled:
            from browser_use.agent.events import PaymentFraudAlertEvent
            event = PaymentFraudAlertEvent(data={
                'transaction_id': transaction.transaction_id,
                'gateway': transaction.gateway.value,
    ...
    async def emit_payment_performance_warning(self, url: str, gateway: PaymentGateway, metric: str, value: float):
        """Emit payment performance warning event."""
        if self.cultural_validation_enabled:
            from browser_use.agent.events import PaymentPerformanceWarningEvent
            event = PaymentPerformanceWarningEvent(data={
                'url': url,
                'gateway': gateway.value,
                'metric': metric,
                'value': value,
    ...
    async def _validate_api_security(self, url: str, method: str, headers: Dict, gateway: PaymentGateway) -> Dict:
        """Validate API request security."""
        return {'is_secure': True, 'violations': []}
    
    async def _validate_response_security(self, headers: Dict, gateway: PaymentGateway) -> Dict:
        """Validate response security headers."""
        return {'is_secure': True, 'violations': []}
    
    async def _process_payment_status_update(self, url: str, status_code: int, headers: Dict, gateway: PaymentGateway) -> None:
        """Process payment status update from gateway."""
        pass
    
    async def _track_api_performance(self, url: str, gateway: PaymentGateway, request_start: datetime) -> None:
        """Track API performance metrics."""
        pass
    
    async def _periodic_security_monitoring(self, session_id: str, gateway: PaymentGateway) -> None:
