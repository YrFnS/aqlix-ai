...
class DomService:
    ...
    def __init__(
        ...
        **config,
        ...
        self.logger = logger or (
            browser_session.logger if browser_session else logging.getLogger(__name__)
        )
    ...
    def _log_performance_summary(self):
        ...
        if metrics["dom_processing_times"]:
            avg_processing_time = sum(metrics["dom_processing_times"]) / len(
                metrics["dom_processing_times"]
            )
            self.logger.info(
                f"DOM Processing Performance - Avg: {avg_processing_time:.3f}s, "
                f"Nodes: {metrics['total_nodes_processed']}, "
                f"Cross-Origin Iframes: {metrics['cross_origin_iframe_count']}"
            )

    async def _get_targets_for_page(
        self, target_id: Optional[TargetID] = None
    ) -> CurrentPageTargets:
        ...
        main_target = next(
            (t for t in targets["targetInfos"] if t["targetId"] == target_id), None
        )
        ...
        for frame_info in all_frames.values():
            ...
            if frame_info.get("isCrossOrigin") and frame_info.get("frameTargetId"):
                ...
                parent_target = frame_info.get(
                    "parentTargetId", frame_info.get("frameTargetId")
                )
                if parent_target == target_id:
                    ...
                    iframe_target = next(
                        (
                            ...
                            if t["targetId"] == frame_info["frameTargetId"]
                        ),
                        None,
                    )
                    if iframe_target:
    ...
    async def _get_viewport_ratio(self, target_id: TargetID) -> float:
        ...
        try:
            ...
            width = css_visual_viewport.get(
                "clientWidth", css_layout_viewport.get("clientWidth", 1920.0)
            )
            ...
            if self.config.get("enable_comprehensive_logging"):
                self.logger.debug(
                    f"Viewport ratio calculated: {device_pixel_ratio} (CSS: {css_width}, Device: {device_width})"
                )
    ...
    async def _get_ax_tree_for_all_frames(
        ...
        try:
            ...
            if self.config.get("enable_comprehensive_logging"):
                self.logger.debug(
                    f"Accessibility tree processing completed in {processing_time:.3f}s with {len(merged_nodes)} nodes"
                )
    ...
    async def _get_all_trees(self, target_id: TargetID) -> TargetAllTrees:
        ...
        try:
            ...
            if (
                scroll_result
                and "result" in scroll_result
                and "value" in scroll_result["result"]
            ):
        ...
        def create_snapshot_request():
            return cdp_session.cdp_client.send.DOMSnapshot.captureSnapshot(
                params={
                    ...
                    "includeBlendedBackgroundColors": self.config.get(
                        "include_background_colors", False
                    ),
                    "includeTextColorOpacities": self.config.get(
                        "include_text_colors", False
                    ),
                },
                session_id=cdp_session.session_id,
            )
        ...
        tasks = {
            ...
            "device_pixel_ratio": asyncio.create_task(
        ...
        if pending:
            ...
            retry_map = {
                tasks["snapshot"]: lambda: asyncio.create_task(
                    create_snapshot_request()
                ),
                tasks["dom_tree"]: lambda: asyncio.create_task(
                    create_dom_tree_request()
                ),
            ...
            done2, pending2 = await asyncio.wait(
                [t for t in tasks.values() if not t.done()], timeout=retry_timeout
            )
        ...
        if failed:
            ...
            if critical_failed:
                raise TimeoutError(
                    f"Critical CDP requests failed or timed out: {', '.join(critical_failed)}"
                )
        ...
        if self.config.get("enable_comprehensive_logging"):
            if snapshot and "documents" in snapshot:
                total_nodes = sum(
                    len(doc.get("nodes", [])) for doc in snapshot["documents"]
                )
                ...
                for doc_idx, doc in enumerate(snapshot["documents"]):
                    if doc_idx > 0:  # Not the main document
                        self.logger.debug(
                            f"Document {doc_idx} has {len(doc.get('nodes', []))} nodes"
                        )
    ...
    async def get_dom_tree(
        ...
        async def _construct_enhanced_node(
            ...
            snapshot_data = (
                snapshot_lookup.get(backend_node_id, None) if backend_node_id else None
            )
            ...
            if (
                node_type_value == NodeType.ELEMENT_NODE.value
                and node_name == "HTML"
            ...
            if self.enhanced_visibility_detection:
                dom_tree_node.is_visible = (
                    self.is_element_visible_according_to_all_parents(
                ...
                if frame_id:
                    try:
                        ...
                        if iframe_document_target:
                            if self.config.get("enable_comprehensive_logging"):
                                self.logger.debug(
                                    f"Processing cross-origin iframe {frame_id}"
                                )
                        ...
                        self.logger.debug(
                            f"Failed to process cross-origin iframe {frame_id}: {e}"
                        )
