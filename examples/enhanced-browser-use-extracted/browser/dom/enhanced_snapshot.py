...
class SnapshotNodeData:
    ...
    def _analyze_arabic_content(self):
        ...
        self.rtl_layout_active = (
            direction == 'rtl' or
            'rtl' in unicode_bidi or
            'vertical' in writing_mode
        )
        
        # Detect Arabic text characteristics
        font_family = self.computed_styles.get('font-family', '').lower()
        arabic_fonts = [
            'arabic', 'naskh', 'kufi', 'tahoma', 'arial unicode ms',
            'times new roman', 'amiri', 'scheherazade', 'lateef'
        ]
        
        self.arabic_text_detected = any(
            arabic_font in font_family for arabic_font in arabic_fonts
    ...
    def _assess_accessibility(self):
        ...
        try:
            ...
            font_size = float(font_size_str.replace('px', '').replace('pt', '').replace('em', ''))
            if font_size >= 14:
                score += 0.2
            elif font_size >= 12:
                score += 0.1
    ...
    try:
        ...
        for doc_index, document in enumerate(snapshot['documents']):
            ...
            for node_index, node_data in enumerate(nodes):
                ...
                scroll_rect = None
                if node_index < len(scroll_rects) and len(scroll_rects[node_index]) >= 4:
                ...
                client_rect = None
                if node_index < len(client_rects) and len(client_rects[node_index]) >= 2:
...
def _enhance_node_with_iraqi_processing(
