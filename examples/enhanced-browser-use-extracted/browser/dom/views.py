...
class DOMRect:
    ...
    def get_rtl_center(self) -> tuple[float, float]:
        ...
        return (self.rtl_adjusted_x + self.width / 2, self.rtl_adjusted_y + self.height / 2)
...
class EnhancedDOMTreeNode:
    ...
    def _analyze_iraqi_characteristics(self):
        ...
        if text_content:
            ...
            self.arabic_content_score = min(1.0, arabic_char_count / max(1, len(text_content)))
        ...
        if self.attributes:
            ...
            self.rtl_layout_applied = (
                dir_attr == 'rtl' or
                'rtl' in class_attr or
                'arabic' in class_attr
            )
        ...
        if tag_name in ['form', 'input', 'select', 'textarea']:
            ...
            form_indicators = text_content.lower() + ' '.join(self.attributes.values()).lower()
    ...
    def _validate_cultural_compliance(self):
        ...
        if self.node_name and self.node_name.lower() in ['img', 'button', 'input']:
            ...
            if not alt_text and not aria_label:
                validation_result['recommendations'].append('Add alt text or aria-label for accessibility')
    ...
    def get_cultural_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'accessibility': {
                ...
                'accessibility_score': self.ax_node.accessibility_score if self.ax_node else 0.5,
            },
            'government_integration': {
                'government_form_element': self.government_form_element,
                'government_portal_element': self.ax_node.government_portal_element if self.ax_node else False,
            },
            'islamic_compliance': {
                'validated': self.islamic_compliance_validated,
                'compliant': self.cultural_validation_result['islamic_compliant'] if self.cultural_validation_result else True,
            }
        }
...
class CurrentPageTargets:
    ...
    def _analyze_targets(self):
        ...
        if self.page_session:
            ...
            government_domains = ['.gov.iq', 'ministry', 'government', 'baghdad', 'iraq']
