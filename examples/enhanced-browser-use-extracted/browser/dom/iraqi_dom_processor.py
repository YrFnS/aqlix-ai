...
class IraqiDOMValidation:
    ...
    def has_warnings(self) -> bool:
        ...
        return self.result in [
            IraqiValidationResult.WARNING,
            IraqiValidationResult.REQUIRES_REVIEW,
        ]
...
class IraqiDOMProcessor(DomService):
    ...
    async def validate_cultural_compliance(
        self, dom_tree: EnhancedDOMTreeNode
    ) -> IraqiDOMValidation:
        ...
        validation.accessibility_score = sum(accessibility_scores) / len(
            accessibility_scores
        )
        validation.arabic_support_quality = sum(arabic_quality_scores) / len(
            arabic_quality_scores
        )
        ...
        if self.enable_government_optimization:
            validation.government_compliance = self._calculate_government_compliance(
                all_nodes
            )
            validation.overall_score = (
                validation.overall_score * 0.8 + validation.government_compliance * 0.2
            )
        ...
        self.logger.info(
            f"Cultural validation completed - Score: {validation.overall_score:.2f}, "
        ...
        for pattern in self.inappropriate_content_patterns:
            if pattern in text_lower:
                ...
                analysis["violations"].append(
                    f"Inappropriate content detected: {pattern}"
                )
        ...
        if arabic_chars > 0:
            ...
            if rtl_quality < 0.5:
                analysis["recommendations"].append(
                    "Improve RTL layout support for Arabic text"
                )
    ...
    def _analyze_rtl_support(self, node: EnhancedDOMTreeNode) -> float:
        ...
        if node.snapshot_node and node.snapshot_node.computed_styles:
            text_align = node.snapshot_node.computed_styles.get(
                "text-align", ""
            ).lower()
    ...
    def _analyze_government_compliance(
        self, node: EnhancedDOMTreeNode, text_content: str
    ) -> float:
        ...
        gov_indicators = sum(
            1 for pattern in self.government_portal_patterns if pattern in text_lower
        )
    ...
    def _calculate_government_compliance(
        self, all_nodes: List[EnhancedDOMTreeNode]
    ) -> float:
    ...
    async def process_arabic_content(
            ...
            if arabic_chars > 0:
                ...
                if element_info["rtl_support_score"] < 0.7:
                    ...
                    if not node.attributes.get("dir"):
                        ...
                        rtl_corrections.append(
                            f"Node {node.node_id}: Add RTL direction"
                        )
                    ...
                    if node.snapshot_node and node.snapshot_node.computed_styles:
                        ...
                        if text_align not in ["right", "center"]:
                            element_info["optimizations"].append(
                                "Adjust text alignment for RTL"
                            )
                            rtl_corrections.append(
                                f"Node {node.node_id}: Fix text alignment"
                            )

                # Check font optimization
                if node.snapshot_node and node.snapshot_node.computed_styles:
                    font_family = node.snapshot_node.computed_styles.get(
                        "font-family", ""
                    ).lower()
                    arabic_fonts = [
                        "amiri",
                    ...
                    if not has_arabic_font:
                        element_info["optimizations"].append(
                            "Use Arabic-optimized fonts"
                        )
                        font_optimizations.append(
                            f"Node {node.node_id}: Improve Arabic font"
                        )
        ...
        return {
            ...
            "elements_needing_optimization": sum(
                1 for e in arabic_elements if e["needs_optimization"]
            ),
