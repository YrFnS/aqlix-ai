...
class IraqiDomainValidator:
    ...
    IRAQI_DOMAINS = {
        ...
        # Healthcare domains
        IraqiPortalType.HEALTHCARE: ["moh.gov.iq", "health.krg.org", "medical-city.iq"],
        # Payment and telecom
    ...
    def is_iraqi_domain(cls, url: str) -> bool:
        ...
        return cls.get_portal_type(
            url
        ) != IraqiPortalType.GENERAL or url.lower().endswith(".iq")

    @classmethod
    def validate_domain_access(
        cls, url: str, allowed_types: List[str] = None
    ) -> Tuple[bool, str]:
        ...
        if allowed_types and portal_type not in allowed_types:
            return (
                False,
                f"Portal type {portal_type} not in allowed types: {allowed_types}",
            )
...
class IraqiPortalConfig:
    ...
    def get_config(cls, portal_type: str) -> Dict[str, Any]:
        ...
        return cls.PORTAL_CONFIGS.get(
            portal_type, cls.PORTAL_CONFIGS[IraqiPortalType.GENERAL]
        )
...
class IraqiEnhancedBrowserSession:
    ...
    def __init__(self, cultural_compliance: bool = True, islamic_values: bool = True):
        ...
        logger.info(
            f"Iraqi Enhanced Browser Session initialized (Cultural: {cultural_compliance}, Islamic: {islamic_values})"
        )

    async def start_session(
        self, allowed_portal_types: List[str] = None, headless: bool = False
    ) -> bool:
        """
    ...
    def _create_iraqi_browser_profile(self, headless: bool = False) -> BrowserProfile:
        ...
        profile_data = {
            ...
            "user_data_dir": str(
                Path.home() / ".config" / "iraqi-browser-use" / "profiles" / "default"
            ),
    ...
    async def _validate_page_culture(self) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(
                f"Cultural validation completed (Cultural: {cultural_score}, Islamic: {islamic_score})"
            )

            return {"cultural_score": cultural_score, "islamic_score": islamic_score}
    ...
    async def _detect_arabic_content(self) -> bool:
        ...
        try:
            ...
            if total_chars > 0:
                ...
                logger.info(
                    f"Arabic content detected: {self.arabic_content_detected} (ratio: {arabic_ratio:.2%})"
                )
    ...
    async def navigate_iraqi_portal(
        self, url: str, portal_type: str = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Iraqi portal navigation completed successfully: {detected_type}"
            )
            return result
    ...
    async def fill_arabic_form(
        self, form_data: Dict[str, str], validate_islamic: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            for field_name, field_value in form_data.items():
                ...
                if has_arabic:
                    ...
                    if validate_islamic:
                        is_compliant = await self._validate_islamic_content(
                            processed_value
                        )
                        if not is_compliant:
                            results.append(
    ...
    async def get_session_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_metrics": {
                "average_navigation_time": sum(self.navigation_times)
                / len(self.navigation_times)
                if self.navigation_times
                else 0,
            ...
            "cultural_validation": {
                ...
                "average_cultural_score": sum(
                    score["cultural_score"] for score in self.cultural_scores.values()
                )
                / len(self.cultural_scores)
                if self.cultural_scores
                else 0,
                "average_islamic_score": sum(
                    score["islamic_score"] for score in self.cultural_scores.values()
                )
                / len(self.cultural_scores)
                if self.cultural_scores
                else 0,
                "recent_scores": list(self.cultural_scores.values())[
                    -5:
                ],  # Last 5 validations
            },
            "content_analysis": {
    ...
    async def close_session(self):
        ...
        try:
            ...
            logger.info(
                f"Iraqi Enhanced Browser Session closed. Final analytics: {analytics['session_info']}"
            )
...
async def create_iraqi_browser_session(
    ...
    session = IraqiEnhancedBrowserSession(
        cultural_compliance=cultural_compliance, islamic_values=islamic_values
    )

    success = await session.start_session(
        allowed_portal_types=allowed_portal_types, headless=headless
    )
