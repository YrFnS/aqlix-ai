...
export class ArabicRTLIntegrationManager {
  ...
  async processArabicText(
    ...
    if (culturalContext?.culturalSensitivity === 'high') {
      ...
      const validationResult = await this.culturalValidator.validateContent(
        validationRequest
      );
      ...
      processingResult.culturalCompliance = {
        ...
        recommendations: validationResult.recommendations.map(rec => rec.description),
  ...
  getPerformanceMetrics(): IIntegrationMetrics {
    ...
    const bridgeCache = this.arabicBridge.getCacheStats?.() ?? { size: 0, hitRate: 0 };
...
export type { ICulturalSyncContext, IArabicSyncEvent, IArabicSyncPayload } from '../real-time/ArabicContentSync';
