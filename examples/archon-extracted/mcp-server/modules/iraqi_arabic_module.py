...
class ArabicTextProcessor:
    ...
    def detect_text_direction(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if arabic_percentage > 0.6:
                ...
                suggested_direction = "rtl" if arabic_percentage > latin_percentage else "ltr"
    ...
    def recognize_iraqi_dialect(self, arabic_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            best_dialect = max(dialect_scores.keys(), key=lambda x: dialect_scores[x]["confidence_score"])
            best_score = dialect_scores[best_dialect]["confidence_score"]
            
            # Overall assessment
            is_iraqi_dialect = best_score >= 0.3
            confidence_level = "high" if best_score >= 0.7 else "medium" if best_score >= 0.4 else "low"
            ...
            return {
                ...
                "all_markers_found": sum([scores["markers_found"] for scores in dialect_scores.values()], []),
                "recognition_accuracy": round(best_score * 100, 1) if is_iraqi_dialect else 0.0
            }
    ...
    def process_mixed_content(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "formatting_suggestions": self._generate_formatting_suggestions(segments),
    ...
    def translate_technical_terms(self, text: str, domain: str = None, target_lang: str = "arabic") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if target_lang == "arabic":
                ...
                for arabic_term, english_term in reverse_translations.items():
                    if arabic_term in text:
                        ...
                        translated_text = translated_text.replace(arabic_term, english_term)
            ...
            return {
                ...
                "domain_specific_terms": len([t for t in found_terms if t in domain_translations])
            }
...
def register_arabic_tools(mcp: FastMCP):
    ...
    async def search_arabic_code_examples(
        ...
        try:
            ...
            translation_result = arabic_processor.translate_technical_terms(query, target_lang="arabic")
    ...
    return (
        ...
        >
            {children}
                    ...
                    "language": "javascript",
                    "arabic_features": ["rtl_support", "arabic_fonts", "text_direction"],
...
const arabicValidationMessages = {
    ...
    pattern: 'التنسيق غير صحيح'
                    ...
                    "language": "javascript",
                    "arabic_features": ["arabic_messages", "rtl_validation", "cultural_patterns"],
                    "localization_level": "high",
            ...
            return json.dumps({
                ...
                "arabic_code_examples": {
                    ...
                    "query_translation": translation_result if translation_result["success"] else None,
                    "search_metadata": {
            ...
            return json.dumps({
                ...
                "translation_results": {
                    ...
                    "translation_summary": {
                        ...
                        "translation_coverage": round(translation_result["translation_coverage"] * 100, 1),
                        "domain_specific_terms": translation_result.get("domain_specific_terms", 0),
                        "professional_domain": domain or "general"
                    },
                    "translations_applied": translation_result["translations_applied"],
                    "mixed_content_analysis": mixed_analysis if mixed_analysis.get("is_mixed_content") else None
                },
                "translation_quality": {
        ...
        try:
            ...
            if html_content:
                html_analysis = {
                    ...
                    "arabic_text_detected": bool(re.search(RTL_PATTERNS["arabic_text"], html_content)),
                    "mixed_content_detected": bool(re.search(RTL_PATTERNS["mixed_content"], html_content))
                }
            ...
            if css_rules:
                css_analysis = {
                    ...
                    "arabic_fonts": any(font in css_rules for font in ["Amiri", "Scheherazade", "Cairo", "Tajawal"]),
            ...
            if html_content and not html_analysis.get("dir_attribute_present"):
                recommendations.append("Add dir='rtl' attribute to root elements containing Arabic text")
            
            if css_rules and not css_analysis.get("direction_rtl"):
                recommendations.append("Set CSS direction: rtl for Arabic content containers")
                
            if not css_analysis.get("arabic_fonts"):
                recommendations.append("Include Arabic web fonts like Amiri, Cairo, or Tajawal for better readability")
            ...
            return json.dumps({
                ...
                "rtl_validation": {
                    ...
                    "cultural_design": validation_results["cultural_design_compliance"],
