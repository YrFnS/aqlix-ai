...
class IraqiKeywordExtractor:
    ...
    def __init__(self):
        ...
        self.arabic_stop_words = {
            ...
            "التي", "الذي", "التي", "التني", "قد", "لقد", "كل", "بعض", "أكثر",
            "أول", "آخر", "بعد", "قبل", "عند", "لدى", "أمام", "وراء", "تحت", "فوق"
        }
        ...
        self.professional_terms = {
            "legal": [
                "قانون", "محكمة", "قاضي", "عدالة", "حكم", "دعوى", "محام", "برلمان",
                "دستور", "تشريع", "قرار", "لائحة", "نظام", "مرسوم"
            ],
            "medical": [
                "طب", "طبيب", "مريض", "علاج", "دواء", "مستشفى", "عيادة", "تشخيص",
                "عملية", "فحص", "صحة", "مرض", "وقاية", "تطعيم"
            ],
            "education": [
                "تعليم", "تربية", "مدرسة", "جامعة", "معلم", "أستاذ", "طالب", "دراسة",
                "منهج", "امتحان", "شهادة", "كلية", "قسم", "تخرج"
            ],
            "government": [
                "حكومة", "وزارة", "وزير", "دولة", "حكومي", "رسمي", "خدمة", "إدارة",
                "مؤسسة", "هيئة", "مجلس", "لجنة", "سلطة", "منصب"
            ]
        }
    ...
    def _extract_base_keywords(self, text: str, config: KeywordExtractionConfig) -> List[str]:
        ...
        for word in words:
            ...
            # Remove punctuation
            clean_word = re.sub(r'[^\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFFa-zA-Z]', '', word)
    ...
    def _filter_and_rank_keywords(self, keywords: List[str], config: KeywordExtractionConfig) -> List[str]:
...
class IraqiHybridSearchEngine:
    ...
    def __init__(self):
        ...
        self.cultural_rules = {
            ...
            "positive_indicators": [
                "family", "community", "education", "justice", "health"
            ],
    ...
    async def search(self, query: SearchQuery, max_results: int = 10) -> HybridSearchResult:
        """
        ...
        try:
            ...
            keywords = self.keyword_extractor.extract_keywords(enhanced_query.text, extraction_config)
            
            # Step 3: Execute searches based on mode
            search_results = await self._execute_search_strategy(enhanced_query, keywords, max_results)
            
            # Step 4: Apply cultural validation and filtering
            validated_results = await self._apply_cultural_validation(search_results, query)
    ...
    async def _enhance_query_for_iraqi_context(self, query: SearchQuery) -> SearchQuery:
        ...
        if query.professional_domain:
            enhanced_text = self._add_professional_context(enhanced_text, query.professional_domain)
    ...
    async def _execute_search_strategy(self, query: SearchQuery, keywords: List[str], 
                                     max_results: int) -> Dict[str, List[SearchMatch]]:
        """Execute search strategy based on query mode"""
        
        results = {}
        
        if query.mode in [SearchMode.VECTOR_ONLY, SearchMode.HYBRID_BALANCED, 
                         SearchMode.HYBRID_VECTOR_HEAVY, SearchMode.CULTURAL_OPTIMIZED]:
        ...
        if query.mode in [SearchMode.KEYWORD_ONLY, SearchMode.HYBRID_BALANCED,
                         SearchMode.HYBRID_KEYWORD_HEAVY, SearchMode.CULTURAL_OPTIMIZED]:
            # Execute keyword search
            keyword_start = time.time()
            results["keyword"] = await self._keyword_search(query, keywords, max_results * 2)
            self.metrics["keyword_search_time"] = (time.time() - keyword_start) * 1000
        
        return results
    
    async def _vector_search(self, query: SearchQuery, max_results: int) -> List[SearchMatch]:
    ...
    async def _keyword_search(self, query: SearchQuery, keywords: List[str], 
                            max_results: int) -> List[SearchMatch]:
    ...
    async def _apply_cultural_validation(self, search_results: Dict[str, List[SearchMatch]], 
                                       query: SearchQuery) -> Dict[str, List[SearchMatch]]:
        ...
        for search_type, matches in search_results.items():
            ...
            for match in matches:
                ...
                cultural_validation = await self._validate_cultural_appropriateness(match, query)
                
                match.cultural_appropriateness_score = cultural_validation["score"]
                match.islamic_compliance_passed = cultural_validation["islamic_compliant"]
                
                # Apply filtering based on requirements
                if query.islamic_compliance_required and not match.islamic_compliance_passed:
                    continue
                ...
                match.arabic_quality_score = self._calculate_arabic_quality(match, query)
                
                # Calculate professional accuracy if applicable
                if query.professional_domain:
                    match.professional_accuracy_score = await self._calculate_professional_accuracy(
        ...
        self.metrics["cultural_validation_time"] = (time.time() - validation_start) * 1000
        
        return validated_results
    
    async def _validate_cultural_appropriateness(self, match: SearchMatch, 
    ...
    def _calculate_arabic_quality(self, match: SearchMatch, query: SearchQuery) -> float:
    ...
    async def _calculate_professional_accuracy(self, match: SearchMatch, domain: str) -> float:
    ...
    async def _merge_and_rank_results(self, validated_results: Dict[str, List[SearchMatch]], 
                                    query: SearchQuery) -> List[SearchMatch]:
    ...
    async def _merge_score_weighted(self, results: Dict[str, List[SearchMatch]], 
                                  query: SearchQuery) -> List[SearchMatch]:
        ...
        for search_type, matches in results.items():
            for match in matches:
                if match.id not in seen_ids:
                    ...
                    cultural_component = match.cultural_appropriateness_score * query.cultural_boost
                    
                    match.hybrid_score = vector_component + keyword_component + cultural_component
    ...
    async def _merge_rank_fusion(self, results: Dict[str, List[SearchMatch]], 
                               query: SearchQuery) -> List[SearchMatch]:
        ...
        rank_maps = {}
        for search_type, matches in results.items():
            rank_maps[search_type] = {match.id: rank + 1 for rank, match in enumerate(matches)}
    ...
    async def _merge_cultural_priority(self, results: Dict[str, List[SearchMatch]], 
                                     query: SearchQuery) -> List[SearchMatch]:
    ...
    async def _merge_professional_priority(self, results: Dict[str, List[SearchMatch]], 
                                         query: SearchQuery) -> List[SearchMatch]:
    ...
    async def _build_search_result(self, query: SearchQuery, matches: List[SearchMatch],
                                 start_time: float, search_results: Dict[str, List[SearchMatch]]) -> HybridSearchResult:
        ...
        if matches:
            ...
            cultural_compliance_rate = sum(1 for m in matches if m.islamic_compliance_passed) / len(matches)
            arabic_quality_avg = sum(m.arabic_quality_score for m in matches) / len(matches)
            professional_accuracy_avg = sum(m.professional_accuracy_score for m in matches) / len(matches)
        else:
            avg_relevance = 0.0
            cultural_compliance_rate = 0.0
        ...
        if cultural_compliance_rate < 0.8:
            recommendations.append("Consider refining query for better cultural compliance")
        ...
        if professional_accuracy_avg < 0.8 and query.professional_domain:
            recommendations.append(f"Professional accuracy for {query.professional_domain} domain needs improvement")
    ...
    def _update_performance_metrics(self, result: HybridSearchResult):
        ...
        if total > 0:
            ...
            self.metrics["average_search_time"] = (
                self.metrics["average_search_time"] * (total - 1) + result.search_time_ms
            ) / total
            
            self.metrics["average_relevance_score"] = (
                self.metrics["average_relevance_score"] * (total - 1) + result.average_relevance
            ) / total
            
            self.metrics["cultural_compliance_rate"] = (
                self.metrics["cultural_compliance_rate"] * (total - 1) + result.cultural_compliance_rate
            ) / total
    ...
    def get_performance_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_cultural_validation_time_ms": self.metrics["cultural_validation_time"],
            ...
            "search_engine_health": "healthy" if success_rate > 0.95 else "warning" if success_rate > 0.85 else "critical"
        }