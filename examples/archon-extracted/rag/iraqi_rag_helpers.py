...
from .iraqi_rag_orchestrator import (
    IraqiSearchContext, SearchResult, ContentType, ProfessionalDomain, 
    SearchStrategy, CulturalSensitivityLevel
)
...
class IraqiRAGHelpers:
    ...
    def _initialize_arabic_patterns(self) -> Dict[str, Any]:
        ...
        return {
            "rtl_markers": ["ا", "ب", "ت", "ث", "ج", "ح", "خ", "د", "ذ", "ر", "ز", "س", "ش"],
            "islamic_terms": ["الله", "إسلام", "قرآن", "حديث", "فقه", "شريعة", "صلاة", "زكاة"],
            "formal_connectors": ["إن", "أن", "لكن", "غير", "سوى", "إلا", "بل", "لكن"],
            "courtesy_phrases": ["بسم الله", "الحمد لله", "إن شاء الله", "جزاك الله خيرا"],
    ...
    async def create_multi_strategy_embeddings(self, context: IraqiSearchContext) -> Dict[str, EmbeddingStrategyResult]:
        """
        ...
        try:
            ...
            if context.governorate_filter or self._contains_iraqi_context(context.query):
            ...
            self.logger.debug(f"Multi-strategy embeddings created: {len(results)} strategies in {processing_time:.3f}s")
    ...
    async def _create_standard_embedding(self, context: IraqiSearchContext) -> EmbeddingStrategyResult:
    ...
    async def _create_cultural_enhanced_embedding(self, context: IraqiSearchContext) -> EmbeddingStrategyResult:
        ...
        cultural_score = await self._calculate_cultural_enhancement_score(context, enhanced_query)
    ...
    async def _create_professional_embedding(self, context: IraqiSearchContext) -> EmbeddingStrategyResult:
        """Create professional domain-specific embedding"""
        start_time = time.time()
        
        # Enhance query with professional terminology
        professional_query = await self._enhance_query_with_professional_context(context)
        
        # Generate professional-focused embedding
        embedding = self._generate_mock_embedding(professional_query, strategy="professional")
        ...
        result = EmbeddingStrategyResult(
            ...
            processing_time=processing_time,
            metadata={
                "professional_domain": context.professional_domain.value if context.professional_domain else None,
                "professional_terms_added": self._count_professional_terms(professional_query, context),
    ...
    async def _create_arabic_linguistic_embedding(self, context: IraqiSearchContext) -> EmbeddingStrategyResult:
        ...
        result = EmbeddingStrategyResult(
            ...
            metadata={
                ...
                "rtl_processing": True,
                "linguistic_enhancements": self._get_linguistic_enhancements(linguistic_query)
            }
        )
        
        return result
    
    async def _create_iraqi_regional_embedding(self, context: IraqiSearchContext) -> EmbeddingStrategyResult:
    ...
    async def _create_fallback_embedding(self, context: IraqiSearchContext) -> EmbeddingStrategyResult:
    ...
    async def perform_comprehensive_cultural_analysis(self, result: SearchResult, 
                                                    context: IraqiSearchContext) -> CulturalAnalysisResult:
        """
        ...
        try:
            ...
            cultural_analysis = await self._analyze_iraqi_cultural_appropriateness(result, context)
            
            # Professional domain alignment analysis
            professional_analysis = await self._analyze_professional_domain_alignment(result, context)
            
            # Linguistic authenticity analysis
            linguistic_analysis = await self._analyze_linguistic_authenticity(result, context)
            ...
            self.logger.debug(f"Cultural analysis completed: score={overall_score:.3f} in {processing_time:.3f}s")
    ...
    async def _analyze_islamic_compliance(self, result: SearchResult, 
                                        context: IraqiSearchContext) -> Dict[str, Any]:
        ...
        prohibited_terms = self.cultural_rules.get("islamic_compliance", {}).get("prohibited_content", [])
        ...
        if values_found > 0:
            ...
            analysis["recommendations"].append(f"Content aligns with Islamic values: {values_found} values identified")
        
        # Check for religious context appropriateness
        if context.requires_islamic_compliance:
            if any(term in result.content for term in self.arabic_patterns["islamic_terms"]):
                analysis["score"] += 0.1
                analysis["areas_checked"].append("religious_terminology")
            else:
                analysis["recommendations"].append("Consider adding appropriate Islamic context")
    ...
    async def _analyze_iraqi_cultural_appropriateness(self, result: SearchResult, 
                                                   context: IraqiSearchContext) -> Dict[str, Any]:
        ...
        cultural_elements = [indicator for indicator in iraqi_indicators if indicator in result.content]
        
        if cultural_elements:
            analysis["score"] += 0.2
            analysis["cultural_elements"] = cultural_elements
            analysis["recommendations"].append(f"Contains Iraqi cultural elements: {', '.join(cultural_elements)}")
        ...
        if context.governorate_filter and context.governorate_filter.lower() in result.content.lower():
            analysis["score"] += 0.1
            analysis["recommendations"].append(f"Contains relevant regional context: {context.governorate_filter}")
    ...
    async def _analyze_professional_domain_alignment(self, result: SearchResult, 
                                                   context: IraqiSearchContext) -> Dict[str, Any]:
        ...
        domain_terms = self.professional_terminology.get(context.professional_domain.value, {})
        
        # Check for Arabic professional terms
        if "arabic" in domain_terms:
            arabic_matches = sum(1 for term in domain_terms["arabic"] if term in result.content)
            if arabic_matches > 0:
                analysis["score"] += min(0.1 * arabic_matches, 0.3)
                analysis["domain_relevance"].append(f"Arabic professional terms: {arabic_matches}")
        
        # Check for Iraqi-specific terms
        if "iraqi_specific" in domain_terms:
            iraqi_matches = sum(1 for term in domain_terms["iraqi_specific"] if term in result.content)
            if iraqi_matches > 0:
                analysis["score"] += min(0.1 * iraqi_matches, 0.2)
                analysis["domain_relevance"].append(f"Iraqi-specific terms: {iraqi_matches}")
        
        # Check for Islamic professional context
        if context.requires_islamic_compliance and "islamic" in domain_terms:
            islamic_matches = sum(1 for term in domain_terms["islamic"] if term in result.content)
            if islamic_matches > 0:
                analysis["score"] += min(0.05 * islamic_matches, 0.15)
                analysis["domain_relevance"].append(f"Islamic professional terms: {islamic_matches}")
        
        # Professional accuracy validation
        if context.professional_domain == ProfessionalDomain.MEDICAL:
            if any(term in result.content.lower() for term in ["treatment", "diagnosis", "medical"]):
                analysis["recommendations"].append("Medical content detected - ensure clinical accuracy")
        elif context.professional_domain == ProfessionalDomain.LEGAL:
            if any(term in result.content.lower() for term in ["law", "legal", "court"]):
                analysis["recommendations"].append("Legal content detected - verify legal accuracy")
    ...
    async def _analyze_linguistic_authenticity(self, result: SearchResult, 
                                             context: IraqiSearchContext) -> Dict[str, Any]:
        ...
        if arabic_ratio > 0:
            ...
            analysis["linguistic_features"].append(f"Arabic content ratio: {arabic_ratio:.2f}")
        ...
        if context.dialect == "iraqi":
            ...
            if dialect_terms:
                analysis["score"] += 0.1
                analysis["linguistic_features"].append(f"Iraqi dialect terms: {len(dialect_terms)}")
            else:
                analysis["recommendations"].append("Consider adding Iraqi dialect context for local relevance")
    ...
    def _generate_mock_embedding(self, text: str, strategy: str) -> List[float]:
        ...
        embedding = [float(int(hash_value[i:i+8], 16)) / (16**8) for i in range(0, min(len(hash_value), 256), 8)]
    ...
    def _calculate_arabic_text_ratio(self, text: str) -> float:
        ...
        arabic_chars = len(re.findall(self.arabic_patterns["text_direction_indicators"]["strong_rtl"], text))
    ...
    def _create_default_cultural_analysis(self, result: SearchResult, 
                                        context: IraqiSearchContext) -> CulturalAnalysisResult:
    ...
    async def _enhance_query_with_cultural_context(self, context: IraqiSearchContext) -> str:
        """Enhance query with Islamic and Iraqi cultural context"""
    ...
    async def _enhance_query_with_professional_context(self, context: IraqiSearchContext) -> str:
        ...
        if context.professional_domain:
            ...
            # Add primary terms
            if "arabic" in domain_terms:
                enhanced += " " + " ".join(domain_terms["arabic"][:3])  # Add top 3 terms
            
            # Add Iraqi-specific terms if high sensitivity
            if (context.cultural_sensitivity_level == CulturalSensitivityLevel.STRICT and 
                "iraqi_specific" in domain_terms):
    ...
    async def _process_arabic_linguistics(self, context: IraqiSearchContext) -> str:
        ...
        for iraqi_term, standard_terms in self.iraqi_dialect_patterns["common_words"].items():
            if iraqi_term in enhanced:
                enhanced += " " + " ".join(standard_terms[:2])  # Add top 2 standard equivalents
    ...
    async def _enhance_query_with_iraqi_regional_context(self, context: IraqiSearchContext) -> str:
    ...
    async def _track_search_performance(self, search_type: str, processing_time: float, 
                                      result_count: int, context: IraqiSearchContext):
        ...
        if processing_time > 1.0:  # Slow threshold
            self.logger.warning(f"Slow {search_type} performance: {processing_time:.3f}s for {result_count} results")
    
    async def _track_search_error(self, search_type: str, error_message: str, context: IraqiSearchContext):
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        for category, metrics in self.performance_metrics.items():
            if "embedding_generation" in category:
                cache_hits = sum(1 for m in metrics if m.get("metadata", {}).get("cache_hit"))
                total = len(metrics)
        ...
        for category in self.performance_metrics:
            if "_errors" in category:
                ...
                summary["error_rates"][base_category] = error_count / max(total_count, 1)
