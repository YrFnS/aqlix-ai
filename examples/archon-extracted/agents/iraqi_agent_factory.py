        ...
        if cultural_compliance > 0:
            if self.total_requests == 1:
                ...
                self.avg_cultural_compliance = (
                    (self.avg_cultural_compliance * (self.total_requests - 1) + cultural_compliance)
                    / self.total_requests
                )
        ...
        if islamic_compliance > 0:
            if self.total_requests == 1:
                ...
                self.avg_islamic_compliance = (
                    (self.avg_islamic_compliance * (self.total_requests - 1) + islamic_compliance)
                    / self.total_requests
                )
        ...
        if arabic_accuracy > 0:
            if self.total_requests == 1:
                ...
                self.avg_arabic_processing_accuracy = (
                    (self.avg_arabic_processing_accuracy * (self.total_requests - 1) + arabic_accuracy)
                    / self.total_requests
                )
...
class IraqiAgentFactory:
    ...
    def list_agents(
        ...
        for agent_id, instance in self.agents.items():
            ...
            if professional_domain and instance.configuration.professional_domain != professional_domain:
                continue
    ...
    def get_agent_metrics(self, agent_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "last_used": agent_instance.last_used.isoformat() if agent_instance.last_used else None,
            ...
            "avg_arabic_processing_accuracy": round(agent_instance.avg_arabic_processing_accuracy, 3),
    ...
    def get_factory_metrics(self) -> Dict[str, Any]:
        ...
        """
        active_agents = [instance for instance in self.agents.values() if instance.is_active]
        ...
        uptime = (datetime.now() - self.factory_metrics["factory_start_time"]).total_seconds()
        ...
        return {
            ...
            "total_requests_processed": self.factory_metrics["total_requests_processed"],
    ...
    def _validate_configuration(self, config: IraqiAgentConfiguration) -> None:
        ...
        if config.cultural_compliance_threshold < 0.0 or config.cultural_compliance_threshold > 1.0:
            raise ValueError("Cultural compliance threshold must be between 0.0 and 1.0")
        
        if config.islamic_compliance_threshold < 0.0 or config.islamic_compliance_threshold > 1.0:
