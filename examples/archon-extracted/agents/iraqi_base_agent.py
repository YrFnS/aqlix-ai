...
class IraqiAgentOutput(BaseModel):
    ...
    cultural_compliance_score: float = Field(default=0.0, description="Cultural appropriateness score")
    islamic_compliance_score: float = Field(default=0.0, description="Islamic compliance score")
    arabic_processing_accuracy: float = Field(default=0.0, description="Arabic text processing accuracy")
    dialect_recognition_accuracy: float = Field(default=0.0, description="Iraqi dialect recognition accuracy")
    
    # Professional context
    professional_domain: Optional[str] = Field(default=None, description="Professional domain context")
    professional_accuracy: float = Field(default=0.0, description="Professional domain accuracy")
...
class IraqiCulturalIntelligence:
    ...
    CULTURAL_THEMES = {
        "islamic_values": {
            "weight": 0.4,
            "keywords": ["halal", "haram", "islamic", "muslim", "faith", "prayer", "ramadan", "quran"],
            "negative_keywords": ["alcohol", "gambling", "interest", "usury", "haram"]
        },
        "iraqi_culture": {
            "weight": 0.3,
            "keywords": ["iraqi", "baghdad", "basra", "mosul", "mesopotamian", "arabic", "kurdish"],
            "traditions": ["hospitality", "family_values", "respect_elders", "community"]
        },
        "professional_ethics": {
            "weight": 0.2,
            "keywords": ["professional", "ethical", "responsible", "integrity", "honesty"],
    ...
    PROFESSIONAL_DOMAINS = {
        ...
        "medical": {
            "terms": ["doctor", "patient", "hospital", "medicine", "treatment", "health"],
            "compliance_factors": ["medical_ethics", "patient_privacy", "islamic_medical_ethics"],
            "cultural_sensitivity": 0.98
        },
        "educational": {
            "terms": ["student", "teacher", "school", "university", "education", "learning"],
            "compliance_factors": ["educational_standards", "islamic_education", "curriculum"],
            "cultural_sensitivity": 0.92
        },
        "government": {
            "terms": ["citizen", "government", "ministry", "public", "service", "administration"],
    ...
    @staticmethod
    def analyze_cultural_compliance(content: str, context: Optional[Dict] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for theme_name, theme_data in IraqiCulturalIntelligence.CULTURAL_THEMES.items():
                score = 0.0
                matches = []
            ...
            # Overall assessment
            compliance_level = "high" if total_score >= 0.8 else "medium" if total_score >= 0.6 else "low"
            ...
            return {
                ...
                "recommendations": IraqiCulturalIntelligence._generate_cultural_recommendations(theme_scores),
    ...
    def analyze_islamic_compliance(content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            positive_indicators = [
                "ethical", "honest", "trustworthy", "responsible", "charitable",
                "family", "respect", "dignity", "justice", "peace", "halal"
            ]
            
            negative_indicators = [
                "alcohol", "gambling", "interest", "usury", "haram", 
                "inappropriate", "unethical", "dishonest"
            ]
            
            positive_score = sum(0.15 for indicator in positive_indicators if indicator in content_lower)
            negative_score = sum(0.25 for indicator in negative_indicators if indicator in content_lower)
            ...
            return {
                ...
                "positive_indicators_found": [i for i in positive_indicators if i in content_lower],
                "negative_indicators_found": [i for i in negative_indicators if i in content_lower],
                "processing_time_ms": processing_time
            }
    ...
    def detect_professional_domain(content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for domain_name, domain_data in IraqiCulturalIntelligence.PROFESSIONAL_DOMAINS.items():
                score = 0.0
                matches = []
            ...
            return {
                ...
                "cultural_sensitivity_required": domain_scores[best_domain]["cultural_sensitivity"] if best_score > 0.2 else 0.5
            }
        ...
        for theme_name, theme_data in theme_scores.items():
            if theme_data["score"] < 0.5:
                if theme_name == "islamic_values":
                    recommendations.append("Consider incorporating Islamic ethical principles and values")
                elif theme_name == "iraqi_culture":
                    recommendations.append("Enhance content with Iraqi cultural context and sensitivity")
                elif theme_name == "professional_ethics":
                    recommendations.append("Strengthen professional ethical standards and practices")
                elif theme_name == "social_responsibility":
                    recommendations.append("Include social responsibility and community values")
...
class IraqiRateLimitHandler:
    ...
    def __init__(self, max_retries: int = 5, base_delay: float = 1.0):
        ...
        self.cultural_processing_overhead = 0.05  # Additional time for cultural processing
        ...
        **kwargs
        ...
        while retries <= self.max_retries:
            try:
                ...
                min_interval = self.min_request_interval + self.cultural_processing_overhead
                ...
                if is_rate_limit:
                    ...
                    if retries > self.max_retries:
                        ...
                        raise Exception(f"Cultural AI rate limit exceeded after {self.max_retries} retries: {str(e)}")
                    
                    wait_time = self.base_delay * (2 ** (retries - 1))
                    logger.info(f"Cultural AI rate limit hit. Waiting {wait_time:.2f}s before retry {retries}/{self.max_retries}")
        ...
        raise Exception(f"Failed after {self.max_retries} retries with cultural processing")
...
class IraqiBaseAgent(ABC, Generic[IraqiDepsT, IraqiOutputT]):
    ...
    def __init__(
        ...
        **agent_kwargs
        ...
        self.logger.info(f"✓ Iraqi agent {self.name} initialized with cultural intelligence")
    ...
    def get_enhanced_system_prompt(self, cultural_context: Optional[IraqiCulturalContext] = None) -> str:
        """
...
"""
    ...
    async def run(self, user_prompt: str, deps: IraqiDepsT) -> IraqiOutputT:
        ...
        try:
            ...
            if self.enable_cultural_intelligence:
                cultural_analysis = await self._analyze_cultural_context(user_prompt, deps)
            ...
            if self.rate_limiter:
                ...
                result = await self._run_agent_with_cultural_intelligence(user_prompt, deps)
            ...
            self.logger.info(f"Iraqi agent {self.name} completed successfully in {processing_time}ms")
            return result
    ...
    async def _run_agent_with_cultural_intelligence(
        ...
        try:
            ...
            raise Exception(f"Iraqi agent {self.name} operation timed out - cultural processing taking too long")
        except Exception as e:
    ...
    async def _analyze_cultural_context(
        ...
        try:
            ...
            cultural_analysis = self.cultural_intelligence.analyze_cultural_compliance(prompt)
            
            # Islamic compliance analysis
            islamic_analysis = self.cultural_intelligence.analyze_islamic_compliance(prompt)
            
            # Professional domain detection
            domain_analysis = self.cultural_intelligence.detect_professional_domain(prompt)
            ...
            cultural_context = IraqiCulturalContext(
                ...
                professional_context={
                    ...
                    "cultural_sensitivity_required": domain_analysis.get("cultural_sensitivity_required", 0.5)
                }
            )
    ...
    async def _validate_cultural_compliance(
        ...
        try:
            ...
            cultural_analysis = self.cultural_intelligence.analyze_cultural_compliance(
                result.message, 
                context=deps.cultural_context.professional_context if deps.cultural_context else None
            )
            
            islamic_analysis = self.cultural_intelligence.analyze_islamic_compliance(result.message)
            
            # Update result with cultural metrics
            if hasattr(result, 'cultural_compliance_score'):
                result.cultural_compliance_score = cultural_analysis.get("cultural_compliance_score", 0.0)
                result.islamic_compliance_score = islamic_analysis.get("islamic_compliance_score", 0.0)
                
                # Set professional context
                if deps.cultural_context and deps.cultural_context.professional_domain:
                    result.professional_domain = deps.cultural_context.professional_domain
            ...
            if cultural_score < self.cultural_compliance_threshold:
                self.logger.warning(f"Cultural compliance below threshold: {cultural_score:.3f} < {self.cultural_compliance_threshold}")
            
            if islamic_score < self.islamic_compliance_threshold:
                self.logger.warning(f"Islamic compliance below threshold: {islamic_score:.3f} < {self.islamic_compliance_threshold}")
    ...
    def add_cultural_system_prompt(self, func):
        ...
        async def enhanced_prompt_func(*args, **kwargs):
            base_prompt = await func(*args, **kwargs) if asyncio.iscoroutinefunction(func) else func(*args, **kwargs)
            
            # Add cultural context if available
            cultural_context = kwargs.get('cultural_context')
            return self.get_enhanced_system_prompt(cultural_context) if cultural_context else base_prompt
