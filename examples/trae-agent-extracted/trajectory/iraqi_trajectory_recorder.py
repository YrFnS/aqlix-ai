...
class IraqiTrajectoryRecorder:
    ...
    def __init__(
        ...
        task_context: IraqiExecutionContext = IraqiExecutionContext.GENERAL,
    ):
        ...
        if trajectory_path is None:
            ...
            trajectory_path = (
                f"trajectories/iraqi_trajectory_{context_suffix}_{timestamp}.json"
            )
        ...
        try:
            ...
            print(
                "Error creating trajectory directory. Trajectories may not be properly saved."
            )
    ...
    async def record_agent_step(
        ...
        if tool_calls and any("payment" in str(call).lower() for call in tool_calls):
            payment_tracking = await self.payment_tracker.analyze_step(
        ...
        if tool_calls and any(
            "government" in str(call).lower() or "ministry" in str(call).lower()
            for call in tool_calls
        ):
            government_service_tracking = await self.government_tracker.analyze_step(
                tool_calls, tool_results
            )
    ...
    def finalize_recording(
        self, success: bool, final_result: Optional[str] = None
    ) -> None:
    ...
    def save_trajectory(self) -> None:
        ...
        try:
            ...
            print(
                f"Warning: Failed to save Iraqi trajectory to {self.trajectory_path}: {e}"
            )

    def get_cultural_performance_report(self) -> Dict[str, Any]:
        """Generate comprehensive cultural performance report"""
        return {
            "cultural_compliance": self.trajectory_data.get(
                "cultural_compliance_summary", {}
            ),
            "professional_performance": self.trajectory_data.get(
                "professional_performance_summary", {}
            ),
            "arabic_processing": self.trajectory_data.get(
                "arabic_processing_summary", {}
            ),
            "recommendations": self.trajectory_data.get(
                "iraqi_specific_recommendations", []
            ),
            "overall_score": self.trajectory_data.get(
                "overall_iraqi_ai_performance_score", 0.0
            ),
        }
    ...
    def _calculate_final_cultural_summary(self) -> Dict[str, Any]:
        """Calculate final cultural compliance summary"""
        return {
            "average_islamic_compliance": self.trajectory_data[
                "cultural_compliance_summary"
            ]["overall_islamic_compliance"],
            "cultural_issues_resolved": len(
                [
                    step
    ...
    def _calculate_cultural_grade(self) -> str:
        ...
        score = self.trajectory_data["cultural_compliance_summary"][
            "overall_islamic_compliance"
        ]
        if score >= 0.95:
            return "A+"
    ...
    def _calculate_overall_iraqi_score(self) -> float:
        ...
        cultural_score = self.trajectory_data["cultural_compliance_summary"][
            "overall_islamic_compliance"
        ]
        professional_score = self.trajectory_data["professional_performance_summary"][
            "domain_compliance_average"
        ]
        arabic_score = self.trajectory_data["arabic_processing_summary"][
            "overall_rtl_accuracy"
        ]

        # Weighted average: Cultural (40%), Professional (35%), Arabic (25%)
        return (
            (cultural_score * 0.4) + (professional_score * 0.35) + (arabic_score * 0.25)
        )
...
class IraqiCulturalValidator:
    ...
    async def validate_step(
        ...
        return await self.validate_interaction(
            llm_messages or [], llm_response or {}, context
        )
...
class IraqiProfessionalMonitor:
    ...
    async def validate_step(
        ...
        return await self.validate_interaction(
            llm_messages or [], llm_response or {}, domain
        )
