...
class IraqiCulturalValidator:
    ...
    async def validate_cultural_appropriateness(
        self, content: str, context: IraqiCulturalContext, language_mode: LanguageMode
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if context in [
                IraqiCulturalContext.PROFESSIONAL_LEGAL,
                IraqiCulturalContext.PROFESSIONAL_MEDICAL,
                IraqiCulturalContext.PROFESSIONAL_EDUCATIONAL,
            ]:
                prof_score = await self._validate_professional_appropriateness(
                    content, context
                )
            ...
            if language_mode in [
                LanguageMode.ARABIC_STANDARD,
                LanguageMode.ARABIC_IRAQI_DIALECT,
                LanguageMode.MIXED_ARABIC_ENGLISH,
            ]:
                lang_score = await self._validate_arabic_accuracy(
    ...
    async def _analyze_cultural_sensitivity(
        ...
        cultural_count = sum(
            1 for indicator in cultural_indicators if indicator in content.lower()
        )
    ...
    async def _validate_professional_appropriateness(
        ...
        prof_count = sum(
            1 for indicator in professional_indicators if indicator in content.lower()
        )
        professional_score += min(0.15, prof_count * 0.03)

        return min(1.0, professional_score)

    async def _validate_arabic_accuracy(
        self, content: str, language_mode: LanguageMode
    ) -> float:
        ...
        if language_mode == LanguageMode.ARABIC_IRAQI_DIALECT:
            dialect_count = sum(
                1 for indicator in iraqi_indicators if indicator in content
            )
...
class IraqiDebuggingIntelligence:
    ...
    async def analyze_error(
        ...
        try:
            ...
            debug_metadata.resolution_confidence = (
    ...
    async def _collect_context_metrics(
        self, debug_metadata: IraqiDebugMetadata, context: Dict[str, Any]
    ) -> None:
        ...
        if "arabic" in str(context).lower():
            debug_metadata.arabic_processing_stats = {
                "text_segments_detected": len(
                    re.findall(r"[\u0600-\u06FF]+", str(context))
                ),
                "mixed_content_detected": bool(
                    re.search(r"[\u0600-\u06FF].*[a-zA-Z]", str(context))
                ),
                "rtl_indicators": len(
                    re.findall(r"direction.*rtl|text-align.*right", str(context))
                ),
        ...
        for server in mcp_servers:
            debug_metadata.mcp_server_status[
                server
            ] = await self._check_mcp_server_health(server)
    ...
    def _estimate_processing_time(self, context: Dict[str, Any]) -> float:
        ...
        complexity_factor = (
            self._calculate_context_complexity(context) * 200
        )  # Up to 200ms for complexity
    ...
    async def _perform_root_cause_analysis(
        ...
        if error_category == IraqiErrorCategory.ARABIC_ENCODING:
            analysis_parts.append(
                "Arabic text encoding failure likely due to UTF-8 conversion issues"
            )
            ...
            analysis_parts.append(
                "Cultural compliance violation detected in content or behavior"
            )
            ...
            if "4001" in error_message:
                analysis_parts.append(
                    "ZainCash authentication error - verify merchant credentials"
                )
            elif "timeout" in error_message.lower():
                analysis_parts.append(
                    "Payment gateway timeout - check network connectivity"
                )
        ...
        if "timeout" in error_message.lower():
            analysis_parts.append(
                "Timeout condition indicates network or processing delays"
            )
        ...
        if complexity > 0.7:
            analysis_parts.append(
                f"High context complexity ({complexity:.2f}) may contribute to processing issues"
            )

        return (
            "; ".join(analysis_parts)
            if analysis_parts
            else "No specific root cause identified"
        )
    ...
    async def monitor_performance(
        ...
        if baseline_key in self.performance_baselines:
            ...
            if duration_ms <= max_time:
                ...
                performance_analysis["recommendations"].append(
                    f"Performance slightly degraded: {duration_ms:.1f}ms vs {max_time:.1f}ms baseline"
                )
            else:
                performance_analysis["performance_status"] = "poor"
                performance_analysis["recommendations"].append(
                    f"Performance significantly degraded: {duration_ms:.1f}ms vs {max_time:.1f}ms baseline"
                )
                performance_analysis["recommendations"].append(
                    "Consider optimizing operation or checking system resources"
                )
        ...
        if cultural_context and "arabic" in operation_name.lower():
            if duration_ms > 300:  # Arabic processing should be fast
                performance_analysis["recommendations"].append(
                    "Arabic text processing taking longer than expected - check encoding and text complexity"
                )
    ...
    def get_debugging_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_metrics": {
                ...
                "average_duration_ms": sum(m["duration_ms"] for m in recent_metrics)
                / len(recent_metrics)
                if recent_metrics
                else 0,
            ...
            "error_categories_detected": list(
                set(p["category"] for p in self.debugging_cache.values())
            ),
    ...
    def _calculate_system_health_score(
        ...
        recent_10 = (
            recent_metrics[-10:] if len(recent_metrics) >= 10 else recent_metrics
        )
    ...
    def _generate_system_recommendations(
        ...
        if success_rate < 0.9:
            recommendations.append(
                f"System success rate is {success_rate:.1%}, below 90% target. Investigate frequent failure patterns."
            )
        ...
        if avg_duration > 2000:
            recommendations.append(
                f"Average operation time is {avg_duration:.0f}ms, consider performance optimization."
            )
        ...
        if error_categories:
            ...
            if common_errors[most_common] > 3:
                recommendations.append(
                    f"Recurring {most_common} errors detected. Implement preventive measures."
                )
        ...
        cultural_operations = [
            ...
            if "cultural" in m["operation"].lower()
            or "arabic" in m["operation"].lower()
        ]
        if cultural_operations:
            cultural_success = sum(
                1 for m in cultural_operations if m["success"]
            ) / len(cultural_operations)
            if cultural_success < 0.95:
                recommendations.append(
                    f"Iraqi cultural operations success rate is {cultural_success:.1%}, below 95% target. Review cultural validation logic."
                )
...
class IraqiPerformanceMonitor:
    ...
    async def record_performance(
        ...
        alerts = await self._check_performance_alerts(
            operation_type, performance_record
        )
        ...
        return {
            ...
            "recommendations": self._generate_performance_recommendations(
                operation_type, performance_record, trends
            ),
        }

    async def _check_performance_alerts(
        self, operation_type: str, performance_record: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        if threshold_category in self.alert_thresholds:
            ...
            # Response time alert
            if performance_record["duration_ms"] > thresholds.get(
                ...
                alerts.append(
                    {
                        ...
                        "severity": "warning"
                        if performance_record["duration_ms"]
                        < thresholds["response_time_ms"] * 2
                        else "critical",
            ...
            if threshold_category in [
                ...
                if recent_ops:
                    success_rate = sum(1 for op in recent_ops if op["success"]) / len(
                        recent_ops
                    )
            ...
            # Accuracy alert
            if performance_record["accuracy_score"] > 0 and performance_record[
                "accuracy_score"
            ] < thresholds.get("accuracy_score", 0.0):
                alerts.append(
                    {
                        "type": "accuracy_low",
                        "severity": "warning",
    ...
    def _get_recent_operations(
        self, operation_type: str, count: int
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_performance_trends(self, operation_type: str) -> Dict[str, Any]:
        ...
        response_times = [
            op["duration_ms"] for op in reversed(recent_ops)
        ]  # Chronological order
        ...
        success_trend = (
            self._calculate_trend(success_rates) if success_rates else "stable"
        )
        ...
        accuracy_scores = [
            ...
            if op["accuracy_score"] > 0
        ]
        accuracy_trend = (
            self._calculate_trend(accuracy_scores)
    ...
    def _assess_performance_status(
        ...
        # Check response time
        if (
            performance_record["duration_ms"]
            > thresholds.get("response_time_ms", float("inf")) * 2
        ):
            return "poor"
        elif performance_record["duration_ms"] > thresholds.get(
            "response_time_ms", float("inf")
        ):
            return "fair"
        ...
        if performance_record["accuracy_score"] > 0:
            if performance_record["accuracy_score"] < thresholds.get(
                "accuracy_score", 0.0
            ):
                return "fair"
    ...
    def _generate_performance_recommendations(
        ...
        if performance_record["duration_ms"] > 1000:  # Over 1 second
            if "arabic" in operation_type.lower():
                recommendations.append(
                    "Consider optimizing Arabic text processing algorithms or caching Arabic language models"
                )
            elif "cultural" in operation_type.lower():
                recommendations.append(
                    "Cultural validation taking too long - consider pre-computing common validations or optimizing rule engine"
                )
            elif "payment" in operation_type.lower():
                recommendations.append(
                    "Payment processing slow - check network connectivity to Iraqi payment gateways and consider timeout optimizations"
                )

        # Trend-based recommendations
        if trends.get("response_time_trend") == "increasing":
            recommendations.append(
                f"Response times for {operation_type} are increasing over time - investigate resource constraints or code degradation"
            )

        if trends.get("success_rate_trend") == "decreasing":
            recommendations.append(
                f"Success rates for {operation_type} are declining - review error patterns and implement preventive measures"
            )

        if trends.get("accuracy_trend") == "decreasing":
            recommendations.append(
                f"Accuracy for {operation_type} is declining - review model performance and retrain if necessary"
            )

        # Iraqi-specific recommendations
        if not performance_record["success"] and "cultural" in operation_type.lower():
            recommendations.append(
                "Cultural validation failure - review Iraqi cultural guidelines and Islamic compliance rules"
            )

        if not performance_record["success"] and "arabic" in operation_type.lower():
            recommendations.append(
                "Arabic processing failure - check UTF-8 encoding, RTL rendering, and Iraqi dialect recognition"
            )
    ...
    def get_performance_dashboard(self) -> Dict[str, Any]:
        ...
        overall_success_rate = (
            sum(1 for op in recent_history if op["success"]) / total_operations
            if total_operations > 0
            else 0
        )
        average_response_time = (
            sum(op["duration_ms"] for op in recent_history) / total_operations
            if total_operations > 0
            else 0
        )
        ...
        for category in [
            ...
            category_ops = [
                ...
                if self._map_operation_to_threshold_category(op["operation_type"])
                == category
            ]
            ...
            if category_ops:
                category_metrics[category] = {
                    ...
                    "success_rate": sum(1 for op in category_ops if op["success"])
                    / len(category_ops),
                    "average_response_time_ms": sum(
                        op["duration_ms"] for op in category_ops
                    )
                    / len(category_ops),
                    ...
                    / len([op for op in category_ops if op["accuracy_score"] > 0])
                    if any(op["accuracy_score"] > 0 for op in category_ops)
                    else 0,
                }
        ...
        for op in recent_operations:
            alerts = asyncio.run(
                self._check_performance_alerts(op["operation_type"], op)
            )
        ...
        return {
            ...
            "top_recommendations": self._generate_dashboard_recommendations(
                category_metrics, active_alerts
            ),
        }
    ...
    def _calculate_overall_health_score(
        ...
        for category, metrics in category_metrics.items():
            ...
            if threshold_category in self.alert_thresholds:
                max_time = self.alert_thresholds[threshold_category].get(
                    "response_time_ms", 1000
                )
                time_score = max(
                    0, 1 - (metrics["average_response_time_ms"] / (max_time * 2))
                )  # Score 0 if 2x threshold
    ...
    def _generate_dashboard_recommendations(
        ...
        if critical_alerts:
            recommendations.append(
                f"CRITICAL: {len(critical_alerts)} critical performance issues require immediate attention"
            )
        ...
        for category, metrics in category_metrics.items():
            if metrics["success_rate"] < 0.9:
                recommendations.append(
                    f"Low success rate in {category}: {metrics['success_rate']:.1%} - investigate error patterns"
                )
            ...
            if threshold_category in self.alert_thresholds:
                ...
                if metrics["average_response_time_ms"] > max_time:
                    recommendations.append(
                        f"Slow response times in {category}: {metrics['average_response_time_ms']:.0f}ms - optimize processing"
                    )
        ...
        if "arabic_processing" in category_metrics:
            ...
            if arabic_metrics["success_rate"] < 0.95:
                recommendations.append(
                    "Arabic text processing issues detected - review encoding, RTL rendering, and dialect recognition"
                )
        ...
        if "cultural_validation" in category_metrics:
            ...
            if cultural_metrics["success_rate"] < 0.95:
                recommendations.append(
                    "Cultural validation issues detected - review Islamic compliance rules and Iraqi professional standards"
                )
...
class IraqiSequentialThinkingEngine:
    ...
    def __init__(
        ...
        debugging_intelligence: IraqiDebuggingIntelligence,
    ):
    ...
    async def process_thought(
        ...
        **kwargs,
        ...
        try:
            ...
            cultural_validation = (
                await self.cultural_validator.validate_cultural_appropriateness(
            ...
            thought_data.islamic_compliance_check = (
                cultural_validation["islamic_compliance"] >= 0.90
            )
            thought_data.cultural_validation_status = cultural_validation[
                "approval_status"
            ]
            ...
            # Confidence scoring
            thought_data.confidence_score = self._calculate_thought_confidence(
                thought_data
            )
            ...
            logger.info(
                f"Processed thought {thought_number}/{total_thoughts} with {thought_data.cultural_appropriateness:.2f} cultural score"
            )
            return thought_data
            ...
            if debug_metadata.resolution_confidence > 0.7:
                ...
                if (
                    self.error_recovery_attempts[recovery_key] < 2
                ):  # Max 2 recovery attempts
                    self.error_recovery_attempts[recovery_key] += 1
                    logger.info(
                        f"Attempting error recovery for thought processing (attempt {self.error_recovery_attempts[recovery_key]})"
                    )
                    ...
                    try:
                        ...
                        thought_data.cultural_appropriateness = simplified_validation[
                            "overall_score"
                        ]
                        ...
                        thought_data.error_details["recovery_error"] = str(
                            recovery_error
                        )
    ...
    async def _process_arabic_content(self, thought_data: IraqiThoughtData) -> None:
        ...
        if arabic_matches:
            ...
            thought_data.english_translation = (
                f"[Arabic content detected: {len(arabic_matches)} segments]"
            )

    async def _analyze_professional_relevance(
        self, thought_data: IraqiThoughtData
    ) -> None:
        ...
        if thought_data.cultural_context in domain_mapping:
            thought_data.professional_domain_relevance = domain_mapping[
                thought_data.cultural_context
            ]

            # Calculate professional relevance score
            professional_keywords = {
                ProfessionalDomain.LEGAL_LAW: [
                    "law",
                    "legal",
                    "court",
                    "judgment",
                ...
                ProfessionalDomain.MEDICAL_HEALTHCARE: [
                    "medical",
                    "health",
                    "patient",
                    "treatment",
                ...
                ProfessionalDomain.EDUCATIONAL_ACADEMIC: [
                    "education",
                    "student",
                    "learning",
                    "curriculum",
                ...
                ProfessionalDomain.GOVERNMENT_PUBLIC: [
                    "government",
                    "public",
                    "service",
                    "policy",
            ...
            if thought_data.professional_domain_relevance in professional_keywords:
                keywords = professional_keywords[
                    thought_data.professional_domain_relevance
                ]
                relevance_count = sum(
                    1 for keyword in keywords if keyword in thought_data.thought.lower()
                )
                thought_data.professional_relevance_score = min(
                    1.0, relevance_count * 0.15
                )
    ...
    def get_thought_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cultural_contexts": list(
                set(t.cultural_context.value for t in self.thought_history)
            ),
            "language_modes": list(
                set(t.language_detected.value for t in self.thought_history)
            ),
            "average_cultural_score": sum(
                t.cultural_appropriateness for t in self.thought_history
            )
            / len(self.thought_history)
            if self.thought_history
            else 0,
            "islamic_compliance_rate": sum(
                1 for t in self.thought_history if t.islamic_compliance_check
            )
            / len(self.thought_history)
            if self.thought_history
            else 0,
            "professional_thoughts": sum(
                1 for t in self.thought_history if t.professional_domain_relevance
            )
            / len(self.thought_history)
            if self.thought_history
            else 0,
            "branch_count": len(self.branches),
            "arabic_content_detected": sum(
                1 for t in self.thought_history if t.arabic_content
            )
            / len(self.thought_history)
            if self.thought_history
            else 0,
...
class IraqiTrajectoryRecorder:
    ...
    def __init__(
        ...
        enable_advanced_debugging: bool = True,
    ):
        ...
        self.debugging_intelligence = (
            IraqiDebuggingIntelligence() if enable_advanced_debugging else None
        )
        self.thinking_engine = (
            IraqiSequentialThinkingEngine(
                self.cultural_validator, self.debugging_intelligence
            )
            if self.debugging_intelligence
            else IraqiSequentialThinkingEngine(self.cultural_validator, None)
        )
        self.performance_monitor = (
            IraqiPerformanceMonitor() if enable_advanced_debugging else None
        )
            ...
            "metadata": {
                "system_info": {
                    ...
                    "professional_domains": [
                        domain.value for domain in ProfessionalDomain
                    ],
                }
            },
            "task_info": {
    ...
    async def start_recording(
        ...
        logger.info(
            f"Started Iraqi trajectory recording: {task} | Context: {cultural_context.value}"
        )
    ...
    async def record_llm_interaction(
        ...
        if self.enable_cultural_validation:
            await self._perform_interaction_cultural_validation(
                messages, response, cultural_metadata
            )
        ...
        if "usage" in response and response["usage"]:
            ...
            self.trajectory_data["performance_metrics"]["total_tokens_used"] += (
                input_tokens + output_tokens
            )

        await self._save_trajectory()
        logger.debug(
            f"Recorded LLM interaction: {provider}/{model} | Cultural context: {cultural_context.value}"
        )
    ...
    async def record_agent_step(
        ...
        **kwargs,
        ...
        if self.enable_cultural_validation and "reflection" in kwargs:
            await self._perform_step_cultural_validation(
                kwargs["reflection"], cultural_metadata
            )
        ...
        await self._save_trajectory()
        logger.debug(
            f"Recorded agent step {step_number}: {state.value} | Cultural context: {cultural_context.value}"
        )
    ...
    async def record_sequential_thought(
        ...
        **kwargs,
        ...
        await self._save_trajectory()
        logger.debug(
            f"Recorded thought {thought_number}/{total_thoughts}: Cultural score {thought_data.cultural_appropriateness:.2f}"
        )
    ...
    async def _perform_interaction_cultural_validation(
        ...
        try:
            ...
            # Update metadata
            cultural_metadata.islamic_compliance_score = validation_result[
                "islamic_compliance"
            ]
            cultural_metadata.cultural_appropriateness_score = validation_result[
                "overall_score"
            ]
            cultural_metadata.cultural_validation_notes = validation_result.get(
                "recommendations", []
            )
            ...
            self.trajectory_data["cultural_validations"].append(
                {
                    ...
                    "content_preview": content_to_validate[:100] + "..."
                    if len(content_to_validate) > 100
                    else content_to_validate,
                    "validation_result": validation_result,
                }
            )

            self.trajectory_data["performance_metrics"][
                "cultural_validation_calls"
            ] += 1

        except Exception as e:
            logger.error(f"Cultural validation error in interaction: {e}")
            cultural_metadata.cultural_validation_notes.append(
                f"Validation error: {str(e)}"
            )

    async def _perform_step_cultural_validation(
        self, reflection: str, cultural_metadata: IraqiCulturalMetadata
    ) -> None:
    ...
    async def _process_interaction_arabic_content(
        self, interaction: IraqiLLMInteraction
    ) -> None:
        ...
        if input_arabic or response_arabic:
            ...
            interaction.cultural_metadata.arabic_text_accuracy = (
                0.90  # Default good accuracy
            )
    ...
    async def finalize_recording(
        ...
        # Update task info
        self.trajectory_data["task_info"].update(
            {"end_time": end_time.isoformat(), "execution_time_seconds": execution_time}
        )
        ...
        logger.info(
            f"Finalized Iraqi trajectory recording: {success} | Duration: {execution_time:.2f}s | Cultural score: {self.trajectory_data['cultural_intelligence']['cultural_appropriateness_score']:.2f}"
        )
    ...
    async def _calculate_cultural_intelligence_metrics(self) -> None:
        ...
        self.trajectory_data["cultural_intelligence"].update(
            {
                "overall_islamic_compliance": sum(islamic_scores) / len(islamic_scores)
                if islamic_scores
                else 0.0,
                "cultural_appropriateness_score": sum(cultural_scores)
                / len(cultural_scores)
                if cultural_scores
                else 0.0,
                "arabic_processing_accuracy": sum(arabic_accuracies)
                / len(arabic_accuracies)
                if arabic_accuracies
                else 0.0,
                "professional_compliance_score": sum(professional_scores)
        ...
        self.trajectory_data["cultural_intelligence"][
            "cultural_context_distribution"
        ] = context_dist
        ...
        self.trajectory_data["cultural_intelligence"]["language_distribution"] = (
            lang_dist
        )
    ...
    async def _generate_recommendations(self) -> List[str]:
        ...
        cultural_score = self.trajectory_data["cultural_intelligence"][
            "cultural_appropriateness_score"
        ]
        islamic_score = self.trajectory_data["cultural_intelligence"][
            "overall_islamic_compliance"
        ]
        arabic_score = self.trajectory_data["cultural_intelligence"][
        ...
        if cultural_score < 0.85:
            recommendations.append(
                "Improve cultural sensitivity by incorporating more Iraqi cultural awareness in responses"
            )

        # Islamic compliance recommendations
        if islamic_score < 0.90:
            recommendations.append(
                "Enhance Islamic compliance by ensuring all content respects Islamic principles and values"
            )

        # Arabic processing recommendations
        if arabic_score > 0 and arabic_score < 0.90:
            recommendations.append(
                "Improve Arabic text processing accuracy and dialect recognition capabilities"
            )

        # Professional domain recommendations
        prof_score = self.trajectory_data["cultural_intelligence"][
            "professional_compliance_score"
        ]
        if prof_score > 0 and prof_score < 0.85:
            recommendations.append(
                "Strengthen professional domain expertise and compliance standards"
            )
        ...
        if total_steps > 15:
            recommendations.append(
                "Optimize execution efficiency to reduce number of steps required"
            )

        # Language diversity recommendations
        lang_dist = self.trajectory_data["cultural_intelligence"][
            "language_distribution"
        ]
        if len(lang_dist) == 1 and "english" in str(lang_dist).lower():
            recommendations.append(
                "Consider incorporating Arabic language processing for better Iraqi cultural integration"
            )
    ...
    async def _save_trajectory(self) -> None:
        ...
        try:
            ...
            with open(self.trajectory_path, "w", encoding="utf-8") as f:
                json.dump(
                    self.trajectory_data, f, indent=2, ensure_ascii=False, default=str
                )
    ...
    def get_summary_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cultural_intelligence_summary": self.trajectory_data[
                "cultural_intelligence"
            ],
            "performance_metrics": self.trajectory_data["performance_metrics"],
            "thinking_summary": self.thinking_engine.get_thought_summary(),
            "execution_time": self.trajectory_data["task_info"][
                "execution_time_seconds"
            ],
            "recommendations": self.trajectory_data["execution_summary"][
                "recommendations"
            ],
        }
...
async def main():
    ...
    await recorder.record_llm_interaction(
        messages=[
            {
                "role": "system",
                "content": "You are an Iraqi legal assistant respecting Islamic principles",
            },
            {
                "role": "user",
                "content": "Please analyze this contract while respecting Iraqi civil law and Islamic jurisprudence",
            },
        ],
        response={
    ...
    print(
        f"- Cultural Appropriateness Score: {summary['cultural_intelligence_summary']['cultural_appropriateness_score']:.2f}"
    )
    print(
        f"- Islamic Compliance Score: {summary['cultural_intelligence_summary']['overall_islamic_compliance']:.2f}"
    )
    print(
        f"- Professional Compliance Score: {summary['cultural_intelligence_summary']['professional_compliance_score']:.2f}"
    )
