...
class IraqiSequentialThinkingTool:
    ...
    async def add_thought(
        ...
        self._validate_thought_parameters(
            thought, thought_number, total_thoughts, next_thought_needed
        )

        # Create cultural context
        cultural_context = await self._create_cultural_context(
        ...
        if self.professional_domain:
            professional_context = await self._create_professional_context(
                thought, self.professional_domain
            )
        ...
        if professional_context:
            professional_compliance_score = (
                await self.professional_analyzer.validate_thought(
                    thought, self.professional_domain, professional_context
                )
            )

        # Analyze Arabic context considerations
        arabic_context_considered = await self.arabic_context_processor.analyze_thought(
            thought
        )

        # Check for Islamic principles application
        islamic_principles_applied = await self._check_islamic_principles_application(
            thought
        )

        # Assess family and community impact
        family_community_impact_assessed = await self._assess_family_community_impact(
            thought
        )

        # Analyze government service implications
        government_service_implications = (
            await self.government_service_analyzer.analyze_implications(
                thought, self.thinking_context
            )
        )
    ...
    async def get_thinking_summary(self) -> IraqiThinkingSummary:
        ...
        professional_scores = [
            t.professional_compliance_score
            for t in self.thought_history
            if t.professional_compliance_score > 0
        ]

        cultural_compliance_average = (
            sum(cultural_scores) / len(cultural_scores) if cultural_scores else 0.0
        )
        professional_compliance_average = (
        ...
        for thought in self.thought_history:
            if thought.islamic_principles_applied:
                islamic_principles.extend(
                    ["justice", "compassion", "community_welfare", "family_values"]
                )
        ...
        if self.thought_history:
            final_solution_cultural_score = self.thought_history[
                -1
    ...
    def format_thought_for_display(self, thought_data: IraqiThoughtData) -> str:
        ...
        if thought_data.is_revision:
            ...
            context = (
                f" ({branch_type_ar} from thought {thought_data.branch_from_thought})"
            )
        else:
...
└{border}┘"""
    ...
    def _validate_thought_parameters(
        self,
        thought: str,
        thought_number: int,
        total_thoughts: int,
    ...
    async def _create_cultural_context(
        self, thought: str, thinking_context: IraqiThinkingContext
    ) -> IraqiCulturalContext:
        """Create cultural context for the thought"""
        return IraqiCulturalContext(
            islamic_compliance_required=thinking_context
            in [IraqiThinkingContext.FAMILY, IraqiThinkingContext.PROFESSIONAL],
            family_impact_consideration=thinking_context == IraqiThinkingContext.FAMILY,
            professional_ethics_applicable=thinking_context
            == IraqiThinkingContext.PROFESSIONAL,
    ...
    async def _create_professional_context(
        self, thought: str, domain: IraqiProfessionalDomain
    ) -> IraqiProfessionalContext:
    ...
    async def _generate_cultural_recommendations(
        self, thought: str, cultural_score: float, context: IraqiThinkingContext
    ) -> List[str]:
    ...
    async def _update_insights_and_recommendations(
        self, thought_data: IraqiThoughtData
    ):
        """Update cultural insights and professional recommendations"""
        if thought_data.cultural_validation_score >= 0.90:
            self.cultural_insights.append(
                f"Excellent cultural alignment in thought {thought_data.thought_number}"
            )

        if thought_data.professional_compliance_score >= 0.90:
            self.professional_recommendations.append(
                f"Professional excellence demonstrated in thought {thought_data.thought_number}"
            )

    async def _format_thought_response(
        self, thought_data: IraqiThoughtData
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_cultural_compliance": sum(
                t.cultural_validation_score for t in self.thought_history
            )
...
class IraqiCulturalThinkingValidator:
    ...
    async def validate_thought(
        self,
        thought: str,
        context: IraqiThinkingContext,
        cultural_context: IraqiCulturalContext,
    ) -> float:
...
class IraqiProfessionalThinkingAnalyzer:
    ...
    async def validate_thought(
        ...
        professional_context: IraqiProfessionalContext,
    ) -> float:
...
class GovernmentServiceThinkingAnalyzer:
    ...
    async def analyze_implications(
        self, thought: str, context: IraqiThinkingContext
    ) -> List[str]:
        """Analyze government service implications"""
        if context == IraqiThinkingContext.GOVERNMENT:
            return [
                "ministry_coordination",
                "citizen_service_improvement",
