...
async def test_enhanced_system():
    ...
    try:
        ...
        print(
            f"      - Category: {debug_result.error_category.value if debug_result.error_category else 'unknown'}"
        )
        print(f"      - Debug Level: {debug_result.debug_level.value}")
        print(
            f"      - Resolution Confidence: {debug_result.resolution_confidence:.2f}"
        )
        ...
        print(
            f"      - Advanced Debugging: {integration_status['advanced_debugging_enabled']}"
        )
        print(
            f"      - Engine Available: {integration_status['debugging_engine_available']}"
        )
        ...
        integration_result = await integration_manager.integrate_with_trajectory_step(
            step_data=test_step_data, cultural_context="business_commercial"
        )

        print(f"   📈 Step Integration Result:")
        print(
            f"      - Debugging Enabled: {integration_result.get('debugging_enabled', False)}"
        )
        print(
        ...
        if integration_result.get("recommendations"):
            print(
                f"      - Recommendations: {len(integration_result['recommendations'])}"
            )
        ...
        print(
            f"      - Dialect Classification: {arabic_analysis['dialect_classification']}"
        )
        ...
        cultural_validation = await debug_engine._perform_cultural_validation(
            cultural_test_content, "professional_educational"
        )

        print(f"   ☪️  Cultural Validation:")
        print(
            f"      - Islamic Compliance: {cultural_validation['islamic_compliance_score']:.2f}"
        )
        print(
            f"      - Cultural Appropriateness: {cultural_validation['cultural_appropriateness_score']:.2f}"
        )
