...
class IraqiTrajectoryRecorder:
    ...
    def __init__(self, trajectory_path: Optional[str] = None, 
        ...
        if trajectory_path is None:
            ...
            domain_prefix = iraqi_context.professional_domain.value if iraqi_context else "general"
            trajectory_path = f"trajectories/iraqi_{domain_prefix}_trajectory_{timestamp}.json"
    ...
    def start_recording(self, task: str, provider: str, model: str, max_steps: int, 
                       iraqi_context: IraqiContext = None) -> None:
        ...
        self.trajectory_data["professional_domain_tracking"]["domain"] = self.iraqi_context.professional_domain.value
        
        # Log initialization
        self.logger.info(f"Started Iraqi trajectory recording for {task} in {self.iraqi_context.professional_domain.value} domain")
    ...
    def record_iraqi_agent_step(self,
        ...
        iraqi_context = {
            ...
            "islamic_compliance": {
                ...
                "halal_status": islamic_compliance_result.halal_status.value if islamic_compliance_result else None
            },
            "arabic_processing": {
            ...
            "professional_domain": {
                "domain": professional_domain_result.domain.value if professional_domain_result else None,
        ...
        if arabic_processing_result:
            ...
            if arabic_processing_result.rtl_accuracy:
                self.trajectory_data["language_processing_metrics"]["rtl_layout_events"] += 1
            if arabic_processing_result.dialect_recognition:
                self.trajectory_data["language_processing_metrics"]["dialect_recognition_events"] += 1
        
        if professional_domain_result:
            self.trajectory_data["professional_domain_tracking"]["professional_workflow_events"].append({
                "step": step_number,
                "timestamp": datetime.now().isoformat(),
    ...
    def record_government_service_interaction(self, step_number: int, 
                                            service_type: str,
        ...
        interaction = {
            ...
            "ministry": ministry,
            "citizen_id_hash": hashlib.sha256(citizen_id.encode()).hexdigest() if citizen_id else None,
            "interaction_type": interaction_type,
            "success": success,
            "response_summary": self._summarize_response_data(response_data) if response_data else None
        }
        
        self.trajectory_data["government_service_interactions"].append(interaction)
        self.save_trajectory()

    def record_citizen_impact_assessment(self, step_number: int,
                                       impact_level: str,
        ...
        assessment = {
            ...
            "assessment_id": hashlib.md5(f"{step_number}_{impact_level}_{datetime.now().isoformat()}".encode()).hexdigest()[:8]
        }
        
        self.trajectory_data["citizen_impact_assessments"].append(assessment)
        self.save_trajectory()

    def record_family_context_preservation(self, step_number: int,
    ...
    def finalize_iraqi_recording(self, success: bool, final_result: Optional[str] = None,
                               cultural_compliance_summary: Optional[Dict[str, Any]] = None,
                               islamic_compliance_summary: Optional[Dict[str, Any]] = None) -> None:
        ...
        iraqi_summary = {
            "cultural_compliance_summary": cultural_compliance_summary or self._generate_cultural_summary(),
            "islamic_compliance_summary": islamic_compliance_summary or self._generate_islamic_summary(),
        ...
        self.logger.info(f"Iraqi trajectory recording completed: Success={success}, "
    ...
    def _generate_cultural_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "compliance_trend": "improving" if scores[-1] > scores[0] else "declining" if len(scores) > 1 else "stable"
        }
    ...
    def _generate_islamic_summary(self) -> Dict[str, Any]:
        ...
        all_considerations = [cons for c in compliance_checks for cons in c["result"]["considerations"]]
        ...
        return {
            ...
            "final_halal_status": compliance_checks[-1]["result"]["halal_status"] if compliance_checks else "unknown"
        }

    def _generate_professional_summary(self) -> Dict[str, Any]:
        """Generate professional domain summary"""
        domain_events = self.trajectory_data["professional_domain_tracking"]["professional_workflow_events"]
        
        return {
            "domain": self.iraqi_context.professional_domain.value,
            "domain_specific_events": len(domain_events),
            "standards_validated": len(set(
                std for event in domain_events 
                for std in event.get("domain_result", {}).get("standards", [])
            )),
            "requirements_tracked": len(set(
                req for event in domain_events 
    ...
    def _generate_arabic_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_rtl_accuracy": sum(
                e["result"]["rtl_accuracy"] for e in processing_events 
                if e["result"]["rtl_accuracy"]
            ) / len([e for e in processing_events if e["result"]["rtl_accuracy"]]) if processing_events else 0.0
        }
    ...
    def _generate_government_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "unique_ministries": len(set(i["ministry"] for i in interactions if i["ministry"])),
            "success_rate": len([i for i in interactions if i["success"]]) / len(interactions) if interactions else 0.0
        }
    ...
    def _generate_citizen_impact_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "impact_distribution": {level: impact_levels.count(level) for level in set(impact_levels)},
            "services_affected": len(set(
                service for assessment in assessments 
    ...
    def _generate_family_context_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "privacy_protection_rate": len([r for r in preservation_records if r["family_privacy_protected"]]) / len(preservation_records),
            "cultural_norms_respect_rate": len([r for r in preservation_records if r["cultural_norms_respected"]]) / len(preservation_records)
        }
    ...
    def _serialize_llm_response(self, response: Any) -> Dict[str, Any]:
        ...
        if hasattr(response, 'tool_calls') and response.tool_calls:
            data["tool_calls"] = [self._serialize_tool_call(tc) for tc in response.tool_calls]
    ...
    def save_trajectory(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to save Iraqi trajectory to {self.trajectory_path}: {e}")
...
class IraqiCulturalValidator:
    ...
    async def validate_step_content(self, content: str, context: Dict[str, Any]) -> CulturalValidationResult:
...
class ProfessionalDomainTracker:
    ...
    async def track_domain_activity(self, domain: ProfessionalDomain, 
                                  activity: str) -> ProfessionalDomainResult:
