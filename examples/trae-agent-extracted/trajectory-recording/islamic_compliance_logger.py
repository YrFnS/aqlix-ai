...
class ComplianceLevel(Enum):
    ...
    CRITICAL_VIOLATION = "critical_violation" # Serious violation requiring immediate attention
...
class IslamicValidator(ABC):
    ...
    @abstractmethod
    async def validate_principle(self, content: str, 
                               context: IslamicContext) -> List[ComplianceIssue]:
...
class IslamicComplianceLogger:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.cultural_sensitivity_weight = self.config.get("cultural_sensitivity_weight", 0.3)
    ...
    async def validate_islamic_compliance(self, content: str,
                                        islamic_context: Optional[IslamicContext] = None,
                                        content_id: Optional[str] = None,
                                        trajectory_step: Optional[int] = None) -> IslamicComplianceResult:
        """
        ...
        islamic_context = islamic_context or IslamicContext(ValidationContext.GENERAL_INTERACTION)
        content_id = content_id or self._generate_content_id(content)
        
        self.logger.info(f"Starting Islamic compliance validation for content: {content_id}")
        ...
        try:
            ...
            for principle, validator in self.principle_validators.items():
                principle_issues = await validator.validate_principle(content, islamic_context)
                all_issues.extend(principle_issues)
                
                # Calculate principle score
                principle_score = await self._calculate_principle_score(principle, principle_issues)
            ...
            rule_issues = await self._validate_against_islamic_rules(content, islamic_context)
            all_issues.extend(rule_issues)
            
            # Cultural appropriateness analysis
            cultural_score = await self._analyze_cultural_appropriateness(content, islamic_context)
            
            # Community acceptance analysis
            community_score = await self._analyze_community_acceptance(content, islamic_context)
            ...
            if result.scholar_review_required or overall_compliance in [
                ComplianceLevel.NON_COMPLIANT, ComplianceLevel.CRITICAL_VIOLATION
            ]:
                scholar_request = await self._request_scholar_review(result, islamic_context)
                if scholar_request:
                    result.validation_notes.append(f"Scholar review requested: {scholar_request.request_id}")
            
            # Track validation result
            await self._track_compliance_result(result, trajectory_step)
            
            processing_time = time.time() - start_time
            self.logger.info(f"Islamic compliance validation completed: {overall_compliance.value}, "
                           f"Issues={len(all_issues)}, Time={processing_time:.2f}s")
            
            return result
            
        except Exception as e:
            self.logger.error(f"Islamic compliance validation failed for {content_id}: {str(e)}")
            result.overall_compliance = ComplianceLevel.CRITICAL_VIOLATION
            result.issues_found = [ComplianceIssue(
    ...
    async def track_trajectory_step_compliance(self, trajectory_id: str, step_number: int,
                                             step_content: str, 
                                             islamic_context: Optional[IslamicContext] = None) -> Dict[str, Any]:
        ...
        compliance_result = await self.validate_islamic_compliance(
            step_content, islamic_context, f"{trajectory_id}_step_{step_number}", step_number
        )
        ...
        step_compliance = {
            ...
            "principle_scores": {p.value: score for p, score in compliance_result.principle_scores.items()},
            "issues_count": len(compliance_result.issues_found),
            "critical_issues": len([i for i in compliance_result.issues_found if i.severity == "critical"]),
    ...
    async def resolve_compliance_issue(self, issue_id: str, resolution: str,
                                     resolved_by: str, scholar_approved: bool = False) -> bool:
        ...
        if issue_id in self.active_issues:
            ...
            # Update statistics
            await self._update_resolution_statistics(issue, resolution, scholar_approved)
            
            self.logger.info(f"Islamic compliance issue {issue_id} resolved by {resolved_by}")
            return True
        else:
            self.logger.warning(f"Attempted to resolve unknown compliance issue: {issue_id}")
            return False
    
    async def get_compliance_report(self, timeframe: str = "day",
        ...
        recent_validations = [
            v for v in self.compliance_history 
            if v.validation_timestamp >= cutoff
        ]
        
        if not recent_validations:
            return {"report": "No Islamic compliance validations found for specified timeframe"}
        
        # Overall statistics
        total_validations = len(recent_validations)
        compliant_validations = len([
            v for v in recent_validations 
            if v.overall_compliance in [ComplianceLevel.COMPLIANT, ComplianceLevel.MOSTLY_COMPLIANT]
        ])
        ...
        for principle in IslamicPrinciple:
            ...
            if principle_scores:
                principle_analysis[principle.value] = {
                    ...
                    "compliance_trend": "improving" if len(principle_scores) > 1 and principle_scores[-1] > principle_scores[0] else "stable"
                }
        ...
        scholar_reviews_requested = len([v for v in recent_validations if v.scholar_review_required])
        scholar_review_rate = scholar_reviews_requested / total_validations if total_validations > 0 else 0.0
        
        # Cultural appropriateness analysis
        cultural_scores = [v.cultural_appropriateness_score for v in recent_validations if v.cultural_appropriateness_score > 0]
        avg_cultural_score = sum(cultural_scores) / len(cultural_scores) if cultural_scores else 0.0
        
        # Community acceptance analysis
        community_scores = [v.community_acceptance_score for v in recent_validations if v.community_acceptance_score > 0]
        avg_community_score = sum(community_scores) / len(community_scores) if community_scores else 0.0
        ...
        report = {
            ...
            "issue_analysis": {
                ...
                "top_recurring_issues": [{"issue": issue, "frequency": freq} for issue, freq in top_issues]
            },
            "cultural_compliance": {
                "cultural_appropriateness_trend": await self._calculate_cultural_trend(recent_validations),
                "community_acceptance_trend": await self._calculate_community_trend(recent_validations),
                "regional_variations": await self._analyze_regional_variations(recent_validations)
            },
            "recommendations": await self._generate_compliance_recommendations(recent_validations),
            "scholar_review_summary": {
                "pending_reviews": len([r for r in self.scholar_review_requests.values() if r.review_status == "pending"]),
    ...
    async def _validate_against_islamic_rules(self, content: str,
                                           islamic_context: IslamicContext) -> List[ComplianceIssue]:
        ...
        for rule in self.validation_rules.values():
            ...
            if rule.madhab_specific and islamic_context.madhab_preference not in rule.applicable_madhabs:
                continue
    ...
    async def _calculate_principle_score(self, principle: IslamicPrinciple,
        ...
        total_penalty = sum(severity_weights.get(issue.severity, 0.5) for issue in principle_issues)
        max_penalty = len(principle_issues) * 1.0
        
        penalty_ratio = min(total_penalty / max_penalty, 1.0) if max_penalty > 0 else 0.0
        
        return max(0.0, 1.0 - penalty_ratio)
    
    async def _calculate_overall_compliance(self, principle_scores: Dict[IslamicPrinciple, float],
                                         all_issues: List[ComplianceIssue],
                                         cultural_score: float,
                                         community_score: float) -> ComplianceLevel:
    ...
    async def _analyze_cultural_appropriateness(self, content: str,
                                              islamic_context: IslamicContext) -> float:
        """Analyze cultural appropriateness for Iraqi Islamic context"""
        
        score = 0.8  # Base score
        
        # Check for Islamic greetings and expressions
        islamic_expressions = ["السلام عليكم", "بسم الله", "الحمد لله", "إن شاء الله", "ما شاء الله"]
    ...
    async def _analyze_community_acceptance(self, content: str,
                                          islamic_context: IslamicContext) -> float:
    ...
    async def _request_scholar_review(self, compliance_result: IslamicComplianceResult,
                                    islamic_context: IslamicContext) -> Optional[ScholarReviewRequest]:
        ...
        for issue in compliance_result.issues_found:
            if issue.requires_scholar_review:
                questions.append(f"Regarding {issue.principle.value}: {issue.description}")
    ...
    def _generate_issue_id(self, base: str) -> str:
        ...
        return hashlib.md5(f"{base}_{datetime.now().isoformat()}".encode()).hexdigest()[:8]
    
    async def _track_compliance_result(self, result: IslamicComplianceResult,
                                     trajectory_step: Optional[int] = None) -> None:
    ...
    async def _update_compliance_statistics(self, result: IslamicComplianceResult,
                                          trajectory_step: Optional[int] = None) -> None:
        ...
        if today not in self.compliance_statistics:
            self.compliance_statistics[today] = {
                ...
                "principle_issues": {principle.value: 0 for principle in IslamicPrinciple},
        ...
        stats["average_cultural_score"] = (
            (current_cultural_avg * (total_validations - 1) + result.cultural_appropriateness_score) / total_validations
        )
        stats["average_community_score"] = (
            (current_community_avg * (total_validations - 1) + result.community_acceptance_score) / total_validations
        )
    ...
    def _initialize_validation_rules(self):
        ...
        self.validation_rules["justice_001"] = IslamicValidationRule(
            ...
            applicable_contexts=[ValidationContext.LEGAL_PROCEEDING, ValidationContext.GOVERNMENT_SERVICE],
            validation_criteria=["Equal treatment", "Fair procedures", "No discrimination"],
        ...
        self.validation_rules["privacy_001"] = IslamicValidationRule(
            ...
            applicable_contexts=[ValidationContext.FAMILY_MATTERS, ValidationContext.HEALTHCARE_SERVICE],
            ...
            severity="high",
            cultural_considerations=["Family honor", "Women's privacy", "Home sanctity"]
        )
...
class TawhidValidator(IslamicValidator):
    async def validate_principle(self, content: str, context: IslamicContext) -> List[ComplianceIssue]:
        issues = []
...
class JusticeValidator(IslamicValidator):
    async def validate_principle(self, content: str, context: IslamicContext) -> List[ComplianceIssue]:
        ...
        if context.validation_context in [ValidationContext.LEGAL_PROCEEDING, ValidationContext.GOVERNMENT_SERVICE]:
            justice_indicators = ["fair", "equal", "just", "عادل", "منصف"]
            if not any(indicator in content.lower() for indicator in justice_indicators):
                issues.append(ComplianceIssue(
