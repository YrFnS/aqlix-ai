...
async def test_core_functionality():
    ...
    try:
        ...
        print(f"   🎯 Cultural appropriateness: {cultural_meta.cultural_appropriateness_score}")
        ...
        print(f"   💭 Thought data created: {thought_data.thought_number}/{thought_data.total_thoughts}")
        ...
        test_content = "Educational content that respects Islamic principles and Iraqi culture"
        ...
        for error in test_errors:
            if any(keyword in error.lower() for keyword in ["arabic", "cultural", "payment", "mcp"]):
                pattern_matches += 1
        
        print(f"   🎯 Pattern recognition test: {pattern_matches}/{len(test_errors)} patterns detected")
        ...
        iraqi_detected = sum(1 for indicator in iraqi_indicators if indicator in iraqi_text)
        ...
        print(f"   🏢 Professional domain detection: {domain_matches}/{len(test_content_domains)}")
