...
class CulturalContextAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # Analysis components
        self.regional_analyzers: Dict[IraqiRegionalContext, 'RegionalCulturalAnalyzer'] = {}
        ...
        self.enable_professional_analysis = self.config.get("professional_analysis", True)
        self.sensitivity_threshold = self.config.get("sensitivity_threshold", CulturalSensitivityLevel.MODERATE)
    ...
    async def analyze_cultural_context(self, 
                                     ...
                                     citizen_facing: bool = False) -> CulturalContextAnalysis:
        """
        ...
        try:
            ...
            cultural_patterns = await self._identify_cultural_patterns(content, regional_context)
            analysis.cultural_patterns = cultural_patterns
            
            # Determine primary cultural contexts
            analysis.primary_cultural_contexts = await self._determine_primary_contexts(cultural_patterns)
            
            # Assess sensitivity level
            analysis.sensitivity_assessment = await self._assess_sensitivity_level(cultural_patterns, citizen_facing)
            
            # Regional context analysis
            if self.enable_regional_analysis:
                regional_analysis = await self._perform_regional_analysis(content, regional_context)
                analysis.recommendations.extend(regional_analysis.get("recommendations", []))
            ...
            if self.enable_professional_analysis and professional_context:
                professional_analysis = await self._perform_professional_analysis(content, professional_context)
                analysis.professional_considerations = professional_analysis.get("considerations", [])
            ...
            intercommunity_analysis = await self._analyze_intercommunity_sensitivities(content)
            analysis.inter_community_sensitivities = intercommunity_analysis
            
            # Calculate cultural compliance score
            analysis.cultural_compliance_score = await self._calculate_cultural_compliance_score(analysis)
            
            # Generate contextual recommendations
            context_recommendations = await self._generate_contextual_recommendations(analysis, thinking_domain)
            analysis.recommendations.extend(context_recommendations)
            
            self.logger.info(f"Cultural context analysis completed: Score={analysis.cultural_compliance_score:.2f}, "
            ...
            return CulturalContextAnalysis(
                ...
                recommendations=["Manual cultural review recommended due to analysis failure"]
            )
    
    async def get_cultural_guidance(self, 
                                  cultural_contexts: List[CulturalContextType],
                                  thinking_domain: str = "general") -> Dict[str, Any]:
        ...
        if thinking_domain == "legal":
            guidance["domain_specific_advice"].append("Iraqi legal thinking requires understanding of Islamic law integration")
            guidance["domain_specific_advice"].append("Consider both civil law and sharia principles")
        elif thinking_domain == "medical":
            guidance["domain_specific_advice"].append("Medical discussions should respect Islamic bioethics")
            guidance["domain_specific_advice"].append("Consider family decision-making in medical contexts")
        elif thinking_domain == "educational":
            guidance["domain_specific_advice"].append("Educational contexts require respect for traditional knowledge")
            guidance["domain_specific_advice"].append("Balance modern and traditional educational values")
        
        return guidance
    
    async def _identify_cultural_patterns(self, content: str, 
                                        regional_context: IraqiRegionalContext) -> List[CulturalPattern]:
        """Identify cultural patterns in content"""
        
        patterns = []
        
        # Family-related patterns
        family_terms = ["عائلة", "أسرة", "والد", "والدة", "أخ", "أخت", "زوج", "زوجة", "أطفال", 
                       "family", "father", "mother", "brother", "sister", "spouse", "children"]
        ...
        # Religious patterns
        islamic_terms = ["الله", "إسلام", "صلاة", "مسجد", "قرآن", "حج", "رمضان", 
                        "Allah", "Islam", "prayer", "mosque", "Quran", "hajj", "Ramadan"]
        ...
        professional_titles = ["دكتور", "مهندس", "أستاذ", "محامي", "طبيب", "قاضي",
                             "doctor", "engineer", "professor", "lawyer", "physician", "judge"]
        ...
        if any(title in content for title in professional_titles):
            patterns.append(CulturalPattern(
                ...
                professional_domain_relevance=["legal", "medical", "educational", "engineering"]
            ))
        
        # Government and authority patterns
        government_terms = ["حكومة", "وزير", "محافظ", "مدير", "دائرة", "خدمات حكومية",
                           "government", "minister", "governor", "director", "department", "public services"]
        ...
        sectarian_terms = ["شيعة", "سني", "طائفة", "مذهب", "Shia", "Sunni", "sectarian", "denomination"]
    ...
    async def _determine_primary_contexts(self, patterns: List[CulturalPattern]) -> List[CulturalContextType]:
    ...
    async def _assess_sensitivity_level(self, patterns: List[CulturalPattern], 
                                      citizen_facing: bool) -> CulturalSensitivityLevel:
        """Assess overall sensitivity level"""
        
        if not patterns:
            return CulturalSensitivityLevel.LOW
        
        max_sensitivity = max((pattern.sensitivity_level for pattern in patterns), 
                            key=lambda x: ["low", "moderate", "high", "critical", "sacred"].index(x.value))
        
        # Increase sensitivity if citizen-facing
        if citizen_facing:
            sensitivity_levels = [CulturalSensitivityLevel.LOW, CulturalSensitivityLevel.MODERATE, 
                                CulturalSensitivityLevel.HIGH, CulturalSensitivityLevel.CRITICAL, 
    ...
    async def _perform_regional_analysis(self, content: str, 
                                       regional_context: IraqiRegionalContext) -> Dict[str, Any]:
        ...
        if regional_context == IraqiRegionalContext.BAGHDAD:
            analysis["recommendations"].append("Consider Baghdad's diverse cultural mix in communications")
            if "عشيرة" in content or "tribe" in content.lower():
                analysis["warnings"].append("Tribal references less prominent in Baghdad urban context")
        
        elif regional_context == IraqiRegionalContext.NAJAF:
            analysis["recommendations"].append("Show extra respect for religious significance of Najaf")
            if "علماء" in content or "scholars" in content.lower():
                analysis["recommendations"].append("Religious scholarship particularly respected in Najaf")
        
        elif regional_context == IraqiRegionalContext.BASRA:
            analysis["recommendations"].append("Consider Basra's commercial and port city culture")
            if "تجارة" in content or "business" in content.lower():
                analysis["recommendations"].append("Commercial expertise highly valued in Basra")
        
        elif regional_context == IraqiRegionalContext.ERBIL:
            analysis["recommendations"].append("Consider Kurdish cultural elements in Erbil context")
            if "كردي" in content or "Kurdish" in content.lower():
                analysis["recommendations"].append("Show respect for Kurdish cultural identity")
        
        return analysis
    
    async def _perform_professional_analysis(self, content: str, 
                                           professional_context: str) -> Dict[str, Any]:
        """Perform professional context analysis"""
        
        analysis = {"considerations": []}
        
        if professional_context == "legal":
            analysis["considerations"].append("Legal discussions must consider both civil law and Islamic jurisprudence")
            analysis["considerations"].append("Professional legal ethics are paramount")
            if "فتوى" in content or "fatwa" in content.lower():
                analysis["considerations"].append("Religious legal opinions require scholarly authority")
        
        elif professional_context == "medical":
            analysis["considerations"].append("Medical advice must respect Islamic bioethics")
            analysis["considerations"].append("Family involvement in medical decisions is culturally expected")
            if "علاج" in content or "treatment" in content.lower():
                analysis["considerations"].append("Treatment recommendations should consider cultural acceptability")
        
        elif professional_context == "educational":
            analysis["considerations"].append("Educational content should respect traditional knowledge")
            analysis["considerations"].append("Teacher-student relationships follow traditional hierarchy")
    ...
    async def _analyze_family_privacy_implications(self, content: str) -> List[str]:
        ...
        privacy_sensitive_terms = ["زواج", "طلاق", "أطفال", "دخل", "مرض", 
                                 "marriage", "divorce", "children", "income", "illness"]
        
        found_terms = [term for term in privacy_sensitive_terms if term.lower() in content.lower()]
        
        if found_terms:
            concerns.append(f"Privacy-sensitive family topics detected: {', '.join(found_terms)}")
            concerns.append("Ensure family privacy is protected and information is handled confidentially")
    ...
    async def _analyze_religious_considerations(self, content: str) -> List[str]:
        ...
        religious_terms = ["حلال", "حرام", "صلاة", "زكاة", "حج", "صيام",
                          "halal", "haram", "prayer", "zakat", "hajj", "fasting"]
        
        found_religious = [term for term in religious_terms if term in content]
        
        if found_religious:
            considerations.append(f"Religious concepts mentioned: {', '.join(found_religious)}")
            considerations.append("Ensure religious accuracy and sensitivity")
            considerations.append("Consider consulting religious authorities for complex religious questions")
        
        # Check for potentially sensitive religious content
        sensitive_religious = ["جهاد", "كفر", "shirk", "jihad", "infidel"]
        found_sensitive = [term for term in sensitive_religious if term.lower() in content.lower()]
        
        if found_sensitive:
            considerations.append(f"Highly sensitive religious terms detected: {', '.join(found_sensitive)}")
    ...
    async def _analyze_intercommunity_sensitivities(self, content: str) -> List[str]:
        ...
        if found_sectarian:
            sensitivities.append("Sectarian references require neutral, inclusive approach")
        ...
        if found_ethnic:
            sensitivities.append("Ethnic references should celebrate diversity while promoting unity")
            sensitivities.append("Respect for all Iraqi ethnic communities is essential")
        
        return sensitivities
    
    async def _calculate_cultural_compliance_score(self, analysis: CulturalContextAnalysis) -> float:
        """Calculate cultural compliance score"""
        
        base_score = 1.0
        
        # Penalty for critical sensitivity patterns without proper handling
        critical_patterns = [p for p in analysis.cultural_patterns 
        ...
        sacred_patterns = [p for p in analysis.cultural_patterns 
    ...
    async def _generate_contextual_recommendations(self, analysis: CulturalContextAnalysis, 
                                                 thinking_domain: str) -> List[str]:
        """Generate contextual recommendations based on analysis"""
        
        recommendations = []
        
        if analysis.sensitivity_assessment in [CulturalSensitivityLevel.CRITICAL, CulturalSensitivityLevel.SACRED]:
            recommendations.append("High sensitivity content - consider expert cultural review")
        
        if CulturalContextType.RELIGIOUS_OBSERVANCE in analysis.primary_cultural_contexts:
        ...
        if analysis.family_privacy_concerns:
            recommendations.append("Implement additional privacy protections for family-related content")
        ...
        if thinking_domain == "government" and analysis.cultural_compliance_score < 0.9:
            recommendations.append("Government communications require highest cultural compliance standards")
    ...
    def _generate_content_id(self, content: str) -> str:
        ...
        import hashlib
        return hashlib.md5(f"{content[:100]}_{datetime.now().isoformat()}".encode()).hexdigest()[:12]
    ...
    def _initialize_cultural_knowledge(self):
        ...
        self.cultural_knowledge["family_honor"] = IraqiCulturalKnowledge(
            ...
            government_service_implications=["Privacy protection", "Confidentiality requirements"]
        )
        ...
        self.cultural_knowledge["religious_observance"] = IraqiCulturalKnowledge(
            ...
            government_service_implications=["Prayer time accommodation", "Religious holiday recognition"]
        )
