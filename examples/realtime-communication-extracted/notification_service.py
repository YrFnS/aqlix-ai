...
notification_counter = Counter(
    "iraqi_notifications_total",
    "Total notifications sent",
    ["channel", "type", "status"],
)
notification_duration = Histogram(
    "iraqi_notification_duration_seconds", "Time spent processing notifications"
)
active_notifications = Gauge(
    "iraqi_active_notifications", "Currently active notifications"
)
...
class IraqiNotificationService:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.celery_app = Celery(
                "iraqi_notifications", broker=self.celery_broker, backend=self.redis_url
            )
            ...
            logger.info(
                "Iraqi Notification Service initialization completed successfully"
            )
        ...
        try:
            ...
            templates = [
                ...
                NotificationTemplate(
                    ...
                    cultural_context=IraqiCulturalContext(
                        professional_domain="legal", islamic_compliance_required=True
                    ),
                    required_variables=[
                        "user_name",
                        "document_type",
    ...
    async def send_notification(
        ...
        try:
            logger.info(
                f"Processing notification {request.notification_id} for user {request.user_id}"
            )
            ...
            if request.notification_type in [
                NotificationType.LEGAL_NOTIFICATION,
                NotificationType.EDUCATIONAL_UPDATE,
            ]:
            ...
            logger.info(
                f"Notification {request.notification_id} processed with status: {delivery_result.status}"
            )
            ...
            logger.error(
                f"Error processing notification {request.notification_id}: {error_msg}"
            )
    ...
    async def _validate_cultural_context(
        ...
        try:
            ...
            if request.cultural_context.language_preference == "ar":
                rtl_result = await self.cultural_validators["arabic_rtl"](
                    request.message
                )
            ...
            sensitivity_result = await self.cultural_validators["cultural_sensitivity"](
                request.message, request.cultural_context.cultural_sensitivity_level
            )
            ...
            logger.info(
                f"Cultural validation completed for notification {request.notification_id}: {validation_result['passed']}"
            )
    ...
    async def _validate_ministry_protocol(
        self, subject: str, content: str, ministry: str
    ) -> Dict[str, Any]:
        ...
        return {"appropriate": True, "violations": []}
    ...
    async def _apply_template(
        ...
        try:
            ...
            logger.info(
                f"Applied template {template.name} to notification {request.notification_id}"
            )
    ...
    async def _schedule_notification(
        ...
        try:
            ...
            schedule_key = (
                f"scheduled_notifications:{request.scheduled_time.isoformat()}"
            )
            await self.redis_client.lpush(
                schedule_key, json.dumps(asdict(request), default=str)
            )

            # Set expiration
            ttl = int(
                (request.scheduled_time - datetime.now(IRAQI_TIMEZONE)).total_seconds()
            )
            await self.redis_client.expire(
                schedule_key, ttl + 3600
            )  # Extra hour buffer

            logger.info(
                f"Scheduled notification {request.notification_id} for {request.scheduled_time}"
            )
    ...
    async def _schedule_for_business_hours(
        self, request: NotificationRequest
    ) -> NotificationResult:
        ...
        scheduled_time = next_business_day.replace(
            hour=self.business_hours["start"], minute=0, second=0, microsecond=0
        )
    ...
    async def _deliver_in_app(self, request: NotificationRequest) -> NotificationResult:
        ...
        try:
            ...
            await self.redis_client.lpush(
                f"user_notifications:{request.user_id}", request.notification_id
            )
    ...
    async def _deliver_ministry_portal(
        self, request: NotificationRequest
    ) -> NotificationResult:
    ...
    async def _store_delivery_record(
        ...
        try:
            record = {
                ...
                "delivery_time": result.delivery_time.isoformat()
    ...
    async def get_notification_status(
        ...
        try:
            record_data = await self.redis_client.get(
                f"delivery_record:{notification_id}"
            )
            if not record_data:
                return None
    ...
    async def get_user_notifications(
        ...
        try:
            notification_ids = await self.redis_client.lrange(
                f"user_notifications:{user_id}", 0, limit - 1
            )
            notifications = []

            for notification_id in notification_ids:
                notification_data = await self.redis_client.get(
                    f"in_app_notification:{user_id}:{notification_id}"
                )
    ...
    def get_delivery_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            * 100,
            "cultural_validation_failures": self.delivery_stats[
                "cultural_validation_failures"
            ],
            "cultural_compliance_rate": (
                (
                    self.delivery_stats["total_sent"]
                    - self.delivery_stats["cultural_validation_failures"]
                )
                / max(1, self.delivery_stats["total_sent"])
            )
            * 100,
...
async def create_iraqi_notification_service(
    config: Dict[str, Any],
    ...
    async def example_usage():
        ...
        payment_request = NotificationRequest(
            ...
            template_variables={
                ...
                "payment_date": format_datetime(
                    datetime.now(IRAQI_TIMEZONE), locale="ar"
                ),
            },
