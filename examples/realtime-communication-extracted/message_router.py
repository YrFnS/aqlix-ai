...
routing_counter = Counter(
    "iraqi_messages_routed_total",
    "Total messages routed",
    ["route", "priority", "status"],
)
routing_duration = Histogram(
    "iraqi_routing_duration_seconds", "Time spent routing messages"
)
active_routes = Gauge("iraqi_active_routes", "Currently active routes")
delivery_success_rate = Summary(
    "iraqi_delivery_success_rate", "Message delivery success rate"
)
circuit_breaker_trips = Counter(
    "iraqi_circuit_breaker_trips_total", "Circuit breaker activations", ["channel"]
)
...
class IraqiMessageRouter:
    ...
    async def route_message(self, envelope: MessageEnvelope) -> Dict[str, Any]:
        ...
        routing_counter.labels(
            route="initiated", priority=envelope.priority.value, status="processing"
        ).inc()

        try:
            logger.info(
                f"Routing message {envelope.message_id} for user {envelope.user_id}"
            )
            ...
            if not target_routes:
                logger.warning(
                    f"No valid routes found for message {envelope.message_id}"
                )
                return {"status": "failed", "error": "No valid routes available"}
            ...
            logger.info(
                f"Message {envelope.message_id} routed successfully to {len(selected_routes)} channels"
            )
            ...
            routing_counter.labels(
                route="error", priority=envelope.priority.value, status="failed"
            ).inc()
    ...
    async def _validate_message_culturally(
        ...
        try:
            ...
            if envelope.cultural_context.language_preference == "ar":
                arabic_result = await self.cultural_validators["arabic_rtl"](
                    content_text
                )
        ...
        for rule in self.routing_rules:
            ...
            if self._evaluate_rule_conditions(rule, envelope):
                ...
                for channel in rule.target_channels:
                    ...
                    channel_routes = [
                        route_id
                        for route_id, config in self.route_configs.items()
                        if config.channel == channel
                        and self._is_route_available(route_id)
                    ]
                ...
                if rule.cultural_requirements:
                    envelope.routing_metadata["cultural_requirements"] = (
                        rule.cultural_requirements
                    )

        return list(matching_routes)

    def _evaluate_rule_conditions(
        self, rule: RoutingRule, envelope: MessageEnvelope
    ) -> bool:
    ...
    def _extract_condition_value(
        self, envelope: MessageEnvelope, condition_path: str
    ) -> Any:
    ...
    async def _select_optimal_routes(
        self, candidate_routes: List[str], envelope: MessageEnvelope
    ) -> List[str]:
        ...
        strategy = getattr(
            envelope.routing_metadata.get("strategy"), "value", self.default_strategy
        )
    ...
    def _select_performance_based_routes(
        self, candidate_routes: List[str]
    ) -> List[str]:
        ...
        for route_id in candidate_routes:
            ...
            score = (
                ...
                + config.weight * 0.2
                + (1.0 - health.active_connections / max(config.max_connections, 1))
                * 0.1
            )
    ...
    def _select_least_connections_routes(
        self, candidate_routes: List[str]
    ) -> List[str]:
    ...
    async def _execute_routing(
        self, envelope: MessageEnvelope, selected_routes: List[str]
    ) -> Dict[str, Any]:
        """Execute message routing to selected channels"""
        routing_tasks = []

        for route_id in selected_routes:
            task = asyncio.create_task(self._deliver_to_route(envelope, route_id))
        ...
        for route_id, task in routing_tasks:
            try:
                result = await asyncio.wait_for(
                    task, timeout=self.route_configs[route_id].timeout_seconds
                )
                ...
                delivery_results[route_id] = {"status": "error", "error": str(e)}
                failed_deliveries += 1
    ...
    async def _deliver_to_route(
        self, envelope: MessageEnvelope, route_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self.route_health[route_id].active_connections = self.active_connections[
                route_id
            ]
            ...
            if (
                config.channel == NotificationChannel.WEBSOCKET
                and self.websocket_manager
            ):
                result = await circuit_breaker.async_call(
                    self._deliver_websocket_message, envelope, notification_request
                )
            else:
                result = await circuit_breaker.async_call(
                    self._deliver_notification_service, notification_request
                )
            ...
            routing_counter.labels(
                route=route_id, priority=envelope.priority.value, status="success"
            ).inc()
            ...
            routing_counter.labels(
                route=route_id, priority=envelope.priority.value, status="failed"
            ).inc()

            logger.error(f"Failed to deliver to route {route_id}: {str(e)}")

            return {"status": "failed", "route_id": route_id, "error": str(e)}

        finally:
            # Decrease connection count
            self.active_connections[route_id] -= 1
            if route_id in self.route_health:
                self.route_health[route_id].active_connections = max(
                    0, self.active_connections[route_id]
                )

    def _create_notification_request(
        self, envelope: MessageEnvelope, config: RouteConfig
    ) -> NotificationRequest:
        ...
        return NotificationRequest(
            ...
            notification_type=notification_type_mapping.get(
                envelope.message_type, NotificationType.SYSTEM_ALERT
            ),
            priority=priority_mapping.get(
                envelope.priority, NotificationPriority.NORMAL
            ),
    ...
    async def _deliver_websocket_message(
        self, envelope: MessageEnvelope, notification_request: NotificationRequest
    ) -> Dict[str, Any]:
        ...
        success = await self.websocket_manager.send_message_to_user(
            envelope.user_id, websocket_data
        )
    ...
    def _update_route_performance(
        self, route_id: str, delivery_time: float, success: bool
    ) -> None:
        ...
        recent_deliveries = [
            p for p in performance_history if p["timestamp"] > time.time() - 300
        ]  # Last 5 minutes
        if recent_deliveries:
            health.avg_response_time = sum(
                p["delivery_time"] for p in recent_deliveries
    ...
    async def _store_routing_record(
        self, envelope: MessageEnvelope, routing_results: Dict[str, Any]
    ) -> None:
                ...
                for route_id, health in self.route_health.items():
                    ...
                    active_routes.set(
                        len(
                            [
                                h
                                for h in self.route_health.values()
                                if h.status == RouteStatus.ACTIVE
                            ]
                        )
                    )
            ...
            try:
                ...
                for priority in [
                    ...
                    while queue:
                        ...
                        if (
                            envelope.expires_at
                            and datetime.now(IRAQI_TIMEZONE) > envelope.expires_at
                        ):
                            logger.warning(
                                f"Message {envelope.message_id} expired, skipping"
                            )
                            continue
    ...
    async def queue_message(self, envelope: MessageEnvelope) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Queued message {envelope.message_id} with priority {envelope.priority}"
            )
            return True
        ...
        return {
            ...
            * 100,
            "cultural_compliance_rate": (
                (
                    self.routing_stats["total_routed"]
                    - self.routing_stats["cultural_violations"]
                )
                / max(1, self.routing_stats["total_routed"])
            )
            * 100,
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if self.websocket_manager:
            ...
            health_status["components"]["websocket"] = websocket_health.get(
                "status", "unknown"
            )
        ...
        if self.notification_service:
            ...
            health_status["components"]["notification_service"] = (
                notification_health.get("status", "unknown")
            )
...
if __name__ == "__main__":
    ...
    async def example_usage():
        ...
        arabic_message = MessageEnvelope(
            ...
            cultural_context=IraqiCulturalContext(
                language_preference="ar", islamic_compliance_required=True
            ),
        )
        ...
        payment_message = MessageEnvelope(
            ...
            cultural_context=IraqiCulturalContext(
                language_preference="en", islamic_compliance_required=True
            ),
        )
