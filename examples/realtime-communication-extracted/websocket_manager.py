...
messages_sent_total = Counter(
    "messages_sent_total", "Total messages sent", ["channel", "type", "status"]
)
message_delivery_duration = Histogram(
    "message_delivery_duration_seconds", "Message delivery duration"
)
notification_delivery_rate = Gauge(
    "notification_delivery_rate", "Notification delivery success rate"
)
...
class IraqiWebSocketManager:
    ...
    def __init__(self, config: CommunicationConfig):
        ...
        self.arabic_processor = (
            ArabicMessageProcessor() if config.arabic_support else None
        )
        self.cultural_validator = (
            CulturalMessageValidator() if config.cultural_validation else None
        )
    ...
    async def connect_user(
        ...
        try:
            ...
            self.logger.info(
                f"User {user_id} connected with connection {connection_id}"
            )
    ...
    async def disconnect_user(
        self, connection_id: str, reason: str = "normal_disconnect"
    ):
        """
        ...
        try:
            ...
            await self._log_connection_event(
                user_id, connection_id, "disconnected", {"reason": reason}
            )
            ...
            self.logger.error(
                f"Failed to disconnect connection {connection_id}: {str(e)}"
            )

    async def send_message(
        self, message: Message, target_connections: List[str] = None
    ) -> Dict[str, MessageStatus]:
        """
        ...
        try:
            ...
            if self.config.cultural_validation and self.cultural_validator:
                validation_result = await self.cultural_validator.validate_message(
                    message
                )
                if not validation_result["valid"]:
                    ...
                    return {
                        conn: MessageStatus.FAILED
                        for conn in (target_connections or [])
                    }

            # Process Arabic content if needed
            if message.arabic_content and self.arabic_processor:
                message.content = await self.arabic_processor.process_text(
                    message.content
                )
            ...
            if not target_connections:
                ...
                target_connections = list(
                    self.channel_subscribers.get(message.channel_id, set())
                )
    ...
    async def subscribe_to_channel(self, connection_id: str, channel_id: str) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Connection {connection_id} subscribed to channel {channel_id}"
            )
            return True

        except Exception as e:
            self.logger.error(
                f"Failed to subscribe connection {connection_id} to channel {channel_id}: {str(e)}"
            )
            return False
    ...
    async def unsubscribe_from_channel(
        ...
        try:
            ...
            self.logger.info(
                f"Connection {connection_id} unsubscribed from channel {channel_id}"
            )
            return True

        except Exception as e:
            self.logger.error(
                f"Failed to unsubscribe connection {connection_id} from channel {channel_id}: {str(e)}"
            )
            return False
    ...
    async def _handle_connection(self, connection_id: str):
        ...
        try:
            ...
            while True:
                try:
                    ...
                    message_data = await asyncio.wait_for(
                        websocket.receive_text(), timeout=self.config.connection_timeout
                    )
                    ...
                    self.logger.error(
                        f"Connection handler error for {connection_id}: {str(e)}"
                    )
            ...
            self.logger.error(
                f"Connection handler failed for {connection_id}: {str(e)}"
            )
    ...
    async def _process_incoming_message(self, connection_id: str, message_data: str):
        ...
        try:
            ...
            self.logger.error(
                f"Failed to process incoming message from {connection_id}: {str(e)}"
            )
    ...
    async def _send_message_to_connection(
        ...
        try:
            ...
            message_data = {
                ...
                else self._decrypt_content(message.content),
...
class CulturalMessageValidator:
    ...
    async def validate_message(self, message: Message) -> Dict[str, Any]:
        ...
        try:
            result = {"valid": True, "score": 1.0, "violations": [], "warnings": []}
            ...
            if message.message_type in [
                MessageType.ISLAMIC_GREETING,
                MessageType.CULTURAL_REMINDER,
            ]:
