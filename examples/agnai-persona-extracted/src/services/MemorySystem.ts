...
export class MemorySystem {
  ...
  async getProfessionalKnowledge(
    ...
    const strongAreas = knowledge.filter(k => k.content.confidence >= 8).map(k => k.content.topic);
    const weakAreas = knowledge.filter(k => k.content.confidence <= 5).map(k => k.content.topic);
  ...
  async storeArabicTerminology(
    ...
    return this.storeMemory({
      ...
      culturalContext: {
        ...
        formalityLevel:
          terminology.usage === 'formal'
            ? 'formal'
            : terminology.usage === 'professional'
              ? 'semi_formal'
              : 'casual',
      },
  ...
  async getArabicTerminology(
    ...
    if (context?.dialect) {
      terminology = terminology.filter(t => t.culturalContext?.dialectUsed === context.dialect);
    }

    if (context?.domain) {
      terminology = terminology.filter(t => t.professionalContext?.domain === context.domain);
    }

    if (context?.usage) {
      terminology = terminology.filter(t => t.content.usage === context.usage);
    }
    ...
    const frequencies = terminology.map(t => ({
      term: t.content.arabicTerm,
      freq: t.content.frequency,
    }));
  ...
  private async calculateStats(personaId?: PersonaID): Promise<MemoryStats> {
    const endpoint = personaId ? `/api/memory/${personaId}/stats` : '/api/memory/stats';
  ...
  private generateCulturalRecommendations(adaptations: MemoryEntry[]): string[] {
    ...
    if (lowEffectiveness.length > 0) {
      recommendations.push(
        'Consider adjusting cultural adaptation strategies for better user satisfaction'
      );
    }
    ...
    if (highEffectiveness.length > 0) {
      ...
      recommendations.push(
        `Continue using successful cultural elements: ${successfulElements.join(', ')}`
      );
    }
...
export const getMemorySystem = (baseUrl?: string, apiKey?: string): MemorySystem => {
  ...
  if (!memorySystemInstance) {
    throw new Error(
      'MemorySystem not initialized. Call getMemorySystem with baseUrl and apiKey first.'
    );
  }
