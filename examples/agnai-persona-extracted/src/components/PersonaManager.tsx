...
const PersonaManager: React.FC<PersonaManagerProps> = ({
  ...
  const professionalDomains = useMemo(
    () => [
      {
        value: 'legal' as IraqiProfessionalDomain,
        label: 'Legal Professional',
        labelAr: 'متخصص قانوني',
        icon: '⚖️',
      },
      {
        value: 'medical' as IraqiProfessionalDomain,
        label: 'Medical Professional',
        labelAr: 'متخصص طبي',
        icon: '🏥',
      },
      {
        value: 'educational' as IraqiProfessionalDomain,
        label: 'Educational Professional',
        labelAr: 'متخصص تعليمي',
        icon: '📚',
      },
      {
        value: 'engineering' as IraqiProfessionalDomain,
        label: 'Engineering Professional',
        labelAr: 'مهندس متخصص',
        icon: '🔧',
      },
      {
        value: 'business' as IraqiProfessionalDomain,
        label: 'Business Professional',
        labelAr: 'متخصص أعمال',
        icon: '💼',
      },
      {
        value: 'government' as IraqiProfessionalDomain,
        label: 'Government Official',
        labelAr: 'موظف حكومي',
        icon: '🏛️',
      },
      {
        value: 'religious' as IraqiProfessionalDomain,
        label: 'Religious Scholar',
        labelAr: 'عالم دين',
        icon: '🕌',
      },
      {
        value: 'cultural' as IraqiProfessionalDomain,
        label: 'Cultural Expert',
        labelAr: 'خبير ثقافي',
        icon: '🎭',
      },
      {
        value: 'technology' as IraqiProfessionalDomain,
        label: 'Technology Professional',
        labelAr: 'متخصص تقني',
        icon: '💻',
      },
      {
        value: 'general' as IraqiProfessionalDomain,
  ...
  const filteredPersonas = useMemo(() => {
    ...
    if (filters.governorate && filters.governorate.length > 0) {
      filtered = filtered.filter(persona => filters.governorate!.includes(persona.governorate));
    }

    // Cultural compliance filter
    if (filters.culturalCompliance) {
      filtered = filtered.filter(
        persona => persona.culturalProfile.culturalSensitivity >= filters.culturalCompliance!
      );
    }
    ...
    if (filters.islamicCompliance) {
      filtered = filtered.filter(
        persona => persona.islamicCompliance.islamicValuesCompliance >= filters.islamicCompliance!
      );
    }
    ...
    if (filters.tags && filters.tags.length > 0) {
      filtered = filtered.filter(persona => filters.tags!.some(tag => persona.tags.includes(tag)));
    }
    ...
    filtered.sort((a, b) => {
      ...
      switch (filters.sortBy) {
        ...
        case 'compliance':
          aValue =
            (a.culturalProfile.culturalSensitivity + a.islamicCompliance.islamicValuesCompliance) /
            2;
          bValue =
            (b.culturalProfile.culturalSensitivity + b.islamicCompliance.islamicValuesCompliance) /
            2;
          break;
        default:
          aValue = a.name;
  ...
  const selectAllFiltered = useCallback(() => {
    ...
    setSelectedPersonas(selectedPersonas.length === allIds.length ? [] : allIds);
  ...
  const renderMetrics = () => {
    ...
    return (
      <div className="bg-white rounded-lg shadow mb-6">
        <div
          className={`flex items-center justify-between p-6 border-b ${isRTL ? 'flex-row-reverse' : ''}`}
        >
        ...
        <div className="p-6">
          ...
          <div className="mt-6">
            <h4
              className={`text-sm font-medium text-gray-700 mb-3 ${isRTL ? 'text-right font-arabic' : ''}`}
            >
      ...
      >
        ...
        >
          <div className={`flex items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <div>
              <h3
                className={`font-semibold text-gray-900 ${isRTL ? 'text-right font-arabic' : ''}`}
              >
        ...
        <div className="p-4">
          <p
            className={`text-gray-600 text-sm mb-4 line-clamp-2 ${isRTL ? 'text-right font-arabic' : ''}`}
          >
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="text-center">
              <div
                className={`text-lg font-bold ${getComplianceColor(culturalScore, 0).split(' ')[0]}`}
              >
            ...
            <div className="text-center">
              <div
                className={`text-lg font-bold ${getComplianceColor(0, islamicScore).split(' ')[0]}`}
              >
        ...
        {/* Actions */}
        <div
          className={`flex items-center justify-between p-4 bg-gray-50 border-t ${isRTL ? 'flex-row-reverse' : ''}`}
        >
    ...
    <div className="space-y-6">
      ...
      <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
        <div className={`flex items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <div>
            <h1
              className={`text-2xl font-bold text-gray-900 ${isRTL ? 'text-right font-arabic' : ''}`}
            >
      ...
      <div className="bg-white rounded-lg shadow">
        <div className="p-6">
          <div className={`flex items-center space-x-4 mb-4 ${isRTL ? 'space-x-reverse' : ''}`}>
            ...
            {/* View Mode */}
            <div
              className={`flex items-center border border-gray-300 rounded-lg ${isRTL ? 'flex-row-reverse' : ''}`}
            >
            ...
            >
              ...
              <option value="compliance-desc">
                {language === 'ar' ? 'الامتثال (عالي)' : 'Compliance (High)'}
              </option>
              <option value="compliance-asc">
                {language === 'ar' ? 'الامتثال (منخفض)' : 'Compliance (Low)'}
              </option>
            </select>
          </div>

          {/* Active Filters Display */}
          {(filters.professionalDomain?.length ||
          ...
          {selectedPersonas.length > 0 && (
            <div
              className={`flex items-center justify-between p-3 bg-blue-50 border border-blue-200 rounded-lg mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}
            >
              <div className={`flex items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
