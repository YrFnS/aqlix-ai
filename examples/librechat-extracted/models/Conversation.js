...
conversationSchema.index({
  "professionalSession.domain": 1,
  "professionalSession.isActive": 1,
});
...
conversationSchema.methods.addMessage = function (messageData) {
  ...
  if (messageData.culturalValidation?.validationScore) {
    ...
    this.messageStats.averageCulturalScore =
      (currentAvg * (totalMessages - 1) +
        messageData.culturalValidation.validationScore) /
      totalMessages;
...
conversationSchema.methods.startProfessionalSession = function (
  domain,
  specialist,
) {
...
conversationSchema.statics.findByProfessionalDomain = function (
  domain,
  userId,
) {
  return this.find({
