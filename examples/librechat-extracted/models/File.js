...
const documentCulturalContext = new Schema({
  ...
  culturalTemplate: {
    type: String,
    enum: ['islamic', 'professional', 'government', 'educational', 'legal', 'medical'],
...
const professionalContext = new Schema({
  domain: {
    type: String,
    enum: ['legal', 'medical', 'educational', 'government', 'finance', 'business', 'general'],
    default: 'general'
  },
  specialist: String,
...
fileSchema.methods.generateArabicDocument = async function(content, options = {}) {
  const { 
  ...
  const arabicWordCount = content.split(/\s+/).filter(word => 
    /[\u0600-\u06FF]/.test(word)
  ).length;
...
fileSchema.methods.validateProfessionalCompliance = function(domain) {
  const requiredFieldsByDomain = {
    ...
    medical: ['patient_id', 'doctor_name', 'medical_license', 'date', 'diagnosis'],
    government: ['reference_number', 'department', 'official_stamp', 'date'],
    educational: ['institution_name', 'academic_year', 'student_id', 'signature']
  };
  
  const required = requiredFieldsByDomain[domain] || [];
  const missing = required.filter(field => !this.governmentCompliance.requiredFields.includes(field));
...
fileSchema.statics.findGeneratedDocuments = function(userId, generationType = 'ai_generated') {
  return this.find({
    user: userId,
