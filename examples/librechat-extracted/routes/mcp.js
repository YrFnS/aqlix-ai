...
} = require("../services/iraqiAgentOrchestrator");
...
router.get("/servers", requireJwtAuth, async (req, res) => {
  try {
    ...
    const enhancedServers = servers.map((server) => ({
      ...server,
      iraqiEnhancements: {
        culturalValidation: server.capabilities?.includes(
          "cultural_validation",
        ),
...
router.post("/servers", requireJwtAuth, async (req, res) => {
  try {
    ...
    logger.info(
      `MCP server registered with Iraqi enhancements: ${server.name} for user: ${user.id}`,
    );
...
router.put("/servers/:serverId", requireJwtAuth, async (req, res) => {
  try {
    ...
    const updatedServer = await mcpManager.updateServer(
      serverId,
      enhancedUpdates,
    );
    ...
    if (
      updates.iraqiEnhancements?.culturalValidation !==
      existingServer.iraqiEnhancements?.culturalValidation
    ) {
    ...
    logger.info(
      `MCP server updated with Iraqi enhancements: ${serverId} by user: ${user.id}`,
    );
...
router.delete("/servers/:serverId", requireJwtAuth, async (req, res) => {
  try {
    ...
    logger.info(
      `MCP server removed with Iraqi cleanup: ${serverId} by user: ${user.id}`,
    );
  ...
  async (req, res) => {
    try {
      ...
      if (connectionConfig.agentPreferences.enableCulturalValidation) {
        await agentOrchestrator.activateAgentsForConnection(
          connection.id,
          culturalContext,
          professionalContext,
        );
      ...
      logger.info(
        `MCP connection established with Iraqi context: ${serverId} for user: ${user.id}`,
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      logger.info(
        `MCP disconnection completed with Iraqi cleanup: ${serverId} for user: ${user.id}`,
      );
  ...
  async (req, res) => {
    try {
      ...
      const enhancedCapabilities = {
        ...
        iraqiEnhancements: {
          culturalValidation:
            await mcpManager.getCulturalCapabilities(serverId),
          arabicProcessing: await mcpManager.getArabicCapabilities(serverId),
          professionalServices:
            await mcpManager.getProfessionalCapabilities(serverId),
          agentIntegration:
            await agentOrchestrator.getAgentCapabilities(serverId),
        },
      };
...
router.get("/agents/status", requireJwtAuth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      activeAgents: agentStatus.filter((agent) => agent.status === "active")
        .length,
    });
  } catch (error) {
