...
export class IraqiDocumentFormatsService {
   ...
   */
  private identifyDocumentFormat(
    text: string,
    filename: string,
  ): IraqiDocumentFormat {
    // Legal document detection
    if (
      this.matchesPatterns(text, IRAQI_LEGAL_PATTERNS.courtDocuments.patterns)
    ) {
      return {
        type: "legal",
    ...
    if (
      this.matchesPatterns(text, IRAQI_MEDICAL_PATTERNS.prescriptions.patterns)
    ) {
      return {
        type: "medical",
    ...
    if (
      this.matchesPatterns(text, IRAQI_MEDICAL_PATTERNS.medicalReports.patterns)
    ) {
      return {
        type: "medical",
    ...
    if (
      this.matchesPatterns(text, IRAQI_EDUCATIONAL_PATTERNS.diplomas.patterns)
    ) {
      return {
        type: "educational",
        subtype: "diploma",
    ...
    if (
      this.matchesPatterns(
        text,
        IRAQI_GOVERNMENT_PATTERNS.officialLetters.patterns,
      )
    ) {
      return {
        type: "government",
  ...
  private parseDocumentHeader(
    ...
    const issuingAuthority =
      this.extractMatchingPattern(headerText, authorityPatterns) || "غير محدد";
  ...
  private checkCulturalCompliance(text: string): boolean {
    ...
    return (
      !inappropriateTerms.some((term) => text.includes(term)) &&
  ...
  private isSectionHeader(
    ...
    return (
      (line.length < 100 && line.trim().endsWith(":")) ||
      /^\d+\.\s/.test(line) ||
  ...
  private extractMatchingPattern(
    text: string,
    patterns: string[],
  ): string | null {
    for (const pattern of patterns) {
      if (text.includes(pattern)) {
        ...
        return text
          .substring(index, endIndex > -1 ? endIndex : index + 50)
          .trim();
      }
    }
    return null;
