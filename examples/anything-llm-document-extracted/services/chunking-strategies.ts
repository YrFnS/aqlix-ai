...
export class IraqiDocumentChunker {
  ...
  private islamicTerms = [
    ...
    'Allah', 'Muhammad', 'Islam', 'Quran', 'Sunnah', 'Hadith', 'Prayer', 'Zakat',
    'Hajj', 'Fasting', 'Ramadan', 'Mosque', 'Imam', 'Khutbah'
  ];
  ...
  async chunkDocument(
    ...
    try {
      ...
      const strategy = this.selectChunkingStrategy(content, options, arabicAnalysis);
      
      let chunks: DocumentChunk[];
      
      switch (strategy) {
        case 'semantic':
          chunks = await this.semanticChunking(content, documentId, options, arabicAnalysis);
          break;
        case 'arabic-structure':
          chunks = this.arabicStructureChunking(content, documentId, options, arabicAnalysis);
          break;
        case 'mixed-content':
          chunks = this.mixedContentChunking(content, documentId, options, arabicAnalysis);
          break;
        case 'professional-domain':
          chunks = this.professionalDomainChunking(content, documentId, options, arabicAnalysis);
          break;
        default:
          chunks = this.standardChunking(content, documentId, options, arabicAnalysis);
      }
  ...
  private analyzeArabicContent(content: string): {
    ...
    const arabicMatches = content.match(new RegExp(this.arabicTextRegex, 'g')) || [];
    ...
    for (const [dialect, markers] of Object.entries(this.iraqiDialectMarkers)) {
      const matches = markers.filter(marker => content.includes(marker)).length;
    ...
    const arabicTextRegex = new RegExp(`[${this.arabicTextRegex.source}][^${this.arabicTextRegex.source}]*[${this.arabicTextRegex.source}]|[${this.arabicTextRegex.source}]+`, 'g');
    let match;
  ...
  private selectChunkingStrategy(
    ...
    options: ChunkingOptions,
    arabicAnalysis: any
  ): 'semantic' | 'arabic-structure' | 'mixed-content' | 'professional-domain' | 'standard' {
  ...
  private async semanticChunking(
    ...
    const sentences = this.splitIntoSentences(content, arabicAnalysis.hasArabicText);
    let currentChunk = '';
    ...
    for (let i = 0; i < sentences.length; i++) {
      ...
      if (potentialChunk.length > options.chunkSize && currentChunk) {
        ...
        if (options.embeddings && i < sentences.length - 1) {
          const currentEmbedding = await options.embeddings.generate(currentChunk);
          const nextSentenceEmbedding = await options.embeddings.generate(sentence);
          const similarity = this.calculateCosineSimilarity(currentEmbedding, nextSentenceEmbedding);
          ...
          if (similarity < options.embeddings.threshold) {
            ...
            currentChunkStart = content.indexOf(sentence, currentChunkStart + currentChunk.length);
            continue;
          }
        }
  ...
  private arabicStructureChunking(
    ...
    for (const segment of segments) {
      const potentialChunk = currentChunk + (currentChunk ? ' ' : '') + segment.trim();
  ...
  private mixedContentChunking(
    ...
    for (const segment of segments) {
      ...
      if (potentialChunk.length > options.chunkSize && currentChunk) {
        ...
        if (this.isGoodBreakPoint(currentChunk, segment.text, segment.language)) {
          const chunk = this.createChunkSync(
            currentChunk.trim(),
            documentId,
            chunkStart,
  ...
  private professionalDomainChunking(
    ...
    switch (domain) {
      case 'legal':
        breakPoints = [
          'المادة', 'الفقرة', 'البند', 'Article', 'Section', 'Clause',
          'القانون', 'التشريع', 'Law', 'Regulation'
        ];
        break;
      case 'medical':
        breakPoints = [
          'التشخيص', 'العلاج', 'الدواء', 'Diagnosis', 'Treatment', 'Medicine',
          'المريض', 'Patient', 'الأعراض', 'Symptoms'
        ];
        break;
      case 'educational':
        breakPoints = [
          'الدرس', 'الوحدة', 'الفصل', 'Lesson', 'Unit', 'Chapter',
          'التمرين', 'Exercise', 'الواجب', 'Assignment'
        ];
        break;
      case 'business':
        breakPoints = [
          'الاتفاقية', 'العقد', 'البند', 'Agreement', 'Contract', 'Term',
          'الميزانية', 'Budget', 'التقرير', 'Report'
        ];
        break;
      case 'engineering':
        breakPoints = [
          'المواصفات', 'التصميم', 'المتطلبات', 'Specifications', 'Design', 'Requirements',
          'الحسابات', 'Calculations', 'المعايير', 'Standards'
        ];
        break;
      default:
        breakPoints = ['.', '؟', '!', '\n\n'];
    ...
    return this.chunkByBreakPoints(content, documentId, breakPoints, options, arabicAnalysis);
  }
    ...
    return {
      ...
      metadata: {
        ...
        quality: {
          ...
          culturalIntegrityScore: this.calculateCulturalIntegrityScore(content, chunkArabicAnalysis),
        },
  ...
  private splitByMultipleDelimiters(content: string, delimiters: string[]): string[] {
    let result = [content];
  ...
  private identifyLanguageSegments(content: string): Array<{
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      if (currentLanguage !== wordLanguage && currentSegment) {
        ...
        segmentStart = content.indexOf(word, segmentStart + currentSegment.length);
      } else {
  ...
  private isGoodBreakPoint(currentChunk: string, nextSegment: string, language: string): boolean {
    if (language === 'arabic') {
  ...
  private async calculateCoherenceScore(content: string): Promise<number> {
    ...
    const transitionWords = [
      ...
      'لكن', 'لذلك', 'بالإضافة', 'علاوة على ذلك', 'نتيجة لذلك'
    ];
  ...
  private calculateCompletenessScore(content: string, options: ChunkingOptions): number {
  ...
  private calculateCulturalIntegrityScore(content: string, arabicAnalysis: any): number {
  ...
  private hasContextualDependency(content: string): boolean {
    const dependencyMarkers = [
      'as mentioned above', 'as stated previously', 'referring to',
      'كما ذكر أعلاه', 'كما ذكر سابقاً', 'بالإشارة إلى',
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (chunk.metadata.quality.coherenceScore < 0.5) {
        ...
        if (i < chunks.length - 1) {
          ...
          const combinedContent = chunk.content + ' ' + nextChunk.content.substring(0, 50);
  ...
  private establishChunkRelationships(chunks: DocumentChunk[]): DocumentChunk[] {
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (i < chunks.length - 1) {
        ...
        const commonWords = this.getCommonWords(chunk.content, chunks[i + 1].content);
        chunk.metadata.relationships.semanticSimilarity = commonWords.length / 10;
      }
    }
