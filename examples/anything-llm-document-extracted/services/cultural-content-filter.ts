...
export class IraqiCulturalContentFilter {
  ...
  async filterContent(
    ...
    try {
      ...
      const culturalAnalysis = await this.analyzeCulturalContent(
        content,
        context,
      );
      ...
      const modifications = await this.applyContentModifications(
        content,
        culturalAnalysis,
        context,
      );
      ...
      const professionalContext = context?.professionalDomain
        ? await this.analyzeProfessionalContext(
            content,
  ...
  private async analyzeCulturalContent(
    ...
    const culturalSensitivityAnalysis =
      this.analyzeCulturalSensitivity(content);
   ...
   */
  private analyzeIslamicCompliance(
    content: string,
  ): CulturalAnalysis["islamicCompliance"] {
    let score = 100;
      ...
      if (lowerContent.includes(term.toLowerCase())) {
        const severity =
          this.config.islamicCompliance.strictness === "strict"
            ? 20
            : this.config.islamicCompliance.strictness === "moderate"
              ? 10
              : 5;
        ...
        if (this.config.islamicCompliance.strictness === "strict") {
          suggestions.push(
            `Consider removing or providing Islamic context for: ${term}`,
          );
        } else {
          suggestions.push(
            `Consider adding Islamic perspective or context for: ${term}`,
          );
        }
      }
    });
    ...
    this.islamicTerms.contextDependent.forEach((term) => {
      if (lowerContent.includes(term.toLowerCase())) {
        ...
        if (
          !hasProperContext &&
          this.config.islamicCompliance.strictness !== "lenient"
        ) {
          score -= 5;
    ...
    if (this.config.islamicCompliance.preserveEducationalContent) {
      ...
      if (isEducational) {
        ...
        suggestions.push(
          "Educational content detected - maintaining with Islamic perspective",
        );
      }
    }
   ...
   */
  private analyzePoliticalNeutrality(
    content: string,
  ): CulturalAnalysis["politicalNeutrality"] {
    let score = 100;
    ...
    if (this.config.politicalNeutrality.blockSectarian) {
      this.politicalTerms.sectarian.forEach((term) => {
        if (lowerContent.includes(term.toLowerCase())) {
          ...
          neutralAlternatives.push(
            `Iraqi citizen/community instead of ${term}`,
          );
          riskLevel = "medium";
        }
      });
    ...
    if (this.config.politicalNeutrality.blockPartisan) {
      this.politicalTerms.partisan.forEach((term) => {
        if (
          ...
          !this.isNeutralGovernmentContext(content, term)
        ) {
          score -= 12;
          sensitivePhrases.push(term);
    ...
    this.politicalTerms.sensitive.forEach((term) => {
      if (lowerContent.includes(term.toLowerCase())) {
        ...
        sensitivePhrases.push(term);
        neutralAlternatives.push(
          `Historical context or neutral reference for ${term}`,
        );
        riskLevel = "high";
      }
    });
  ...
  private analyzeCulturalSensitivity(
    content: string,
  ): CulturalAnalysis["culturalSensitivity"] {
    let score = 100;
    ...
    if (this.config.culturalSensitivity.respectFamilyValues) {
      const familyReferences = this.iraqiCulturalTerms.familyValues.filter(
    ...
    if (this.config.culturalSensitivity.filterInappropriate) {
      this.inappropriateContent.forEach((term) => {
        if (lowerContent.includes(term.toLowerCase())) {
          ...
          culturalEnhancements.push(
            `Remove or provide appropriate context for: ${term}`,
          );
        }
      });
    }
  ...
  private determineContentAllowance(analysis: CulturalAnalysis): boolean {
    const minScore =
      this.config.contentModeration.filterLevel === "comprehensive"
        ? 80
        : this.config.contentModeration.filterLevel === "enhanced"
          ? 70
          : 60;

    // Must pass all critical checks
    const passesIslamic =
      !this.config.islamicCompliance.enabled ||
      analysis.islamicCompliance.score >=
        (this.config.islamicCompliance.strictness === "strict" ? 85 : 70);

    const passesPolitical =
      !this.config.politicalNeutrality.enabled ||
      analysis.politicalNeutrality.riskLevel !== "high";

    const passesCultural =
      !this.config.culturalSensitivity.enabled ||
      analysis.culturalSensitivity.inappropriateContent.length === 0;

    return (
      analysis.overallScore >= minScore &&
      passesIslamic &&
      passesPolitical &&
      passesCultural
    );
  ...
  private async applyContentModifications(
        ...
        if (term) {
          if (this.config.islamicCompliance.strictness === "strict") {
            ...
            modifiedContent = modifiedContent.replace(
              new RegExp(term, "gi"),
              "[content filtered for Islamic compliance]",
            );
          } else {
            const islamicContext = this.getIslamicContext(term);
            modifications.addedContext.push(
              `Islamic perspective: ${islamicContext}`,
            );
          }
        }
      }
    ...
    if (this.config.politicalNeutrality.enabled) {
      analysis.politicalNeutrality.sensitivePhrases.forEach((phrase, index) => {
        const alternative =
          analysis.politicalNeutrality.neutralAlternatives[index];
        if (alternative) {
  ...
  private async analyzeProfessionalContext(
    ...
    const appropriateness = this.calculateProfessionalAppropriateness(
      content,
      domain,
    );
  ...
  private calculateConfidence(analysis: CulturalAnalysis): number {
    ...
    if (analysis.islamicCompliance.preservedElements.length > 0)
      confidence += 0.1;
    if (analysis.politicalNeutrality.sensitivePhrases.length > 0)
      confidence += 0.1;
    if (analysis.culturalSensitivity.culturalReferences.length > 0)
      confidence += 0.1;
  ...
  private hasIslamicContext(content: string, term: string): boolean {
    ...
    return contextWords.some((context) =>
  ...
  private isNeutralGovernmentContext(content: string, term: string): boolean {
    ...
    return neutralContexts.some((context) =>
      content.toLowerCase().includes(context),
    );
  }
  ...
  private getIslamicContext(term: string): string {
    const contexts: Record<string, string> = {
      alcohol:
        "Islam prohibits alcohol consumption for health and spiritual reasons",
      gambling:
        "Islam discourages gambling as it can lead to addiction and social harm",
      خمر: "الإسلام يحرم الخمر للمحافظة على الصحة والروحانية",
      قمار: "الإسلام ينهى عن القمار لما قد يؤدي إليه من أضرار اجتماعية",
    };

    return (
      contexts[term.toLowerCase()] ||
      "Please consider Islamic guidelines regarding this topic"
    );
  }

  private extractProfessionalTerminology(
    content: string,
    domain: string,
  ): string[] {
  ...
  private calculateProfessionalAppropriateness(
    content: string,
    domain: string,
  ): number {
    const terminologyCount = this.extractProfessionalTerminology(
      content,
      domain,
    ).length;
