...
import { IDesignRequest, ICulturalDesignRequirements, ICulturalValidationResult } from './IraqiDesignEngine';
...
export class CulturalPromptSystem extends EventEmitter {
  ...
  async generatePrompt(request: IDesignRequest): Promise<ICulturalPrompt> {
    ...
    if (this.options.cachePrompts && this.promptCache.has(cacheKey)) {
      ...
      this.emit('promptGenerated', { promptId, cached: true, prompt: cachedPrompt });
      return cachedPrompt;
    }
    ...
    try {
      ...
      const ministrySpecificRequirements = this.generateMinistryRequirements(request);
      
      // Add accessibility instructions
      const accessibilityInstructions = this.generateAccessibilityInstructions(request);
      
      // Add performance considerations
      const performanceConsiderations = this.generatePerformanceConsiderations(request);
  ...
  getCulturalRules(ministry?: string): ICulturalRule[] {
    ...
    return allRules.filter(rule => !rule.ministry || rule.ministry === ministry);
  }
...
- Responsive and mobile-friendly`;
  ...
  private generateMinistryRequirements(request: IDesignRequest): string[] {
    const ministryGuidelines = this.ministryBrandingGuidelines.get(request.targetMinistry) || [];
  ...
  private generatePerformanceConsiderations(request: IDesignRequest): string[] {
    ...
    if (request.functionalRequirements.performanceTarget === 'high-performance') {
      considerations.push(
  ...
  private generateValidationCriteria(request: IDesignRequest): IValidationCriteria {
    return {
      islamicCompliance: {
        modestImagery: request.culturalRequirements.islamicCompliance,
        appropriateColors: true,
        respectfulLanguage: true,
        culturalSensitivity: request.culturalRequirements.culturalSensitivity === 'high',
      ...
      culturalAppropriateness: {
        ...
        professionalFormality: request.culturalRequirements.professionalContext === 'government',
      ...
      professionalStandards: {
        ministryBrandingCompliance: request.culturalRequirements.ministryBranding,
        accessibilityStandards: request.functionalRequirements.accessibilityLevel !== 'basic',
        securityRequirements: true,
        usabilityStandards: true,
        performanceRequirements: request.functionalRequirements.performanceTarget !== 'standard'
      }
    };
  }
  ...
  private async validateIslamicCompliance(
    ...
    const prohibitedPatterns = [
      /alcohol/i, /wine/i, /beer/i,
      /gambling/i, /casino/i, /lottery/i,
      /dating/i, /romance/i
    ];
    
    for (const pattern of prohibitedPatterns) {
      if (pattern.test(code)) {
        issues.push(`Potentially inappropriate content detected: ${pattern.source}`);
        score -= 20;
    ...
    if (code.toLowerCase().includes('sexy') || code.toLowerCase().includes('hot')) {
  ...
  private async validateArabicRTLAccuracy(
    ...
    requirements: ICulturalDesignRequirements,
    appliedRules: string[]
  ): Promise<{ score: number; layoutIssues: string[]; typographyIssues: string[]; recommendations: string[] }> {
    ...
    if (requirements.bilingualSupport) {
      recommendations.push('Implement bidirectional text support for mixed content');
      appliedRules.push('Bilingual Content Support');
    }
    
    return { score: Math.max(0, score), layoutIssues, typographyIssues, recommendations };
  }
  ...
  private async validateCulturalAppropriateness(
    ...
    if (requirements.professionalContext === 'government') {
      if (code.toLowerCase().includes('fun') || code.toLowerCase().includes('cool')) {
    ...
    if (code.includes('green') || code.includes('blue') || code.includes('gold')) {
  ...
  private async validateProfessionalStandards(
    ...
    if (!code.includes('<main>') && !code.includes('<section>') && !code.includes('<header>')) {
      score -= 10;
    }
