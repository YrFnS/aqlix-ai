...
class TestIraqiAgent:
    ...
    async def test_agent_creation(self):
        ...
        agent = await create_iraqi_agent(
            name="test-agent", system_prompt="Test system prompt"
        )
    ...
    async def test_cultural_validation_pass(self, basic_agent):
        ...
        cultural_context = IraqiCulturalContext(
            islamic_compliance_required=True, cultural_sensitivity_level="high"
        )
        ...
        assert (
            result.cultural_validation.cultural_appropriateness
            >= settings.min_cultural_appropriateness
        )
        assert (
            result.cultural_validation.islamic_compliance
            >= settings.min_islamic_compliance
        )
    ...
    async def test_arabic_text_detection(self, basic_agent):
        ...
        input_msg = IraqiAgentInput(
            message="شلونك؟ شكو ماكو اليوم؟ وين رايحين؟", require_validation=True
        )
    ...
    async def test_mixed_language_processing(self, basic_agent):
        ...
        assert result.metadata.get("cultural_context", {}).get("language") in [
            "mixed",
            "arabic",
            "english",
        ]
    ...
    async def test_agent_performance_tracking(self, basic_agent):
        ...
        for i in range(3):
            input_msg = IraqiAgentInput(
                message=f"Test message {i + 1}: مرحبا", require_validation=True
            )
    ...
    async def test_error_handling(self, basic_agent):
        ...
        # Test empty message
        try:
            input_msg = IraqiAgentInput(message="", require_validation=True)
            result = await basic_agent.process_message(input_msg)
...
class TestIraqiTools:
    ...
    async def test_cultural_content_validation(self, tool_context):
        ...
        positive_content = (
            "السلام عليكم، أهلاً وسهلاً. نحن نحترم التقاليد العراقية والقيم الإسلامية."
        )
    ...
    async def test_arabic_text_processing(self, tool_context):
        ...
        found_phrases = [
            indicator["phrase"] for indicator in result.data["dialect_indicators"]
        ]
    ...
    async def test_payment_validation(self, tool_context):
        ...
        result = await validate_payment_request(
            amount=5000.0, currency="IQD", gateway="zaincash", context=tool_context
        )
...
class TestCulturalValidator:
    ...
    async def test_cultural_appropriateness_validation(self, validator):
        ...
        context = IraqiCulturalContext(
            cultural_sensitivity_level="high", islamic_compliance_required=True
        )
    ...
    async def test_islamic_compliance_validation(self, validator):
        ...
        # Compliant content
        compliant_content = (
            "نحن نقدر قيم العدالة والسلام والمعرفة والخير في ديننا الحنيف."
        )
...
class TestIntegrationScenarios:
    ...
    async def test_complete_iraqi_conversation_flow(self):
        ...
        for turn, message in enumerate(conversation_turns, 1):
            input_msg = IraqiAgentInput(
                message=message, cultural_context=context, require_validation=True
            )
    ...
    async def test_professional_domain_workflow(self):
        ...
        for domain, question in domains:
            context = IraqiCulturalContext(
                professional_domain=domain, islamic_compliance_required=True
            )

            input_msg = IraqiAgentInput(
                message=question, cultural_context=context, require_validation=True
            )
