...
if BaseModel != object:
    ...
    class IraqiCulturalContext(IraqiBaseModel):
        ...
        user_cultural_background: str = Field(
            default="iraqi", description="User's cultural background"
        )
        primary_language: str = Field(
            default="arabic", description="Primary language preference"
        )
        dialect: str = Field(
            default="iraqi_arabic", description="Arabic dialect preference"
        )
        islamic_compliance_required: bool = Field(
            default=True, description="Require Islamic compliance validation"
        )
        ...
        cultural_sensitivity_level: Literal["low", "medium", "high"] = Field(
            default="high", description="Cultural sensitivity requirement level"
        )
        prayer_time_awareness: bool = Field(
            default=True, description="Consider Islamic prayer times"
        )
        regional_context: str = Field(
            default="baghdad", description="Iraqi regional context"
        )
    ...
    class CulturalValidationResult(IraqiBaseModel):
        ...
        islamic_compliance: float = Field(
            ge=0.0, le=1.0, description="Islamic compliance score (0.0-1.0)"
        )
        arabic_accuracy: float = Field(
            ge=0.0, le=1.0, description="Arabic text processing accuracy (0.0-1.0)"
        )
        dialect_recognition: float = Field(
            ge=0.0, le=1.0, description="Iraqi dialect recognition accuracy (0.0-1.0)"
        )
        professional_relevance: float = Field(
            ge=0.0, le=1.0, description="Professional domain relevance (0.0-1.0)"
        )

        validation_passed: bool = Field(description="Overall validation success")
        confidence: float = Field(
            ge=0.0, le=1.0, description="Confidence in validation results"
        )
        ...
        processing_time: float = Field(
            ge=0.0, description="Validation processing time in seconds"
        )
        ...
        def validate_overall_pass(cls, values):
            ...
            passed = (
                cultural_score >= cultural_threshold
                and islamic_score >= islamic_threshold
                and (arabic_score >= arabic_threshold if arabic_score > 0 else True)
            )
    ...
    class IraqiAgentInput(IraqiBaseModel):
        ...
        cultural_context: Optional[IraqiCulturalContext] = Field(
            None, description="Iraqi cultural context"
        )
        require_validation: bool = Field(
            default=True, description="Require cultural and Islamic validation"
        )
        ...
        request_id: UUID4 = Field(
            default_factory=uuid.uuid4, description="Unique request identifier"
        )
    ...
    class IraqiAgentOutput(IraqiBaseModel):
        ...
        # Response metadata
        request_id: UUID4 = Field(..., description="Original request identifier")
        ...
        contains_arabic: bool = Field(
            default=False, description="Response contains Arabic text"
        )
        text_direction: Literal["ltr", "rtl", "mixed"] = Field(
            default="ltr", description="Text direction for display"
        )
    ...
    class PaymentRequest(IraqiBaseModel):
        ...
        currency: str = Field(default="IQD", description="Payment currency")
        gateway: PaymentGateway = Field(..., description="Iraqi payment gateway")
        description: str = Field(..., max_length=500, description="Payment description")
        user_id: str = Field(..., description="User making payment")
        cultural_context: Optional[IraqiCulturalContext] = Field(
            None, description="Cultural context for validation"
        )
    ...
    class PaymentValidationResult(IraqiBaseModel):
        ...
        valid: bool = Field(..., description="Payment validation passed")
        islamic_compliant: bool = Field(..., description="Payment is Islamic compliant")
        estimated_fee: Decimal = Field(ge=0, description="Estimated processing fee")
        processing_time_estimate: str = Field(
            ..., description="Estimated processing time"
        )
        warnings: List[str] = Field(
            default_factory=list, description="Payment warnings"
        )
        errors: List[str] = Field(
            default_factory=list, description="Payment validation errors"
        )
        gateway_info: Dict[str, Any] = Field(
            default_factory=dict, description="Gateway-specific information"
        )
    ...
    class ProfessionalQuery(IraqiBaseModel):
        ...
        cultural_context: Optional[IraqiCulturalContext] = Field(
            None, description="Cultural context for guidance"
        )
        urgency: Literal["low", "medium", "high", "critical"] = Field(
            default="medium", description="Query urgency level"
        )
        user_role: Optional[str] = Field(None, description="User's professional role")
        ...
        def validate_question_content(cls, v):
            ...
            if any(term in v_lower for term in inappropriate_terms):
                raise ValueError(
                    "Question contains inappropriate content for professional guidance"
                )
    ...
    class ProfessionalGuidanceResponse(IraqiBaseModel):
        ...
        domain: ProfessionalDomain = Field(..., description="Professional domain")
        guidance: str = Field(..., description="Professional guidance content")
        guidance_arabic: Optional[str] = Field(None, description="Guidance in Arabic")
        disclaimer: str = Field(..., description="Professional disclaimer")
        cultural_notes: str = Field(..., description="Iraqi cultural considerations")
        professional_relevance: float = Field(
            ge=0.0, le=1.0, description="Professional relevance score"
        )
        recommended_next_steps: List[str] = Field(
            default_factory=list, description="Recommended next steps"
        )
        resources: List[Dict[str, str]] = Field(
            default_factory=list, description="Professional resources and references"
        )
        confidence: float = Field(
            ge=0.0, le=1.0, description="Guidance confidence level"
        )
    ...
    class AgentPerformanceMetrics(IraqiBaseModel):
        ...
        avg_processing_time: float = Field(
            ge=0.0, description="Average processing time in seconds"
        )
        cultural_validation_count: int = Field(
            ge=0, description="Number of cultural validations performed"
        )
        validation_success_rate: float = Field(
            ge=0.0, le=1.0, description="Cultural validation success rate"
        )
        islamic_compliance_rate: float = Field(
            ge=0.0, le=1.0, description="Islamic compliance success rate"
        )
        arabic_processing_accuracy: float = Field(
            ge=0.0, le=1.0, description="Arabic text processing accuracy"
        )
        uptime_hours: float = Field(ge=0.0, description="Agent uptime in hours")
