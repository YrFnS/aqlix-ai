...
class IraqiAgentSettings(BaseSettings):
    ...
    fallback_model: str = Field(
        "openai:gpt-3.5-turbo", description="Fallback LLM model"
    )
    local_model: Optional[str] = Field(
        None, description="Local Iraqi model if available"
    )
    ...
    cultural_mode: IraqiCulturalMode = Field(
        IraqiCulturalMode.STRICT, description="Cultural validation strictness level"
    )
    islamic_compliance_level: IslamicComplianceLevel = Field(
        IslamicComplianceLevel.FULL, description="Islamic compliance validation level"
    )
    arabic_processing_mode: ArabicProcessingMode = Field(
        ArabicProcessingMode.MIXED, description="Arabic text processing configuration"
    )
    ...
    min_islamic_compliance: float = Field(
        1.0, ge=0.0, le=1.0, description="Minimum Islamic compliance score (0.0-1.0)"
    )
    ...
    arabic_processing_timeout: int = Field(
        300, description="Arabic processing timeout (ms)"
    )
    ...
    enable_input_validation: bool = Field(
        True, description="Enable comprehensive input validation"
    )
    enable_output_filtering: bool = Field(
        True, description="Enable cultural output filtering"
    )
    security_scan_level: str = Field("high", description="Security scanning level")

    # Monitoring and Logging
    enable_cultural_metrics: bool = Field(
        True, description="Enable cultural compliance metrics"
    )
