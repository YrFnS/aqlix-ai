...
class PaymentGatewayService(Protocol):
    ...
    async def validate_payment(
        self, amount: float, currency: str, gateway: str
    ) -> Dict[str, Any]:
...
class ProfessionalDomainService(Protocol):
    ...
    async def get_domain_guidance(
        self, domain: str, question: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
...
class DefaultCulturalService:
    ...
    async def validate_cultural_appropriateness(
        ...
        positive_score = sum(
            1
            for term in self.cultural_patterns["respectful_terms"]
            if term in content_lower
        )
        positive_score += sum(
            1
            for term in self.cultural_patterns["islamic_values"]
            if term in content_lower
        )
        ...
        negative_score = sum(
            1
            for term in self.cultural_patterns["inappropriate_content"]
            if term in content_lower
        )
        ...
        score = min(
            1.0, max(0.0, base_score + (positive_score * 0.05) - (negative_score * 0.2))
        )
    ...
    async def validate_islamic_compliance(self, content: str) -> float:
        ...
        compliance = min(
            ...
            max(
                0.0,
                base_compliance + (islamic_positive * 0.02) - (prohibited_score * 0.3),
            ),
        )
    ...
    async def get_cultural_recommendations(self, content: str) -> List[str]:
        ...
        if not any(
            ...
            recommendations.append(
                "Consider using respectful Arabic terms like 'أستاذ' or 'حضرة'"
            )

        if not any(
            term in content.lower() for term in self.cultural_patterns["islamic_values"]
        ):
            recommendations.append(
                "Include Islamic values like peace, knowledge, or community"
            )
...
class DefaultArabicProcessingService:
    ...
    async def recognize_iraqi_dialect(self, text: str) -> Dict[str, Any]:
        ...
        found_indicators = [
            indicator for indicator in iraqi_indicators if indicator in text
        ]
...
class DefaultPaymentGatewayService:
    ...
    async def get_supported_gateways(self) -> List[str]:
        ...
        return [
            gateway
            for gateway, config in settings.payment_gateways.items()
...
class IraqiAgentDependencies:
    ...
    # Core Services
    cultural_service: IraqiCulturalService = field(
        default_factory=DefaultCulturalService
    )
    arabic_service: ArabicProcessingService = field(
        default_factory=DefaultArabicProcessingService
    )
    payment_service: PaymentGatewayService = field(
        default_factory=DefaultPaymentGatewayService
    )
    ...
    cultural_mode: IraqiCulturalMode = field(
        default_factory=lambda: settings.cultural_mode
    )
    islamic_compliance_level: IslamicComplianceLevel = field(
        default_factory=lambda: settings.islamic_compliance_level
    )
    enabled_domains: List[str] = field(
        default_factory=lambda: settings.enabled_domains.copy()
    )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        failure_rate = (
            self.islamic_compliance_failures / self.cultural_validation_count
            if self.cultural_validation_count > 0
            else 0.0
        )
