...
class ModelPerformanceMetrics:
    ...
    def get_overall_score(self, config: IraqiModelConfig) -> float:
        ...
        if config.performance_priority == "speed":
            ...
            speed_score = min(
                1.0, 5.0 / max(0.1, self.avg_response_time)
            )  # 5s = 1.0 score
            return (
                0.4 * speed_score
...
class IraqiModelProvider:
    ...
    def _initialize_providers(self):
        ...
        if settings.anthropic_api_key and AnthropicModel:
            try:
                ...
                self.providers["anthropic:claude-3-haiku"] = AnthropicModel(
    ...
    def _setup_fallback_chain(self):
        ...
        self.fallback_chain = [
            model for model in priority_order if model in available_models
        ]

        if not self.fallback_chain:
            raise RuntimeError(
                "No model providers available. Check API keys and configuration."
            )
