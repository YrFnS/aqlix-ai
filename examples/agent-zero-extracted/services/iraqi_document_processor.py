...
from ..utils.iraqi_helpers import detect_iraqi_dialect, extract_arabic_keywords, clean_arabic_text
...
class IraqiDocumentProcessor:
    ...
    async def process_document(
        ...
        try:
            ...
            # Cultural validation
            callback("التحقق من الملاءمة الثقافية...")  # Checking cultural appropriateness
            ...
            log.info(f"Document processed successfully: {len(content)} chars, {processing_time:.2f}s")
    ...
    async def _process_pdf(self, file_path: str, callback: Callable[[str], None]) -> str:
        ...
        try:
            ...
            for page_num, page in enumerate(doc):
                callback(f"معالجة الصفحة {page_num + 1} من {len(doc)}")  # Processing page X of Y
    ...
    async def _ocr_page(self, page, page_num: int) -> str:
        ...
        try:
            ...
            text = pytesseract.image_to_string(
                img, 
                lang=self.arabic_ocr_config['lang'],
    ...
    async def _pdf_to_ocr_fallback(self, file_path: str, callback: Callable[[str], None]) -> str:
        """Fallback OCR processing for entire PDF"""
        callback("تحويل PDF إلى صور للمعالجة...")  # Converting PDF to images for processing
        ...
        try:
            ...
            for i, page_img in enumerate(pages):
                callback(f"معالجة الصورة {i + 1} من {len(pages)}")  # Processing image X of Y
    ...
    async def _process_image(self, file_path: str, callback: Callable[[str], None]) -> str:
    ...
    async def _process_text_file(self, file_path: str, callback: Callable[[str], None]) -> str:
        ...
        raise ValueError(f"Could not decode text file with any Arabic-compatible encoding")
    
    async def _process_word_document(self, file_path: str, callback: Callable[[str], None]) -> str:
    ...
    async def _process_unstructured(self, file_path: str, callback: Callable[[str], None]) -> str:
        ...
        try:
            ...
            log.warning(f"Unstructured processing not fully implemented for: {file_path}")
            return ""
    ...
    async def _validate_cultural_content(
        ...
        cultural_context = create_cultural_context(
            ...
            is_professional=document_type in [
        ...
        validation_result = self.cultural_validator.validate_text_content(content, cultural_context)
        ...
        return CulturalValidationMeta(
            ...
            language_appropriateness="appropriate" if arabic_meta.arabic_percentage > 0.3 else "mixed"
        )
    ...
    async def _extract_iraqi_entities(
        ...
        # Iraqi locations
        iraqi_locations = [
            "بغداد", "البصرة", "أربيل", "النجف", "كربلاء", "الموصل", "كركوك",
            "الأنبار", "بابل", "ديالى", "ذي قار", "ميسان", "المثنى", "القادسية",
            "صلاح الدين", "واسط", "السليمانية", "دهوك"
        ]
        
        # Iraqi institutions
        iraqi_institutions = [
            "جامعة بغداد", "جامعة البصرة", "وزارة التربية", "وزارة الصحة",
            "البنك المركزي العراقي", "مجلس النواب", "مجلس الوزراء"
        ]
    ...
    async def chunk_document(
        ...
        for i, chunk_content in enumerate(chunks):
            # Analyze each chunk
            chunk_arabic_chars = len([c for c in chunk_content if '\u0600' <= c <= '\u06FF'])
            chunk_arabic_percentage = chunk_arabic_chars / len(chunk_content) if chunk_content else 0
        ...
        try:
            ...
            if result.success:
                ...
                if result.arabic_processing:
                    Files.update_arabic_processing_meta(file_id, result.arabic_processing)
                
                if result.cultural_validation:
                    Files.update_cultural_validation(file_id, result.cultural_validation)
