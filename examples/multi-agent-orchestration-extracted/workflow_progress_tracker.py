...
class IraqiWorkflowProgressTracker:
    ...
    async def _start_websocket_server(self) -> None:
        ...
        try:
            async def handle_client(websocket, path):
                ...
                self._performance_metrics['websocket_connection_count'] = len(self._websocket_clients)
    ...
    async def create_workflow_tracker(
        ...
        try:
            if len(self._active_workflows) >= self.max_concurrent_workflows:
                raise ValueError(f"Maximum concurrent workflows ({self.max_concurrent_workflows}) reached")
            ...
            for i, phase_config in enumerate(phases):
                phase_progress = WorkflowPhaseProgress(
                    ...
                    estimated_duration_ms=phase_config.get('estimated_duration_ms', 30000)
                )
            ...
            self.logger.info(f"Created workflow tracker for: {workflow_name} ({workflow_id})")
        ...
        try:
            ...
            if status == WorkflowStatus.COMPLETED:
                ...
                workflow_state.total_processing_time_ms = int(
                    (workflow_state.actual_completion_time - workflow_state.start_time).total_seconds() * 1000
                )
                ...
                default_message = f"Workflow '{workflow_state.workflow_name}' completed successfully"
                ...
                default_message = f"Workflow '{workflow_state.workflow_name}' is now running"
            ...
            await self._send_progress_notification(
                workflow_id=workflow_id,
                update_type=ProgressUpdateType.WORKFLOW_COMPLETE if status == WorkflowStatus.COMPLETED else ProgressUpdateType.PHASE_PROGRESS,
                title=title,
    ...
    async def update_phase_progress(
        ...
        try:
            ...
            if cultural_validation_required and progress_percentage > 50.0:
                ...
                if (validation_result.cultural_score < self.cultural_validation_threshold or
                    validation_result.islamic_compliance_score < self.islamic_compliance_threshold):
            ...
            if progress_percentage != old_progress or status != old_status:
                update_type = ProgressUpdateType.PHASE_COMPLETE if status == PhaseStatus.COMPLETED else ProgressUpdateType.PHASE_PROGRESS
                
                await self._send_progress_notification(
                    workflow_id=workflow_id,
                    update_type=update_type,
                    title=f"Phase Progress: {phase.phase_name}",
                    message=message or f"Phase '{phase.phase_name}' is {progress_percentage:.1f}% complete",
                    phase_name=phase.phase_name,
                    progress_percentage=progress_percentage,
                    priority=NotificationPriority.HIGH if status == PhaseStatus.COMPLETED else NotificationPriority.NORMAL
                )
            
            self.logger.info(f"Updated phase {phase_id} progress: {old_progress:.1f}% -> {progress_percentage:.1f}%")
        ...
        try:
            ...
            if not agent:
                ...
                agent = AgentProgress(
                    ...
                    cultural_validation=cultural_validation_status or CulturalValidationStatus.NOT_VALIDATED,
                    start_time=datetime.now(timezone.utc)
                )
            ...
            self.logger.debug(f"Updated agent {agent_id} progress: {old_progress:.1f}% -> {progress_percentage:.1f}%")
    ...
    async def report_error(
        ...
        try:
            ...
            if any(keyword in error_message.lower() for keyword in 
                   ['cultural', 'islamic', 'arabic', 'compliance', 'validation']):
            ...
            await self._send_progress_notification(
                ...
                islamic_compliance_note="Review required for Islamic compliance" if cultural_impact else None,
                requires_user_attention=True,
                metadata={
    ...
    async def request_user_feedback(
        ...
        try:
            ...
            feedback_notification = ProgressNotification(
                ...
                estimated_completion_time=datetime.now(timezone.utc) + timedelta(minutes=timeout_minutes),
                metadata={
                    'options': options or [],
            ...
            self.logger.info(f"User feedback requested for workflow {workflow_id}: {feedback_request}")
    ...
    async def _perform_cultural_validation(
        ...
        try:
            ...
            if cache_key in self._cultural_validation_cache:
                ...
                if (datetime.now(timezone.utc) - cached_result.validation_timestamp).total_seconds() < 300:
            ...
            for validator_name, validator_func in self._cultural_validators.items():
                try:
                    ...
                    validation_result.cultural_score = min(validation_result.cultural_score, score)
                except Exception as e:
                    self.logger.warning(f"Cultural validator '{validator_name}' failed: {e}")
                    validation_result.issues_found.append(f"Validator {validator_name} failed: {str(e)}")
            
            # Run Islamic compliance validators
            for validator_name, validator_func in self._islamic_compliance_validators.items():
                try:
                    score = await validator_func(workflow_id, phase_id, phase_name)
                    validation_result.islamic_compliance_score = min(validation_result.islamic_compliance_score, score)
                except Exception as e:
                    self.logger.warning(f"Islamic compliance validator '{validator_name}' failed: {e}")
                    validation_result.issues_found.append(f"Islamic validator {validator_name} failed: {str(e)}")
            ...
            if validation_result.islamic_compliance_score < self.islamic_compliance_threshold:
                validation_result.requires_human_review = True
                validation_result.recommendations.append(
    ...
    async def _update_overall_progress(self, workflow_id: str) -> None:
        ...
        try:
            ...
            for phase in workflow_state.phases.values():
                ...
                weight = max(1, phase.estimated_duration_ms / 1000)  # Convert to seconds
                total_weight += weight
                weighted_progress += (phase.progress_percentage * weight)
            
            if total_weight > 0:
                workflow_state.overall_progress_percentage = weighted_progress / total_weight
            else:
                # Fallback to simple average
                workflow_state.overall_progress_percentage = sum(
                    phase.progress_percentage for phase in workflow_state.phases.values()
                ) / len(workflow_state.phases)
            
            # Update cultural scores
            cultural_scores = [phase.cultural_validation.cultural_score 
                             for phase in workflow_state.phases.values() 
                             if phase.cultural_validation]
            if cultural_scores:
                workflow_state.overall_cultural_score = sum(cultural_scores) / len(cultural_scores)
            
            islamic_scores = [phase.cultural_validation.islamic_compliance_score 
                            for phase in workflow_state.phases.values() 
                            if phase.cultural_validation]
            if islamic_scores:
                workflow_state.overall_islamic_compliance = sum(islamic_scores) / len(islamic_scores)
            
            # Update estimated completion time
            if workflow_state.overall_progress_percentage > 0:
                elapsed_time = datetime.now(timezone.utc) - workflow_state.start_time
                estimated_total_time = elapsed_time / (workflow_state.overall_progress_percentage / 100)
                workflow_state.estimated_completion_time = workflow_state.start_time + estimated_total_time
    ...
    async def _send_notification_to_clients(self, notification: ProgressNotification) -> None:
    ...
    async def _monitor_workflow_progress(self, workflow_id: str) -> None:
        ...
        try:
            while workflow_id in self._active_workflows:
                ...
                time_since_update = (now - workflow_state.last_update_time).total_seconds()
                ...
                if workflow_state.estimated_completion_time:
                    if now > workflow_state.estimated_completion_time + timedelta(minutes=30):
                        await self.update_workflow_status(workflow_id, WorkflowStatus.TIMEOUT)
                        break
            ...
            try:
                ...
                if active_count > 0:
                    ...
                    if completed_workflows:
                        avg_time = sum(w.total_processing_time_ms for w in completed_workflows) / len(completed_workflows)
                        self._performance_metrics['average_completion_time_ms'] = avg_time
                    ...
                    if cultural_scores:
                        self._performance_metrics['average_cultural_score'] = sum(cultural_scores) / len(cultural_scores)
                    if islamic_scores:
                        self._performance_metrics['average_islamic_compliance'] = sum(islamic_scores) / len(islamic_scores)
    ...
    def register_cultural_validator(
        self,
        validator_name: str,
        validator_func: Callable[[str, str, str], float]
    ) -> None:
    ...
    def get_workflow_state(self, workflow_id: str) -> Optional[WorkflowProgressState]:
        ...
        return self._active_workflows.get(workflow_id) or self._workflow_history.get(workflow_id)
    ...
    async def cleanup_completed_workflows(self, max_age_hours: int = 24) -> int:
        ...
        try:
            ...
            for workflow_id, workflow_state in self._workflow_history.items():
                if (workflow_state.actual_completion_time and 
                    workflow_state.actual_completion_time < cutoff_time):
...
async def main():
    ...
    async def sample_cultural_validator(workflow_id: str, phase_id: str, phase_name: str) -> float:
    ...
    async def sample_islamic_validator(workflow_id: str, phase_id: str, phase_name: str) -> float:
        """Sample Islamic compliance validation function"""
        # Simulate Islamic compliance validation
        if any(term in phase_name.lower() for term in ['islamic', 'compliance', 'halal']):
            return 98.0  # High Islamic compliance
        return 90.0  # Standard compliance
    
    # Register validators
    progress_tracker.register_cultural_validator("sample_cultural", sample_cultural_validator)
    progress_tracker.register_islamic_compliance_validator("sample_islamic", sample_islamic_validator)
    ...
    error_workflow = await progress_tracker.create_workflow_tracker(
        ...
        phases=[{'phase_id': 'test', 'name': 'Test Phase', 'description': 'Testing errors'}],
        expected_agents=['test_agent']
    )
    
    await progress_tracker.update_workflow_status(error_workflow.workflow_id, WorkflowStatus.RUNNING)
    ...
    await progress_tracker.report_error(
        ...
        recovery_suggestions=["Review content for cultural appropriateness", "Consult cultural validation team"]
    )
