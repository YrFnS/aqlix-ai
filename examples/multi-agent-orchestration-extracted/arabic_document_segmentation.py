...
class DocumentSegment(BaseModel):
    ...
    # Cultural analysis
    cultural_importance: int = Field(
        0, ge=0, le=100, description="Cultural importance score"
    )
    islamic_compliance_score: int = Field(
        100, ge=0, le=100, description="Islamic compliance score"
    )
    professional_terms: Dict[str, str] = Field(
        default_factory=dict, description="Professional terminology found"
    )
    cultural_elements: List[CulturalElement] = Field(
        default_factory=list, description="Cultural elements in segment"
    )

    # Section metadata
    is_header: bool = Field(False, description="Whether this segment is a header")
    hierarchy_level: int = Field(
        0, description="Header hierarchy level (0=not header, 1=h1, etc.)"
    )
    ...
    # Processing metadata
    processing_complexity: float = Field(
        0.0, description="Processing complexity score 0.0-1.0"
    )
...
class ArabicDocumentSegmentationAgent:
    ...
    async def should_use_document_segmentation(
        self, document_content: str, complexity_analysis: bool = True
        ...
        try:
            ...
            if content_length > self.default_segmentation_threshold:
                return (
                    True,
                    f"Document exceeds size threshold ({content_length:,} > {self.default_segmentation_threshold:,} chars)",
                    1.0,
                )
            ...
            if not complexity_analysis:
                return (
                    False,
                    "Under size threshold, no complexity analysis requested",
                    0.1,
                )
            ...
            arabic_chars = sum(
                1 for char in document_content if self._is_arabic_char(char)
            )
            english_chars = sum(
                1 for char in document_content if char.isascii() and char.isalpha()
            )
            ...
            if cultural_matches > 10:
                ...
                reasons.append(
                    f"High cultural content density ({cultural_matches} matches)"
                )
            ...
            for pattern in self.islamic_patterns:
                islamic_matches += len(
                    re.findall(pattern, document_content, re.UNICODE)
                )

            if islamic_matches > 0:
                complexity_score += 0.1
                reasons.append(
                    f"Islamic content requires careful segmentation ({islamic_matches} matches)"
                )

            # Document structure complexity
            potential_headers = 0
            for pattern in self.arabic_header_patterns:
                potential_headers += len(
                    re.findall(pattern, document_content, re.MULTILINE | re.UNICODE)
                )

            if potential_headers > 5:
                complexity_score += 0.2
                reasons.append(
                    f"Complex document structure ({potential_headers} potential headers)"
                )
            ...
            if len(non_empty_lines) > 100:
                complexity_score += 0.1
                reasons.append(
                    f"High line density ({len(non_empty_lines)} non-empty lines)"
                )
            ...
            if boundary_matches > 5:
                ...
                reasons.append(
                    f"Multiple language boundaries ({boundary_matches} boundaries)"
                )

            # Decision logic
            should_segment = complexity_score >= 0.4 or content_length > (
                self.default_segmentation_threshold * 0.5
            )

            if should_segment:
                reason = (
                    f"Complexity analysis recommends segmentation (score: {complexity_score:.2f}): "
                    + "; ".join(reasons)
                )
            else:
            ...
            return (
                len(document_content) > self.default_segmentation_threshold,
                f"Fallback: size check only",
                0.5,
            )
    ...
    async def segment_document(
        ...
        try:
            ...
            doc_analysis = await self._analyze_document_characteristics(
                document_content
            )
            ...
            stats = self._calculate_segmentation_statistics(
                analyzed_segments, doc_analysis
            )
        ...
        try:
            ...
            for category, patterns in self.iraqi_dialect_patterns.items():
                ...
                if category == "expressions":
                    dialect_score += (
                        category_matches * 3
                    )  # Religious expressions are strong indicators
                elif category == "greetings":
                    dialect_score += (
                        category_matches * 2
                    )  # Greetings are good indicators
                else:
            ...
            if total_chars > 0:
                analysis["cultural_density"] = cultural_matches / (
                    total_chars * 0.01
                )  # Normalize
    ...
    async def _determine_segmentation_strategy(
        ...
        if doc_analysis["document_type"] in [
            DocumentType.LEGAL,
            DocumentType.RELIGIOUS,
        ]:
            strategy["preserve_cultural_blocks"] = True
            strategy["max_segment_size"] = min(
                self.max_segment_size, 8000
            )  # Smaller segments for precision

        if doc_analysis["cultural_density"] > 5.0:  # High cultural density
            strategy["preserve_cultural_blocks"] = True
            strategy["min_segment_size"] = max(
                self.min_segment_size, 1000
            )  # Larger minimum for context
    ...
    async def _perform_segmentation(
        self, content: str, strategy: Dict[str, Any], target_size: int
        ...
        try:
            ...
            if not segments or strategy["approach"] == "linguistic":
                ...
                segments = await self._segment_by_language_boundaries(
                    content, target_size
                )
    ...
    async def _segment_by_structure(
        ...
        try:
            for line in lines:
                ...
                current_content = "\n".join(current_segment_lines)
                if (
                    len(current_content) > target_size
                    and len(current_segment_lines) > 1
                ):
                    ...
                    if len(current_segment_lines) > 1:
                        ...
                        current_segment_lines = [
                            line
                        ]  # Start new segment with current line
    ...
    async def _segment_by_language_boundaries(
        self, content: str, target_size: int
    ) -> List[DocumentSegment]:
        ...
        try:
            ...
            for boundary in boundaries:
                ...
                if (
                    len(current_segment_content) >= target_size
                    or boundary.language
                    != boundaries[max(0, boundaries.index(boundary) - 1)].language
                ):
                    if len(current_segment_content.strip()) > self.min_segment_size:
                        segment = DocumentSegment(
                            ...
                            end_position=current_segment_start
                            + len(current_segment_content),
                        ...
                        segments.append(segment)
                        segment_id += 1
                        current_segment_start = current_segment_start + len(
                            current_segment_content
                        )
                        current_segment_content = ""
            ...
            if (
                current_segment_content
                and len(current_segment_content.strip()) > self.min_segment_size
            ):
                segment = DocumentSegment(
                    segment_id=f"seg_{segment_id}",
                    content=current_segment_content,
    ...
    async def _segment_by_cultural_blocks(
        self, content: str, target_size: int
    ) -> List[DocumentSegment]:
            ...
            while remaining_content and len(remaining_content) > self.min_segment_size:
                ...
                if relevant_blocks:
                    ...
                    if block_end > current_position + segment_end:
                        segment_end = min(
                            len(remaining_content), block_end - current_position + 100
                        )  # Buffer
    ...
    async def _segment_by_semantic_meaning(
        self, content: str, target_size: int
    ) -> List[DocumentSegment]:
        ...
        try:
            while current_position < len(content):
                ...
                if segment_end < len(content):
                    ...
                    for i in range(
                        segment_end, max(segment_end - 200, current_position), -1
                    ):
                        if content[i] in [" ", "\n", "\t"]:
                            segment_end = i
                            break
        ...
        try:
            ...
            for pattern in self.mixed_boundary_patterns:
                for match in re.finditer(pattern, content, re.UNICODE):
                    ...
                    arabic_chars = sum(
                        1 for c in boundary_text if self._is_arabic_char(c)
                    )
                    english_chars = sum(
                        1 for c in boundary_text if c.isascii() and c.isalpha()
                    )
                    ...
                    confidence = min(
                        1.0,
                        max(arabic_chars, english_chars) / max(1, len(boundary_text)),
                    )
                    ...
                    boundary = LanguageBoundary(
                        ...
                        content_preview=boundary_text[:50]
                        + ("..." if len(boundary_text) > 50 else ""),
                    )
    ...
    async def _analyze_segment_cultural_content(
        ...
        try:
            ...
            for doc_type, indicators in self.document_type_indicators.items():
                score = sum(
                    len(
                        re.findall(
                            re.escape(indicator), content, re.IGNORECASE | re.UNICODE
                        )
                    )
                    for indicator in indicators
                )
            ...
            if type_scores:
                segment.document_type = max(
                    type_scores.keys(), key=lambda k: type_scores[k]
                )
            ...
            islamic_matches = sum(
                len(re.findall(pattern, content, re.UNICODE))
                for pattern in self.islamic_patterns
            )
    ...
    async def _post_process_segments(
        self, segments: List[DocumentSegment], strategy: Dict[str, Any]
    ) -> List[DocumentSegment]:
        ...
        try:
            ...
            for segment in segments:
                ...
                if len(segment.content) > self.max_segment_size:
                    # Split large segment
                    sub_segments = await self._split_large_segment(
                        segment, self.max_segment_size
                    )
    ...
    async def _split_large_segment(
        self, segment: DocumentSegment, max_size: int
    ) -> List[DocumentSegment]:
    ...
    def _calculate_segmentation_statistics(
        self, segments: List[DocumentSegment], doc_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Calculate statistics from segmentation results"""

        arabic_segments = sum(
            1 for s in segments if s.language == DocumentLanguage.ARABIC
        )
        english_segments = sum(
            1 for s in segments if s.language == DocumentLanguage.ENGLISH
        )
        mixed_segments = sum(
            1 for s in segments if s.language == DocumentLanguage.MIXED
        )
    ...
    def _update_performance_metrics(self, result: SegmentationResult) -> None:
        ...
        try:
            ...
            if result.overall_dialect_confidence != IraqiDialectConfidence.VERY_LOW:
                ...
                current_dialect_avg = self._performance_metrics[
                    "average_dialect_confidence"
                ]
                confidence_value = confidence_values[result.overall_dialect_confidence]
                new_dialect_avg = (
                    (current_dialect_avg * (count - 1)) + confidence_value
                ) / count
                self._performance_metrics["average_dialect_confidence"] = (
                    new_dialect_avg
                )

            # Update cultural element detection rate
            segments_with_cultural = sum(
                1 for s in result.segments if s.cultural_elements
            )
            detection_rate = segments_with_cultural / max(1, result.total_segments)

            current_detection = self._performance_metrics[
                "cultural_element_detection_rate"
            ]
...
"""
    ...
    (
        ...
        reason,
        complexity,
    ) = await segmentation_agent.should_use_document_segmentation(
    ...
    if should_segment:
        ...
        for i, segment in enumerate(result.segments):
            ...
            if segment.is_header:
                print(
                    f"Header (Level {segment.hierarchy_level}): {segment.section_title}"
                )
            if segment.professional_terms:
                print(
                    f"Professional terms: {list(segment.professional_terms.keys())[:3]}..."
                )
            ...
            print(
                f"Content preview: {preview}{'...' if len(segment.content) > 150 else ''}"
            )
