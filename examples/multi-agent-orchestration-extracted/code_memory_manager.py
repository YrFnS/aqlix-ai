...
class IraqiCulturalContext(BaseModel):
    ...
    professional_domain: Optional[str] = Field(
        None, description="Legal, Medical, Educational, etc."
    )
    ...
    islamic_compliance_score: int = Field(
        100, description="Islamic compliance score 0-100"
    )
    ministry_references: List[str] = Field(
        default_factory=list, description="Government/ministry references"
    )
    dialect_patterns: List[str] = Field(
        default_factory=list, description="Iraqi dialect patterns found"
    )
    professional_terminology: Dict[str, str] = Field(
        default_factory=dict, description="Professional term mappings"
    )
...
class ArabicProcessingResult(BaseModel):
    ...
    rtl_segments: List[str] = Field(
        default_factory=list, description="Right-to-left text segments"
    )
    mixed_language_boundaries: List[Tuple[int, int]] = Field(
        default_factory=list, description="Mixed language boundaries"
    )
    dialect_confidence: float = Field(0.0, description="Iraqi dialect confidence score")
    cultural_keywords: List[str] = Field(
        default_factory=list, description="Identified cultural keywords"
    )
...
class CodeSummary(BaseModel):
    ...
    cultural_retention_score: float = Field(
        description="Cultural context retention score 0-100"
    )
...
class MemoryOptimizationStrategy(str, Enum):
    ...
    PROFESSIONAL_FOCUS = (
        "professional_focus"  # Focus on professional domain preservation
    )
...
class IraqiCodeMemoryManager:
    ...
    def __init__(
        ...
        self.summary_trigger_tokens = (
            self.max_context_tokens - self.token_buffer - self.cultural_context_reserve
        )
    ...
    async def should_create_summary(
        self, file_path: str, content: str, current_context_tokens: int = 0
    ) -> bool:
        """
    ...
    async def get_optimized_content(
        self, file_path: str, content: str, force_summary: bool = False
    ) -> str:
        """
        ...
        try:
            ...
            importance_score = min(
                ...
                (
                    ...
                    + (
                        50 if cultural_context.professional_domain else 0
            ...
            for pattern in problematic_patterns:
                if re.search(pattern, content, re.IGNORECASE | re.UNICODE):
                    islamic_compliance = max(
                        70, islamic_compliance - 20
    ...
    async def _process_arabic_content(self, content: str) -> ArabicProcessingResult:
        ...
        try:
            ...
            if rtl_segments:
                ...
                dialect_score = min(
                    1.0, total_matches / max(1, len(rtl_segments)) * 0.3
                )
        ...
        try:
            ...
            for line in lines[:50]:  # Check first 50 lines for imports/declarations
                ...
                if (
                    ...
                    or stripped.startswith(('"""', "'''"))  # Docstrings
                    or any(
                        keyword in stripped
                        for keyword in arabic_processing.cultural_keywords
                    )
                ):
            ...
            for i, line in enumerate(lines):
                ...
                if any(
                    keyword in line for keyword in arabic_processing.cultural_keywords
                ):
                    importance_score += 25

                # Check for professional terminology
                if any(
                    term in line for term in arabic_processing.professional_terms.keys()
                ):
                    importance_score += 20
                ...
                if line.strip().startswith(("def ", "class ", "async def ")) and any(
                    keyword in line for keyword in arabic_processing.cultural_keywords
                ):
                    importance_score += 40
            ...
            for i, line, score in culturally_important_lines[:lines_to_preserve]:
                if line not in summary_lines:
                    summary_lines.append(
                        f"# Line {i + 1}: Cultural importance score {score}"
                    )
                    summary_lines.append(line)
            ...
            if strategy == MemoryOptimizationStrategy.CULTURAL_PRIORITY:
                ...
                if cultural_context.ministry_references:
                    summary_lines.append(
                        f"# Ministry References: {', '.join(cultural_context.ministry_references)}"
                    )

                if arabic_processing.cultural_keywords:
                    summary_lines.append(
                        f"# Cultural Keywords: {', '.join(arabic_processing.cultural_keywords[:10])}"
                    )
                ...
                if cultural_context.arabic_terminology_count > 0:
                    essential_markers.append(
                        f"Arabic:{cultural_context.arabic_terminology_count}"
                    )
                if cultural_context.professional_domain:
                    essential_markers.append(
                        f"Domain:{cultural_context.professional_domain}"
                    )
                if cultural_context.islamic_compliance_score < 100:
                    essential_markers.append(
                        f"Islamic:{cultural_context.islamic_compliance_score}"
                    )
                ...
                if cultural_context.professional_domain:
                    ...
                    domain_terms = {
                        k: v
                        for k, v in arabic_processing.professional_terms.items()
                        if v == cultural_context.professional_domain
                    }
                    if domain_terms:
                        summary_lines.append(
                            f"# Domain Terms: {', '.join(domain_terms.keys())}"
                        )

            # Add function/class signatures for code structure
            for line in lines:
                stripped = line.strip()
                if stripped.startswith(("class ", "def ", "async def ")) and not any(
                    existing.strip() == stripped for existing in summary_lines
                ):
                    # Preserve signatures but compress implementation
                    if ":" in line:
                        summary_lines.append(
                            line.split(":")[0] + ":  # Implementation compressed"
                        )
                    else:
    ...
    async def _calculate_cultural_retention_score(
        ...
        try:
            ...
            if arabic_processing.cultural_keywords:
                ...
                score += (
                    retained_keywords / len(arabic_processing.cultural_keywords)
                ) * 30
                total_possible += 30
            ...
            if arabic_processing.professional_terms:
                retained_terms = sum(
                    1
                    for term in arabic_processing.professional_terms.keys()
                    if term in summary_content
                )
                score += (
                    retained_terms / len(arabic_processing.professional_terms)
                ) * 25
                total_possible += 25

            # Check Islamic compliance elements
            islamic_patterns = self._cultural_patterns.get("islamic_terms", [])
            original_islamic = sum(
                1
                for pattern in islamic_patterns
            ...
            if cultural_context.ministry_references:
                retained_refs = sum(
                    1
                    for ref in cultural_context.ministry_references
                    if ref in summary_content
                )
                score += (
                    retained_refs / len(cultural_context.ministry_references)
                ) * 15
                total_possible += 15

            # Check dialect pattern retention
            if cultural_context.dialect_patterns:
                retained_dialect = sum(
                    1
                    for pattern in cultural_context.dialect_patterns
                    if pattern in summary_content
                )
                score += (
                    retained_dialect / len(cultural_context.dialect_patterns)
                ) * 10
                total_possible += 10
    ...
    def _update_performance_metrics(
        self, summary: CodeSummary, start_time: datetime
    ) -> None:
        ...
        try:
            ...
            self._performance_metrics["total_tokens_saved"] += (
                summary.token_reduction_percentage
            )
            ...
            new_avg = (
                (current_avg * (count - 1)) + summary.cultural_retention_score
            ) / count
    ...
    def get_summary_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_usage": {
                ...
                "arabic_terminology_entries": sum(
                    len(terms) for terms in self._arabic_terminology.values()
                ),
                "professional_glossary_entries": sum(
                    len(glossary) for glossary in self._professional_glossaries.values()
                ),
            },
            "cultural_analysis": {
                "avg_cultural_importance": sum(
                    s.cultural_context.cultural_importance
                    for s in self._code_summaries.values()
                )
                / max(1, len(self._code_summaries)),
                "avg_islamic_compliance": sum(
                    s.cultural_context.islamic_compliance_score
                    for s in self._code_summaries.values()
                )
                / max(1, len(self._code_summaries)),
                "files_with_arabic_content": sum(
                    1
                    for s in self._code_summaries.values()
                    if s.arabic_processing.rtl_segments
                ),
                "professional_domains": list(
                    set(
                        s.cultural_context.professional_domain
                        for s in self._code_summaries.values()
                        if s.cultural_context.professional_domain
                    )
                ),
            },
        }
    ...
    async def clear_cache(self, max_age_hours: int = 24) -> int:
        ...
        try:
            ...
            self.logger.info(
                f"Cleared {cleared_count} cache entries older than {max_age_hours} hours"
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Islamic compliance: {summary.cultural_context.islamic_compliance_score}/100"
    )
