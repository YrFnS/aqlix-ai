...
class IraqiResearchAgent:
            ...
            for database, config in self.search_engines.items():
                if config:  # If database is configured
                    db_results = await self._search_database(
                        database, enhanced_terms, date_range, languages, max_sources // len(self.search_engines)
                    )
    ...
    async def generate_literature_review(
        ...
        if not project.sources_found:
            return {"success": False, "error": "No sources found. Conduct literature search first."}
        ...
        try:
            ...
            if project.islamic_compliance_required:
                compliance_result = await self._validate_islamic_compliance(review_content)
    ...
    async def analyze_research_gaps(
        ...
        try:
            ...
            islamic_perspective_gaps = await self._analyze_islamic_perspective_gaps(project)
    ...
    async def generate_research_proposal(
        ...
        try:
            ...
            proposal_sections = {
                ...
                "literature_review": project.literature_review or await self._generate_brief_literature_review(project),
            ...
            if include_budget:
                proposal_sections["budget"] = await self._generate_budget_section(project)
            
            # Add Iraqi-specific sections
            if project.islamic_compliance_required:
                proposal_sections["islamic_considerations"] = await self._generate_islamic_considerations(project)
            
            if project.arabic_sources_required:
                proposal_sections["arabic_sources_justification"] = await self._generate_arabic_sources_justification(project)
            
            # Combine sections into full proposal
            full_proposal = await self._combine_proposal_sections(proposal_sections, proposal_type)
            
            # Validate proposal
            validation_result = await self._validate_research_proposal(full_proposal, project)
    ...
    async def _validate_research_project(self, project: ResearchProject) -> Dict[str, Any]:
    ...
    async def _enhance_search_terms(
        ...
        domain_terms = {
            ...
            AcademicDomain.ARABIC_LITERATURE: ["الأدب العربي", "الشعر العربي", "النثر العربي"],
    ...
    async def _deduplicate_sources(self, sources: List[ResearchSource]) -> List[ResearchSource]:
    ...
    async def _generate_methodological_review(
        ...
        for source in sources:
            if "survey" in source.title.lower() or "questionnaire" in source.title.lower():
                methodologies["كمي"].append(source)
            elif "interview" in source.title.lower() or "case study" in source.title.lower():
    ...
    async def _analyze_arabic_research_gaps(self, project: ResearchProject) -> List[str]:
    ...
    async def _analyze_islamic_perspective_gaps(self, project: ResearchProject) -> List[str]:
    ...
    async def _generate_proposal_abstract(self, project: ResearchProject) -> str:
        ...
        return f"ملخص البحث حول {project.title}. يهدف هذا البحث إلى {project.description}."
    ...
    async def _generate_arabic_sources_justification(self, project: ResearchProject) -> str:
    ...
    async def _combine_proposal_sections(
        ...
        section_order = [
            ...
            "methodology", "timeline", "expected_outcomes", "budget",
            "islamic_considerations", "arabic_sources_justification", "references"
        ]
        ...
        for section_name in section_order:
            if section_name in sections and sections[section_name]:
                if section_name == "title":
                    ...
                    proposal_parts.append(f"## {section_name.replace('_', ' ').title()}\n\n{sections[section_name]}\n")
