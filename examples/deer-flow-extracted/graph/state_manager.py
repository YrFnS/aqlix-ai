...
class IraqiStateManager:
    ...
    async def update_cultural_context(
        self, state_id: str, cultural_updates: Dict[str, Any]
    ) -> bool:
    ...
    async def search_states(
        self, criteria: Dict[str, Any], include_cultural_context: bool = True
    ) -> List[Dict[str, Any]]:
        ...
        for state_id, state in self.memory_store.items():
            if self._matches_criteria(state, criteria):
                ...
                if include_cultural_context:
                    result["cultural_context"] = await self.get_cultural_context(
                        state_id
                    )
    ...
    async def _validate_cultural_consistency(
        ...
        if (
            cultural_context.religious_context == "islamic"
            and not cultural_context.islamic_compliance
        ):
            return False
    ...
    async def _update_compliance_data(
        self, state_id: str, compliance_updates: Dict[str, Any]
    ) -> None:
    ...
    def _matches_criteria(
        self, state: Dict[str, Any], criteria: Dict[str, Any]
    ) -> bool:
        ...
        for key, value in criteria.items():
            if key == "type" and state.get("type") != value:
                ...
                and state.get("cultural_context", {}).get("language") != value
            ):
                return False
