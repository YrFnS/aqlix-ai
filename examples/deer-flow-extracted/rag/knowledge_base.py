...
class IraqiKnowledgeBase:
    ...
    async def get_domain_statistics(self, domain: IraqiDomain) -> Dict[str, Any]:
        ...
        domain_docs = [doc for doc in self.documents.values() if doc.domain == domain]
        ...
        for doc in domain_docs:
            ...
            stats["document_types"][doc_type] = (
                stats["document_types"].get(doc_type, 0) + 1
            )

            # Languages
            stats["languages"][doc.language] = (
                stats["languages"].get(doc.language, 0) + 1
            )
            ...
            if doc.date_updated:
                if (
                    ...
                    or doc.date_updated > stats["last_updated"]
                ):
    ...
    async def get_related_documents(
        self, doc_id: str, relationship_types: List[str] = None, max_results: int = 10
    ...
    async def update_document(
        self, doc_id: str, updates: Dict[str, Any], reprocess: bool = True
    ) -> bool:
    ...
    async def _generate_document_embeddings(
        self, document: IraqiDocument, chunks: List[str]
    ) -> None:
        ...
        if document.domain not in self.vector_stores:
            self.vector_stores[document.domain] = FAISS.from_documents(
