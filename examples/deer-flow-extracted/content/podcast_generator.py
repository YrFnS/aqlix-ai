...
class IraqiPodcastGenerator:
    ...
    async def _generate_educational_script(
        ...
        introduction = await self._create_introduction(topic, target_audience, "تعليمي")
    ...
    async def _validate_legal_content(self, script: PodcastScript) -> Dict[str, Any]:
        ...
        disclaimer_found = any(
            ...
            + [script.introduction, script.conclusion]
        )
        ...
        if script.voice_style not in [
            VoiceStyle.PROFESSIONAL,
            VoiceStyle.FORMAL_ARABIC,
        ]:
    ...
    async def _create_audio_segments(
        self, script: PodcastScript, voice_style: VoiceStyle
    ) -> List[AudioSegment]:
    ...
    async def _create_introduction(
        self, topic: str, audience: str, content_type: str
    ) -> str:
    ...
    async def _process_educational_content(
        self, content: str, duration_minutes: int, include_examples: bool
    ) -> List[str]:
