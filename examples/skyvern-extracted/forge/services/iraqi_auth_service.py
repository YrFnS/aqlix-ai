...
class IraqiInstitutionConfig(BaseModel):
    ...
    security_clearance_level: str = Field(default="public")  # public, restricted, confidential, secret
...
class IraqiInstitutionAuthService:
    ...
    async def authenticate_iraqi_institution(
        ...
        try:
            ...
            auth_result = IraqiAuthResult(
                ...
                islamic_compliance_level=institution.get('islamic_compliance_level', 'standard'),
                ...
                expires_at=datetime.now(timezone.utc) + timedelta(
                    minutes=institution_config.session_timeout_minutes
                )
            )
    ...
    async def _validate_iraqi_requirements(
        ...
        if config.institution_type == "banking" and config.islamic_banking_certified:
            if not await self._validate_islamic_banking_compliance(credentials):
                raise IraqiComplianceError("Islamic banking compliance validation failed")
    ...
    async def _authenticate_user_credentials(
        ...
        query = select("*").where(
            "username = :username AND active = true"
    ...
    async def _log_auth_event(
        ...
        log_data = {
            ...
            "national_id": credentials.national_id[:6] + "****" if credentials.national_id else None,
