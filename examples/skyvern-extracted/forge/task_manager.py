...
class IraqiTaskManager:
    ...
    async def _validate_task_requirements(self, task: IraqiTask) -> None:
        ...
        if task.task_type == IraqiTaskType.GOVERNMENT_PORTAL:
            ...
            if not task.user_id or not task.institution_id:
                raise TaskExecutionError("Government portal tasks require user and institution")
                
        # Validate ministry coordination
        if task.task_type == IraqiTaskType.MINISTRY_COORDINATION:
            if not task.config.ministry_codes:
                raise TaskExecutionError("Ministry coordination requires ministry codes")
                
            if task.config.security_clearance_required not in ["public", "restricted", "confidential"]:
        ...
        if task.task_type == IraqiTaskType.BANKING_ISLAMIC:
            if not task.config.islamic_compliance_check:
                raise TaskExecutionError("Islamic banking tasks require compliance check")
                
        # Validate URL for Iraqi domains if government task
        if task.task_type in [IraqiTaskType.GOVERNMENT_PORTAL, IraqiTaskType.MINISTRY_COORDINATION]:
            if task.url and not await self._is_iraqi_government_domain(task.url):
                logger.warning(f"Task URL may not be Iraqi government domain: {task.url}")
    ...
    async def _schedule_iraqi_task(self, task: IraqiTask) -> None:
        ...
        if task.priority == IraqiTaskPriority.EMERGENCY:
            ...
            scheduled_time = await self._get_next_business_hour_slot(
                baghdad_now, task
            )
    ...
    async def _get_next_business_hour_slot(
        ...
        if await self._is_iraqi_business_hours(current_time):
            ...
            if (task.config.avoid_friday_prayer and 
                current_time.weekday() == 4 and  # Friday
                self.business_schedule.friday_prayer_start <= current_time.hour < self.business_schedule.friday_prayer_end):
    ...
    async def _process_priority_queue(self, priority: IraqiTaskPriority) -> None:
        ...
        ready_tasks = [
            ...
            if (task.status == IraqiTaskStatus.SCHEDULED and
                task.scheduled_at and
                task.scheduled_at <= now and
                task.task_id not in self._task_locks)
        ]
        ...
        for key, value in task.parameters.items():
            if isinstance(value, str):
                if not await self._is_islamically_compliant_content(value):
                    raise TaskExecutionError(f"Parameter {key} failed Islamic compliance")
    ...
    async def _handle_task_failure(self, task: IraqiTask, error: str) -> None:
        ...
        if task.retry_count < task.config.max_retries:
            ...
            logger.error(f"Task failed after {task.retry_count} retries: {task.task_id}")
    ...
    async def _is_peak_hours(self, current_time: datetime) -> bool:
        ...
        return (self.business_schedule.peak_hours_start <= 
                current_time.hour < 
    ...
    async def _is_islamically_compliant_content(self, content: str) -> bool:
        ...
        prohibited_terms = [
            "interest", "usury", "riba", "gambling", "alcohol",
            "casino", "lottery", "adult content", "pornography"
        ]
