...
class WorkflowService:
    ...
    async def _execute_islamic_compliance_block(
        self, block: IslamicComplianceBlock, workflow_run: WorkflowRun
    ) -> None:
    ...
    async def _execute_arabic_form_block(
        self, block: ArabicFormProcessingBlock, workflow_run: WorkflowRun
    ) -> None:
        ...
        await self.context_manager.set_parameter(
            workflow_run, f"{block.block_id}_processed_data", processed_data
        )

    async def _execute_multi_ministry_block(
        self, block: MultiMinistryCoordinationBlock, workflow_run: WorkflowRun
    ) -> None:
        ...
        await self.context_manager.set_parameter(
            workflow_run, f"{block.block_id}_coordination_results", coordination_results
        )

    async def _execute_standard_block(
        self, block: WorkflowBlock, workflow_run: WorkflowRun
    ) -> None:
    ...
    async def _execute_task_block(
        ...
        # Store result
        await self.context_manager.set_parameter(
            workflow_run, f"{block.block_id}_result", task_result
        )
    ...
    async def _validate_arabic_processing(
        ...
        if has_arabic_content and not self.iraqi_config.arabic_processing_enabled:
            logger.warning(
                "Workflow contains Arabic content but Arabic processing is disabled"
            )
    ...
    async def _is_iraqi_business_hours(self) -> bool:
        ...
        if (
            current_hour < self.iraqi_config.business_start_hour
            or current_hour >= self.iraqi_config.business_end_hour
        ):
            return False

        # Check for Friday prayer time (12:00-13:30)
        if (
            self.iraqi_config.friday_prayer_break
            and current_time.weekday() == 4  # Friday
            and 12 <= current_hour < 14
        ):
            return False
    ...
    async def _process_arabic_form_data(
        self, form_data: Dict[str, Any], block: ArabicFormProcessingBlock
    ) -> Dict[str, Any]:
    ...
    async def _process_arabic_text(
        self, text: str, processing_options: Dict[str, Any]
    ) -> str:
    ...
    async def _execute_government_portal_task(
        self, ministry_config: Dict[str, Any], block: MultiMinistryCoordinationBlock
    ) -> Dict[str, Any]:
