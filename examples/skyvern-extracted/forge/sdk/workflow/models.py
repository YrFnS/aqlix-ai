...
class IraqiWorkflowModel(Base):
    ...
    cultural_validation_level = Column(SQLEnum(IraqiCulturalValidationLevel), default=IraqiCulturalValidationLevel.STANDARD)
    islamic_compliance_required = Column(Boolean, default=True)
    arabic_language_support = Column(Boolean, default=True)
    
    # Government portal integration
    government_portals = Column(JSON)  # List of government portals this workflow interacts with
    ...
    requires_approval = Column(Boolean, default=True)  # Whether execution needs approval
...
class IraqiTaskModel(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    workflow_run_id = Column(String, ForeignKey("iraqi_workflow_runs.id"), nullable=False)
    organization_id = Column(String, nullable=False, index=True)
...
class IraqiStepModel(Base):
    ...
    element_detected = Column(Boolean, default=False)  # Whether target element was found
...
class IraqiWorkflowParameterModel(Base):
    ...
    parameter_type = Column(String(50), nullable=False)  # string, integer, boolean, file, etc.
...
class IraqiAuditLogModel(Base):
    ...
    # Audit event details
    event_type = Column(String(100), nullable=False)  # workflow_started, task_completed, etc.
...
class IraqiWorkflowCreate(BaseModel):
    ...
    cultural_validation_level: IraqiCulturalValidationLevel = IraqiCulturalValidationLevel.STANDARD
