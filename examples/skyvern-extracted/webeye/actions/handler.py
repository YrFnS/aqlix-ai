...
class IraqiActionHandler:
    ...
    def __init__(self, page: Page, config: Optional[Dict[str, Any]] = None):
        ...
        self.arabic_text_pattern = re.compile(
            r"[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]"
        )
    ...
    async def execute_action(
        ...
        try:
            logger.info(
                f"Executing action {action_id}: {action_type.value} on {target}"
            )
            ...
            if result.success:
                logger.info(
                    f"Action {action_id} completed successfully in {execution_time:.2f}s"
                )
            else:
    ...
    async def _dispatch_action(
        ...
        if not handler:
            return ActionResult(
                success=False, message=f"Unsupported action type: {action_type.value}"
            )
    ...
    async def _handle_click_action(
        ...
        try:
            ...
            await element.wait_for(
                state="visible", timeout=self.government_portal_timeout
            )
            ...
            return ActionResult(
                success=True, message="Click action completed successfully"
            )

        except Exception as e:
            return ActionResult(success=False, message=f"Click action failed: {str(e)}")
    ...
    async def _handle_input_text_action(
        ...
        try:
            if not data or "text" not in data:
                return ActionResult(
                    success=False, message="No text provided for input action"
                )
            ...
            if data.get("validate_input", True):
                ...
                if entered_value != text:
                    logger.warning(
                        f"Input validation failed. Expected: {text}, Got: {entered_value}"
                    )
            ...
            return ActionResult(success=False, message=f"Text input failed: {str(e)}")
    ...
    async def _handle_select_option_action(
        ...
        try:
            if not data or "option" not in data:
                return ActionResult(
                    success=False, message="No option provided for select action"
                )
            ...
            selection_strategies = [
                ...
                lambda: element.select_option(
                    index=int(option) if option.isdigit() else 0
                ),
            ...
            return ActionResult(
                success=False, message=f"Option selection failed: {str(e)}"
            )
    ...
    async def _handle_upload_file_action(
        ...
        try:
            if not data or "file_path" not in data:
                return ActionResult(
                    success=False, message="No file path provided for upload action"
                )
    ...
    async def _handle_authenticate_action(
        ...
        try:
            if not data:
                return ActionResult(
                    success=False, message="No authentication data provided"
                )
    ...
    async def _validate_iraqi_document(
        self, file_path: str, context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_info["size"] > max_size:
                ...
                validation_result["message"] = (
                    f"File size exceeds limit: {file_info['size']} bytes"
                )
                return validation_result
            ...
            if file_info["extension"] not in allowed_extensions:
                validation_result["valid"] = False
                validation_result["message"] = (
                    f"File type not allowed: {file_info['extension']}"
                )
                return validation_result
    ...
    async def _handle_basic_auth(self, data: Dict[str, Any]) -> ActionResult:
        ...
        try:
            ...
            if not username_filled:
                return ActionResult(
                    success=False, message="Could not find username field"
                )
            ...
            if not password_filled:
                return ActionResult(
                    success=False, message="Could not find password field"
                )
            ...
            return ActionResult(success=True, message="Basic authentication completed")

        except Exception as e:
            return ActionResult(
                success=False, message=f"Basic authentication failed: {str(e)}"
            )
...
async def fill_iraqi_form(
    ...
    for field in form_fields:
        ...
        result = await handler.execute_action(
            ActionType.INPUT_TEXT, field_selector, action_data, context
        )
...
async def automate_government_portal_workflow(
    handler: IraqiActionHandler, workflow_steps: List[Dict[str, Any]]
) -> List[ActionResult]:
    """
    ...
    for i, step in enumerate(workflow_steps):
        logger.info(
            f"Executing workflow step {i + 1}/{len(workflow_steps)}: {step.get('name', 'Unnamed step')}"
        )
        ...
        if not result.success:
            if step.get("critical", True):
                ...
                logger.warning(
                    f"Non-critical step failed, continuing: {result.message}"
                )
