...
import {
  ...
  optimizeArabicText,
} from '../utils/arabicFontOptimizer';
 ...
 */
export const useArabicTextRendering = (options: UseArabicTextRenderingOptions = {}) => {
  ...
  const analyzeTextContent = useCallback(
    (
      ...
      let detectedDialect: 'iraqi' | 'baghdad' | 'basra' | 'mosul' | 'standard' | 'unknown' =
        'unknown';
      let maxMatches = 0;
  ...
  const applyCulturalAdaptations = useCallback(
    (element: HTMLElement, currentDialect: string, currentDomain: string) => {
