...
interface NassWalletTransactionResponse {
  ...
  status: 'pending_user_action' | 'processing' | 'completed' | 'failed' | 'cancelled' | 'expired';
  paymentUrl: string;
  qrCodeData: string;
  amount: number;
  currency: 'IQD';
...
export class NassWalletPaymentNode extends IraqiGovernmentNodeBase {
    ...
    return [
        ...
        options: [
          {
            ...
            required: true,
            options: [
              { name: 'Government Services (خدمات حكومية)', value: 'government_services' },
              { name: 'Healthcare (رعاية صحية)', value: 'healthcare' },
              { name: 'Education (تعليم)', value: 'education' },
              { name: 'Retail (تجارة تجزئة)', value: 'retail' },
              { name: 'Food & Beverage (أطعمة ومشروبات)', value: 'food_beverage' },
              { name: 'Transportation (نقل)', value: 'transportation' },
              { name: 'Utilities (مرافق عامة)', value: 'utilities' },
              { name: 'Professional Services (خدمات مهنية)', value: 'professional_services' },
              { name: 'Other Halal Services (خدمات حلال أخرى)', value: 'other_halal' },
            ],
          ...
          },
          {
            ...
            description: 'Official business name (Arabic preferred for government)',
          },
          {
      ...
      },
      {
        ...
        options: [
          ...
          { name: 'Duplicate Payment (دفعة مكررة)', value: 'duplicate_payment' },
          { name: 'Service Not Delivered (الخدمة لم تُقدم)', value: 'service_not_delivered' },
  ...
  async execute(this: INodeExecuteFunctions): Promise<any[][]> {
    ...
    // Get credentials
    const credentials = await this.getCredentials('nassWalletApi') as NassWalletCredentials;
    ...
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        switch (operation) {
          case 'createPayment':
            result = await this.createPayment.call(this, i, credentials, culturalContext, securitySettings, advancedOptions);
            break;

          case 'checkStatus':
            result = await this.checkPaymentStatus.call(this, i, credentials, culturalContext, securitySettings);
            break;

          case 'confirmPayment':
            result = await this.confirmPayment.call(this, i, credentials, culturalContext, securitySettings, advancedOptions);
            break;

          case 'checkBalance':
            result = await this.checkWalletBalance.call(this, i, credentials, culturalContext, securitySettings);
            break;

          case 'reversePayment':
            result = await this.reversePayment.call(this, i, credentials, culturalContext, securitySettings);
            break;
  ...
  private async createPayment(
    ...
    const redirectUrls = this.getNodeParameter('redirectUrls', itemIndex) as any;
    ...
    if (!cleanPhone.match(/^964[0-9]{10}$/) && !cleanPhone.match(/^0[0-9]{10}$/)) {
      throw new NodeOperationError(
        this.getNode(),
    ...
    if (advancedOptions.requireIslamicCompliance !== false) {
      ...
      const complianceCheck = this.validateIslamicCompliance(transactionData, culturalContext);
    ...
    const language = advancedOptions.language || culturalContext.language || 'ar';
    const notifications = advancedOptions.notificationPreferences || ['sms', 'webhook'];
    ...
    const transactionRequest: NassWalletTransactionRequest = {
      ...
      customerInfo: {
        ...
        phone: cleanPhone.startsWith('964') ? `+${cleanPhone}` : `+964${cleanPhone.substring(1)}`,
      ...
      transactionMetadata: {
        ...
        purpose: description,
        beneficiaryType: merchantInfo.businessType === 'government' ? 'government' : 'business',
        isGovernmentTransaction: advancedOptions.isGovernmentTransaction !== false,
      },
      paymentOptions: {
    ...
    const signature = this.generateNassWalletSignature(transactionRequest, credentials.clientSecret);
    ...
    try {
      ...
      const formattedBalance = this.formatIraqiCurrency(nasswalletResponse.balance.availableBalance);
      const formattedFees = this.formatIraqiCurrency(nasswalletResponse.fees.totalFees);
      ...
      const auditLog = this.createAuditLog(
        ...
        { 
          ...
          customerInfo: {
            ...
            nationalId: customerInfo.nationalId ? this.maskNationalId(customerInfo.nationalId) : undefined,
          }
        },
        culturalContext,
        securitySettings,
      ...
      return {
        json: {
          ...
          fees: {
            ...
            breakdown: {
              transaction: this.formatIraqiCurrency(nasswalletResponse.fees.transactionFee),
              service: this.formatIraqiCurrency(nasswalletResponse.fees.serviceFee),
              tax: this.formatIraqiCurrency(nasswalletResponse.fees.governmentTax),
            },
          },
          balance: {
            available: {
          ...
          language,
          governmentTransaction: advancedOptions.isGovernmentTransaction !== false,
  ...
  private async checkPaymentStatus(
    ...
    try {
      ...
      const timestamp = Date.now().toString();
      const signature = this.generateStatusCheckSignature(transactionId, credentials.clientSecret, timestamp);
      ...
      const formattedNetAmount = this.formatIraqiCurrency(statusResponse.netAmount);
      const formattedFees = this.formatIraqiCurrency(statusResponse.fees.totalFees);
      ...
      return {
        json: {
          ...
          fees: {
            ...
            totalFormatted: formattedFees,
            breakdown: {
              transaction: this.formatIraqiCurrency(statusResponse.fees.transactionFee),
          ...
          securityChecks: {
            ...
            complianceLevel: this.getComplianceLevel(statusResponse.securityChecks.complianceScore),
          },
          statusMessages: {
  ...
  private async confirmPayment(
    ...
    const statusResult = await this.checkPaymentStatus(itemIndex, credentials, culturalContext, securitySettings);
    ...
    const minComplianceScore = advancedOptions.isGovernmentTransaction !== false ? 70 : 60;
    ...
    const feePercentage = (paymentData.fees.original.totalFees / paymentData.amount.original) * 100;
    ...
    return {
      json: {
        ...
        confirmationChecks: {
          ...
          complianceScoreValid: securityChecks.complianceScore >= minComplianceScore,
  ...
  private async checkWalletBalance(
    ...
    try {
      ...
      const timestamp = Date.now().toString();
      const signature = this.generateBalanceCheckSignature(customerId, credentials.clientSecret, timestamp);
      ...
      const formattedAvailable = this.formatIraqiCurrency(balanceData.availableBalance);
      const formattedReserved = this.formatIraqiCurrency(balanceData.reservedBalance || 0);
  ...
  private async reversePayment(
    ...
    const transactionId = this.getNodeParameter('transactionId', itemIndex) as string;
    const reversalReason = this.getNodeParameter('reversalReason', itemIndex) as string;
    ...
    try {
      ...
      const timestamp = Date.now().toString();
      const signature = this.generateNassWalletSignature(reversalRequest, credentials.clientSecret);
      ...
      const formattedReversalAmount = this.formatIraqiCurrency(reversalResponse.reversalAmount);
      ...
      return {
        json: {
          ...
          processingFee: reversalResponse.processingFee ? {
            ...
            formatted: this.formatIraqiCurrency(reversalResponse.processingFee),
          } : null,
          auditLog,
   ...
   */
  private generateStatusCheckSignature(transactionId: string, clientSecret: string, timestamp: string): string {
    const signatureData = `${transactionId}:${timestamp}`;
    return createHmac('sha256', clientSecret).update(signatureData).digest('hex');
  }

  /**
   * Generate balance check signature
   */
  private generateBalanceCheckSignature(customerId: string, clientSecret: string, timestamp: string): string {
  ...
  private getStatusMessage(status: string, language: 'en' | 'ar'): string {
    ...
    return messages[status as keyof typeof messages]?.[language] || `Unknown status: ${status}`;
  }
}