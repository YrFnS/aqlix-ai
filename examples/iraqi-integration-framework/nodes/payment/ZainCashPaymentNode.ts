...
interface ZainCashTransactionRequest {
  ...
  serviceType: 'book' | 'food' | 'grocery' | 'pharmacy' | 'transportation' | 'other';
  orderId: string;
  redirectUrl: string;
  production: boolean;
  referenceId: string;
...
export class ZainCashPaymentNode extends IraqiGovernmentNodeBase {
  ...
  getNodeProperties(): INodeProperties[] {
    return [
      ...
      },
      {
        ...
        required: true,
        options: [
          { name: 'Book', value: 'book', description: 'Books and educational materials' },
          { name: 'Food', value: 'food', description: 'Food and dining services' },
          { name: 'Grocery', value: 'grocery', description: 'Grocery and household items' },
          { name: 'Pharmacy', value: 'pharmacy', description: 'Medical and pharmaceutical services' },
          { name: 'Transportation', value: 'transportation', description: 'Transport and delivery services' },
          { name: 'Other', value: 'other', description: 'Other halal services' },
        ],
        default: 'other',
  ...
  async execute(this: INodeExecuteFunctions): Promise<any[][]> {
    ...
    // Get credentials
    const credentials = await this.getCredentials('zainCashApi') as ZainCashCredentials;
      ...
      try {
        ...
        switch (operation) {
          ...
          case 'validatePayment':
            result = await this.validatePayment.call(this, i, credentials, culturalContext, securitySettings, advancedOptions);
            break;
  ...
  private async createPayment(
    ...
    if (advancedOptions.requireIslamicCompliance !== false) {
      const transactionData = { amount, serviceType, merchantCategory: serviceType };
    ...
    const token = jwt.sign(jwtPayload, credentials.merchantSecret, { algorithm: 'HS256' });
  ...
  private async checkPaymentStatus(
    ...
    try {
      ...
      const formattedTotal = this.formatIraqiCurrency(statusResponse.totalAmount);
      const formattedFee = this.formatIraqiCurrency(statusResponse.serviceCharge);
  ...
  private async validatePayment(
    ...
    const feePercentage = (paymentData.serviceCharge.original / paymentData.amount.original) * 100;
