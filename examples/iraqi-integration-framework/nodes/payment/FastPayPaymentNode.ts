...
interface FastPayTransactionResponse {
  ...
  status: 'initiated' | 'pending' | 'completed' | 'failed' | 'cancelled' | 'expired';
  paymentUrl: string;
  qrCode?: string;
  amount: number;
  currency: 'IQD';
...
export class FastPayPaymentNode extends IraqiGovernmentNodeBase {
  ...
  getNodeProperties(): INodeProperties[] {
    return [
      ...
      },
      {
        ...
        required: true,
        options: [
          { name: 'FastPay Wallet Only', value: 'wallet', description: 'FastPay mobile wallet only' },
          { name: 'Credit/Debit Cards Only', value: 'card', description: 'Credit and debit cards only' },
          { name: 'Both Wallet and Cards', value: 'both', description: 'Allow both payment methods' },
        ],
        default: 'both',
  ...
  async execute(this: INodeExecuteFunctions): Promise<any[][]> {
    ...
    // Get credentials
    const credentials = await this.getCredentials('fastPayApi') as FastPayCredentials;
    ...
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        switch (operation) {
          ...
          case 'refundPayment':
            result = await this.refundPayment.call(this, i, credentials, culturalContext, securitySettings);
            break;
  ...
  private async createPayment(
    ...
    const transactionRequest: FastPayTransactionRequest = {
      ...
      orderId,
      description,
      customerPhone: cleanPhone.startsWith('964') ? `+${cleanPhone}` : `+964${cleanPhone.substring(1)}`,
      customerName,
    ...
    const signature = this.generateFastPaySignature(transactionRequest, credentials.secretKey);
    ...
    try {
      ...
      const formattedFees = this.formatIraqiCurrency(fastpayResponse.fees.totalFees);
      ...
      const auditLog = this.createAuditLog(
        ...
        { amount, orderId, customerPhone: this.maskPhoneNumber(customerPhone), description },
        { ...fastpayResponse, customerPhone: this.maskPhoneNumber(fastpayResponse.customerPhone) },
        culturalContext,
        securitySettings,
        true
      );
      ...
      return {
        json: {
          ...
          fees: {
            ...
            breakdown: {
              customer: this.formatIraqiCurrency(fastpayResponse.fees.customerFee),
              merchant: this.formatIraqiCurrency(fastpayResponse.fees.merchantFee),
            },
          },
          currency: 'IQD',
          orderId,
  ...
  private async checkPaymentStatus(
    ...
    try {
      ...
      const auditLog = this.createAuditLog(
        ...
        { transactionId },
        { ...statusResponse, customerPhone: this.maskPhoneNumber(statusResponse.customerPhone) },
        culturalContext,
        securitySettings,
        true
      );
      ...
      return {
        json: {
          ...
          fees: {
            ...
            totalFormatted: formattedFees,
            breakdown: {
              customer: this.formatIraqiCurrency(statusResponse.fees.customerFee),
              merchant: this.formatIraqiCurrency(statusResponse.fees.merchantFee),
            },
          },
  ...
  private async confirmPayment(
    ...
    const auditLog = this.createAuditLog(
      ...
      { confirmed: true, hashValid, fraudCheck: 'passed', feeCheck: reasonableFee },
      culturalContext,
      securitySettings,
      true
    );
  ...
  private async refundPayment(
    ...
    const refundReason = this.getNodeParameter('refundReason', itemIndex) as string;
    ...
    const actualRefundAmount = refundAmount > 0 ? refundAmount : paymentData.netAmount.original;
    ...
    try {
      ...
      const signature = this.generateFastPaySignature(refundRequest, credentials.secretKey);
      ...
      // Format amounts
      const formattedRefundAmount = this.formatIraqiCurrency(actualRefundAmount);
      ...
      const auditLog = this.createAuditLog(
        ...
        { transactionId, refundAmount: actualRefundAmount, reason: refundReason },
        refundResponse,
        culturalContext,
        securitySettings,
        true
  ...
  private generateFastPaySignature(data: any, secretKey: string): string {
    ...
    return createHash('sha256').update(dataString + secretKey).digest('hex');
  }
  ...
  private inferMerchantCategory(description: string): string {
    ...
    if (desc.includes('food') || desc.includes('restaurant') || desc.includes('طعام') || desc.includes('مطعم')) {
      return 'food';
    }
    if (desc.includes('pharmacy') || desc.includes('medicine') || desc.includes('صيدلية') || desc.includes('دواء')) {
      return 'pharmacy';
    }
    if (desc.includes('transport') || desc.includes('taxi') || desc.includes('نقل') || desc.includes('تاكسي')) {
      return 'transportation';
    }
    if (desc.includes('book') || desc.includes('education') || desc.includes('كتاب') || desc.includes('تعليم')) {
      return 'book';
    }
    if (desc.includes('grocery') || desc.includes('shopping') || desc.includes('بقالة') || desc.includes('تسوق')) {
      return 'grocery';
    }
