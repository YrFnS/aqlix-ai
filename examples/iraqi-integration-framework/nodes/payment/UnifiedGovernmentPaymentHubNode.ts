...
export class UnifiedGovernmentPaymentHubNode extends IraqiGovernmentNodeBase {
  ...
  async execute(this: INodeExecuteFunctions): Promise<any[][]> {
    ...
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        switch (operation) {
          case 'processPayment':
            result = await this.processPayment.call(this, i, culturalContext, securitySettings, advancedOptions);
            break;

          case 'checkStatus':
            result = await this.checkPaymentStatus.call(this, i, culturalContext, securitySettings);
            break;

          case 'healthCheck':
            result = await this.performHealthCheck.call(this, i, culturalContext, securitySettings);
            break;

          case 'getAnalytics':
            result = await this.getPaymentAnalytics.call(this, i, culturalContext, securitySettings);
            break;

          case 'fraudAnalysis':
            result = await this.performFraudAnalysis.call(this, i, culturalContext, securitySettings, advancedOptions);
            break;
  ...
  private async processPayment(
    ...
    const preferences = this.getNodeParameter('preferences', itemIndex, {}) as any;
    ...
    const fraudAnalysis = await this.performComprehensiveFraudAnalysis(paymentRequest, culturalContext, advancedOptions);
    ...
    const formattedAmountWithFees = this.formatIraqiCurrency(paymentResult.amountWithFees);
    ...
    const unifiedResponse: UnifiedPaymentResponse = {
      ...
      securityValidation: {
        ...
        complianceScore: gatewaySelection.complianceCheck.complianceScore,
        securityScore: this.calculateOverallSecurityScore(fraudAnalysis, gatewaySelection),
      },
  ...
  private async getGatewayConfigurations(): Promise<PaymentGatewayConfig[]> {
    const configs: PaymentGatewayConfig[] = [
      ...
      },
      {
        ...
        supportedFeatures: ['bank_transfer', 'wallet', 'government_integration', 'aml_compliance'],
        isGovernmentPreferred: true,
        islamicCompliant: true,
        status: 'active',
      },
    ...
    return configs.filter(config => config.status === 'active' && config.availabilityScore > 70);
  }
  ...
  private async checkGatewayAvailability(gateway: string): Promise<number> {
    try {
      ...
      const response = await axios.get(baseUrls[gateway as keyof typeof baseUrls], {
        timeout: 5000,
  ...
  private async performComprehensiveFraudAnalysis(
    ...
    if (request.amount > 10000000) { // > 10M IQD
      riskScore += 40;
      triggers.push('Very high amount transaction');
      recommendations.push('Require enhanced verification for amounts over 10M IQD');
    } else if (request.amount > 5000000) { // > 5M IQD
      riskScore += 25;
  ...
  private async selectOptimalGateway(
    ...
    let eligibleGateways = gateways.filter(g => 
    ...
    if (request.preferences?.preferredGateway && request.preferences.preferredGateway !== 'auto') {
      const preferred = eligibleGateways.find(g => g.name === request.preferences.preferredGateway);
      if (preferred) {
        // Move preferred to front but still evaluate all
        eligibleGateways = [preferred, ...eligibleGateways.filter(g => g.name !== request.preferences.preferredGateway)];
      }
    }
    ...
    const scoredGateways = eligibleGateways.map(gateway => {
      ...
      if (gateway.isGovernmentPreferred && request.merchantInfo.businessType === 'government') {
        score += 5;
      }
  ...
  private async processPaymentThroughGateway(
    ...
    try {
      ...
      switch (gateway.name) {
        case 'zaincash':
          result = await this.processZainCashPayment(request, culturalContext, advancedOptions);
          break;
        case 'fastpay':
          result = await this.processFastPayPayment(request, culturalContext, advancedOptions);
          break;
        case 'nasswallet':
          result = await this.processNassWalletPayment(request, culturalContext, advancedOptions);
          break;
        default:
      ...
      if (advancedOptions.enableFailover && selection.backupGateways.length > 0) {
        return await this.attemptFailover(request, selection, culturalContext, advancedOptions, startTime);
      }
  ...
  private async attemptFailover(
    ...
    for (const backupGateway of selection.backupGateways) {
      ...
      if (request.amount < backupGateway.minimumAmount || request.amount > backupGateway.maximumAmount) {
        continue;
      }
      ...
      try {
        ...
        return {
          ...
          amountWithFees: request.amount + ((request.amount * backupGateway.feePercentage) / 100),
          processingTime: Date.now() - startTime,
   ...
   */
  private async processZainCashPayment(request: PaymentRequest, culturalContext: IraqiCulturalContext, advancedOptions: any): Promise<any> {
   ...
   */
  private async processFastPayPayment(request: PaymentRequest, culturalContext: IraqiCulturalContext, advancedOptions: any): Promise<any> {
    return {
   ...
   */
  private async processNassWalletPayment(request: PaymentRequest, culturalContext: IraqiCulturalContext, advancedOptions: any): Promise<any> {
    return {
  ...
  private async performHealthCheck(
    ...
    const healthData = {
      ...
      gateways: gateways.map(gateway => ({
        ...
        healthStatus: gateway.availabilityScore > 80 ? 'healthy' : 
                     gateway.availabilityScore > 60 ? 'degraded' : 'unhealthy',
  ...
  private async convertToIQD(amount: number, currency: 'IQD' | 'USD'): Promise<number> {
  ...
  private generateSelectionReason(selected: any, fraudAnalysis: FraudDetectionResult): string {
    const reasons = [];
  ...
  private calculateOverallSecurityScore(fraudAnalysis: FraudDetectionResult, selection: GatewaySelectionResult): number {
    ...
    return Math.round((fraudScore * 0.4) + (complianceScore * 0.35) + (gatewayScore * 0.25));
  }
  ...
  private async createPaymentMonitoring(
    ...
    return {
      ...
      expectedCompletionTime: new Date(Date.now() + selection.estimatedProcessingTime * 1000).toISOString(),
  ...
  private generateHealthRecommendations(gateways: PaymentGatewayConfig[]): string[] {
    const recommendations = [];
    
    const unhealthyGateways = gateways.filter(g => g.availabilityScore < 80);
    if (unhealthyGateways.length > 0) {
      recommendations.push(`Monitor ${unhealthyGateways.map(g => g.displayName).join(', ')} - availability below optimal`);
    }

    const lowPerformance = gateways.filter(g => g.performanceScore < 80);
    if (lowPerformance.length > 0) {
      recommendations.push(`Consider performance optimization for ${lowPerformance.map(g => g.displayName).join(', ')}`);
    }
