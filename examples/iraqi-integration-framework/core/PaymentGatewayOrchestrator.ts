...
export class PaymentGatewayOrchestrator extends EventEmitter {
  ...
  async registerGateway(gateway: IPaymentGateway): Promise<void> {
    try {
      ...
      const culturalValidation =
        await this.validateGatewayCulturalCompliance(gateway);
      if (!culturalValidation.islamicCompliance.halalStatus) {
        throw new Error(
          `Gateway ${gateway.id} does not meet Islamic compliance requirements`,
        );
      }
  ...
  async processPayment(request: IPaymentRequest): Promise<IPaymentResponse> {
    ...
    try {
      ...
      if (!culturalValidation.islamicCompliance.halalStatus) {
        throw new Error(
          "Payment does not meet Islamic compliance requirements",
        );
      }
  ...
  async refundPayment(
    ...
    if (!gateway) {
      throw new Error(
        `Gateway ${originalPayment.gatewayUsed} not available for refund`,
      );
    }

    if (!gateway.features.refundSupport) {
      throw new Error(
        `Gateway ${originalPayment.gatewayUsed} does not support refunds`,
      );
    }
    ...
    this.routingRules = [
      {
        ...
        condition: (req) =>
          req.customer.phone.startsWith("+9647") && req.amount >= 1000,
        gateway: "zaincash",
        priority: 1,
      },
      {
      ...
      },
      {
        ...
        condition: (req) =>
          req.amount >= 1000 &&
          req.culturalContext.businessContext === "government_service",
        gateway: "nasswallet",
        priority: 1,
      },
    ];
  ...
  private async validateGatewayConfig(gateway: IPaymentGateway): Promise<void> {
    ...
    if (!gateway.compliance.islamicBankingCompliant) {
      throw new Error(
        "Islamic banking compliance is required for Iraqi payment gateways",
      );
    }
  }
  ...
  private async validateGatewayCulturalCompliance(
    gateway: IPaymentGateway,
  ): Promise<ICulturalPaymentValidation> {
    return {
      islamicCompliance: {
        score: 95,
        halalStatus: true,
  ...
  private async validatePaymentRequest(
    ...
    if (!request.description || !request.descriptionArabic) {
      throw new Error(
        "Payment description in both Arabic and English is required",
      );
    }
  }

  private async validatePaymentCulture(
    request: IPaymentRequest,
  ): Promise<ICulturalPaymentValidation> {
  ...
  private async selectOptimalGateway(
    ...
    if (request.preferredGateway) {
      const preferredGateway = availableGateways.find(
        (g) => g.provider === request.preferredGateway,
      );
      if (preferredGateway) {
  ...
  private selectBestPerformingGateway(
  ...
  private calculateFees(
    ...
    return {
      ...
      breakdown: [
        {
          name: "Gateway Fee",
          nameArabic: "رسوم البوابة",
          amount: gatewayFee,
          percentage: 2,
          description: "Payment gateway processing fee",
          descriptionArabic: "رسوم معالجة بوابة الدفع",
        },
        {
          name: "Processing Fee",
          nameArabic: "رسوم المعالجة",
          amount: processingFee,
          description: "Transaction processing fee",
          descriptionArabic: "رسوم معالجة المعاملة",
        },
        {
          name: "Government Tax",
          nameArabic: "الضريبة الحكومية",
          amount: governmentTax,
          percentage: 0.5,
          description: "Government tax on transaction",
          descriptionArabic: "الضريبة الحكومية على المعاملة",
        },
      ],
    };
  }
  ...
  private getPaymentErrorContext(
    culturalContext: ICulturalPaymentContext,
  ): string {
  ...
  private getPaymentErrorResolution(message: string): string {
    ...
    return (
      resolutions[message] || "Please contact customer support for assistance"
    );
  }

  private getPaymentErrorSeverity(
    message: string,
  ): "low" | "medium" | "high" | "critical" {
    if (message.includes("timeout") || message.includes("network"))
      return "medium";
    if (message.includes("declined") || message.includes("invalid"))
      return "high";
  ...
  private getPaymentSystemStatus(): "healthy" | "degraded" | "critical" {
    if (
      this.performanceMetrics.successRate > 95 &&
      this.performanceMetrics.averageProcessingTime < 1000
    ) {
      return "healthy";
    } else if (
      this.performanceMetrics.successRate > 80 &&
