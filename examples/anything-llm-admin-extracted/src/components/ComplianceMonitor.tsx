...
const ComplianceMonitor: React.FC<ComplianceMonitorProps> = ({
  ...
  const [selectedFilter, setSelectedFilter] = useState<'all' | 'violations' | 'pending' | 'resolved'>('all');
  ...
  const getComplianceStatus = (score: number) => {
    if (score >= 95) return { label: translations.excellent, color: 'green', bgColor: 'bg-green-100', textColor: 'text-green-800' };
    if (score >= 85) return { label: translations.good, color: 'blue', bgColor: 'bg-blue-100', textColor: 'text-blue-800' };
    if (score >= 70) return { label: translations.acceptable, color: 'yellow', bgColor: 'bg-yellow-100', textColor: 'text-yellow-800' };
    if (score >= 50) return { label: translations.needsImprovement, color: 'orange', bgColor: 'bg-orange-100', textColor: 'text-orange-800' };
    return { label: translations.critical, color: 'red', bgColor: 'bg-red-100', textColor: 'text-red-800' };
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-medium text-gray-900">{translations.culturalAppropriateness}</h3>
          </div>
          <div className="h-64">
            <Radar 
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-medium text-gray-900">{translations.arabicContentCompliance}</h3>
          </div>
              ...
              {mockViolations.map((violation) => (
                <tr key={violation.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {new Date(violation.reportedDate).toLocaleDateString(language === 'ar' ? 'ar-IQ' : 'en-US')}
                  </td>
                  
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {violation.resolvedDate 
                      ? new Date(violation.resolvedDate).toLocaleDateString(language === 'ar' ? 'ar-IQ' : 'en-US')
                      : '-'
                    }
                  </td>
        ...
        <div className="space-y-4">
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Consider implementing advanced Arabic NLP to better handle dialect variations and mixed-language content.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Implement more sophisticated cultural appropriateness detection for professional contexts.
              </p>
            </div>
          </div>
