...
export class OfflineArabicProcessor {
  ...
  private getDefaultConfig(): ArabicProcessingConfig {
    return {
      ...
      supportedDialects: ['baghdadi', 'basrawi', 'moslawi', 'southern', 'kurdish_arab', 'standard'],
  ...
  private async loadDialectPatterns(): Promise<void> {
    ...
    this.dialectPatterns.set('baghdadi', {
      ...
      commonWords: [
        'شلونك', 'ماكو', 'كلش', 'هسا', 'ويا', 'شوية', 'زين', 'خوش',
        'شگد', 'لتروح', 'عدكم', 'اكلكم', 'هاي', 'هايا', 'يالله'
      ],
    ...
    this.dialectPatterns.set('basrawi', {
      ...
      commonWords: [
        'شلونگ', 'گدام', 'مومو', 'حلو', 'يمعود', 'چان', 'چنت', 
        'قالولك', 'شگولة', 'راح', 'هوای', 'گاله', 'چشسوي'
      ],
    ...
    this.dialectPatterns.set('moslawi', {
      ...
      commonWords: [
        'دايش', 'بەس', 'ئيوا', 'لابد', 'قايل', 'هوسا', 'شنهو', 
        'كيفاش', 'منگول', 'اوعلي', 'هايگ', 'چيكا'
      ],
    ...
    this.dialectPatterns.set('southern', {
      ...
      commonWords: [
        'شلون', 'ماكو', 'مخاف', 'گالولي', 'شنت', 'راح', 'يايب',
        'شگولت', 'هايچي', 'گاعد', 'ويا', 'علي'
      ],
    ...
    this.dialectPatterns.set('kurdish_arab', {
      ...
      commonWords: [
        'چي', 'بەڵێ', 'کردوە', 'دەگەڕێ', 'هەر', 'ئێوە', 'چۆن',
        'کێ', 'کوێ', 'کەی', 'ناکرێ', 'دەبێ'
      ],
    ...
    this.dialectPatterns.set('standard', {
      ...
      commonWords: [
        'كيف', 'حالك', 'اسمك', 'أين', 'متى', 'لماذا', 'عمرك',
        'تتكلم', 'أريد', 'يمكنني', 'شكرا', 'من', 'فضلك', 'نعم', 'لا'
      ],
   ...
   */
  private async detectDialect(text: string): Promise<{ dialect: IraqiDialect; confidence: number }> {
  ...
  private hasArabicScript(text: string): boolean {
    return this.ARABIC_MAIN.test(text) ||
  ...
  private updateProcessingStats(result: ProcessingResult): void {
    ...
    stats.averageTime = ((stats.averageTime * (stats.totalProcessed - 1)) + result.metadata.processingTime) / stats.totalProcessed;
    
    if (result.dialect && result.confidence > 0.8) {
      stats.dialectAccuracy = Math.min((stats.dialectAccuracy + result.confidence) / 2, 1);
    }
