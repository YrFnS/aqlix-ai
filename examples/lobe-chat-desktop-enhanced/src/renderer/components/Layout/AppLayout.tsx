...
const AppContainer = styled.div<{
  ...
  background: ${({ $theme, $culturalMode }) => {
    ...
    return $theme === "dark" ? "var(--dark-bg-primary)" : "var(--bg-primary)";
  }};
...
const ProfessionalBanner = styled(motion.div)<{ $domain: ProfessionalDomain }>`
  ...
  background: ${({ $domain }) =>
    `var(--domain-${$domain}-bg, var(--bg-secondary))`};
  border-bottom: 1px solid
    ${({ $domain }) => `var(--domain-${$domain}-color, var(--text-secondary))`};
  color: ${({ $domain }) =>
...
export const AppLayout: React.FC<AppLayoutProps> = ({ children }) => {
  ...
  const { currentDomain } = useSelector(
    (state: AppState) => state.professional,
  );
  ...
  useKeyboardShortcuts({
    ...
    "alt+1": () =>
      dispatch({ type: "SET_PROFESSIONAL_DOMAIN", payload: "legal" }),
    "alt+2": () =>
      dispatch({ type: "SET_PROFESSIONAL_DOMAIN", payload: "medical" }),
    "alt+3": () =>
      dispatch({ type: "SET_PROFESSIONAL_DOMAIN", payload: "educational" }),
    "alt+4": () =>
      dispatch({ type: "SET_PROFESSIONAL_DOMAIN", payload: "engineering" }),
    "alt+5": () =>
      dispatch({ type: "SET_PROFESSIONAL_DOMAIN", payload: "business" }),
    "alt+6": () =>
      dispatch({ type: "SET_PROFESSIONAL_DOMAIN", payload: "government" }),
  });
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {showDomainBanner && currentDomain && currentDomain !== "general" && (
          <ProfessionalBanner
            ...
            exit="exit"
          >
            <div className="domain-icon">{domainConfig?.icon || "⚖️"}</div>
            <span>
      ...
      <NotificationCenter culturalMode={activeCulturalMode} isRTL={isRTL} />
