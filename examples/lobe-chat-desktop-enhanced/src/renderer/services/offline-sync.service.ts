...
export class OfflineSyncService extends EventEmitter {
  ...
  private async loadPersistedQueue(): Promise<void> {
    const persistedQueue = this.store.get("syncQueue") as Record<
      string,
      SyncQueueItem
    >;
  ...
  private async initializeOfflineCapabilities(): Promise<void> {
    ...
    this.offlineCapabilities.set("settings", {
      ...
      limitations: ["Settings sync when connection restored"],
  ...
  private handleOffline(): void {
    ...
    this.emit("connectionLost", {
      offlineCapabilities: Array.from(this.offlineCapabilities.values()),
    });
  ...
  private handleFailedItem(item: SyncQueueItem, error: Error): void {
    console.error(
      `Item ${item.id} failed after ${item.retryCount} retries:`,
      error,
    );
  ...
  private updateSyncStats(
    ...
    stats.lastSuccessfulSync =
      processedCount > 0 ? new Date() : stats.lastSuccessfulSync;
    stats.averageSyncTime =
      (stats.averageSyncTime * (stats.totalSynced - processedCount) +
        syncTime) /
      stats.totalSynced;
