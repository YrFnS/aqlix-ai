...
from .orchestration import IraqiMultiAgentSystem, OrchestrationConfig, OrchestrationStrategy
...
async def example_multi_agent_orchestration():
    ...
    if "cultural_results" in result:
        print(f"Cultural validation completed: {len(result['cultural_results'])} agents")
...
async def example_performance_monitoring():
    ...
    test_cases = [
        ...
        {"content": "Complex multi-domain test with legal and medical content", "complexity": "high"}
    ]
    ...
    for i, test_case in enumerate(test_cases):
        ...
        print(f"Test {i+1} - Status: {result['status']}, Time: {result.get('execution_time', 0):.3f}s")
    
    # Get performance metrics
    if hasattr(system, 'performance_tracker'):
        performance_summary = system.performance_tracker.get_performance_summary()
        print(f"Total Orchestrations: {performance_summary.get('total_orchestrations', 0)}")
        print(f"Success Rate: {performance_summary.get('success_rate', 0):.2%}")
        print(f"Average Execution Time: {performance_summary.get('average_execution_time', 0):.3f}s")
