...
async def test_basic_agent_creation():
    ...
    try:
        ...
        print(
            f"✅ Cultural compliance required: {agent.config.cultural_compliance_required}"
        )
        print(
            f"✅ Islamic principles enabled: {agent.config.islamic_principles_enabled}"
        )
...
async def test_cultural_validation():
    ...
    try:
        ...
        test_content = {"text": "هذا اختبار للنظام الذكي العراقي", "context": "testing"}
...
async def test_agent_processing():
    ...
    try:
        ...
        test_input = {"content": "Test processing input", "type": "basic_test"}
        ...
        if "cultural_validation" in result:
            print(
                f"✅ Cultural validation included: {result['cultural_validation']['is_compliant']}"
            )
    ...
    try:
        ...
        if "cultural_validation_tool" in status["available_tools"]:
            ...
            result = await tool_system.execute_tool(
                "cultural_validation_tool", test_input
            )
...
async def test_orchestration_system():
    ...
    try:
        ...
        config = OrchestrationConfig(
            cultural_validation_required=True, islamic_principles_enforcement=True
        )
        ...
        result = await orchestration_system.orchestrate(
            test_task, agent_selection=["orchestration_test_agent"]
        )
        ...
        if "cultural_validation" in result:
            print(
                f"✅ Cultural validation: {result['cultural_validation']['is_compliant']}"
            )
