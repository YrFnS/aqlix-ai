...
class IraqiMultiAgentSystem(CulturalMixin):
        ...
        try:
            ...
            if strategy == OrchestrationStrategy.SEQUENTIAL:
                result = await self._execute_sequential(
                    task, selected_agents, orchestration_id
                )
                ...
                result = await self._execute_cultural_priority(
                    task, selected_agents, orchestration_id
                )
            else:  # HYBRID
            ...
            if self.config.cultural_validation_required:
                ...
                if (
                    not cultural_validation["is_compliant"]
                    and self.config.fail_fast_on_cultural_violations
                ):
    ...
    async def _select_agents(
        self, task: Dict[str, Any], explicit_selection: Optional[List[str]] = None
    ) -> List[IraqiAgent]:
        """Select appropriate agents for the task"""

        if explicit_selection:
            return [
                self.agents[name] for name in explicit_selection if name in self.agents
            ]
        ...
        cultural_agents = [
            ...
            if "cultural" in agent.config.name.lower()
            or "validator" in agent.config.name.lower()
        ]
        ...
        if self._contains_arabic_content(task):
            arabic_agents = [
                agent
                for agent in self.agents.values()
                if "arabic" in agent.config.name.lower()
                or "rtl" in agent.config.name.lower()
            ]
        ...
        for domain in ["legal", "medical", "educational"]:
            if domain in task_text:
                domain_agents = [
                    agent
                    for agent in self.agents.values()
                    if domain in agent.config.name.lower()
                    or domain in str(agent.config.professional_domains)
                ]
        ...
        if any(term in task_text for term in ["payment", "zain", "fastpay", "nasswal"]):
            payment_agents = [
                agent
                for agent in self.agents.values()
                if "payment" in agent.config.name.lower()
                or "financial" in agent.config.name.lower()
            ]
    ...
    async def _execute_sequential(
        self, task: Dict[str, Any], agents: List[IraqiAgent], orchestration_id: str
    ) -> Dict[str, Any]:
    ...
    async def _execute_cultural_priority(
        ...
        cultural_agents = [
            ...
            for agent in agents
            if "cultural" in agent.config.name.lower()
            or getattr(agent.config, "cultural_compliance_required", False)
        ]
        ...
        return {
            "status": "success",
            "orchestration_id": orchestration_id,
            "response": self._synthesize_cultural_priority_results(
                cultural_results, other_results
            ),
            "cultural_results": cultural_results,
    ...
    def _synthesize_parallel_results(self, results: List[Dict[str, Any]]) -> str:
        ...
        responses = [
            r.get("response", "") for r in successful_results if r.get("response")
        ]

        return (
            f"Parallel processing completed with {len(successful_results)}/{len(results)} successful agents\n\n"
            + "\n---\n".join(responses)
        )
    ...
    def _synthesize_loop_results(self, results: List[Dict[str, Any]]) -> str:
        ...
        final_responses = [
            r.get("response", "") for r in final_iteration if r.get("response")
        ]
    ...
    def _synthesize_cultural_priority_results(
        ...
        cultural_summary = (
            f"Cultural validation completed with {len(cultural_results)} agents"
        )

        if other_results:
            other_summary = (
                f"Additional processing completed with {len(other_results)} agents"
            )
            other_responses = [
                r.get("response", "") for r in other_results if r.get("response")
            ]
    ...
    def _check_loop_convergence(
        self, iteration_results: List[Dict[str, Any]], threshold: float
    ) -> bool:
        """Check if loop has converged"""
        # Simple convergence check - can be enhanced with more sophisticated logic
        successful_results = [
            r for r in iteration_results if r.get("status") == "success"
        ]
        return len(successful_results) / len(iteration_results) >= threshold

    def _extract_loop_state(
        self, iteration_results: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Extract state for next loop iteration"""
        return {
            "previous_iteration_success_rate": len(
                [r for r in iteration_results if r.get("status") == "success"]
            )
            / len(iteration_results),
            "agent_outputs": [
                r.get("response") for r in iteration_results if r.get("response")
            ],
        }

    def _extract_cultural_context(
        self, cultural_results: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for result in cultural_results:
            if cultural_validation := result.get("cultural_validation"):
                context["cultural_compliance_verified"] &= cultural_validation.get(
                    "is_compliant", True
                )
                context["islamic_principles_validated"] &= cultural_validation.get(
                    "islamic_compliance", True
                )
                context["cultural_recommendations"].extend(
                    cultural_validation.get("recommendations", [])
                )
...
class IraqiSequentialAgent(IraqiAgent):
    ...
    def __init__(self, name: str, sub_agents: List[IraqiAgent], **kwargs):
        ...
        self.orchestration_system = IraqiMultiAgentSystem(
            OrchestrationConfig(strategy=OrchestrationStrategy.SEQUENTIAL)
        )
...
class IraqiLoopAgent(IraqiAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.orchestration_system = IraqiMultiAgentSystem(
            OrchestrationConfig(strategy=OrchestrationStrategy.LOOP)
        )
...
class PerformanceTracker:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        successful_orchestrations = len(
            [m for m in self.orchestration_metrics if m["status"] == "success"]
        )
        avg_execution_time = (
            sum(m["execution_time"] for m in self.orchestration_metrics)
