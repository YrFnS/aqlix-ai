...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (access.archivedAt) {
      return jsonFailure(
        "CONFLICT",
        "Archived workspaces are read-only. Restore the workspace before discarding a proposal.",
    ...
    if (access.role === "viewer") {
      return jsonFailure(
        "FORBIDDEN",
        "Viewer membership does not permit discarding draft proposals.",
    ...
    if (draft.status !== "active") {
      return jsonFailure(
        "CONFLICT",
        "Archived drafts are read-only. Restore the draft before discarding a proposal.",
        { status: 409, requestId: auth.context.requestId },
      );
    }
    if (generation.status !== "complete") {
      return jsonFailure(
        "CONFLICT",
        "Only a completed proposal can be discarded.",
        { status: 409, requestId: auth.context.requestId },
      );
    }

    const discarded = await discardDraftGeneration(
      auth.context.supabase,
      parsed.data.workspaceId,
      parsed.data.draftId,
      parsed.data.generationId,
    ...
    if (!discarded) {
      return jsonFailure(
        "CONFLICT",
        "The proposal is no longer available to discard.",
        { status: 409, requestId: auth.context.requestId },
      );
    }
    ...
    if (!detail) {
      return jsonFailure(
        "PERSISTENCE_ERROR",
        "The proposal was discarded but the draft could not be reloaded.",
    ...
    if (error instanceof DraftRepositoryError) {
      console.error("Draft proposal discard persistence failure", {
        requestId: auth.context.requestId,
      ...
      console.error("Unexpected draft proposal discard failure", {
        requestId: auth.context.requestId,
        error,
      });
    }
    ...
    return jsonFailure(
      "PERSISTENCE_ERROR",
      "The draft proposal could not be discarded.",
      { status: 503, requestId: auth.context.requestId },
    );
  }
}
