...
test.describe("Navigation", () => {
  test("should navigate between marketing pages and show active states", async ({
    page,
  }) => {
    // Start at home
    await page.goto("/");
  ...
  test("should handle dynamic routes for blog correctly", async ({
    ...
    await expect(page.locator("h1")).toContainText(
      "Introducing Iraqi AI Chat System",
    );
  ...
  test("should navigate between app pages and show active states", async ({
    page,
  }) => {
    // Go to dashboard
  ...
  test("should have proper metadata for pages", async ({ page }) => {
    ...
    await expect(page).toHaveTitle(
      /Introducing Iraqi AI Chat System.*Iraqi AI Blog/,
    );
  });
