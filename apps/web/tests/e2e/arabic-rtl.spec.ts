...
test.describe("Arabic RTL Support", () => {
  ...
  test("should display Arabic text correctly with RTL direction", async ({
    page,
  }) => {
    ...
    // Check RTL direction
    const direction = await testElement.evaluate(
  ...
  test("should render Arabic numbers correctly", async ({ page: _page }) => {
    ...
    await expect(page.locator("#english-numbers")).toContainText(
      englishNumbers,
    );
  });
