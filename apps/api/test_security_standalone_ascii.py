...
try:
    ...
    print(f"[OK] Correct code verification: PASS")

    assert not verify_mfa_code("654321", hashed), "Wrong code should not verify"
    print(f"[OK] Wrong code rejection: PASS")

    # Test timing safety (different length codes)
    assert not verify_mfa_code("12345", hashed), "Short code should not verify"
    print(f"[OK] Timing-safe comparison: PASS")

    print(f"[OK] MFA BCRYPT IMPLEMENTATION: ALL TESTS PASSED")
    print()

except Exception as e:
    print(f"[FAIL] MFA BCRYPT IMPLEMENTATION: FAILED")
    print(f"   Error: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"[OK] Valid token decoding: PASS")
    ...
    try:
        ...
        print(f"[FAIL] Expired token should be rejected")
        sys.exit(1)
    except jwt.ExpiredSignatureError:
        print(f"[OK] Expired token rejection: PASS")
    ...
    try:
        ...
        print(f"[FAIL] Token with missing claims should be rejected")
        sys.exit(1)
    except jwt.MissingRequiredClaimError:
        print(f"[OK] Missing required claims rejection: PASS")
    ...
    try:
        ...
        print(f"[FAIL] Token with wrong signature should be rejected")
        sys.exit(1)
    except jwt.InvalidSignatureError:
        print(f"[OK] Invalid signature rejection: PASS")

    print(f"[OK] JWT CLAIM VALIDATION: ALL TESTS PASSED")
    print()

except Exception as e:
    print(f"[FAIL] JWT CLAIM VALIDATION: FAILED")
    print(f"   Error: {e}")
    sys.exit(1)
...
print("=" * 80)
print()
print("[OK] CRITICAL FIX #1: MFA Bcrypt Implementation - VERIFIED")
...
print()
print("[OK] CRITICAL FIX #2: JWT Claim Validation - VERIFIED")
...
print()
print("[SUCCESS] ALL SECURITY FIXES VERIFIED SUCCESSFULLY")
print("=" * 80)
