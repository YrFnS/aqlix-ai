...
export default function ImageGeneration({
  ...
  const processArabicPrompt = useCallback(
    async (arabicPrompt: string): Promise<string> => {
      const task = new Task({
  ...
  const validateCulturally = useCallback(
    async (imageUrl: string, prompt: string): Promise<any> => {
      const task = new Task({
  ...
  const generateImage = useCallback(async () => {
    ...
    try {
      ...
      // Cultural validation
      const culturalValidation = await validateCulturally(
        imageUrl,
        enhancedPrompt,
      );
  ...
  const generateWithMidjourney = async (prompt: string): Promise<string> => {
    const response = await fetch("/api/generate-image/midjourney", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        prompt: islamicArtMode
          ? `${prompt} --style islamic art --ar 1:1`
          : prompt,
        aspect_ratio: "1:1",
        version: "6.0",
      }),
  ...
  const generateWithStableDiffusion = async (
    ...
    const response = await fetch("/api/generate-image/stable-diffusion", {
      ...
      body: JSON.stringify({
        prompt: islamicArtMode
          ? `${prompt}, islamic art style, geometric patterns`
          : prompt,
    ...
    if (!response.ok) {
      throw new Error(
        `Stable Diffusion generation failed: ${response.statusText}`,
      );
    }
    ...
    <div className="w-full max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        >
          ...
          {generating
            ? "جاري الإنتاج... / Generating..."
            : "أنتج الصورة / Generate Image"}
        </button>
      </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-center space-x-3 rtl:space-x-reverse mb-3">
            ...
            <span className="text-blue-800 font-medium">
              جاري إنتاج الصورة... / Generating Image...
            </span>
          </div>
